...
HFSAT::ORS::ThrottleManagerSIM *throttle_manager_ = nullptr;
HFSAT::ORS::ThrottleManagerSIM *throttle_manager_reserved_ = nullptr;
...
int main(int argc, char** argv) {
  ...
  if (argc < 6) {  // 5 is min of live and sim
    std::cerr
        << "expecting :\n"
        << " $tradeengineexec STRAT_LIVE_FILE TRADINGDATE START_TIME END_TIME PROGID [ADD_DBG_CODE CODE1 CODE2 ......] --exchange=[NSE]/BSE/BSE_NSE --use_l1=[false]/true --throttle_limit=[-1]/... --throttle_reserved=[-1]/..."
        << '\n';
  ...
  ("use_l1", boost::program_options::value<std::string>()->default_value("false"), "use_l1 true sets to l1 book manager")
  ("throttle_limit", boost::program_options::value<int>()->default_value(-1))
  ("throttle_reserved", boost::program_options::value<int>()->default_value(-1));
  ...
  std::string use_l1_str = vm["use_l1"].as<std::string>();
  bool use_l1_book = false;
  use_l1_book = (use_l1_str == "true") ? true : false ;

  int throttle_val = vm["throttle_limit"].as<int>();
  int throttle_reserved_val = vm["throttle_reserved"].as<int>();

  std::cout << "PREP TIME : "<< prep_time_int << " USE_L1 " << use_l1_str << std::endl ;
  ...
  common_intializer->Initialize(use_l1_book);
  ...
  if(throttle_val >= 0){
    throttle_manager_ = new HFSAT::ORS::ThrottleManagerSIM(dbglogger_,throttle_val);
    std::cout<< "Will use throttle: "<<throttle_val << std::endl;
  }else{
    std::cout << "Throttle value is not set, will run without throttle" << std::endl;
  }
  if(throttle_reserved_val >= 0){
    throttle_manager_reserved_ = new HFSAT::ORS::ThrottleManagerSIM(dbglogger_,throttle_reserved_val);
    std::cout<< "Will use throttle reserved: "<< throttle_reserved_val << std::endl;
  }else{
    std::cout << "Throttle reserved val is not set, will run without throttle reserved" << std::endl;
  }
  for (auto theo_calc_ : theo_vec_) {
    ...
    smm_->SetThrottleManagerLimitOrders(throttle_manager_);
    smm_->SetThrottleManagerIOCOrders(throttle_manager_reserved_);
