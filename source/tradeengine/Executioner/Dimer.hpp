...
class Dimer : public BaseExecutioner, public HFSAT::ThrottleNumberListener {
 private:
  ...
  int num_modify_;
  int num_priority_modify_;
  int priority_limit_start_mfm_;
  int priority_relative_percent_lower_limit_;
  int priority_relative_percent_upper_limit_;
  int priority_self_percent_lower_limit_;
  int priority_self_percent_upper_limit_;
  int last_priority_check_time_;
  int priority_check_time_cooloff_;
  bool disallow_priority_modify_;
  double bad_fill_check_multiplier_;
  int num_modify_threshold_;
  double disclosed_qty_factor_;
          ...
          if ((safe_dimer_price_int_ <= dimer_bid_int_price_) && (theo_values_.is_valid_)) {
            if (bid_order_->int_price_ != current_order_bid_int_price_) {
              ...
              bool is_modified_ = basic_om_->Modify(bid_order_, current_order_bid_price_, current_order_bid_int_price_,
                                                    dimer_bid_size_, use_reserve_msg_, disclosed_qty_factor_);
              if (is_modified_) {
          ...
          if ((safe_dimer_price_int_ >= dimer_ask_int_price_) && (theo_values_.is_valid_)) {
            if (ask_order_->int_price_ != current_order_ask_int_price_) {
              ...
              bool is_modified_ = basic_om_->Modify(ask_order_, current_order_ask_price_, current_order_ask_int_price_,
                                                    dimer_ask_size_, use_reserve_msg_, disclosed_qty_factor_);
              if (is_modified_) {
                /*								dbglogger_ << watch_.tv() << " "
                                                                                        << sec_shortcode_ << " " <<
                   identifier_
