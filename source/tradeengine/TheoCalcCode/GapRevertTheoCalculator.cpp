...
GapRevertTheoCalculator::GapRevertTheoCalculator(std::map<std::string, std::string>* key_val_map, HFSAT::Watch& _watch_,
  ...
  dbglogger_ << watch_.tv() << " Creating GAPREVERT THEO CALCULATOR secId " << secondary_id_ << DBGLOG_ENDL_FLUSH;
  LoadParams();
  InitializeDataSubscriptions();
}
/** Loads basic parameters and historic volatility, volume and signal values
*/
void GapRevertTheoCalculator::LoadParams() {
  MidTermTheoCalculator::LoadParams();
  num_bars_ = Parser::GetInt(key_val_map_, "NUM_BARS", 30);
  trade_bar_ = Parser::GetInt(key_val_map_, "TRADE_BAR", 6);
  open_move_threshold_ = Parser::GetDouble(key_val_map_, "OPEN_MOVE_THRESHOLD", 0.25);
  if (watch_.msecs_from_midnight() < trading_start_utc_mfm_) {
    subclass_ = 6;
    BarGenerator bg_;
    bg_.getGapRevertValues(dbglogger_, watch_, secondary_smv_->shortcode(), ticker_name_, watch_.YYYYMMDD(), num_bars_,
                      granularity_,adj_ratio_, lt_days_,long_term_vol_mean_,prev_day_close_, open_obv_max_);
    ...
    dbglogger_ << ticker_name_ << " open_obv_max: " << open_obv_max_ << " long_term_vol_mean: " << long_term_vol_mean_ << " prev_close: " << prev_day_close_ << DBGLOG_ENDL_FLUSH;
  }
}
...
void GapRevertTheoCalculator::onBarUpdate(const unsigned int _security_id_,
  ...
  if(bar_count_ == 0){
    ...
    min_close_ = bar_close_price_;
    max_close_ = bar_close_price_;
  }  
  else{
    if(min_close_ > bar_close_price_){
      min_close_ = bar_close_price_;
    }
    if(max_close_ < bar_close_price_){
      max_close_ = bar_close_price_;
    }
  }
  ...
  if (current_cascade_ != 0 && (midterm_pnl_ < midterm_stop_loss_ || bar_volume_ > day_open_volume_|| (current_directional_cascade_ > 0 && (support_price_ >= bar_close_price_ || (bar_close_price_ < open_bar_close_px_))) ||
      (current_directional_cascade_ < 0 && (resist_price_ <= bar_close_price_ || ( bar_close_price_ > open_bar_close_px_))))) {
    ...
    if (status_mask_ & LOSS_STATUS_SET) {
      dbglogger_ << watch_.tv() << " Hitting STOP LOSS on BarUpdate " << secondary_smv_->shortcode()
                 << " PNL: " << midterm_pnl_ << " pos: " << -1*midterm_pos_offset_ << " GAPREVERT SL: " << midterm_stop_loss_
    ...
    if (bar_open_price_ != 0) {
      ...
      if (bar_count_ == 0){
        if(open_bar_close_px_ > bar_open_price_ && bar_low_price_ >= prev_day_close_ &&
          (open_bar_close_px_ - bar_open_price_)*bar_volume_ >= open_move_threshold_* open_obv_max_){

          cascading_price_ = bar_close_price_;
          current_cascade_++;
          current_directional_cascade_--;
          dbglogger_ << "GAPREVERT " << secondary_smv_->shortcode() << " CASCADE SELL "
                     << " " << bar_close_price_ << " " << prev_day_close_ << " " << day_open_price_ << DBGLOG_ENDL_FLUSH;
          last_exec_obv_ = current_obv_;
          if (status_mask_ == BIT_SET_ALL) {
            ComputeAndUpdateTheoListeners();
          }
        }
        else if(open_bar_close_px_ < bar_open_price_ && bar_high_price_ <= prev_day_close_ &&
          -1*(open_bar_close_px_ - bar_open_price_)*bar_volume_ >= open_move_threshold_* open_obv_max_){

          cascading_price_ = bar_close_price_;
          current_cascade_++;
          current_directional_cascade_++;
          dbglogger_ << "GAPREVERT " << secondary_smv_->shortcode() << " CASCADE BUY "
                     << " " << bar_close_price_ << " " << prev_day_close_ << " " << day_open_price_ << DBGLOG_ENDL_FLUSH;
          last_exec_obv_ = current_obv_;
          if (status_mask_ == BIT_SET_ALL) {
            ComputeAndUpdateTheoListeners();
      ...
      //11 action
        ...
        if(current_directional_cascade_ > 0 && min_close_ == open_bar_close_px_ 
          && current_obv_ > last_exec_obv_  && bar_close_price_ > open_bar_close_px_){
          ...
          && current_obv_ < last_exec_obv_ && max_close_ == open_bar_close_px_){
...
void GapRevertTheoCalculator::updateTrailingSL(double _last_price_) {
...
void GapRevertTheoCalculator::OnMarketUpdate(const unsigned int _security_id_,
                                           const HFSAT::MarketUpdateInfo &_market_update_info_) {
  MidTermTheoCalculator::OnMarketUpdate(_security_id_, _market_update_info_);
  int current_time_ = watch_.tv().tv_sec;
  if ((current_time_ >= last_aggregation_time_ + granularity_/3 && mid_bar_counter_ == 0 && current_time_ < last_aggregation_time_ + 2*granularity_/3) 
    || (current_time_ >= last_aggregation_time_ + 2*granularity_/3 && mid_bar_counter_ == 1 && current_time_ < last_aggregation_time_ + granularity_)){
    if (bar_close_price_ && ((current_directional_cascade_ > 0 && bar_close_price_ < open_bar_close_px_) ||
        (current_directional_cascade_ < 0 && bar_close_price_ > open_bar_close_px_))) {
      // If loss check is set, unset it
      if (status_mask_ & LOSS_STATUS_SET) {
        dbglogger_ << watch_.tv() << " Hitting STOP LOSS on MidBarUpdate " << secondary_smv_->shortcode()
                   << " PNL: " << midterm_pnl_ << " pos: " << -1*midterm_pos_offset_ << " GAPREVERT SL: " << midterm_stop_loss_
                   << " SL: " << stop_loss_ << " HardSL: " << hard_stop_loss_ << " Support px: " << support_price_
                   << " Resist px: " << resist_price_ << " secmkt[" << _market_update_info_.bestbid_price_ << " x "
                   << _market_update_info_.bestask_price_ << "] Stop loss decay "
...
void GapRevertTheoCalculator::ReloadConfig()  {
