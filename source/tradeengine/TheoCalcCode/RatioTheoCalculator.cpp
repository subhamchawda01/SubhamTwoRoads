...
RatioTheoCalculator::RatioTheoCalculator(std::map<std::string, std::string>* key_val_map, HFSAT::Watch& _watch_,
    ...
    : BaseTheoCalculator(key_val_map, _watch_, _dbglogger_, _trading_start_utc_mfm_, _trading_end_utc_mfm_,
      ...
      hist_std_(0),
      hist_factor_(0),
      after_first_trade_(false),
      spread_check_(false),
      primary_upper_int_price_threshold_autogetflat_(0.0),
      primary_lower_int_price_threshold_autogetflat_(0.0),
      primary_upper_int_price_threshold_autoresume_(0.0),
      primary_lower_int_price_threshold_autoresume_(0.0),
      secondary_upper_int_price_threshold_autogetflat_(0.0),
      secondary_lower_int_price_threshold_autogetflat_(0.0),
      secondary_upper_int_price_threshold_autoresume_(0.0),
      secondary_lower_int_price_threshold_autoresume_(0.0),
      day_open_px_(0) {
...
void RatioTheoCalculator::LoadCircuitVars(){
  // Primary and secondary auto getflat and auto resume near circuit variables initialization
  if( true == are_we_using_auto_getflat_near_circuit_ ) {
    dbglogger_ << watch_.tv() << " Updating circuit variable for " << secondary_smv_->shortcode() << DBGLOG_ENDL_FLUSH;

    //We would have called squareoff theo and that has to be reset
    if(true == is_squaredoff_due_to_autogetflat_near_primary_circuit_ || true == is_squaredoff_due_to_autogetflat_near_secondary_circuit_){
      NoSquareOff();
      DBGLOG_TIME_CLASS_FUNC << "RESETTING SQUAREOFF THEO DUE TO LOAD PARAMS FOR : " << secondary_smv_->shortcode() << DBGLOG_ENDL_FLUSH;
    ...
    // is_theoturnoff_due_to_autogetflat_near_primary_circuit_ = true;
    // autogetflat variables
    if( true == is_primary_last_close_valid_ ){
      primary_upper_int_price_threshold_autogetflat_ = (primary0_smv_->upper_int_price_limit_ - primary_int_px_value_for_autogetflat_);
      primary_lower_int_price_threshold_autogetflat_ = (primary0_smv_->lower_int_price_limit_ + primary_int_px_value_for_autogetflat_);
    }else{
      primary_upper_int_price_threshold_autogetflat_ = 0.99*primary0_smv_->upper_int_price_limit_;
      primary_lower_int_price_threshold_autogetflat_ = 1.01*primary0_smv_->lower_int_price_limit_;
    }
    if (ipo_settlement_price_>0){
      primary_upper_int_price_threshold_autogetflat_ = (ipo_settlement_price_*(1+ipo_circuit_)/tick_size_ - primary_int_px_value_for_autogetflat_);
      primary_lower_int_price_threshold_autogetflat_ = (ipo_settlement_price_*(1-ipo_circuit_)/tick_size_ + primary_int_px_value_for_autogetflat_);
    }

    // autoresume variables
    if( true == is_primary_last_close_valid_ ){
      primary_upper_int_price_threshold_autoresume_ = (primary0_smv_->upper_int_price_limit_ - primary_int_px_value_for_autoresume_);
      primary_lower_int_price_threshold_autoresume_ = (primary0_smv_->lower_int_price_limit_ + primary_int_px_value_for_autoresume_);
    }else{
      primary_upper_int_price_threshold_autoresume_ = 0.98*primary0_smv_->upper_int_price_limit_;
      primary_lower_int_price_threshold_autoresume_ = 1.02*primary0_smv_->lower_int_price_limit_;
    }
    if (ipo_settlement_price_>0){
      primary_upper_int_price_threshold_autoresume_ = (ipo_settlement_price_*(1+ipo_circuit_)/tick_size_ - primary_int_px_value_for_autoresume_);
      primary_lower_int_price_threshold_autoresume_ = (ipo_settlement_price_*(1-ipo_circuit_)/tick_size_ + primary_int_px_value_for_autoresume_);
    }

    // autogetflat variables
    if( true == is_secondary_last_close_valid_ ){
      secondary_upper_int_price_threshold_autogetflat_ = (secondary_smv_->upper_int_price_limit_ - secondary_int_px_value_for_autogetflat_);
      secondary_lower_int_price_threshold_autogetflat_ = (secondary_smv_->lower_int_price_limit_ + secondary_int_px_value_for_autogetflat_);
    }else{
      secondary_upper_int_price_threshold_autogetflat_ = 0.99*secondary_smv_->upper_int_price_limit_;
      secondary_lower_int_price_threshold_autogetflat_ = 1.01*secondary_smv_->lower_int_price_limit_;
    }
    if (ipo_settlement_price_>0){
      secondary_upper_int_price_threshold_autogetflat_ = (ipo_settlement_price_*(1+ipo_circuit_)/tick_size_ - primary_int_px_value_for_autogetflat_);
      secondary_lower_int_price_threshold_autogetflat_ = (ipo_settlement_price_*(1-ipo_circuit_)/tick_size_ + primary_int_px_value_for_autogetflat_);
    }
    
    // autoresume variables
    if( true == is_secondary_last_close_valid_ ){
      secondary_upper_int_price_threshold_autoresume_ = (secondary_smv_->upper_int_price_limit_ - secondary_int_px_value_for_autoresume_);
      secondary_lower_int_price_threshold_autoresume_ = (secondary_smv_->lower_int_price_limit_ + secondary_int_px_value_for_autoresume_);
    }else{
      secondary_upper_int_price_threshold_autoresume_ = 0.98*secondary_smv_->upper_int_price_limit_;
      secondary_lower_int_price_threshold_autoresume_ = 1.02*secondary_smv_->lower_int_price_limit_;
    }
    if (ipo_settlement_price_>0){
      secondary_upper_int_price_threshold_autoresume_ = (ipo_settlement_price_*(1+ipo_circuit_)/tick_size_ - secondary_int_px_value_for_autoresume_);
      secondary_lower_int_price_threshold_autoresume_ = (ipo_settlement_price_*(1-ipo_circuit_)/tick_size_ + secondary_int_px_value_for_autoresume_);
    }

    if (ipo_settlement_price_>0){
      dbglogger_ << watch_.tv() << " IPO secondary symbol autogetflat limits: [" << secondary_lower_int_price_threshold_autogetflat_ << \
      "," << secondary_upper_int_price_threshold_autogetflat_ << "] and autoresume limits: [" << \
      secondary_lower_int_price_threshold_autoresume_ << "," << secondary_upper_int_price_threshold_autoresume_ << "]" << DBGLOG_ENDL_FLUSH;
    }
  }
}
...
void RatioTheoCalculator::LoadParams() {
  ...
  using_grt_control_file_ = Parser::GetBool(key_val_map_, "USE_GRT_CONTROL_FILE", false);
  if(true == using_grt_control_file_){
    grt_control_filename_ = Parser::GetString(key_val_map_, "GRT_CONTROL_FILE", "/home/irage/default_strat_control_paramfile");
    dbglogger_ << "Using GRT Control file " << "\n";
    dbglogger_ << "File path " << grt_control_filename_ << "\n";
    Parser::ParseConfig(grt_control_filename_, *grt_control_key_map_);
  }

  bool status_ = Parser::GetBool(key_val_map_, "STATUS", false);

  if(true == using_grt_control_file_){
    status_ = Parser::GetBool(grt_control_key_map_, "STATUS", false);
    dbglogger_ << "UPDATING GRT STATUS " << status_ << "\n";
  }
  ...
  double secondary_last_close_price_=HFSAT::SecurityDefinitions::GetClosePriceFromShortCode(secondary_smv_->shortcode());
  bool spread_check_=Parser::GetBool(key_val_map_, "SPREAD_CHECK", false);
  double spread_percent_=Parser::GetDouble(key_val_map_, "SPREAD_PERCENT", 0.007);
  ...
  if (spread_check_ && secondary_last_close_price_>0)
    max_primary_spread_ = std::min(max_primary_spread_, (spread_percent_*secondary_last_close_price_));
  ...
  if (spread_check_ && secondary_last_close_price_>0)
    max_secondary_spread_ = std::min(max_secondary_spread_, (spread_percent_*secondary_last_close_price_));

  stop_loss_ = Parser::GetDouble(key_val_map_, "STOP_LOSS", 0);

  if(true == using_grt_control_file_){
    stop_loss_ = Parser::GetDouble(grt_control_key_map_, "STOP_LOSS", 0);
    dbglogger_ << "UPDATING GRT STOP LOSS " <<  stop_loss_ << "\n";
  }
  ...
  after_first_trade_ = Parser::GetBool(key_val_map_, "AFTER_FIRST_TRADE", false);
...
void RatioTheoCalculator::UpdateTheoPrices(const unsigned int _security_id_,
  ...
  if( true == are_we_using_auto_getflat_near_circuit_ ) {
    if((_security_id_ == primary0_id_) && (true == is_primary_book_valid)) { 
      ...
      double primary_bid_int_px = primary0_smv_->market_update_info_.bestbid_int_price_;
      double primary_ask_int_px = primary0_smv_->market_update_info_.bestask_int_price_;
      //auto getflat bits are not unset

      if(false == is_theoturnoff_due_to_autogetflat_near_primary_circuit_){
        //violation
        if(primary_ask_int_px >= primary_upper_int_price_threshold_autogetflat_ || primary_bid_int_px <= primary_lower_int_price_threshold_autogetflat_){
          TurnOffTheo(NEAR_CIRCUIT_PRIMARY_STATUS_UNSET);
          is_theoturnoff_due_to_autogetflat_near_primary_circuit_ = true ;
          DBGLOG_TIME_CLASS_FUNC << "AUTO THEO TURNOFF CALLED : " << secondary_smv_->shortcode() << " ON PRIMARY : " << primary0_smv_->shortcode() << " NEARING A CIRCUIT LEVEL : " << primary0_smv_->market_update_info_.bestbid_int_price_ << " X " << primary0_smv_->market_update_info_.bestask_int_price_ << " THRESHOLD : " << primary_lower_int_price_threshold_autogetflat_ << " X "  << primary_upper_int_price_threshold_autogetflat_ << " AGG ? " << are_we_using_agg_auto_getflat_near_circuit_ << DBGLOG_ENDL_FLUSH;
        }
      }else{
        //resume
        if(primary_ask_int_px < primary_upper_int_price_threshold_autoresume_ && primary_bid_int_px > primary_lower_int_price_threshold_autoresume_){
          TurnOnTheo(NEAR_CIRCUIT_PRIMARY_STATUS_SET);
          is_theoturnoff_due_to_autogetflat_near_primary_circuit_ = false;
          DBGLOG_TIME_CLASS_FUNC << "AUTO THEO TURNON CALLED : " << secondary_smv_->shortcode() << " ON PRIMARY : " << primary0_smv_->shortcode() << " MOVING AWAY FROM CIRCUIT LEVEL : " << primary0_smv_->market_update_info_.bestbid_int_price_ << " X " << primary0_smv_->market_update_info_.bestask_int_price_ << " THRESHOLD : " << primary_lower_int_price_threshold_autoresume_ << " X "  << primary_upper_int_price_threshold_autoresume_ << DBGLOG_ENDL_FLUSH ;
        }
      }
      ...
      if(false == is_squaredoff_due_to_autogetflat_near_primary_circuit_){
        ...
        if(primary_ask_int_px >= primary_upper_int_price_threshold_autogetflat_ || primary_bid_int_px <= primary_lower_int_price_threshold_autogetflat_){
          ...
          DBGLOG_TIME_CLASS_FUNC << "AUTO SQUAREOFF CALLED : " << secondary_smv_->shortcode() << " ON PRIMARY : " << primary0_smv_->shortcode() << " NEARING A CIRCUIT LEVEL : " << primary0_smv_->market_update_info_.bestbid_int_price_ << " X " << primary0_smv_->market_update_info_.bestask_int_price_ << " THRESHOLD : " << primary_lower_int_price_threshold_autogetflat_ << " X "  << primary_upper_int_price_threshold_autogetflat_ << " AGG ? " << are_we_using_agg_auto_getflat_near_circuit_ << DBGLOG_ENDL_FLUSH;
        }
      }else{ 
        //resume
        if(primary_ask_int_px < primary_upper_int_price_threshold_autoresume_ && primary_bid_int_px > primary_lower_int_price_threshold_autoresume_){
          NoSquareOff();
          is_squaredoff_due_to_autogetflat_near_primary_circuit_ = false;
          DBGLOG_TIME_CLASS_FUNC << "AUTO RESUME CALLED : " << secondary_smv_->shortcode() << " ON PRIMARY : " << primary0_smv_->shortcode() << " MOVING AWAY FROM CIRCUIT LEVEL : " << primary0_smv_->market_update_info_.bestbid_int_price_ << " X " << primary0_smv_->market_update_info_.bestask_int_price_ << " THRESHOLD : " << primary_lower_int_price_threshold_autoresume_ << " X "  << primary_upper_int_price_threshold_autoresume_ << DBGLOG_ENDL_FLUSH ;
        }
      }
    }
    ...
    if((_security_id_ == secondary_id_) && (true == is_secondary_book_valid)){
      ...
      double secondary_bid_int_px = secondary_smv_->market_update_info_.bestbid_int_price_ ;
      double secondary_ask_int_px = secondary_smv_->market_update_info_.bestask_int_price_ ;
      //auto getflat bits are not unset

      if(false == is_theoturnoff_due_to_autogetflat_near_secondary_circuit_){
        //violation
        if(secondary_ask_int_px >= secondary_upper_int_price_threshold_autogetflat_ || secondary_bid_int_px <= secondary_lower_int_price_threshold_autogetflat_){
          TurnOffTheo(CTRLMSG_STATUS_UNSET);
          is_theoturnoff_due_to_autogetflat_near_secondary_circuit_ = true;
          DBGLOG_TIME_CLASS_FUNC << "AUTO THEO TURNOFF CALLED : " << secondary_smv_->shortcode() << " ON SELF NEARING A CIRCUIT LEVEL : " << secondary_smv_->market_update_info_.bestbid_int_price_ << " X " << secondary_smv_->market_update_info_.bestask_int_price_ << " THRESHOLD : " << secondary_lower_int_price_threshold_autogetflat_ << " X "  << secondary_upper_int_price_threshold_autogetflat_ << " AGG ? " << are_we_using_agg_auto_getflat_near_circuit_ << DBGLOG_ENDL_FLUSH;
        }
      }else{
        //resume
        if(secondary_ask_int_px < secondary_upper_int_price_threshold_autoresume_ && secondary_bid_int_px > secondary_lower_int_price_threshold_autoresume_){
          TurnOnTheo(CTRLMSG_STATUS_SET);
          is_theoturnoff_due_to_autogetflat_near_secondary_circuit_ = false;
          DBGLOG_TIME_CLASS_FUNC << "AUTO THEO TURNON CALLED : " << secondary_smv_->shortcode() << " ON SELF : " << secondary_smv_->shortcode() << " MOVING AWAY FROM CIRCUIT LEVEL : " << secondary_smv_->market_update_info_.bestbid_int_price_ << " X " << secondary_smv_->market_update_info_.bestask_int_price_ << " THRESHOLD : " << secondary_lower_int_price_threshold_autoresume_ << " X "  << secondary_upper_int_price_threshold_autoresume_ << DBGLOG_ENDL_FLUSH ;
        }
      }
  ...
  if (_security_id_ == primary0_id_) {
    ...
    if ( false == is_primary_book_valid ){
      ...
      if ((!is_ready_ || config_reloaded_) && (msecs_from_midnight > trading_start_utc_mfm_) && (start_trading_) &&
          ...
          (theo_values_.last_traded_int_price_!=0 || !after_first_trade_)) {
