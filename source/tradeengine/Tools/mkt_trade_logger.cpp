...
class TestBook : public HFSAT::Thread, public HFSAT::SecurityMarketViewChangeListener, public HFSAT::FastTradeListener {
 ...
 public:
...

  {
    ...
    cpucycle_profiler_.SetTag(0, "GetDeepPL");
  }
  ...
  void thread_main() {
    ...
    std::cout << HFSAT::CpucycleProfiler::GetUniqueInstance().GetCpucycleSummaryString()  << std::endl;
  }
...
int main(int argc, char **argv) {
  ...
  bool isBSE = (argc >= 4 && strcmp(argv[argc - 1], "BSE") == 0);
  if(isBSE) {
  ...
  if (livetrading_) {
    ...
    if (argc >= 4) {
      max_levels_ = std::min(15, atoi(argv[3]));
    }
    if (argc >= 5) {
    ...
    if (argc >= 5) {  // change this OF,PF
      max_levels_ = std::min(15, atoi(argv[4]));
    }
    if (argc >= 6) {
