...
def get_results_for_single_run(exec_, live_file_path_, date_, start_time_, end_time_, strat_name_, results_folder_, pid=1234,bar_data=0,exchange="NSE"):
    ...
    try:
        output = subprocess.getoutput(' '.join([exec_, live_file_path_, date_,
                                                start_time_, end_time_, str(runtime_id_),"--exchange",exchange]))
