...
void Dimer::LoadParams() {
  ...
  using_grt_control_file_ = Parser::GetBool(exec_key_val_map_, "USE_GRT_CONTROL_FILE", false);
  if(true == using_grt_control_file_){
    grt_control_filename_ = Parser::GetString(exec_key_val_map_, "GRT_CONTROL_FILE", "/home/irage/default_strat_control_paramfile");
...
//  DBGLOG_CLASS_FUNC_LINE_INFO << "DIMER :" << secondary_smv_->shortcode() << " " << bid_size_ << " " << ask_size_ << " " << max_bid_join_size_ << " " << max_ask_join_size_ << " " << min_bid_dime_size_ << " " << min_ask_dime_size_ << DBGLOG_ENDL_FLUSH;
  ...
  if(true == using_grt_control_file_){
    order_limit_ = Parser::GetInt(grt_control_key_map_, "ORDER_LIMIT", 0);
    dbglogger_ << "UPDATING GRT ORDER LIMIT " <<  order_limit_ << "\n";
  }
  ...
  if(true == using_grt_control_file_){
    delay_between_orders_ = Parser::GetInt(grt_control_key_map_, "DELAY_BETWEEN_ORDERS", 0);
    dbglogger_ << "UPDATING GRT DELAY_BETWEEN_ORDERS LIMIT " <<  delay_between_orders_ << "\n";
  }
  ...
  disclosed_qty_factor_ = Parser::GetDouble(exec_key_val_map_, "DISCLOSED_FACTOR", 1.0);
...
void Dimer::SendBidOrder(int current_order_bid_int_price_) {
  ...
  if (bid_order_) {
    ...
    if (bid_order_->int_price_ != current_order_bid_int_price_ && bid_order_->canceled_ == false &&
      ...
      bool is_modified_ =
          basic_om_->Modify(bid_order_, current_order_bid_price_, current_order_bid_int_price_, dimer_bid_size_,
                            ((use_reserve_msg_) && (is_priority_)), disclosed_qty_factor_);
      if (is_modified_) {
        /*dbglogger_ << watch_.tv() << " "
               << sec_shortcode_ << " " << identifier_
    ...
    bid_order_ = basic_om_->SendTrade(current_order_bid_price_, current_order_bid_int_price_, dimer_bid_size_,
                                      HFSAT::kTradeTypeBuy, 'B', HFSAT::kOrderDay, unique_exec_id_, 1, false, disclosed_qty_factor_);
...
void Dimer::SendAskOrder(int current_order_ask_int_price_) {
  ...
  if (ask_order_) {
    ...
    ask_order_ = basic_om_->SendTrade(current_order_ask_price_, current_order_ask_int_price_, dimer_ask_size_,
                                      HFSAT::kTradeTypeSell, 'B', HFSAT::kOrderDay, unique_exec_id_, 1, false, disclosed_qty_factor_);
