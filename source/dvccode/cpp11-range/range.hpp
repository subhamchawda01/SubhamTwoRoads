...
struct range_iter_base : std::iterator<std::input_iterator_tag, T> {
  ...
  bool operator!=(range_iter_base const& other) const { return not(*this == other); }
...
struct range_proxy {
  ...
  struct step_range_proxy {
    struct iter : detail::range_iter_base<T> {
      ...
      bool operator==(iter const& other) const { return step > 0 ? current >= other.current : current < other.current; }
