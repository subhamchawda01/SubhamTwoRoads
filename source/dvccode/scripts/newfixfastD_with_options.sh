...
MDS_EXEC=/home/pengine/prod/live_execs/mktFullDD
...
PIDFILE=$PIDDIR/$EXCH"_"$MODE"_FULL_"PIDfile.txt
...
case $EXCH in
	...
	;;
    NASDAQ)
	MDS_EXEC=$HOME/LiveExec/bin/nasdaqDD
	if [ $OPTARG == "ALPES" ] ; then MDS_EXEC=$MDS_EXEC" --inst_file /spare/local/files/NASDAQ/nasdaq-instruments-alpes.txt " ; fi
	;;
    NASDAQ_L1)
        MDS_EXEC=$HOME/LiveExec/bin/nasdaqL1DD
        if [ $OPTARG == "ALPES" ] ; then MDS_EXEC=$MDS_EXEC" --inst_file /spare/local/files/NASDAQ/nasdaq-instruments-alpes.txt " ; fi
        ;;
