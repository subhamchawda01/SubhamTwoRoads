...
int main(int argc, char** argv) {
  ...
  std::string token, msg_seq_no, aquantity, bprice, bquantity;
  ...
  while (filename_1 >> name >> token >> msg_seq_no >> tv >> aprice >> aquantity >> bprice >> bquantity) {
    ...
    latency_numbers.push_back(aprice-tv);
//     std::cout << std::fixed<< "ID1: " << id_ << " "<< tv  << " " << shc_to_symbol_map_[id_]<< std::endl;
  ...
  std::cout << ((long double)0 / latency_numbers.size()) * 1000000 << "\t";
  ...
  std::cout << 0 << "\t";
  std::cout << 0 << "\n";
  std::cout << std::fixed << std::setprecision(9) << "Size Read From File1: " << shc_to_symbol_map_.size() << std::endl;
  return 0;
 //  for(auto it = shc_to_symbol_map_.begin();
 //   it != shc_to_symbol_map_.end(); ++it)
// std::cout << it->first << " " << it->second << "\n";
  int count_faster_1 = 0, count_faster_2 = 0;
  while (filename_2 >> name >> token >> msg_seq_no >> aprice >> tv >> aquantity >> bprice >> bquantity) {
    std::string id_ = token + msg_seq_no;
