...
int ComputeNextExpiryWeeklyOptions(const int t_date_) {
  ...
  while (
      HFSAT::HolidayManagerNoThrow::IsExchangeHoliday(HFSAT::EXCHANGE_KEYS::kExchSourceNSEStr, t_expiry_date_, true)) {
...
int ComputeExpiryDateWeeklyOptions(const int expiry_number, int intdate_) {
...
int main(int argc, char **argv) {
  ...
  while (startdate_ < enddate) {
    int week_exp = ComputeNextExpiryWeeklyOptions(startdate_);
