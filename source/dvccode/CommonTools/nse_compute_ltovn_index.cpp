...
void ParseCommandLineParams(const int argc, const char **argv, std::string &ex_file, int &input_date_) {
  ...
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0] << " ex_file input_date_YYYYMMDD" << std::endl;
    exit(0);
  } else {
    ex_file = argv[1];
...
int main(int argc, char **argv) {
  ...
  ParseCommandLineParams(argc, (const char **)argv, ex_file, input_date_);
    ...
    std::string this_exchange_symbol_ = HFSAT::ExchangeSymbolManager::GetExchSymbol(my_shc_list_[i]);
     if (ex_vec.find(this_exchange_symbol_) != ex_vec.end() && ex_lot[this_exchange_symbol_] != -1) {
      ...
      //		  # AGGRE_NSE2241045 = 0,5.350000
		...
		std::cout << HFSAT::NSESecurityDefinitions::GetLastCloseForOptions(my_shc_list_[i]) << std::endl;
