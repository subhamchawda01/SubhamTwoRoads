...
std::string GetSampleFileName(std::string feature_tag_) {
  static std::map<std::string, std::string> fname_map_{
      {"VOL", "RollingSumVolume300.txt"},    {"STDEV", "RollingStdev300.txt"},
      {"L1SZ", "RollingAvgL1Size300.txt"},   {"L1EVPerSec", "L1EventsPerSecond.txt"},
      {"TREND", "SimpleTrend300.txt"},       {"ORDSZ", "RollingAvgOrdSize300.txt"},
      {"TRADES", "RollingSumTrades300.txt"}, {"SSTREND", "StableScaledTrend300.txt"},
      {"TOR", "TurnOverRate300.txt"},        {"BidAskSpread", "MovingAvgBidAskSpread300.txt"},
      {"AvgPrice", "AvgPrice300.txt"},       {"DELTA", "OptionsGreek300_1.txt"},
      {"GAMMA", "OptionsGreek300_2.txt"},    {"VEGA", "OptionsGreek300_3.txt"},
      {"THETA", "OptionsGreek300_4.txt"},    {"TrendStdev", "RollingTrendStdev300.txt"}};
