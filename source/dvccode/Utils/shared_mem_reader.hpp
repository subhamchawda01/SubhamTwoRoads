...
class SharedMemReader {
    ...
    void thread_main(void) {
      ...
      while (keepAlive) {
        ...
        if (p->writer_id == -1 && server_assigned_client_id_mantissa_ < ORS_MAX_NUM_OF_CLIENTS) {
          ...
          // Check if number of clients is more than 80% of ORS_MAX_NUM_OF_CLIENTS
          if (server_assigned_client_id_mantissa_ > (ORS_MAX_NUM_OF_CLIENTS * 0.8) && counter_mail % 20 == 0) { // don't want to be spammed with mails
            std::cout << "Alert ORS NUM OF CLIENTS ALERT ABOVE 80% . Cuurent Number " << server_assigned_client_id_mantissa_ << std::endl;
            HFSAT::Email e;
            e.setSubject("ORS NUM OF CLIENTS ALERT ABOVE 80%");
            e.addRecepient(
                "nseall@tworoads.co.in, "
                );
            e.addSender("ravi.parikh@tworoads.co.in");
            e.content_stream << "host_machine: " << HFSAT::GetCurrentHostName() << "<br/>";
            e.content_stream << "CURRENT SACI VALUE : " << server_assigned_client_id_mantissa_
                              << " MAX SACI VALUE : " << ORS_MAX_NUM_OF_CLIENTS << "<br/>";
            e.sendMail();
          }
          counter_mail++;
      ...
      while (keepAlive) {
        ...
        for (auto i = 0u; i < vec.size(); i++) {
          ...
          if (!isAlivePID(vec.at(i).first)) {
            ...
            std::pair<int, int> temp_pair = std::make_pair(-1, -1);
            vec.replace_element(i,temp_pair);
          }
        }
 ...
 public:
  ...
  bool readTNonBlocking(DataWriterIdPair<T>& dest) { return dataQ->popNonBlocking(dest); }
