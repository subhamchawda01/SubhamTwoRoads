...
#define DEF_BSE_STRATEGY_MD5SUM_ALGOCODE_FILENAME "/spare/local/files/EUREX/bse_md5sum_strategy_algocode_database.db"
...
class BSEAlgoTagging {
 ...
 private:
  ...
  void EmailBSEAlgoTaggingAlert(std::string _alert_body_, bool _is_alert_ = true) {
    // also send an alert
    char hostname[128];
    hostname[127] = '\0';
    ...
    if (_is_alert_) {
      e.setSubject("BSEAlgoTagginFailure");

    } else {
      e.setSubject("BSEAlgoTagginInfo");
    }
  ...
  void LoadMD5SUMStrategyNamesToCodeDatabase(const std::string& exchange) {
    ...
    if (exchange == "ICE") {
      ...
      md5sum_strategy_names_to_code_database_stream_.open(DEF_BSE_STRATEGY_MD5SUM_ALGOCODE_FILENAME, std::ios::in);
    }

    if (!md5sum_strategy_names_to_code_database_stream_.is_open()) {
      std::cerr << " Failed To Open Strategy-Algo Mapping Database : " << DEF_BSE_STRATEGY_MD5SUM_ALGOCODE_FILENAME
                << " With : " << strerror(errno) << "\n";
      EmailBSEAlgoTaggingAlert(std::string(" Failed To Open Strategy-Algo Mapping Database : ") +
                               std::string(DEF_BSE_STRATEGY_MD5SUM_ALGOCODE_FILENAME));
...
#undef MAX_LINE_SIZE
  ...
  BSEAlgoTagging(const BSEAlgoTagging& disabled_copy_constructor_);

  BSEAlgoTagging(const std::string& exchange)
 ...
 public:
  static BSEAlgoTagging& GetUniqueInstance(const std::string& exchange) {
    static BSEAlgoTagging unique_instance_(exchange);
  ...
  void BSEAlgoTaggingReloadDatabase(const std::string& exch_) { LoadMD5SUMStrategyNamesToCodeDatabase(exch_); }
  ...
  bool AddClientInfo(const int32_t& _client_id_, const std::string& _md5sum_, const std::string& _strategy_name_) {
    ...
    if ((key_obtained_from_saci < (int)client_id_to_algo_mapping_.size()) &&
      ...
      EmailBSEAlgoTaggingAlert(t_temp_oss_.str());
    ...
    //        EmailBSEAlgoTaggingAlert ( t_temp_oss_.str (), false ) ;
  ...
  void TagAlgo(const int& _client_id_, const std::string& _md5sum_, const std::string& _strategy_name_,
    ...
    if (write_length_ < (int)((t_temp_oss_.str()).length())) {
      std::ostringstream t_temp_oss_;
      t_temp_oss_ << " Failed To Tag BSE Algo, Control Write Failed, Expected to Write : "
