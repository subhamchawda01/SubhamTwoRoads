...
class BMFEPEngine : public BaseEngine, SimpleSecuritySymbolIndexerListener {
 public:
  ...
  void ModifyOrder(const Order* r_order_, BMFEPFIX::CancelReplace* p_target_cancel_replace_, ORS::Order* orig_order);

  void SendOrder(Order* order) override;
  void CancelOrder(Order* order) override;
  void ModifyOrder(Order* order, ORS::Order* orig_order) override;

  // From Thread class inherited from BaseEngine.
  void thread_main();

  virtual int onInputAvailable(int sock, char* buffer, int32_t length);
