...
class MarginChecker {
 private:
  ...
  MarginChecker(DebugLogger& _dbglogger_, HFSAT::ExchSource_t exchange)
      : dbglogger_(_dbglogger_),
        ...
        max_exposure_(0),
        use_max_exposure_(false),
        ors_orders_enabled(true){
 ...
 public:
  ...
  inline void Set(int t_security_id_, int pos_, int max_ord_size_, int worst_pos, int numords) {
    if (t_security_id_ < DEF_MAX_SEC_ID) {
      ...
      //std::cout << "ADDTS VALUE IS " << pos_ << " " << max_ord_size_  << " " << worst_pos << std::endl;
      //std::cout << "SETTING ADDTS" << std::endl;
    }
  }
  ...
  /// accessor functions
  inline int getMaxPos(int sec_id) const { return limits_[sec_id].max_position_; }

  inline int getMaxOrdSize(int sec_id) const { return limits_[sec_id].max_order_size_; }

  inline int getWorstPos(int sec_id) const { return limits_[sec_id].max_worst_case_position_; }

  inline int getMaxLiveOrd(int sec_id) const { return limits_[sec_id].max_live_order_; }
  ...
  inline ORSRejectionReason_t Allows(const int _security_id_, const double _price_, int _size_requested_,
    ...
    if (_buysell_ == kTradeTypeBuy) {
      ...
      int resultant_global_worst_bid_pos = (_size_requested_ + global_worst_bid_pos);
      ...
      int resultant_global_worst_ask_pos = (_size_requested_ + global_worst_ask_pos);
    ...
    if (false == ors_pnl_manager_.AllowsPnlCheck() || false == ors_orders_enabled) return kORSRejectFailedPnlCheck;
  ...
  inline void setMaxExposure(long int max_exposure) { 
    max_exposure_ = max_exposure; 
  }

  inline long int getMaxExposure() { return max_exposure_; }

  inline void setUseMaxExposure(bool use_max_exposure){ 
    use_max_exposure_ = use_max_exposure;
  }

  inline void setOrsOrderCheck(bool _ors_orders_enabled_ ){
    ors_orders_enabled = _ors_orders_enabled_;
  }
  inline bool getUseMaxExposure(){ return use_max_exposure_; }
 ...
 private:
  ...
  long int max_exposure_;
  bool use_max_exposure_;
  bool ors_orders_enabled;
};
}
}
