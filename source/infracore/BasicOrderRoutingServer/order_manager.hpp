...
class OrderManager {
 public:
  ...
  inline Order *GetInactiveOrderByExchAssignedOrderId(const uint64_t _exch_ord_id_) const {
    Order *retval = NULL;
    for (unsigned int i = 0; i < inactive_order_vec_.size(); i++) {
      if (inactive_order_vec_[i]->exch_assigned_seq_ == _exch_ord_id_){
  ...
  /// Attempt to find this order among active orders by exch_ord_id
  inline Order *GetOrderByExchAssignedOrderId(const uint64_t _exch_ord_id_) const {
    Order *retval = NULL;
    ...
    while (_citer_ != saos_to_order_ptr_map_.end()) {
      Order *p_this_order_ = _citer_->second;
      if (p_this_order_->exch_assigned_seq_ == _exch_ord_id_) {
        retval = p_this_order_;
        break;
      }
      _citer_++;
...
//      std::cout << "DeallocateActiveOrder " << "SAOS: " << p_active_order_->server_assigned_order_sequence_ 
//                << "SizeRem: " << p_active_order_->size_remaining_ <<  std::endl;
    } else {
  ...
  std::string DumpOMState(bool all_pos_flag = false) {
    std::ostringstream temp_oss;
    std::tr1::unordered_map<int, Order *>::iterator mapiter;
    int t_counter_ = 0;
    int max_count = (all_pos_flag) ? saos_to_order_ptr_map_.size() : 500;

    // lock saos_to_order_ptr_map_lock_
    //    saos_to_order_ptr_map_lock_.LockMutex();

    for (mapiter = saos_to_order_ptr_map_.begin(); mapiter != saos_to_order_ptr_map_.end() && t_counter_ < max_count;
         ++mapiter, ++t_counter_) {
      temp_oss << "SAOS:\t" << mapiter->first <<"\tCAOS\t" << mapiter->second->client_assigned_order_sequence_ 
               <<"\tSACI\t" << mapiter->second->server_assigned_client_id_  << "\t Size_Remaining\t" << mapiter->second->size_remaining_
               ...
               << "\tStatus:\t" << (mapiter->second->is_confirmed_ ? "CONF" : "SEQ") << "\tActive:\t" <<
               (mapiter->second->is_active_order ? "Yes" : "No") << "\n";
    }
 ...
 private:
  ...
  /// Using LockFreeSimpleMempool instead of calling "new" everytime to reduce the number of system calls.
  ...
  /// we'll probably need a LockFreeSimpleMempool < EUREXORSOrder > and LockFreeSimpleMempool < CMEORSOrder >,
  /// with EUREXORSOrder and CMEORSOrder child classes of Order.
  /// perhaps we could make this a template class on typename Order ?
  LockFreeSimpleMempool<Order> order_mempool_;
  ...
  LockFreeSimpleMempool<Order> inactive_order_mempool_;
