...
while(IsExchangeHoliday("SGX", $next_date_) == 1)
{
...
my %sgx_exch_future_code_to_trading_hours_= GetFutureCodeToTradingHoursMap();
...
for (my $i = 0; $i <= $#ne_trades1_file_lines_; $i++) {
  ...
  if ( $ne_exch_future_code_to_exchange_{$exch_future_code_} ne "SGX"){
    next;
  }
  if ( int ( $trade_time_  ) >= $sgx_exch_future_code_to_trading_hours_{$exch_future_code_}[0] && int ( $trade_time_ ) < $sgx_exch_future_code_to_trading_hours_{$exch_future_code_}[1] ){}
  else{next;}
...
for (my $i = 0; $i <= $#ne_trades2_file_lines_; $i++) {
  ...
  if ( int ( $trade_time_ ) < $sgx_exch_future_code_to_trading_hours_{$exch_future_code_}[0] || int ( $trade_time_  ) >= $sgx_exch_future_code_to_trading_hours_{$exch_future_code_}[2]){}
  else{ next ;}
  my $date_ = substr ($fields_[$date_offset_], 1, -1); $date_ =~ s/^\s+|\s+$//g;
