...
my $USAGE="$0 NewEdge_trades_filename_ YYYYMMDD";
...
#	print "Malformatted line : $ne_trades_file_lines_[$i] No. of fields : $#fields_\n";
  ...
  if ( $ne_exch_future_code_to_exchange_{$exch_future_code_} eq "SGX"){
    next;
  }
  ...
  if(! exists $broker_symbol_to_exchange_symbol_{$broker_symbol}){
    ...
    {
      print STDERR "RECON : TRANSLATION FAILED $exch_future_code_ $broker_symbol \n";
