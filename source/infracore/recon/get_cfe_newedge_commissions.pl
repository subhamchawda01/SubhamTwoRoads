...
for (my $i = 0; $i <= $#ne_trades1_file_lines_; $i++) {
  ...
  if ( $ne_exch_future_code_to_exchange_{$exch_future_code_} ne "CFE"){
    next;
  }
 ...
 #   next;
  ...
  my $cfe_exchange_fee=1;
  my $rebated_exchange_fee=0.55;    #55% rebate
  #VX handling
  if ( (index ( $exchange_symbol, "VX" ) == 0) ) {
     $fee2_amount_= ($fee2_amount_ + ($quantity_traded_ * ($cfe_exchange_fee - $rebated_exchange_fee))) ;
  }
