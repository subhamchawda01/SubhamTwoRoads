...
for ( my $i = 0; $i <= $#shortcodes_; $i++)
{
    ...
    my $interval_ = 900;
    my $outfile_ = $HOME_DIR."/plots/".$symbol_.".txt".$yyyymmdd;   
    my $outpng_ =  $HOME_DIR."/plots/".$symbol_.$yyyymmdd.".png";   
    `~/basetrade_install/bin/get_periodic_volume_on_day $symbol_ $yyyymmdd $interval_ > $outfile_`;
