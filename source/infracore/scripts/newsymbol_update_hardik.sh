...
new_prod_contract_file_="/tmp/fo_contract_${symbol}"
>${new_prod_contract_file_=}
echo "Check and update in ${tradeinfo_dir}/ContractFiles/nse_contracts.${next_working_day}"
echo "cmd1---> cat ${new_prod_contract_file_} >> ${tradeinfo_dir}/ContractFiles/nse_contracts.${next_working_day}"

for exp_d in "${!date_map[@]}";
do 
      grep "\b$symbol[0-9]" $contract_file | grep FUT | grep $exp_d >$tmp_file1 
      price=`cat $tmp_file1 | cut -d'|' -f68`
      lotsize=`cat $tmp_file1 | cut -d'|' -f32`
      price=`perl -e "print sprintf(\"%.2f\", $price/100);"`
      echo -e "STKFUT\t$symbol\t$price\t$lotsize\t0.05\t${date_map[$exp_d]}" >>${new_prod_contract_file_}
done
...
do
      ...
      echo -e "STKOPT\t$symbol\t$price\t$lotsize\t0.05\t${date_map[$exp_d]}\t10\t$tick\t$tick\t\t10" >>${new_prod_contract_file_}
      echo "${date_map[$exp_d]}" >>$expiry_days
done

data_source="/spare/local/tradeinfo/NSE_Files/datasource_exchsymbol.txt"
new_ds_file_="/tmp/ds_file_${symbol}"
>${new_ds_file_}
...
echo "cmd2 ---> cat ${new_ds_file_} >> $data_source"
...
for exp_d in "${!date_map[@]}";
do
    echo -e "NSE${last_num}\tNSE_${symbol}_FUT_${date_map[$exp_d]}" >>${new_ds_file_}
    last_num=$((last_num+1))
done
exp_days=($( <$expiry_days))
echo -e "NSE${last_num}\tNSE_${symbol}_FUT_${exp_days[0]}_${exp_days[1]}" >>${new_ds_file_}
last_num=$((last_num+1))
...
do       
        ...
        echo -e "NSE${last_num}\tNSE_${symbol}_PE_${exp_days[0]}_${showprice}" >>${new_ds_file_}
        last_num=$((last_num+1))
        echo -e "NSE${last_num}\tNSE_${symbol}_PE_${exp_days[1]}_${showprice}" >>${new_ds_file_}
        last_num=$((last_num+1))
#        echo -e "NSE${last_num}\tNSE_${symbol}_PE_${exp_days[2]}_${showprice}"
#        last_num=$((last_num+1))
        echo -e "NSE${last_num}\tNSE_${symbol}_CE_${exp_days[0]}_${showprice}" >>${new_ds_file_}
        last_num=$((last_num+1))
        echo -e "NSE${last_num}\tNSE_${symbol}_CE_${exp_days[1]}_${showprice}" >>${new_ds_file_}
        last_num=$((last_num+1))
...
new_sec_margin_file_="/tmp/sec_margin_file_${symbol}"
>${new_sec_margin_file_}
...
grep SBIN /home/pengine/prod/live_configs/nse_midterm_shortcodes_1 >>/tmp/filenewShortcode
new_sym=$symbol
echo $new_sym
...
awk -v pr=$price '{print $1" " pr }' /tmp/securitymarginTobeadded >${new_sec_margin_file_}

echo "sec margin update cmd ---> cat ${new_sec_margin_file_} >> /spare/local/tradeinfo/NSE_Files/SecuritiesMarginFiles/security_margin_${next_working_day}.txt"
echo ""
echo ""
