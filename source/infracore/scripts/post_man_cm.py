...
def get_token(usr_pass_comb_, nonce_ddmmyyyyhhmmssSSS_):
  ...
  print("base64: " + base64_usr_pass, flush=True)
  ...
  for key in headers:
    print(key + " : " + headers[key], flush=True)

  conn.request("POST", "/token", payload, headers)
  res = conn.getresponse()
  print("TOKEN STATUS: " + str(res.status) + " REASON: " + str(res.reason), flush=True)
  data = res.read()
  print(data.decode("utf-8"))
  sys.stdout.flush()
  return json.loads(data.decode("utf-8"))

def get_tradeexport_file(trade_export_dir, date_, base64_nonce_, next_seq_no_, token_, trade_inquiry_str_, cm_fo_, curr_time_str_, next_msg_seq_no ="0"):
  is_start_ = True
  #next_msg_seq_no = "0"
  prev_msg_seq_no = next_msg_seq_no
  #current time:
  #curr_time_str_ = datetime.now().strftime("%H%M") 
  curr_time_secs = time.time()
#
  comb_filename = trade_export_dir + "/Trade_export_" + cm_fo_ + "_" + date_ + "_" + curr_time_str_ + "_comb.txt"
  comb_file = open(comb_filename,"a+")
  ...
  while is_start_:
    ...
    seq_no_str_ = ''
    if update_needed:
      print("Curr_Details: seq_no_str: " + str(seq_no_str_) + " Next_seq_no: " + str(next_seq_no_), flush=True)
      seq_no_str_ = get_seq_no(next_seq_no_)
      next_seq_no_ = str(int(next_seq_no_) + 1)
      print("Updated_Details: seq_no_str: " + str(seq_no_str_) + " Next_seq_no: " + str(next_seq_no_), flush=True)
      
    
    update_needed = True
    print("############\n\nSeq_str: " + seq_no_str_, flush=True)
    msgId_ = member_code_ + date_ + seq_no_str_
    print("msgId: " + msgId_, flush=True)
    conn = http.client.HTTPSConnection("www.connect2nse.com")
    ...
    print("PayLOAD: \n" + payload, flush=True)
    print("header_Authorization: " + headers['Authorization'], flush=True)
    try:
        conn.request("POST", trade_inquiry_str_, payload, headers)
        res = conn.getresponse()
        print("Response Done\n", flush=True)
        print(str(res), flush=True) #for testing
        data = res.read()
        print("Get Data Done\n", flush=True)
        ...
        print("****** Error ******")
        traceback.print_exc()
        print("SLEEP for 15 secs and CONTINUE......")
        update_needed = False
        time.sleep(15)
        continue
        #exit(0)
    #
    print("Response: \n" + response_str_, flush=True)
    strt_index = 0
    ...
    print("msgId, status, msg_code :: " + msgId_ + ", " + status + ", " + message_code, flush=True)
    ...
    print("trade_inq_info: " + seq_entries, flush=True)
    strt = 3
    
    prev_msg_seq_no = next_msg_seq_no
    next_msg_seq_no = seq_entries[strt:seq_find]
    print("Prev_Seq_str: " + prev_msg_seq_no + " Next seq_str: " + next_msg_seq_no, flush=True)
#    
    print("seq_str: " + next_msg_seq_no + " seq_find: " + str(seq_find), flush=True)
    no_of_entries = int(seq_entries[seq_find+1:])
    print("no_of_entries: " + str(no_of_entries), flush=True)

    if no_of_entries <= 0 :
      print("END no_of_entries: " + str(no_of_entries), flush=True)
      break
...
#
    ...
    print("last_find: " + str(lst_find), flush=True)
    ...
    print("time: " + str(time.ctime(lst_entry_time)), flush=True)
    print("last_entry: " + last_entry, flush=True)


    #
    filename = trade_export_dir + "/Trade_export_" + cm_fo_ + "_" + date_ + "_" + seq_no_str_ + "_" + next_msg_seq_no + "_" + curr_time_str_ + ".txt"
    ...
    if curr_time_secs < lst_entry_time :
      print("time extended", flush=True)
      is_start_ = False
      break
    print("Wait 30 secs", flush=True)
    time.sleep(30)
    #
  
  comb_file.close()
  print("***LAST Prev_Seq_str: " + prev_msg_seq_no + " Next seq_str: " + next_msg_seq_no, flush=True)
  return next_msg_seq_no
#
...
#  exit(0)
  ...
  n = len(sys.argv)
  if n < 3 :
    print("USAGE: script CM/FO next_seq_no next_msg_seq")
    exit(0)
...
#  hhmm_ = str(sys.argv[3])
  ...
  last_time_limit = datetime.now().replace(hour=16, minute=0, second=0, microsecond=0)
  curr_time_str_ = curr_time.strftime("%H%M") 
  ...
  print("Trade export file is getting downloaded. Kindly please don't stop before 4pm", flush=True)

  #path for saving trade export file
  trade_export_dir = "C:/Users/DELL/Desktop/Postman_Position_File/" + cm_fo_ + "/" + \
  str(curr_time.year) + "/" + mnth + "/" + day
  os.makedirs(trade_export_dir, exist_ok = True)
   
  output_print_file = trade_export_dir + "/output_file_" + date_ + "_" + curr_time_str_ + ".txt"
  
  op_file = open(output_print_file, "a+")
  
  sys.stdout = op_file
  sys.stdout.flush()  
  print('date ' + date_ + ' HHMM: ' + hhmm_, flush=True)
  print('DIR: ' + trade_export_dir, flush=True)  
  
  next_seq_no_ = sys.argv[2]
  next_msg_seq_no_ = sys.argv[3]

  print("INQUIRY: " + trade_inquiry_str_, flush=True)
  #exit(0)
  
#  while True:
  hhmm_ = datetime.now().strftime("%H%M")
  nonce_ = date_ + hhmm_ + "00000:123456"
  print("date_: " + date_ + "hhmm: " + hhmm_, flush=True)
  print("nonce: " + nonce_, flush=True)
    #exit(0)
  base64_nonce_ = get_base64(nonce_)

  token_key_value = get_token(usr_pass_comb_, base64_nonce_)
  token_ = token_key_value['access_token']
  print("TOKEN: " + token_, flush=True)

  while True:  
  #get Trade export file
    ret_next_msg_seq_no = get_tradeexport_file(trade_export_dir, date_, base64_nonce_, next_seq_no_, token_, trade_inquiry_str_, cm_fo_, curr_time_str_, next_msg_seq_no_)
    print("RETURN VALUE ::: Next seq_str: " + ret_next_msg_seq_no, flush=True)
    next_msg_seq_no_ = ret_next_msg_seq_no
  
    time_now = datetime.now()
    #if is_get_trade_limit_exceed:
    if time_now > last_time_limit:
      print("****** Time exceeded Stoping Scrit ******", flush=True)
      break
    print("********* Go to Sleep for 3 mins *********", flush=True)
    time.sleep(200)
