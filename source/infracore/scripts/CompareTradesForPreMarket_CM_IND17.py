...
MDS_LOGGER_VOL = '/home/pengine/prod/live_execs/mds_fast_first_trade_read_Volume'
BHAV_DIR = '/spare/local/tradeinfo/NSE_Files/Margin_Files/Exposure_Files/'
OUT_FILE = '/home/dvcinfra/important/PreMarketTrades/Product_Ratio_NONFO/'
LOT_SIZE_DIR = '/spare/local/tradeinfo/NSE_Files/Lotsizes/' 
MDS_INPUT_FILE = '/tmp/mds_fast_reader_input_file_nonfo'
MDS_OUTPUT_FILE = '/tmp/mds_fast_out_nonfo'
MDS_OUTPUT_FILE_VOL = '/tmp/mds_fast_out_nonfo_vol'
...
def Update_html():
    command = '/home/dvcinfra/important/PreMarketTrades/scriptsNonFO/generate_preMarket_trades_report.sh \
            >>/tmp/Pre_Markets_Update_log_nonfo 2>&1'
    BashExec(command)
...
def Exec_mds_fast_logger_vol(pre_market_start, pre_market_end):
    command = MDS_LOGGER_VOL + ' ' + 'GENERIC'  + ' \"' \
                                      + MDS_INPUT_FILE + '\" ' + str(pre_market_start) + ' ' + str(pre_market_end) \
                  + ' | less  > ' + MDS_OUTPUT_FILE_VOL
    print (command)
    BashExec(command)
...
def Dump_data_output(date_,sym_close_dic):
    ...
    file_read_mds_out_vol = open(MDS_OUTPUT_FILE_VOL ,'r')
    sym_trade_dic = {}
    sym_quan_dic = {}
    for line in file_read_mds_out_vol:
        words = line.split(' ')
        if len(words) != 3:
            print "count does not match no trade format"
            continue
        sym = words[0].split('_')[1]
        if sym not in sym_close_dic:
            continue
        sym_trade_dic[sym] = words[1].rstrip() 
        sym_quan_dic[sym] = words[2].rstrip()
    file_read_mds_out_vol.close()
    ...
    for line in file_read_mds_out:
        words = line.split(' ')
        if len(words) != 8:
            print "count does not match no trade format", len(words)
            continue
        trade_price = words[6]
        ...
        file_output.write(sym + ' ' + str(round(ratio_trade_closing,2)) + ' '
                    + str(round((int(trade_price) / 100.0),2)) + ' '
                    + str(sym_close_dic[sym]) + ' ' + sym_trade_dic[sym] + ' ' + sym_quan_dic[sym] + '\n')
    file_output.close()
...
def main():
    ...
    if os.path.exists(BHAV_DIR + 'cm' + PDD + month_name + PYYYY
        ...
        command = 'echo "" | mailx -s "Contract File missing For"' + PDD + month_name + ' -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in'
        BashExec(command)
        exit(0)
    ...
    Exec_mds_fast_logger_vol(pre_market_start,pre_market_end)
