...
USAGE1="$0 EXCH LOC YYYYMMDD CMD[START/STOP] ACCELERATOR[ONLOAD(ON)/VMA/OFF] AFFINITY[AF/NF] OPTION[SET_TIME]<- LOC & YYYYMMDD unused.";
USAGE2="$0 EXCH LOC YYYYMMDD CMD[COPYDATA/CLEARDATA/INTRMECOPYDATA]";
...
MDS_LOG_EXEC=$HOME/LiveExec/bin/mds_logger_MCAST

COPY_LOG_EXEC=$HOME/LiveExec/scripts/mds_log_backup.sh
INTERMCOPY_LOG_EXEC=$HOME/LiveExec/scripts/mds_intermediate_log_backup.sh
CLEAR_LOG_EXEC=/home/pengine/prod/live_scripts/mds_log_clear.sh
...
PIDFILE=$PIDDIR/MDSLOGGER_$EXCH"_PIDfile.txt"
...
case $CMD in
	...
	then
		...
		if [ "$OPTION" == "SET_TIME" ] ; then onload $MDS_LOG_EXEC --exch $EXCH --ip $MCAST_IP --port $MCAST_PORT --set_time &
		...
		then
		    ...
		    if [ "$OPTION" == "SET_TIME" ] ; then LD_PRELOAD=libvma.so $MDS_LOG_EXEC --exch $EXCH --ip $MCAST_IP --port $MCAST_PORT --set_time &
		    else LD_PRELOAD=libvma.so $MDS_LOG_EXEC --exch $EXCH --ip $MCAST_IP --port $MCAST_PORT &
		    fi

		else

		   if [ "$OPTION" == "SET_TIME" ] ; then $MDS_LOG_EXEC --exch $EXCH --ip $MCAST_IP --port $MCAST_PORT --set_time &
    ...
    intrmecopydata|INTRMECOPYDATA)
	$INTERMCOPY_LOG_EXEC $EXCH $LOC $YYYYMMDD &
	;;
