...
PerDayORSReportsCash () { 
  ...
  echo "<th>Symbol</th><th>Total_Order</th><th>Total_Exec</th><th>Exec_Ratio</th><th>Total_Modify</th><th>Total_Modify_OF</th><th>ModifyOF%</th><th>Total_MDS_Msg</th><th>Total_Trade</th><th>Trade_Ratio</th><th>Total_Trade_Modify</th></tr></thead><tbody>" >> $PerDayORS_cash
  ...
  for sym in "${!day_product_fo[@]}"; do
    ...
    echo "<tr><td><b>$symbol</b></td><td>${day_product_fo[$sym]}</td><td>${day_exec_fo[$sym]}</td><td>$day_ratio_order</td><td>${day_modify_fo[$sym]}</td><td>${day_modifyof_fo[$sym]}</td><td>$day_percent_modify</td><td>${day_mds_msg_fo[$sym]}</td><td>${day_trade_fo[$sym]}</td><td>$day_ratio_trade</td><td>${day_mds_modify_fo[$sym]}</td></tr>" >> $PerDayORS_cash
  done
...
month_file_gen(){
  ...
  echo "<th>Symbol</th><th>Total_Order</th><th>Total_Exec</th><th>Exec_Ratio</th><th>Total_Modify</th><th>Total_Modify_OF</th><th>ModifyOF%</th><th>Total_MDS_Msg</th><th>Total_Trade</th><th>Trade_Ratio</th><th>Total_Trade_Modify</th></tr></thead><tbody>" >> $month_cash ;
  ...
  for sym in "${!month_product_fo[@]}"; do
    ...
    symbol=`grep $sym $datasource | awk '{print $NF}'`
    echo "<tr><td><b>$symbol</b></td><td>${month_product_fo[$sym]}</td><td>${month_exec_fo[$sym]}</td><td>$ratio_order</td><td>${month_modify_fo[$sym]}</td><td>${month_modifyof_fo[$sym]}</td><td>$percent_modify</td><td>${month_mds_msg_fo[$sym]}</td><td>${month_trade_fo[$sym]}</td><td>$ratio_trade</td><td>${month_mds_modify_fo[$sym]}</td></tr>" >> $month_cash
  done
...
#dailyreport="/home/dvcinfra/important/subham/dailyreport.txt"
  ...
  echo "<h3>ORS REPORTS OF LAST 20 DAYS</h3></div>" >> $REPORTING_FILE
  echo "<table class="table table-striped" style='border: 1px solid grey;margin-bottom: 0px;'><thead><tr>" >> $REPORTING_FILE
  echo "<th>Month</th><th>Total_Order</th><th>Total_Exec</th><th>Exec_Ratio</th><th>Total_Modify</th><th>Total_Modify_OF</th><th>ModifyOF%</th><th>Total_MDS_Msg</th><th>Total_Trade</th><th>Trade_Ratio</th><th>Total_Trade_Modify</th></tr></thead><tbody>" >> $REPORTING_FILE
  yyyymm=${today:0:6}
  yyyymmdd=$today
  ...
  echo "<tr><td><b><a href="daily_ors_file/"$yyyymm".fo.index.html" style="color:blue">$yyyymm</a></b></td><td>$total_order</td><td>$total_exec</td><td>$total_ratio_order</td><td>$total_modify</td><td>$total_modifyof</td><td>$total_percent_modify</td><td>$total_mds_msg</td><td>$total_trade</td><td>$total_ratio_trade</td><td>$total_mds_modify</td>" >> $REPORTING_FILE
  ...
  echo "<th>Date</th><th>Total_Order</th><th>Total_Exec</th><th>Exec_Ratio</th><th>Total_Modify</th><th>Total_Modify_OF</th><th>ModifyOF%</th><th>Total_MDS_Msg</th><th>Total_Trade</th><th>Trade_Ratio</th><th>Total_Trade_Modify</th>" >> $REPORTING_FILE;
  ...
  echo "<tr><td><a href ="daily_ors_file/"$yyyymmdd".fo.index.html" style="color:blue">$yyyymmdd</a></td><td>$day_total_order</td><td>$day_total_exec</td><td>$day_total_ratio_order</td><td>$day_total_modify</td><td>$day_total_modifyof</td><td>$day_total_percent_modify</td><td>$day_total_mds_msg</td><td>$day_total_trade</td><td>$day_total_ratio_trade</td><td>$day_total_mds_modify</td>" >> $dailyreport;
