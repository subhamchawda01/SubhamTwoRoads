...
declare -A max_order_shortcode_position 
declare -A under_ban_shortcode_position 

declare -A hostname_to_ip
hostname_to_ip=( ["sdv-ind-srv11"]="10.23.227.61" \
              ["sdv-ind-srv14"]="10.23.227.64" \
              ...
              ["sdv-ind-srv24"]="10.23.227.74" )

declare -A hostname_to_profile
hostname_to_profile=( ["sdv-ind-srv11"]="MSFO" \
                     ["sdv-ind-srv14"]="MSFO7" \
                     ["sdv-ind-srv15"]="MSFO4" \
                     ["sdv-ind-srv16"]="MSEQ2" \
                     ["sdv-ind-srv17"]="MSEQ3" \
...
do_file_addts() {
#  /home/pengine/prod/live_scripts/ors_control.pl NSE ${profile} LOADTRADINGSYMBOLFILE $AddtsRejectFile
   /home/pengine/prod/live_scripts/ADDTRADINGSYMBOL.sh $AddtsRejectFile
}

addts_prod_file() {
  if [ $position -ne -1 ]; then
    echo "NSE $profile ADDTRADINGSYMBOL \"$SymHandle\" $position $position $position $position" >> $AddtsRejectFile
    ...
    if [ $worst_case_pos -eq 1 ]; then
      ...
      echo "NSE $profile ADDTRADINGSYMBOL \"$SymHandle\" $((pos1 + 20)) $((pos2 + 20)) $((pos3 + 20)) $((pos4 + 20))" >> $AddtsRejectFile
      #echo "ORS log Position - ADDTS : ${SymHandle} $((pos1 + 20)) $((pos2 + 20)) $((pos3 + 20)) $((pos4 + 20))"
    else
      echo "ORS log Position - ADDTS : ${SymHandle} $pos1 $pos2 $pos3 $pos4"
      echo "NSE $profile ADDTRADINGSYMBOL \"$SymHandle\" $pos1 $pos2 $pos3 $pos4" >> $AddtsRejectFile
    fi
    ...
    echo "NSE $profile ADDTRADINGSYMBOL \"$SymHandle\" 20 20 20 20" >> $AddtsRejectFile
    echo "NSE $profile ADDTRADINGSYMBOL $SymHandle 20 20 20 20" >> $DefaultAddtsFile
...
addts_prod() {
  ...
  unset max_order_shortcode_position 
...
max_order() {
  [ `hostname` == "sdv-ind-srv11" ] || print_msg_and_exit "maxorder is for sdv-ind-srv11";

  unset addts_shortcode_position
  declare -A addts_shortcode_position
  for key in ${!max_order_shortcode_position[@]}; do
    SymHandle=$key
    addts_shortcode_position[$key]=${max_order_shortcode_position[${key}]}
    echo "SymHandel : $SymHandle"

    cd $MidtermDir
    grep -a "ALERT" *_execlogic_dbglog.${date_} > $tmpfil1
    ...
    if grep -waq "$SymHandle" $tmpfil1 ; then
      midexec=`grep -a -w "$SymHandle" $tmpfil1 | tail -1 |cut -d' ' -f1 | cut -d'_' -f1` 
      livefile="${ExecOrder}$midexec"
      echo "mid exec : $midexec  livefile : $livefile"
  
      if ssh dvctrader@${ip_add} "grep -waq "$SymHandle" $livefile"; then
        ssh dvctrader@${ip_add} "awk -F',' -v sym=$SymHandle '{ if (\$1==sym) print \$1\",\"\$2*2; else print \$0}' $livefile > $tmpfil2"
        ssh dvctrader@${ip_add} "cp $tmpfil2 $livefile"
        echo "livefile : $livefile symbol : $SymHandle value doubled"
      else
        ssh dvctrader@${ip_add} "echo \"${SymHandle},60\" >> $livefile"
...
common_init_margin() {
  ...
  for key in ${!common_margin_shortcode_position[@]}; do
    ...
    if ! grep -waq "$SymHandle" $HardMargin ; then	
      append_line=`grep -a -n "NSE_" $HardMargin | tail -1 | cut -d':' -f1`
  ...
  /home/pengine/prod/live_scripts/ors_control.pl NSE $profile RELOADMARGINFILE
  sleep 3
  addts_prod
...
security_not_found() {
  ...
  /home/pengine/prod/live_scripts/ors_control.pl NSE $profile RELOADSECURITYMARGINFILE
  sleep 3
  addts_prod
...
[ "$is_holiday" == "1" ] && print_msg_and_exit "NSE Holiday. Exiting..."
...
UNDERBAN_FILE="/spare/local/tradeinfo/NSE_Files/SecuritiesUnderBan/fo_secban_${date_}.csv"
...
if [ $# -eq 2 ] || [ $# -eq 3 ]; then
        case "$reject_type" in
                "maxorder")
                   if [ `echo $SymHandle | grep -a "NSE_" | wc -l` -eq 1 ]; then
                     max_order_shortcode_position[$SymHandle]=$position
                   else
                     while read -u 9 line
                     do
                ...
                "underban")
                   ...
                   under_ban
                   ;;
                *)  echo "INVALID REJECT TYPE ENTERED <maxorder(ind11)/worstcasepos/resetting/addts/margin/physical/underban>"
        esac
else
        echo "USAGE: SCRIPT <maxorder(ind11)/worstcasepos/resetting/addts/margin/physical/underban> <(NSE_PRODUCT)/(NSE_PRODUCT POSITION)/infile(NSE_PRODUCT POSITION)/infile(NSE_PRODUCT)>"
fi
