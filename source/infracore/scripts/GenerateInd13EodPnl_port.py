...
CONFIG_FILE = PCONFIG+"ind_pnl_config_for_backup_line.ini"
...
class SymbolInfo:
    def __init__(self, symbol, **options):
        ...
        if "shc" in options:
            ...
            self.shc = subprocess.check_output(["grep", self.exch_symbol, COMMISSION_FILE]).split()[1].strip().decode('utf-8')
            if self.shc == '1e+06':
...
def get_sftp_client(user, server_ip, server_port):
...
def LoadCorporateEventDetails(date):
    yyyy_ = str(date)[0:4]
    corp_events_path_ = corp_events_dir_ + yyyy_
    corp_event_files = [x for x in listdir(corp_events_path_)
                  if int(x.split('_')[-1]) == date]
    df = pd.DataFrame(columns=['old_size', 'new_size', 'adjustment_factor'])
    for file in corp_event_files:
        ...
        if 'NEW_MARKET_LOT' in details.index:
            df.loc[ticker, 'new_size'] = int(details.loc['NEW_MARKET_LOT'].iloc[0])
            df.loc[ticker, 'old_size'] = int(details.loc['OLD_MARKET_LOT'].iloc[0])
            df.loc[ticker, 'adjustment_factor'] = float(details.loc['ADJUSTMENT_FACTOR'].iloc[0])
        elif 'ADJUSTMENT_FACTOR' in details.index:
            #filling 1 (divide and multiply to the position should not affect) 
            #termination and reintroduction case
            df.loc[ticker, 'new_size'] = 1
            df.loc[ticker, 'old_size'] = 1
            df.loc[ticker, 'adjustment_factor'] = float(details.loc['ADJUSTMENT_FACTOR'].iloc[0])
    return df

def LoadOvnPosition(filename, corporate_events):
    """
        ...
        for line in eod_pos_file:
            ...
            if exch_sym_ in symbol_info_map:
                ...
                if subprocess.check_output(["grep", exch_sym_, COMMISSION_FILE]).split()[1].strip().decode(
                    ...
                    underlying = subprocess.check_output(["grep", exch_sym_, COMMISSION_FILE]
                        ).split()[1].strip().decode('utf-8').split('_')[1]
                    if underlying in corporate_events.index:
                        lots = pos_ / corporate_events.loc[underlying]['old_size']
                        pos_ = lots * corporate_events.loc[underlying]['new_size']
                        lpx_ = round(lpx_ * corporate_events.loc[underlying]['adjustment_factor'], 2)
...
def update_file_paths():
    ...
    prev_day = subprocess.check_output([GET_PREV_WORKING_DAY, EXCHANGE, str(date)]).strip().decode('utf-8')
...
def main():
    ...
    parser.add_argument('-D', '--dir', action="store", default="", type=str,
                        ...
                        required=True)
    parser.add_argument('-s', '--servers', nargs='+', help='<Required> List of servers containing the ors trades. '
                                                           'Available servers: ' + ', '.join(config.sections()),
                        required=True)
    ...
    '''Verify requested servers are present in config'''
    if not all(server in config._sections for server in args.servers):
        print("Invalid Server Arguments. Use -h flag to check supported servers")
        exit()

    update_file_paths()
    #cleanup()
    create_directories_if_not_exists()
    '''Cleanup the temporary directories before starting computation
    Get all trades from the requested servers and append to trade files'''

    '''Fetch all the trades from remote servers and move to tmp directory'''
    trade_files_ = list()
    for server in args.servers:
        server_ip = '202.189.245.205';
        server_port = config.get(server, 'Port')
        remote_file_path = "/spare/local/ORSlogs/"+config.get(server,'Exch')+"/"+config.get(server,
                                                                                           "Profile")+"/trades."+str(
            date)
        local_file_path = TMP_TRADE_DIR+server+"_trades."+str(date)
        '''Copy the file from remote server'''
        sftp_client = get_sftp_client(INFRA_USER, server_ip, server_port)
        try:
            sftp_client.get(remote_file_path,local_file_path)
            trade_files_.append(local_file_path)
            sftp_client.close()
        except IOError:
            print("Tradefile not found. Filename: "+ remote_file_path +"\n")
    '''
...
#Add symbols for neat trades
    ...
    if exch_dir == "FO":
        corporate_events = LoadCorporateEventDetails(date);
    LoadOvnPosition(OVERNIGHT_FILE, corporate_events)
    for file in trade_files_:
        try:
            trade_file = open(file, "r")
            for line in trade_file:
