...
sub LoadInstructionFile 
{
	...
	{ # $#instruction_line_words_ >= 0
		...
		{ # $current_instruction_ is valid
			...
			{
				push ( @products_list_, $instruction_line_words_[0] );
				push ( @products_start_duration_, $instruction_line_words_[1] );
				push ( @products_end_duration_, $instruction_line_words_[2] );
				push ( @products_maxlosslimits_, $instruction_line_words_[3] );
				push ( @max_risk_limits, $instruction_line_words_[4] );
				push ( @min_risk_limits, $instruction_line_words_[5] );
				push ( @products_to_lookback_days_map_, $instruction_line_words_[6] );
			}
			when ( "CONSTRAINTS" )
			{
...
sub CollectData
{
	...
	for ( my $index_ = 0; $index_ <= $#products_list_; $index_ ++ )
	{
		...
		{ 
		my $exec_cmd_ = "$HOME_DIR/infracore_install/bin/get_pnl_for_shortcode_bytime_stats $products_list_[$index_] $dt 1 UTC_$products_start_duration_[$index_] UTC_$products_end_duration_[$index_] DETAILS";
		$norm_pnl = 0;
		$iter = $iter + 1;
		...
		{
			my $exec_cmd_2_ = "$HOME_DIR/infracore_install/bin/get_volume_on_day $products_list_[$index_] $dt $products_start_duration_[$index_] $products_end_duration_[$index_]";
			...
			my $exec_cmd_3_ = "$HOME_DIR/infracore_install/bin/get_avg_l1sz_on_day $products_list_[$index_] $dt $products_start_duration_[$index_] $products_end_duration_[$index_]";
...
sub ComputeOptimalRisk
{
	...
	my @min_var_lim_for_prod = @max_risk_limits;
	...
	for ( my $i = 0; $i <= $#products_list_; $i ++ )
	{
		...
		print $file2_handle "\n";
		print $file3_handle "<= ";
		print $file4_handle "$max_risk_limits[$i] ";
		...
		print $file2_handle "\n";
		print $file3_handle ">= ";
		print $file4_handle "$min_risk_limits[$i] ";
	}
	...
	{
		my $mail_body = "";
		$mail_body = $mail_body."prod\t avg_real_risk\t optimal_risk\t avg_pnl_per_unit_risk\t max_risk\t min_risk\t var_constraint\n";
        	for (my $i = 0; $i<=$#optimal_risk; $i++)
        	{
                my $temp_line = sprintf "$products_list_[$i]\t\t %.2f\t\t %.2f\t\t %.2f\t\t %.2f\t\t %.2f\t\t %.2f\n", $product_to_avg_risk_map_[$i], $optimal_risk[$i], $product_to_avg_normalized_pnl_map_[$i], $max_risk_limits[$i], $min_risk_limits[$i], $min_var_lim_for_prod[$i];
	...
	for ( my $index_ = 0; $index_ <= $#products_list_; $index_ ++ )
	{		
		my $exec_cmd_ = "$HOME_DIR/infracore_install/bin/get_pnl_for_shortcode_bytime_stats $products_list_[$index_] $result_yyyymmdd 1 UTC_$products_start_duration_[$index_] UTC_$products_end_duration_[$index_] DETAILS";
        print $main_log_file_handle_ "$exec_cmd_\n" ;
        my $cache_hit = 0 ;
		my $result_lines = "";
		for (my $i = 0; $i <= $#data_cache_lines; $i++)
	...
	print $main_result_file_handle_ "prod\t avg_real_risk\t optimal_risk\t avg_pnl_per_unit_risk\t max_risk\t min_risk\t var_constraint\n";
	for (my $i = 0; $i<=$#optimal_risk; $i++)
	{
		printf $main_result_file_handle_ "$products_list_[$i]\t\t %.2f\t %.2f\t %.2f\t %.2f\t %.2f\t %.2f\n", $product_to_avg_risk_map_[$i], $optimal_risk[$i], $product_to_avg_normalized_pnl_map_[$i], $max_risk_limits[$i], $min_risk_limits[$i], $min_var_lim_for_prod[$i];
	}
