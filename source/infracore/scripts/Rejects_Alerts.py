...
def SendSlackAlerts():
    for symbol in rejection_map_:
        line = hostname+' ALERT : %s Rejections for the symbol %s with reason %s' \
            % (rejection_map_[symbol][0], symbol, ','.join([':'.join([key,
               str(val)]) for (key, val) in rejection_map_[symbol][1].items()]))
        command = slack_exec + ' ' + channel + ' '+ mode + ' \'' + line + '\'';
        BashExec(command)
...
if not os.path.isfile(log_file):
    ...
    sys.exit(-1)
t_rejects_file = '/tmp/t_rejects_file_' + today
