...
stop_bseshmwriter() {
  ...
  sleep 5 ;
  echo "SHUTTING DOWN BSEShmWriter WRITER...." ;
  /home/pengine/prod/live_scripts/SmartDaemonController.sh BSEShmWriter STOP >/dev/null 2>&1
}

move_old_data() {
    mkdir -p  "/spare/local/MDSlogs/BSE_$date"
    if [ -z "$(ls -A ${converted_dest_dir})" ];
    then
        mv $converted_dest_dir /spare/local/MDSlogs/BSE_$date
    fi
...
#some times mv fails if file are above 50k
  echo "mv ls | grep -v ORS_BSE | grep BSE_ | grep $date | grep A ${data_dir}"
  mv `ls | grep -v ORS_BSE | grep BSE_ | grep $date | grep A` ${data_dir}
  mv `ls | grep -v ORS_BSE | grep BSE_ | grep $date | grep M` ${data_dir}
  mv `ls | grep -v ORS_BSE | grep BSE_ | grep $date | grep N` ${data_dir}
  mv `ls | grep -v ORS_BSE | grep BSE_ | grep $date` ${data_dir}
  cd ${data_dir};
  count=`ls | wc -l`
...
#  mkdir -p ${data_alpha}
  ...
  if [ $1 == "CASH" ];then
    ...
    mv `ls | grep _CE_` ${data_to_convert_dir}
        ...
        mv `ls | grep -v ORS_BSE | grep BSE_ | grep A` ${data_to_convert_dir}
        mv `ls | grep -v ORS_BSE | grep BSE_ | grep M` ${data_to_convert_dir}
        mv `ls | grep -v ORS_BSE | grep BSE_ | grep N` ${data_to_convert_dir}
  ...
  mv `ls | egrep "BSE_[A-J a-j]."` ${data_alpha}
  cd ${data_alpha}
  count=`ls  | wc -l`;
  echo "STARTING $data_type DATA CONVESRIONS, COUNT : ${count}";
  /home/pengine/prod/live_scripts/convert_bse_generic_to_dotex.sh $date ${data_alpha} ${converted_dest_dir}
...
#  [ -f ${running_status} ] && print_msg_and_exit "CANNOT START ANOTHER INSTANCE FILE  EXISTS : ${running_status}" ;
  ...
  data_dir="/spare/local/MDSlogs/GENERIC/ALLDATA_${date}_BSE/"
  data_to_convert_dir="${data_dir}/DATA_TO_CONVERT/"
  data_alpha="${data_dir}/ALPHA_DATA"
  converted_dest_dir="/spare/local/MDSlogs/BSE/"
...
#  gzip BSE_B*
  conv_count=`ls $dest_data_dir | wc -l` ;
  remote_count=`ssh $DEST_SERVER "ls $local_Dir | wc -l"` ;
#  ls | grep -v BSE_A | grep -v BSE_B | xargs gzip
