...
def get_lot_size(filename_, exp_num):
    print("filenameLOTsize: " + filename_)
...
def get_position_limits(row,base_num_spreads_factor,physical_factor,yesterday_ban_factor,max_num_spreads_diff,banned_products,yesterday_banned_products,physical_settlement_products, exp_num):
    if exp_num == 1 and "NIFTY" in row['Stock']:
      curr_exp_num = 0
    else:
      curr_exp_num = exp_num
    if row['Stock'] in banned_products:
        base_num_spreads = 0
        max_num_spreads = 0
    else:
        if row['Stock'] in physical_settlement_products:
            base_num_spreads =int( int(base_num_spreads_factor*row['NumSpread'])*physical_factor)
...
>>>>>>> e8ecc09556972755c6d619eb2fb7700d77751305
    ...
    pos_limit_string = prefix + "_FUT" + str(curr_exp_num + 1) + "_MAXLONGPOS = " + str(int(max_long_pos)) + "*ONE_LOT_SIZE" + "\n" + \
                       ...
                       prefix + "_FUT" + str(curr_exp_num) + "_MAXLONGPOS = " + str(math.ceil(int(max_short_exposure)*float(row['Lotsize'])/float(row['Lotsize0']))) +  "*ONE_LOT_SIZE"  + "\n" + \
                       prefix + "_FUT" + str(curr_exp_num) + "_MAXSHORTPOS = " + str(math.ceil(int(max_long_exposure)*float(row['Lotsize'])/float(row['Lotsize0']))) +  "*ONE_LOT_SIZE"  +"\n" + \
                       prefix + "_FUT" + str(curr_exp_num) + "_MAXLONGEXPOSURE = " + str(math.ceil(int(max_short_exposure)*float(row['Lotsize'])/float(row['Lotsize0'])) + 1) +  "*ONE_LOT_SIZE" + "\n" + \
                       prefix + "_FUT" + str(curr_exp_num) + "_MAXSHORTEXPOSURE = " + str(math.ceil(int(max_long_exposure)*float(row['Lotsize'])/float(row['Lotsize0'])) + 1) + "*ONE_LOT_SIZE"  + "\n"
    return pos_limit_string
def get_agg_position_limits(row,base_num_spreads_factor,physical_factor,aggressive_factor,max_num_spreads_diff,banned_products,physical_settlement_products):
    if row['Stock'] in banned_products:
        base_num_spreads = 0
        max_num_spreads = 0
    else:
    ...
    max_long_pos = max_long_pos*int(row['Lotsize'])
    max_short_pos = max_short_pos*int(row['Lotsize'])
    max_long_exposure = max_long_pos + 2*int(row['Lotsize'])
    max_short_exposure = max_short_pos + 2*int(row['Lotsize'])
    pos_limit_string = prefix + "_FUT1_MAXLONGPOS = " + str(int(max_long_pos)) + "\n" + \
                       prefix + "_FUT1_MAXSHORTPOS = " + str(int(max_short_pos)) + "\n" + \
                       prefix + "_FUT1_MAXLONGEXPOSURE = " + str(int(max_long_exposure)) + "\n" + \
                       prefix + "_FUT1_MAXSHORTEXPOSURE = " + str(int(max_short_exposure)) + "\n" + \
                       prefix + "_FUT0_MAXLONGPOS = " + str(int(max_short_exposure)) + "\n" + \
                       prefix + "_FUT0_MAXSHORTPOS = " + str(int(max_long_exposure)) + "\n" + \
                       prefix + "_FUT0_MAXLONGEXPOSURE = " + str(int(max_short_exposure) + int(row['Lotsize'])) + "\n" + \
                       prefix + "_FUT0_MAXSHORTEXPOSURE = " + str(int(max_long_exposure) + int(row['Lotsize'])) + "\n"
    return pos_limit_string
...
next_working_date_2 = subprocess.Popen(["/home/pengine/prod/live_execs/update_date" , date, "N" ,"W", "2"], stdout=subprocess.PIPE).communicate()[0]
next_working_date_2 = next_working_date_2.rstrip().decode("utf-8")
next_working_date_3 = subprocess.Popen(["/home/pengine/prod/live_execs/update_date" , date, "N" ,"W", "3"], stdout=subprocess.PIPE).communicate()[0]
next_working_date_3 = next_working_date_3.rstrip().decode("utf-8")
...
output_position_file_ = "/home/dvctrader/usarraf/PositionLimits." + date 
...
overnight_contract = 'FUT2' if ((date == expiry) or (next_working_date_ == expiry) or (next_working_date_2 == expiry)) else 'FUT1'
exp_num = 0 if ((expiry_date != next_working_date_) and (expiry_date != next_working_date_2) and (expiry_date != next_working_date_3)) else 1
print('Date: {0} Next_date: {1} 2_next_date: {2} expiry: {5} overnight_contract: {3} exp_num: {4}'.format(date, next_working_date_, next_working_date_2, overnight_contract, exp_num, expiry))
...
list_pos = [item for x in pos_df.reset_index().apply(get_position_limits, args=(base_num_spreads_factor_,physical_factor_,yesterday_ban_factor_,max_num_spreads_diff_,banned_products,yesterday_banned_products,physical_settlement_products, exp_num),axis=1) for item in (x.strip().split('\n'))]
...
risk_check_ = []
risk_check_.insert(0, {'Field': 'GROSS_EXPOSURE_LIMIT', 'Sign': '=', 'Value': '65'})
risk_check_.insert(0, {'Field': 'TOTAL_PORTFOLIO_STOPLOSS', 'Sign': '=', 'Value': '250000'})
