...
do
  for strat_id in `cat $script | grep -v "#" | grep CONFIG | awk '{print $6}'| sort | uniq`; do
	...
	echo "Channels Added: $added_channel	Getting Data: $rec_channel "
 	...
 	echo "Strat_id:$strat_id FUT0:$fut0_count FUT1:$fut1_count FUT2:$fut2_count Channels_Added:$added_channel Getting_Data:$rec_channel" >> $strat_mail_file
