...
my %exchange_to_margin_utilized_ = ();
...
foreach $shortcode_ (sort keys %shortcode_to_pnl_) {
    if ($shortcode_to_exchange_{$shortcode_} eq "BMF") {
        ...
        $shortcode_to_open_position_margin_requirment_ { $shortcode_ } = 0 ;
    ...
    $exchange_to_margin_utilized_{$shortcode_to_exchange_{$shortcode_}} += $shortcode_to_open_position_margin_requirment_ { $shortcode_ } ;
}
...
foreach $exchange_ (sort keys %exchange_to_pnl_) {
    ...
    if( $exchange_to_margin_utilized_{ $exchange_ } > 0 ) {

      `echo "Date : "$expected_date_ " Exchange : "$exchange_ " MARGIN UTILIZED : "$exchange_to_margin_utilized_{ $exchange_ } >> $MARGIN_TRACKER_FILE ` ;
