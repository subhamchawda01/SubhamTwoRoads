#!/bin/bash
LOCKFILE=$HOME/locks/sertsdc.lock
if [ ! -e $LOCKFILE ] ; then
touch $LOCKFILE;

FILE="$HOME/trades/ALL_RTSDC_TRADES";
...
#    > $HOME/trades/bmfdc_trades.$YYYYMMDD;
    ...
    scp -q dvcinfra@172.18.244.107:/spare/local/ORSlogs/RTSDC/FORTSDC/trades.$YYYYMMDD $HOME/trades/fortsdc_trades.$YYYYMMDD
    cat $HOME/trades/fortsdc_trades.$YYYYMMDD >> $FILE; 

    > $HOME/trades/fortsdc_trades.$YYYYMMDD;
