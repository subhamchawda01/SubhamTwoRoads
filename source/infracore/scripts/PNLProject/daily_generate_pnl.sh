...
echo -e "stratfiles=> ${stratfiles[@]}\n"
 ...
 do
   # mkdir -p
	echo "path:-  $DestPath$server/$yyyy/$mm/$dd"
    [ -d "$DestPath$server/$yyyy/$mm/$dd" ] || continue 
    cd $DestPath$server"/"$yyyy"/"$mm"/"$dd
    echo -e "$PWD\n"
    echo -e "for $server\n"
    echo "$path$server"/"$yyyy"/"$mm"/"$dd"/dailyStratPNL."$yyyy$mm$dd"
    > $path$server"/"$yyyy"/"$mm"/"$dd"/dailyStratPNL."$yyyy$mm$dd;
    for stratID in `ls -I '*.*'`;
    do
	...
	#compute Yearly
		...
		done < $path$server"/"$yyyy"/"$mnth"/TotalMonthlyStratPNL."$yyyy$mnth
	    done
