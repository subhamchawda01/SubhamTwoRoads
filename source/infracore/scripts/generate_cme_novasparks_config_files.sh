...
python ~/LiveExec/scripts/CME-MDP3-SecDef_Parsing.py $base_dir/

#Filter only the securities for which we are actually using data
for secid in `awk '{printf "%s:%s ",$4,$1;}' /spare/local/files/CMEMDP/cme-ref.txt`; do sec_pair=`echo $secid | tr ':' ' '`; grep "$sec_pair" $base_dir/cme_instrument_dict_raw.txt ; done | sort | uniq > $base_dir/cme_instrument_dict.txt
