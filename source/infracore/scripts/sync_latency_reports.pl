...
my $local_archive_location_ = "/home/dvcinfra/logs/latencylogs";
...
for (my $location_ = 0; $location_ <= $#other_location_list_; $location_++) {
    ...
    my $mkdir_cmd_ = `taskset -c 20,21,22,23 ssh dvcinfra\@10.23.199.51 'mkdir -p $t_remote_file_location_'`;
    my $rsync_cmd_ = `taskset -c 20,21,22,23 rsync -avz --quiet $t_local_file_location_ dvcinfra\@10.23.199.51:$t_remote_file_location_`;
