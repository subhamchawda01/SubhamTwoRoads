...
echo "Getting Link... "
notebook_link=`ssh dvcinfra@10.23.5.67 ssh dvctrader@54.90.155.232 "grep localhost /home/dvctrader/Jupyter_NoteBooks/jupyter_notebook_daemon_logs.txt | grep -v NotebookApp" | awk '{print $1}'`
...
#notebook_link=`ssh dvctrader@54.90.155.232 "grep localhost /tmp/logj | grep -v NotebookApp" | awk '{print $1}'`
#PORT=`echo $notebook_link | awk -F'/' '{print $3}' | awk -F: '{print $2}'`
...
#  ssh  -X -L 8080:localhost:8080 dvcinfra@10.23.5.67 ssh -X -L 8080:localhost:8080 dvctrader@54.90.155.232


  ssh -f -X -L ${PORT}:localhost:${PORT} -N dvcinfra@10.23.5.67
