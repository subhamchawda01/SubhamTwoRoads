...
{
  HFSAT::CpucycleProfiler& cpucylcle_profiler_ = HFSAT::CpucycleProfiler::GetUniqueInstance(1);
  cpucylcle_profiler_.SetTag(1, "Ping/Send");
  cpucylcle_profiler_.SetTag(2, "Pong/Receive");
  int c;
  ...
  ZF_TRY(zf_deinit());
  std::cout << std::endl << HFSAT::CpucycleProfiler::GetUniqueInstance(9).GetCpucycleSummaryString() << std::endl;
  return 0;
}
