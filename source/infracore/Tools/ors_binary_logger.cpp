  ...
  void processMsgRecvd() {
    while (true) {
      ...
      if (num_bytes >= (int)sizeof(HFSAT::GenericORSReplyStructLiveProShm)) {
        ...
        // Ignore margin message from begin getting logged
        if(generic_ors_reply_struct_.orr_type_ == HFSAT::kORRType_Margin) {
          std::cout << "Margin Breach Exchange Message Ignoring " << std::endl;
          std::cout << "Current Margin value " <<  generic_ors_reply_struct_.int_price_<< std::endl;
          continue;
        } 
