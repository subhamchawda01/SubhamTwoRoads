...
class PNLStatsComputer {
 ...
 public:
  ...
  int ComputePNLForDay(std::string filename_to_read, int yyyymmdd_, unsigned int start_time_, unsigned int end_time_) {
    unsigned int this_day_volume_ = PreComputeTotalVolume(filename_to_read, start_time_, end_time_);
    if (this_day_volume_ <= 0) {
      return -1;
    }
    ...
    while (true) {
      ...
      if (filter_exec_after_cxld_.find(reply_struct.server_assigned_order_sequence_) != filter_exec_after_cxld_.end())
        continue;  // executions after cancel
      ...
      if (filter_imatch_trades_saos_.find(reply_struct.server_assigned_order_sequence_) !=
        ...
        if (filter_imatch_trades_saos_[reply_struct.server_assigned_order_sequence_] == this_time_)
          continue;  // internal execution SAOS && TIME match
      }
      ...
      if (reply_struct.orr_type_ == HFSAT::kORRType_Exec) {
        ...
        if (start_pos_ == -1) {
          start_pos_ = reply_struct.global_position_ -
                       ((reply_struct.buysell_ == HFSAT::kTradeTypeBuy) ? size_exec_ : -size_exec_);
        }
        end_pos_ = reply_struct.global_position_;
        (void)end_pos_;
        ...
        if (first_trade_price_ == -1) {
          first_trade_price_ = reply_struct.price_;
        }
    ...
    //    unreal_pnl_ += ( position_ ) * last_trade_price_ * number_to_dollars_  - abs( position_ ) *
    //    this_day_commission_ ;

    unreal_pnl_ +=
        -1 * (start_pos_ * first_trade_price_ - (start_pos_ + position_) * last_trade_price_) * number_to_dollars_ -
        abs(position_) * this_day_commission_;
  ...
  void GeneratePNLStats() {
    ...
    for (int days_counter_ = 0, max_attempt_counter_ = 0;
      ...
      if (ComputePNLForDay(filename_to_read, start_date_, unixtime_start_, unixtime_end_) == -1) {
        days_counter_--;
      }
