...
//  std::string logfilename = "/spare/local/MDSlogs/bseshm_writer_dbg_snapshot_zocket" + std::string("_") +
  ...
  HFSAT::BSEMD::BSETBTRawMDHandler& ds = HFSAT::BSEMD::BSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher, p_cmd_control_live_source_, mode, "BSE", true);
  global_ds = &ds;
