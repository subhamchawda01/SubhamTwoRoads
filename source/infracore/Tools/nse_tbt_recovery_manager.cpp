...
// =====================================================================================
//
//       Filename:  nse_tbt_recovery_manager.cpp
//
//    Description:
//
//        Version:  1.0
...
class BaseMDHandlerInterface {
 public:
  virtual bool ProcessRequest(char *in_buffer, int32_t length, char *out_buffer) = 0;
  virtual ~BaseMDHandlerInterface() {}
};

template <class T>
class MDHandlerInterface : public BaseMDHandlerInterface {
 private:
  T *raw_md_handler_ptr_;

 public:
  MDHandlerInterface(T *t_raw_md_handler_ptr) : raw_md_handler_ptr_(t_raw_md_handler_ptr) {}

  bool ProcessRequest(char *in_buffer, int32_t length, char *out_buffer) {
#if DEBUG_REC_MGR
    std::cout << " MDHandlerInterface::ProcessRequest " << length << std::endl;
#endif

    return raw_md_handler_ptr_->ProcessRequest(in_buffer, length, out_buffer);
  }
};

class RecoveryInterface : public HFSAT::Utils::TCPServerSocketListener {
 private:
  HFSAT::Utils::TCPServerManager &tcp_server_manager_;
  std::map<std::string, BaseMDHandlerInterface *> exch_src_to_handler_interface_;
  HFSAT::DebugLogger &dbglogger_;

 public:
  RecoveryInterface(HFSAT::Utils::TCPServerManager &tcp_server_manager, HFSAT::DebugLogger &dbglogger)
  ...
  void AddExchangeSource(std::string exchange_src, BaseMDHandlerInterface *md_handler_interface_ptr_) {
    exch_src_to_handler_interface_[exchange_src] = md_handler_interface_ptr_;
  }
  ...
  void ParseMetaData(char *&in_buffer, std::string &exchange_name) {
    ...
    dbglogger_ << "Received recovery request for exchange source : " << int_exchange_src << " , <" << exchange_name
               << ">" << '\n';
    dbglogger_.DumpCurrentBuffer();

    in_buffer += RECOVERY_REQUEST_EXCHANGE_SRC_LENGTH;

    // we can parse dummy flags here in future
    in_buffer += RECOVERY_REQUEST_DUMMY_FLAGS_LENGTH;
    return;
  }

  void OnClientRequest(int32_t client_fd, char *in_buffer, uint32_t const &length) {
#if DEBUG_REC_MGR
    std::cout << "RecoveryInterface::OnClientRequest: received length: " << length
              << " exch symbol: " << *(in_buffer + 20) << std::endl;
#endif
    if (length != RECOVERY_REQUEST_MESSAGE_LENGTH) return;
    ...
    if (false == exch_src_to_handler_interface_[exchange_name]->ProcessRequest(in_buffer, length, out_buffer)) {
      tcp_server_manager_.RespondFailureToClient(client_fd);
      return;
    }
...
int main(int argc, char *argv[]) {
  ...
  t_temp_oss << "/spare/local/MDSlogs/new_tbt_recovery_manager_" << HFSAT::DateTime::GetCurrentIsoDateLocalAsString()
  ...
  dbglogger << "STARTING RECOVERY_MANAGER TCP SERVER AT PORT: " << client_recovery_port << "\n";

  RecoveryInterface recovery_interface(tcp_server_manager, dbglogger);
  ...
  for (auto exch_ : exchange_sources_to_listen) {
    ...
    if (exch_ == "NSE" || "NSE_FO" == exch_ || "NSE_EQ" == exch_ || "NSE_CD" == exch_) {
      HFSAT::NSEMD::NSETBTRawMDHandler *nse_tbt_raw_handler = &(HFSAT::NSEMD::NSETBTRawMDHandler::GetUniqueInstance(
          dbglogger, simple_live_dispatcher, nullptr, HFSAT::kRecoveryHost, exch_));

      MDHandlerInterface<HFSAT::NSEMD::NSETBTRawMDHandler> *nse_tbt_raw_md_handler_interface_ptr =
          new MDHandlerInterface<HFSAT::NSEMD::NSETBTRawMDHandler>(nse_tbt_raw_handler);
