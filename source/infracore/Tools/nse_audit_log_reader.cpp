...
#define ERROR_FILE_PATH "/home/pengine/prod/live_configs/error_code_description"
...
std::string segment;
std::map<int,std::string> error_code_description;


void ReadFile(std::string filename) {
  std::string description_,segment_;
  int error_code_;
  std::ifstream file_read;

  file_read.open(filename);
  if(!file_read.is_open()) {
    std::cout << "file not present: " << ERROR_FILE_PATH << endl;
  }
  else {
    std::cout << ERROR_FILE_PATH << endl; 
    while(!file_read.eof()){
     file_read >> segment_;
     file_read >> error_code_;
     file_read >> description_;
     if(segment.find(segment_) != string::npos) {
       error_code_description[error_code_] = description_;
     }
    }
  }
  file_read.close();
      ...
      case ORDER_MOD_IN_TR: {
        if (segment == "NSE_EQ") {
          ...
          std::cout << "ORDER_MOD_IN_TR: "
                    ...
                    << " entry_date: " << entry_date << " modified_date: " << modified_date << " "
                    << inst_desc.ToString() << " nnf: " << std::setprecision(16) << nnf << std::endl;
        } else {
      ...
      case ORDER_CANCEL_IN_TR: {
        if (segment == "NSE_EQ") {
          ...
          std::cout << "ORDER_CANCEL_IN_TR: "
                    << "packet_sequence_number: " << packet_seq_no << " user_id: " << user_id
                    << " buy_sell: " << buy_sell << " vol: " << vol << " price: " << price << " saos: " << saos
                    << " order_no: " << order_no << " entry_date: " << entry_date << " modified_date: " << modified_date
                    << " " << inst_desc.ToString() << " nnf: " << std::setprecision(16) << nnf << std::endl;
        } else {
          reader2.read(nse_msg_ptr, NSE_CHANGE_REQUEST_LENGTH);
          HFSAT::NSE::InstrumentDesc inst_desc;
  ...
  while (true) {
    ...
    switch (processed_response_header->transaction_code) {
      case SIGN_ON_REQUEST_OUT: {
        ...
        std::cout << "error_code: " << error_code << " " << "error_description: " << error_code_description[error_code] << " "
                  << processed_logon_response_->ToString() << std::endl;
      ...
      case GR_REQUEST_OUT: {
        HFSAT::NSE::ProcessedGRResponse* processed_gr_response_ =
            nse_msgs_handler_->gr_response_->ProcessGRResponse(msg_ptr);
        std::cout << " GR LOGIN : " << std::endl;
        std::cout << processed_gr_response_->ToString() << std::endl;
      } break;

      case BoxLogin_REQUEST_OUT: {
        HFSAT::NSE::ProcessedBoxLoginResponse* processed_boxlogin_response_ =
            nse_msgs_handler_->boxlogin_response_->ProcessBoxLoginResponse(msg_ptr);
        std::cout << " BOX LOGIN : " << std::endl;
        std::cout << processed_boxlogin_response_->ToString() << std::endl;
      } break;
      case BOX_SIGN_OFF: {
        HFSAT::NSE::ProcessedBoxSignOffResponse* processed_boxlogout_response_ =
            nse_msgs_handler_->boxsignoff_response_->ProcessBoxSignOffResponse(msg_ptr);
      ...
      case ORDER_CONFIRMATION: {
        ...
        std::cout << "error_code: " << error_code << " " 
                  << "error_description: " << error_code_description[processed_order_response_->error_code] << " "
                  << processed_order_response_->ToString() << std::endl;
        msg_ptr += NSE_RESPONSE_MESSAGE_HEADER_LENGTH;
      } break;
      ...
      case ORDER_CANCEL_REJECT: {
        ...
        std::cout << "error_code: " << error_code << " " 
                  << "error_description: " << error_code_description[error_code] << " "
                  << processed_order_response_non_tr_->ToString() << std::endl;
      } break;
      ...
      case ORDER_ERROR: {
        ...
        std::cout << " Reason : " << processed_response_header->error_code << " "
                  << "error_description: " << error_code_description[processed_response_header->error_code] << "\n" << std::endl;
        processed_order_response_non_tr_ = nse_msgs_handler_->order_response_non_tr_.ProcessOrderResponse(msg_ptr);
      ...
      case TRADE_CONFIRMATION_TR: {
        ...
        std::cout << "error_code: " << error_code << " "
                  << "error_description: " << error_code_description[error_code] << " " 
                  << ((HFSAT::NSE::ProcessedTradeConfirmationResponse*)(processed_trade_response_))->ToString() << std::endl;

        msg_ptr += NSE_RESPONSE_MESSAGE_HEADER_LENGTH;
      } break;
      case BATCH_ORDER_CANCELLATION_TR: {
        std::cout << "Batch Order Cancellation : Reason : " << processed_response_header->error_code << "\n" << std::endl;
      } break;
      ...
      case SYSTEM_INFORMATION_OUT: {
        ...
        std::cout << "error_code: " << error_code << " " << "error_description: " << error_code_description[error_code] << " "
                  << processed_system_information_response_->ToString() << std::endl;
      ...
      case CTRL_MSG_TO_TRADER: {
        std::cout << "CTRL_MSG_TO_TRADER: ";
        HFSAT::NSE::ProcessedControlMsgResponse* processed_control_msg_response_ = 
            nse_msgs_handler_->control_msg_response_->ProcessControlMsgResponse(msg_ptr);
        std::cout << "error_code: " << error_code << " " << processed_control_msg_response_->ToString()
                  << std::endl;
      } break;

      case KILL_SWITCH_RESPONSE: {
        std::cout << "KILL SWITCH RESPONSE ";
      } break;
