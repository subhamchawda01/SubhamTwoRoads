...
sub MakeStratVecFromListMatchBase
{
    ...
    {
	open STRAT_LIST_FILEHANDLE, "< $strat_list_filename_ " or die "Could not open $strat_list_filename_\n";
	while ( my $thisline_ = <STRAT_LIST_FILEHANDLE> ) 
	{
		...
		if ( ( -f $sfile_ ) &&
		   ( ( $basepx_pxtype_ eq "ALL" ) || ( GetBasepxStratFirstModel ( $sfile_ ) eq $basepx_pxtype_ ) ) )
		 {
		     push ( @files_, $sfile_ );
		 }
	    }
