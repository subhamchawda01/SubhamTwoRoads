# \file GenPerlLib/make_strat_vec_from_dir_in_tp_excluding_sets.pl
#
...
sub MakeStratVecFromDirInTpExcludingSets
{ # expects an argument like ~/modelling/strats/ZN_0
  my ($top_directory_, $time_period_, @exclude_tp_dirs_) = @_;
...
#	      print STDERR "checking $dir_item_\n";
	      ...
	      { 
#		  print STDERR "admit              $dir_item_\n".join ( "\n", MakeStratVecFromDir ( "$top_directory_/$dir_item_", $time_period_, @exclude_tp_dirs_ ) ) ;
		  push @ret_abs_file_paths_, MakeStratVecFromDir ( "$top_directory_/$dir_item_", $time_period_, @exclude_tp_dirs_ ) ; 
	      }
