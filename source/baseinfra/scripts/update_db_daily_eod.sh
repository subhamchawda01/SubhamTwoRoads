...
if [[ $HOSTNAME == "sdv-ny4-srv222.dvcap.local" ]]; then
    db_generate_exec="/home/dvcinfra/raghu/DB_UPDATE/cvquant_install/baseinfra/bin/db_nse_update_for_day_Whole_Book_faster"
    db_bhav_exec="/home/dvcinfra/raghu/DB_UPDATE/cvquant_install/baseinfra/bin/db_bhavcopy_techincalIndicator_update_for_day"
fi
...
MKT_DATA_DIR="/NAS1/data/NSELoggedData/"

if [[ $YYYYMMDD -lt 20180201 ]]; then
          MKT_DATA_DIR="/run/media/dvcinfra/Hardrive1/data/NSELoggedData/";
elif [[ $YYYYMMDD -lt 20190101 ]]; then
          MKT_DATA_DIR="/run/media/dvcinfra/Hardrive2/NSELoggedData/";
elif [[ $YYYYMMDD -lt 20200101 ]];then
          MKT_DATA_DIR= "/run/media/dvcinfra/NSE_MTBT_2018_and_2019/data/NSELoggedData/";
elif [[ $YYYYMMDD -lt 20200511 ]]; then
          MKT_DATA_DIR="/run/media/dvcinfra/NSE_MTBT_2020_01_05/data/NSELoggedData/";
elif [[ $YYYYMMDD -lt 20201201 ]];then
          MKT_DATA_DIR="/run/media/dvcinfra/NSE_MTBT_2020_05_11/data/NSELoggedData/";
elif [[ $YYYYMMDD -lt 20210501 ]];then
        MKT_DATA_DIR="/run/media/dvcinfra/NSE_MTBT_2020_12_2021_01_04/data/NSELoggedData/";
elif [[ $YYYYMMDD -lt 20210801 ]];then
        MKT_DATA_DIR="/run/media/dvcinfra/NSE_MTBT_2021_05_07/data/NSELoggedData/";
elif [[ $YYYYMMDD -lt 20211101 ]];then
        MKT_DATA_DIR="/run/media/dvcinfra/NSE_MTBT_2021_08_10/data/NSELoggedData/";
fi
echo "MKT DATA DIR: $MKT_DATA_DIR"
MKT_DATA_DIR="$MKT_DATA_DIR/NSE/$YYYY/$MM/$DD/"
...
for product in `ls $MKT_DATA_DIR | grep _FUT_  | grep -v FINNIFTY | cut -d'_' -f2 | sort | uniq`; do
