...
std::unordered_map<int,pair<double,double>> sec_id_price_range_map;
...
class BookManager : public HFSAT::Thread,
 ...
 public:
  BookManager(HFSAT::DebugLogger& dbglogger, HFSAT::Watch& watch, vector<std::string> const& _shortcode_vec_, std::vector<int> & _saci_vec_,
              HFSAT::Utils::ClientLoggingSegmentInitializer* _client_logging_segment_initializer_,int yyyymmdd)
      : dbglogger_(dbglogger),
        ...
        bse_refdata_loader_(HFSAT::Utils::BSERefDataLoader::GetUniqueInstance(yyyymmdd)) {
  ...
  int  GetTokenFromShortcode(char const* exchange_symbol_,char &segment){

    std::cout << "Segment is " << segment << std::endl;


    if('F' == segment){
        for (auto& itr : bse_refdata_loader_.GetBSERefData(segment)) {
          ...
          std::string exchange_symbol =
              HFSAT::BSESecurityDefinitions::ConvertDataSourceNametoExchSymbol(internal_symbol_str.str());

          if(exchange_symbol == exchange_symbol_){
            return itr.first;
          }
        }
    }else if('E' == segment){
  ...
  void thread_main() {
    ...
    std::map<int32_t, BSE_UDP_MDS::BSERefData> bse_ref_data = bse_refdata_loader_.GetBSERefData('F');
    ...
    bse_ref_data = bse_refdata_loader_.GetBSERefData('F');

    char segment = 'F';
    if (shortcode_vec_[0].find("BSE_BSX_") != std::string::npos || 
        shortcode_vec_[0].find("BSE_BKX_") != std::string::npos) {
      bse_ref_data = bse_refdata_loader_.GetBSERefData('F');
    }else{
      bse_ref_data = bse_refdata_loader_.GetBSERefData('E');
      segment = 'E';
    }
    ...
    for(uint32_t shc_indx_ = 0; shc_indx_ < shortcode_vec_.size(); shc_indx_++){
      ...
      int token_ = GetTokenFromShortcode(exchange_symbol,segment);
      std::cout << "Token : " << token_ << std::endl;

      BSE_UDP_MDS::BSERefData ref_info = bse_ref_data[token_];

      std::cout << "For Shortcode " << shortcode_vec_[shc_indx_] << std::endl;
      std::cout << ref_info.ToString() << std::endl;
      std::cout << "Lower Limit : " << ref_info.lower_price_range << " Upper Limit : " << ref_info.upper_price_range << std::endl;

      sec_id_price_range_map[sec_id] = pair<int,int>(ref_info.lower_price_range,ref_info.upper_price_range);
...
void SendOrder(int rnd_sec_id_, BookManager& book_manager, HFSAT::BaseTrader* tr, const char* inst,
  ...
  try {
    ...
    // bord.price_ = book_manager.GetPrice(rnd_sec_id_); // Sending orders at lower & upper circuit prices

    std::cout << "Sending new order for sec id " << rnd_sec_id_ << std::endl;
    if(bord.buysell_ == HFSAT::kTradeTypeBuy){
        bord.price_ = sec_id_price_range_map[rnd_sec_id_].second;
        std::cout << "Sending buy order with upper limit price " <<  bord.price_ << std::endl;
    }else{
        bord.price_ = sec_id_price_range_map[rnd_sec_id_].first;
        std::cout << "Sending sell order with lower limit price " <<  bord.price_ << std::endl;
    }
...
void ModifyOrder(int rnd_sec_id_, BookManager& book_manager, HFSAT::BaseTrader* tr, HFSAT::FastPriceConvertor* fast_px_converter_,
  ...
  try {
    ...
    std::cout << "Sending modify order for sec id " << rnd_sec_id_ << std::endl;
    if(bord.buysell_ == HFSAT::kTradeTypeBuy){
        bord.price_ = sec_id_price_range_map[rnd_sec_id_].second;
        std::cout << "Sending buy order with upper limit price " <<  bord.price_ << std::endl;
    }else{
...
int main(int argc, char** argv) {
  ...
  BookManager book_manager(dbglogger_, *watch_, shortcode_vec_, saci_vec_, &client_logging_segment_initializer,yyyymmdd);
  book_manager.run();
