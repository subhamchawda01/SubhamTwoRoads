/**
   \file MDSMessages/combined_mds_messages_tmx_obf_of_processor.hpp
...
class CombinedMDSMessagesTMXOBFOFProcessor {
 protected:
 ...
 public:
  CombinedMDSMessagesTMXOBFOFProcessor(DebugLogger& _dbglogger_, SecurityNameIndexer& _sec_name_indexer_)
  ...
  ~CombinedMDSMessagesTMXOBFOFProcessor() {}

  inline void SetPriceLevelGlobalListener(OrderFeedGlobalListener* p_new_listener_) {
  ...
  bool RecoverLiveOrders(int security_id, const char* exchange_symbol, int32_t current_seq_num) {
    ...
    int32_t int_exchange_src = HFSAT::exchange_name_str_to_int_mapping["TMX_OBF"];
    ...
    for (uint32_t event_counter = 0; event_counter < data_length / sizeof(TMX_OBF_MDS::TMXRecoveryOrderInfo);
         event_counter++) {
      max_seq_number = std::max(max_seq_number, ((TMX_OBF_MDS::TMXRecoveryOrderInfo*)(msg_ptr))->msg_seq_number);
      msg_ptr += sizeof(TMX_OBF_MDS::TMXRecoveryOrderInfo);
    }
    ...
    DBGLOG_CLASS_FUNC_LINE_INFO << "RECOVERY SUCCESSFUL..., TOTAL PACKETS RECEIVED : "
                                << data_length / sizeof(TMX_OBF_MDS::TMXRecoveryOrderInfo)
    ...
    for (uint32_t event_counter = 0; event_counter < data_length / sizeof(TMX_OBF_MDS::TMXRecoveryOrderInfo);
         event_counter++) {
      TMX_OBF_MDS::TMXRecoveryOrderInfo* event = (TMX_OBF_MDS::TMXRecoveryOrderInfo*)(msg_ptr);
  ...
  inline void ProcessTMXOBFOFEvent(TMX_OBF_MDS::TMXCommonStruct* next_event) {
    ...
    if (-1 == security_id_to_recovery_complete_[security_id]) {
      ...
      DBGLOG_CLASS_FUNC_LINE_INFO << "SECURITY : " << security_id << " EXCHANGE SYMBOL : " << next_event->getContract()
                                  << " IS GOING FOR RECOVERY NOW... CURRENT SEQUENCE : " << next_event->msg_seq_number_
      ...
      if (RecoverLiveOrders(security_id, next_event->getContract(), next_event->msg_seq_number_)) {
        gettimeofday(&current_time, NULL);
    ...
    //      DBGLOG_DUMP;
    if (security_id_to_recovery_complete_[security_id] >= (int32_t)next_event->msg_seq_number_) {
      return;
    }
    ...
    switch (next_event->msg_type) {
      ...
      case TMX_OBF_MDS::TMXMsgType::kTMXAdd: {
        order_feed_global_listener_->OnOrderAdd(security_id, next_event->data.add.order_id, next_event->data.add.side,
                                                ...
                                                next_event->data.add.priority, next_event->intermediate_);
      } break;

      // Order Replaced
      case TMX_OBF_MDS::TMXMsgType::kTMXModify: {
        order_feed_global_listener_->OnOrderReplace(security_id, next_event->data.modify.old_order_id,
                                                    next_event->data.modify.side, next_event->data.modify.price,
                                                    next_event->data.modify.size, next_event->data.modify.order_id,
      ...
      case TMX_OBF_MDS::TMXMsgType::kTMXDelete: {
        order_feed_global_listener_->OnOrderDelete(security_id, next_event->data.del.order_id,
                                                   next_event->data.del.side, next_event->intermediate_);
      } break;

      // Order Exec
      case TMX_OBF_MDS::TMXMsgType::kTMXExec: {
        order_feed_global_listener_->OnOrderExec(security_id, next_event->data.exec.order_id,
                                                 next_event->data.exec.side, next_event->data.exec.price,
                                                 next_event->data.exec.size_exec, next_event->intermediate_);
      } break;
      ...
      case TMX_OBF_MDS::TMXMsgType::kTMXExecWithTrade: {
        ...
        order_feed_global_listener_->OnOrderExec(security_id, next_event->data.exec_info.order_id,
                                                 next_event->data.exec_info.side, next_event->data.exec_info.price,
                                                 next_event->data.exec_info.size_exec, next_event->intermediate_);
      } break;

      // Market Status
      case TMX_OBF_MDS::TMXMsgType::kTMXTradingStatus: {
        //      order_book_state_ = next_event.data.status.state; // TODO : Fix this
        // order_book_state_ = 'O';
      } break;
