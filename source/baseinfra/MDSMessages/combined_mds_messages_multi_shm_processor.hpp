...
/*
      runtime_profiler_.Start(HFSAT::ProfilerType::kCOMBINEDSHMWRITER, bse_generic_mds_message_.t2t_cshmw_start_time);
      runtime_profiler_.End(HFSAT::ProfilerType::kCOMBINEDSHMWRITER, bse_generic_mds_message_.t2t_cshw_end_time);
      runtime_profiler_.Start(HFSAT::ProfilerType::kTRADEINIT);

      if ((kMsgTypeControl == bse_generic_mds_message_.msg_type) && (control_processor_ != NULL)) {
        HFSAT::GenericControlRequestStruct ctrl_msg;
        memset((void *)&ctrl_msg, 0, sizeof(HFSAT::GenericControlRequestStruct));

        ctrl_msg.time_set_by_frontend_ = bse_generic_mds_message_.source_time;
        ctrl_msg.trader_id_ = bse_generic_mds_message_.msg_seq;
        ctrl_msg.control_message_.message_code_ =
            (HFSAT::ControlMessageCode_t)(bse_generic_mds_message_.data.control_msg.message_code_);
        ctrl_msg.control_message_.intval_1_ = bse_generic_mds_message_.data.control_msg.intval_1_;
        memcpy((void *)ctrl_msg.control_message_.strval_1_, (void *)bse_generic_mds_message_.data.control_msg.strval_1_,
               sizeof(bse_generic_mds_message_.data.control_msg.strval_1_));
        control_processor_->ProcessControlEvent(&ctrl_msg);

      } else {
*/
	//std::cout<<"bse_generic_mds_message_.msg_type != kMsgTypeControl\n";
...
/*
      ...
      if (kMsgTypeControl == bse_generic_mds_message_.msg_type) {
        HFSAT::GenericControlRequestStruct ctrl_msg;
        memset((void *)&ctrl_msg, 0, sizeof(HFSAT::GenericControlRequestStruct));
...
*/
        //std::cout<<"generic_mds_message_.msg_type != kMsgTypeControl\n";
        bse_mds_processor_->ProcessBSEEvent(&bse_generic_mds_message_);
      }
    }
