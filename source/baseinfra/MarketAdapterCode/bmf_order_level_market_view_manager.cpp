/**
   \file MarketAdapterCode/bmf_order_level_market_view_manager.cpp
...
BMFOrderLevelMarketViewManager::BMFOrderLevelMarketViewManager(
    ...
    : BaseMarketViewManager(t_dbglogger_, t_watch_, t_sec_name_indexer_, t_security_market_view_map_),
      bmfmaporder_mempool_(100),
      security_id_to_bmf_omv_map_() {
  security_id_to_bmf_omv_map_.resize(security_market_view_map_.size());
...
void BMFOrderLevelMarketViewManager::SetTimeToSkipUntilFirstEvent(const ttime_t r_start_time_) {
  for (auto i = 0u; i < security_market_view_map_.size(); i++) {
...
void BMFOrderLevelMarketViewManager::OnOrderLevelNew(const unsigned int t_security_id_, uint64_t t_order_id_,
  ...
  if (t_level_added_ > MAX_BMF_ORDER_BOOK_DEPTH) return;
  ...
  if (dbglogger_.CheckLoggingLevel(BOOK_TEST)) {
    DBGLOG_TIME_CLASS_FUNC_LINE << "( secid= " << t_security_id_ << " orderid= " << t_order_id_
                                ...
                                << " px= " << t_price_ << " sz= " << t_new_size_ << " ordc= " << t_new_ordercount_
  ...
  BMFOrderMarketView &this_omv_ = security_id_to_bmf_omv_map_[t_security_id_];
  ...
  BMFMapOrder *p_bmf_map_order_ = bmfmaporder_mempool_.Alloc();

  if (!p_bmf_map_order_) {
    p_bmf_map_order_ = (BMFMapOrder *)malloc(sizeof(BMFMapOrder));
  }

  p_bmf_map_order_->set(t_order_id_, t_int_price_, t_price_, t_new_size_, t_buysell_);
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      if (level_to_add_ > (int)this_smv_.market_update_info_.bidlevels_.size() ||
        ...
        std::vector<BMFMapOrder *>::iterator _itr_ = this_omv_.bid_order_depth_book_.begin();
        ...
        MarketUpdateInfoLevelStruct new_level_(0, t_int_price_, t_price_, t_new_size_ + total_bid_size_,
                                               t_new_ordercount_ + 1 + total_bid_order_count_, watch_.tv());
        ...
        if (level_to_add_ == 1) {
          ...
          if (!t_is_intermediate_message_) {
            ...
            // TODO {} it never comes here for bmf since all messages are
            // non-intermediate
            this_smv_.l1_changed_since_last_ = true;
          }
          ...
          if (!t_is_intermediate_message_) {
            ...
            if (this_smv_.pl_change_listeners_present_) {
              this_smv_.NotifyOnPLChangeListeners(
                  t_security_id_, this_smv_.market_update_info_, kTradeTypeBuy, level_to_add_ - 1,
                  this_level_->limit_int_price_, this_level_->limit_int_price_level_,
                  this_level_->limit_size_ - t_new_size_, this_level_->limit_size_, this_level_->limit_ordercount_ - 1,
    ...
    case kTradeTypeSell: {
      ...
      if (level_to_add_ > (int)this_smv_.market_update_info_.asklevels_.size() ||
        ...
        std::vector<BMFMapOrder *>::iterator _itr_ = this_omv_.ask_order_depth_book_.begin();
        ...
        MarketUpdateInfoLevelStruct new_level_(0, t_int_price_, t_price_, t_new_size_ + total_ask_size_,
                                               t_new_ordercount_ + 1 + total_ask_order_count_, watch_.tv());
        ...
        if (level_to_add_ == 1) {
          ...
          if (!t_is_intermediate_message_) {
            ...
            if (this_smv_.pl_change_listeners_present_) {
              this_smv_.NotifyOnPLChangeListeners(
                  t_security_id_, this_smv_.market_update_info_, kTradeTypeSell, level_to_add_ - 1,
                  this_level_->limit_int_price_, this_level_->limit_int_price_level_,
                  this_level_->limit_size_ - t_new_size_, this_level_->limit_size_, this_level_->limit_ordercount_ - 1,
                  this_level_->limit_ordercount_, t_is_intermediate_message_, 'C');
            }
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      this_omv_.bid_order_depth_book_.insert(_itr_, p_bmf_map_order_);
    } break;
    ...
    case kTradeTypeSell: {
      ...
      this_omv_.ask_order_depth_book_.insert(_itr_, p_bmf_map_order_);
    } break;
...
void BMFOrderLevelMarketViewManager::OnOrderLevelDelete(const unsigned int t_security_id_, uint64_t t_order_id_,
                                                        const TradeType_t t_buysell_, const int t_level_removed_,
                                                        const double t_price_, const bool t_is_intermediate_message_,
                                                        const bool t_is_delete_on_change_) {
  if (t_level_removed_ > MAX_BMF_ORDER_BOOK_DEPTH) return;
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      int t_int_price_ = kInvalidIntPrice;
      ...
      /*
        ...
        if( (*_itr_) && ( !strcmp ( (*_itr_) -> order_id_, t_order_id_ ) ) ){
        ...
        dbglogger_ << " Our Level : " << bid_level_to_be_removed_ << " Exch Level : " << t_level_removed_ << " Bid Size
        : " << this_omv_.bid_order_depth_book_.size() << "\n";
        dbglogger_.DumpCurrentBuffer ( );
        }

        if( !found_bid_order_id_ ) bid_level_to_be_removed_ = t_level_removed_ ;  //should not really get here
      ...
      */
      ...
      if (bid_level_to_be_removed_ >= (int)this_omv_.bid_order_depth_book_.size()) {
        dbglogger_ << " Level Doesn't exist, bid book size : " << this_omv_.bid_order_depth_book_.size()
                   << " level removed : " << bid_level_to_be_removed_ << "\n";
        dbglogger_.DumpCurrentBuffer();
        return;
      }
      ...
      if (!this_omv_.bid_order_depth_book_[bid_level_to_be_removed_ - 1]) {
        ...
        // than max depth
        dbglogger_ << " Level Doesn't Exist " << t_level_removed_
                   << " Bid Book Size : " << this_omv_.bid_order_depth_book_.size() << "\n";
        return;
      }
      ...
      BMFMapOrder *p_bmf_map_order_ = this_omv_.bid_order_depth_book_[bid_level_to_be_removed_ - 1];

      if (!p_bmf_map_order_) {
        return;
      }

      if (p_bmf_map_order_->order_id_ != t_order_id_) {
        return;
      }
      ...
      if (level_removed_ == (int)this_smv_.market_update_info_.bidlevels_.size()) {
        ...
        if (_itr_ != this_omv_.bid_order_depth_book_.end()) {
          bmfmaporder_mempool_.DeAlloc((*_itr_));
      ...
      if (this_smv_.market_update_info_.bidlevels_[level_removed_ - 1].limit_size_ <= p_bmf_map_order_->size_ ||
        ...
        this_level_->limit_size_ -= p_bmf_map_order_->size_;
        --(this_level_->limit_ordercount_);
        ...
        if (level_removed_ == 1) {
          ...
          if (!t_is_intermediate_message_) {
            ...
            if (this_smv_.pl_change_listeners_present_) {
              this_smv_.NotifyOnPLChangeListeners(
                  t_security_id_, this_smv_.market_update_info_, kTradeTypeBuy, level_removed_ - 1,
                  this_level_->limit_int_price_, this_level_->limit_int_price_level_,
                  this_level_->limit_size_ + p_bmf_map_order_->size_, this_level_->limit_size_,
                  this_level_->limit_ordercount_ + 1, this_level_->limit_ordercount_, t_is_intermediate_message_, 'C');
            }
    ...
    case kTradeTypeSell: {
      ...
      */
      ...
      if (ask_level_to_removed_ >= (int)this_omv_.ask_order_depth_book_.size()) {
        dbglogger_ << " Level Doesn't exist, ask book size : " << this_omv_.ask_order_depth_book_.size()
                   << " level removed : " << ask_level_to_removed_ << "\n";
        dbglogger_.DumpCurrentBuffer();
        return;
      }
      ...
      if (!this_omv_.ask_order_depth_book_[ask_level_to_removed_ - 1]) {
        ...
        // than max depth
        dbglogger_ << " Level Doesn't Exist " << t_level_removed_
                   << " Bid Book Size : " << this_omv_.ask_order_depth_book_.size() << "\n";
        return;
      }
      ...
      BMFMapOrder *p_bmf_map_order_ = this_omv_.ask_order_depth_book_[ask_level_to_removed_ - 1];
      ...
      // Find the price of the order to be deleted, so we can look it up in the smv.
      t_int_price_ = p_bmf_map_order_->int_price_;
      ...
      // 1. If there is only one ordercount left, and a new delete message come we should be removing the whole
      // price level 2. If the the size to be removed is equal/more than the current size remove the entire level
      if (this_smv_.market_update_info_.asklevels_[level_removed_ - 1].limit_size_ <= p_bmf_map_order_->size_ ||
        ...
        if (level_removed_ == 1) {
          ...
          if (!t_is_intermediate_message_) {
            ...
            if (this_smv_.pl_change_listeners_present_) {
              this_smv_.NotifyOnPLChangeListeners(
                  t_security_id_, this_smv_.market_update_info_, kTradeTypeSell, level_removed_ - 1,
                  this_level_->limit_int_price_, this_level_->limit_int_price_level_,
                  this_level_->limit_size_ + p_bmf_map_order_->size_, this_level_->limit_size_,
                  this_level_->limit_ordercount_ + 1, this_level_->limit_ordercount_, t_is_intermediate_message_, 'C');
            }
...
void BMFOrderLevelMarketViewManager::OnOrderLevelChange(const unsigned int t_security_id_, uint64_t t_order_id_,
  ...
  BMFMapOrder *p_bmf_map_order_ = NULL;

  uint64_t old_order_id_ = 0;
  double t_old_price_ = kInvalidPrice;

  switch (t_buysell_) {
    case kTradeTypeBuy: {
      if (t_level_changed_ < 1 || t_level_changed_ > (int)this_omv_.bid_order_depth_book_.size()) {
        break;
      }

      p_bmf_map_order_ = this_omv_.bid_order_depth_book_[t_level_changed_ - 1];
    } break;
    case kTradeTypeSell: {
      if (t_level_changed_ < 1 || t_level_changed_ > (int)this_omv_.ask_order_depth_book_.size()) {
        break;
      }

      p_bmf_map_order_ = this_omv_.ask_order_depth_book_[t_level_changed_ - 1];
    } break;

    default: { } break; }

  if (p_bmf_map_order_ != NULL) {
    // We need to save these quantities, because OnOrderLevelDelete will delete p_bmf_map_order_.
    old_order_id_ = p_bmf_map_order_->order_id_;
    t_old_price_ = p_bmf_map_order_->price_double_;
    ...
    OnOrderLevelDelete(t_security_id_, old_order_id_, t_buysell_, t_level_changed_, t_old_price_, true, true);
    OnOrderLevelNew(t_security_id_, t_order_id_, t_buysell_, t_level_changed_, t_price_, t_new_size_, t_new_ordercount_,
                    t_is_intermediate_message_);
  }
}

void BMFOrderLevelMarketViewManager::OnOrderLevelSnapNew(const unsigned int t_security_id_, uint64_t t_order_id_,
                                                         const TradeType_t t_buysell_, const int t_level_added_,
                                                         const double t_price_, const int t_new_size_,
                                                         const int t_new_ordercount_,
                                                         const bool t_is_intermediate_message_) {
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      // create structure
      MarketUpdateInfoLevelStruct new_level_(0, t_int_price_, t_price_, t_new_size_, t_new_ordercount_ + 1,
                                             watch_.tv());
  ...
  std::vector<BMFMapOrder *>::iterator _itr_;
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      this_omv_.bid_order_depth_book_[t_level_added_ - 1] = p_bmf_map_order_;
    } break;
    ...
    case kTradeTypeSell: {
      ...
      this_omv_.ask_order_depth_book_[t_level_added_ - 1] = p_bmf_map_order_;
    } break;
...
void BMFOrderLevelMarketViewManager::OnOrderLevelSnapDelete(const unsigned int t_security_id_, uint64_t t_order_id_,
                                                            const TradeType_t t_buysell_, const int t_level_removed_,
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      if (t_level_removed_ < 1 || t_level_removed_ > (int)this_omv_.bid_order_depth_book_.size()) {
        break;
      }

      BMFMapOrder *p_bmf_map_order_ = this_omv_.bid_order_depth_book_[t_level_removed_ - 1];
      ...
      // Find the price of the order to be deleted, so we can look it up in the smv.
      t_int_price_ = this_omv_.bid_order_depth_book_[t_level_removed_ - 1]->int_price_;
      ...
      if (this_smv_.market_update_info_.bidlevels_[level_removed_ - 1].limit_size_ == p_bmf_map_order_->size_) {
        ...
        if (level_removed_ == 1) {
          ...
          if (dbglogger_.CheckLoggingLevel(BOOK_TEST)) {
            DBGLOG_TIME_CLASS_FUNC << " RemoveTopBid level : " << level_removed_ << "\n"
                                   << this_smv_.ShowMarket() << DBGLOG_ENDL_FLUSH;
          }
        } else {
          this_smv_.RemoveNonTopBid(level_removed_);
          ...
          if (dbglogger_.CheckLoggingLevel(BOOK_TEST)) {
            DBGLOG_TIME_CLASS_FUNC << " RemoveNonTopBid level : " << level_removed_ << "\n"
                                   << this_smv_.ShowMarket() << DBGLOG_ENDL_FLUSH;
          }
        }
      } else {
        ...
        if (dbglogger_.CheckLoggingLevel(BOOK_TEST)) {
          DBGLOG_TIME_CLASS_FUNC << " Updating Bid level: " << level_removed_ << "\n"
                                 << this_smv_.ShowMarket() << DBGLOG_ENDL_FLUSH;
        }
      }
    } break;
    ...
    case kTradeTypeSell: {
      ...
      if (t_level_removed_ < 1 || t_level_removed_ > (int)this_omv_.ask_order_depth_book_.size()) {
        break;
      }

      BMFMapOrder *p_bmf_map_order_ = this_omv_.ask_order_depth_book_[t_level_removed_ - 1];
      ...
      if (this_smv_.market_update_info_.asklevels_[level_removed_ - 1].limit_size_ == p_bmf_map_order_->size_) {
        ...
        if (level_removed_ == 1) {
          ...
          if (dbglogger_.CheckLoggingLevel(BOOK_TEST)) {
            DBGLOG_TIME_CLASS_FUNC << " RemoveTopAsk level : " << level_removed_ << "\n"
                                   << this_smv_.ShowMarket() << DBGLOG_ENDL_FLUSH;
          }
        } else {
          this_smv_.RemoveNonTopAsk(level_removed_);
          ...
          if (dbglogger_.CheckLoggingLevel(BOOK_TEST)) {
            DBGLOG_TIME_CLASS_FUNC << " RemoveNonTopAsk level : " << level_removed_ << "\n"
                                   << this_smv_.ShowMarket() << DBGLOG_ENDL_FLUSH;
          }
        }
      } else {
        ...
        if (dbglogger_.CheckLoggingLevel(BOOK_TEST)) {
          DBGLOG_TIME_CLASS_FUNC << " Updating Ask level: " << level_removed_ << "\n"
                                 << this_smv_.ShowMarket() << DBGLOG_ENDL_FLUSH;
        }
      }
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      for (_itr_ = this_omv_.bid_order_depth_book_.begin(), level_skipped_ = 1; level_skipped_ < t_level_removed_;
           ++_itr_, ++level_skipped_)
        ;
      if (_itr_ != this_omv_.bid_order_depth_book_.end()) {
        bmfmaporder_mempool_.DeAlloc((*_itr_));
    ...
    case kTradeTypeSell: {
      ...
      for (_itr_ = this_omv_.ask_order_depth_book_.begin(), level_skipped_ = 1; level_skipped_ < t_level_removed_;
           ++_itr_, ++level_skipped_)
        ;
      if (_itr_ != this_omv_.ask_order_depth_book_.end()) {
        bmfmaporder_mempool_.DeAlloc((*_itr_));
  ...
  // dbglogger_ << "Order depth book:\n" << ShowOrderDepthBook ( this_omv_ ) << "\n\n";
  //dbglogger_ << this_smv_.ShowMarket() << DBGLOG_ENDL_FLUSH;
}

void BMFOrderLevelMarketViewManager::OnOrderLevelSnapChange(const unsigned int t_security_id_, uint64_t t_order_id_,
                                                            const TradeType_t t_buysell_, const int t_level_changed_,
                                                            const double t_price_, const int t_new_size_,
                                                            const int t_new_ordercount_,
                                                            const bool t_is_intermediate_message_) {
  ...
  // Here we make an assumption that the book was consistent before we received this CHANGE message.
  // This means no missed messages.
  // If this assumption is not true, the book will be worse after this update -- .
...
void BMFOrderLevelMarketViewManager::OnOrderLevelDeleteFrom(const unsigned int t_security_id_,
                                                            const TradeType_t t_buysell_,
                                                            const int t_min_level_deleted_,
                                                            const bool t_is_intermediate_message_) {
  BMFOrderMarketView &this_omv_ = security_id_to_bmf_omv_map_[t_security_id_];

  if (t_buysell_ == kTradeTypeBuy) {
    for (int i = 1; i <= t_min_level_deleted_ && (i < (int)this_omv_.bid_order_depth_book_.size()); i++) {
      if (this_omv_.bid_order_depth_book_[0] == NULL) {
        // can happen if we didnt receive new level information for some level but have received that of some higher
        // level
        if (dbglogger_.CheckLoggingLevel(BOOK_ERROR)) {
          DBGLOG_TIME_CLASS_FUNC_LINE << " No Entry Found for level : 0 in iteration: " << i
                                      << " Levels to delete: " << t_min_level_deleted_ << DBGLOG_ENDL_FLUSH;
        }

        this_omv_.bid_order_depth_book_.erase(this_omv_.bid_order_depth_book_.begin());
        continue;
      } else {
        OnOrderLevelDelete(t_security_id_, this_omv_.bid_order_depth_book_[0]->order_id_, t_buysell_, 1, 0, false);
      }
    }

  } else if (t_buysell_ == kTradeTypeSell) {
    for (int i = 1; i <= t_min_level_deleted_ && (i < (int)this_omv_.ask_order_depth_book_.size()); i++) {
      if (this_omv_.bid_order_depth_book_[0] == NULL) {
        if (dbglogger_.CheckLoggingLevel(BOOK_ERROR)) {
          DBGLOG_TIME_CLASS_FUNC_LINE << " No Entry Found for level : 0 in iteration: " << i
                                      << " Levels to delete: " << t_min_level_deleted_ << DBGLOG_ENDL_FLUSH;
        ...
        OnOrderLevelDelete(t_security_id_, this_omv_.ask_order_depth_book_[0]->order_id_, t_buysell_, 1, 0, false);
      }
    }
  }
}
void BMFOrderLevelMarketViewManager::OnOrderLevelDeleteThrough(const unsigned int t_security_id_,
                                                               const TradeType_t t_buysell_,
                                                               const int t_max_level_deleted_,
                                                               const bool t_is_intermediate_message_) {}
void BMFOrderLevelMarketViewManager::OnOrderLevelOverlay(const unsigned int t_security_id_,
                                                         const int t_level_overlayed_, const double bid_price_,
                                                         const int bid_size_, const int bid_ordercount_,
                                                         const double ask_price_, const int ask_size_,
                                                         const int ask_ordercount_,
                                                         const bool t_is_intermediate_message_) {}

void BMFOrderLevelMarketViewManager::OnTrade(const unsigned int t_security_id_, const double t_trade_price_,
...
std::string BMFOrderLevelMarketViewManager::ShowOrderDepthBook(const BMFOrderMarketView &this_omv_) const {
...
void BMFOrderLevelMarketViewManager::resetBook(int sec_id_) {
  if (sec_id_ < 0 || sec_id_ >= (int)security_id_to_bmf_omv_map_.size()) {
    // Probably garbage security or one that we are not interested in.
    return;
  }

  // Since the BMFMapOrder *s are allocated by the simple mempool, we need to free each of thenm individually.
  for (std::vector<BMFMapOrder *>::iterator _order_itr_ =
           security_id_to_bmf_omv_map_[sec_id_].bid_order_depth_book_.begin();
       _order_itr_ != security_id_to_bmf_omv_map_[sec_id_].bid_order_depth_book_.end(); ++_order_itr_) {
    if (*_order_itr_) {
      bmfmaporder_mempool_.DeAlloc((*_order_itr_));
    }
  }

  for (std::vector<BMFMapOrder *>::iterator _order_itr_ =
           security_id_to_bmf_omv_map_[sec_id_].ask_order_depth_book_.begin();
       _order_itr_ != security_id_to_bmf_omv_map_[sec_id_].ask_order_depth_book_.end(); ++_order_itr_) {
    if (*_order_itr_) {
      bmfmaporder_mempool_.DeAlloc((*_order_itr_));
    }
  }
  ...
  security_id_to_bmf_omv_map_[sec_id_].bid_order_depth_book_.clear();
  security_id_to_bmf_omv_map_[sec_id_].ask_order_depth_book_.clear();
