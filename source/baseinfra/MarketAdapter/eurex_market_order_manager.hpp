/**
   \file MarketAdapter/eurex_market_order_manager.hpp
...
class EurexMarketOrderManager : public BaseMarketOrderManager, public OrderLevelListenerSim {
 protected:
 public:
  EurexMarketOrderManager(DebugLogger &t_dbglogger_, const Watch &t_watch_, SecurityNameIndexer &t_sec_name_indexer_,
                          const std::vector<MarketOrdersView *> &t_market_orders_view_map_);

  ~EurexMarketOrderManager() {}
  ...
  void OnOrderModify(const unsigned int t_security_id_, const TradeType_t t_buysell_, const int t_level_,
    ...
    OnOrderModify(t_security_id_, t_buysell_, t_new_order_id_, t_new_price_, t_new_size_, t_old_order_id_, t_old_price_,
                  t_old_size_);
  }
  ...
  void OnOrderExec(const unsigned int t_security_id_, const TradeType_t t_buysell_, const int t_level_,
    ...
    OnPartialOrderExecution(t_security_id_, t_buysell_, t_order_id_, t_price_, t_size_executed_);
  }
  ...
  void OnTrade(const unsigned int t_security_id_, const double t_trade_price_, const int t_trade_size_,
    ...
    OnExecutionSummary(t_security_id_, t_buysell_, t_trade_price_, t_trade_size_);
  }
  ...
  void OnOrderModify(const uint32_t t_security_id_, const TradeType_t t_buysell_, const int64_t t_new_order_id_,
                     const double t_price_, const uint32_t t_size_, const int64_t t_prev_order_id_,
  ...
  void OnPartialOrderExecution(const uint32_t t_security_id_, const TradeType_t t_buysell_, const int64_t t_order_id_,
                               const double t_traded_price_, const uint32_t t_traded_size_);

  void OnFullOrderExecution(const uint32_t t_security_id_, const TradeType_t t_buysell_, const int64_t t_order_id_,
                            const double t_traded_price_, const uint32_t t_traded_size_);
