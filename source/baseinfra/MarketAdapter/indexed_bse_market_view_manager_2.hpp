/**
    \file MarketAdapter/indexed_bse_market_view_manager.hpp
...
class IndexedBSEMarketViewManager2 : public OrderGlobalListenerBSE,
                                     public BaseMarketViewManager,
                                     public OrderCanceledListener {
 public:
  IndexedBSEMarketViewManager2(DebugLogger& t_dbglogger_, const Watch& t_watch_,
  ...
  void OnOrderModify(const uint32_t t_security_id_, const TradeType_t t_buysell_, 
  ...
  void OnOrderDelete(const uint32_t t_security_id_, const TradeType_t t_buysell_, const uint64_t order_id_, 
  ...
  void OnTrade(const uint32_t t_security_id_, const TradeType_t t_buysell_, const uint64_t order_id_,
                                       const double t_traded_price_, const int32_t t_traded_size_);
  ...
  void OnIndexPrice(const unsigned int t_security_id_,const double t_spot_price_);

  void OnOpenInterestUpdate(const unsigned int t_security_id_,const double t_oi_price_);
 ...
 private:
  ...
  void SanitizeBook(const uint32_t t_security_id_, HFSAT::TradeType_t t_sanitize_side_, double t_sanitize_price_, bool delete_order_ = true);
 ...
 private:
  ...
  bool use_self_book_;
  std::vector<bool> is_auction_vec_;
  double market_order_auction_price_;
