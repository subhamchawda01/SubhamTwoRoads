...
enum JoinChannelReturn {
  ...
  INSTRUMENT_FILTER_NOT_AVAILABLE =
      3,  ///< \brief Instrument filter configuration was not accepted. Check the log for more information.
  CONFIGURATION_NOT_SUPORTED =
      4  ///< \brief Configuration not supported to joinChannel. Check the log for more information.
};
...
enum EventsType {
  ...
  BOOK_AND_MDENTRIES_BVMF_PUMA_1_6 =
      4,  ///< \brief  Returned when returnEventsConfiguration is set to BOOK_AND_MDENTRIES.
  BOOK_AND_MDENTRIES_BVMF_PUMA_2_0 =
      5,                ///< \brief  Returned when returnEventsConfiguration is set to BOOK_AND_MDENTRIES.
  ...
  CHANNEL_TIMEOUT = 13,    ///< \brief  Channel timeout. Channel has not received messages for the time configured on
                           ///STREAMS_TIMEOUT parameter in \a device.cfg file.
  ...
  TCP_RECOVERY_STARTED =
      16,  ///< \brief  API detected a missing message on the channel and will request it on TCP Recovery.
  TCP_RECOVERY_ENDED = 17,  ///< \brief  The TCP Recovery has finished the requisitions for the channel.
};

enum Error {
  MISSING_MESSAGE =
      0,  ///< \brief A missing message was detected on the channel and the API is unable to retrieve by TCP Recovery
  SECURITY_ID_NOT_FOUND = 1,   ///< \brief Message with SecurityID not found on the list (deprecated). This case is now
                               ///reported as warning on log
...
struct TcpRecoveryConfiguration_t {
  ...
  unsigned
      requisitionTimeout;  ///< \brief  Time to wait before sending a requisition (in milliseconds). Default is \e 20ms.
...
struct sumdfConfiguration_t {
  ...
  LogNameFormat siliconUMDF_logNameFormat;  ///< \brief Format of log name. Appends date or date and time to log base
                                            ///name set on \ref sumdfConfiguration_t.siliconUMDF_logFile.
...
struct MDEntriesAllowed_t {
  ...
  bool openingPrice;  ///< \brief Allows \b OpeningPrice (MDEntryType '4'). Check if your API version supports this
                      ///MDEntryType.
  bool imbalance;     ///< \brief Allows \b Imbalance (MDEntryType 'A'). Check if your API version supports this
                      ///MDEntryType.
  MDEntriesAllowed_t();
};
...
struct ChannelStatus_t {
  ...
  int synchronizationAttempts;  ///< \brief How many synchronization attempts it has tried (Number of snapshot loops
                                ///received)
