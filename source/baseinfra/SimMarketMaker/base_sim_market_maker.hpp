...
#include "infracore/BasicOrderRoutingServer/throttle_manager_sim.hpp"

#define DEFAULT_DELAY_SECONDS 0
#define DEFAULT_DELAY_MSEC 100000
#define DEFAULT_BARDATA_SLIPPAGE 1

#define DEBUG_THROTTLE_MANAGER 1
...
class BaseSimMarketMaker : public OrderNotFoundListener,
 ...
 protected:
  ...
  HFSAT::BroadcastManagerSim* bcast_manager_;
  ...
  std::unordered_map<double,int> bardata_price_size_;
  ...
  std::unordered_map<int, BaseSimOrder*> saos_to_order_map_;
  std::unordered_map<std::string, double> slippage_value_map_;
  char slippage_flag_;
  ...
  int ProcessBardataSendRequest(SimGenRequest& t_sim_genrequest_);

 private:
  HFSAT::ORS::ThrottleManagerSIM *p_throttler_;
  HFSAT::ORS::ThrottleManagerSIM *p_ioc_throttler_;
 ...
 public:
  ...
  BaseSimMarketMaker(DebugLogger& t_dbglogger_, Watch& t_watch_, SecurityMarketView& t_dep_market_view_,
                     MarketModel t_market_model_, HFSAT::SimTimeSeriesInfo& t_sim_time_series_info_, bool _are_we_using_bardata_ = false);
  ...
  bool peek_reject_for_time_throttle(int server_sequence, int no_of_orders_) {
    if(p_throttler_) return p_throttler_->peek_reject_for_time_throttle(no_of_orders_,watch_.tv());
    else {
      #if DEBUG_THROTTLE_MANAGER
        DBGLOG_CLASS_FUNC_LINE_ERROR << "The pointer is not set for throttle manager" << DBGLOG_ENDL_DUMP;
      #endif
  ...
  bool reject_for_time_throttle(int server_sequence) { 
    if(p_throttler_) return p_throttler_->reject_for_time_throttle(watch_.tv()); 
    else {
      #if DEBUG_THROTTLE_MANAGER
        static bool is_not_printed = 1;
        if(is_not_printed){
          DBGLOG_CLASS_FUNC_LINE_ERROR << "The pointer is not set for throttle manager" << DBGLOG_ENDL_DUMP;
          is_not_printed=0;
        }
  ...
  bool reject_for_time_throttle_ioc(int server_sequence) { 
    if(p_ioc_throttler_) return p_ioc_throttler_->reject_for_time_throttle(watch_.tv());
    else {
      #if DEBUG_THROTTLE_MANAGER
        static bool is_not_printed = 1;
        if(is_not_printed){
  ...
  void throttle_update(int new_throttle_val) { 
    if(!p_throttler_){
      DBGLOG_CLASS_FUNC_LINE_ERROR << "The pointer is not set for throttle manager" << DBGLOG_ENDL_DUMP;
      return;
    }
    p_throttler_->throttle_value_update(new_throttle_val); 
  } 

  void throttle_update_ioc(int new_throttle_val) { 
    if(!p_ioc_throttler_){
      DBGLOG_CLASS_FUNC_LINE_ERROR << "The pointer is not set for throttle manager" << DBGLOG_ENDL_DUMP;
      return;
    }
    p_ioc_throttler_->throttle_value_update(new_throttle_val); 
  ...
  ttime_t min_cycle_throttle_wait() { 
    ...
    return p_throttler_->min_cycle_throttle_wait(); 
  }

  ttime_t min_cycle_throttle_wait_ioc() { 
    if(!p_ioc_throttler_){
      DBGLOG_CLASS_FUNC_LINE_ERROR << "The pointer is not set for throttle manager" << DBGLOG_ENDL_DUMP;
      return ttime_t();
    }
    return p_ioc_throttler_->min_cycle_throttle_wait(); 
    
  } 

  void SetThrottleManagerLimitOrders(HFSAT::ORS::ThrottleManagerSIM *p_throttler){
    if(p_throttler_){
      DBGLOG_CLASS_FUNC_LINE_INFO << "The pointer is ALREADY set for throttle manager will overwrite it" << DBGLOG_ENDL_DUMP;
    }
    p_throttler_ = p_throttler;
  }
  void SetThrottleManagerIOCOrders(HFSAT::ORS::ThrottleManagerSIM *p_ioc_throttler){
    if(p_ioc_throttler_){
      DBGLOG_CLASS_FUNC_LINE_INFO << "The pointer is ALREADY set for throttle manager will overwrite it" << DBGLOG_ENDL_DUMP;
    }
    p_ioc_throttler_ = p_ioc_throttler;
  }
  ...
  virtual void ProcessBardataRequestQueue();
  ...
  virtual void setMargin( double margin_);
  ...
  virtual void BroadcastCancelRejection(const int saci, const BaseSimOrder* sim_order, CxlRejectReason_t reject_reason);
