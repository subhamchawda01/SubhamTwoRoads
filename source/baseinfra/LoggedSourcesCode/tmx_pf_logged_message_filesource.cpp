...
#include "baseinfra/LoggedSources/tmx_pf_logged_message_filesource.hpp"

namespace HFSAT {

TMXPFLoggedMessageFileSource::TMXPFLoggedMessageFileSource(
  ...
  if (HFSAT::HybridSecurityManager::IsHybridExch(std::string(t_exchange_symbol_))) {
    ...
    t_ose_filename_ = CommonLoggedMessageFileNamer::GetName(kExchSourceTMX, t_exchange_symbol_, t_preevent_YYYYMMDD_,
  ...
  DBGLOG_CLASS_FUNC << "For TMXPF symbol " << t_exchange_symbol_ << " date " << t_preevent_YYYYMMDD_
  ...
  if (kTLocMAX != trading_location_file_read_) {
    ...
    std::cerr << "For TMXPF symbol " << t_exchange_symbol_ << " date " << t_preevent_YYYYMMDD_ << " trading_location "
              << trading_location_file_read_ << " returned filename = " << t_ose_filename_ << std::endl;
  }
}

void TMXPFLoggedMessageFileSource::SeekToFirstEventAfter(const ttime_t r_start_time_, bool& rw_hasevents_) {
...
void TMXPFLoggedMessageFileSource::ComputeEarliestDataTimestamp(bool& rw_hasevents_) {
...
inline void TMXPFLoggedMessageFileSource::_ProcessThisMsg() {
  ...
  switch (next_event_.msg_) {
    case TMX_OBF_MDS::TMXPriceFeedmsgType::TMX_PF_DELTA: {
      ...
      switch (next_event_.data_.delta_.action_) {
        ...
        default: {
          std::cerr << "Unknown action in TMX_PF_DELTA: " << next_event_.data_.delta_.action_ << std::endl;
        } break;
      }
    } break;
    ...
    default: {
      std::cerr << "Unknown msg_type in TMX_OBF_PF: " << static_cast<int>(next_event_.msg_) << std::endl;
    } break;
  }
}

inline bool TMXPFLoggedMessageFileSource::_SetNextTimeStamp() {
  // read the next_event_
  // set next_event_timestamp_
  register size_t available_len_ = bulk_file_reader_.read(&next_event_, sizeof(TMX_OBF_MDS::TMXPFCommonStruct));
  if (available_len_ < sizeof(TMX_OBF_MDS::TMXPFCommonStruct)) {
...
void TMXPFLoggedMessageFileSource::ProcessAllEvents() {
  while (true) {
    _ProcessThisMsg();
...
void TMXPFLoggedMessageFileSource::ProcessEventsTill(const ttime_t _endtime_) {
...
void TMXPFLoggedMessageFileSource::SetTimeToSkipUntilFirstEvent(const ttime_t r_start_time_) {}

void TMXPFLoggedMessageFileSource::AddPriceLevelGlobalListener(PriceLevelGlobalListener* p_this_listener_) {
  if (p_this_listener_ != NULL) {
    VectorUtils::UniqueVectorAdd(price_level_global_listener_vec_, p_this_listener_);
  }
}

void TMXPFLoggedMessageFileSource::NotifyPriceLevelUpdate(void* ptr_to_price_level_update, int length_of_bytes) {
  for (auto listener_ : price_level_global_listener_vec_) {
    listener_->OnPriceLevelUpdate(ptr_to_price_level_update, length_of_bytes, HFSAT::MDS_MSG::TMX_OBF);
  }
}

void TMXPFLoggedMessageFileSource::NotifyPriceLevelNew(const unsigned int t_security_id_, const TradeType_t t_buysell_,
...
void TMXPFLoggedMessageFileSource::NotifyPriceLevelDelete(const unsigned int t_security_id_,
...
void TMXPFLoggedMessageFileSource::NotifyPriceLevelChange(const unsigned int t_security_id_,
...
void TMXPFLoggedMessageFileSource::NotifyTrade(const unsigned int t_security_id_, const double t_trade_price_,
