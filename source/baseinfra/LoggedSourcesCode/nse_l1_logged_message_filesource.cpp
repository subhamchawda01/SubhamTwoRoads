...
#include <algorithm>
#include <cstring>
#include "baseinfra/LoggedSources/nse_l1_logged_message_filesource.hpp"

namespace HFSAT {

NSEL1LoggedMessageFileSource::NSEL1LoggedMessageFileSource(
  ...
  strcpy(shortcode_, NSESecurityDefinitions::GetShortCodeFromExchangeSymbol(exchange_symbol_).c_str());
  std::string t_nse_filename_ =
      NSEL1LoggedMessageFileNamer::GetName(exchange_symbol_, t_preevent_YYYYMMDD_, trading_location_file_read_);
  is_spot_ = NSESecurityDefinitions::IsSpotIndex(shortcode_);
...
void NSEL1LoggedMessageFileSource::SeekToFirstEventAfter(const ttime_t r_start_time_, bool& rw_hasevents_) {
...
void NSEL1LoggedMessageFileSource::ComputeEarliestDataTimestamp(bool& _hasevents_) {
  if (bulk_file_reader_.is_open()) {
    // read the next_event_
    // set next_event_timestamp_
    size_t available_len_;
...
void NSEL1LoggedMessageFileSource::_ProcessThisMsg() {
  ...
  if (listener_) {
	...
	if (is_spot_)
        ...
        break;
      default:
        dbglogger_ << "NSEL1LoggedMessageFileSource::ProcessThisMsg : Error. Invalid Msge Type\n";
        break;
    }
  }
}

void NSEL1LoggedMessageFileSource::ProcessAllEvents() {
  while (true) {
    _ProcessThisMsg();
    size_t available_len_;
    do {
...
void NSEL1LoggedMessageFileSource::ProcessEventsTill(const ttime_t _endtime_) {
...
void NSEL1LoggedMessageFileSource::SetTimeToSkipUntilFirstEvent(const ttime_t start_time) {
  if (listener_) {
    listener_->SetTimeToSkipUntilFirstEvent(start_time);
  }
}

void NSEL1LoggedMessageFileSource::SetL1DataListener(L1DataListener* listener) { listener_ = listener; }
}
