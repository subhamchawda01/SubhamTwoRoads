/**
    \file MDSMessagesCode/krx_logged_message_filesource.cpp
...
#include "baseinfra/LoggedSources/krx_logged_message_filesource.hpp"

namespace HFSAT {

KRXLoggedMessageFileSource::KRXLoggedMessageFileSource(
    ...
    : ExternalDataListener(),
      ...
      /*krx_trade_time_manager_(
          HFSAT::TmxTradeTimeManager::GetUniqueInstance(t_sec_name_indexer_, t_preevent_YYYYMMDD_)),*/
  ...
  std::string m_krx_filename_ =
      KRXLoggedMessageFileNamer::GetName(t_exchange_symbol_, t_preevent_YYYYMMDD_, trading_location_file_read_);
  // std::cout << "KRXFileName: " << m_krx_filename_ << std::endl;
  ...
  if (kTLocMAX != trading_location_file_read_) {
    bulk_file_reader_.open(m_krx_filename_);
  } else {
    std::cerr << "For KRX symbol " << t_exchange_symbol_ << " date " << t_preevent_YYYYMMDD_ << " trading_location "
              << trading_location_file_read_ << " returned filename = " << m_krx_filename_ << std::endl;
  }
}

void KRXLoggedMessageFileSource::SeekToFirstEventAfter(const ttime_t r_start_time, bool& rw_hasevents_) {
  if (bulk_file_reader_.is_open()) {
    do {
      size_t available_len = bulk_file_reader_.read(&next_event_, sizeof(KRX_MDS::KRXCommonStruct));
      if (available_len < sizeof(KRX_MDS::KRXCommonStruct)) {
...
void KRXLoggedMessageFileSource::ComputeEarliestDataTimestamp(bool& _hasevents_) {
  if (bulk_file_reader_.is_open()) {
    size_t available_len_ = bulk_file_reader_.read(&next_event_, sizeof(KRX_MDS::KRXCommonStruct));
    if (available_len_ < sizeof(KRX_MDS::KRXCommonStruct)) {
      next_event_timestamp_ = ttime_t(time_t(0), 0);
      _hasevents_ = false;
    } else {
      next_event_timestamp_ = next_event_.time_;
...
void KRXLoggedMessageFileSource::_ProcessThisMsg() {
  ...
  switch (next_event_.msg_type) {
    case KRX_MDS::MsgType::KRX_QUOTE:
      break;
    case KRX_MDS::MsgType::KRX_TRADE: {
      if (next_event_.data.trade.side == 'B')
        NotifyTrade(security_id_, next_event_.data.trade.price, next_event_.data.trade.size, kTradeTypeBuy);
      else if (next_event_.data.trade.side == 'S')
        NotifyTrade(security_id_, next_event_.data.trade.price, next_event_.data.trade.size, kTradeTypeSell);
      else
        NotifyTrade(security_id_, next_event_.data.trade.price, next_event_.data.trade.size, kTradeTypeNoInfo);

    } break;
    case KRX_MDS::MsgType::KRX_BOOK: {
      FullBook* fullbook = (FullBook*)&(next_event_.data.book);
      NotifyFullBookChange(security_id_, fullbook);
    } break;
    default: {
      dbglogger_ << "Weird msgtype " << int(next_event_.msg_type) << " in " << __func__ << DBGLOG_ENDL_FLUSH;
      DBGLOG_DUMP;
      exit(-1);
    }
  }
...
void KRXLoggedMessageFileSource::ProcessAllEvents() {
  while (true) {
    _ProcessThisMsg();
...
void KRXLoggedMessageFileSource::ProcessEventsTill(const ttime_t _endtime_) {
...
void KRXLoggedMessageFileSource::AddFullBookGlobalListener(FullBookGlobalListener* p_this_listener_) {
  if (p_this_listener_ != NULL) {
    VectorUtils::UniqueVectorAdd(fullbook_listener_vec_, p_this_listener_);
  }
}

void KRXLoggedMessageFileSource::NotifyFullBookChange(const unsigned int t_security_id_, const FullBook* t_full_book_) {
...
void KRXLoggedMessageFileSource::NotifyTrade(const unsigned int t_security_id_, const double t_trade_price_,
