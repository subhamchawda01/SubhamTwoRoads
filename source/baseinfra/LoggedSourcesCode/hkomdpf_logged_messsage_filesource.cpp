/**
    ...
    \author: (c) Copyright Two Roads Technological Solutions Pvt Ltd 2011
     Address:
         ...
         +91 80 4060 0717
*/
#ifndef HKOMDPF_LOGGED_MESSSAGE_FILESOURCE_CPP_
#define HKOMDPF_LOGGED_MESSSAGE_FILESOURCE_CPP_
...
void HKOMDPFLoggedMessageFileSource::SeekToFirstEventAfter(const ttime_t r_start_time, bool& rw_hasevents_) {
...
void HKOMDPFLoggedMessageFileSource::ComputeEarliestDataTimestamp(bool& _hasevents_) {
  if (bulk_file_reader_.is_open()) {
...
void HKOMDPFLoggedMessageFileSource::ProcessAllEvents() {
  ...
  while (true) {
    ...
    price_level_global_listener_hkomd_->OnPriceLevelUpdate(&next_event_, sizeof(next_event_), HFSAT::MDS_MSG::HKOMDPF);

    switch (next_event_.msg_) {
      case HKOMD_MDS::HKOMD_PF_TRADE:
        // filter msgs which don't have type as BLANK
        {
          if (!IsNormalTradeTime(security_id_, next_event_timestamp_) && !is_hk_eq_) break;
...
void HKOMDPFLoggedMessageFileSource::ProcessEventsTill(const ttime_t _endtime_) {
  NotifyExternalDataListenerListener(security_id_);
...
#endif /* HKOMDPF_LOGGED_MESSSAGE_FILESOURCE_CPP_ */
