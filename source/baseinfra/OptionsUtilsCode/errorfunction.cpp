...
#include "baseinfra/OptionsUtils/errorfunction.hpp"
#include <float.h>
#include <cmath>
...
const double ErrorFunction::tiny = std::numeric_limits<double>::epsilon(),
  ...
  */
  ...
  if (ax < 0.84375) {                                             /* |x|<0.84375 */
    if (ax < 3.7252902984e-09) {                                  /* |x|<2**-28 */
      if (ax < DBL_MIN * 16) return 0.125 * (8.0 * x + efx8 * x); /*avoid underflow */
      return x + efx * x;
    }
    z = x * x;
