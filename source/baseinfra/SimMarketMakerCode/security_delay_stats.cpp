 ...
 */
SecurityDelayStats::SecurityDelayStats(Watch& watch, const unsigned security_id, TradingLocation_t trading_location,
                                       int _normal_delay_usecs) {
  dbglogger_ = new DebugLogger(10240, 1);
  ors_message_stats_ =
      new ORSMessageStats(*dbglogger_, watch, security_id, trading_location, MIN_DELAY_USECS, _normal_delay_usecs,
 ...
 * @return
 */
SecurityDelayStats& SecurityDelayStats::GetUniqueInstance(const unsigned security_id, Watch& watch,
  ...
  if (secid_tradingdate_to_delay_stats_map_.find(secid_tradingdate_key) ==
    ...
    secid_tradingdate_to_delay_stats_map_[secid_tradingdate_key] =
        new SecurityDelayStats(watch, security_id, trading_location, _normal_delay_usecs);
  }
