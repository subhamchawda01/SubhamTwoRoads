...
BaseSimMarketMaker::BaseSimMarketMaker(DebugLogger& dbglogger, Watch& watch, SecurityMarketView& dep_market_view,
                                       MarketModel market_model, HFSAT::SimTimeSeriesInfo& sim_time_series_info, bool _are_we_using_bardata_)
    : dbglogger_(dbglogger),
      watch_(watch),
      smv_(dep_market_view),
      bcast_manager_(BroadcastManagerSim::GetUniqueInstance(dbglogger,watch, 0)),
      ...
      are_we_using_bardata_(_are_we_using_bardata_),
      ...
      saos_to_order_map_(),
      slippage_value_map_(),
      p_throttler_(nullptr),
      p_ioc_throttler_(nullptr) {
  ...
  security_delay_stats_ = &SecurityDelayStats::GetUniqueInstance(
      security_id, watch_, TradingLocationUtils::GetTradingLocationExch(smv_.exch_source()), market_model.com_usecs_);
...
void BaseSimMarketMaker::LoadSimConfigFile() {
      ...
      if (tokens_.size() >= 3) {
        if (tokens_[0][0] == '#') {
          continue;
            ...
            if(!strncmp(tokens_[1], "EXCHSLIPPAGEFLAG", strlen("EXCHSLIPPAGEFLAG"))) {
              slippage_flag_ = *(tokens_[2]);
            } else {
...
void BaseSimMarketMaker::BroadcastRejection(const int saci, const BaseSimOrder* sim_order,
  ...
  bcast_manager_->BroadcastORSRejection(sim_order,reject_reason,0); // no need to pass saci as server_assigned_client_id_ exist in BasicSimORder
}

void BaseSimMarketMaker::BroadcastCancelRejection(const int saci, const BaseSimOrder* sim_order, CxlRejectReason_t reject_reason) {
  for (auto cxl_rejc_listener : order_canceled_listener_vec_) {
    cxl_rejc_listener->OrderCancelRejected(saci, sim_order->client_assigned_order_sequence(), sim_order->server_assigned_order_sequence(),
                                 dep_security_id_, sim_order->price(), sim_order->buysell(), sim_order->size_remaining(), reject_reason,
                                 0, 0, sim_order->int_price_, 0, watch_.tv());
  }
  bcast_manager_->BroadcastCancelRejection(sim_order,reject_reason,0); // no need to pass saci as server_assigned_client_id_ exist in BasicSimORder
}
...
void BaseSimMarketMaker::BroadcastSequenced(const int saci, const BaseSimOrder* sim_order) {
  ...
  bcast_manager_->BroadcastSequenced(sim_order);
}
...
void BaseSimMarketMaker::BroadcastConfirm(const int saci, const BaseSimOrder* sim_order) {
  ...
  bcast_manager_->BroadcastConfirm(sim_order);
}

void BaseSimMarketMaker::BroadcastCancelNotification(const int saci, const BaseSimOrder* sim_order) {
  saos_to_order_map_.erase(sim_order->server_assigned_order_sequence_);
  ...
  bcast_manager_->BroadcastCancelNotification(sim_order);
}
...
void BaseSimMarketMaker::BroadcastExecNotification(const int saci, const BaseSimOrder* sim_order) {
  ...
  if (dbglogger_.CheckLoggingLevel(SIM_ORDER_INFO))
    dbglogger_ << "BaseSimMarketMaker::BroadcastExecNotification: " << watch_.tv().tv_sec << "." << watch_.tv().tv_usec << DBGLOG_ENDL_FLUSH;

  if (sim_order->size_remaining_ == 0) {
    saos_to_order_map_.erase(sim_order->server_assigned_order_sequence_);
  }
  ...
  bcast_manager_->BroadcastExecNotification(sim_order);
}
...
void BaseSimMarketMaker::BroadcastOrderModifyRejectNotification(
    ...
    const int intprice, const int32_t server_assigned_message_sequence, const CxlReplaceRejectReason_t rejection_reason_) {
  for (auto cxl_replace_rejected_listener : order_conf_cxlreplace_rejected_listener_vec_) {
    cxl_replace_rejected_listener->OrderConfCxlReplaceRejected(
        ...
        buysell, size_remaining, client_position, global_position, intprice, rejection_reason_, 0, 0, watch_.tv());
  }
  SecurityNameIndexer& sec_name_indexer_ = SecurityNameIndexer::GetUniqueInstance();
  const char *sec_name = sec_name_indexer_.GetSecurityNameFromId(security_id);
  bcast_manager_->BroadcastORSCancelReplaceRejection(sec_name, server_assigned_client_id, client_assigned_order_sequence, buysell,
                                                    size_remaining, price, intprice, rejection_reason_);
}
...
void BaseSimMarketMaker::BroadcastOrderModifyNotification(const int _server_assigned_client_id_,
  ...
  bcast_manager_->BroadcastCxlReSequenced(p_sim_order_);
}
...
void BaseSimMarketMaker::addRequest(SimGenRequest& genrequest) {
  if (dbglogger_.CheckLoggingLevel(SIM_ORDER_INFO))
    dbglogger_ << "BaseSimMarketMaker::addRequest: " << genrequest.wakeup_time_.tv_sec << "." << genrequest.wakeup_time_.tv_usec << DBGLOG_ENDL_FLUSH;
...
void BaseSimMarketMaker::SendOrderExch(const int saci, const char* security_name, const TradeType_t buysell,
  ...
  if (dbglogger_.CheckLoggingLevel(SIM_ORDER_INFO))
    dbglogger_ << "BaseSimMarketMaker::SendOrderExch: " << security_name << " " << price << " " << int_price << " " << watch_.tv().tv_sec << "." << watch_.tv().tv_usec << DBGLOG_ENDL_FLUSH;
  ...
  //Check for throttle
  bool is_reserved = new_order->is_ioc_;
  bool is_reject_for_time_throttle = reject_for_time_throttle(new_order->server_assigned_order_sequence_);
  if (is_reject_for_time_throttle && is_reserved) {
    is_reject_for_time_throttle = reject_for_time_throttle_ioc(new_order->server_assigned_order_sequence_);
  }

  if (is_reject_for_time_throttle) {
    BroadcastRejection(saci,new_order, kORSRejectThrottleLimitReached);
    return;
  }
...
/*
  // Using bardata instead of market data
  if (are_we_using_bardata_) {
    new_order->Confirm();
    new_order->order_confirmation_time_ = watch_.tv();

    BroadcastConfirm(saci, new_order);

    double conf_price = new_order->price();
  
    // Adding slippage to price
    double exchange_slippage = 0;
    double config_slippage = DEFAULT_BARDATA_SLIPPAGE;
    std::string inst_type_str = "NONE";
    std::string exchange_sym = new_order->security_name_;
    ...
    if (slippage_flag_ == 'Y') {
      exchange_slippage = HFSAT::NSESecurityDefinitions::GetNSECommission(shortcode); 
    }
    ...
    } else if (slippage_value_map_.find("NSE") != slippage_value_map_.end()) {
      config_slippage = slippage_value_map_["NSE"];
    }
      
    new_order->price_ = conf_price + config_slippage - exchange_slippage;
    new_order->int_price_ = new_order->price_ * 100 ;

    new_order->ExecuteRemaining(); 

    new_order->queue_size_behind_ = 0;
    new_order->queue_size_ahead_ = new_order->size_remaining_;
    new_order->order_id_ = 0;

    BroadcastExecNotification(saci, new_order);
    return;
  }
*/
  ...
  if (are_we_using_bardata_)
    genrequest.wakeup_time_ = watch_.tv();
  else
  ...
  saos_to_order_map_[new_order->server_assigned_order_sequence_] = new_order;
...
void BaseSimMarketMaker::CancelOrderExch(const int saci, const int saos, const TradeType_t buysell,
                                         const int int_price) {
  // find the order by saci
  BaseSimOrder* sim_order = nullptr;
  auto sim_order_it = saos_to_order_map_.find(saos);
  if (sim_order_it != saos_to_order_map_.end()) {
    sim_order = sim_order_it->second;
  }

  //Checking throttle only for a valid cancel otherwise ignoring for the flow to remain same
  if (((sim_order != nullptr ) && (sim_order->server_assigned_client_id() == saci) && (sim_order->server_assigned_order_sequence() == saos))) {
    
    //Check for throttle
    bool is_reserved = sim_order->is_ioc_;
    bool is_reject_for_time_throttle = reject_for_time_throttle(saos);
    if (is_reject_for_time_throttle && is_reserved) {
      is_reject_for_time_throttle = reject_for_time_throttle_ioc(saos);
    }

    if (is_reject_for_time_throttle) {
      BroadcastCancelRejection(saci,sim_order, kCxlRejectReasonThrottle);
      return;
    }
  }
  ...
  if (it != saos_to_wakeup_map_.end()) {
    ...
    saos_to_wakeup_map_[saos] = genrequest.wakeup_time_;
  }
...
void BaseSimMarketMaker::CancelReplaceOrderExch(const int saci, const int saos, const TradeType_t buysell,
  ...
  //Checking throttle only for a valid cancelReplace otherwise ignoring for the flow to remain same
  if (((sim_order != nullptr ) && (sim_order->server_assigned_client_id() == saci) && (sim_order->server_assigned_order_sequence() == saos))){
    //Check for throttle
    bool is_reserved = sim_order->is_ioc_;
    bool is_reject_for_time_throttle = reject_for_time_throttle(saos);
    ...
    if (is_reject_for_time_throttle) {
      ...
      BroadcastOrderModifyRejectNotification(saci, sim_order->client_assigned_order_sequence(), saos, smv_.security_id(), _old_price_, sim_order->buysell(), _old_size_,
                                           client_position_map_[saci], global_position_, _old_int_price_, saos, kORSCxReRejectThrottleLimitReached);
      return;
    }
  }
...
int BaseSimMarketMaker::ProcessBardataSendRequest(SimGenRequest& sim_genrequest) {
  if (dbglogger_.CheckLoggingLevel(SIM_ORDER_INFO))
    dbglogger_ << "BaseSimMarketMaker::ProcessBardataRequest: " << watch_.tv().tv_sec << "." << watch_.tv().tv_usec << DBGLOG_ENDL_FLUSH;
  ...
  int size_executed = new_sim_order->size_remaining_;
  new_sim_order->queue_size_behind_ = 0;
  new_sim_order->queue_size_ahead_ = size_executed;
  new_sim_order->order_id_ = 0;
  new_sim_order->ExecuteRemaining();
  switch (new_sim_order->buysell_) {
    case kTradeTypeBuy: {
      client_position_map_[saci] += size_executed;
      global_position_to_send_map_[saci] += size_executed;
  ...
  BroadcastExecNotification(saci, new_sim_order);
...
int BaseSimMarketMaker::ProcessCancelRequest(SimGenRequest& sim_genrequest) {
  ...
  // find the order by saci
  BaseSimOrder* sim_order = NULL;
  auto it = saos_to_order_map_.find(saos);
  if (it != saos_to_order_map_.end()) {
    sim_order = it->second;
  }
  ...
  if (buysell == kTradeTypeBuy) { /* BidSide order */
    ...
    if ((!sim_genrequest.postponed_once_) && false) {  // if never postponed then see if it needs to be postponed
...
int BaseSimMarketMaker::ProcessCxlReplaceRequest(SimGenRequest& t_sim_genrequest_) {
  ...
  if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
    ...
    DBGLOG_TIME_CLASS_FUNC << "ProcCxlReplaceReq " << smv_.secname() << " " << GetTradeTypeChar(_buysell_)
                           ...
                           << smv_.bestbid_ordercount() << " " << smv_.bestbid_size() << " " << smv_.bestbid_price()
                           << " * " << smv_.bestask_price() << " " << smv_.bestask_size() << " "
                           << smv_.bestask_ordercount() << " ] " << (t_sim_genrequest_.postponed_once_ ? "YES" : "NO")
                           << DBGLOG_ENDL_FLUSH;
  }

  // find the order by saci
  BaseSimOrder* p_sim_order_ = NULL;
  auto it = saos_to_order_map_.find(saos_);
  if (it != saos_to_order_map_.end()) {
    p_sim_order_ = it->second;
  }
  ...
  if ((p_sim_order_ != NULL) &&
    ...
    if (_buysell_ == kTradeTypeBuy) {
      ...
      if ((!t_sim_genrequest_.postponed_once_) && false) {
      ...
      if (to_postpone_ == 0) {
        ...
        int caos = p_sim_order_->server_assigned_order_sequence();
        DBGLOG_TIME_CLASS_FUNC << "Postponed modification " << to_postpone_ << " of " << p_sim_order_->security_name()
                               << ' ' << GetTradeTypeChar(p_sim_order_->buysell()) << ' ' << p_sim_order_->price()
                               << ' ' << smv_.normal_spread_increments() << ' ' << p_sim_order_->size_remaining()
                               ...
                               << ']' << " CAOS: " << caos << " SAOS: " << saos_ << " SACI: " << saci_
                               << DBGLOG_ENDL_FLUSH;
      }
    } else {
      if ((!t_sim_genrequest_.postponed_once_) && false) {  // if never postponed then see if it needs to be postponed
    ...
    BroadcastOrderModifyRejectNotification(saci_, 0, saos_, smv_.security_id(), _price_, _buysell_, _new_size_,
                                           client_position_map_[saci_], global_position_, _int_price_, saos_, HFSAT::CxlReplaceRejectReason_t::kCxlReRejectOrderNotFound);
  }
...
void BaseSimMarketMaker::ProcessRequestQueue() {
  if (all_requests_busy_ || !smv_.is_ready_) {
    return;
  }
...
void BaseSimMarketMaker::ProcessBardataRequestQueue() {
  if (dbglogger_.CheckLoggingLevel(SIM_ORDER_INFO))
    dbglogger_ << "BaseSimMarketMaker::ProcessBardataRequestQueue(): " << watch_.tv().tv_sec << "." << watch_.tv().tv_usec << DBGLOG_ENDL_FLUSH;
  ...
  for (auto sim_request_iter_ = all_requests_.begin();
    ...
    //to_postpone_ = ProcessBardataRequest(genrequest);

    switch (genrequest.orq_request_type_) {
      case ORQ_SEND: {
        to_postpone_ = ProcessBardataSendRequest(genrequest);
        break;
      }
...
void BaseSimMarketMaker::setMargin( double margin_) {}
