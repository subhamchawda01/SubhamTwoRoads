/**
    \file LoggedSources/cfe_logged_message_filenamer.hpp
...
/// Returns the name of the data file where CSM_MDS has logged data
/// Typically it will be of the format /NAS1/data/CSMLoggedData/2014/04/28/VXX201406_20140428
class CFELoggedMessageFileNamer {
 public:
  static std::string GetName(const char* _exchange_symbol_, const unsigned int _preevent_YYYYMMDD_,
    ...
    static std::string logging_dir = "/NAS1/data/CSMLoggedData/";
    ...
    if (use_todays_data_) {
      ...
      return ((todays_logging_dir) + "CSM/" + std::string(_exchange_symbol_) + "_" + t_temp_oss_.str());
    }

    return HFSAT::LoggedMessageFileNamer::GetName(logging_dir, _exchange_symbol_, _preevent_YYYYMMDD_,
                                                  rw_trading_location_, kTLocCFE);
  }
};
}
