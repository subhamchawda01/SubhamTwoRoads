...
generate_product_html_files() {
	...
	IND17Stat_html_report_file="$IND17Stat_html_file_path/IND17Stat_report_.html"
	...
	do
		...
		total_pnl=`grep -w $product $IND17Stat_30days_data_file | sed 's/<td>/ /g; s/<\/td>/ /g;' | awk '{sum += $(NF-1)} END {print sum}'`
	        ...
	        average_vV=`echo "scale=4; $total_vV / $product_count" | bc`

	        echo "<tr><td><a href=product/$product.html style=color:blue>$product</a></td><td>$product_count</td><td>$average_vV</td><td>$total_vV</td><td>$average_pnl</td><td>$total_pnl</td></tr>" >> $IND17Stat_html_report_file
...
generate_html_files() {
	...
	do
        echo "${f1%=>} <tr><td>$today</td><td>$f5</td><td>$f9</td><td>$f12</td></tr>" >> $IND17Stat_30days_data_file
