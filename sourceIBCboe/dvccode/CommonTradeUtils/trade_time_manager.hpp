...
class TradeTimeManager {
 private:
  ...
  std::map<std::string , std::pair<int,int>> segment_to_start_end_time_;
 ...
 public:
  ...
  TradeTimeManager(const HFSAT::SecurityNameIndexer& sec_indexer, int YYYYMMDD) {
    ...
    filevec_.push_back((std::string("/spare/local/files/BSE/bse-trd-hours.txt")));
    exchange_to_break_time_file_map_["BSE"] = std::string("/spare/local/files/BSE/bse-trd-hours.txt");
    filename_to_id_map_["/spare/local/files/BSE/bse-trd-hours.txt"] = 40;
  ...
  void parseTimeFile(const HFSAT::SecurityNameIndexer& sec_indexer, const char* trade_time_filename_) {
    ...
    while (file.getline(line, sizeof(line))) {
      ...
      if (tokens_[0].find('#') != std::string::npos) {
        continue;
      }

      SetStartEndTimeForSegments(tokens_);
      // shc_ start_time_ end_time_ tz_ fbst_ fbet_ sbst_ sbet_ ....

      if (!(HFSAT::ExchangeSymbolManager::CheckIfContractSpecExists(tokens_[0])) && (tokens_[0].substr(0, 4) != "NSE_") && (tokens_[0].substr(0, 4) != "BSE_"))
        continue;
      ...
      } else if (tokens_[0].substr(0, 4) == "BSE_") {
        // Different handling for BSE, setting product class wise times
        char segment = BSE_FO_SEGMENT_MARKING;
        if (tokens_[0] == "BSE_FO") {
          segment = BSE_FO_SEGMENT_MARKING;
        } else if (tokens_[0] == "BSE_CD") {
          segment = BSE_CD_SEGMENT_MARKING;
        } else if (tokens_[0] == "BSE_EQ") {
          segment = BSE_EQ_SEGMENT_MARKING;
        }

        for (unsigned index = 0; index < sec_indexer.NumSecurityId(); index++) {
          std::string secname = sec_indexer.GetShortcodeFromId(index);
          if (BSESecurityDefinitions::GetSegmentTypeFromShortCode(secname) == segment) {
            tokens_[0] = secname.c_str();
            SetBreaksForSecID(index, tokens_);
            if(BSESecurityDefinitions::IsShortcodeAnIPOListingThisDay(secname)) {
              start_times[index] = 270*60;
            }
          }
        }
      } else {
  ...
  int GetStartTimeForSegment(std::string segment_) { return segment_to_start_end_time_[segment_].first; }

  int GetEndTimeForSegment(std::string segment_) { return segment_to_start_end_time_[segment_].second; }
  
  void SetStartEndTimeForSegments(const std::vector<std::string>& tokens_) {
      // Read information for this
      char this_token_[32];
      std::string segment_ = tokens_[0];
      ...
      segment_to_start_end_time_[segment_] = std::make_pair(st + offset,et + offset);
