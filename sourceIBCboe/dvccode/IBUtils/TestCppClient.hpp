...
public:
	...
	void setConnectOptions(const std::string&);
        void RequestMarketData(std::vector<Contract> &required_contracts,std::vector<std::string> &req_contract_sym);
		void RequestMarketDataSingleContract(Contract &req_contract, std::string &req_contract_sym);
		void cancelAllMktDataReq();
...
public:
    void AddIBDataListener(IBDataListener *ibl);
private:
    static std::unordered_set<std::string> isDataAlreadyReq;
	static std::vector<std::pair<Contract,std::string>> seqOfDataReq;
	static bool used_once;
