...
int main(int argc, char const* argv[]) {
  if (argc < 4) {
    std::cout << "USAGE : <exec> <date> <products_data_filenames_file> <output_dir>" << std::endl;
    exit(-1);
  }
  ...
  while (std::getline(if_stream_, line_)) {
    ...
    for (it_type iterator = hash_string_to_metrics_map.begin(); iterator != hash_string_to_metrics_map.end();
      ...
      for (it_typeX iteratorX = iterator->second.begin(); iteratorX != iterator->second.end(); iteratorX++) {
        ...
        time_t raw_time = iteratorX->first;
        ...
        std::string date_str(formatted_date);

        outstream_ << time << "\t" << iterator->first << "\t" << iteratorX->second.start_time << "\t"
                   << iteratorX->second.end_time << "\t" << date_str << "\t" <<iteratorX->second.open_price << "\t"
                   ...
                   << iteratorX->second.total_trade_qty_price / iteratorX->second.volume << "\n";
      }
    }
