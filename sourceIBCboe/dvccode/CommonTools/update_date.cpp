  ...
  if ('P' == search_type) {
    if ('W' == day_type) {
      for (int32_t counter = 0; counter < num_of_times; counter++) {
        input_date = HFSAT::HolidayManagerUtils::GetPrevBusinessDayForExchange("NSE",input_date, true);
    ...
    if ('W' == day_type) {
      for (int32_t counter = 0; counter < num_of_times; counter++) {
        input_date = HFSAT::HolidayManagerUtils::GetNextBusinessDayForExchange("NSE",input_date, true);
