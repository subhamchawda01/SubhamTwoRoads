...
int main(int argc, char **argv) {
  ...
  } else if (strncmp(shortcode_.c_str(), "CBOE_", 4) == 0) {
  ...
  if (strncmp(shortcode_.c_str(), "BSE_", 4) == 0) {
    ...
    internal_symbol = HFSAT::CBOESecurityDefinitions::GetDatasourcenameCBOE(shortcode_);
  }
  ...
  HFSAT::Utils::CBOEDailyTokenSymbolHandler &cboe_daily_token_symbol_handler_ =
      HFSAT::Utils::CBOEDailyTokenSymbolHandler::GetUniqueInstance(input_date_);
  ...
  if (strncmp(shortcode_.c_str(), "BSE_", 4) == 0) {
    ...
    segment = HFSAT::CBOESecurityDefinitions::GetSegmentTypeFromShortCode(shortcode_);
    token = cboe_daily_token_symbol_handler_.GetTokenFromInternalSymbol(internal_symbol.c_str(), segment);
  }
