...
int main(int argc, char** argv) {
  std::cout << "Crypto Struct  Size: " << sizeof(CRYPTO_MDS::CoinBaseMktStruct) <<std::endl;
  std::cout << "Crypto " << sizeof(CRYPTO_MDS::CoinBaseRecievedOrder) <<std::endl;
  std::cout << "Crypto " << sizeof(CRYPTO_MDS::CoinBaseOpenOrder)<<std::endl;
  std::cout << "Crypto " << sizeof(CRYPTO_MDS::CoinBaseDoneOrder) <<std::endl;
  std::cout << "Crypto " << sizeof(CRYPTO_MDS::CoinBaseMatchOrder) <<std::endl;
  std::cout << "Crypto " << sizeof(CRYPTO_MDS::CoinBaseChangeOrder) <<std::endl;
  std::cout << "Crypto " << sizeof(CRYPTO_MDS::CoinBaseActiveOrder) <<std::endl;
  std::cout << "Crypto FTX Struct  Size: " << sizeof(CRYPTO_MDS::FTXMktStruct) <<std::endl;
  ...
  std::cout << "BARDATA " << sizeof(NSE_MDS::NSEBarDataCommonStruct) << std::endl;
  if (argc != 3 && argc != 4 && argc != 5 && argc != 7 ) {
  // std::cout << sizeof(HFSAT::MDS_MSG::GenericMDSMessage) << std::endl;
  // std::cout << sizeof(EOBI_MDS::EOBICommonStruct) << std::endl;
  // std::cout << sizeof(NSE_MDS::NSETBTDataCommonStruct) << std::endl;
    if (argc != 4 && argc != 6) {
        std::cout
          << " USAGE: EXEC <exchange> <file_path> <Date> <start-time-optional-unix-time-sec> <end-time-optional-unix-time-sec>"
    << " USAGE: EXEC <SEG_IND> <Dir_path> <YYYYMMDD> <IND15> <start_time[GMT_0345]> <end_time[GMT_1000]>" << std::endl
          << " USAGE: EXEC <GEN_THROTTLE> <Dir_path> <YYYYMMDD> <NSE/BSE>" << std::endl;
        exit(0);
    }
  }
  std::string exch = argv[1];
  HFSAT::BulkFileReader reader;
  if(exch!="GEN_THROTTLE"){
  	reader.open(argv[2]);
  }

  std::string filename = argv[2];

  if(filename.find("BSE_") != std::string::npos){
    std::string dt_str = filename.substr(filename.find_last_of("_")+1);
    std::cout << " DATE : " << dt_str << std::endl;
    HFSAT::Utils::BSEDailyTokenSymbolHandler::GetUniqueInstance(atoi(dt_str.c_str()));
  }

  if(filename.find("NSE_") != std::string::npos){
    std::string dt_str = filename.substr(filename.find_last_of("_")+1);
    std::cout << " DATE : " << dt_str << std::endl;
    HFSAT::Utils::NSEDailyTokenSymbolHandler::GetUniqueInstance(atoi(dt_str.c_str()));
  }
  ...
  if (exch == "EUREX")
    ...
    MDSLogReader<EOBI_MDS::EOBICommonStructOld>::ReadMDSStructs(reader, st, et);
  else if (exch == "HKOMD")
    ...
    MDSLogReader<HFSAT::GenericORSReplyStructLiveProShm>::ReadMDSStructs(reader, st, et);
    ...
    MDSLogReader<NSE_MDS::NSEBarDataCommonStruct>::ReadMDSStructs(reader, st, et);
  }
  else if (exch == "CRYPTO_COINBASE"){
    MDSLogReader<CRYPTO_MDS::CoinBaseMktStruct>::ReadMDSStructs(reader, st, et);
  }
  else if (exch == "CRYPTO_FTX"){
    MDSLogReader<CRYPTO_MDS::FTXMktStruct>::ReadMDSStructs(reader, st, et);
  }
  else if (exch == "NSE_MKT"){
    MDSLogReader<FPGA_MDS::MktUpdateStruct>::ReadMDSStructs(reader, st, et);
  }
  else if (exch == "GEN_THROTTLE") {
        std::string exchange = "NSE";
        if (argc == 5) {
          exchange = argv[4];
        }
	HFSAT::ThrottleLogReader<HFSAT::GenericORSReplyStructLiveProShm>::ReadMDSStructsFiles(argv[3],argv[2],exchange);
  }
  else if(exch == "SEG_IND"){
	...
	HFSAT::IndLogReader<HFSAT::GenericORSReplyStructLiveProShm>::ReadMDSStructsFile(argv[2],argv[3],argv[4],reader,start_time,end_time);  
  }
  else if(exch == "OI_LIVE"){
    MDSLogReader<NSE_MDS::ST_TICKET_INDEX_INFO>::ReadMDSStructs(reader, st, et); 
  }
  else{
    ...
    while (true) {
      ...
      switch (exch_type) {
        ...
        case HFSAT::MDS_MSG::BSE: {
          EOBI_MDS::EOBICommonStruct next_event;
          mds_available_len_ = bulk_file_reader_.read(&next_event, sizeof(EOBI_MDS::EOBICommonStruct));
          if (mds_available_len_ < sizeof(next_event)) break;
          std::cout << next_event.ToString();
        } break;
