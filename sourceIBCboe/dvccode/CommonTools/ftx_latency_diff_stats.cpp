    ...
    for ( auto & itr : files_list_vec ) {
      ...
      if ( bulk_file_reader.is_open () ) {
        while ( true ) {
         ...
         CRYPTO_MDS::FTXMktStruct ftx_mkt_struct;

          size_t read_length = bulk_file_reader.read(reinterpret_cast<char*>(&ftx_mkt_struct), sizeof(CRYPTO_MDS::FTXMktStruct));
          if (read_length < sizeof(CRYPTO_MDS::FTXMktStruct)) break ;

          std::ostringstream source_time_ ;
          source_time_ << ftx_mkt_struct.source_time.tv_sec << "." << std::setw(6) << std::setfill('0') << ftx_mkt_struct.source_time.tv_usec ;
          std::ostringstream local_time_ ;
          local_time_ << ftx_mkt_struct.local_time.tv_sec << "." << std::setw(6) << std::setfill('0') << ftx_mkt_struct.local_time.tv_usec;

          symbol = ftx_mkt_struct.product_id ;
