...
void ParseCommandLineParams(const int argc, const char **argv, std::string &underlying, int &input_date_, int &topn_, std::string &exchange_) {
  // expect :
  // 1. $0 shortcode date_YYYYMMDD
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0] << " Underlying YYYYMMDD EXCHANGE" << std::endl;
    exit(0);
  } else {
    underlying = argv[1];
...
int main(int argc, char **argv) {
  ...
  ParseCommandLineParams(argc, (const char **)argv, underlying, input_date_, topn_, exchange_);
  ...
  if (exchange_ == "BSE") {
    opt_2_iv_ = HFSAT::BSESecurityDefinitions::GetOptionsIntrinsicValue(underlying);
  }
  ...
  if (exchange_ == "BSE") {
    t_temp_shc_ = "BSE_" + underlying + "_FUT0";
  }
  ...
  if (exchange_ == "BSE") {
    fut_last_close_ = HFSAT::BSESecurityDefinitions::GetLastClose(t_temp_shc_);
  }
  ...
  if (exchange_ == "BSE") {
    return_val1_ = HFSAT::NSESecurityDefinitions::GetBhavCopyToken(input_date_, t_temp_shc_.c_str(), 13, "OPTSTK",
                                                                   t_shortcode_2_token_map1_);
  }
  ...
  if (exchange_ == "BSE") {
    return_val2_ = HFSAT::BSESecurityDefinitions::GetBhavCopyToken(this_date_, t_temp_shc_.c_str(), 11, "OPTSTK",
                                                                   t_shortcode_2_token_map2_);
  }
  ...
  if (exchange_ == "BSE") {
    std::vector<std::string> t_call_options_ =
        HFSAT::BSESecurityDefinitions::GetOptionShortcodesForUnderlyingInCurrScheme(underlying, 0, 1, topn_);
    std::vector<std::string> t_put_options_ =
        HFSAT::BSESecurityDefinitions::GetOptionShortcodesForUnderlyingInCurrScheme(underlying, 0, -1, topn_);
  }
