...
class BulkFileWriter {
 protected:
  ...
  bool first_open_;
  bool can_open_;
  size_t max_buffer_size_upperbound_ = DEFAULT_BUFFER_SIZE;
 ...
 public:
  ...
  inline void CheckToFlushBuffer() {
    if (front_marker_ > flush_trigger_size_) {
      ...
      if (buf_capacity_ <
        max_buffer_size_upperbound_)  // Double the buffer capacity and trigger size if less than max_buffer_size_upperbound_
      {
        buf_capacity_ *= 2;
  ...
  inline void setUpperBoundOfBufferCapacity(const size_t &_max_buffer_size_upperbound_){
    max_buffer_size_upperbound_ = _max_buffer_size_upperbound_;
  }
