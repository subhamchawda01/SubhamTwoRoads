...
class NSEMDSShmInterface {
 private:
  key_t key;
  int shmid;

  volatile NSE_MDS::NSETBTDataCommonStructProShm *mds_shm_queue_, *mds_shm_queue_pointer_;
  ...
  NSEMDSShmInterface(const NSEMDSShmInterface &_disabled_copy_construction_);

 public:
  static NSEMDSShmInterface &GetUniqueInstance() {
    static NSEMDSShmInterface mds_shm_interface_;
  ...
  void InitializeOldSHM() {
    ...
    if ((shmid =
             shmget(key, (size_t)(NSE_MDS_QUEUE_SIZE * (sizeof(NSE_MDS::NSETBTDataCommonStructProShm)) + sizeof(int)),
                    IPC_CREAT | 0666)) < 0) {
    ...
    if ((mds_shm_queue_ = (volatile NSE_MDS::NSETBTDataCommonStructProShm *)shmat(shmid, NULL, 0)) ==
    ...
    shm_queue_index_ = (volatile int *)(mds_shm_queue_ + NSE_MDS_QUEUE_SIZE);
  }
  ...
  void InitializeThreadSafeSHM() {
    ...
    if ((shm_queue_ = (HFSAT::Shm1Queue<NSE_MDS::NSETBTDataCommonStructProShm> *)shmat(shmid, NULL, 0)) ==
  ...
  void Push(NSE_MDS::NSETBTDataCommonStructProShm *_generic_mds_struct_) {
    memcpy((void *)mds_shm_queue_pointer_, (void *)(_generic_mds_struct_),
           sizeof(NSE_MDS::NSETBTDataCommonStructProShm));

    *shm_queue_index_ = last_write_seq_num_;

    last_write_seq_num_ = (last_write_seq_num_ + 1) & (NSE_MDS_QUEUE_SIZE - 1);
  ...
  void WriteGenericStruct(NSE_MDS::NSETBTDataCommonStructProShm *_generic_mds_struct_) {
