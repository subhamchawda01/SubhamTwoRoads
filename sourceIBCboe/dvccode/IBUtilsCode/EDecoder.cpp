...
#include "dvccode/IBUtils/FamilyCode.hpp"
#include "dvccode/IBUtils/CommissionReport.hpp"
#include "dvccode/IBUtils/TwsSocketClientErrors.hpp"
#include "dvccode/IBUtils/EDecoder.hpp"
#include "dvccode/IBUtils/EClientMsgSink.hpp"
#include "dvccode/IBUtils/PriceIncrement.hpp"
#include "dvccode/IBUtils/EOrderDecoder.hpp"
...
const char* EDecoder::processTickPriceMsg(const char* ptr, const char* endPtr) {
	...
	// process ver 2 fields
	{
		...
		if( sizeTickType != NOT_SET){
                   ...
                   IBL1UpdateTick l1_update_tick;
                   memset((void*)&l1_update_tick, 0, sizeof(IBL1UpdateTick));

                   gettimeofday(&l1_update_tick.time,NULL);
                   //default
                   l1_update_tick.ib_update_type = IB_TICK_UPDATE;
                   l1_update_tick.ticker_id = tickerId;
                   memcpy((void*)l1_update_tick.symbol, (TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).c_str(), std::string(TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).length());

                   if(LAST_SIZE == sizeTickType){
                     l1_update_tick.ib_update_type = IB_TRADE;
                     l1_update_tick.side = ' ';
                     l1_update_tick.trade_price = price;
                     l1_update_tick.trade_size = size;
                   }else if(BID_SIZE == sizeTickType){
                     l1_update_tick.ib_update_type = IB_TICK_UPDATE;
                     l1_update_tick.side = 'B';
                     l1_update_tick.bid_price = price;
                     l1_update_tick.bid_size = size;
                   }else if(ASK_SIZE == sizeTickType){
                     l1_update_tick.ib_update_type = IB_TICK_UPDATE;
                     l1_update_tick.side = 'A';
                     l1_update_tick.ask_price = price;
                     l1_update_tick.ask_size = size;
                   }

                   if(nullptr != ib_data_listener){
                     ib_data_listener->OnIBKRRawDataUpdate(&l1_update_tick);
                   }

	  	   m_pEWrapper->tickSize( tickerId, sizeTickType, size);

                }else if(HIGH == tickTypeInt || LOW == tickTypeInt || CLOSE == tickTypeInt){
                   ...
                   gettimeofday(&l1_update_tick.time,NULL);
                   //default
                   l1_update_tick.ib_update_type = IB_INFO_UPDATE;
                   l1_update_tick.ticker_id = tickerId;
                   memcpy((void*)l1_update_tick.symbol, (TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).c_str(), std::string(TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).length());

                   if(HIGH == tickTypeInt){
                     l1_update_tick.side = ' ';
                     l1_update_tick.high_price = price;
                   }else if(LOW == tickTypeInt){
                     l1_update_tick.side = ' ';
                     l1_update_tick.low_price = price;
                   }else if(CLOSE == tickTypeInt){
                     l1_update_tick.side = ' ';
                     l1_update_tick.last_close_price = price;
                   }
...
const char* EDecoder::processTickSizeMsg(const char* ptr, const char* endPtr) {
	...
	DECODE_FIELD( size);
        ...
        if(BID_SIZE == tickTypeInt || ASK_SIZE == tickTypeInt || VOLUME == tickTypeInt){
          ...
          gettimeofday(&l1_update_tick.time,NULL);
          //default
          l1_update_tick.ib_update_type = IB_TICK_SIZE_ONLY_UPDATE;
          l1_update_tick.ticker_id = tickerId;
          memcpy((void*)l1_update_tick.symbol, (TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).c_str(), std::string(TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).length());

          if(BID_SIZE == tickTypeInt){
            l1_update_tick.side = 'B';
            l1_update_tick.bid_size = size;
          }else if(ASK_SIZE == tickTypeInt){
            l1_update_tick.side = 'S';
            l1_update_tick.ask_size = size;
          }else if(VOLUME == tickTypeInt){
            l1_update_tick.ib_update_type = IB_INFO_UPDATE;
            l1_update_tick.side = ' ';
            l1_update_tick.volume = size;
          }
...
const char* EDecoder::processTickOptionComputationMsg(const char* ptr, const char* endPtr) {
	...
	if( version >= 6) {
        ...
        gettimeofday(&l1_update_tick.time,NULL);
        //default
        l1_update_tick.ib_update_type = IB_GREEKS_UPDATE;
        l1_update_tick.ticker_id = tickerId;
        memcpy((void*)l1_update_tick.symbol, (TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).c_str(), std::string(TickerMapping::GetUniqueInstance().ticker_id_to_symbol[tickerId]).length());

        l1_update_tick.side = ' ';
        l1_update_tick.opg.imp_vol = impliedVol;
        l1_update_tick.opg.delta = delta;
        l1_update_tick.opg.option_price = optPrice;
        l1_update_tick.opg.p_div = pvDividend;
        l1_update_tick.opg.gamma = gamma;
        l1_update_tick.opg.vega = vega;
        l1_update_tick.opg.theta = theta;
        l1_update_tick.opg.spot = undPrice;
...
const char* EDecoder::processSecurityDefinitionOptionalParameterMsg(const char* ptr, const char* endPtr) {
	...
	for (int i = 0; i < strikesSize; i++) {
        ...
        for(auto & itr : expirations){
          for( auto & jtr : strikes){
            std::cout << exchange << " " << underlyingConId << " " << tradingClass << " " << multiplier << " " << itr << " " << jtr << std::endl;
          }
        }
...
int EDecoder::parseAndProcessMsg(const char*& beginPtr, const char* endPtr) {
	...
	// std::cout << "EDecoder::parseAndProcessMsg\n";
	...
	// std::cout << "Decoding message type\n";
	...
	try {
		...
		int msgId;
		DECODE_FIELD( msgId);
		// std::cout << "Message id is " << msgId << std::endl;
		...
		case TICK_PRICE:
			...
			break;
                        ...
                        //duplicate info, already received in price info
		case TICK_SIZE:
