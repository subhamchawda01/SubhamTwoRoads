...
#include "dvccode/IBUtils/Utils.hpp"
...
std::unordered_set<std::string> TestCppClient::isDataAlreadyReq;
std::vector<std::pair<Contract,std::string>> TestCppClient::seqOfDataReq;
bool TestCppClient::used_once = false;
...
void TestCppClient::AddIBDataListener(IBDataListener *ibl){
  m_pReader->AddIBDataListenerFromReader(ibl);
}
...
void TestCppClient::cancelAllMktDataReq(){

  	m_pClient->reqMarketDataType(1); // send delayed-frozen (4) market data type
	int id = 1000;
	for(int itr=0; itr<(int)isDataAlreadyReq.size(); itr++){
		m_pClient->cancelMktData(id++);
	}
	std::cout<<"Cancelled all: "<<isDataAlreadyReq.size()<<"\n"<<std::endl;
	used_once = false;
}
void TestCppClient::RequestMarketData(std::vector<Contract> &req_contracts, std::vector<std::string> &req_contract_sym){

  if(true == used_once) return;
  m_pClient->reqMarketDataType(1); // send delayed-frozen (4) market data type
  int id = 1000;
  std::cout<<"Required contracts size:"<<req_contracts.size()<<std::endl;

  for(auto &contract: seqOfDataReq){
    TickerMapping::GetUniqueInstance().ticker_id_to_symbol[id] = contract.second;
    m_pClient->reqMktData(id++, contract.first, "", false, false, TagValueListSPtr());
  }

  for(int itr=0; itr<(int)req_contracts.size(); itr++){
	if(isDataAlreadyReq.find(req_contract_sym[itr]) != isDataAlreadyReq.end())continue;

    TickerMapping::GetUniqueInstance().ticker_id_to_symbol[id] = req_contract_sym[itr];
    m_pClient->reqMktData(id++, req_contracts[itr], "", false, false, TagValueListSPtr());
	isDataAlreadyReq.insert(req_contract_sym[itr]);
	seqOfDataReq.push_back(std::make_pair(req_contracts[itr],req_contract_sym[itr]));
  }
  std::cout<<"Total data requested: "<<isDataAlreadyReq.size()<<std::endl;
  m_state = ST_TICKDATAOPERATION_ACK;
  used_once = true;

}

void TestCppClient::RequestMarketDataSingleContract(Contract &req_contract, std::string &req_contract_sym){

   //Here there is a potential synchronization problem when multiple  request are made concurrently Do we need locks or there is just one request? :Sanskar
  if(isDataAlreadyReq.count(req_contract_sym)!=0){
	std::cerr<<"Data already subscribed"<<std::endl;
	return;
  }
  else isDataAlreadyReq.insert(req_contract_sym);

//   std::cout<<"Ticker ID size till now: "<<TickerMapping::GetUniqueInstance().ticker_id_to_symbol.size()<<std::endl;
  m_pClient->reqMarketDataType(1); // send delayed-frozen (4) market data type
  int id = 1000+TickerMapping::GetUniqueInstance().ticker_id_to_symbol.size();
  TickerMapping::GetUniqueInstance().ticker_id_to_symbol[id] = req_contract_sym;
  m_pClient->reqMktData(id, req_contract, "", false, false, TagValueListSPtr());

  m_state = ST_TICKDATAOPERATION_ACK;
  seqOfDataReq.push_back(std::make_pair(req_contract,req_contract_sym));
//   std::cout<<"Data requested for: "<<req_contract_sym<<std::endl;
//   std::cout<<"Contract Details/n";
//   std::cout<<req_contract.symbol<<std::endl;
//   std::cout<<req_contract.secType <<std::endl;
//   std::cout<<req_contract.currency<<std::endl;
//   std::cout<<req_contract.exchange<<std::endl;
//   std::cout<<req_contract.comboLegs->size()<<std::endl;
}
...
void TestCppClient::tickDataOperation()
{
   ...
   Contract contract;
   contract.conId = 734263931;
   contract.exchange = "CBOE";


   m_pClient->reqMktData(999, contract, "", false, false, TagValueListSPtr());
   used_once = true;
   m_state = ST_TICKDATAOPERATION_ACK;
...
//   m_pClient->reqMktData(999, ContractSamples::OptionComboContract(), "", false, false, TagValueListSPtr());
    ...
    if( false == used_once ){
      ...
      for(int i=0; i<25; i++){
        ...
        Contract contract;
        contract.symbol = "SPX";
//        contract.localSymbol = "SPX   240920C05575000";
        contract.secType = "OPT";
        contract.exchange = "SMART";
        contract.currency = "USD";
        contract.lastTradeDateOrContractMonth = "20241022";
        contract.strike = atm_price - i*5;
        contract.right = "C";
        contract.multiplier = "100";

        std::string lsym_base = "SPXW  ";
        lsym_base += contract.lastTradeDateOrContractMonth.substr(2);

        std::string lsym = lsym_base + "C0" + std::to_string(contract.strike).substr(0,std::to_string(contract.strike).find(".")) + "000";
        contract.localSymbol = lsym;


        t_temp_oss << "CBOE_" << contract.symbol << "_CE_" << contract.strike << "_" << contract.lastTradeDateOrContractMonth;
        TickerMapping::GetUniqueInstance().ticker_id_to_symbol[id] = t_temp_oss.str();

        std::cout << " Requesting Data For : " << id << " " << t_temp_oss.str() << " " << lsym << "\n";
        m_pClient->reqMktData(id++, contract, "", false, false, TagValueListSPtr());
        t_temp_oss.str("");
        t_temp_oss.clear();
        contract.right = "P";

        t_temp_oss << "CBOE_" << contract.symbol << "_PE_" << contract.strike << "_" << contract.lastTradeDateOrContractMonth;
        TickerMapping::GetUniqueInstance().ticker_id_to_symbol[id] = t_temp_oss.str();

        lsym = lsym_base + "P0" + std::to_string(contract.strike).substr(0,std::to_string(contract.strike).find(".")) + "000";
        contract.localSymbol = lsym;
        std::cout << " Requesting Data For : " << id << " " << t_temp_oss.str() << " " << lsym << "\n";
        m_pClient->reqMktData(id++, contract, "", false, false, TagValueListSPtr());
        ...
        if(i!=0){
          ...
          lsym = lsym_base + "C0" + std::to_string(contract.strike).substr(0,std::to_string(contract.strike).find(".")) + "000";
          contract.localSymbol = lsym;
...
//	m_pClient->reqMktData(1003, ContractSamples::USStock(), "", false, true, TagValueListSPtr());
/*
    //! [reqmktdata]
...
///////////////// WORKING -----	m_pClient->reqMktData(1013, ContractSamples::USOptionContract(), "", false, false, TagValueListSPtr());
	//! [reqoptiondatagenticks]
...
{
//        Contract spx = Index("SPX", "CBOE");
	//! [reqsecdefoptparams]
	m_pClient->reqSecDefOptParams(5001, "SPX", "", "IND", 416904);
	//! [reqsecdefoptparams]
...
void TestCppClient::contractOperations()
{
        ...
        std::cout <<" reqContractDetails Options : " << std::endl;
	m_pClient->reqContractDetails(210, ContractSamples::OptionForQuery());
        std::cout <<" reqContractDetails Options end : " << std::endl;

	m_state = ST_CONTRACTOPERATION_ACK;
        return ;

        std::cout << " CALLING contractOperations " << std::endl;
...
void TestCppClient::orderOperations()
{
	...
	m_pClient->reqOpenOrders();
        ...
        if(true == place_order){
          ...
          m_orderId+=tv.tv_sec;
//          m_pClient->placeOrder(m_orderId++, ContractSamples::USOptionContract(), OrderSamples::LimitOrder("BUY", stringToDecimal("1"), 0.05));
	  m_pClient->placeOrder(m_orderId++, ContractSamples::OptionComboContract(), OrderSamples::ComboLimitOrder("SELL", stringToDecimal("1"), -4.3, false));
          std::this_thread::sleep_for(std::chrono::seconds(1));
          place_order = false;
        }
