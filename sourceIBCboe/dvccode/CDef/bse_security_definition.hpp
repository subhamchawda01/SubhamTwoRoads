/**
   \file dvccode/CDef/bse_security_definition.hpp
...
// Option contracts listed on BSE
...
typedef struct {
  ...
  short int precision; // NEW FIELD
  int lot_size_multiplier; // NEW FIELD
...
class BSESecurityDefinitions {
 private:
  explicit BSESecurityDefinitions(int t_intdate_);
  static BSESecurityDefinitions* p_uniqueinstance_;

  static int intdate_;
  static std::vector<BSEInstrumentParams_t*> inst_params_;
  // Vector to store all products with non zero OI. Populated from bhavcopy
  static std::vector<std::string> non_zero_OI_;
  static std::map<BSEOPtionsIdentifier_t, double> options_param_2_last_close_;
  static std::map<std::string, double> shortcode_2_options_last_close_;

  //
  static std::map<std::string, BSETradingLimit_t*> shortcode_2_price_limit_;
  ...
  static std::map<std::string, BSEInstrumentType_t> shortcode_2_type_;
  static std::map<int, BSEInstrumentType_t> token_2_type_;
  static std::map<int, std::string> token_to_shortcode_;
  static std::map<std::string, int > shortcode_to_token_;
  static std::map<std::string, BSEInstrumentType_t> generic_shortcode_2_type_;
  static std::map<std::string, double> shortcode_2_commissions_;
  static std::set<std::string> ipo_listing_for_this_date_;
  ...
  static std::map<std::string, std::string> token_2_shortcode;
  static std::map<int32_t, uint32_t> token_2_productid;
  static std::map<std::string, std::string> shortcode_2_token;
  static std::map<std::string, double> shortcode_2_tick;
  // OptionMetaData
  static std::map<std::string, std::vector<BSEOptionMetaInfo>> underlying_to_options_;

  static std::map<std::string, std::tuple<int, char, double>> underlying_2_corporate_action_value_;

  static std::tr1::unordered_map<std::string, std::vector<uint32_t>> partition_segmentid_to_securityid_map_;

 public:
  static inline BSESecurityDefinitions& GetUniqueInstance(int t_intdate_) {
    if (p_uniqueinstance_ == NULL) {
      p_uniqueinstance_ = new BSESecurityDefinitions(t_intdate_);
    }
  ...
  static char GetSegmentTypeFromShortCode(std::string const& shortcode) {
    if (shortcode_2_type_.end() != shortcode_2_type_.find(shortcode)) {
      BSEInstrumentType_t inst_type = shortcode_2_type_[shortcode];

      if (BSE_IF == inst_type || BSE_SF == inst_type || BSE_IO == inst_type || BSE_SO == inst_type)
        return BSE_FO_SEGMENT_MARKING;

      else if (BSE_IRFFUT == inst_type || BSE_CURFUT == inst_type || BSE_CUROPT == inst_type)
        return BSE_CD_SEGMENT_MARKING;

      else if (BSE_EQ == inst_type)
        return BSE_EQ_SEGMENT_MARKING;
      else if( BSE_IDXSPT == inst_type)
        return BSE_IX_SEGMENT_MARKING;
    } else if (generic_shortcode_2_type_.find(shortcode) != generic_shortcode_2_type_.end()) {
      return BSE_FO_SEGMENT_MARKING;
    }
    return BSE_INVALID_SEGMENT;
  }

  static BSEInstrumentType_t GetInstrumentTypeFromShortCode(std::string const& shortcode) {
    if (shortcode_2_type_.end() != shortcode_2_type_.find(shortcode)) {
      BSEInstrumentType_t inst_type = shortcode_2_type_[shortcode];
      return inst_type;
    }
   ...
   static char GetSegmentTypeFromToken(int token_) {
    if (token_2_type_.end() != token_2_type_.find(token_)) {
      BSEInstrumentType_t inst_type = token_2_type_[token_];
  ...
  static int GetTokenFromShortcode(std::string const& shortcode) {
    if (shortcode_to_token_.end() != shortcode_to_token_.find(shortcode)) {
        return shortcode_to_token_[shortcode];
      }
      return -1;
  }

  static std::string GetShortcodeFromToken(int token_) {
    if (token_to_shortcode_.end() != token_to_shortcode_.find(token_)) {
        return token_to_shortcode_[token_];
      }
      return "";
  }
  ...
  // adds contract specs for BSE IDX/STK FUT/OPT
  static void AddBSEContractSpecifications(ShortcodeContractSpecificationMap& t_contract_spec_map_);
  static void LoadIndexExpiries(const int t_date_);
  static void AddEquityContractSpecifications(ShortcodeContractSpecificationMap& t_contract_spec_map_);
  static void RefFOSpecifications();
  ...
  static void LoadIPOProductsForThisDate(int dt);
  ...
  static bool IsWeeklyFutureExpiry(int expiry, char* underlying);
  static bool IsMonthlyFutureExpiry(int expiry, char* underlying);
  ...
  static bool IsWeeklyOptionExpiry(const int yyyymmdd, char *underlying = NULL);
  static bool IsWeeklyFutureExpiry(const int yyyymmdd);
  ...
  static void LoadShortCode_Token(const int t_date_);
  ...
  static void LoadCMProductPriceLess100(const int t_date_);
  ...
  // Returns Productid for the exchangeid
  static int32_t GetProductIdFromExchangeId(const uint32_t _exchange_id_);

  // Exchange Symbol function for BSE
  static std::string GetExchSymbolBSE(const std::string& _shortcode_);
  static int GetNumberItmForUnderlying(std::string underlying_,int expiry);
  // Commission function for BSE - returns total commissions in rupees per unit transacted
  static double GetBSECommission(std::string _shortcode_);

  // Returns true iff we are interested in contracts of the specified expiry for the specified segment
  static bool LoadThisExpiryContracts(const int t_expiry_, const BSEInstrumentType_t t_inst_type_, char* underlying);

  // Returns the  contract number for the expirydate
  static int GetContractNumberFromExpiry(const int t_date_, const BSEInstrumentType_t t_inst_type_, char* underlying);
  ...
  static int GetExpiryFromContractNumber(const uint32_t t_number_, const BSEInstrumentType_t t_inst_type_, char* underlying);

  // Returns contract multiplier
  static double GetContractMultiplier(const BSEInstrumentType_t t_inst_type_);
  ...
  static bool IsShortcodeAnIPOListingThisDay(const std::string& shortcode);
  ...
  static BSEMonth_t StringToMonthNumber(const char* _cstr_);

  // fills shortcode to options_last_close price map
  static void AddtoOptionsLastClosePrice(BSEInstrumentType_t t_inst_type_, char* underlying_, int expiry_,
                                         double strike_price_, char* call_or_put_, std::string shortcode_);

  static bool IsOptionStrikePresentInBhavCopy(BSEInstrumentParams_t param, double strike_price);
  static double GetNearestAvailableMultipleStrike(BSEInstrumentParams_t param, double price, double multiple,
  ...
  static std::string GetShortCodeFromExchangeId(std::string& _shortcode_);
  // just for fuures now
...
15 CHG_IN_OI*/
  ...
  static BSETradingLimit_t* GetTradingLimits(const std::string& shortcode_);
  static BSETradingLimit_t* ChangeTradingLimits(const std::string& shortcode_, int int_px_to_cross_);
  ...
  static void LoadPartitionIdSegmentIdToSecurityIdMap(const int t_date_);
  static std::vector<uint32_t> GetSecurityIdVectorFromPartionSegmentId(const std::string partition_segmentid_);
  ...
  static void findLastDatesOfMonth(const std::vector<int>& expiry_dates_sensex_weekly, std::vector<int>& expiry_dates_sensex) {
    std::vector<int> sorted_dates = expiry_dates_sensex_weekly; 
    std::sort(sorted_dates.begin(), sorted_dates.end());

    int current_month = -1;
    int current_year = -1;
    int last_date_of_month = -1;
    ...
    for (int date : sorted_dates) {
        ...
        if (month != current_month || year != current_year) {
            if (last_date_of_month != -1) {
                expiry_dates_sensex.push_back(last_date_of_month);
            }
            current_month = month;
            current_year = year;
        }
