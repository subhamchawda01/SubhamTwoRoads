...
struct MktUpdateStruct{
  char contract_[32];
  int64_t seq_no_;
  int fd_id_;
  uint64_t no_of_cycles_;
  ...
  std::string ToString() {
    ...
    ss << fd_id_ << " " << seq_no_ << " " <<  no_of_cycles_ << " " 
       << std::fixed << time.tv_sec << time.tv_nsec << "\n";
    return ss.str();
  }
...
#define COINBASE_MKT_SYMBOL_LENGTH 32
#define COINBASE_ORDER_ID_SIZE 64
#define FTX_MKT_SYMBOL_LENGTH 32
#define BARDATA_SHORTCODE_LENGTH 32
#define CBOE_DOTEX_OFFLINE_SYMBOL_LENGTH 48
// TODO OFFLINE CONVERT MKTDATA
//
#define BSE_DOTEX_OFFLINE_SYMBOL_LENGTH 48 
...
enum class MsgType {
  ...
  kNSESpotIndexUpdate,
  kNSEOpenInterestTick
};

#define kNSEMsgTypeInvalid '0'
#define kNSEMsgTypekNSEOrderDelta '1'
#define kNSEMsgTypekNSETrade '2'
#define kNSEMsgTypekNSETradeDelta '3'
#define kNSEMsgTypekNSEOrderSpreadDelta '4'
#define kNSEMsgTypekNSESpreadTrade '5'
#define kNSEMsgTypekNSETradeExecutionRange '6'
#define kNSEMsgTypekNSESpotIndexUpdate '7'
#define KNSEMsgTypekNSEOpenInterestTick '8'
#define kMsgTypeControl 'C'
...
struct NSEDotexTrade {
  ...
  std::string ToStringLine(){
    std::ostringstream t_temp_oss;
    t_temp_oss << std::fixed << std::setprecision(6) << trade_price << " " << trade_quantity;
    return t_temp_oss.str();
  }
};
...
struct NSETBTSpotIndex{
  int32_t IndexValue;
  int32_t PercentChange;

  std::string ToString() {
    std::ostringstream t_temp_oss;
    t_temp_oss << "VALUE:        " << IndexValue << "\n";
    t_temp_oss << "PERCENTAGECHANGE:        " << PercentChange << "\n";
    return t_temp_oss.str();
  }
};

struct NSETBTOpenInterestTick {
  ...
  std::string ToString(char segment_type = 'F') {
    std::ostringstream t_temp_oss;
    t_temp_oss <<"\nToken:                  " << Token
...
struct NSEDotexOfflineCommonStruct {
  ...
  union {
    ...
    NSETBTSpotIndex nse_spot_index;
    NSETBTOpenInterestTick nse_oi_data;
    
  } data;
  
  inline uint32_t GetTradeSize() { return data.nse_dotex_trade.trade_quantity; }
  std::string ToString() {
    std::ostringstream t_temp_oss;
    ...
    switch (msg_type) {
      case MsgType::kNSEOrderDelta: {
       ...
       case MsgType::kNSESpotIndexUpdate: {
      t_temp_oss << "\n=================== NSESpotIndex ===================\n\n";
        t_temp_oss << "Time:            " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Symbol:          " << symbol << "\n";
        
        t_temp_oss << data.nse_spot_index.ToString() << "===================================================\n";
      } break;
	case MsgType::kNSEOpenInterestTick: {
      t_temp_oss << "\n=================== NSEOpenInterest ===================\n\n";
        t_temp_oss << "Time:            " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Symbol:          " << symbol << "\n";

        t_temp_oss << data.nse_oi_data.ToString() << "===================================================\n";
      } break;
  ...
  std::string ToStringLine() {
	...
	switch (msg_type) {
	  case MsgType::kNSETrade: {
		t_temp_oss << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                << source_time.tv_usec << " " << order_number << " " << symbol << " " 
		<< strike_price << " " << option_type << " " << data.nse_dotex_trade.ToStringLine()<<"\n";
		} break;
...
struct NSETBTTrade {
  ...
  std::string ToStringLine() {
    std::ostringstream t_temp_oss;
    t_temp_oss << buy_order_id << " ";
    t_temp_oss << sell_order_id << " ";
    t_temp_oss << std::fixed << std::setprecision(6) << trade_price << " ";
    t_temp_oss << trade_qty ;
...
struct NSETBTDataCommonStruct {
  ...
  std::string ToStringLine(){
    ...
    switch (msg_type){
  	  case MsgType::kNSETrade: {
		t_temp_oss <<source_time.tv_sec << "." << std::setw(6) << std::setfill('0') << source_time.tv_usec << " " << 
		HFSAT::Utils::NSEDailyTokenSymbolHandler::GetUniqueInstance(-1).GetInternalSymbolFromToken(token, segment_type)
		<< " " <<  msg_seq << " " << activity_type << segment_type << " " << data.nse_trade.ToStringLine() << "\n";
    ...
    switch (msg_type) {
      ...
      t_temp_oss << "\n=================== NSESpotIndex ===================\n\n";
        t_temp_oss << "Time:            " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Symbol:          "
                   << HFSAT::SpotTokenGenerator::GetUniqueInstance().GetSpotIndex(token)
                   << "\n";
        t_temp_oss << data.nse_spot_index.ToString() << "===================================================\n";
      } break;
  ...
  char const* getContract() {
    if ( msg_type == MsgType::kNSESpotIndexUpdate ) {
      return HFSAT::SpotTokenGenerator::GetUniqueInstance().GetSpotIndexChar(token);
    }
    return HFSAT::Utils::NSEDailyTokenSymbolHandler::GetUniqueInstance(-1).GetInternalSymbolFromToken(token,
                                                                                                      segment_type);
  }

  char const* getContract(int date_) {
    if ( msg_type == MsgType::kNSESpotIndexUpdate ) {
      return HFSAT::SpotTokenGenerator::GetUniqueInstance().GetSpotIndexChar(token);
    }

    if ( msg_type == MsgType::kNSEOpenInterestTick ) {
      
      //std::cout << "MARKET TYPE " << data.nse_oi_data.MarketType << std::endl; 
      if(data.nse_oi_data.MarketType == 1){
        return "NSE_NIFTYMIDSELECTOI";
      }else if (data.nse_oi_data.MarketType == 2){
        return "NSE_NIFTYFINSERVICEOI";
      }else if (data.nse_oi_data.MarketType == 3){
        return "NSE_NIFTYBANKOI";
      }else if (data.nse_oi_data.MarketType == 4){
        return "NSE_NIFTY50OI";
      }

      return "INVALID";
    }

    //std::cout << "Token " << token << " Segment " << segment_type << std::endl;
    return HFSAT::Utils::NSEDailyTokenSymbolHandler::GetUniqueInstance(date_).GetInternalSymbolFromToken(token,
                                                                                                      segment_type);
  }
...
struct ST_TICKET_INDEX_INFO {
  ...
  std::string ToString(char segment_type = 'F') {
    std::ostringstream t_temp_oss;
    t_temp_oss <<"\nTime:                    " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec 
               <<"\nToken:                   " << Token 
               <<"\nShortCode:               "<< HFSAT::Utils::NSEDailyTokenSymbolHandler::GetUniqueInstance(-1).GetInternalSymbolFromToken( Token, segment_type)
               <<"\nMarkeType:               " << MarketType 
               <<"\nFillPrice:               " << FillPrice 
               <<"\nFillVOlume:              " << FillVolume
               <<"\nOpen Interest:           " << OpenInterest 
  ...
  char const* getContract(char segment_type = 'F') {
    std::string contractname, str;
    contractname = HFSAT::Utils::NSEDailyTokenSymbolHandler::GetUniqueInstance(-1).GetInternalSymbolFromToken(Token,
                                                                                                      segment_type);
    std::stringstream ss(contractname);                                                                                       
    std::ostringstream t_temp_oss;
    if(getline(ss, str, '_'))
    t_temp_oss << str.c_str();
    if(getline(ss, str, '_'))
    t_temp_oss << "_" << str.c_str();

    return t_temp_oss.str().c_str();
  }
  char const* getShortcode() {
    return "INVALID";
  }
...
struct ControlMsg {
  HFSAT::ControlMessageCode_t message_code_;
  int intval_1_;
  char strval_1_[16];
};

struct NSETBTDataCommonStructProShm {
  HFSAT::ttime_t source_time;  // used by control msg as well
  int32_t token;               // this will be set to 0 for control msg
  uint32_t msg_seq;             // this is a trader id //trader_id_
  uint64_t t2t_cshmw_start_time;
  uint64_t t2t_cshw_end_time;
  ...
  char activity_type;
  char segment_type;
  inline bool isTradeMsg() { return (kNSEMsgTypekNSETrade == msg_type || kNSEMsgTypekNSESpreadTrade == msg_type); }
  inline double GetTradeDoublePrice() { return data.nse_trade.trade_price; }
  inline HFSAT::TradeType_t GetTradeAggressorSide() { return HFSAT::TradeType_t::kTradeTypeNoInfo; }
  inline uint32_t GetTradeSize() { return data.nse_trade.trade_qty; }
  inline void SetIntermediate(bool flag) {
  ...
  std::string ToString() {
    ...
    switch (msg_type) {
      case kNSEMsgTypekNSEOrderDelta: {
        t_temp_oss << "\n=================== NSEBookDelta ===================\n\n";
        t_temp_oss << "Time:            " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Symbol:          "
      ...
      case kNSEMsgTypekNSESpotIndexUpdate: {
      t_temp_oss << "\n=================== NSESpotIndex ===================\n\n";
        t_temp_oss << "Time:            " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Symbol:          "
      ...
      case KNSEMsgTypekNSEOpenInterestTick: {
      t_temp_oss << "\n=================== NSEOpenInterest ===================\n\n";
        t_temp_oss << "Time:            " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Symbol:          "
      ...
      case kNSEMsgTypekNSETradeExecutionRange: {
        t_temp_oss << "\n=================== NSETradeExecRange ===================\n\n";
        t_temp_oss << "Time:            " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Symbol:          "
      ...
      case kMsgTypeControl: {
        HFSAT::GenericControlRequestStruct ctrl_msg;
        memset((void*)&ctrl_msg, 0, sizeof(HFSAT::GenericControlRequestStruct));

        ctrl_msg.time_set_by_frontend_ = source_time;
        ctrl_msg.trader_id_ = msg_seq;
        ctrl_msg.control_message_.message_code_ = (HFSAT::ControlMessageCode_t)data.control_msg.message_code_;
        ctrl_msg.control_message_.intval_1_ = data.control_msg.intval_1_;
        memcpy((void*)ctrl_msg.control_message_.strval_1_, (void*)data.control_msg.strval_1_,
               sizeof(data.control_msg.strval_1_));

        t_temp_oss << ctrl_msg.ToString();
...
namespace CRYPTO_MDS {
enum class CB_MsgType { 
  kInvalid = 0,
  received, 
  open, 
  done, 
...
struct CoinBaseRecievedOrder {
    char order_id[COINBASE_ORDER_ID_SIZE];    
    char client_oid[COINBASE_ORDER_ID_SIZE];
    double size;
    double price;
    double funds;
    HFSAT::TradeType_t buysell;
    ...
    std::ostringstream t_temp_oss;
    t_temp_oss << "Orderid:             " << order_id << "\n";
    t_temp_oss << "Client Orderid:      " << client_oid << "\n";
    t_temp_oss << "Size:                " << std::setprecision(16) << size << "\n";
    t_temp_oss << "Price:               " << std::setprecision(16) << price << "\n";
    t_temp_oss << "funds:               " << std::setprecision(16) << funds << "\n";
    t_temp_oss << "BuySell:             " << HFSAT::GetTradeTypeChar(buysell) << "\n";
    t_temp_oss << "OrderType:           " << order_type << "\n";
    return t_temp_oss.str();
  }
};

struct CoinBaseOpenOrder {
  char order_id[COINBASE_ORDER_ID_SIZE];
  double price;
  ...
  std::string ToString() {
    ...
    t_temp_oss << "Orderid:             " << order_id << "\n";
    t_temp_oss << "Price:               " << std::setprecision(16) << price << "\n";
    t_temp_oss << "Remaining Size:      " << std::setprecision(16) << remaining_size << "\n";
    t_temp_oss << "BuySell:             " << HFSAT::GetTradeTypeChar(buysell) << "\n";
    return t_temp_oss.str();
  }
};

struct CoinBaseDoneOrder {
  char order_id[COINBASE_ORDER_ID_SIZE];
  double price;
  double remaining_size;
  int cancel_reason;
  ...
  std::string ToString() { 
    ...
    t_temp_oss << "Price:               " << std::setprecision(16) << price << "\n";
    t_temp_oss << "Remaining Size:      " << std::setprecision(16) << remaining_size << "\n";
    t_temp_oss << "Cancel Reason:       " << cancel_reason << "\n";
    t_temp_oss << "BuySell:             " << HFSAT::GetTradeTypeChar(buysell) << "\n";
    t_temp_oss << "Reason:              " << reason << "\n";
    return t_temp_oss.str();
  }
};

struct CoinBaseMatchOrder {
  char maker_order_id[COINBASE_ORDER_ID_SIZE];
  char taker_order_id[COINBASE_ORDER_ID_SIZE];
  uint64_t trade_id;
  double size;
  double price;
  HFSAT::TradeType_t buysell;
  std::string ToString() {
    std::ostringstream t_temp_oss;
    t_temp_oss << "Maker Orderid:        " << maker_order_id << "\n";
    t_temp_oss << "Taker Orderid:        " << taker_order_id << "\n";
    t_temp_oss << "Trade id:             " << trade_id << "\n";
    t_temp_oss << "Size:                 " << std::setprecision(16) << size << "\n";
    t_temp_oss << "Price:                " << std::setprecision(16) << price << "\n";
    t_temp_oss << "BuySell:              " << HFSAT::GetTradeTypeChar(buysell) << "\n";
    return t_temp_oss.str();
...
struct CoinBaseChangeOrder {
  ...
  std::string ToString() {
    std::ostringstream t_temp_oss;
    t_temp_oss << "Orderid:             " << order_id << "\n";
    t_temp_oss << "Old Size:            " << std::setprecision(16) << old_size << "\n";
    t_temp_oss << "New Size:            " << std::setprecision(16) << new_size << "\n";
    t_temp_oss << "Old Size:            " << std::setprecision(16) << old_price << "\n";
    t_temp_oss << "New Size:            " << std::setprecision(16) << new_price << "\n";
    t_temp_oss << "BuySell:             " << HFSAT::GetTradeTypeChar(buysell) << "\n";
    t_temp_oss << "Reason:              " << reason << "\n";
    return t_temp_oss.str();
  }
...
struct CoinBaseActiveOrder {
  char profile_id[COINBASE_ORDER_ID_SIZE];
  char order_id[COINBASE_ORDER_ID_SIZE];
  double stop_price;
  double size;
  double funds;
  ...
  std::string ToString() {
    std::ostringstream t_temp_oss;
    t_temp_oss << "Profileid:           " << profile_id << "\n";
    t_temp_oss << "Orderid:             " << order_id << "\n";
    t_temp_oss << "Stop Price:          " << std::setprecision(16) << stop_price << "\n";
    t_temp_oss << "Size:                " << std::setprecision(16) << size << "\n";
    t_temp_oss << "Funds:               " << funds << "\n";
    t_temp_oss << "UserId:              " << user_id << "\n";
    t_temp_oss << "BuySell:             " << HFSAT::GetTradeTypeChar(buysell) << "\n";
    t_temp_oss << "Stop Type:           " << stop_type << "\n";
    t_temp_oss << "Private:             " << private_ << "\n";
    return t_temp_oss.str();
  }
};
struct CoinBaseMktStruct {
  char product_id[COINBASE_MKT_SYMBOL_LENGTH];
  struct timeval source_time;
  ...
  union {
    CoinBaseRecievedOrder coinbase_mkt_recieved_order;
    CoinBaseOpenOrder coinbase_mkt_open_order;
    CoinBaseDoneOrder coinbase_mkt_done_order;
    CoinBaseMatchOrder coinbase_mkt_match_order;
    CoinBaseChangeOrder coinbase_mkt_change_order;
    CoinBaseActiveOrder coinbase_mkt_active_order;
  ...
  std::string ToString() {
    ...
    switch (msg_type) {
      ...
      case CB_MsgType::received: {
        t_temp_oss << "\n=================== OrderReceived ===================\n\n";
        t_temp_oss << "Time:                " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
	t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << local_time.tv_usec << "\n";
        t_temp_oss << "Product Id:          " << product_id << "\n";
        t_temp_oss << "Sequence:            " << sequence << "\n";

        t_temp_oss << data.coinbase_mkt_recieved_order.ToString() << "===================================================\n";

      } break;
       case CB_MsgType::open: {
        t_temp_oss << "\n=================== OrderOpen ===================\n\n";
        t_temp_oss << "Time:                " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
        ...
        t_temp_oss << data.coinbase_mkt_open_order.ToString() << "===================================================\n";
      } break;


      case CB_MsgType::done: {
        t_temp_oss << "\n=================== OrderDone ===================\n\n";
        t_temp_oss << "Time:                " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
        ...
        t_temp_oss << data.coinbase_mkt_done_order.ToString() << "===================================================\n";

      } break;

      case CB_MsgType::match: {
        t_temp_oss << "\n=================== OrderMatch ===================\n\n";
        t_temp_oss << "Time:                " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
        ...
        t_temp_oss << data.coinbase_mkt_match_order.ToString() << "===================================================\n";

      } break;
      case CB_MsgType::change: {
        t_temp_oss << "\n=================== OrderChange ===================\n\n";
        t_temp_oss << "Time:                " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
        ...
        t_temp_oss << data.coinbase_mkt_change_order.ToString() << "===================================================\n";
      } break;
      case CB_MsgType::activate: {
        t_temp_oss << "\n=================== OrderActivate ===================\n\n";
        t_temp_oss << "Time:                " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
        t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
        ...
        t_temp_oss << data.coinbase_mkt_active_order.ToString() << "===================================================\n";
      } break;
      default: { t_temp_oss << "NOT_IMPLEMENTED_FOR_THIS_EVENT : " << (int32_t)msg_type << "\n"; } break;
    }
    return t_temp_oss.str();
...
struct FTXMktStruct {
  char product_id[FTX_MKT_SYMBOL_LENGTH];
  struct timeval source_time;
  struct timeval local_time;
  double price;
  double size;
  FTX_MsgType msg_type;
  char buysell;
  char intermediate;
  ...
  std::string ToString() {
	...
	t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
        ...
        t_temp_oss << "Size:                " << std::setprecision(16) << size << "\n";
        t_temp_oss << "BuySell:             " << buysell << "\n";
        t_temp_oss << "Intermediate:        " << intermediate << "\n===================================================\n";

      } break;

       case FTX_MsgType::trade: {
        t_temp_oss << "\n=================== Trade ===================\n\n";
        t_temp_oss << "Time:                " << source_time.tv_sec << "." << std::setw(6) << std::setfill('0')
                   << source_time.tv_usec << "\n";
	t_temp_oss << "Local Time:          " << local_time.tv_sec << "." << std::setw(6) << std::setfill('0')
