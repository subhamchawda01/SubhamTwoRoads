...
#define THROTTLE_MSG_LIMIT_MILLI 120 // Max mssg in every 100 milliseconds
...
#define _DEP_ONE_LOT_SIZE_ "ONE_LOT_SIZE"
#define _INDEP_ONE_LOT_SIZE_ "INDEP_LOT_SIZE"
...
typedef enum { kTradeTypeBuy = 0, kTradeTypeSell, kTradeTypeNoInfo } TradeType_t; //subham
...
inline ExchSource_t StringToExchSource(const std::string& _exch_str_) {
  ...
  if (strcmp(_exch_str_.c_str(), kExchSourceBMF_MULTISHMStr) == 0) return kExchSourceBMF_MULTISHM;
  ...
  if (strcmp(_exch_str_.c_str(), kExchSourceCBOEStr) == 0) return kExchSourceCBOE;
  if (strcmp(_exch_str_.c_str(), kExchSourceCBOE_FOStr) == 0) return kExchSourceCBOE_FO;
  if (strcmp(_exch_str_.c_str(), kExchSourceBSEStr) == 0) return kExchSourceBSE;
  if (strcmp(_exch_str_.c_str(), kExchSourceBSE_FOStr) == 0) return kExchSourceBSE_FO;
  if (strcmp(_exch_str_.c_str(), kExchSourceBSE_CDStr) == 0) return kExchSourceBSE_CD;
  if (strcmp(_exch_str_.c_str(), kExchSourceBSE_EQStr) == 0) return kExchSourceBSE_EQ;
  ...
  if (strcmp(_exch_str_.c_str(), kExchSourceNSE_SPOTStr) == 0) return kExchSourceNSE_IDX;
...
inline std::string ExchSourceStringForm(ExchSource_t exch) {
  ...
  switch (exch) {
    ...
    case kExchSourceBMF_MULTISHM:
      retval = kExchSourceBMF_MULTISHMStr;
      break;
    ...
    case kExchSourceCBOE:
      retval = kExchSourceCBOEStr;
      break;
    case kExchSourceCBOE_FO:
      retval = kExchSourceCBOE_FOStr;
      break;
    case kExchSourceBSE:
      retval = kExchSourceBSEStr;
      break;
    case kExchSourceBSE_FO:
      retval = kExchSourceBSE_FOStr;
      break;

    case kExchSourceBSE_CD:
      retval = kExchSourceBSE_CDStr;
      break;

    case kExchSourceBSE_EQ:
      retval = kExchSourceBSE_EQStr;
      break;
    ...
    case kExchSourceKRX:
      ...
      break;
    case kExchSourceNSE_IDX:
      retval = kExchSourceNSE_SPOTStr;
      break;
...
inline const char* ORSRejectionReasonStr(const ORSRejectionReason_t ors_r_t) {
  switch (ors_r_t) {
    ...
    case kExchOrderReject:
      ...
      break;
    case kExchDataEntryOrderReject:
      return "Exch OrderReject General";
      break;
...
enum class BSESegmentType { BSE_INVALID = 0, BSE_CD, BSE_FO, BSE_CM };
enum class CBOESegmentType { CBOE_INVALID = 0, CBOE_FO };
...
static std::map<std::string, const int> exchange_name_str_to_int_mapping = {
    {"NSE", 1},    {"TMX_OBF", 2}, {"OSE_ITCH", 3}, {"RTS_OF", 4},
    {"NSE_FO", 5}, {"NSE_EQ", 6},  {"NSE_CD", 7},   {"MICEX_CR", 8},
    {"NSE_BARDATA", 9}, {"NSE_FO_BARDATA", 10}, {"NSE_EQ_BARDATA", 11},  {"NSE_CD_BARDATA", 12},
    {"BSE", 13}, {"BSE_FO", 14}, {"BSE_EQ", 15},  {"BSE_CD", 16}, {"CBOE", 17}, {"CBOE_FO", 18}};
}
