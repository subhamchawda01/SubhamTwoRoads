...
BSESpotTokenGenerator* BSESpotTokenGenerator::unique_instance_ = nullptr;

BSESpotTokenGenerator& BSESpotTokenGenerator::GetUniqueInstance() {
  if (unique_instance_ == nullptr) {
    unique_instance_ = new BSESpotTokenGenerator();
  }
  return *(unique_instance_);
}

// Reads last dumped saci generator from the file if present else starts from 1
BSESpotTokenGenerator::BSESpotTokenGenerator() {
  last_increment_value = 0;
  std::string spot_index_token_filename(BSE_SPOT_INDEX_TOKEN_FILE);
  if (FileUtils::ExistsAndReadable(spot_index_token_filename)) {
    ...
    if (!spot_index_token_file.is_open()) {
      std::cout << "Failed To Load The SPOT INDEX File : " << BSE_SPOT_INDEX_TOKEN_FILE << std::endl;
      exit(-1);
    }
    char buffer[1024];
...
std::unordered_map<std::string, int> BSESpotTokenGenerator::GetSpotIndexToTokenMap(){
  return spot_index_token_;
}

int BSESpotTokenGenerator::GetTokenOrUpdate(std::string spot_index) {
...
const char* BSESpotTokenGenerator::GetSpotIndexChar(int token) {
...
std::string BSESpotTokenGenerator::GetSpotIndex(int token) {
  if (token_spot_index_.find(token) == token_spot_index_.end()) {
    std::cout<<"SHOULD NOT COME HERE NOT FOUND, for token " << token <<std::endl;
    return "";
  } else {
...
int BSESpotTokenGenerator::Update_New_token_generated(std::string spot_index) {
