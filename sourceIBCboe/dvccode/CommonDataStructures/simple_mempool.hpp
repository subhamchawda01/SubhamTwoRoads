...
class SimpleMempool {
 private:
  ...
  mutable HFSAT::Lock capacity_incr_lock_;

 public:
  SimpleMempool(unsigned int capacity = MEMPOOL_INCR_LEN)
  ...
  ObjectType* Alloc() {
    ...
    while (!object_pool_.pop(object)) {
      if (object_pool_.empty()) {
        ...
        capacity_incr_lock_.UnlockMutex();
      }
    }
    return object;
  }
