...
*/
class GapTheoCalculator : public MidTermTheoCalculator {
 protected:
  int current_directional_cascade_;
  double open_bar_close_px_;
  int bar_count_;
  double open_move_max_;
  int num_bars_;
  int trade_bar_;
  void LoadParams();
 ...
 public:
  GapTheoCalculator(std::map<std::string, std::string>* key_val_map, HFSAT::Watch& _watch_,
