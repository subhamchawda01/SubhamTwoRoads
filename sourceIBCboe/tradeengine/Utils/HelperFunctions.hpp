...
class HelperFunctions {
 public:
  ...
  static std::map<int, double> GetRatioList(HFSAT::DebugLogger& _dbglogger_, HFSAT::Watch& _watch_,
    ...
    if (HFSAT::SecurityDefinitions::IsEquity(dep_shc_) == false) {
      expiry_date_dep_ = HFSAT::SecurityDefinitions::ComputePreviousExpiry(date_, dep_shc_);
    }
    int expiry_date_indep_ = -1;
    if (HFSAT::SecurityDefinitions::IsEquity(indep_shc_) == false) {
      expiry_date_indep_ = HFSAT::SecurityDefinitions::ComputePreviousExpiry(date_, indep_shc_);
    }
    ...
    if (is_expiry_hit_) {
      ...
      if (is_expiry_hit_dep_) {
        prev_shc_dep_ = HFSAT::SecurityDefinitions::GetFutureSHCInPreviousExpiry(dep_shc_);
      }
      if (is_expiry_hit_indep_) {
        prev_shc_indep_ = HFSAT::SecurityDefinitions::GetFutureSHCInPreviousExpiry(indep_shc_);
      }
