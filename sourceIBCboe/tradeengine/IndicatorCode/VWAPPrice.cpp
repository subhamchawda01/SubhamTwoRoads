...
VWAPPrice::VWAPPrice(HFSAT::SecurityMarketView* smv, HFSAT::Watch& _watch_, HFSAT::DebugLogger& _dbglogger_,
                     double weight_, int vwap_levels)
    : BasePrice(smv, _watch_, _dbglogger_, weight_), vwap_levels_(vwap_levels) {}

VWAPPrice::~VWAPPrice() {}

void VWAPPrice::OnMarketUpdate(const unsigned int _security_id_, const HFSAT::MarketUpdateInfo& _market_update_info_) {
  CalculateVWAPPrice();
}

void VWAPPrice::OnTradePrint(const unsigned int _security_id_, const HFSAT::TradePrintInfo& _trade_print_info_,
                             const HFSAT::MarketUpdateInfo& _market_update_info_) {
  CalculateVWAPPrice();
}

bool VWAPPrice::CalculateVWAPPrice() {
