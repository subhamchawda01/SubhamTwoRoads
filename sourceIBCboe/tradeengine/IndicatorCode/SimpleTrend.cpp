...
SimpleTrend::SimpleTrend(HFSAT::SecurityMarketView* smv, HFSAT::Watch& _watch_, HFSAT::DebugLogger& _dbglogger_,
                         double weight_, double skew_factor, int num_events)
...
void SimpleTrend::OnMarketUpdate(const unsigned int _security_id_,
  ...
  //	<< " sumsz:" << sum_size_ << " trend_avg_px:" << trend_avg_px_ << DBGLOG_ENDL_FLUSH;

  double mid_px = (_market_update_info_.bestbid_price_ + _market_update_info_.bestask_price_) * 0.5;
  double trend_skew = 0;
  if (trend_avg_px_ && trade_info_.size() >= num_events_) {
    trend_skew = (mid_px - trend_avg_px_) * skew_factor_;
  }
  ...
  //	<< " trend_diff:" << mid_px - trend_avg_px_ << DBGLOG_ENDL_FLUSH;

  base_bid_price_ = mkt_px + trend_skew;
  base_ask_price_ = base_bid_price_;
}

void SimpleTrend::OnTradePrint(const unsigned int _security_id_, const HFSAT::TradePrintInfo& _trade_print_info_,
                               const HFSAT::MarketUpdateInfo& _market_update_info_) {
  base_bid_price_ = _market_update_info_.bestbid_price_;
  base_ask_price_ = _market_update_info_.bestask_price_;
  if (base_bid_price_ == kInvalidPrice || base_ask_price_ == kInvalidPrice) {
  ...
  //	<< _trade_print_info_.size_traded_ << " sumpx:" << sum_px_
  //	<< " sumsz:" << sum_size_ << " trend_avg_px:" << trend_avg_px_ << DBGLOG_ENDL_FLUSH;
...
bool SimpleTrend::pruneTradeVector() {
  while (trade_info_.size() > num_events_) {
...
/*bool SimpleTrend::CalculateVWAPPrice() {
