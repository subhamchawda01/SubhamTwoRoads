...
std::vector<TWAP *> twap_exec_vec_;
...
void termination_handler(int signum) {
  ...
  t_oss_ << "NSE TWAP Strategy: " << progid_ << " exiting signal: " << SimpleSignalString(signum) << " on " << hostname_
         << "\n";

  std::string subject_email_string_ = t_oss_.str();
  position_email_string_ = position_email_string_ + t_oss_.str();

  for (auto twap_exec_ : twap_exec_vec_) {
    int pos_remaning_ = twap_exec_->GetPosition() - stoi(shc_pos_map[twap_exec_->GetSecondaryShc()]);
    if (pos_remaning_ != 0) {
      std::ostringstream t_oss_1;
      t_oss_1 << "SHC: " << twap_exec_->GetSecondaryShc() << " POS_EXECUTED: " << twap_exec_->GetPosition()
              << " POS_REMAINING: " << pos_remaning_ << "\n";
      position_email_string_ = position_email_string_ + t_oss_1.str();
    }
    std::cout << "SHC: " << twap_exec_->GetSecondaryShc() << " POS_EXECUTED: " << twap_exec_->GetPosition()
              << " POS_REMAINING: " << pos_remaning_ << "\n";
  }
...
void SubscribeToSIM(std::map<std::string, HFSAT::ExternalDataListener *> &_shortcode_nse_data_filesource_map_,
                    HFSAT::BasicOrderManager *smart_om, std::vector<std::string> &_source_shortcode_list_,
                    HFSAT::BaseSimMarketMaker *smm_, HFSAT::SecurityNameIndexer &sec_name_indexer_) {
  int saci = smart_om->server_assigned_client_id_;
  for (auto filesource_iter_ = _shortcode_nse_data_filesource_map_.begin();
       filesource_iter_ != _shortcode_nse_data_filesource_map_.end(); filesource_iter_++) {
    filesource_iter_->second->AddExternalDataListenerListener(smm_);
  }
  for (auto shc : _source_shortcode_list_) {
    int sec_id_ = sec_name_indexer_.GetIdFromString(shc);
    smm_->AddSecIdToSACI(saci, sec_id_);
  }
  if ((smm_ != nullptr) && (smart_om != nullptr)) {
    smm_->AddOrderNotFoundListener(smart_om);
    smm_->AddOrderConfirmedListener(smart_om);
    smm_->AddOrderConfCxlReplaceRejectedListener(smart_om);
    smm_->AddOrderConfCxlReplacedListener(smart_om);
    smm_->AddOrderCanceledListener(smart_om);
    smm_->AddOrderExecutedListener(smart_om);
    smm_->AddOrderRejectedListener(smart_om);
  }
}
...
int main(int argc, char **argv) {
  ...
  // Arg2 : Trading date
  // Arg3 : start time
  // Arg4 : Time to execute
  // Arg5 : Prog ID
  // Arg6 : Notional to Place
  // Arg7 : LiveTrading(0 or 1) Optional
  ...
  if (argc < 7) {  // 7 is min of live and sim
    std::cerr
        << "expecting :\n"
        << " $posexec pos_file_to_execute TRADINGDATE START_TIME TIME_TO_EXECUTE PROGID NOTIONAL_TO_USE --exchange=[NSE]/BSE --no_recovery=[false]/true"
        << '\n';
  ...
  int time_to_exec_in_secs_ = atoi(argv[4]);
  progid_ = atoi(argv[5]);
  int notional_to_place_ = atoi(argv[6]);
  bool livetrading_ = true;
  std::string exchange_ = "NSE";
  bool no_recovery_allowed = false;

  if (livetrading_) {
    if (HFSAT::DateTime::GetCurrentIsoDateLocal() > tradingdate_) {
      std::cerr << "Can't run pos_exec from an old date in LIVE" << std::endl;
      std::exit(-1);
    }
    // signal handling, Interrupts and seg faults
    signal(SIGINT, termination_handler);
  ...
  desc.add_options()("help", "produce help message.")("no_recovery", boost::program_options::value<std::string>()->default_value("false"))("exchange", boost::program_options::value<std::string>()->default_value("NSE"));

  boost::program_options::variables_map vm;
  boost::program_options::store(boost::program_options::parse_command_line(argc, argv, desc), vm);
  boost::program_options::notify(vm);

  std::string recovery_flag = vm["no_recovery"].as<std::string>();
  std::string exchange_flag = vm["exchange"].as<std::string>();
  no_recovery_allowed = ("false" == recovery_flag) ? true : false;
  ...
  std::vector<std::string> dbg_code_vec = {"OM_INFO", "FILL_TIME_INFO"};
  ...
  int trading_end_utc_mfm_ = trading_start_utc_mfm_ + time_to_exec_in_secs_ * 1000;
  int end_utc_hhmm_ =
      HFSAT::DateTime::GetUTCHHMMFromTime(HFSAT::DateTime::GetTimeUTCFromMFMUTC(tradingdate_, trading_end_utc_mfm_));
  ...
  // Setup Logger
  InitDbglogger(tradingdate_, progid_, dbg_code_vec, common_intializer, logs_directory, livetrading_);
  ...
  if (livetrading_) {
    ...
    t_temp_oss_ << logs_directory << "/trades." << tradingdate_ << "." << progid_;
    std::string tradesfilename = t_temp_oss_.str();

    // open trades file in append mode in livetrading_
    trades_writer_.Open(tradesfilename.c_str(), (livetrading_ ? std::ios::app : std::ios::out));
  }
...
//        control_recv_data_info_.bcast_port_, combined_mds_messages_shm_processor, 'X', false);
  ...
  if( "BSE" == exchange_ || "BSE_NSE" == exchange_){
    HFSAT::BSEMD::BSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher, nullptr, HFSAT::kRaw, exchange_,
                                                        true, combined_mds_messages_shm_processor, no_recovery_allowed,cfn_oss_bse.str());
  ...
  std::map<std::string, HFSAT::ExternalDataListener *> &_shortcode_nse_data_filesource_map_ =
      common_intializer->getShcToFileSrcMap();

  std::map<std::string, std::string> shc_dep_map;
  Parser::ParseConfig(_position_file, shc_dep_map, 2);
  std::map<std::string, TWAP *> shc_twap_map;
  std::vector<HFSAT::BasicOrderManager *> basic_order_manager_vec_;

  for (auto &shc_pos_iter : shc_pos_map) {
    int sec_id_ = sec_name_indexer_.GetIdFromString(shc_pos_iter.first);
    int position_ = stoi(shc_pos_iter.second);
    auto dep_smv_ = p_smv_vec_[sec_id_];
    assert(dep_smv_ != nullptr);

    HFSAT::BaseTrader *base_trader_; // = GetNSETrader(shc_pos_iter.first, sec_id_, network_account_info_manager_, watch_);
    HFSAT::BaseSimMarketMaker *smm_ = NULL;
    if (livetrading_) {
      dbglogger_ << "SHC: " << dep_smv_->shortcode() << " SYM: " << dep_smv_->secname() << " ";
      if (exchange_ == "NSE") {
        base_trader_ = GetNSETrader(shc_pos_iter.first, sec_id_, network_account_info_manager_, watch_);
      }
      else if (exchange_ == "BSE" || exchange_ == "BSE_NSE") {
        base_trader_ = GetBSETrader(shc_pos_iter.first, sec_id_, network_account_info_manager_, watch_);
      } else {
	base_trader_ = GetNSETrader(shc_pos_iter.first, sec_id_, network_account_info_manager_, watch_);
	}
    } else {
      smm_ = sid_to_smm_map_[sec_id_];
      assert(smm_ != nullptr);
      std::string trade_info = "nullptr";
      base_trader_ = HFSAT::SimTraderHelper::GetSimTrader(trade_info, smm_);
    }
    ...
    auto basic_om_ = new HFSAT::BasicOrderManager(dbglogger_, watch_, sec_name_indexer_, *base_trader_, *dep_smv_, 1,
                                                  pos_limits_map_, livetrading_, false, false);
    ...
    // Since it's execution is dependent on other positions, hence setting the initial position
    // to be zero. Positions to execs will be updated once we get execution in dependent product.
    if (shc_dep_map.find(shc_pos_iter.first) != shc_dep_map.end()) {
      // Ensuring dependent exits failing which entry is deleted from dependent map
      if (shc_pos_map.find(shc_dep_map[shc_pos_iter.first]) != shc_pos_map.end()) {
        initial_pos_to_exec_ = 0;
      } else {
        shc_dep_map.erase(shc_pos_iter.first);
      }
    }

    TWAP *twap_exec_ = new TWAP(watch_, dbglogger_, dep_smv_, basic_om_, time_to_exec_in_secs_, trading_start_utc_mfm_,
                                std::abs(initial_pos_to_exec_), buysell_, notional_to_place_);
    twap_exec_vec_.push_back(twap_exec_);
    basic_om_->AddExecutionListener(twap_exec_);
    shc_twap_map[shc_pos_iter.first] = twap_exec_;
    ...
    if (livetrading_) {
      ...
      SubscribeToSIM(_shortcode_nse_data_filesource_map_, basic_om_, source_shortcode_list_, smm_, sec_name_indexer_);
      base_pnl = new HFSAT::SimBasePNL(dbglogger_, watch_, *dep_smv_, runtime_id_, trades_writer_);
    }
    basic_om_->SetBasePNL(base_pnl);

    twap_exec_->SetBasePNL(base_pnl);
    twap_exec_->SubscribeOM();

    basic_order_manager_vec_.push_back(basic_om_);
    runtime_id_++;
  }

  for (auto &shc_dep_iter : shc_dep_map) {
    double multiplier_ = stod(shc_pos_map[shc_dep_iter.first]) / stod(shc_pos_map[shc_dep_iter.second]);
    shc_twap_map[shc_dep_iter.second]->SetHedgePosExec(shc_twap_map[shc_dep_iter.first], multiplier_);
  }


  HFSAT::AllocateCPUUtils::GetUniqueInstance().AllocateCPUOrExit("pos_exec");

  if (livetrading_) {

    UserControlManager *user_control_manager_ =
        new UserControlManager(dbglogger_, watch_, basic_order_manager_vec_, twap_exec_vec_);
