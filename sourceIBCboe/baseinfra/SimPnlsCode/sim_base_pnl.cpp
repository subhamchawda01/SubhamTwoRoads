...
SimBasePNL::SimBasePNL(DebugLogger& t_dbglogger_, Watch& r_watch_, SecurityMarketView& t_dep_market_view_,
                       int t_runtime_id_, BulkFileWriter& t_trades_writer_, bool t_are_we_running_bardata_sim)
    : BasePNL(t_dbglogger_, r_watch_, t_dep_market_view_, t_runtime_id_,t_are_we_running_bardata_sim),
      trades_writer_(t_trades_writer_),
      max_loss_(10000),  // a very high alue so that tis is not used
      set_cons_total_pnl_(false),
      option_(nullptr),
      fut_smv_(nullptr) {
  is_option_ = SecurityDefinitions::IsOption(dep_market_view_.shortcode());
  if (is_option_) {
    ...
    fut_smv_ = ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(
        SecurityDefinitions::GetFutureShortcodeFromOptionShortCode(dep_market_view_.shortcode()));
  }
...
void SimBasePNL::OnExec(const int r_new_position_, const int _exec_quantity_, const TradeType_t _buysell_,
  ...
  } else if (dep_market_view_.exch_source() == kExchSourceCBOE) {
    if (this_trade_price_ >= 1) {
      commish_dollars_per_unit_ = commish_dollars_per_unit_ + 0.09;
    }
    //t_commish_dollars_per_unit_ = commish_dollars_per_unit_ * this_trade_price_;
  ...
  if (_buysell_ == kTradeTypeBuy) {                                        // buy was filled
    ...
    {  // we were short and position and new position are of differetn sign
...

      {
        ...
        dbglogger_ << buf << DBGLOG_ENDL_FLUSH;
