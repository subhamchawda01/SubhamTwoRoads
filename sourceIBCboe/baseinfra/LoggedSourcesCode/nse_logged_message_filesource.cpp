...
NSELoggedMessageFileSource::NSELoggedMessageFileSource(
...
void NSELoggedMessageFileSource::SeekToFirstEventAfter(const ttime_t r_start_time_, bool& rw_hasevents_) {
  ...
  if(false == actual_data_skip_){
    ...
    if (bulk_file_reader_.is_open()) {
      if (next_event_timestamp_.tv_sec == 0 && next_event_timestamp_.tv_usec == 0) {
        ...
        do {
          available_len_ = bulk_file_reader_.read(&next_event_, sizeof(NSE_MDS::NSEDotexOfflineCommonStruct));

        } while (available_len_ == sizeof(NSE_MDS::NSEDotexOfflineCommonStruct) &&
                 strcmp(next_event_.symbol, datasource_symbol_) != 0);

        if (available_len_ <
            sizeof(NSE_MDS::NSEDotexOfflineCommonStruct)) { /* not enough data to fulfill this request to read a struct */
    ...
    if (bulk_file_reader_.is_open()) {
      if (next_event_timestamp_.tv_sec == 0 && next_event_timestamp_.tv_usec == 0) {
        ...
        do {
          ...
          if(strcmp(next_event_.symbol, datasource_symbol_) != 0)continue;
...
void NSELoggedMessageFileSource::ComputeEarliestDataTimestamp(bool& _hasevents_) {
  if (bulk_file_reader_.is_open()) {
    // read the next_event_
    // set next_event_timestamp_
    size_t available_len_;
...
inline void NSELoggedMessageFileSource::_ProcessThisMsg() {
  //std::cout << "NSELoggedMessageFileSource::_ProcessThisMsg" << std::endl;
  ...
  if (p_order_global_listener_nse_) {
    ...
    switch (next_event_.msg_type) {
      case NSE_MDS::MsgType::kNSEOrderDelta: {
        ...
        TradeType_t t_buysell_ = next_event_.data.nse_dotex_order_delta.buysell == 'B' ? kTradeTypeBuy : kTradeTypeSell;

        switch (next_event_.data.nse_dotex_order_delta.activity_type) {
          case '1': {
            if (p_order_level_listener_sim_) {
              p_order_level_listener_sim_->OnOrderAdd(security_id_, t_buysell_, next_event_.order_number,
                                                      next_event_.data.nse_dotex_order_delta.order_price,
                                                      next_event_.data.nse_dotex_order_delta.volume_original, false);
            }
            p_order_global_listener_nse_->OnOrderAdd(security_id_, t_buysell_, next_event_.order_number,
                                                     next_event_.data.nse_dotex_order_delta.order_price,
                                                     next_event_.data.nse_dotex_order_delta.volume_original, false);
          ...
          case '4': {
            ...
            if (p_order_level_listener_sim_) {
              p_order_level_listener_sim_->OnOrderModify(security_id_, t_buysell_, next_event_.order_number,
                                                         next_event_.data.nse_dotex_order_delta.order_price,
                                                         next_event_.data.nse_dotex_order_delta.volume_original);
            }
            p_order_global_listener_nse_->OnOrderModify(security_id_, t_buysell_, next_event_.order_number,
                                                        next_event_.data.nse_dotex_order_delta.order_price,
                                                        next_event_.data.nse_dotex_order_delta.volume_original);
          } break;
          case '3': {
            // max is used to take care of iceberg order induced issues
            if (p_order_level_listener_sim_) {
              p_order_level_listener_sim_->OnOrderDelete(security_id_, t_buysell_, next_event_.order_number,
                                                         next_event_.data.nse_dotex_order_delta.order_price, true,
                                                         false);
            }

            p_order_global_listener_nse_->OnOrderDelete(security_id_, t_buysell_, next_event_.order_number,
                                                        next_event_.data.nse_dotex_order_delta.order_price, true,
                                                        false);
            break;
          }
          default: {
            fprintf(stderr, "Weird message type in NSELoggedMessageFileSource::ProcessAllEvents %d \n",
                    (int)next_event_.data.nse_dotex_order_delta.activity_type);
            break;
          }
        }
        break;
      ...
      case NSE_MDS::MsgType::kNSETrade: {
        if ((int64_t)next_event_.data.nse_dotex_trade.buy_order_number == -1 ||
            (int64_t)next_event_.data.nse_dotex_trade.sell_order_number == -1) {
          if (p_order_level_listener_sim_) {
            p_order_level_listener_sim_->OnHiddenTrade(security_id_, next_event_.data.nse_dotex_trade.trade_price,
                                                       next_event_.data.nse_dotex_trade.trade_quantity,
                                                       next_event_.data.nse_dotex_trade.buy_order_number,
                                                       next_event_.data.nse_dotex_trade.sell_order_number);
          }

          p_order_global_listener_nse_->OnHiddenTrade(security_id_, next_event_.data.nse_dotex_trade.trade_price,
                                                      next_event_.data.nse_dotex_trade.trade_quantity,
                                                      next_event_.data.nse_dotex_trade.buy_order_number,
                                                      next_event_.data.nse_dotex_trade.sell_order_number);
          ...
          if (p_order_level_listener_sim_) {
            p_order_level_listener_sim_->OnTrade(security_id_, next_event_.data.nse_dotex_trade.trade_price,
                                                 next_event_.data.nse_dotex_trade.trade_quantity,
                                                 next_event_.data.nse_dotex_trade.buy_order_number,
                                                 next_event_.data.nse_dotex_trade.sell_order_number);
          }
          p_order_global_listener_nse_->OnTrade(security_id_, next_event_.data.nse_dotex_trade.trade_price,
                                                next_event_.data.nse_dotex_trade.trade_quantity,
                                                next_event_.data.nse_dotex_trade.buy_order_number,
                                                next_event_.data.nse_dotex_trade.sell_order_number);
        }
      ...
      case NSE_MDS::MsgType::kNSETradeExecutionRange: {
        p_order_global_listener_nse_->OnTradeExecRange(
            security_id_,
            next_event_.data.nse_dotex_trade_range.low_exec_band,
            next_event_.data.nse_dotex_trade_range.high_exec_band);
      } break;
      case NSE_MDS::MsgType::kNSESpotIndexUpdate: {
	//std::cout << "NSE_MDS::MsgType::kNSESpotIndexUpdate: " << next_event_.data.nse_spot_index.IndexValue << std::endl;
/*   
        if (p_order_level_listener_sim_) {
...
void NSELoggedMessageFileSource::ProcessAllEvents() {
  while (true) {
    _ProcessThisMsg();
...
void NSELoggedMessageFileSource::ProcessEventsTill(const ttime_t _endtime_) {
  // assumes next_event_timestamp_
  // assumes mdu_type_
  //std::cout << "NSELoggedMessageFileSource::ProcessEventsTill" << std::endl;
