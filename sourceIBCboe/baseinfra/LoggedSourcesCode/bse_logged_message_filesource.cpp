...
#include <algorithm>
#include "baseinfra/LoggedSources/bse_logged_message_filesource.hpp"
 // A historical file for BSE option can contain many symbols - necessary support has been added for that.

namespace HFSAT {

  BSELoggedMessageFileSource::BSELoggedMessageFileSource(
    ...
    //  std::cout << "FILENAME: " << t_bse_filename_ << " " << HFSAT::kTLocNSE << " " << trading_location_file_read_ << " " << r_trading_location_ << std::endl ;
  ...
  void BSELoggedMessageFileSource::SeekToFirstEventAfter(const ttime_t r_start_time_, bool& rw_hasevents_) {
    if (bulk_file_reader_.is_open()) {
      if (next_event_timestamp_.tv_sec == 0 && next_event_timestamp_.tv_usec == 0) {
        ...
        do {
          available_len_ = bulk_file_reader_.read(&next_event_, sizeof(EOBI_MDS::EOBICommonStruct));
        } while (available_len_ == sizeof(EOBI_MDS::EOBICommonStruct) && false);

        if (available_len_ <
          sizeof(EOBI_MDS::EOBICommonStruct)) { /* not enough data to fulfill this request to read a struct */
  ...
  void BSELoggedMessageFileSource::ComputeEarliestDataTimestamp(bool& _hasevents_) {

    //std::cout << "BSELoggedMessageFileSource::ComputeEarliestDataTimestamp" << std::endl;
    if (bulk_file_reader_.is_open()) {
      // read the next_event_
      // set next_event_timestamp_
      size_t available_len_;
      ...
      } while (available_len_ == sizeof(EOBI_MDS::EOBICommonStruct) &&
  ...
  inline void BSELoggedMessageFileSource::_ProcessThisMsg() {
    //std::cout << "BSELoggedMessageFileSource::_ProcessThisMsg" << std::endl;
    NotifyExternalDataListenerListener(security_id_);
    p_time_keeper_->OnTimeReceived(next_event_timestamp_, security_id_);
    //std::cout << "Event_ToString: " << next_event_.ToString() << std::endl;
...
//          next_event_.order_.size - (int32_t)next_event_.order_.prev_price);
  ...
  void BSELoggedMessageFileSource::ProcessAllEvents() {
    //std::cout << "BSELoggedMessageFileSource::ProcessAllEvents" << std::endl;
  ...
  void BSELoggedMessageFileSource::ProcessEventsTill(const ttime_t _endtime_) {
...
//      std::cout << "available_len_: " << available_len_ << " " << sizeof(EOBI_MDS::EOBICommonStruct) << std::endl;
      if (available_len_ <
