...
#include <algorithm>
#include "baseinfra/LoggedSources/oi_bardata_logged_message_filesource.hpp"

#define MIN_OI_BARDATA_TOKENS_SIZE 11

namespace HFSAT {

ttime_t OiBarDataLoggedFileSource::last_processed_bardata_time_ = ttime_t(0,0);

OiBarDataLoggedFileSource::OiBarDataLoggedFileSource(DebugLogger& t_dbglogger_, SecurityNameIndexer& t_sec_name_indexer_,
                                                ...
                                                bool t_is_options_=false, std::string t_options_expiry_="",std::string t_options_type_="", std::string t_exch_="NSE"):
  ...
  if (exch_ == "NSE") {  
    t_bardata_filename_ = OIBarDataLoggedFileNamer::GetName( t_bardata_symbol_, _yyyymmdd_);
  }
  else if (exch_ == "BSE") { 
    t_bardata_filename_ = BseOiBarDataLoggedFileNamer::GetName(t_bardata_symbol_, _yyyymmdd_);
  }

  //std::cout << t_shortcode_<< " HERE Filename : " << t_bardata_filename_ << std::endl;

  bulk_file_reader_.open(t_bardata_filename_);
  options_strike_ = HFSAT::SecurityDefinitions::GetStrikePriceFromShortCode(bardata_mapped_symbol_in_file_);
}

void OiBarDataLoggedFileSource::SeekToFirstEventAfter(const ttime_t r_start_time_, bool& rw_hasevents_){

  //std::cout<<"SeekToFirstEventAfter: "<<bardata_mapped_symbol_in_file_<<std::endl;
  std::string this_bardata_mapped_symbol = "";
  std::string this_line_expiry_for_options = "";
  double this_strike=0;
    ...
    if (next_event_timestamp_.tv_sec == 0 && next_event_timestamp_.tv_usec == 0) {
      ...
      do {
        ...
        if(available_len_> 0){
          ...
          if( tokens_size_from_last_line >= MIN_OI_BARDATA_TOKENS_SIZE ){ 
            this_bardata_mapped_symbol = tokens[2];   //3rd column is mapped symbol 
            this_line_event_time = std::atoi(tokens[0]) + 60;
            next_event_timestamp_ = ttime_t(time_t(this_line_event_time), 0);
            if(true == is_options_){
              this_line_expiry_for_options = tokens[10];
              this_strike = std::atof(tokens[9]);
            }
          }
        }
        if(true == is_options_){
          std::vector<std::string> _tokens_;
           HFSAT::PerishableStringTokenizer::StringSplit(this_bardata_mapped_symbol,'_',_tokens_);
           if ((_tokens_[2][0]==options_type_[0]) && (this_strike==options_strike_) && (this_line_expiry_for_options == options_expiry_)){
            security_found = true;
//            std::cout <<security_id_<<" "<< this_bardata_mapped_symbol<<" "<< bardata_mapped_symbol_in_file_ <<" "<<_tokens_[2][0] <<" "<<options_type_[0] <<" "<<this_line_expiry_for_options <<" "<<options_expiry_ <<" "<<this_strike <<" "<<options_strike_<<std::endl;
           }else{
          ...
          if(this_bardata_mapped_symbol.find(bardata_mapped_symbol_in_file_) == std::string::npos) {
            security_found = false;
            continue;
	        }else{
//            std::cout << this_bardata_mapped_symbol<<" "<< bardata_mapped_symbol_in_file_<<std::endl;
            security_found = true;
          }
        }
      ...
      } while (available_len_ > 0 && tokens_size_from_last_line >= MIN_OI_BARDATA_TOKENS_SIZE && 
               (next_event_timestamp_ < r_start_time_ || (!security_found)));

      if (available_len_ <= 0 || tokens_size_from_last_line < MIN_OI_BARDATA_TOKENS_SIZE ){ 
...
void OiBarDataLoggedFileSource::ComputeEarliestDataTimestamp(bool& rw_hasevents_){
    ...
    if (next_event_timestamp_.tv_sec == 0 && next_event_timestamp_.tv_usec == 0) {
      ...
      do {
        ...
        if(true == is_options_){
          ...
          if(this_bardata_mapped_symbol.find(bardata_mapped_symbol_in_file_) == std::string::npos || ((true == is_options_) && this_line_expiry_for_options != options_expiry_)) continue;
          else break;
        }
      } while (available_len_ > 0 && tokens_size_from_last_line >= MIN_OI_BARDATA_TOKENS_SIZE);
...
void OiBarDataLoggedFileSource::_ProcessThisMsg(){
  ...
  //bar has moved forward
    if(last_processed_bardata_time_.tv_sec != 0 && (next_event_timestamp_ > last_processed_bardata_time_)){
      if(p_nse_bardata_listener_){
        ...
        p_nse_bardata_listener_->TriggerOiBardataUpdate(security_id_,t_temp_oss_.str().c_str());
      }
    }
    ...
    //processing
    if(p_nse_bardata_listener_){
      p_nse_bardata_listener_->TriggerOiBardataUpdate(security_id_,line_read_string.c_str());
    }
  }
  else if (exch_ == "BSE") {
  //bar has moved forward
...
void OiBarDataLoggedFileSource::ProcessAllEvents(){
...
void OiBarDataLoggedFileSource::ProcessEventsTill(const ttime_t _endtime_){
