...
#define BIT_RESET_ALL 0x0000;
  ...
  void SetIndexedBookCheckFalse() { market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_ = false; }
  ...
  void SetL1OnlyFlag(bool _l1only_) { 
    l1_only_ = _l1only_; 
  }
  ...
  void SetMarketOrdersMempool(LockFreeSimpleMempool<MarketOrder> *t_market_orders_mempool_) {
...
#undef MIN_L1EVENTS_TO_MAKE_READY_ON
  ...
  int GetBidIndex(int int_price) const {
    ...
    //std::cout << "GetBidIndex: base_bid_idx: " << base_bid_index_ << std::endl;

    //std::cout << "int_px at base_Idx: " << market_update_info_.bidlevels_[base_bid_index_].limit_int_price_ << std::endl;
  ...
  int GetAskIndex(int int_price) const {
    //std::cout << "GetAskIndex: base_ask_idx: " << base_ask_index_ << std::endl;

    //std::cout << "int_px at base_Idx: " << market_update_info_.bidlevels_[base_ask_index_].limit_int_price_ << std::endl;
  ...
  inline MarketUpdateInfoLevelStruct *bid_info(unsigned int t_level_) {
    if ((this_smv_exch_source_ == kExchSourceNSE) || (this_smv_exch_source_ == kExchSourceBSE)) {
      if (l1_only_) {
        return bid_level_info(t_level_);
      }

      return px_level_manager_->GetSynBidPL(market_update_info_.security_id_, t_level_);
    } else if ((this_smv_exch_source_ == kExchSourceBMF_FPGA) || (this_smv_exch_source_ == kExchSourceBMF)|| (this_smv_exch_source_ == kExchSourceCBOE)) {
      return bid_level_info(t_level_);
    }
    return nullptr;
  }
  ...
  unsigned int GetBaseBidMapIndex() const {
    //std::cout << "GetBaseBidMapIndex" << std::endl;
    if (is_ready_) {
      //std::cout << "is_ready_ : " << GetBidIndex(market_update_info_.bestbid_int_price_) << std::endl;
      return GetBidIndex(market_update_info_.bestbid_int_price_);
    } else {
      //std::cout << "else ! is_ready_: bas_idx: " << base_bid_index_ << std::endl;
  ...
  unsigned int GetBaseAskMapIndex() const {
    //std::cout << "GetBaseAskMapIndex" << std::endl;
    
    if (is_ready_) {
    //std::cout << "is_ready_ : " << GetAskIndex(market_update_info_.bestask_int_price_) << std::endl;
      return GetAskIndex(market_update_info_.bestask_int_price_);
    } else {
    //std::cout << "Else ! is_ready_: ask_idx: " << base_ask_index_ << std::endl;
  ...
  void UpdateL1Struct(HFSAT::GenericL1DataStruct &l1_struct);
  ...
  void subscribe_OpenInterestPrice(SecurityMarketViewChangeListener *t_new_listener_) ;
  ...
  void subscribe_circuit(SecurityMarketViewChangeListener *t_new_listener_);
  void subscribe_SpreadTrades(SecurityMarketViewChangeListener *t_new_listener_);
  void subscribe_SpotIndexPrice(SecurityMarketViewChangeListener *t_new_listener_);
  void subscribe_bardata(SecurityMarketViewBardataListener *t_new_listener_);
  ...
  void OnTradeIBKR(const double t_trade_price_, const int t_trade_size_,
  ...
  void NotifyCircuitListeners();
  void NotifySpotIndexListeners();
  void NotifyOpenInterestListeners(double const t_oi_price_);
  void NotifyL2OnlyListeners();
  void NotifyOnReadyListeners();
  void NotifyBardataListeners(const unsigned int t_security_id_, const char* _bardata_line_);
  ...
  void SetBestLevelAskVariablesOnLiftIBKR();
  void SetBestLevelAskVariablesOnLift();
  void SetBestLevelBidVariablesOnHitIBKR();
