/**
   \file MarketAdapter/ntp_market_order_manager.hpp
...
#include "baseinfra/MarketAdapter/bmf_market_per_security.hpp"

namespace HFSAT {

class NtpMarketOrderManager : public BaseMarketOrderManager, public OrderLevelListenerSim {
 protected:
  SimpleMempool<BmfOrder> bmf_order_mempool_;
  std::vector<BmfMarketPerSecurity *> bmf_markets_;

 public:
  NtpMarketOrderManager(DebugLogger &t_dbglogger_, const Watch &t_watch_, SecurityNameIndexer &t_sec_name_indexer_,
                        const std::vector<MarketOrdersView *> &t_market_orders_view_map_);

  ~NtpMarketOrderManager() {}
  ...
  void OnOrderModify(const unsigned int t_security_id_, const TradeType_t t_buysell_, const int t_level_,
    ...
    OnOrderModify(t_security_id_, t_buysell_, t_new_order_id_, t_new_price_, t_new_size_);
  }

  void OnOrderDelete(const unsigned int t_security_id_, const TradeType_t t_buysell_, const int t_level_,
                     const int64_t t_order_id_, const double t_price_, const int t_size_) {
    OnOrderDelete(t_security_id_, t_buysell_, t_order_id_);
  }
  ...
  void OnTrade(const unsigned int t_security_id_, const double t_trade_price_, const int t_trade_size_,
    ...
    //      OnTrade ( t_security_id_, t_buysell_,
    //          t_trade_price_, t_trade_size_ );
  }
