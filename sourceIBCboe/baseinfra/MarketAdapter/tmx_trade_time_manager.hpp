/**
   \file tmx_trade_time_manager.hpp
...
#ifndef BASE_MARKETADAPTER_TMX_TRADE_TIME_MANAGER_H
#define BASE_MARKETADAPTER_TMX_TRADE_TIME_MANAGER_H

#define TMX_TRADE_TIME_INFO_FILE "/spare/local/files/TMX/tmx-trd-hours.txt"
...
class TmxTradeTimeManager {
 private:
  ...
  TmxTradeTimeManager(const TmxTradeTimeManager&);

  TmxTradeTimeManager(HFSAT::SecurityNameIndexer& sec_indexer, int YYYYMMDD) {
  ...
  void parseTimeFile(HFSAT::SecurityNameIndexer& sec_indexer) {
    std::fstream file(std::string(TMX_TRADE_TIME_INFO_FILE).c_str(), std::ofstream::in);

    if (!file || !file.is_open()) {
      fprintf(stderr, "Could not open file %s in tmx_trade_time_manager\n", TMX_TRADE_TIME_INFO_FILE);
      exit(1);
    }
    char line[1024];
    char* ticker;
 ...
 public:
  static TmxTradeTimeManager& GetUniqueInstance(HFSAT::SecurityNameIndexer& sec_indexer, int YYYYMMDD = -1) {
    static TmxTradeTimeManager uniqueinstance_ = TmxTradeTimeManager(sec_indexer, YYYYMMDD);
...
#endif /* BASE_MARKETADAPTER_TMX_TRADE_TIME_MANAGER_H */
