...
#define CFE_CLEARING_FEE 0.04
  ...
  /* BMF */
  _commish_map_["BR_IND_0"] = 0.22 * CurrencyConvertor::Convert(kCurrencyBRL, kCurrencyUSD);
  _commish_map_["BR_IND_1"] = 0.22 * CurrencyConvertor::Convert(kCurrencyBRL, kCurrencyUSD);
  _commish_map_["BR_WIN_0"] = 0.04 * CurrencyConvertor::Convert(kCurrencyBRL, kCurrencyUSD);
  _commish_map_["BR_WIN_1"] = 0.04 * CurrencyConvertor::Convert(kCurrencyBRL, kCurrencyUSD);
  ...
  _commish_map_["BR_WDO_0"] = 0.10;
  _commish_map_["BR_WDO_1"] = 0.10;
...
double BaseCommish::GetCommishPerContract(const std::string& _shortcode_, int _YYYYMMDD_) {
  ...
  if (commish_map_.find(_shortcode_) != commish_map_.end()) {
    ...
    commish_map_[_shortcode_] = BSESecurityDefinitions::GetBSECommission(_shortcode_);
    return commish_map_[_shortcode_];
  } else if (_shortcode_.find("CBOE") == 0) {
    commish_map_[_shortcode_] = CBOESecurityDefinitions::GetCBOECommission(_shortcode_);
    return commish_map_[_shortcode_];
  }
...
//  } else if (exch_src_ == kExchSourceBSE) {
//    commission_per_contract *= _trade_price_ * CurrencyConvertor::Convert(kCurrencyINR, kCurrencyUSD);
  }
...
double BaseCommish::GetBSECommishPerContract(const std::string& _shortcode_, const double _trade_price_,
                                             const int _qty_) {
  return (_trade_price_ * _qty_ * BSESecurityDefinitions::GetBSECommission(_shortcode_) *
          CurrencyConvertor::Convert(kCurrencyINR, kCurrencyUSD));
}
