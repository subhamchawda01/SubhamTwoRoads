...
class CombinedMDSMessagesProNSEProcessor {
 protected:
 ...
 public:
  CombinedMDSMessagesProNSEProcessor(DebugLogger& _dbglogger_, SecurityNameIndexer& _sec_name_indexer_)
...
//  std::map<int32_t, NSE_UDP_MDS::NSERefData>& ix_nse_ref_data = *(segment_to_nse_ref_data_[NSE_IX_SEGMENT_MARKING]);
    ...
    for (auto& itr : spot_index_2_token_map_) {
        ...
        segment_to_token_secid_map_[NSE_IX_SEGMENT_MARKING].insert(std::make_pair(itr.second, security_id));  // currently wrong// need to check other things
      }
  ...
  inline void ProcessNSEEvent(NSE_MDS::NSETBTDataCommonStructProShm* next_event_) {
    // There will be a few instruments for which datasymbol/exchange won't be defined, TODO : use bool to reduce latency

    if (segment_to_token_secid_map_[next_event_->segment_type].find(next_event_->token) ==
        segment_to_token_secid_map_[next_event_->segment_type].end())
      return;

    int32_t security_id = segment_to_token_secid_map_[next_event_->segment_type][next_event_->token];
    ...
    // all live orders
    if (-1 == security_id_to_recovery_complete_[security_id] && next_event_->segment_type != NSE_IX_SEGMENT_MARKING ) {
    ...
    switch (next_event_->msg_type) {
      case kNSEMsgTypekNSEOrderDelta: {
        ...
        if ('N' == next_event_->activity_type) {
          ...
          //          std::cout << "ORDER ADD : " << next_event_->ToString ();
          // HFSAT::CpucycleProfiler::GetUniqueInstance().Start(1);
          p_order_global_listener_nse_->OnOrderAdd(
              security_id, (next_event_->data).nse_order.buysell, (next_event_->data).nse_order.order_id,
              (next_event_->data).nse_order.order_price * (double)security_id_to_inverse_price_mult_[security_id],
          ...
          //          std::cout << "ORDER MOD : " << next_event_->ToString ();
          ...
          //          std::cout << "ORDER DELETE: " << next_event_->ToString ();
          // HFSAT::CpucycleProfiler::GetUniqueInstance().Start(3);
          p_order_global_listener_nse_->OnOrderDelete(
              security_id, (next_event_->data).nse_order.buysell, (next_event_->data).nse_order.order_id,
              (next_event_->data).nse_order.order_price * (double)security_id_to_inverse_price_mult_[security_id], true,
              false);
      ...
      case kNSEMsgTypekNSETrade: {
        ...
        p_order_global_listener_nse_->OnTrade(
            security_id,
            (next_event_->data).nse_trade.trade_price * (double)security_id_to_inverse_price_mult_[security_id],
      ...
      case kNSEMsgTypekNSEOrderSpreadDelta: {
      } break;

      case kNSEMsgTypekNSESpreadTrade: {
      } break;
      ...
      case kNSEMsgTypekNSETradeExecutionRange: {
        ...
        p_order_global_listener_nse_->OnTradeExecRange(
            security_id,
            (next_event_->data).nse_trade_range.low_exec_band * (double)security_id_to_inverse_price_mult_[security_id],
            (next_event_->data).nse_trade_range.high_exec_band * (double)security_id_to_inverse_price_mult_[security_id]);
      } break;

      case kNSEMsgTypekNSESpotIndexUpdate: {
        // DBGLOG_CLASS_FUNC_LINE_ERROR << "NSE_MDS::MsgType::kNSESpotIndexUpdate: " << security_id  << " INDEXVALUE "<< next_event_->data.nse_spot_index.IndexValue << "\n ";
        p_time_keeper_->OnTimeReceived(next_event_->source_time);
        p_order_global_listener_nse_->OnIndexPrice(security_id, (double)next_event_->data.nse_spot_index.IndexValue * 0.01);
     } break;
      case KNSEMsgTypekNSEOpenInterestTick: {
        p_time_keeper_->OnTimeReceived(next_event_->source_time);
        p_order_global_listener_nse_->OnOpenInterestUpdate(security_id, (double)next_event_->data.nse_oi_data.OpenInterest * 0.01);
      } break;
      default: {
