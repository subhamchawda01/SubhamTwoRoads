...
class CombinedMDSMessagesDirectProcessor : public HFSAT::MDSMessages::CombinedMDSMessagesORSProShmBase,
 ...
 public:
  ...
  void ResetRecoveryIndex(){
    ...
    }else if(ibkr_mds_processor_!=NULL){
      //Sanskar:Implement recovery logic here FOR IBKR
    }
  }
  ...
  inline void AddDataSourceForProcessing(HFSAT::MDS_MSG::MDSMessageExchType _source_, void *_p_book_listener_,
    ...
    switch (_source_) {
      ...
      case HFSAT::MDS_MSG::IBKR: {
        //To check later if hack or not
        control_msg_trigger_count_ = 5;
  ...
  inline void AddComboProductToExchCombinedProcessor(HFSAT::MDS_MSG::MDSMessageExchType _source_,const std::string internal_symbol_str) {
    switch (_source_) {
      case HFSAT::MDS_MSG::IBKR: {
        //Check if ibkr_md_processor exists
        if(ibkr_mds_processor_){
          ibkr_mds_processor_->AddComboProductToSegmentMap(internal_symbol_str);
        }else{
          std::cerr<<"COMBINED_MDS_MESSAGE_IBKR_PROCESSOR is not set"<<std::endl;
        }
      } break;
      default: { std::cerr << " No handling for this source yet \n"; } break;
    }
  ...
  void OnMarketEventDispatch(IBL1UpdateTick* market_event, bool is_timeout) override{

    if(true == is_timeout){
      OnEventsTimeout();
    }else{
      //sent it for book processing
      // DBGLOG_CLASS_FUNC_LINE_INFO << "Direct processor got the data sendingit to ibkr process event" << DBGLOG_ENDL_NOFLUSH;
      // DBGLOG_DUMP;
