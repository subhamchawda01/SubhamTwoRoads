...
class CombinedMDSMessagesORSReplyProcessor {
 private:
  bool is_filtering_enabled_;
  bool is_saci_filtering_enabled;
  std::set<int32_t> ors_base_set_;
  std::set<int32_t> saci_set;
 protected:
  ...
  std::set<std::string> sec_options_set_;
 ...
 public:
  ...
  CombinedMDSMessagesORSReplyProcessor(DebugLogger& _dbglogger_, SecurityNameIndexer& _sec_name_indexer_,
                                       HFSAT::SecurityMarketViewPtrVec& _sid_to_smv_ptr_map_)
      : is_filtering_enabled_(false),
        is_saci_filtering_enabled(false),
        ors_base_set_(),
  ...
  ~CombinedMDSMessagesORSReplyProcessor() {}
  ...
  inline void EnableSaciFiltering(std::vector<int32_t> &saci_vec) {
    is_saci_filtering_enabled = true;
    for ( auto itr : saci_vec) {
      saci_set.insert(itr);
    }

  }

  inline void EnableORSFilteringAndAddORSBase(std::vector<int32_t> ors_saci_vec) {
  ...
  inline void ProcessORSReplyEvent(HFSAT::GenericORSReplyStructLive* next_event_) {
    if (true == is_saci_filtering_enabled) {
      if ( saci_set.find(next_event_->server_assigned_client_id_) == saci_set.end()) return;
    }
    else if (true == is_filtering_enabled_) {
    ...
    if (security_id_ >= 0) {
      if (!using_shm_for_ors_reply_ && false == is_filtering_enabled_) {
        p_time_keeper_->OnTimeReceived(next_event_->time_set_by_server_);
      }
      ...
      switch (next_event_->orr_type_) {
        case kORRType_None: {
          ...
          for (size_t i = 0; i < order_not_found_listener_vec_.size(); i++) {
            order_not_found_listener_vec_[i]->OrderNotFound(
                ...
                next_event_->int_price_, next_event_->server_assigned_message_sequence_,
        ...
        case kORRType_ORSConf: {
          ...
          for (size_t i = 0; i < order_confirmed_listener_vec_.size(); i++) {
            order_confirmed_listener_vec_[i]->OrderORSConfirmed(
                next_event_->server_assigned_client_id_, next_event_->client_assigned_order_sequence_,
                next_event_->server_assigned_order_sequence_, security_id_, next_event_->price_, next_event_->buysell_,
                next_event_->size_remaining_, next_event_->size_executed_, next_event_->int_price_,
        ...
        case kORRType_Rejc_Funds: {
          ...
          std::vector<OrderRejectedDueToFundsListener*>& order_rejected_fund_listener_vec_ =
              order_rejected_due_to_funds_listener_vec_map_[security_id_];

          for (size_t i = 0; i < order_rejected_fund_listener_vec_.size(); i++) {
            order_rejected_fund_listener_vec_[i]->OrderRejectedDueToFunds(
                next_event_->server_assigned_client_id_, next_event_->client_assigned_order_sequence_, security_id_,
                next_event_->price_, next_event_->buysell_, next_event_->size_remaining_, t_rejection_reason_,
                next_event_->int_price_, next_event_->exch_assigned_sequence_, next_event_->time_set_by_server_);
        ...
        case kORRType_Wake_Funds: {
          std::vector<OrderRejectedDueToFundsListener*>& order_rejected_fund_listener_vec_ =
              order_rejected_due_to_funds_listener_vec_map_[security_id_];
          for (size_t i = 0; i < order_rejected_fund_listener_vec_.size(); i++) {
            order_rejected_fund_listener_vec_[i]->WakeUpifRejectedDueToFunds();
          }
        } break;
