...
class OrderGlobalListenerNSE {
 public:
  ...
  virtual void OnPriceLevelUpdate(void* ptr_to_price_level_update, int length_of_bytes,
                                  HFSAT::MDS_MSG::MDSMessageExchType exch_type) {}
  virtual void OnIndexPrice(const unsigned int t_security_id_, double const t_spot_price_) {}
  virtual void OnOpenInterestUpdate(const unsigned int t_security_id_, double const t_spot_price_) {}
};
class OrderGlobalListenerIBKR {
 public:
  virtual ~OrderGlobalListenerIBKR() {}
  virtual void SetTimeToSkipUntilFirstEvent(const ttime_t r_start_time_) = 0;
  virtual void Ib_Tick_Update(const int32_t t_security_id_,  const TradeType_t t_buysell_,const double t_price_, const int32_t t_size_, const bool is_intermediate_message) = 0;
  virtual void Ib_Tick_Size_Only_Update(const int32_t t_security_id_,  const TradeType_t t_buysell_, const int32_t t_size_, const bool is_intermediate_message) = 0;
  virtual void Ib_Trade(const int32_t t_security_id_, const double trade_price_, const int32_t trade_size_, const TradeType_t t_buysell)=0;
};
class NSEBardataListener{

  public :
   virtual ~NSEBardataListener(){}
   virtual void SetTimeToSkipUntilFirstEvent(const ttime_t r_start_time_) {}
   virtual void TriggerBardataUpdate(const uint32_t t_security_id_, const char* buffer) = 0;
   virtual void TriggerOiBardataUpdate(const uint32_t t_security_id_, const char* buffer) = 0;

};

class BSEBardataListener{

  public :
   virtual ~BSEBardataListener(){}
   virtual void SetTimeToSkipUntilFirstEvent(const ttime_t r_start_time_) {}
   virtual void TriggerBardataUpdate(const uint32_t t_security_id_, const char* buffer) = 0;

};

class CBOEBardataListener{

  public :
   virtual ~CBOEBardataListener(){}
   virtual void SetTimeToSkipUntilFirstEvent(const ttime_t r_start_time_) {}
   virtual void TriggerBardataUpdate(const uint32_t t_security_id_, const char* buffer) = 0;
...
class OrderGlobalListenerBSE {
 public:
  virtual ~OrderGlobalListenerBSE() {}
  ...
  virtual void OnOrderModify(const uint32_t t_security_id_, const TradeType_t t_buysell_, 
			     const uint64_t t_prev_order_id_, const uint64_t order_id_, const double t_price_,
                             const uint32_t t_size_, const double t_prev_price_, const uint32_t t_prev_size_) = 0;

  virtual void OnOrderDelete(const uint32_t t_security_id_, const TradeType_t t_buysell_, const uint64_t order_id_,
			     const double t_price_, //const int t_size_, 
			     const bool t_delete_order_, const bool t_is_intermediate_) = 0;

  virtual void OnOrderMassDelete(const uint32_t t_security_id_) = 0;

  virtual void OnTrade(const uint32_t t_security_id_, const TradeType_t t_buysell_, const uint64_t order_id_,
                                       const double t_traded_price_, const int32_t t_traded_size_) = 0;
  ...
  virtual void OnIndexPrice(const unsigned int t_security_id_, double const t_spot_price_) = 0;  

  virtual void OnOpenInterestUpdate(const unsigned int t_security_id_, double const t_oi_price_) = 0;
};
