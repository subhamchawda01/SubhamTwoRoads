...
class OrderLevelSimMarketMaker2 : public BaseSimMarketMaker, public OrderLevelListenerSim, public TimePeriodListener {
 private:
  ...
  std::map<uint64_t, SimMarketOrder*> bid_order_id_to_order_map_;
  std::map<uint64_t, SimMarketOrder*> ask_order_id_to_order_map_;

  SimpleMempool<SimMarketOrder> mkt_order_mempool_;

  int buy_side_trade_level_;
  int sell_side_trade_level_;
  int last_trade_price_bmf_;
 ...
 protected:
  ...
  bool GetBidLevelfromIntPriceAndOrderId(int _t_int_price_, int64_t _t_order_id_, int& _level_);
  bool GetAskLevelfromIntPriceAndOrderId(int _t_int_price_, int64_t _t_order_id_, int& _level_);
 ...
 public:
  ...
  void OnStatusChange(MktStatus_t _this_market_status_) { current_market_status_ = _this_market_status_; }
