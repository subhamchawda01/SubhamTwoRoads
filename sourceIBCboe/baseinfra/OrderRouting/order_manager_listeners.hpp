...
class ExecutionListener {
 public:
  ...
  virtual void OnExec(const int _new_position_, const int _exec_quantity_, const TradeType_t _buysell_,
                      const double _price_, const int r_int_price_, const int _security_id_, const int _caos_ = 0) = 0;
};
...
class ExchangeRejectsListener {
 public:
  ...
  virtual void OnResetByManualInterventionOverRejects(
      HFSAT::BaseUtils::FreezeEnforcedReason const& freeze_reason) = 0;  // this one's manual to assure one took the control
};
...
class OrderChangeListener {
 public:
  ...
  virtual void OnOrderChange(HFSAT::BaseOrder* _order_) = 0;
...
/** @brief interface for listeners of throttle numbers in some window*/
class ThrottleNumberListener {
 public:
  virtual ~ThrottleNumberListener(){};
  virtual void OnThrottleChange(int _num_throttle_, const unsigned int security_id_) = 0;
};

/** @brief interface for listeners of margin numbers from exchange or manually sent*/
class MarginListener {
 public:
  virtual ~MarginListener(){};
  virtual void OnMarginChange(double _current_margin_) = 0;
};

/** @brief interface for listeners of new order reject*/
class OrderRejectListener {
 public:
  virtual ~OrderRejectListener(){};
  virtual void OnOrderReject(HFSAT::BaseOrder* _order_) = 0;
  virtual void OnOrderReject() = 0;
};
