 ...
 public:
  ...
  void thread_main() {
    ...
    */
    ...
    combined_mds_messages_shm_processor_.RunLiveShmSourceConsole();
 ...
 public:
  ORSReplyManager(HFSAT::DebugLogger& dbglogger, HFSAT::Watch& watch, std::string const& shortcode,
    ...
    HFSAT::ORSMessageLiveSource* new_ors_message_livesource = new HFSAT::ORSMessageLiveSource(
        dbglogger_, sec_name_indexer_, "239.23.0.176", 39864, MULTICAST);
...
int main(int argc, char** argv) {
  ...
  } else if(std::string::npos != shortcode.find("CBOE_")){
    HFSAT::SecurityDefinitions::GetUniqueInstance(yyyymmdd).LoadCBOESecurityDefinitions();
    exchange_source = HFSAT::SecurityDefinitions::GetContractExchSource(shortcode, yyyymmdd);
  }
