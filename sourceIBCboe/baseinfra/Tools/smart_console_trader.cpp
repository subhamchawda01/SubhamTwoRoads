...
std::map<int, int> saos_to_cancel_map;
std::vector<std::string> sec_list_vec;
std::string shortcode_;
HFSAT::BaseTrader* basetrader_global;
...
class BookManager : public HFSAT::Thread,
 ...
 private:
  ...
  HFSAT::MDSMessages::CombinedMDSMessagesDirectProcessor* combined_mds_messages_direct_processor_ptr_;
 ...
 public:
  BookManager(HFSAT::DebugLogger& dbglogger, HFSAT::Watch& watch, std::string const& shortcode, int _saci_,
              HFSAT::Utils::ClientLoggingSegmentInitializer* _client_logging_segment_initializer_,HFSAT::MDSMessages::CombinedMDSMessagesMultiShmProcessor& combined_mds_messages_shm_processor,
              HFSAT::MDSMessages::CombinedMDSMessagesDirectProcessor* combined_mds_messages_direct_processor_ptr)
      : dbglogger_(dbglogger),
        watch_(watch),
        shortcode_(shortcode),
        sec_name_indexer_(HFSAT::SecurityNameIndexer::GetUniqueInstance()),
        traded_volume_(0),
        combined_mds_messages_shm_processor_(combined_mds_messages_shm_processor),
        combined_mds_messages_direct_processor_ptr_(combined_mds_messages_direct_processor_ptr),
        market_update_info_(),
  ...
  void ResendCanceledOrder(const int _server_assigned_order_sequence_){
    dbglogger_ << "Resending Cancel Request for Saos " << _server_assigned_order_sequence_ << "\n";
    dbglogger_.DumpCurrentBuffer();
    const char* exch_symbol_ = HFSAT::ExchangeSymbolManager::GetExchSymbol(shortcode_);
    HFSAT::BaseOrder bord;
    bord.ors_ptr_ = 0;
    bord.server_assigned_order_sequence_ = _server_assigned_order_sequence_;

    if (0 == bord.server_assigned_order_sequence_) {
      std::cerr << "Invalid SAOS in Resending Cancel : " << _server_assigned_order_sequence_ << "\n";
      return;
    }

    bord.security_name_ = exch_symbol_;  // not really needed in the ORS but livetrader segfaults without it
    struct timeval tv;
    ...
    basetrader_global->Cancel(bord);
    dbglogger_ << "Cancel Resend For Saos " << _server_assigned_order_sequence_ << "\n";
    dbglogger_.DumpCurrentBuffer();
  }
  ...
  void OrderCancelRejected(const int t_server_assigned_client_id_, const int _client_assigned_order_sequence_,
    ...
    if (t_server_assigned_client_id_ == saci_) {
      ...
      if (HFSAT::kCxlRejectReasonThrottle == (HFSAT::CxlRejectReason_t)_rejection_reason_) {
	      	// exchange_order_id == Wait time
		uint64_t min_throttle_wait_cycle = HFSAT::GetReadTimeStampCounter() + exchange_order_id;
	        dbglogger_ << "Wait Before Sending Order Cancel request Wait: " << min_throttle_wait_cycle <<
		       	" Current: " << HFSAT::GetReadTimeStampCounter() << "\n";
	        dbglogger_.DumpCurrentBuffer();
	        while (true){
			uint64_t this_timestamp = HFSAT::GetReadTimeStampCounter();
  			if (this_timestamp > min_throttle_wait_cycle ) break;
		}
      		if (saos_to_cancel_map.find(_server_assigned_order_sequence_) == saos_to_cancel_map.end()){
      			saos_to_cancel_map[_server_assigned_order_sequence_] = 1;
			//resend the order
			ResendCanceledOrder(_server_assigned_order_sequence_);
      		}
      		else if (saos_to_cancel_map[_server_assigned_order_sequence_] < 9){
	      		saos_to_cancel_map[_server_assigned_order_sequence_]++;
      			//resend the order
			ResendCanceledOrder(_server_assigned_order_sequence_);
      		}
		else{
		       	dbglogger_ << "Order Due to Throttle But max attempts reached "
                 	<< "SACI: " << t_server_assigned_client_id_ << "\n";
  		    	dbglogger_.DumpCurrentBuffer();
		}
      }
  ...
  void thread_main() {
    ...
    HFSAT::IndexedNSEMarketViewManager2 indexed_nse_market_view_manager2_(dbglogger_, watch_, sec_name_indexer_,
                                                                       sid_to_smv_ptr_map, false);
                                                                  
    HFSAT::IndexedBSEMarketViewManager2 indexed_bse_market_view_manager2_(dbglogger_, watch_, sec_name_indexer_,
                                                                       sid_to_smv_ptr_map, false);
    ...
    switch (this_exch_source) {
      case HFSAT::kExchSourceNSE: {
        ...
        combined_mds_messages_direct_processor_ptr_->AddDataSourceForProcessing(
                HFSAT::MDS_MSG::NSE, (void*)((HFSAT::OrderGlobalListenerNSE*)&(indexed_nse_market_view_manager2_)),
                &watch_);
        
        combined_mds_messages_direct_processor_ptr_->AddORSreplySourceForProcessing(HFSAT::MDS_MSG::ORS_REPLY, &watch_,
                                                                        sid_to_smv_ptr_map);

      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderNotFoundListener(sec_id, this);
      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderSequencedListener(sec_id, this);
      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderConfirmedListener(sec_id, this);
      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderConfCxlReplacedListener(sec_id, this);
      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderConfCxlReplaceRejectListener(sec_id, this);
      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderCanceledListener(sec_id, this);
      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderExecutedListener(sec_id, this);
      combined_mds_messages_direct_processor_ptr_->GetProShmORSReplyProcessor()->AddOrderRejectedListener(sec_id, this);
      ...
      case HFSAT::kExchSourceBSE: {
          ...
          combined_mds_messages_shm_processor_.AddDataSourceForProcessing(
            HFSAT::MDS_MSG::BSE, (void*)((HFSAT::OrderGlobalListenerBSE*)&(indexed_bse_market_view_manager2_)), &watch_);

          combined_mds_messages_direct_processor_ptr_->AddDataSourceForProcessing(
                HFSAT::MDS_MSG::BSE, (void*)((HFSAT::OrderGlobalListenerBSE*)&(indexed_bse_market_view_manager2_)),
                &watch_);
    ...
    // Disabling recovering for Console as only top level update is required
    combined_mds_messages_direct_processor_ptr_->ResetRecoveryIndex();
    ...
    combined_mds_messages_shm_processor_.RunUDPDirectConsole();
  }
};
...
void SendOrder(BookManager& book_manager, HFSAT::BaseTrader* tr, const char* inst,
  ...
  try {
    ...
    HFSAT::SurveillanceHandling::CheckProductUnderSurvilance(shortcode_); 
    while (size_ > 0) {
      if (size_ < mts) {
...
int main(int argc, char** argv) {
  ...
  sec_list_vec.push_back(shortcode);
  int prog_id = 1;
  ...
  if (std::string::npos != shortcode.find("NSE_")) {
  ...
  } else if(std::string::npos != shortcode.find("BSE_")){
    ...
    if (std::string::npos != shortcode.find("INR_") || std::string::npos != shortcode.find("_GIND")) {
      ...
      if (std::string::npos != shortcode.find("_FUT") || HFSAT::NSESecurityDefinitions::IsOption(shortcode)) {
        ...
        std::cout << "SELECTING EXECHANGE SOURCE BSE EQ\n";
  ...
  HFSAT::SurveillanceHandling::GetUniqueInstance().GetSurveillanceLogging(shortcode);
  ...
  if (hostnameStr.find("indb") != std::string::npos) {
    exch_type = "BSE";
  }else{
    exch_type = "NSE";
  }
  ...
  HFSAT::MDSMessages::CombinedMDSMessagesMultiShmProcessor combined_mds_messages_multishm_processor_(dbglogger_, sec_name_indexer_, HFSAT::kComShmConsumer,
                                                                                                      udp_direct_multiple_zockets_ptr);
  ...
  if (exch_type == "NSE") {
    ...
    HFSAT::BSEMD::BSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher_, nullptr, HFSAT::kRaw, "BSE",
                                                        true, combined_mds_messages_direct_processor_ptr_, false,cfn_oss_bse.str());
  ...
  BookManager book_manager(dbglogger_, *watch_, shortcode, saci, &client_logging_segment_initializer,combined_mds_messages_multishm_processor_,
                          combined_mds_messages_direct_processor_ptr_);
  book_manager.run();
  ...
  usleep(3000000);  // 1 sec sleep to complete all print statements in initialization of threads & book 
