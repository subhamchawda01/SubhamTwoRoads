...
#define NSESECURITYMARGINFILEPATH "/spare/local/tradeinfo/NSE_Files/SecuritiesMarginFiles/"
...
#define KSRatioFILEPATH "/spare/local/MidtermStrat/"
...
#define SPOT_INDEX_BHAV_FILE "/spare/local/tradeinfo/NSE_Files/SpotIndexBhav/"
...
static std::map<std::string,double> index_closing_price;
...
std::unordered_map<std::string,std::pair<int,double>> shortcode_to_last_close_map_;
std::unordered_map<std::string,std::pair<int,int>> shortcode_to_pnl_pos_map_;
...
HFSAT::SecurityMarketView* niftymidselectspot_smv = NULL;
HFSAT::SecurityMarketView* sensexspot_smv = NULL;
HFSAT::SecurityMarketView* bankexspot_smv = NULL;
HFSAT::SecurityMarketView* spx_smv = NULL;
...
void SimpleBAwithMarket::DumpHeader() {
  ...
  if (bankniftyspot_smv != NULL) {
    ...
    auto itr    = index_closing_price.find("BANKNIFTY");    
     if(itr!=index_closing_price.end())
       spot_chg[0] = double((spot_val[0]-itr->second)/(double)(itr->second)*100);
  }
  if (niftyspot_smv != NULL) {
    spot_val[1] = niftyspot_smv->market_update_info_.mid_price_;
    auto itr    = index_closing_price.find("NIFTY");     
    if(itr!=index_closing_price.end())
      spot_chg[1] = double((spot_val[1]-itr->second)/(double)(itr->second)*100);
  }
  if (finniftyspot_smv != NULL) {
    spot_val[2] = finniftyspot_smv->market_update_info_.mid_price_;
    auto itr    = index_closing_price.find("FINNIFTY");      
    if(itr!=index_closing_price.end())
      spot_chg[2] = double((spot_val[2]-itr->second)/(double)(itr->second)*100);
  }
  ...
  if( niftymidselectspot_smv != NULL ){
    spot_val[5] = niftymidselectspot_smv->market_update_info_.mid_price_;
    auto itr    = index_closing_price.find("NIFTYMIDSELECT");
    if(itr!=index_closing_price.end())
      spot_chg[5] = double((spot_val[5]-itr->second)/(double)(itr->second)*100);
  }
  if (sensexspot_smv != NULL) {
    spot_val[6] = sensexspot_smv->market_update_info_.mid_price_;
    auto itr    = index_closing_price.find("SENSEX");  
    if(itr!=index_closing_price.end())
      spot_chg[6] = double((spot_val[6]-itr->second)/(double)(itr->second)*100);
  }
  if (bankexspot_smv != NULL) {
    spot_val[7] = bankexspot_smv->market_update_info_.mid_price_;
    auto itr    = index_closing_price.find("BANKEX");        
    if(itr!=index_closing_price.end())
      spot_chg[7] = double((spot_val[7]-itr->second)/(double)(itr->second)*100);
  }
  if (spx_smv != NULL) {
    spot_val[8] = spx_smv->market_update_info_.bestbid_price_;
    auto itr    = index_closing_price.find("SPX"); 
    if(itr!=index_closing_price.end())
      spot_chg[8] = double((spot_val[8]-itr->second)/(double)(itr->second)*100);
  }
  ...
  oebu_info_outstream << "jsonstr= [{ "
                      ...
                      << " \"SPOT_NIFTYMIDSELECT\" : \" " << spot_val[5] << "\","
                      << " \"CHG_NIFTYMIDSELECT\" : \" " << spot_chg[5] << "\","
                      << " \"SPOT_SENSEX\" : \" " << spot_val[6] << "\","
                      << " \"CHG_SENSEX\" : \" " << spot_chg[6] << "\","
                      << " \"SPOT_BANKEX\" : \" " << spot_val[7] << "\","
                      << " \"CHG_BANKEX\" : \" " << spot_chg[7] << "\","
                      << " \"SPOT_SPX\" : \" " << spot_val[8] << "\","
                      << " \"CHG_SPX\" : \" " << spot_chg[8] << "\"}]";
...
void SimpleBAwithMarket::DefLoop(unsigned int this_sid_number, const int _trade_px_,
  ...
  */
  ...
  if (m_m_levels >= 1 && std::min(this_smv_.NumBidLevels(), this_smv_.NumAskLevels()) >= 1) {
    ...
    std::string secname = sec_name_indexer_.GetShortcodeFromId(this_sid_number);
    if (secname.find("SENSEX") != std::string::npos || secname.find("BANKEX") != std::string::npos)  {return; }
    switch (this_smv_.secname()[0]) {
      ...
      default: {
        ...
        if (this_smv_.this_smv_exch_source_ == HFSAT::kExchSourceMICEX ||
          ...
          if (this_smv_.exch_source() == HFSAT::kExchSourceBSE) {
            HFSAT::SecurityNameIndexer& sec_name_indexer_ = HFSAT::SecurityNameIndexer::GetUniqueInstance();
            secname = sec_name_indexer_.GetShortcodeFromId(this_sid_number);

            if (std::string::npos != secname.find("BSE_")) {
              secname = secname.substr(secname.find("BSE_") + std::string("BSE_").length());
            }
          }
          if (this_smv_.exch_source() == HFSAT::kExchSourceCBOE) {
            HFSAT::SecurityNameIndexer& sec_name_indexer_ = HFSAT::SecurityNameIndexer::GetUniqueInstance();
            secname = sec_name_indexer_.GetShortcodeFromId(this_sid_number);

            if (std::string::npos != secname.find("CBOE_")) {
              secname = secname.substr(secname.find("CBOE_") + std::string("CBOE_").length());
            }
          }
          ...
          if (temp_secid_baseorder_map_.find(this_sid_number) != temp_secid_baseorder_map_.end()) {
            ...
            if (secname.find("_FUT") != std::string::npos) {
              ...
              if (secname.find("NIFTY_") != std::string::npos && secname.find("SENSEX") != std::string::npos && secname.find("BANKEX") != std::string::npos ) {
          ...
          shortcode_to_pnl_pos_map_[this_smv_.shortcode()]=pair<int,int>((int)prom_pnl_->total_pnl(),prom_pnl_->position() / min_order_size_);
          std::string start = "\"";
          std::string end = "\",";
          ...
          tempstring << start << (double)prom_pnl_->position() / min_order_size_ << end;
          ...
          //std::cout << "Size of map is " << underlying_to_prom_order_manager_map.size() << std::endl; 
          if (false == underlying_to_prom_order_manager_map.empty()) {
            ...
            }
            {
              ...
              for (UnderlyingToPromPnlMapCIter itr = result.first; itr != result.second; ++itr) {
                ...
                auto id = shortcode_to_pnl_pos_map_.find(itr->second->get_shortcode());
                aggregate_pnl += (int)id->second.first;
                //aggregate_pnl += (int)itr->second->total_pnl();
                aggregate_pnl_diff += itr->second->get_pnl_diff(PNL_DIFF_TIME_);
              }
              std::pair<UnderlyingToPromOrderManagerMapCIter, UnderlyingToPromOrderManagerMapCIter> result1 =
              ...
              for (UnderlyingToPromOrderManagerMapCIter itr = result1.first; itr != result1.second; ++itr) {
                ...
                auto id = shortcode_to_pnl_pos_map_.find(itr->second->get_shortcode());
                aggregate_global_pos += (int)id->second.second;
                //aggregate_global_pos += (double)itr->second->global_position() / min_order_size_;
              }
              std::pair<UnderlyingToSimpleBABookMapCIter, UnderlyingToSimpleBABookMapCIter> result3 =
                  underlying_to_simplebabook_map.equal_range(sstream.str());
...
bool CheckAndAddCBOEDefinitions(std::vector<std::string>& t_shortcode_vec_, int trading_date) {
  bool is_cboe_present_ = false;
  for (auto i = 0u; i < t_shortcode_vec_.size(); i++) {
    //    std::cout << " Shc:" << t_shortcode_vec_[i] << "\n";
    if (strncmp(t_shortcode_vec_[i].c_str(), "CBOE_", 5) == 0) {
      is_cboe_present_ = true;
    }
  }
  if (is_cboe_present_) {
    HFSAT::SecurityDefinitions::GetUniqueInstance(trading_date).LoadCBOESecurityDefinitions();
    //LoadNSEMarginValues();
  }
  
  return is_cboe_present_;
}
...
bool CheckAndAddBSEDefinitions(std::vector<std::string>& t_shortcode_vec_, int trading_date) {
  ...
  if (is_bse_present_) {
    HFSAT::SecurityDefinitions::GetUniqueInstance(trading_date).LoadBSESecurityDefinitions();
    //LoadBSEMarginValues("BSE");
  }
  
  return is_bse_present_;
}

void LoadNSEMarginValues() {
  std::cout << "Loading NSE Margin Values" << std::endl;
  HFSAT::SecurityNameIndexer& sec_name_indexer_ = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  ...
  t_temp_oss << NSESECURITYMARGINFILEPATH << "security_margin_" << HFSAT::DateTime::GetCurrentIsoDateLocal() << ".txt";
  ...
  while (mstream.good()) {
    ...
    std::cout << "SHC: " << tokens[0] << "   NSE_SEC_MARGIN " <<  atof(tokens[1]) << std::endl;
...
void LoadBSEMarginValues() {
  std::cout << "Loading BSE Margin Values" << std::endl;
  HFSAT::SecurityNameIndexer& sec_name_indexer_ = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  std::ostringstream t_temp_oss;
    ...
    t_temp_oss << BSESECURITYMARGINFILEPATH << "security_margin_" << HFSAT::DateTime::GetCurrentIsoDateLocal()
               << ".txt";
    ...
    while (mstream.good()) {
      ...
      if (bse_sec_def_.IsEquity(tokens[0])){
        sid_margin_factor_[sec_name_indexer_.GetIdFromString(tokens[0])] = atof(tokens[1]);
        std::cout << "SHC: " << tokens[0] << "   BSE_SEC_MARGIN " <<  atof(tokens[1]) << std::endl;
      }
      else
        sid_margin_factor_[sec_name_indexer_.GetIdFromString(tokens[0])] = atof(tokens[1]) * 0.1;
    }
...
std::map<std::string,UintPair> LoadCMVolumeValue() {
 ...
 std::unordered_map<std::string,std::pair<int,double>> KSRatioValues() {
 
 std::cout << "Calculating INTER PNL " << std::endl;
 std::ostringstream t_temp_oss;
 //t_temp_oss << KSRatioFILEPATH << HFSAT::DateTime::GetCurrentIsoDateLocal() << "/KSRatio";

 t_temp_oss << KSRatioFILEPATH << "KSRatio/" << HFSAT::DateTime::GetCurrentIsoDateLocal();
 std::string ks_product_file = t_temp_oss.str();
 std::cout << "File name is " << ks_product_file << std::endl;

 std::unordered_map<std::string,std::pair<int,double>> shortcode_to_prevClose_map;

  if (HFSAT::FileUtils::ExistsAndReadable(ks_product_file)) {
    std::ifstream product_file_;
    product_file_.open(ks_product_file.c_str(), std::ifstream::in);
    char readline_buffer_[1024];
    if (product_file_.is_open()) {
      while (product_file_.good()) {
        memset(readline_buffer_, 0, sizeof(readline_buffer_));
        product_file_.getline(readline_buffer_, sizeof(readline_buffer_));
        ...
        if (3!=tokens_.size() || strncmp(tokens_[0],"AGGRE",5)) continue;
      
        std::string shortcode = tokens_[0];
        std::string lot_prev_close =  tokens_[2];
    
        int beg = shortcode.find_first_of('_')+1,end = shortcode.size()-1;

        std::string local_shc  = shortcode.substr(beg, end - beg +1);

        int idx = lot_prev_close.find_first_of(',');
	      std::string lot_size = lot_prev_close.substr(0,idx);
	      std::string prev_close = lot_prev_close.substr(idx+1);

        //std::cout << local_shc << " " << lot_size << " " << prev_close  << std::endl;
        int lot_size_ = stoi(lot_size);
        float prev_close_ = stod(prev_close);        

        //shortcode_to_prevClose_map.insert(local_shc,std::make_pair(lot_size_,prev_close_))
        shortcode_to_prevClose_map[local_shc]=pair<int,float>(lot_size_,prev_close_);
    ...
    std::cerr << "Fatal error - could not read KS RATIO file " << ks_product_file << "\n.Exiting.\n";
    //exit(0);
  }
 
 return shortcode_to_prevClose_map;
}
...
void LoadPreviousDayIndexValue(){


 std::cout << "Loading previous day index(spot) closing price " << std::endl;
 std::ostringstream t_temp_oss;

 t_temp_oss << SPOT_INDEX_BHAV_FILE  << HFSAT::HolidayManagerUtils::GetPrevBusinessDayForExchange("NSE", HFSAT::DateTime::GetCurrentIsoDateLocal());
 std::string index_bhav_file = t_temp_oss.str();
 std::cout << "File name is " << index_bhav_file << std::endl;


  if (HFSAT::FileUtils::ExistsAndReadable(index_bhav_file)) {
    std::ifstream product_file_;
    product_file_.open(index_bhav_file.c_str(), std::ifstream::in);
    char readline_buffer_[1024];
    if (product_file_.is_open()) {
      while (product_file_.good()) {
        memset(readline_buffer_, 0, sizeof(readline_buffer_));
        ...
        if (5!=tokens_.size()) continue;
      
        std::string shortcode = tokens_[0];
        double index_prev_close =  stod(tokens_[4]);
        index_closing_price[shortcode]=index_prev_close;
        std::cout << "SHC : " << shortcode << " CP : " <<  index_closing_price[shortcode] << std::endl;
      }
      product_file_.close();
    }
  }
...
int main(int argc, char** argv) {
  ...
  std::string exch_type;
  bool tbt_recovery_allowed = false;
  std::string exchange_ = "BSE";
  ...
  std::cout << "Value of add_pnl is " << add_pnl << std::endl;
  
  if(add_pnl){
    std::cout << "INTER PNL IS TAKEN INTO ACCOUNT" << std::endl;
    shortcode_to_last_close_map_ = KSRatioValues();
    std::cout << "KS Ratio Value added are " << std::endl;
    for(auto id=shortcode_to_last_close_map_.begin();id!=shortcode_to_last_close_map_.end();id++){
      cout << id->first << " " << id->second.first << " " << id->second.second << std::endl;
    }
  }
  else{
    std::cout << "INTER PNL IS NOT TAKEN INTO ACCOUNT" << std::endl;
  }
  std::cout << "INTER PNL CALCULATED" << std::endl;
  ...
  if (machine.find("INDB") != std::string::npos) {
    HFSAT::SecurityDefinitions::GetUniqueInstance(tradingdate_).SetExchangeType("BSE");
    HFSAT::SecurityDefinitions::GetUniqueInstance(tradingdate_).LoadBSESecurityDefinitions();
    exch_type = "BSE";
  }else if(machine.find("IBKR") != std::string::npos){
    HFSAT::SecurityDefinitions::GetUniqueInstance(tradingdate_).SetExchangeType("CBOE");
    HFSAT::SecurityDefinitions::GetUniqueInstance(tradingdate_).LoadCBOESecurityDefinitions();
    exch_type = "CBOE";
    exchange_ = "CBOE";
  }else{
    HFSAT::SecurityDefinitions::GetUniqueInstance(tradingdate_).SetExchangeType("NSE");
  ...
  while (!sec_file_.eof()) {
    ...
    auto element_present  = std::find(sec_list_vec.begin(),sec_list_vec.end(),tokens[0]);
    if(element_present != sec_list_vec.end()){
       std::cout << "DUPLICATE ENTRIES FOUND IGNORING " << tokens[0] << std::endl;
       continue;
    }
  ...
  //std::cout << "DISABLING PACKET_DROP_INFO & SANITIZE LOGS FOR OEBU" << std::endl;
  dbglogger_.AddLogLevel(SKIP_SANITIZE_INFO);
  dbglogger_.AddLogLevel(SKIP_PACKET_DROPS_INFO);
  ...
  std::cout << "SHORTCODE ADDED : " << sec_list_vec.size() << std::endl;
  ...
  if (!timeout_config_file_.is_open()) {
    std::cerr << " Can't open oebu timeout config file : " << OEBU_MDS_ORS_TIMEOUT_CONFIG << "\n";
    exit(-1);
  }
  ...
  for (unsigned int ii = 0; ii < (unsigned int)sec_name_indexer_.NumSecurityId(); ii++) {
    ...
    } else if(p_smv_->shortcode() == "NSE_NIFTYMIDSELECT" ){
      std::cout << "SMV NIFTYMIDSELECT CREATED" << std::endl;
      niftymidselectspot_smv = p_smv_;
    } else if (p_smv_->shortcode() == "BSE_SENSEX") {
      std::cout << "SMV SENSEX CREATED" << std::endl;
      sensexspot_smv = p_smv_;
    } else if (p_smv_->shortcode() == "BSE_BANKEX") {
      std::cout << "SMV BANKEX CREATED" << std::endl;
      bankexspot_smv = p_smv_;
    }else if( p_smv_->shortcode() == "CBOE_SPX"){
      std::cout << "SMV SPX CREATED" << std::endl;
      spx_smv = p_smv_;
    }
  }
  ...
  HFSAT::IBKRL1PriceMarketViewManager ibkr_l1_price_market_view_manager_(dbglogger_, watch_, sec_name_indexer_,sid_to_smv_ptr_map_); 

  HFSAT::IndexedBSEMarketViewManager2 indexed_bse_market_view_manager_(dbglogger_, watch_, sec_name_indexer_,
                                                                       sid_to_smv_ptr_map_, false);
  ...
  HFSAT::MDSMessages::CombinedMDSMessagesDirectProcessor combined_mds_messages_direct_processor_ (
        dbglogger_, sec_name_indexer_, HFSAT::kComShmConsumer);

  HFSAT::MDSMessages::CombinedMDSMessagesDirectProcessor* combined_mds_messages_direct_processor_ptr_ = &combined_mds_messages_direct_processor_;

  HFSAT::IndexedNSEMarketViewManager2* indexed_nse_market_view_manager = &indexed_nse_market_view_manager_;
  HFSAT::IndexedBSEMarketViewManager2* indexed_bse_market_view_manager = &indexed_bse_market_view_manager_;

  if (machine.find("INDB") != std::string::npos) {
  combined_mds_messages_direct_processor_ptr_->AddDataSourceForProcessing(
                HFSAT::MDS_MSG::BSE, (void*)((HFSAT::OrderGlobalListenerBSE*)(indexed_bse_market_view_manager)),
                &watch_);
  }else if (machine.find("IND") != std::string::npos){
    combined_mds_messages_direct_processor_ptr_->AddDataSourceForProcessing(
                HFSAT::MDS_MSG::NSE, (void*)((HFSAT::OrderGlobalListenerNSE*)(indexed_nse_market_view_manager)),
                &watch_);
  }else{
    combined_mds_messages_direct_processor_ptr_->AddDataSourceForProcessing(
                HFSAT::MDS_MSG::IBKR, (void*)((HFSAT::OrderGlobalListenerIBKR*)(&ibkr_l1_price_market_view_manager_)),
                &watch_);
  }

  dbglogger_ << "SETTING OEBU FLAG TRUE IN COMBINED MDS MESSAGE DIRECT PROCESSOR\n";                                
  combined_mds_messages_direct_processor_ptr_->SetOebuMode(true);                
  dbglogger_.DumpCurrentBuffer();   
  ...
  if("NSE" == exch_type) {
    ...
    cfn_oss_nse << "/spare/local/files/NSE/nse-tbt-mcast-" << prog_id << ".txt";
  ...
  if("BSE" == exch_type){
    ...
    cfn_oss_bse << "/spare/local/files/BSE/bse-tbt-mcast-" << prog_id << ".txt";
  ...
  //udp_direct_multiple_zockets_ptr->AddEventTimeoutNotifyListener(combined_mds_messages_direct_processor_);

 HFSAT::MDSMessages::CombinedMDSMessagesMultiShmProcessor combined_mds_messages_multishm_processor_(
      dbglogger_, sec_name_indexer_, HFSAT::kComShmConsumer, &combined_mds_messages_shm_processor_,udp_direct_multiple_zockets_ptr);

  if (exch_type == "NSE") {
    
     HFSAT::NSEMD::NSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher_, nullptr, HFSAT::kRaw, "NSE",
                                                      true, combined_mds_messages_direct_processor_ptr_, cfn_oss_nse.str());
    
      HFSAT::NSEMD::NSETBTDataProcessor &nse_tbt_data_processor = HFSAT::NSEMD::NSETBTDataProcessor::GetUniqueInstance(
      dbglogger_, HFSAT::kRaw, nullptr, combined_mds_messages_direct_processor_ptr_);
      for (auto shc : sec_list_vec) {
        nse_tbt_data_processor.AddShortCodeForProcessing(shc);
      }
  }
  else if (exch_type == "BSE") {
    HFSAT::BSEMD::BSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher_, nullptr, HFSAT::kRaw, exchange_,
                                                        true, combined_mds_messages_direct_processor_ptr_, tbt_recovery_allowed,cfn_oss_bse.str());
    ...
    HFSAT::IBKRMD::IBKRL1MDHandler &ibkr_data_processor = HFSAT::IBKRMD::IBKRL1MDHandler::GetUniqueInstance(
        dbglogger_, simple_live_dispatcher_, HFSAT::kRaw, &combined_mds_messages_multishm_processor_);
    ibkr_data_processor.SetBardataPeriod(0);

  }
  
  combined_mds_messages_direct_processor_ptr_->AddControlSourceForProcessing(HFSAT::MDS_MSG::CONTROL,1110,NULL,&watch_);
  ...
  if (use_nse_l1) {
    ...
    combined_mds_messages_multishm_processor_.AddDataSourceForProcessing(
        HFSAT::MDS_MSG::IBKR, (void*)((HFSAT::OrderGlobalListenerIBKR*)&(ibkr_l1_price_market_view_manager_)), &watch_);
  }
  ...
  if (USE_COMBINED_SOURCE) {
    combined_mds_messages_shm_processor_.AddORSreplySourceForProcessing(HFSAT::MDS_MSG::ORS_REPLY, &watch_,sid_to_smv_ptr_map_);
  ...
  for (unsigned int ii = 0; ii < (unsigned int)sec_list_vec.size(); ii++) {
    ...
    if (HFSAT::CBOESecurityDefinitions::IsOption(sec_list_vec[ii])) option_flag = true;
    if (HFSAT::BSESecurityDefinitions::IsOption(sec_list_vec[ii])) option_flag = true;
  ...
  if (exch_type == "NSE") {
    shortcode_to_strike_price_ = HFSAT::NSESecurityDefinitions::GetShortcode2StrikePriceMap();
  }
  else if(exch_type == "BSE") {
    shortcode_to_strike_price_ = HFSAT::BSESecurityDefinitions::GetShortcode2StrikePriceMap();
  }else{
    shortcode_to_strike_price_ = HFSAT::CBOESecurityDefinitions::GetShortcode2StrikePriceMap();
  }
  ...
  for (unsigned int ii = 0; ii < (unsigned int)sec_list_vec.size(); ii++) {
    ...
    if (exch_type == "BSE") {
      if (tokens_.size() > 2 &&
          (strncmp(tokens_[2], "FUT", 3) == 0 || HFSAT::BSESecurityDefinitions::IsOption(_this_shortcode_))) {
        std::ostringstream ss1;
        ss1 << tokens_[0] << "_" << tokens_[1] << "_FUT" << expiry_num << "_W";
        min_order_size =
          HFSAT::SecurityDefinitions::GetContractMinOrderSize(ss1.str(), HFSAT::DateTime::GetCurrentIsoDateLocal());
      }
    }
    ...
    p_prom_pnl_map_[_this_shortcode_.c_str()] = 
    ...
    std::cout << "SHORTCODE IS " << _this_shortcode_.c_str() << std::endl; 
    auto id = shortcode_to_last_close_map_.find(_this_shortcode_.c_str());

    if(id!=shortcode_to_last_close_map_.end()){
      cout << id->first << " " << id->second.first << " " << id->second.second << std::endl;
      HFSAT::TradeType_t _buysell_ = HFSAT::kTradeTypeNoInfo;
      double _trade_px_ = id->second.second;
      int position = abs(id->second.first);

      if(id->second.first>0)
        _buysell_ = HFSAT::kTradeTypeBuy;
      else
        _buysell_ = HFSAT::kTradeTypeSell;

      cout << static_cast<std::underlying_type<HFSAT::TradeType_t>::type>(_buysell_) << " " << _trade_px_ << std::endl; 

      p_prom_pnl_map_[_this_shortcode_.c_str()]->OnGlobalOrderExec(_this_sid_,_buysell_,position,id->second.second);
      //p_prom_pnl_map_[_this_shortcode_.c_str()]->get_prom_order_manager().set_global_position(id->second.first);
      //p_prom_pnl_map_[_this_shortcode_.c_str()]->OnGlobalOrderExec(ii,_buysell_,id->second.first,id->second.second);
    }
    shortcode_to_pnl_pos_map_[_this_shortcode_]=pair<int,int>(0,0);
  ...
  //mandate no recovery for oebus
  combined_mds_messages_multishm_processor_.ResetRecoveryIndex();
  combined_mds_messages_direct_processor_.ResetRecoveryIndex();

  std::cout << "EXCHANGE SELECTED " << exch_type << std::endl;
  
  std::cout << "ALLOCATING CORE " << std::endl;
   
  if("CBOE" != exchange_){
    HFSAT::AllocateCPUUtils::GetUniqueInstance().AllocateCPUOrExit(
      "our_extended_bidask_mkt_book_util_generic_web_multishm_all");
  }
  
  std::cout << "CORE ALLOCATION COMPLETED" << std::endl;

  if(exch_type == "NSE"){
    std::cout << "RUNNING NSE OEBU FROM UDP ZOCKETS" << std::endl;
    combined_mds_messages_multishm_processor_.RunUDPDirectNSEOEBU();
  }
  else if (exch_type == "BSE"){
    std::cout << "RUNNING BSE OEBU FROM UDP ZOCKETS" << std::endl;
    combined_mds_messages_multishm_processor_.RunUDPDirectBSEOEBU();
  }else{
