...
void SendOrder(BookManager& book_manager, HFSAT::BaseTrader* tr, const char* inst,
  ...
  try {
    ...
    while (size_ > 0) {
      ...
      HFSAT::RuntimeProfiler::GetUniqueInstance().Start(HFSAT::ProfilerType::kCOMBINEDSHMWRITER);
      tr->SendTrade(bord);
...
//  HFSAT::RuntimeProfiler::GetUniqueInstance(HFSAT::ProfilerType::kCOMBINEDSHMWRITER).Start(time_info);
