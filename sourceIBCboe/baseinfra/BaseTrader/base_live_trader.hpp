 ...
 public:
  BaseLiveTrader(const HFSAT::ExchSource_t& _exchange_, const std::string& ors_account, const std::string& ors_ip,
      ...
      : client_id_(-1),
        ...
        is_bmf_exchange_(false),
        send_order_request_(),
        order_req_(send_order_request_.data),
        runtime_profiler_(HFSAT::RuntimeProfiler::GetUniqueInstance()) {
    if (HFSAT::kExchSourceBMF == _exchange_ || HFSAT::kExchSourceBMFEQ == _exchange_ ||
        HFSAT::kExchSourceNTP == _exchange_ || HFSAT::kExchSourcePUMA == _exchange_ ||
        HFSAT::kExchSourceBMF_FPGA == _exchange_ || kExchSourceBMF_MULTISHM == _exchange_) {
      is_bmf_exchange_ = true;
    }
    ...
    if (null_trader_client_id_ > 0) {  // For null - traders , do not connect to shm / tcp space.
      ...
      if (use_shared_mem) {
        ...
        if (client_id_ == -1) {
          ...
          dbglogger_ << "RMC_SACI: " << client_id_ << " (logging for monitoring)\n";  // Do not change logging format
  ...
  inline void SendTrade(const BaseOrder& _order_) {
    ...
    order_req_.is_mirror_order_ = _order_.is_mirror_order_;
    order_req_.mirror_factor_ = _order_.mirror_factor_;

    order_req_.client_request_time_ = watch_.tv();
    order_req_.is_reserved_type_ = (false == is_bmf_exchange_) ? _order_.is_reserved_type_ : false;

    //runtime_profiler_.End(HFSAT::ProfilerType::kTRADEINIT);
    ...
    order_req_.t2t_cshmw_end_time_ = 0;
    order_req_.t2t_tradeinit_start_time_ = 0;
    order_req_.t2t_tradeinit_end_time_ = 0;
    order_req_.query_order_ptr_ = _order_.this_ptr_;
  ...
  inline void UpdateThrottle(int throttle_number, int ioc_throttle_number) {
    order_req_.orq_request_type_  = ORQ_THROTTLE_UPDATE;
    order_req_.price_ = throttle_number;
    order_req_.int_price_ = ioc_throttle_number;
    std::cout << "DIS: " << order_req_.price_ << " " << order_req_.int_price_ << std::endl;
    shm_writer_->writeT(send_order_request_);
  }
  ...
  inline void Cancel(const BaseOrder& _order_) {
    ...
    order_req_.price_ = _order_.price_;
    order_req_.is_mirror_order_ = false;
    order_req_.mirror_factor_ = 1;
    //@also set the int_price else (it gets -1)
    order_req_.int_price_ = _order_.int_price_;
    ...
    order_req_.t2t_tradeinit_end_time_ = 0;
    order_req_.query_order_ptr_ = _order_.this_ptr_;
    order_req_.ors_order_ptr_ = _order_.ors_ptr_;
