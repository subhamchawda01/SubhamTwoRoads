...
void OrderLevelSimMarketMaker2::OnTimePeriodUpdate(const int num_pages_to_add_) {
  if ((!all_requests_.empty()) && (current_market_status_ == kMktTradingStatusOpen)) {
    ProcessRequestQueue();
  }
}
...
void OrderLevelSimMarketMaker2::AddMarketBid(int int_price, SimMarketOrder* market_order) {
  ...
  bid_order_id_to_order_map_[market_order->order_id_] = market_order;
}
...
void OrderLevelSimMarketMaker2::AddMarketAsk(int int_price, SimMarketOrder* market_order) {
  ...
  ask_order_id_to_order_map_[market_order->order_id_] = market_order;
}
...
inline void OrderLevelSimMarketMaker2::DeleteMarketBid(int t_int_price_, int64_t t_order_id_) {
  ...
  bid_order_id_to_order_map_.erase(t_order_id_);
...
inline void OrderLevelSimMarketMaker2::DeleteMarketAsk(int t_int_price_, int64_t t_order_id_) {
  ...
  ask_order_id_to_order_map_.erase(t_order_id_);
...
void OrderLevelSimMarketMaker2::OnOrderAddBmf(const unsigned int t_security_id_, const TradeType_t t_buysell_,
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      ...
      if ((t_level_ == 1) && (intpx_2_bid_mkt_order_vec_.begin()->first >= int_price_)) {
        if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
          DBGLOG_TIME_CLASS_FUNC << " ERROR: BMF Bid Sanitization Deleting all levels above " << int_price_
                                 << " as the current level is " << t_level_
        ...
        while (intpx_2_bid_mkt_order_vec_.size() > 0 && intpx_2_bid_mkt_order_vec_.begin()->first >= int_price_) {
          ...
          std::vector<SimMarketOrder*> market_order_vec_ = it_->second;
          for (auto ord : market_order_vec_) {
            bid_order_id_to_order_map_.erase(ord->order_id_);
          }
          intpx_2_bid_mkt_order_vec_.erase(it_);
        }
      }
      ...
      // Check if any of our sim orders (orders sent by the query) match with this order. If yes, execute the matching
      // sim orders.
      if (current_market_status_ == kMktTradingStatusOpen) {
        //        ExecuteAsksAboveIntPrice(int_price_);
    ...
    case kTradeTypeSell: {
      // BMF specific sanitization
      if ((t_level_ == 1) &&
          (intpx_2_ask_mkt_order_vec_.begin()->first <= int_price_))  // TODO: do something better for sanitization
      {
        if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
          DBGLOG_TIME_CLASS_FUNC << " ERROR: BMF Ask Sanitization Deleting all levels above " << int_price_
                                 << " as the current level is " << t_level_
                                 << " order_id: " << (unsigned long long)t_order_id_ << DBGLOG_ENDL_FLUSH;
        }
        ...
        while (intpx_2_ask_mkt_order_vec_.size() > 0 && intpx_2_ask_mkt_order_vec_.begin()->first <= int_price_) {
          ...
          std::vector<SimMarketOrder*> market_order_vec_ = it_->second;
          for (auto ord : market_order_vec_) {
            ask_order_id_to_order_map_.erase(ord->order_id_);
          }
          intpx_2_ask_mkt_order_vec_.erase(it_);
        }
      }
  ...
  if ((current_market_status_ == kMktTradingStatusOpen) && (intpx_2_bid_mkt_order_vec_.size() > 0) &&
      (intpx_2_ask_mkt_order_vec_.size() > 0)) {
    if (intpx_2_bid_mkt_order_vec_.begin()->first >= intpx_2_ask_mkt_order_vec_.begin()->first) {
      DBGLOG_TIME_CLASS_FUNC << " ERROR: Book Crossed in Non Auction Market BidTop: "
                             << intpx_2_bid_mkt_order_vec_.begin()->first
                             << " AskTop: " << intpx_2_ask_mkt_order_vec_.begin()->first << " Sanitizing "
                             << ((t_buysell_ == kTradeTypeSell) ? "Buy Side" : "Sell Side") << DBGLOG_ENDL_FLUSH;
      if (t_buysell_ == kTradeTypeSell) {
        int top_ask_ = intpx_2_ask_mkt_order_vec_.begin()->first;
        while (intpx_2_bid_mkt_order_vec_.size() > 0 && intpx_2_bid_mkt_order_vec_.begin()->first >= top_ask_) {
          auto it_ = intpx_2_bid_mkt_order_vec_.begin();
          int this_int_price_ = it_->first;
          ...
          std::vector<SimMarketOrder*> market_order_vec_ = it_->second;
          for (auto ord : market_order_vec_) {
            dbglogger_ << " Deleting Order Id: " << ord->order_id_ << " @ " << this_int_price_ << "\n";
            bid_order_id_to_order_map_.erase(ord->order_id_);
          }
          intpx_2_bid_mkt_order_vec_.erase(it_);
        }
      } else if (t_buysell_ == kTradeTypeBuy) {
        int top_bid_ = intpx_2_bid_mkt_order_vec_.begin()->first;
        while (intpx_2_ask_mkt_order_vec_.size() > 0 && intpx_2_ask_mkt_order_vec_.begin()->first <= top_bid_) {
          auto it_ = intpx_2_ask_mkt_order_vec_.begin();
          int this_int_price_ = it_->first;
...
void OrderLevelSimMarketMaker2::OnOrderModifyBmf(const unsigned int t_security_id_, const TradeType_t t_buysell_,
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      if (bid_order_id_to_order_map_.find(t_old_order_id_) != bid_order_id_to_order_map_.end()) {
        if (t_level_ != 1) {
          DBGLOG_TIME_CLASS_FUNC << "ERROR: Buy Modify of existing order id: " << t_old_order_id_
                                 << " but level is not 1" << DBGLOG_ENDL_FLUSH;
        }

        auto bid_order_ = bid_order_id_to_order_map_[t_old_order_id_];

        // If size is decreased no need for loss of priority
        if ((bid_order_->int_price_ == int_price_)) {
          if (bid_order_->size_ <= t_old_size_) {
            DBGLOG_TIME_CLASS_FUNC << "ERROR: Buy Trade Modifies size: " << t_old_size_
                                   << " is greater than order size:" << bid_order_->size_ << DBGLOG_ENDL_FLUSH;
          }
          bid_order_->size_ = t_old_size_;
          int level_old_ = 0;
          bool order_found_ = GetBidLevelfromIntPriceAndOrderId(bid_order_->int_price_, t_old_order_id_, level_old_);
          if (!order_found_) {
            DBGLOG_TIME_CLASS_FUNC
                << "ERROR: Buy Order Modify BMF. Order exist in order Map but not in price map order_id: "
                << (unsigned long long)t_old_order_id_ << DBGLOG_ENDL_FLUSH;
            level_old_ = -1;
          }

          if (level_old_ > 1) {
            DBGLOG_TIME_CLASS_FUNC << "ERROR: Sanitzing top " << (level_old_ - 1)
                                   << " Bid levels since current level is one" << DBGLOG_ENDL_FLUSH;
            while (level_old_ > 0) {
              auto order_ = GetBuyOrderFromLevel(1);
              DeleteMarketBid(order_->int_price_, order_->order_id_);
              level_old_--;
            }
          }
        } else {
          DBGLOG_TIME_CLASS_FUNC << "ERROR: buy order price: " << bid_order_->int_price_
                                 << " not same as message price: " << t_old_price_ << DBGLOG_ENDL_FLUSH;
        }
      } else {
        auto bid_order_ = GetBuyOrderFromLevel(t_level_);
        if (bid_order_ != NULL) {
          if (bid_order_->int_price_ != int_price_) {
            DBGLOG_TIME_CLASS_FUNC << " ERROR: Non trade buy change is at different price: " << int_price_
                                   << " as compared to level price: " << bid_order_->int_price_
                                   << " order id: " << (unsigned long long)t_old_order_id_ << DBGLOG_ENDL_FLUSH;

          } else {
            if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
              DBGLOG_TIME_CLASS_FUNC << " BMF Buy Modify Order for new "
                                     << " order_id: " << (unsigned long long)t_old_order_id_
                                     << " prev_order_id: " << bid_order_->order_id_ << DBGLOG_ENDL_FLUSH;
            }
            if (bid_order_->size_ <= t_old_size_) {
              DBGLOG_TIME_CLASS_FUNC << "ERROR: Buy Modifies size: " << t_old_size_
                                     << " is greater than order size:" << bid_order_->size_ << DBGLOG_ENDL_FLUSH;
            }
            bid_order_->size_ = t_old_size_;
            bid_order_id_to_order_map_[t_old_order_id_] = bid_order_;
            bid_order_id_to_order_map_.erase(bid_order_->order_id_);
            bid_order_->order_id_ = t_old_order_id_;
          }
        }
      }
      LogBidSimOrder(5, t_old_size_, int_price_);
    ...
    case kTradeTypeSell: {
      if (ask_order_id_to_order_map_.find(t_old_order_id_) != ask_order_id_to_order_map_.end()) {
        if (t_level_ != 1) {
          DBGLOG_TIME_CLASS_FUNC << "ERROR: Sell Modify of existing order id: " << t_old_order_id_
                                 << " but level is not 1" << DBGLOG_ENDL_FLUSH;
        }

        auto ask_order_ = ask_order_id_to_order_map_[t_old_order_id_];

        // If size is decreased no need for loss of priority
        if ((ask_order_->int_price_ == int_price_)) {
          if (ask_order_->size_ <= t_old_size_) {
            DBGLOG_TIME_CLASS_FUNC << "ERROR: Sell Trade Modifies size: " << t_old_size_
                                   << " is greater than order size:" << ask_order_->size_ << DBGLOG_ENDL_FLUSH;
          }
          ask_order_->size_ = t_old_size_;
          int level_old_ = 0;
          bool order_found_ = GetAskLevelfromIntPriceAndOrderId(ask_order_->int_price_, t_old_order_id_, level_old_);
          if (!order_found_) {
            DBGLOG_TIME_CLASS_FUNC
                << "ERROR: Sell Order Modify BMF. Order exist in order Map but not in price map order_id: "
                << (unsigned long long)t_old_order_id_ << DBGLOG_ENDL_FLUSH;
            level_old_ = -1;
          }
          ...
          if (level_old_ > 1) {
            DBGLOG_TIME_CLASS_FUNC << "ERROR: Sanitzing top " << (level_old_ - 1)
                                   << " Ask levels since current level is one" << DBGLOG_ENDL_FLUSH;
            while (level_old_ > 0) {
              auto order_ = GetSellOrderFromLevel(1);
              DeleteMarketAsk(order_->int_price_, order_->order_id_);
              level_old_--;
            }
          }
        } else {
          DBGLOG_TIME_CLASS_FUNC << "ERROR: sell order price: " << ask_order_->int_price_
                                 << " not same as message price: " << t_old_price_ << DBGLOG_ENDL_FLUSH;
        }
      } else {
        auto ask_order_ = GetSellOrderFromLevel(t_level_);
        if (ask_order_ != NULL) {
          if (ask_order_->int_price_ != int_price_) {
            DBGLOG_TIME_CLASS_FUNC << " ERROR: Non trade sell change is at different price: " << int_price_
                                   << " as compared to level price: " << ask_order_->int_price_
                                   << " order id: " << (unsigned long long)t_old_order_id_ << DBGLOG_ENDL_FLUSH;

          } else {
            if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
              DBGLOG_TIME_CLASS_FUNC << " BMF Sell Modify Order for new "
                                     << " order_id: " << (unsigned long long)t_old_order_id_
                                     << " prev_order_id: " << ask_order_->order_id_ << DBGLOG_ENDL_FLUSH;
            }
            if (ask_order_->size_ <= t_old_size_) {
              DBGLOG_TIME_CLASS_FUNC << "ERROR: Sell Modifies size: " << t_old_size_
                                     << " is greater than order size:" << ask_order_->size_ << DBGLOG_ENDL_FLUSH;
            }
            ask_order_->size_ = t_old_size_;
            ask_order_id_to_order_map_[t_old_order_id_] = ask_order_;
            ask_order_id_to_order_map_.erase(ask_order_->order_id_);
            ask_order_->order_id_ = t_old_order_id_;
          }
        }
      }
      LogAskSimOrder(5, t_old_size_, int_price_);
...
bool OrderLevelSimMarketMaker2::GetBidLevelfromIntPriceAndOrderId(int _t_int_price_, int64_t _t_order_id_,
                                                                  int& _level_) {
  _level_ = 0;
  for (auto it_ = intpx_2_bid_mkt_order_vec_.begin(); it_ != intpx_2_bid_mkt_order_vec_.end(); it_++) {
    if (it_->first > _t_int_price_) {
      _level_ += it_->second.size();
    } else if (it_->first == _t_int_price_) {
      for (auto ord : it_->second) {
        _level_++;
        if (ord->order_id_ == _t_order_id_) {
          return true;
        }
      }
    } else {
...
bool OrderLevelSimMarketMaker2::GetAskLevelfromIntPriceAndOrderId(int _t_int_price_, int64_t _t_order_id_,
                                                                  int& _level_) {
  _level_ = 0;
  for (auto it_ = intpx_2_ask_mkt_order_vec_.begin(); it_ != intpx_2_ask_mkt_order_vec_.end(); it_++) {
    if (it_->first < _t_int_price_) {
...
void OrderLevelSimMarketMaker2::OnOrderDeleteBmf(const unsigned int security_id, const TradeType_t buysell,
  ...
  switch (buysell) {
    case kTradeTypeBuy: {
      if (bid_order_id_to_order_map_.find(order_id) != bid_order_id_to_order_map_.end()) {
        auto bid_order_ = bid_order_id_to_order_map_[order_id];
        if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
          int level_ = 0;
          int order_price_ = 0;
          for (auto it_ = intpx_2_bid_mkt_order_vec_.begin(); it_ != intpx_2_bid_mkt_order_vec_.end(); it_++) {
            ...
            for (size_t i = 0; i < market_order_vec_.size(); i++) {
              level_++;
              if (market_order_vec_[i]->order_id_ == order_id) {
                order_price_ = market_order_vec_[i]->int_price_;
                break_from_loop_ = true;
                break;
              }
            }
          ...
          if (level_ != level) {
            if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
              DBGLOG_TIME_CLASS_FUNC << " bid level does not match at int_px: " << int_price_ << ". Expected: " << level
                                     << " Actual: " << level_ << " order_id: " << (unsigned long long)order_id
                                     << " order px: " << order_price_ << DBGLOG_ENDL_FLUSH;
            }
          }
        }
        DeleteMarketBid(bid_order_->int_price_, order_id);
        if (dbglogger_.CheckLoggingLevel(OLSMM_INFO)) {
          LogBidSimOrder(2, bid_order_->size_, bid_order_->int_price_);
        }
      } else {
        DBGLOG_TIME_CLASS_FUNC << " ERROR: buy order id: " << order_id << " not present in order map"
                               << DBGLOG_ENDL_FLUSH;
        if (dbglogger_.CheckLoggingLevel(OLSMM_INFO)) {
          LogBidSimOrder(2, size, int_price_);
        }
      }
    ...
    case kTradeTypeSell: {
      if (ask_order_id_to_order_map_.find(order_id) != ask_order_id_to_order_map_.end()) {
        auto ask_order_ = ask_order_id_to_order_map_[order_id];
        if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
          int level_ = 0;
          int order_price_ = 0;
          ...
          if (level_ != level) {
            if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
              DBGLOG_TIME_CLASS_FUNC << " ask level does not match at int_px: " << int_price_ << ". Expected: " << level
                                     << " Actual: " << level_ << " order_id: " << (unsigned long long)order_id
                                     << " order px: " << order_price_ << DBGLOG_ENDL_FLUSH;
            }
          }
        }
        DeleteMarketAsk(ask_order_->int_price_, order_id);
        if (dbglogger_.CheckLoggingLevel(OLSMM_INFO)) {
          LogAskSimOrder(2, ask_order_->size_, ask_order_->int_price_);
        }
      } else {
        DBGLOG_TIME_CLASS_FUNC << " ERROR: ask order id: " << order_id << " not present in order map"
                               << DBGLOG_ENDL_FLUSH;
        if (dbglogger_.CheckLoggingLevel(OLSMM_INFO)) {
          LogBidSimOrder(2, size, int_price_);
        }
...
void OrderLevelSimMarketMaker2::OnOrderLevelDeleteFrom(const unsigned int t_security_id_, const TradeType_t t_buysell_,
                                                       const int t_min_level_deleted_,
                                                       const bool t_is_intermediate_message_) {
  if (dbglogger_.CheckLoggingLevel(OLSMM_INFO)) {
    DBGLOG_TIME_CLASS_FUNC << " DEL_FROM " << ((t_buysell_ == kTradeTypeBuy) ? " BUY " : " SELL ")
                           << t_min_level_deleted_ << DBGLOG_ENDL_FLUSH;
  }

  if (last_trade_price_bmf_ == 0) {
    DBGLOG_TIME_CLASS_FUNC << "ERROR: DEL_FROM message is not coming after Trade message" << DBGLOG_ENDL_FLUSH;
  }

  switch (t_buysell_) {
    case kTradeTypeBuy: {
      int level_ = 0;
      while (intpx_2_bid_mkt_order_vec_.size() > 0) {
        auto it_ = intpx_2_bid_mkt_order_vec_.begin();
        if (it_->first < last_trade_price_bmf_) {
          if (level_ < t_min_level_deleted_) {
            if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
              DBGLOG_TIME_CLASS_FUNC << "ERROR: Bid Delete Through Level: " << t_min_level_deleted_
                                     << " going beyond trade price " << last_trade_price_bmf_
                                     << ". Levels Till Now: " << level_ << DBGLOG_ENDL_FLUSH;
            }
          }
          break;
        }
        ...
        bool break_from_loop_ = false;
        for (size_t i = 0; i < market_order_vec_.size(); i++) {
          if ((level_ >= t_min_level_deleted_) && (it_->first <= last_trade_price_bmf_)) {
            break_from_loop_ = true;
            break;
          }
          level_++;
          DeleteMarketBid(it_->first, market_order_vec_[i]->order_id_);
        }
        if (break_from_loop_) {
          break;
        }
      ...
      if (level_ > t_min_level_deleted_) {
        DBGLOG_TIME_CLASS_FUNC << "ERROR: Bid Delete Through Level: " << t_min_level_deleted_
                               << " is less than total level deleted: " << level_ << " price: " << last_trade_price_bmf_
                               << DBGLOG_ENDL_FLUSH;
      }
    } break;
    case kTradeTypeSell: {
      int level_ = 0;
      while (intpx_2_ask_mkt_order_vec_.size() > 0) {
        auto it_ = intpx_2_ask_mkt_order_vec_.begin();
        if (it_->first > last_trade_price_bmf_) {
          if (level_ < t_min_level_deleted_) {
            if (dbglogger_.CheckLoggingLevel(PLSMM_INFO)) {
              DBGLOG_TIME_CLASS_FUNC << "ERROR: Ask Delete Through Level: " << t_min_level_deleted_
                                     << " going beyond trade price " << last_trade_price_bmf_
                                     << ". Levels Till Now: " << level_ << DBGLOG_ENDL_FLUSH;
            }
          }
        ...
        for (size_t i = 0; i < market_order_vec_.size(); i++) {
          ...
          level_++;
          DeleteMarketAsk(it_->first, market_order_vec_[i]->order_id_);
        }
        if (break_from_loop_) {
          break;
        }
...
void OrderLevelSimMarketMaker2::OnTradeBmf(const unsigned int t_security_id_, const double t_trade_price_,
  ...
  last_trade_price_bmf_ = int_price;
  ...
  switch (buysell_) {
    case kTradeTypeSell: {
      ...
      int trade_size_remaining_ = t_trade_size_;
      buy_side_trade_level_++;
      SimMarketOrder* trade_order_ = GetBuyOrderFromLevel(buy_side_trade_level_);

      while ((trade_order_ != NULL) && (trade_size_remaining_ == 0)) {
        if ((int_price < trade_order_->int_price_) && (last_trade_price_bmf_ == 0)) {
          DBGLOG_TIME_CLASS_FUNC << " Error: Improve Bid Order Present at Top : " << trade_order_->order_id_
                                 << DBGLOG_ENDL_FLUSH;
          continue;  // Should we sanitize here ?
        }
        if (int_price > trade_order_->int_price_) {
          DBGLOG_TIME_CLASS_FUNC << " Error: Bid Size remaining to match: " << trade_size_remaining_
                                 << " but all orders on the price: " << int_price << " are traded "
                                 << trade_order_->order_id_ << DBGLOG_ENDL_FLUSH;
          break;
        }
        int this_size_executed_ = std::min(trade_size_remaining_, trade_order_->size_);
        ExecuteSimBid(trade_order_->int_price_, trade_order_->order_id_, this_size_executed_,
                      trade_size_remaining_ >= trade_order_->size_);
        trade_size_remaining_ -= this_size_executed_;
        buy_side_trade_level_++;
        trade_order_ = GetBuyOrderFromLevel(buy_side_trade_level_);
      }
    ...
    case kTradeTypeBuy: {
      ...
      while ((trade_order_ != NULL) && (trade_size_remaining_ == 0)) {
        if ((int_price > trade_order_->int_price_) && (last_trade_price_bmf_ == 0)) {
          DBGLOG_TIME_CLASS_FUNC << " Error: Improve Ask Order Present at Top : " << trade_order_->order_id_
                                 << DBGLOG_ENDL_FLUSH;
          continue;  // Should we sanitize here ?
        }
        if (int_price < trade_order_->int_price_) {
          DBGLOG_TIME_CLASS_FUNC << " Error: Ask Size remaining to match: " << trade_size_remaining_
                                 << " but all orders on the price: " << int_price << " are traded "
                                 << trade_order_->order_id_ << DBGLOG_ENDL_FLUSH;
          break;
        }
        ...
        trade_size_remaining_ -= this_size_executed_;
        sell_side_trade_level_++;
        trade_order_ = GetSellOrderFromLevel(sell_side_trade_level_);
      }
