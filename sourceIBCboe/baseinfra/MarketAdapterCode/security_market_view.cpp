...
SecurityMarketView::SecurityMarketView(DebugLogger &t_dbglogger_, const Watch &t_watch_,
    ...
    : dbglogger_(t_dbglogger_),
      ...
      l2_only_listeners_(),
      onready_listeners_(),
      spot_index_listeners_(),
      open_interest_listeners_(),
      ...
      is_book_crossed_(false),
      using_predictive_uncross_(false),
      syn_best_bid_level_info_(NULL),
      syn_best_ask_level_info_(NULL),
      syn_bestbid_index_(0u),
      syn_bestask_index_(0u),
      upper_int_price_limit_(100000000),
      lower_int_price_limit_(-1),
      upper_int_trade_range_limit_(100000000),
      lower_int_trade_range_limit_(-1),
      is_px_managed_smv_(false),
      px_level_manager_(NULL) {
  //std::cout << "security_market_view constructor" << std::endl;
  l1_price_listeners_.reserve(1024);
  l1_size_listeners_.reserve(1024);
  spot_index_listeners_.reserve(1024);
  open_interest_listeners_.reserve(1024);
  l2_listeners_.reserve(1024);
  ...
  if (t_exch_source_ != kExchSourceNSE && t_exch_source_ != kExchSourceNSE_CD && t_exch_source_ != kExchSourceNSE_EQ &&
      t_exch_source_ != kExchSourceNSE_FO && t_exch_source_ != kExchSourceBSE && t_exch_source_ != kExchSourceBSE_CD &&
      t_exch_source_ != kExchSourceBSE_EQ && t_exch_source_ != kExchSourceBSE_FO) {
  //std::cout << "security_market_view hybridhy sec manager" << std::endl;
  ...
  //std::cout << "security_market_view hybridhy sec manager done" << std::endl;
  ...
  //std::cout << "security_market_view constructor done" << std::endl;
}
...
bool SecurityMarketView::subscribe_price_type(SecurityMarketViewChangeListener *t_new_listener_,
  ...
  switch (t_price_type_) {
    ...
    case kPriceTypeSpotIndex: {
      if (t_new_listener_ != nullptr) {
	VectorUtils::UniqueVectorAdd(spot_index_listeners_, t_new_listener_);
      }
    } break;
    default:
...
void SecurityMarketView::subscribe_SpotIndexPrice(SecurityMarketViewChangeListener *t_new_listener_){
  if (t_new_listener_ != nullptr) {
    VectorUtils::UniqueVectorAdd(spot_index_listeners_, t_new_listener_);
  }
}

void SecurityMarketView::subscribe_OpenInterestPrice(SecurityMarketViewChangeListener *t_new_listener_){
  if (t_new_listener_ != nullptr) {
    VectorUtils::UniqueVectorAdd(open_interest_listeners_, t_new_listener_);
  }
}
...
void SecurityMarketView::subscribe_bardata(SecurityMarketViewBardataListener *t_new_listener_){
  if (t_new_listener_ != nullptr) {
    VectorUtils::UniqueVectorAdd(bardata_listeners_, t_new_listener_);
  }
}
...
void SecurityMarketView::subscribe_circuit(SecurityMarketViewChangeListener *t_new_listener_){
  if (t_new_listener_ != nullptr) {
    VectorUtils::UniqueVectorAdd(circuit_listeners_, t_new_listener_);
  }
}
...
void SecurityMarketView::OnTradeIBKR(const double t_trade_price_, const int t_trade_size_, const TradeType_t t_buysell_) {
  if (trade_print_info_.computing_last_book_tdiff_) {
    if (prev_bid_was_quote_ && prev_ask_was_quote_) {  // the difference between last_book_mkt_size_weighted_price_ and
                                                       // mkt_size_weighted_price_ is that
      // in case of CME ( and other trade_before_quote exchanges ) where we might have a number of
  ...
  trade_print_info_.size_traded_ = t_trade_size_;
  trade_print_info_.int_trade_price_ = GetIntPx(t_trade_price_);
  if(is_ready_)trade_print_info_.is_intermediate_=false;
  // do the conditional computations ... before the quote is adjusted for the trade
  SetTradeVarsForIndicatorsIfRequired();

  // if trade information comes before updated quote information
  if (trade_before_quote()) {
    // DBGLOG_CLASS_FUNC_LINE_INFO << "On_Trade trade_before_quote is set : " << DBGLOG_ENDL_NOFLUSH;
    //       DBGLOG_DUMP;
    ...
    switch (trade_print_info_.buysell_) {
      case kTradeTypeBuy: {
        ...
        SetBestLevelAskVariablesOnLiftIBKR();
        if (is_ready_) {
          UpdateL1Prices();
          bool is_ready_actual=is_ready_;
          is_ready_=true;
          NotifyTradeListeners();

          is_ready_=true;
          if(trade_print_info_.is_intermediate_){
            NotifyL1PriceListeners();
          }else NotifyL1SizeListeners();

          is_ready_=is_ready_actual;
          NotifyOnReadyListeners();
        }
      } break;
      case kTradeTypeSell: {
        ...
        SetBestLevelBidVariablesOnHitIBKR();
        if (is_ready_) {
          UpdateL1Prices();
          bool is_ready_actual=is_ready_;
          is_ready_=true;
    ...
    DBGLOG_CLASS_FUNC_LINE_INFO << "On_Trade trade_before_quote is not set will notify trade and ready listeners: " << DBGLOG_ENDL_NOFLUSH;
          DBGLOG_DUMP;
    NotifyTradeListeners();
    NotifyOnReadyListeners();
  }
...
std::string SecurityMarketView::ShowMarket() const {
  //std::cout << "SecurityMarketView::ShowMarket: " << std::endl;
  std::ostringstream t_temp_oss_;

  // bool bid_correct_ = true;
  // bool ask_correct_ = true;
  
  bool is_book_crossed_ = false;

  t_temp_oss_ << market_update_info_.secname_ << "\n";
  //std::cout << "SecurityMarketView::ShowMarket: " << std::endl;
  unsigned int m_m_levels = std::min(MinValidNumBidLevels(5), MinValidNumAskLevels(5));
  //std::cout << "m_m_levels: " << m_m_levels << " " << MinValidNumBidLevels(5) << " " << MinValidNumAskLevels(5) << std::endl;
  for (unsigned int t_level_ = 0; t_level_ < m_m_levels; t_level_++) {
    

    if(((bid_int_price(t_level_) > 0 && ask_int_price(t_level_) > 0 && bid_size(t_level_) > 0 && ask_size(t_level_) > 0) && 
       (bid_int_price(t_level_) >= ask_int_price(t_level_) || bid_price(t_level_) >= ask_price(t_level_))) ||
       (bid_int_price(t_level_) > 0 && bid_size(t_level_) < 0) || (ask_int_price(t_level_) > 0 && ask_size(t_level_) < 0)){
       //bid_size(t_level_) <= 0 || ask_size(t_level_) <= 0)){
      is_book_crossed_ = true;
  ...
  } else if (MinValidNumAskLevels(5) > m_m_levels) {
    for (unsigned int t_level_ = m_m_levels; t_level_ < MinValidNumAskLevels(5); t_level_++) {
      t_temp_oss_.width(6);
      t_temp_oss_ << 0;
      t_temp_oss_ << " ";
      t_temp_oss_.width(5);
  ...
  t_temp_oss_ << "\n";
  if(is_book_crossed_) {
    std::cout << "Book is CROSSED" << std::endl;
  }
  // t_temp_oss_ << "TotalValidBidLevels: " <<
  // px_level_manager_->underlying_bid_ds_->num_levels_vec_[market_update_info_.security_id_]  << " TotalValidAskLevels:
  // " << px_level_manager_->underlying_ask_ds_->num_levels_vec_[market_update_info_.security_id_] << "\n";
  
//  if(is_book_crossed_){
    std::cout << t_temp_oss_.str() << std::endl;
//  }
...
void SecurityMarketView::SetBestLevelBidVariablesOnQuote() {
  ...
  // assign is_ready_
  // std::cout<<"SetBestLevelBidVariablesOnQuote current state is_ready_ before if :"<<is_ready_<<"\n";
  if (!is_ready_) {
  ...
  // std::cout<<"SetBestLevelBidVariablesOnQuote current state is_ready_ after if :"<<is_ready_<<"\n";
...
void SecurityMarketView::SetBestLevelAskVariablesOnQuote() {
  ...
  if (remove_self_orders_from_book_) {
    ...
    market_update_info_.bestask_price_ = ask_price(0);
    market_update_info_.bestask_size_ = ask_size(0);
    market_update_info_.bestask_int_price_ = ask_int_price(0);
    market_update_info_.bestask_ordercount_ = ask_order(0);
    if (!price_to_yield_map_.empty()) {
  ...
  // std::cout<<"SetBestLevelAskVariablesOnQuote current state is_ready_ before if :"<<is_ready_<<"\n";
  // std::cout<<(market_update_info_.bidlevels_.size() > 0)<<" "<< (market_update_info_.bestbid_int_price_ != kInvalidIntPrice)<<" "<< 
  //        (market_update_info_.bestbid_size_ > 0) <<" "<<
  ...
  // std::cout<<"SetBestLevelAskVariablesOnQuote current state is_ready_ after if :"<<is_ready_<<"\n";
}
...
void SecurityMarketView::UpdateL1Prices() {
  ...
  if ((market_update_info_.bestbid_int_price_ == (kInvalidIntPrice)) ||
      (market_update_info_.bestask_int_price_ == (kInvalidIntPrice))) {
        // std::cout<<"Will make is_ready_ false current state,"<<is_ready_<<"\n";
...
unsigned int SecurityMarketView::MinValidNumAskLevels(unsigned int t_max_interested_value_) const {
  //std::cout << "MinValidNumAskLevels : " << t_max_interested_value_ << std::endl;
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
 ...
 //     std::cout << "Start curr_ask_idx: " << current_ask_index_ << " num_valid_levels_: " << num_valid_levels_  << std::endl;
      if (market_update_info_.asklevels_[current_ask_index_].limit_size_ > 0) {
        num_valid_levels_++;
      }
 //     std::cout << "END curr_ask_idx: " << current_ask_index_ << " num_valid_levels_: " << num_valid_levels_  << std::endl;
      if (num_valid_levels_ >= t_max_interested_value_) {
 //       std::cout << "*END curr_ask_idx: " << current_ask_index_ << " num_valid_levels_: " << num_valid_levels_ << std::endl;
        return num_valid_levels_;
      }
    }
 //   std::cout << "MinValidNumAskLevels num_valid_levels_: " << num_valid_levels_ << std::endl;
    return num_valid_levels_;
  } else {
...
unsigned int SecurityMarketView::MinValidNumBidLevels(unsigned int t_max_interested_value_) const {
 // std::cout << "SecurityMarketView::MinValidNumBidLevels: " << t_max_interested_value_ << std::endl;
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    unsigned int num_valid_levels_ = 0;
    for (unsigned int current_bid_index_ = GetBaseBidMapIndex(); current_bid_index_ > 0;
         current_bid_index_--) {  // not optimized
 //     std::cout << "Start curr_bid_idx: " << current_bid_index_ << " num_valid_levels_: " << num_valid_levels_  << std::endl;
      if (market_update_info_.bidlevels_[current_bid_index_].limit_size_ > 0) {
        num_valid_levels_++;
      }
 //     std::cout << "End curr_bid_idx: " << current_bid_index_ << " num_valid_levels_: " << num_valid_levels_  << std::endl;
      if (num_valid_levels_ >= t_max_interested_value_) {
 //       std::cout << "*End curr_bid_idx: " << current_bid_index_ << " num_valid_levels_: " << num_valid_levels_  << std::endl;
        return num_valid_levels_;
      }
...
void SecurityMarketView::SetBestLevelBidVariablesOnHitIBKR() {
  ...
  if (trade_print_info_.int_trade_price_ ==
    ...
    if (t_trade_masked_best_bid_size_ <= 0) {  // even second level cleared
      ...
      if (top_bid_level_to_mask_trades_on_ <
        ...
        trade_print_info_.is_intermediate_=true;
        market_update_info_.bestbid_price_ = kInvalidPrice;
        market_update_info_.bestbid_size_ = kInvalidSize;  // no change in prices
        market_update_info_.bestbid_int_price_ = kInvalidIntPrice;
        market_update_info_.bestbid_ordercount_ = 0;
        if (!price_to_yield_map_.empty()) {
          market_update_info_.bestbid_price_ =
      ...
      if (!price_to_yield_map_.empty()) {
        ...
        hybrid_market_update_info_.bestbid_ordercount_ =
            market_update_info_.bidlevels_[top_bid_level_to_mask_trades_on_].limit_ordercount_;
                 ...
                 market_update_info_.bidlevels_[top_bid_level_to_mask_trades_on_].limit_int_price_)  // do this only for hk, ose has data
  ...
  {  // happens for highly volatile products with an aggressive trade at a non-existant price-level.
    if ((top_bid_level_to_mask_trades_on_ + 1) <
      ...
      trade_print_info_.is_intermediate_=true;
      market_update_info_.bestbid_price_ = kInvalidPrice;
      market_update_info_.bestbid_size_ = kInvalidSize;
      market_update_info_.bestbid_int_price_ = kInvalidIntPrice;
      market_update_info_.bestbid_ordercount_ = 0;
      if (!price_to_yield_map_.empty()) {
...
void SecurityMarketView::SetBestLevelAskVariablesOnLiftIBKR() {
  ...
  if (trade_print_info_.int_trade_price_ ==
    ...
    if (t_trade_masked_best_ask_size_ <= 0) {  // even second level cleared
      ...
      if (top_ask_level_to_mask_trades_on_ <
        ...
        trade_print_info_.is_intermediate_=true;
        market_update_info_.bestask_price_ = kInvalidPrice;
        market_update_info_.bestask_size_ = kInvalidSize;
        market_update_info_.bestask_int_price_ = kInvalidIntPrice;
        market_update_info_.bestask_ordercount_ = 0;
        if (!price_to_yield_map_.empty()) {
          //            std::cout <<
      ...
      //        }
                 ...
                 market_update_info_.asklevels_[top_ask_level_to_mask_trades_on_].limit_int_price_)  // do this only for hk, ose has data
  // problems and this mod makes it worse
  {  // happens for highly volatile products with an aggressive trade at a non-existant price-level.
...
void SecurityMarketView::NotifyFastTradeListeners(const unsigned int t_security_id_, TradeType_t t_aggressive_side,
  ...
  uint32_t vec_size = fast_trade_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    fast_trade_listeners_[vec_count]->OnFastTradeUpdate(t_security_id_, t_aggressive_side, t_last_traded_price);
  }
}
...
void SecurityMarketView::NotifyOnReadyListeners() {
  ...
  for (uint32_t vec_count = 0; vec_count < onready_listeners_.size(); vec_count++) {
    onready_listeners_[vec_count]->SMVOnReady();
  }
#if CCPROFILING_TRADEINIT
...
void SecurityMarketView::NotifyL2OnlyListeners() {
  ...
  uint32_t vec_size = l2_only_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    if (!price_to_yield_map_.empty()) {
      l2_only_listeners_[vec_count]->OnMarketUpdateL2(security_id(), hybrid_market_update_info_);
    } else {
#if CCPROFILING_TRADEINIT
      HFSAT::CpucycleProfiler::GetUniqueInstance().Start(23);
#endif
      l2_only_listeners_[vec_count]->OnMarketUpdateL2(security_id(), market_update_info_);
    }
  }
}
...
void SecurityMarketView::NotifyL2Listeners() {
  ...
  uint32_t vec_size = l2_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    if (!price_to_yield_map_.empty()) {
      l2_listeners_[vec_count]->OnMarketUpdate(security_id(), hybrid_market_update_info_);
    } else {
#if CCPROFILING_TRADEINIT
      HFSAT::CpucycleProfiler::GetUniqueInstance().Start(23);
#endif
      l2_listeners_[vec_count]->OnMarketUpdate(security_id(), market_update_info_);
    }
  }
}

void SecurityMarketView::NotifyBardataListeners(const unsigned int t_security_id_,const char* _bardata_line_){
  uint32_t vec_size = bardata_listeners_.size();

  for(uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    bardata_listeners_[vec_count]->OnBardataUpdate(security_id(), _bardata_line_);
  }
}

void SecurityMarketView::NotifySpotIndexListeners() {
  uint32_t vec_size = spot_index_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    spot_index_listeners_[vec_count]->OnIndexUpdate(security_id(), trade_print_info_.trade_price_);
    spot_index_listeners_[vec_count]->OnTradePrint(security_id(), trade_print_info_, market_update_info_);
  }
}

void SecurityMarketView::NotifyOpenInterestListeners(double const oi_price_t){
  // std::cout << "OI: " << secname() << " " << t_oi_price_ << std::endl;
  uint32_t vec_size = open_interest_listeners_.size();
  
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
//    open_interest_listeners_[vec_count]->OnOpenInterestUpdate(security_id(), t_oi_price_);
      open_interest_listeners_[vec_count]->OnOpenInterestUpdate(security_id(), oi_price_t);
  }
}
...
void SecurityMarketView::NotifyL1SizeListeners() {
  ...
  // std::cout << "NotifyL1SizeListeners " << is_ready_ << std::endl;
  // DBGLOG_CLASS_FUNC_LINE_INFO << "IsReady?" << is_ready_ <<DBGLOG_ENDL_FLUSH;
  // DBGLOG_DUMP;
  if (!is_ready_) {
    return;
  }
  ...
  uint32_t vec_size = l1_size_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    if (price_to_yield_map_.empty()) {
#if CCPROFILING_TRADEINIT
      HFSAT::CpucycleProfiler::GetUniqueInstance().Start(23);
#endif
      l1_size_listeners_[vec_count]->OnMarketUpdate(security_id(), market_update_info_);
    } else {
      l1_size_listeners_[vec_count]->OnMarketUpdate(security_id(), hybrid_market_update_info_);
    }
  }
...
void SecurityMarketView::NotifyL1PriceListeners() {
  ...
  // Have changed the for each loop to normal loop, as the OnMrakteUpdate here is adding a new
  // element in the l1_size_listeners_ ( SquareOffTheo ) which makes iterators invalid and results into
  // memory corruption - ravi
  uint32_t vec_size = l1_price_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    if (price_to_yield_map_.empty()) {
#if CCPROFILING_TRADEINIT
      HFSAT::CpucycleProfiler::GetUniqueInstance().Start(23);
#endif
      l1_price_listeners_[vec_count]->OnMarketUpdate(security_id(), market_update_info_);
    } else {
      l1_price_listeners_[vec_count]->OnMarketUpdate(security_id(), hybrid_market_update_info_);
    }
  }
...
void SecurityMarketView::NotifyCircuitListeners(){
  uint32_t vec_size = circuit_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    circuit_listeners_[vec_count]->OnCircuitUpdate();
  }
}
...
void SecurityMarketView::NotifyRawTradeListeners() {
  ...
  uint32_t vec_size = raw_trade_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    if (price_to_yield_map_.empty()) {
#if CCPROFILING_TRADEINIT
      HFSAT::CpucycleProfiler::GetUniqueInstance().Start(23);
#endif
      raw_trade_listeners_[vec_count]->OnRawTradePrint(security_id(), raw_trade_print_info_, market_update_info_);
    } else {
      raw_trade_listeners_[vec_count]->OnRawTradePrint(security_id(), raw_trade_print_info_,
                                                       hybrid_market_update_info_);
    }
  }
}
...
void SecurityMarketView::NotifyTradeListeners() {
  ...
  if ((market_update_info_.bestbid_int_price_ == (kInvalidIntPrice)) ||
    ...
    for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
      //  for (auto l1_px_listener : l1_price_listeners_) {
      l1_price_listeners_[vec_count]->OnTradePrint(security_id(), trade_print_info_, market_update_info_);
    }
    market_update_info_.l1events_++;
  ...
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    ...
    if (price_to_yield_map_.empty()) {
      ...
      l1_price_listeners_[vec_count]->OnTradePrint(security_id(), trade_print_info_, hybrid_market_update_info_);
    }
  }
...
void SecurityMarketView::NotifySpreadTradeListeners() {
  ...
  uint32_t vec_size = spread_trades_listeners_.size();
  for (uint32_t vec_count = 0; vec_count < vec_size; vec_count++) {
    if (price_to_yield_map_.empty()) {
#if CCPROFILING_TRADEINIT
      HFSAT::CpucycleProfiler::GetUniqueInstance().Start(23);
#endif
      spread_trades_listeners_[vec_count]->OnTradePrint(security_id(), trade_print_info_, market_update_info_);
    } else {
      spread_trades_listeners_[vec_count]->OnTradePrint(security_id(), trade_print_info_, hybrid_market_update_info_);
    }
  }
}
...
void SecurityMarketView::NotifyOTCTradeListeners(const double t_otc_trade_price_, const int t_otc_trade_size_) {
  ...
  for (uint32_t vec_count = 0; vec_count < otc_trades_listeners_.size(); vec_count++) {
    if (price_to_yield_map_.empty()) {
#if CCPROFILING_TRADEINIT
      HFSAT::CpucycleProfiler::GetUniqueInstance().Start(23);
#endif
      otc_trades_listeners_[vec_count]->OnTradePrint(security_id(), trade_print_info_, market_update_info_);
    } else {
      otc_trades_listeners_[vec_count]->OnTradePrint(security_id(), trade_print_info_, hybrid_market_update_info_);
    }
  }
}
...
void SecurityMarketView::UpdateL1Struct(HFSAT::GenericL1DataStruct &l1_struct) {

  l1_struct.bestbid_price = market_update_info_.bestbid_price_;
  l1_struct.bestbid_size = market_update_info_.bestbid_size_;
  l1_struct.bestbid_ordercount = market_update_info_.bestbid_ordercount_;
  l1_struct.bestask_price = market_update_info_.bestask_price_;
  l1_struct.bestask_size = market_update_info_.bestask_size_;
  l1_struct.bestask_ordercount = market_update_info_.bestask_ordercount_;
  memset((void*)&(l1_struct.symbol),0,GENERIC_L1_DATA_EXCHANGE_SYMBOL_SIZE);
  memcpy((void*)&l1_struct.symbol,(void*)(this->shortcode().c_str()),this->shortcode().length());
...
int SecurityMarketView::ask_order(unsigned int t_level_) const {
  ...
  } else if (is_px_managed_smv_) {
    MarketUpdateInfoLevelStruct *px_level_ = px_level_manager_->GetSynAskPL(market_update_info_.security_id_, t_level_);
    if (px_level_ != NULL) {
      return px_level_->limit_ordercount_;
    }
  } else {
...
int SecurityMarketView::ask_order_at_int_price(int t_int_price_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    MarketUpdateInfoLevelStruct *px_level_ =
        px_level_manager_->GetPLAtIntPrice(market_update_info_.security_id_, HFSAT::kTradeTypeSell, t_int_price_);
    if (px_level_ != NULL) {
      return px_level_->limit_ordercount_;
    }
  } else {
...
int SecurityMarketView::ask_size(unsigned int t_level_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    return px_level_manager_->GetSynAskSize(market_update_info_.security_id_, t_level_);
  } else {
    if (t_level_ < market_update_info_.asklevels_.size()) {
...
int SecurityMarketView::ask_size_at_int_price(int t_int_price_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    unsigned int t_level_ = 0;
    MarketUpdateInfoLevelStruct *t_px_level_ = NULL;
    do {
      t_px_level_ = px_level_manager_->GetSynAskPL(market_update_info_.security_id_, t_level_);
      if (t_px_level_ != NULL && t_px_level_->limit_int_price_ == t_int_price_) {
        return t_px_level_->limit_size_;
      }
      t_level_++;
    } while (t_px_level_ != NULL && t_int_price_ > t_px_level_->limit_int_price_);
    return 0;
  } else {
    for (int i = 0; i < NumAskLevels(); i++) {
      if (ask_int_price(i) == t_int_price_) {
...
double SecurityMarketView::ask_price(unsigned int t_level_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    return px_level_manager_->GetSynAskPrice(market_update_info_.security_id_, t_level_);
  } else {
    if (t_level_ < market_update_info_.asklevels_.size()) {
...
//      std::cout << "SecurityMarketView::ask_int_price: " << t_level_ << std::endl;
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
//      std::cout << "market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_" << std::endl;
    if (market_update_info_.compute_stable_levels_) {

      //std::cout << "market_update_info_.compute_stable_levels_" << std::endl;
      int size_seen_ = 0;
      int orders_seen_ = 0;
      //std::cout << "get_next_stable_ask_level_index: base_idx, level_sz_thresh, size_seen, order_seen :: "
//		<< base_ask_index_ << " , " << level_size_thresh_ << " , " << size_seen_ << " , " << orders_seen_ << std::endl;
      unsigned int current_ask_index_ =
          get_next_stable_ask_level_index(base_ask_index_, level_size_thresh_, size_seen_, orders_seen_);
     
      //std::cout << "current_ask_index_, base_ask_index_ :: " << current_ask_index_ << " " << base_ask_index_ << std::endl;
      for (unsigned int level_counter_ = 0; current_ask_index_ > 0; level_counter_++) {
        if (level_counter_ >= t_level_) {
	  
          //std::cout << "level_counter_ >= t_level_:: " << level_counter_ << " >= " << t_level_ << std::endl;
      ...
      if (t_level_ == 0 && is_ready_) {
        ...
        //std::cout << "t_level_ == 0 && is_ready_ so bst_ask_int_px: " << market_update_info_.bestask_int_price_ << std::endl;
      ...
      int level_diff = 0;
      if (is_ready_) {
//	std::cout << "is_ready_" << std::endl;
        level_diff =
            market_update_info_.bestask_int_price_ - market_update_info_.asklevels_[base_ask_index_].limit_int_price_;

//	std::cout << "level_diff = bestask_int_price_ - ask_index.limit_int_price_ :: "
//		  << level_diff << " = " << market_update_info_.bestask_int_price_
//		  << " - " << market_update_info_.asklevels_[base_ask_index_].limit_int_price_ << std::endl;
      }

      // If level_diff is < 0 ( possible ?), the smv book might not have info for that level.
      if (level_diff > 0) {
//	std::cout << "level_diff > 0 :: " << level_diff << std::endl;
        t_level_ += level_diff;
      }

//      std::cout << "t_level_: " << t_level_ 
//		<< "\nask_access_bitmask_: " << ask_access_bitmask_ << "bitmask_access_lookup_table_: " 
//		<< bitmask_access_lookup_table_[t_level_] << std::endl;
      if ((ask_access_bitmask_ & bitmask_access_lookup_table_[t_level_]) == bitmask_access_lookup_table_[t_level_]) {
	
//	std::cout << "IF : " << " asklevels_[base_ask_index_ - t_level_].limit_int_price_ :: base - level :: "
//		  << base_ask_index_ << " - " << t_level_ << " :: " << base_ask_index_ - t_level_ << " => "
//		  << market_update_info_.asklevels_[base_ask_index_ - t_level_].limit_int_price_ << std::endl;
      ...
      if (t_level_ == 0) {
//	std::cout << "IF t_level_ == 0 base_ask_idx: " << base_ask_index_ << std::endl;
        return (market_update_info_.asklevels_[base_ask_index_].limit_int_price_);
      } else {
//	std::cout << "ELSE: " << std::endl;
        unsigned int current_ask_index_ = IndexedBookGetNextNonEmptyAskMapIndex(base_ask_index_);
//	std::cout << "current_ask_index_: " << current_ask_index_ << std::endl;
        for (unsigned int level_counter_ = 1u; current_ask_index_ > 0; level_counter_++) {
          if (level_counter_ >= t_level_) {
//            std::cout << "level_counter_ >= t_level_:: " << level_counter_ << " >= " << t_level_ << std::endl;
            return market_update_info_.asklevels_[current_ask_index_].limit_int_price_;
            ...
            current_ask_index_ = IndexedBookGetNextNonEmptyAskMapIndex(current_ask_index_);
//	    std::cout << "ELSE current_ask_index_: " << current_ask_index_ << std::endl;	
          }
        }
      }
    }
...
//      std::cout << "else if (is_px_managed_smv_)" << std::endl;
    return px_level_manager_->GetSynAskIntPrice(market_update_info_.security_id_, t_level_);
  } else {
  
 //     std::cout << "ELSE" << std::endl;

 //     std::cout << "t_level_, asklevels_.size() :: " << t_level_ << " " << market_update_info_.asklevels_.size() << std::endl;
    if (t_level_ < market_update_info_.asklevels_.size()) {
...
MarketUpdateInfoLevelStruct *SecurityMarketView::ask_level_info(unsigned int t_level_) {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    // Finds out the diff between the bestask variable and the bestask in the smv book.
    int level_diff = 0;
    if (is_ready_) {
      level_diff =
          market_update_info_.bestask_int_price_ - market_update_info_.asklevels_[base_ask_index_].limit_int_price_;
    ...
    if ((ask_access_bitmask_ & bitmask_access_lookup_table_[t_level_]) == bitmask_access_lookup_table_[t_level_]) {
      return &market_update_info_.asklevels_[base_ask_index_ - t_level_];
    }

    if (base_ask_index_ < 0) return nullptr;
    if (t_level_ == 0) {  // top level
      return (&market_update_info_.asklevels_[base_ask_index_]);
    } else {
      unsigned int current_ask_index_ = IndexedBookGetNextNonEmptyAskMapIndex(base_ask_index_);
      for (unsigned int level_counter_ = 1u; current_ask_index_ > 0; level_counter_++) {
        if (level_counter_ >= t_level_) {
    ...
    if (t_level_ < market_update_info_.asklevels_.size()) {
      return &market_update_info_.asklevels_[t_level_];
    }
  }
  return nullptr;
}
...
bool SecurityMarketView::IsAskBookEmpty() const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    if (index >= 0 && index < (int)market_update_info_.asklevels_.size()) {
      //std::cout << "IF index: " << index << " " << (int)market_update_info_.asklevels_.size() << std::endl;
      return (market_update_info_.asklevels_[index].limit_size_ <= 0);
    } else {
      return true;
    }
  } else if (is_px_managed_smv_) {
    //std::cout << "is_px_managed_smv_" << std::endl;
    return (market_update_info_.bestask_int_price_ == kInvalidIntPrice);
  } else {
    //std::cout << "ELSE " << market_update_info_.asklevels_.empty() << std::endl;
...
unsigned int SecurityMarketView::get_next_stable_ask_level_index(unsigned int current_ask_index_,
  ...
  //std::cout << "SecurityMarketView::get_next_stable_ask_level_index:: " << std::endl;
  size_seen_ = 0;
  orders_seen_ = 0;
...
/*
    std::cout << "FOR: current_ask_index_: " << current_ask_index_ 
	      << "limit_size_, ord_cnt: " << market_update_info_.asklevels_[current_ask_index_].limit_size_ 
	      << " , " << market_update_info_.asklevels_[current_ask_index_].limit_ordercount_ 
	      << "  size_seen, Ord_seen :: " << size_seen_ << " , " << orders_seen_ << std::endl;
*/
    if (size_seen_ >= _level_size_thresh_) {
//      std::cout << "size_seen_ >= _level_size_thresh_ :: " << size_seen_ << " >= " << _level_size_thresh_ << std::endl;
...
int SecurityMarketView::bid_order(unsigned int t_level_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    MarketUpdateInfoLevelStruct *px_level_ = px_level_manager_->GetSynBidPL(market_update_info_.security_id_, t_level_);
    if (px_level_ != NULL) {
      return px_level_->limit_ordercount_;
    }
  } else {
...
int SecurityMarketView::bid_order_at_int_price(int t_int_price_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    MarketUpdateInfoLevelStruct *px_level_ =
        px_level_manager_->GetPLAtIntPrice(market_update_info_.security_id_, HFSAT::kTradeTypeBuy, t_int_price_);
    if (px_level_ != NULL) {
      return px_level_->limit_ordercount_;
    }
  } else {
...
int SecurityMarketView::bid_size(unsigned int t_level_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    return px_level_manager_->GetSynBidSize(market_update_info_.security_id_, t_level_);
  } else {
    if (t_level_ < market_update_info_.bidlevels_.size()) {
...
int SecurityMarketView::bid_size_at_int_price(int t_int_price_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    MarketUpdateInfoLevelStruct *t_px_level_ = NULL;
    do {
      t_px_level_ = px_level_manager_->GetSynBidPL(market_update_info_.security_id_, t_level_);
      if (t_px_level_ != NULL && t_px_level_->limit_int_price_ == t_int_price_) {
        return t_px_level_->limit_size_;
      }
      t_level_++;
...
double SecurityMarketView::bid_price(unsigned int t_level_) const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    return px_level_manager_->GetSynBidPrice(market_update_info_.security_id_, t_level_);
  } else {
    if (t_level_ < market_update_info_.bidlevels_.size()) {
...
//  std::cout << "SecurityMarketView::bid_int_price: " << t_level_ << std::endl;
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    if (market_update_info_.compute_stable_levels_) {
...
//      std::cout << "ELSE***" << std::endl;
      if (t_level_ == 0 && is_ready_) {
	//std::cout << "t_level_ == 0 && is_ready_ so bst_bid_int_px: " << market_update_info_.bestbid_int_price_ << std::endl;
	...
	//std::cout << "is_ready_" << std::endl;
        level_diff =
            market_update_info_.bidlevels_[base_bid_index_].limit_int_price_ - market_update_info_.bestbid_int_price_;
/*
      std::cout << "level_diff = bestbid_int_price_ - bid_index.limit_int_price_ :: "
                      << level_diff << " = " << market_update_info_.bestbid_int_price_
                      << " - " << market_update_info_.bidlevels_[base_bid_index_].limit_int_price_ << std::endl;
*/
      }
...
/*
      std::cout << "t_level_: " << t_level_ 
              << "\nbid_access_bitmask_: " << bid_access_bitmask_ << "bitmask_access_lookup_table_: " 
                            << bitmask_access_lookup_table_[t_level_] << std::endl;
*/
      if ((bid_access_bitmask_ & bitmask_access_lookup_table_[t_level_]) == bitmask_access_lookup_table_[t_level_]) {
/*
	std::cout << "IF : " << " bidlevels_[base_bid_index_ - t_level_].limit_int_price_ :: base - level :: "
                << base_bid_index_ << " - " << t_level_ << " :: " << base_bid_index_ - t_level_ << " => "
                                << market_update_info_.bidlevels_[base_bid_index_ - t_level_].limit_int_price_ << std::endl;
*/
      ...
      if (t_level_ == 0) {
	//std::cout << "IF t_level_ == 0 base_bid_idx: " << base_bid_index_ << std::endl;
        return (market_update_info_.bidlevels_[base_bid_index_].limit_int_price_);
      } else {
	//std::cout << "ELSE: " << std::endl;
        unsigned int current_bid_index_ = IndexedBookGetNextNonEmptyBidMapIndex(base_bid_index_);
	//std::cout << "current_bid_index_: " << current_bid_index_ << std::endl;
        for (unsigned int level_counter_ = 1u; current_bid_index_ > 0; level_counter_++) {
          if (level_counter_ >= t_level_) {
	  //  std::cout << "level_counter_ >= t_level_:: " << level_counter_ << " >= " << t_level_ << std::endl;
            return market_update_info_.bidlevels_[current_bid_index_].limit_int_price_;
            ...
            current_bid_index_ = IndexedBookGetNextNonEmptyBidMapIndex(current_bid_index_);
	    //std::cout << "ELSE current_bid_index_: " << current_bid_index_ << std::endl;
          }
        }
      }
    }
  } else if (is_px_managed_smv_) {
    return px_level_manager_->GetSynBidIntPrice(market_update_info_.security_id_, t_level_);
  } else {
    if (t_level_ < market_update_info_.bidlevels_.size()) {
...
MarketUpdateInfoLevelStruct *SecurityMarketView::bid_level_info(unsigned int t_level_) {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    // Finds out the diff between the bestbid variable and the bestbid in the smv book.
    int level_diff = 0;
    if (is_ready_) {
    ...
    if ((bid_access_bitmask_ & bitmask_access_lookup_table_[t_level_]) == bitmask_access_lookup_table_[t_level_]) {
      return (&market_update_info_.bidlevels_[base_bid_index_ - t_level_]);
    }

    if (base_bid_index_ < 0) return nullptr;
    if (t_level_ == 0) {  // top level
      return (&market_update_info_.bidlevels_[base_bid_index_]);
    } else {
      unsigned int current_bid_index_ = IndexedBookGetNextNonEmptyBidMapIndex(base_bid_index_);
      for (unsigned int level_counter_ = 1u; current_bid_index_ > 0; level_counter_++) {
        if (level_counter_ >= t_level_) {
    ...
    if (t_level_ < market_update_info_.bidlevels_.size()) {
      return &market_update_info_.bidlevels_[t_level_];
    }
  }
  return nullptr;
}
...
bool SecurityMarketView::IsBidBookEmpty() const {
  if (market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
    ...
    return (market_update_info_.bestbid_int_price_ == kInvalidIntPrice);
  } else {
    ...
    case kTradeTypeBuy: {
      ...
      if (offset_ <= 0) {
        DBGLOG_TIME_CLASS_FUNC_LINE << " " << secname() << " offset should be +ve valued but is " << offset_
                                    << DBGLOG_ENDL_FLUSH;
        //std::cout << " " << secname() << " offset should be +ve valued but is " << offset_
        //                            << std::endl;
        DBGLOG_DUMP;
  ...
  if (!SecurityDefinitions::GetRemoveSelfOrdersFromBook(shortcode(), watch_.YYYYMMDD())) {
    DBGLOG_TIME_CLASS_FUNC_LINE << " For " << shortcode() << " GetRemoveSelfOrdersFromBook = "
                                << (SecurityDefinitions::GetRemoveSelfOrdersFromBook(shortcode(), watch_.YYYYMMDD())
                                        ? "true"
                                        : "false")
                                << " not using non_self." << DBGLOG_ENDL_FLUSH;
    return;
  }
...
/*
    std::cout << std::setprecision(10) << "[" << this->shortcode() << " " << ((!IsBidBookEmpty()) ? this->bid_order(0) : 0)
              << " " << ((!IsBidBookEmpty()) ? this->bid_size(0) : 0) << " "
              << ((!IsBidBookEmpty()) ? this->bid_price(0) : 0) << " " << ((!IsAskBookEmpty()) ? this->ask_price(0) : 0)
              << " " << ((!IsAskBookEmpty()) ? this->ask_size(0) : 0) << " "
              << ((!IsAskBookEmpty()) ? this->ask_order(0) : 0) << "] " << " bid_info.limit_int_price_: " << this->bid_info(0)->limit_int_price_ << " bid_info.limit_size_: " << this->bid_info(0)->limit_size_
              << " " << " ask_info.limit_int_price_: " << this->ask_info(0)->limit_int_price_ << " ask_info.limit_size_: " << this->ask_info(0)->limit_size_ 
              << " bid_int_price: " << this->bid_int_price(0) << " " << " ask_int_price: " << this->ask_int_price(0) << std::endl;
*/
  }
  else {
    market_update_info_.bidlevels_.insert(market_update_info_.bidlevels_.begin(),
...
// Synthetic variables related functions. Currently NSE specific as in NSE we allow the book to be crossed.
void SecurityMarketView::ComputeSyntheticVarsFromBaseVars(const unsigned int previous_best_index_,
                                                          const TradeType_t t_buysell_) {
  if (t_buysell_ == kTradeTypeBuy) {
    SetSyntheticBestBid(previous_best_index_);
  } else {
    SetSyntheticBestAsk(previous_best_index_);
  }
}

void SecurityMarketView::SetSyntheticBestBid(const unsigned int t_bid_index_) {
  syn_bestbid_index_ = t_bid_index_;
  syn_bestbid_price_ = market_update_info_.bidlevels_[t_bid_index_].limit_price_;
  syn_bestbid_int_price_ = market_update_info_.bidlevels_[t_bid_index_].limit_int_price_;
  syn_bestbid_size_ = market_update_info_.bidlevels_[t_bid_index_].limit_size_;
  syn_bestbid_ordercount_ = market_update_info_.bidlevels_[t_bid_index_].limit_ordercount_;
}

void SecurityMarketView::SetSyntheticBestAsk(const unsigned int t_ask_index_) {
  syn_bestask_index_ = t_ask_index_;
  syn_bestask_price_ = market_update_info_.asklevels_[t_ask_index_].limit_price_;
  syn_bestask_int_price_ = market_update_info_.asklevels_[t_ask_index_].limit_int_price_;
  syn_bestask_size_ = market_update_info_.asklevels_[t_ask_index_].limit_size_;
  syn_bestask_ordercount_ = market_update_info_.asklevels_[t_ask_index_].limit_ordercount_;
}

void SecurityMarketView::ComputeBestVarsFromSyntheticVars(const TradeType_t t_buysell_) {
  if (t_buysell_ == kTradeTypeBuy) {
    market_update_info_.bestbid_int_price_ = syn_bestbid_int_price_;
    market_update_info_.bestbid_price_ = syn_bestbid_price_;
    market_update_info_.bestbid_size_ = syn_bestbid_size_;
    market_update_info_.bestbid_ordercount_ = syn_bestbid_ordercount_;
  } else {
    market_update_info_.bestask_int_price_ = syn_bestask_int_price_;
    market_update_info_.bestask_price_ = syn_bestask_price_;
    market_update_info_.bestask_size_ = syn_bestask_size_;
    market_update_info_.bestask_ordercount_ = syn_bestask_ordercount_;
  }
}

void SecurityMarketView::SetSynVarsAsInvalid(const TradeType_t t_buysell_) {
  if (t_buysell_ == kTradeTypeBuy) {
    syn_bestbid_index_ = 0;
    syn_bestbid_price_ = kInvalidPrice;
    syn_bestbid_int_price_ = kInvalidIntPrice;
    syn_bestbid_size_ = -1;
    syn_bestbid_ordercount_ = -1;
  } else {
    syn_bestask_index_ = 0;
    syn_bestask_price_ = kInvalidPrice;
    syn_bestask_int_price_ = kInvalidIntPrice;
    syn_bestask_size_ = -1;
    syn_bestask_ordercount_ = -1;
  }
}
}
