...
FpgaEobiPriceLevelMarketViewManager::FpgaEobiPriceLevelMarketViewManager(
  ...
  for (size_t sec_id_ = 0; sec_id_ < security_market_view_map_.size(); sec_id_++) {
    ...
    security_market_view_map_[sec_id_]->market_update_info_.bidlevels_.resize(
        book_depth_, MarketUpdateInfoLevelStruct(0, kInvalidIntPrice, kInvalidPrice, 0, 0, watch_.tv()));
    security_market_view_map_[sec_id_]->market_update_info_.asklevels_.resize(
        book_depth_, MarketUpdateInfoLevelStruct(0, kInvalidIntPrice, kInvalidPrice, 0, 0, watch_.tv()));
  }
}

void FpgaEobiPriceLevelMarketViewManager::OnPriceLevelNew(const unsigned int t_security_id_,
  ...
  int old_price_ = 0;
  if (int_price_ == 0 && t_new_size_ == 0 &&
      t_new_ordercount_ == 0)  // End of Book update => delete this price level and those below this
  {
    switch (t_buysell_) {
      case kTradeTypeBuy: {
        bid_index_ = t_level_added_ - 1;
        for (int index_ = bid_index_; index_ < book_depth_; index_++) {
          smv_.market_update_info_.bidlevels_[index_] =
      ...
      case kTradeTypeSell: {
        ...
        for (int index_ = ask_index_; index_ < book_depth_; index_++) {
          smv_.market_update_info_.asklevels_[index_] =
              MarketUpdateInfoLevelStruct(0, kInvalidIntPrice, kInvalidPrice, 0, 0, watch_.tv());
        }
      } break;
      default: { } break; }
    ...
    switch (t_buysell_) {
      case kTradeTypeBuy: {
        ...
        if (bid_index_ < 0 || bid_index_ >= book_depth_) return;  // We are not concerned about this level
        ...
        if (bid_index_ == 0 && last_trade_side_[t_security_id_] == kTradeTypeSell &&
            ((expected_bestbid_int_price_[t_security_id_] == int_price_ &&
              (t_new_size_ <= expected_bestbid_size_[t_security_id_])) ||
             (expected_bestbid_int_price_[t_security_id_] > int_price_))) {
        ...
        {
          if (old_price_ != int_price_)
            l1_price_changed_[t_security_id_] = true;
          else if (old_size_ != t_new_size_)
        ...
        if (int_price_ >= smv_.market_update_info_.asklevels_[0].limit_int_price_) {
          ...
          DBGLOG_TIME_CLASS_FUNC_LINE << " " << smv_.secname()
                                      << " Ask side sanitization required even while using FPGA (Weird!) "
                                      << DBGLOG_ENDL_FLUSH;
          DBGLOG_DUMP;
          ...
          for (int temp_index_ = index_;
               (temp_index_ < book_depth_) && (smv_.market_update_info_.asklevels_[temp_index_].limit_size_ > 0);
               temp_index_++) {
            smv_.market_update_info_.asklevels_[temp_index_ - index_] =
                smv_.market_update_info_.asklevels_[temp_index_];
            smv_.market_update_info_.asklevels_[temp_index_] =
                MarketUpdateInfoLevelStruct(0, kInvalidIntPrice, kInvalidPrice, 0, 0, watch_.tv());
          }
      ...
      case kTradeTypeSell: {
        ...
        if (ask_index_ < 0 || ask_index_ >= book_depth_) return;  // We are not concerned about this level
        ...
        if (ask_index_ == 0 && last_trade_side_[t_security_id_] == kTradeTypeBuy &&
            ((expected_bestask_int_price_[t_security_id_] == int_price_ &&
              (t_new_size_ <= expected_bestask_size_[t_security_id_])) ||
             (expected_bestask_int_price_[t_security_id_] < int_price_))) {
          last_trade_side_[t_security_id_] = kTradeTypeNoInfo;
        }
        ...
        if (int_price_ <= smv_.market_update_info_.bidlevels_[0].limit_int_price_) {
          ...
          DBGLOG_TIME_CLASS_FUNC_LINE << " " << smv_.secname()
                                      << " Bid side sanitization required even while using FPGA (Weird!) "
                                      << DBGLOG_ENDL_FLUSH;
          DBGLOG_DUMP;
          ...
          for (int temp_index_ = index_;
               (temp_index_ < book_depth_) && (smv_.market_update_info_.bidlevels_[temp_index_].limit_size_ > 0);
               temp_index_++) {
            smv_.market_update_info_.bidlevels_[temp_index_ - index_] =
                smv_.market_update_info_.bidlevels_[temp_index_];
            smv_.market_update_info_.bidlevels_[temp_index_] =
                MarketUpdateInfoLevelStruct(0, kInvalidIntPrice, kInvalidPrice, 0, 0, watch_.tv());
          }
  ...
  if (!smv_.is_ready_) {
    if (smv_.market_update_info_.bidlevels_[0].limit_size_ > 0 &&
        smv_.market_update_info_.asklevels_[0].limit_size_ > 0 && smv_.ArePricesComputed()) {
      smv_.is_ready_ = true;
    }
  }
  ...
  if (smv_.pl_change_listeners_present_) {
    if (t_buysell_ == kTradeTypeBuy) {
      smv_.NotifyOnPLChangeListeners(t_security_id_, smv_.market_update_info_, t_buysell_, t_level_added_, int_price_,
                                     smv_.GetIntPx(smv_.market_update_info_.bestbid_price_ - t_price_), old_size_,
                                     t_new_size_, old_ordercount_, t_new_ordercount_, t_is_intermediate_message_,
                                     old_size_ == 0 ? 'N' : 'C');
    } else if (t_buysell_ == kTradeTypeSell) {
      smv_.NotifyOnPLChangeListeners(t_security_id_, smv_.market_update_info_, t_buysell_, t_level_added_, int_price_,
                                     smv_.GetIntPx(t_price_ - smv_.market_update_info_.bestask_price_), old_size_,
                                     t_new_size_, old_ordercount_, t_new_ordercount_, t_is_intermediate_message_,
                                     old_size_ == 0 ? 'N' : 'C');
    }
  }
...
void FpgaEobiPriceLevelMarketViewManager::OnPriceLevelChange(const unsigned int t_security_id_,
  ...
  OnPriceLevelNew(t_security_id_, t_buysell_, t_level_changed_, price_, t_new_size_, order_count_,
                  t_is_intermediate_message_);
}

void FpgaEobiPriceLevelMarketViewManager::OnPriceLevelDelete(const unsigned int t_security_id_,
                                                             const TradeType_t t_buysell_, const int t_level_removed_,
                                                             const double t_price_,
                                                             const bool t_is_intermediate_message_) {}

void FpgaEobiPriceLevelMarketViewManager::OnTrade(const unsigned int t_security_id_, const double t_trade_price_,
      ...
      else if (t_trade_int_price_ == smv_.market_update_info_.asklevels_[0].limit_int_price_)  // At best ask price
      {
        ...
        // Otherwise, update the best variables with the current level
          ...
          smv_.market_update_info_.bestask_size_ =
              smv_.market_update_info_.asklevels_[0].limit_size_ - (int)t_trade_size_;
          smv_.market_update_info_.bestask_price_ = smv_.market_update_info_.asklevels_[0].limit_price_;
          smv_.market_update_info_.bestask_ordercount_ = smv_.market_update_info_.asklevels_[0].limit_ordercount_;
        }
        ...
        for (index_ = 0; (index_ < book_depth_) && (smv_.market_update_info_.asklevels_[index_].limit_size_ > 0) &&
                         (smv_.market_update_info_.asklevels_[index_].limit_int_price_ < t_trade_int_price_);
             index_++) {
        ...
        if (size_deduction_ >= smv_.market_update_info_.asklevels_[index_].limit_size_) {
          ...
          if (next_ask_index_ >= book_depth_ ||
          ...
          smv_.market_update_info_.bestask_size_ =
              smv_.market_update_info_.asklevels_[index_].limit_size_ - size_deduction_;
      ...
      else if (t_trade_int_price_ == smv_.market_update_info_.bidlevels_[0].limit_int_price_)  // At best bid price
      {
        ...
        // Otherwise, update the best variables with the current level
          ...
          smv_.market_update_info_.bestbid_size_ =
              smv_.market_update_info_.bidlevels_[0].limit_size_ - (int)t_trade_size_;
          smv_.market_update_info_.bestbid_price_ = smv_.market_update_info_.bidlevels_[0].limit_price_;
          smv_.market_update_info_.bestbid_ordercount_ = smv_.market_update_info_.bidlevels_[0].limit_ordercount_;
        }
        ...
        for (index_ = 0; (index_ < book_depth_) && (smv_.market_update_info_.bidlevels_[index_].limit_size_ > 0) &&
                         (smv_.market_update_info_.bidlevels_[index_].limit_int_price_ > t_trade_int_price_);
             index_++) {
        ...
        if (size_deduction_ >= smv_.market_update_info_.bidlevels_[index_].limit_size_) {
          ...
          if (next_bid_index_ >= book_depth_ ||
          ...
          smv_.market_update_info_.bestbid_size_ =
              smv_.market_update_info_.bidlevels_[index_].limit_size_ - size_deduction_;
