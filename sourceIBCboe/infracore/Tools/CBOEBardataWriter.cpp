// =====================================================================================
//
//       Filename:  CBOEBardataWriter.cpp
//
//    Description:
//
//        Version:  1.0
...
int main(int argc, char* argv[]) {
  ...
  std::string logfilename = "/spare/local/MDSlogs/cboebardata_writer_dbg" + std::string("_") + exch_ + std::string("_") +
  ...
  HFSAT::SecurityDefinitions::GetUniqueInstance().LoadCBOESecurityDefinitions();
  HFSAT::Utils::CBOEDailyTokenSymbolHandler::GetUniqueInstance(tradingdate_);
  HFSAT::IBKRMD::IBKRL1MDHandler::GetUniqueInstance(dbglogger_, *simple_live_dispatcher, mode, nullptr);


  if (argc > 2 ) HFSAT::IBKRMD::IBKRL1MDHandler::GetUniqueInstance(dbglogger_, *simple_live_dispatcher, mode, nullptr).SetBardataPeriod(stoi(argv[1]));
  ...
  if (std::string(hostname).find("sdv-ny4-srv") == std::string::npos) {
    HFSAT::AllocateCPUUtils::GetUniqueInstance().AllocateCPUOrExit("CBOEBardataWriter");
  }
