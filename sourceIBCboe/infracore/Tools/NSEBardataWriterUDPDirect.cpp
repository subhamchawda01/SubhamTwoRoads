...
int main(int argc, char* argv[]) {
  ...
  std::string logfilename = "/spare/local/MDSlogs/nsebardata_writer_dbg" + std::string("_") +
  ...
  HFSAT::NSEMD::NSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher, nullptr, mode, exch_, true);
  if (argc > 2 ) HFSAT::NSEMD::NSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher, nullptr, mode, exch_, true).SetBardataPeriod(stoi(argv[1]));
  ...
  if (std::string(hostname).find("sdv-ny4-srv") == std::string::npos) {
    HFSAT::AllocateCPUUtils::GetUniqueInstance().AllocateCPUOrExit("NSEBardataWriter");
  }
