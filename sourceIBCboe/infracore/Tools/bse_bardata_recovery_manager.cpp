...
// =====================================================================================
//
//       Filename:  bse_bardata_recovery_manager.cpp
//
//    Description:
//
//        Version:  1.0
...
int main(int argc, char *argv[]) {
  ...
  std::string this_exchange = "BSE_BARDATA";

  // BSE_EQ_BARDATA OR BSE_FO_BARDATA
  if (argc > 1) {
  ...
  HFSAT::SecurityDefinitions::GetUniqueInstance(HFSAT::DateTime::GetCurrentIsoDateLocal()).LoadBSESecurityDefinitions();

  dbglogger << "LISTENING TO EXCHANGE(S):";
  for (auto exch_ : exchange_sources_to_listen) {
    dbglogger << " " << exch_;
    if (exch_ == "BSE" || "BSE_FO" == exch_ || "BSE_EQ" == exch_ || "BSE_CD" == exch_) {
      HFSAT::BSEMD::BSETBTRawMDHandler *bse_tbt_raw_handler = &(HFSAT::BSEMD::BSETBTRawMDHandler::GetUniqueInstance(
          dbglogger, simple_live_dispatcher, nullptr, HFSAT::kBardataRecoveryHost, exch_));

      BardataHandlerInterface<HFSAT::BSEMD::BSETBTRawMDHandler> *bse_tbt_raw_md_handler_interface_ptr =
          new BardataHandlerInterface<HFSAT::BSEMD::BSETBTRawMDHandler>(bse_tbt_raw_handler);

      recovery_interface.AddExchangeSource(exch_, bse_tbt_raw_md_handler_interface_ptr);
