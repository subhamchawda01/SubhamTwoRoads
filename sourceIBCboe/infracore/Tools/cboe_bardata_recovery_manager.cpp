...
// =====================================================================================
//
//       Filename:  cboe_bardata_recovery_manager.cpp
//
//    Description:
//
//        Version:  1.0
...
int main(int argc, char *argv[]) {
  ...
  std::string this_exchange = "CBOE_BARDATA";

  // CBOE_EQ_BARDATA OR CBOE_FO_BARDATA
  if (argc > 1) {
  ...
  t_temp_oss << "/spare/local/MDSlogs/cboe_bardata_recovery_manager_" << HFSAT::DateTime::GetCurrentIsoDateLocalAsString()
  ...
  HFSAT::SecurityDefinitions::GetUniqueInstance(HFSAT::DateTime::GetCurrentIsoDateLocal()).LoadCBOESecurityDefinitions();

  dbglogger << "LISTENING TO EXCHANGE(S):";
  for (auto exch_ : exchange_sources_to_listen) {
    dbglogger << " " << exch_;
    if ("CBOE_FO" == exch_) {
      HFSAT::IBKRMD::IBKRL1MDHandler *cboe_tbt_raw_handler = &(HFSAT::IBKRMD::IBKRL1MDHandler::GetUniqueInstance(
          dbglogger, simple_live_dispatcher, HFSAT::kBardataRecoveryHost));

      BardataHandlerInterface<HFSAT::IBKRMD::IBKRL1MDHandler> *cboe_tbt_raw_md_handler_interface_ptr =
          new BardataHandlerInterface<HFSAT::IBKRMD::IBKRL1MDHandler>(cboe_tbt_raw_handler);

      recovery_interface.AddExchangeSource(exch_, cboe_tbt_raw_md_handler_interface_ptr);
  ...
  if (std::string(hostname).find("sdv-ny4-srv") == std::string::npos) {
    HFSAT::AllocateCPUUtils::GetUniqueInstance().AllocateCPUOrExit("cboe_bardata_recovery_manager");
  }
