...
#define GIVEUP_APP_CONFIRM_TM 4506
...
#define BCAST_SEC_MSTR_CHNG_PERIODIC 7340
...
#define USER_ORDER_LIMIT_UPDATE_OUT 5731
#define DEALER_LIMIT_UPDATE_OUT 5733
#define BATCH_ORDER_CANCELLATION_TR 9002
#define KILL_SWITCH_REQUEST 2062
#define KILL_SWITCH_RESPONSE 2061
#define KILL_SWITCH_REJECT 2231
#define KILL_SWITCH_IN 2062
#define SECURE_BOX_REGISTRATION_REQUEST 23008
#define SECURE_BOX_REGISTRATION_RESPONSE 23009
...
struct ProcessedGRResponse {
  ...
  std::string ToString() {
    ...
    t_temp_oss << "BOX -> " << (int32_t)box_id << " "
               ...
               << "PORT -> " << port << " "
               << "KEY -> " << signon_key << " "
               << "CRYTPOGRAPHIC KEY -> " << cryptographic_key << " "
               << "CRYTPOGRAPHIC IV-> " << cryptographic_iv<<std::endl;
...
struct ProcessedControlMsgResponse {
  int32_t trade_id;
  char action_code[4];
  int16_t msg_len;
  char msg[241];
...
struct ProcessedOrderLimitUpdateResponseDerivatives {
  char broker_id[6];
  int16_t branch_id;
  int32_t user_id;
  int64_t branch_buy_limit;
  int64_t branch_sell_limit;
  int64_t branch_used_sell_limit;
  int64_t branch_used_buy_limit;
  int64_t user_order_buy_limit;
  int64_t user_order_sell_limit;
  int64_t user_order_user_sell_limit;
  int64_t user_order_user_buy_limit;
  std::string ToString() {
    std::stringstream ss;
    ss << "ProcessedOrderLimitUpdateResponse ->" 
       << " broker_id: " << broker_id
       << " branch_id: " << branch_id
       << " user_id: " << user_id
       << " branch_buy_limit: " << branch_buy_limit
       << " branch_sell_limit: " << branch_sell_limit
       << " branch_used_sell_limit: " << branch_used_sell_limit 
       << " branch_used_buy_limit: " << branch_used_buy_limit
       << " user_order_buy_limit: " << user_order_buy_limit
       << " user_order_sell_limit: " << user_order_sell_limit 
       << " user_order_user_sell_limit: " << user_order_user_sell_limit
       << " user_order_user_buy_limit: " << user_order_user_buy_limit << "\n";
    std::string str = ss.str();
    return str;
  }
};

struct ProcessedOrderLimitUpdateResponseCashMarket {
  char broker_id[6];
  int16_t branch_id;
  int32_t user_id;
  int64_t user_order_buy_limit;
