...
#ifndef BASE_BASICORDERROUTINGSERVER_THROTTLEMANAGER_H
#define BASE_BASICORDERROUTINGSERVER_THROTTLEMANAGER_H
...
class ThrottleManager {
 private:
  void EmailForMemoryCorruption(std::string _mail_body_) {
    // also send an alert
    char hostname[128];
    hostname[127] = '\0';
    ...
    e.setSubject("ERROR : Definite Memory Corruption");
 ...
 public:
  virtual bool reject_for_time_throttle() {
...
//      EmailForMemoryCorruption(t_temp_oss.str());
    ...
    min_throttle_wait = min_throttle_lag_ - (this_timestamp - throttle_timestamp_buffer_[oldest_time_stamp_indx]);
#if TROTTLE_DEBUG_LOGGING
  ...
  uint64_t min_cycle_throttle_wait(){
  ...
  void throttle_value_update(int new_throttle_size){
    ...
    curr_indx_ = 0;
    min_throttle_wait = 0;
    throttle_timestamp_buffer_ = (uint64_t*)calloc(new_throttle_size, sizeof(uint64_t));
    DBGLOG_CLASS_FUNC_LINE_INFO << "THROTTLE UPDATING. SIZE: " << size_ << " TIME: " <<GetReadTimeStampCounter() << DBGLOG_ENDL_FLUSH;
    this_thread::sleep_for(std::chrono::milliseconds(1000) );
    DBGLOG_CLASS_FUNC_LINE_INFO << "THROTTLE UPDATED. SIZE: " << size_ << " TIME: " << GetReadTimeStampCounter() << DBGLOG_ENDL_FLUSH;
  }
  ...
  virtual bool peek_reject_for_time_throttle(int no_of_orders_) {
...
// we are free to send message
  ...
  ThrottleManager(DebugLogger& dbg, int t_default_size = THROTTLE_MSG_LIMIT, bool use_lock = true)
      : dbglogger_(dbg),
        size_(t_default_size),
        curr_indx_(0),
        size_minus_1_(t_default_size - 1),
        throttle_timestamp_buffer_((uint64_t*)calloc(t_default_size, sizeof(uint64_t))),
        min_throttle_wait(0) {
    // TROTTLE_LAG_MULTIPLIER = 1.05, signifies 1050 milliseconds.
    uint64_t no_cpucycles_per_second = HFSAT::GetNoOfCpuCyclesPerSecond();
    min_throttle_lag_ = TROTTLE_LAG_MULTIPLIER * no_cpucycles_per_second;

    DBGLOG_CLASS_FUNC_LINE_INFO << "THROTTLE no_cpucycles_per_second: " << no_cpucycles_per_second
                                << " min_throttle_lag_: " << min_throttle_lag_ << DBGLOG_ENDL_FLUSH;
  }

 private:
  ThrottleManager(const ThrottleManager&);  // no-copy
  ...
  uint64_t* throttle_timestamp_buffer_;

  //! This field is machine dependent(number of CPU cycles per second).
  uint64_t min_throttle_lag_;
  uint64_t min_throttle_wait;
};
}
}
