...
class AccountThread : public EngineListener {
 ...
 public:
  ...
  void OnORSReject(Order* ord, const ORSRejectionReason_t rejection_reason_ = kExchOrderReject, uint64_t min_throttle_wait_ = 0) override;
  void OnReject(int server_assigned_seqnum, const ORSRejectionReason_t rejection_reason_,
                uint64_t _exch_seq_ = 0, uint64_t min_throttle_wait_ = 0) override;
  ...
  void OnOrderConf(int server_assigned_seqnum, const char* exch_assigned_seqnum, double price, int size,
                   ...
                   int entry_dt = 0, int64_t last_activity_ref = 0) override;  // NSE Conf
  ...
  void OnOrderExec(int server_assigned_seqnum, const char* symbol, TradeType_t trade_type, double price,
                   ...
                   int64_t last_activity_ref = 0) override;
  ...
  void DumpSettingsToFile() override;
  ...
  void ForceBatchCancelBroadcastSAOS(char const * saos) override;
  void ForceBatchCancelBroadcastSymbol(char const * shortcode) override;
  void ForceBatchCancelBroadcastAll(bool isMarginBreached) override;
  void ForceBatchCancelBroadcastAllOrderRemoval() override;
  void RequestORSPNL();
  void RequestORSOpenPositions();

  void SetMarginValue(double margin);
