...
class MultiSessionEngine : public BaseEngine, public SimpleSecuritySymbolIndexerListener {
 ...
 public:
  ...
  void SendOrder(Order* ord) override {
    ...
    dbglogger_ << "Sending order to CBOE Engine\n";
    eng->SendOrder(ord);
    dbglogger_ << "Sending order to CBOE Engine completed\n";

    dbglogger_ << "Adding to Order Manager map\n";
    order_manager_.AddToActiveMap(ord);
    dbglogger_ << "Adding to Order Manager map completed\n";
...
*/
 ...
 private:
  ...
  dbglogger_ << "MultiSessions::RequestORSPNL\n";
  ...
  void RequestORSOpenPositions(){
    ...
    dbglogger_ << "MultiSessions::RequestORSOpenPositions\n";

   for (auto e : engines_) {
      e->RequestORSOpenPositions();
    } 
  }

  void FetchMarginUsage() {

    dbglogger_ << "MultiSessions::FetchMarginUsage\n";
  ...
  void RequestExecutedOrders() {
    ...
    dbglogger_ << "MultiSessions::RequestExecutedOrders\n";

    for (auto e : engines_) {
      e->RequestExecutedOrders();
    }
...
BaseEngine* makeMultiSessionEngine(const std::string& ex, Settings& settings, HFSAT::DebugLogger& logger,
  ...
  else if ("CBOE_EQ" == ex || "CBOE_FO" == ex || "CBOE_CD" == ex)
    en = new MultiSessionEngine<HFSAT::CBOE::CBOEEngine>(settings, logger, output_log_dir, ex);
  else if ("MSSIM" == ex)
