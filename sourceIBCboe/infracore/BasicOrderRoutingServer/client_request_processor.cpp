...
ClientRequestProcessor::ClientRequestProcessor(DebugLogger& dbglogger, AccountThread& account_thread,
  ...
  if (std::string("NSE_FO") == settings_.getValue("Exchange") ||
      ...
      std::string("BSE_FO") == settings_.getValue("Exchange") ||
      std::string("BSE_CD") == settings_.getValue("Exchange") ||
      std::string("BSE_EQ") == settings_.getValue("Exchange") || 
      std::string("CBOE_EQ") == settings_.getValue("Exchange") ||
      std::string("CBOE_FO") == settings_.getValue("Exchange") ||
      std::string("CBOE_CD") == settings_.getValue("Exchange") ) {
  ...
  next_new_order_for_send_->ors_order_ptr_ = reinterpret_cast<uint64_t>(next_new_order_for_send_);
}
  ...
  switch (req.orq_request_type_) {
    ...
    case ORQ_IOC: {
      if (req.is_mirror_order_ && req.mirror_factor_ > 1) {
        ...
        dbglogger_ << "ClientRequestProcessor::ProcessClientRequest Processing Order New Order Send Request\n";
        ProcessOrderSendRequest(req, saci);
        dbglogger_ << "ClientRequestProcessor::ProcessClientRequest Processing Order New Order Send Request Completed\n";
      }
      return;
    }
    case ORQ_CANCEL: {
      dbglogger_ << "ClientRequestProcessor::ProcessClientRequest Processing Cancel Order Request\n";
      ProcessOrderCancelRequest(req, saci);
      dbglogger_ << "ClientRequestProcessor::ProcessClientRequest Processing Cancel Order Request Completed\n";
      return;
    }
    case ORQ_CXLREPLACE: {
      dbglogger_ << "ClientRequestProcessor::ProcessClientRequest Processing Modify Order Request\n";
      ProcessOrderCancelReplace(req, saci);
      dbglogger_ << "ClientRequestProcessor::ProcessClientRequest Processing MOdify Order Request Completed\n";
      return;
    }
    case ORQ_HEARTBEAT: {
      dbglogger_ << "ClientRequestProcessor::ProcessClientRequest Processing Heartbeat Request\n";
    ...
    case ORQ_EXCHANGE_TCP_READ: {
      ProcessExchangeTCPReadRequest();
      return;
    }
    case ORQ_THROTTLE_UPDATE: {
      ProcessUpdateThrottle(req);
      return;
    }

    case ORQ_KILLSWITCH_REQ: {
      ProcessKillSwitch(req,saci);
      return;
    }
...
void ClientRequestProcessor::ProcessHeartBeatRequest() { p_base_engine_->CheckToSendHeartbeat(); }

void ClientRequestProcessor::ProcessUpdateThrottle(GenericORSRequestStruct& req) { 
    p_base_engine_->ProcessThrottleIncreaseReq(req.price_, req.int_price_); 
}
...
void ClientRequestProcessor::ProcessExchangeTCPReadRequest() { p_base_engine_->ProcessLockFreeTCPDirectRead(); }
  ...
  switch (fake_cached_req_.orq_request_type_) {
    case ORQ_SEND: {
      ...
      if (ORQ_SEND == fake_cached_req_.orq_request_type_) {
        margin_retval = margin_checker_.Allows(security_id, fake_cached_req_.price_, fake_cached_req_.size_requested_,
                                               fake_cached_req_.buysell_);
      }
...
void ClientRequestProcessor::ProcessOrderMirrorRequest(GenericORSRequestStruct& req, int saci) {
  ...
  ORSRejectionReason_t margin_retval =
  ...
  if (margin_retval != kORSOrderAllowed) {
    if (dbglogger_.CheckLoggingLevel(ORS_ERROR)) {
      ...
      dbglogger_ << "OrderRequest: Side/Size/Price: " << req.buysell_ << " " << req.size_requested_ * mirror_factor
                 ...
                 << " LiveOrderBid: " << position_manager_.GetGlobalBidSize(security_id)
                 << " LiveOrderAsk: " << position_manager_.GetGlobalAskSize(security_id)
  ...
  p_new_order->is_active_order = true;
  p_new_order->is_reserved_type_ = req.is_reserved_type_;
  

  const ProfilerTimeInfo& time_info = runtime_profiler_.GetProfilerTimeInfo();
  p_new_order->csw_start_cycle_count_ = time_info.cshmw_start_time;
  //p_new_order->csw_end_cycle_count_ = time_info.cshmw_end_time;
  //p_new_order->query_start_cycle_count_ = time_info.tradeinit_start_time;
  //p_new_order->query_end_cycle_count_ = time_info.tradeinit_end_time;
  //p_new_order->ors_start_cycle_count_ = time_info.ors_start_time;
  p_new_order->query_order_ptr_ = req.query_order_ptr_;

  //========================================================================

  BaseEngineMirrorOrder(p_new_order, mirror_factor);

  // Done at the multisession engine
...
void ClientRequestProcessor::ProcessOrderSendRequest(GenericORSRequestStruct& req, int saci) {
#if BSE_DEBUG_INFO
  DBGLOG_CLASS_FUNC_LINE_INFO << "ClientRequestProcessor::ProcessOrderSendRequest " << DBGLOG_ENDL_DUMP;
#endif

  const int _security_id_ = simple_security_symbol_indexer_.GetIdFromChar16(req.symbol_);
#if BSE_DEBUG_INFO
  DBGLOG_CLASS_FUNC_LINE_INFO << "security_id: " << _security_id_ << " " 
                              << "symbol: " << req.symbol_ << DBGLOG_ENDL_DUMP;
#endif
  ...
  if (_security_id_ < 0) {
    ...
    DBGLOG_CLASS_FUNC << "NO ADDTS FOR " << req.symbol_ << "\n";
    DBGLOG_CLASS_FUNC << "orq_request_type_ = ORQ_SEND margin_retval = "
                        << "Failed Worstcaseposition Check:" << "\n";

    dbglogger_ << "OrderRequest: Side/Size/Price: " << req.buysell_ << " " << req.size_requested_ << " @ "
                 << req.price_ << " CurrPos: " << 0
                 << " LiveOrderBid: " << 0
                 << " LiveOrderAsk: " << 0
  ...
  ORSRejectionReason_t margin_retval =
      margin_checker_.Allows(_security_id_, req.price_, req.size_requested_, req.buysell_);
  if (margin_retval != kORSOrderAllowed) {
    if (dbglogger_.CheckLoggingLevel(ORS_ERROR)) {
      DBGLOG_CLASS_FUNC << "orq_request_type_ = ORQ_SEND margin_retval = "
                        << HFSAT::ORSRejectionReasonStr(margin_retval) << "\n";
      dbglogger_ << "OrderRequest: Side/Size/Price: " << req.buysell_ << " " << req.size_requested_ << " @ "
                 ...
                 << " LiveOrderBid: " << position_manager_.GetGlobalBidSize(_security_id_)
                 << " LiveOrderAsk: " << position_manager_.GetGlobalAskSize(_security_id_)
                 << " Current Risk Limit for " << req.symbol_
  ...
  //============================================================================
#if BSE_DEBUG_INFO
  DBGLOG_CLASS_FUNC_LINE_INFO << "MarginCheck: " << DBGLOG_ENDL_DUMP;
#endif
  ...
  p_new_order_->is_active_order = true;
  p_new_order_->is_reserved_type_ = req.is_reserved_type_;
  p_new_order_->csw_start_cycle_count_ = req.t2t_cshmw_start_time_;

  const ProfilerTimeInfo& time_info = runtime_profiler_.GetProfilerTimeInfo();
  p_new_order_->csw_start_cycle_count_ = time_info.cshmw_start_time;
  //p_new_order_->csw_end_cycle_count_ = time_info.cshmw_end_time;
  //p_new_order_->query_start_cycle_count_ = time_info.tradeinit_start_time;
  //p_new_order_->query_end_cycle_count_ = time_info.tradeinit_end_time;
  //p_new_order_->ors_start_cycle_count_ = time_info.ors_start_time;
  //p_new_order_->query_order_ptr_ = req.query_order_ptr_;

  p_new_order_->is_ioc = (req.orq_request_type_ != ORQ_IOC) ? false : true;
  ...
  dbglogger_ << "ClientRequestProcessor::ProcessOrderSendRequest \n"; 
  dbglogger_ << p_new_order_->toString() << "\n";
...
// Done At the multisession engine send call
...
void ClientRequestProcessor::ProcessOrderCancelRequest(GenericORSRequestStruct& req, int saci) {
  ...
  Order* p_this_order_ = reinterpret_cast<Order*>(req.ors_order_ptr_);

  if (p_this_order_ == NULL || p_this_order_->server_assigned_order_sequence_ != req.server_assigned_order_sequence_) {
    p_this_order_ = order_manager_.GetOrderByOrderSequence(req.server_assigned_order_sequence_);
    //DBGLOG_CLASS_FUNC_LINE_ERROR << "REVERT BACK TO FAIL SAFE FOR : " << req.server_assigned_order_sequence_
    //                             << DBGLOG_ENDL_FLUSH;
  }

  if (p_this_order_ == NULL) {
    DBGLOG_CLASS_FUNC_LINE_INFO << "ClientRequestProcessor::ProcessOrderCancelRequest NULL " << DBGLOG_ENDL_DUMP;
    // Most likely already cancelled or filled
    return;
  }

  if (p_this_order_->order_not_found_) {
    DBGLOG_CLASS_FUNC_LINE_INFO << "ClientRequestProcessor::ProcessOrderCancelRequest order_not_found_ kCxlRejectReasonOrderNotConfirmed" << DBGLOG_ENDL_DUMP;
  ...
  if ((false == allow_cxl_before_conf_) && (false == p_this_order_->is_confirmed_)) {
    DBGLOG_CLASS_FUNC_LINE_INFO << "ClientRequestProcessor::ProcessOrderCancelRequest is_confirmed_ kCxlRejectReasonOrderNotConfirmed" << DBGLOG_ENDL_DUMP;
    bcast_manager_->BroadcastCancelRejection(*p_this_order_, kCxlRejectReasonOrderNotConfirmed);
    return;
  }

  // Check for throttle before sending cancel to exchange
  p_this_order_->is_reserved_type_ = req.is_reserved_type_;
  ...
  const ProfilerTimeInfo& time_info = runtime_profiler_.GetProfilerTimeInfo();
  p_this_order_->csw_start_cycle_count_ = time_info.cshmw_start_time;
  //p_this_order_->csw_end_cycle_count_ = time_info.cshmw_end_time;
  //p_this_order_->query_start_cycle_count_ = time_info.tradeinit_start_time;
  //p_this_order_->query_end_cycle_count_ = time_info.tradeinit_end_time;
  //p_this_order_->ors_start_cycle_count_ = time_info.ors_start_time;
  p_this_order_->query_order_ptr_ = req.query_order_ptr_;
...
void ClientRequestProcessor::ProcessOrderCancelReplace(GenericORSRequestStruct& req, int saci) {
  ...
  if (p_this_order_ == NULL || p_this_order_->server_assigned_order_sequence_ != req.server_assigned_order_sequence_ || p_this_order_->size_remaining_ == 0) {
    p_this_order_ = order_manager_.GetOrderByOrderSequence(req.server_assigned_order_sequence_);
    //DBGLOG_CLASS_FUNC_LINE_ERROR << "REVERT BACK TO FAIL SAFE FOR : " << req.server_assigned_order_sequence_
    //                             << DBGLOG_ENDL_FLUSH;
  }
  ...
  if (p_this_order_ != NULL && p_this_order_->is_confirmed_) {  // Found order
...
#if BSE_DEBUG_INFO
    DBGLOG_CLASS_FUNC_LINE_INFO << "ClientRequestProcessor::ProcessOrderCancelReplace is_confirmed_ " << DBGLOG_ENDL_DUMP;
#endif
    ...
    // Check if this order is allowed according to margin values
    ORSRejectionReason_t margin_retval = margin_checker_.Allows(_security_id_, req.price_, req.size_requested_,
                                                                req.buysell_, p_this_order_->size_remaining_);

    if (margin_retval != kORSOrderAllowed) {
      DBGLOG_CLASS_FUNC << "orq_request_type_ = ORQ_SEND margin_retval = "
                        << HFSAT::ORSRejectionReasonStr(margin_retval) << "\n";
      dbglogger_ << " Current Risk Limit for " << req.symbol_ << " : MaxPos : " << margin_checker_.getMaxPos(_security_id_)
                 << " LiveOrderBid: " << position_manager_.GetGlobalBidSize(_security_id_)
                 << " LiveOrderAsk: " << position_manager_.GetGlobalAskSize(_security_id_)
                 << " Max OrdSize : " << margin_checker_.getMaxOrdSize(_security_id_)
                 << " Max Live Orders : " << margin_checker_.getMaxLiveOrd(_security_id_)
    ...
    p_base_engine_->ModifyOrder(&modify_order_, p_this_order_);
...
//    DBGLOG_CLASS_FUNC_LINE_INFO << "kCxlReRejectOrderNotFound " 
//                                << "SAOS: " << req.server_assigned_order_sequence_ << DBGLOG_ENDL_DUMP;
...
void ClientRequestProcessor::CancelPendingOrders(int saci) {
  ...
  DBGLOG_CLASS_FUNC_LINE_INFO << "CancelPendingOrders " << saci << " Orders_Vec : " << orders.size()
                              << DBGLOG_ENDL_DUMP;
  /*
  ...
  for (auto order : orders) {
    if (order && (order->server_assigned_client_id_ == saci) && order->is_confirmed_ && order->size_remaining_ > 0) {
      DBGLOG_CLASS_FUNC_LINE_INFO << "CancelPendingOrders " << saci
                                  << " SAOS : " << order->server_assigned_order_sequence_
                                  << " CONF : " << order->is_confirmed_ << " SR : " << order->size_remaining_
                                  << " OrderSACI : " << order->server_assigned_client_id_ << DBGLOG_ENDL_DUMP;
...
void ClientRequestProcessor::BaseEngineModify(Order* this_order, Order* orig_order) {
  p_base_engine_->ModifyOrder(this_order, orig_order);
}
...
void ClientRequestProcessor::ProcessKillSwitch(GenericORSRequestStruct& req, int saci) {

  const int _security_id_ = simple_security_symbol_indexer_.GetIdFromChar16(req.symbol_);

  if (_security_id_ < 0) {
    bcast_manager_->BroadcastORSRejection(req, saci, kORSRejectSecurityNotFound);
    DBGLOG_CLASS_FUNC << "NO KILL SWITCH FOR " << req.symbol_ << " SYMBOL NOT FOUND : SACI: "<<saci<<"\n";
    return;
  }

  p_base_engine_->KillSwitch(_security_id_);
}
}
}
