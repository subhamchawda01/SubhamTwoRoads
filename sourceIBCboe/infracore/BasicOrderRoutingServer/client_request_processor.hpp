...
class ClientRequestProcessor {
 public:
  ClientRequestProcessor(DebugLogger& dbglogger, AccountThread& account_thread, Settings& settings);
  ...
  void ProcessUpdateThrottle(GenericORSRequestStruct& req);
  ...
  void ProcessExchangeTCPReadRequest();
  ...
  void BaseEngineModify(Order* this_order, Order* orig_order);
  ...
  void ProcessKillSwitch(GenericORSRequestStruct& req, int saci);
