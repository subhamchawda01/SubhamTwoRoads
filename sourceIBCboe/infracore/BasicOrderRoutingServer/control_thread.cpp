...
void ControlThread::AddtradingSymbol(std::string& shortcode_temp, int max_pos, int max_order_size,std::ostringstream& report_string){
  ...
  if (t_security_id_ < 0) {
    ...
    dbglogger_ << "SEC ID " << t_security_id_ << " SHC : " << shortcode_temp << "\n";
...
*/
    ...
    if ((tokens_.size() >= 2) && (strcmp(tokens_[0], "CONTROLCOMMAND") == 0)) {
      ...
      switch (_this_control_command_) {
        ...
        case kAddComboProduct: {
          /// CONTROLCOMMAND ADDCOMBOSYMBOL COMBOPRODUCTSTRING

          std::vector<std::pair<std::string,int32_t>> comboProducts;
          std::cout << "Inside kAddComboProduct " << std::endl;
          if(tokens_.size()<=2){
            std::cerr<<"Please send a valid control command"<<std::endl;
            break;
          } 
          std::cout << "The combo string is :" << tokens_[2] << std::endl;
          if(tokens_.empty()){
            std::cerr<<"The combo string is empty please try again\n"<<std::endl;
            break;
          }
          comboProducts = ComboProductHandler::convertComboStringToVector(tokens_[2]);
 
          const std::string shortcodeOfDay=ControlReceiver::getUniqueIdForComboSymbol(comboProducts);
          HFSAT::IBUtils::AddUpdateComboProductMap::updateTheInternalMaps(tokens_[2],shortcodeOfDay,HFSAT::GlobalSimDataManager::GetUniqueInstance(dbglogger_).GetTradingDate());
          std::cout<<"Shortcode received is: "<<shortcodeOfDay<<" "<<"Exchange SYmbol is: "<<HFSAT::ExchangeSymbolManager::GetExchSymbol(shortcodeOfDay)<<"\n";

          // simple_security_symbol_indexer_.AddString(HFSAT::ExchangeSymbolManager::GetExchSymbol(shortcodeOfDay));

          //Will update the max pos and sz 
          // control_receiver_->addMaxPosAndOrdSzForComboProductCBOE(shortcodeOfDay,2,2);
          //AddSSSIListener do we need do this also? Check(Sanskar)
          // Now send shortcodeOfDay over the socket
          ssize_t sent_size = send(connected_socket_file_descriptor_,
                                  shortcodeOfDay.c_str(),
                                  shortcodeOfDay.size(),
                                  0);

          if (sent_size == -1) {
            std::cerr<<"Error sending the shortcode"<<std::endl;
          }
        } break;

        case kListenComboProductData: {
          ...
          std::cout << "Inside kListenComboProductData " << std::endl; 
  ...
  case kDumpORSPositions: {
	...
	case kClearPositionManager: {
	  if (2 == tokens_.size()) {
        ...
        case kRequestPNL:{
              dbglogger_ << "Request PNL For TWS ORS... "<< "\n";
              m_athread->RequestORSPNL();
              dbglogger_.DumpCurrentBuffer();
              
         } break;
        
        case kRequestOpenPositions:{
            dbglogger_ << "Request TWS Open postions... "<< "\n";
            m_athread->RequestORSOpenPositions();
            dbglogger_.DumpCurrentBuffer();
        } break;

        case kMarginUsage:{
            dbglogger_ << "Request TWS Margin Usage... "<< "\n";
            m_athread->FetchMarginUsage();
            dbglogger_.DumpCurrentBuffer();
        } break;

        case kReqExecutions:{
          dbglogger_ << "Request TWS Orders Executions\n";
          m_athread->RequestExecutedOrders();
          dbglogger_.DumpCurrentBuffer();
        } break;
