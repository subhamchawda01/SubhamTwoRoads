 ...
 public:
  ...
  void OnOrderExec(int32_t sec_id, int32_t size_executed, double price, HFSAT::TradeType_t trade_type) {
    ...
    if (0 == secdb.n2d) {
      ...
      else if (std::string::npos != secdb.shortcode.find("CBOE_")) {
        DBGLOG_CLASS_FUNC_LINE_INFO << "CBOE ORDER EXECUTED" << DBGLOG_ENDL_FLUSH;
        secdb.commission = HFSAT::CBOESecurityDefinitions::GetCBOECommission(secdb.shortcode);
        secdb.n2d = 100;
        secdb.is_cboe = true;
      }
    ...
    DBGLOG_CLASS_FUNC_LINE_INFO << "SEC: " << secdb.shortcode << " " << secdb.unreal_pnl << " " << secdb.pos << " " << secdb.realized_pnl << DBGLOG_ENDL_FLUSH;
