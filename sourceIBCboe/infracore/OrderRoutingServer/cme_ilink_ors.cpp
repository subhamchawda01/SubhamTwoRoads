...
void termination_handler_segv(int signum) {
  ...
  std::string subject = "Fatal Error: " + exchange + " ORS received SIGSEGV in " + std::string(hostname);
...
int main(int argc, char** argv) {
  ...
  std::cout << "Zocket Error Lists::\n" 
	    << "Incorrect Arguements supplied: " << -EINVAL << "\n"
	    << "Zocket is not in a valid TCP state for sending: " << -ENOTCONN << "\n"
	    << "Not Enough Space in the send queue: " << -EAGAIN << "\n"
	    << "Not enough packet buffers available: " << -ENOMEM << "\n"
	    << std::endl;
/*
...
*/
  ...
  HFSAT::ORSUtils::ORSMarginManager& ors_margin_manager =
      HFSAT::ORSUtils::ORSMarginManager::GetUniqueInstance(dbglogger_,exch_);
  if (p_settings_->getValue("Exchange") == std::string("NSE_CD") ||
      p_settings_->getValue("Exchange") == std::string("NSE_FO") ||
      p_settings_->getValue("Exchange") == std::string("MSSIM") ||
      p_settings_->getValue("Exchange") == std::string("NSE_EQ") ||
      p_settings_->getValue("Exchange") == std::string("BSE_CD") ||
      p_settings_->getValue("Exchange") == std::string("BSE_FO") ||
      p_settings_->getValue("Exchange") == std::string("BSE_EQ") ||
      p_settings_->getValue("Exchange") == std::string("CBOE_EQ") ||
      p_settings_->getValue("Exchange") == std::string("CBOE_FO") ||
      p_settings_->getValue("Exchange") == std::string("CBOE_CD")) {
    if (p_settings_->has("GrossMarginCheck") && p_settings_->has("NetMarginCheck")) {
      ...
      if (p_settings_->getValue("Exchange") == std::string("NSE_FO") ||
          p_settings_->getValue("Exchange") == std::string("BSE_FO")) {  // Margin Factor Can Only Be Used For FO
 ...
 //std::cout<<"LOADING security"<< std::string(p_settings_->getValue("Exchange")<<std::endl;
  ...
  std::cout << "benchmark 1.1\n";

  long int max_exposure_limit = 0;
  if(p_settings_->has("MaxExposure")){
      max_exposure_limit = atoi(p_settings_->getValue("MaxExposure").c_str());
      dbglogger_ << "Max Exposure Value Present  " <<  max_exposure_limit << "\n";
      dbglogger_.DumpCurrentBuffer();
  }
  else{
      dbglogger_ << "Max Exposure Value Absent  " <<  max_exposure_limit << "\n";
      dbglogger_.DumpCurrentBuffer();
  }

  std::string use_max_exposure = "N";
  if(p_settings_->has("UseMaxExposure")){
      use_max_exposure = std::string(p_settings_->getValue("UseMaxExposure"));
      dbglogger_ << "Use Exposure Value Present  " <<  use_max_exposure << "\n";
      dbglogger_.DumpCurrentBuffer();
  }
  else{
    dbglogger_ << "Use Exposure Value Absent  " <<  use_max_exposure << "\n";
    dbglogger_.DumpCurrentBuffer();
  }
  ...
  HFSAT::ORS::MarginChecker::GetUniqueInstance(dbglogger_,
                                               HFSAT::StringToExchSource(p_settings_->getValue("Exchange"))).setMaxExposure(max_exposure_limit);

  
  if("Y" == use_max_exposure){
    HFSAT::ORS::MarginChecker::GetUniqueInstance(dbglogger_,
                                               HFSAT::StringToExchSource(p_settings_->getValue("Exchange"))).setUseMaxExposure(true);
  }  
