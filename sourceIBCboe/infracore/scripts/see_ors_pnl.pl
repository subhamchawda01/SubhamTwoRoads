...
#=============================================================#
my $is_sgx_cn_commish_updated = 0;  #Flag to check whether SGX_CN commmission is updated or not
my $SGX_CN_0_EXCH_SYMBOL = `$BASETRADE_BIN/get_exchange_symbol  "SGX_CN_0" $input_date_ ` ; chomp ( $SGX_CN_0_EXCH_SYMBOL); #Get exchange symbol for SGX_CN_0
...
#compute discount in commission only for US hours volumes
  ...
  {
	if(exists $symbol_to_total_commish_map_{$symbol_} && $symbol_to_currency_map_{$symbol_} != 0) {
		printf "%s|%s|%s\n", $symbol_, $symbol_to_total_commish_map_{$symbol_}/$symbol_to_currency_map_{$symbol_},$symbol_to_total_commish_map_{$symbol_};
	}
  }
...
sub GetCurrencyConversion
{
  ...
  my $curr_filename_ = '/spare/local/files/CurrencyData/currency_rates_'.$input_date_.'.txt' ;
  my $attempt_ = 1 ;
  ...
  for ( my $itr = 0 ; $itr <= $#curr_file_lines_; $itr ++ )
  {
    ...
    if ( $#cwords_ >= 1 )
    {
      ...
      if ( index ( $currency_, "EURUSD" ) == 0 ){
        $EUR_TO_DOL = sprintf( "%.6f", $cwords_[1] );
      }

      if( index ( $currency_, "USDCAD" ) == 0 ){
        $CD_TO_DOL = sprintf( "%.6f", 1 / $cwords_[1] );
      }

      if( index ( $currency_, "USDBRL" ) == 0 ){
        $BR_TO_DOL = sprintf( "%.6f", 1 / $cwords_[1] );
      }

      if( index ( $currency_, "GBPUSD" ) == 0 ){
        $GBP_TO_DOL = sprintf( "%.6f", $cwords_[1] );
      }

      if( index ( $currency_, "USDHKD" ) == 0 ){
        $HKD_TO_DOL = sprintf( "%.6f", 1 / $cwords_[1] );
      }

      if( index ( $currency_, "USDJPY" ) == 0 ){
        $JPY_TO_DOL = sprintf( "%.6f", 1 / $cwords_[1] );
      }

      if( index ( $currency_, "USDRUB" ) == 0 ){
        $RUB_TO_DOL = sprintf( "%.6f", 1 / $cwords_[1] );
      }

      if( index ( $currency_, "USDAUD" ) == 0 ){
        $AUD_TO_DOL = sprintf( "%.6f", 1 / $cwords_[1] );
      }

      if(index ( $currency_, "USDINR" ) == 0){
        $INR_TO_DOL = sprintf( "%.6f", 1 / $cwords_[1] );
      }
    }
  }
}
...
sub LoadOvernightPositions
{
  ...
  my $overnight_pos_file = "/spare/local/logs/pnl_data/hft/EODPos/overnight_pos_$yesterday_date_.txt";
...
sub SplitTodTomSpread
{
  ...
  if ( defined $tom_price_ && defined $tod_price_ ) {
	...
	$symbol_to_total_commish_map_{ "USD000UTSTOM" } += abs($todtom_proj_pos_) * $symbol_to_commish_map_{"USD000UTSTOM"};
   	$symbol_to_total_commish_map_{ "USD000000TOD" } += abs($todtom_proj_pos_) * $symbol_to_commish_map_{"USD000000TOD"};
