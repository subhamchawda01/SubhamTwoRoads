...
do
      ...
      lotsize=`cat $tmp_file1 | cut -d'|' -f32`
      price=`perl -e "print sprintf(\"%.2f\", $price/100);"`
      tick1=`grep ${symbol}[0-9] $ref_file | grep OPTIDX|grep $exp_d|grep PE|tail -1| cut -d' ' -f8| awk -v exp_=$exp_d 'BEGIN{ FS=exp_; } {print $2}'`
      tick1=${tick1::-2}
      tick2=`grep ${symbol}[0-9] $ref_file | grep OPTIDX|grep $exp_d|grep PE|tail -2|head -1| cut -d' ' -f8| awk -v exp_=$exp_d 'BEGIN{ FS=exp_; } {print $2}'`
      tick2=${tick2::-2}
...
grep $symbol[0-9] $ref_file | grep OPTIDX| grep PE | grep $exp_d >$tmp_file1
