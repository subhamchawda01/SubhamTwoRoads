...
for (my $i = 0; $i <= $#ne_trades1_; $i++) {
    ...
    if ( int ( $time_  ) >= 845 && int ( $time_ ) < 1515 )                   # yyyymmdd_
    {
#	print "SESSION 1 :: " ;
#	print $time_ , "\n" ;
...
#	exit (1);
	...
	$shortcode_to_pnl_{$shortcode_} = 0.0;
        $shortcode_to_last_trade_time_{$shortcode_}=-1;
    }
...
for (my $i = 0; $i <= $#ne_trades2_; $i++) {
    ...
    if ( int ( $time_  ) >= 000000 && int ( $time_ ) <= 845 )    # yyyymmdd_ + 1
    {
#	print "$fields_[ $date_offset_ ] $fields_[ $quantity_offset_ ] $fields_[ $buysell_offset_ ] $fields_[ $ne_future_code_offset_ ] $fields_[ $exch_code_offset_ ] $fields_[ $contract_yr_month_offset_ ] $fields_[ $trade_price_offset_ ] $fields_[ $time_offset_ ] $fields_[ $price_multiplier_offset_ ]\n";
#	print "SESSION 2 :: " ;
#	print $time_ , "\n" ;
    }
#    elsif ( int ( $time_  ) >= 170000 && int ( $time_ ) <= 235959 )    # yyyymmdd_
    elsif ( int ( $time_  ) >= 1630 )    # yyyymmdd_
    {
#	print "$fields_[ $date_offset_ ] $fields_[ $quantity_offset_ ] $fields_[ $buysell_offset_ ] $fields_[ $ne_future_code_offset_ ] $fields_[ $exch_code_offset_ ] $fields_[ $contract_yr_month_offset_ ] $fields_[ $trade_price_offset_ ] $fields_[ $time_offset_ ] $fields_[ $price_multiplier_offset_ ]\n";
#	print "SESSION 3 :: " ;
#	print $time_ , "\n" ;
...
#	exit (1);
    ...
    if ($time_ >= $shortcode_to_last_trade_time_{$shortcode_} && $time_ <= 900) 
    {
   $shortcode_to_last_trade_time_{$shortcode_}= $time_;
