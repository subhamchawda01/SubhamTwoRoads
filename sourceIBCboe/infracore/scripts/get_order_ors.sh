...
ignore_entry="/home/dvcinfra/important/ORS_REPLY_INTRADAILY/ignore_product_for_machine_ratio"
...
#for prod in `cat /spare/local/tradeinfo/NSE_Files/RefData/nse_fo_${date_}_contracts.txt | grep -i FUTSTK | awk '{print $4}' | sort | uniq  | awk '{print "ORS_NSE_"$1"_"}'`;
#do 
#    rsync -avz --bwlimit=1024 10.23.227.63:/spare/local/MDSlogs/GENERIC/${prod}${date_} /home/dvcinfra/important/ORS_REPLY_INTRADAILY/IND13_DAILY_FILES/ --delete-after ; 
#done

#rsync -avz --bwlimit=1024 10.23.227.63:/spare/local/MDSlogs/GENERIC/ORS_NSE_* /home/dvcinfra/important/ORS_REPLY_INTRADAILY/IND13_DAILY_FILES/ --delete-after

echo "PARENT SYCN IS RESPONSIBLE FOR SYCNING"
ssh dvcinfra@10.23.227.63 /home/pengine/prod/live_scripts/rsync_67_ratio.sh
...
do
    ...
    price_int=`echo $f9 | cut -d'.' -f1`
    echo "${Monthly_data_tot[$sym]} ${Monthly_ratio_tot[$sym]} $price_int"
    [[ ${Monthly_data_tot[$sym]} -gt 1500000 ]] && [[ ${Monthly_ratio_tot[$sym]} -gt 490 ]] && [[ $price_int -lt 10000 ]] && { echo "SSS1Daily Ratio $sym"; continue; } #prev 800000
    [[ ${Monthly_data_tot[$sym]} -gt 1500000 ]] && [[ ${Monthly_ratio_tot[$sym]} -gt 490 ]] && [[ $price_int -gt 10000 ]] && { echo "SPP2Daily Ratio $sym"; continue; } # 1400000
    if [[ $f7 -gt 50000 ]] && [[ $ratio_tot -eq $f7 ]]; then
    ...
    if [[ $price -gt 1000 ]] ; then
          if [[ $mon_ord_tot -gt 1500000 ]] && [[ $mon_ratio_tot -gt 490 ]];then # prev 1450000
    ...
    if [[ $mon_ord_tot -gt 1500000 ]] && [[ $mon_ratio_tot -gt 490 ]];then # prev 950000
              if [[ $mon_ratio_tot -gt 50000 ]];then
                   echo "${sym} $mon_ord_ind16 $mon_trade_ind16 $mon_ratio_ind16 $mon_ord_ind17 $mon_trade_ind17 $mon_ratio_ind17 $mon_ord_ind18 $mon_trade_ind18 $mon_ratio_ind18 $mon_ord_tot $mon_trade_tot -1 $f9 ${Monthly_ratio_tot[$sym]}" >>/tmp/monthly_limit_position_mail_tmp
              else
                   echo "${sym} $mon_ord_ind16 $mon_trade_ind16 $mon_ratio_ind16 $mon_ord_ind17 $mon_trade_ind17 $mon_ratio_ind17 $mon_ord_ind18 $mon_trade_ind18 $mon_ratio_ind18 $mon_ord_tot $mon_trade_tot ${mon_ratio_tot} $f9  ${Monthly_ratio_tot[$sym]}" >>/tmp/monthly_limit_position_mail_tmp
...
then
 ...
 do
  ssh dvctrader@${server_to_ip_map[$server]} "cat /home/dvctrader/ATHENA/run.sh | grep -v \"^#\"|grep -v START_RATIO | grep -v CM_VWAP | grep LIVE | cut -d' ' -f2" >/tmp/live_file_path.txt
  ...
  do
    ...
    do
        ...
        if grep -q "${prod} $server" $ignore_entry
        then
              echo "Ignoring: Entry Exist in $ignore_entry file for ${prod} $server"
              continue
        else
              echo "Adding Values $prod"
        fi
        echo "Symbol Change:- $prod" 
...
#  for i in 123805 123806 123807 123808 123810 ; do ssh dvctrader@10.23.227.82 "/home/pengine/prod/live_execs/user_msg --traderid $i --setmaxpos 1" ; done
