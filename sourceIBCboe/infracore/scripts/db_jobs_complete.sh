...
mm=${today:4:2}
dd=${today:6:2}
mailfile=/tmp/data_db_report.html;
...
echo "running"
ssh dvctrader@54.90.155.232 "/home/dvctrader/EOD_SCRIPTS/update_db_daily_eod.sh ${today}";
sleep 30;

EOD_Instance=`ssh dvctrader@54.90.155.232 'ps aux | grep /home/dvctrader/EOD_SCRIPTS/update_db_daily_eod.sh | grep -v grep | wc -l'`
...
mysql -D NSE_MTBT -u root -h 54.90.155.232 -p'pu2@n6db' -e "select count(*) from MKT_DAILY_MIN_DATA where day = \"$db_date\"" | tail -1 >$data_temp
echo "MIN CASH AND FUT COUNT:-     `cat $data_temp`"
echo "MIN CASH AND FUT COUNT:-     `cat $data_temp`" >> $mailfile
...
echo 
cat $mailfile | /bin/mail -s "DB WORKER JOBS FOR ${today} COMPLETED" -r $HOSTNAME raghunandan.sharma@tworoads-trading.co.in hardik.dhakate@tworoads-trading.co.in  subham.chawda@tworoads-trading.co.in 
