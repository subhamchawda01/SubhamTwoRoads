...
LatencyReportsDir='/var/www/html/LatencyReportsExchange/'
LatencyReportsAnalysisDir='/var/www/html/LatencyReportsAnalysisExchange/'
...
do
	...
	| sed 's/<\/td>//g' | awk '{if (NF>7) {print $0}}' | sort -nrk 1 | grep -A120 ${Date} | awk '{if ($6 < 1000) {print $0}}' > ${LatencyReportsAnalysisDir}/${server}.datapoints.txt.tmp;
	...
	| tr '~' ' '  | awk '{print $1" "$3" "$4" "$5" "$6" " $7" "$8" "$9" "$10}' > ${LatencyReportsAnalysisDir}/${server}.datapoints.txt; 
