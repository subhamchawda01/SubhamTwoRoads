...
Local42ThrottleFilesDir="/home/dvcinfra/important/ThrottleProject/product_throttle_generated/"
Local42Generate_Throttle_Exec="/home/dvcinfra/important/ThrottleProject/mds_log_reader_throttle_pnl"
...
ThrottleFilesDir="/home/dvcinfra/PNLProject/product_throttle_generated/";
#ThrottleFilesDir="/home/dvcinfra/PNLProject/test/msgcount/product_throttle_generated/";
...
sync_throttle_verify() {
 ...
 [ ! -d ${ThrottleFilesDir}${yyyymmdd} ] && ssh 10.23.5.42 "/home/dvcinfra/important/ThrottleProject/mds_log_reader_overall_product_throttle GEN_THROTTLE '/NAS1/BACKUP/ORSBCAST_MULTISHM/' ${yyyymmdd} >/home/dvcinfra/important/ThrottleProject/output_overall_product_throttle" && rsync -ravz 10.23.5.42:${Local42ThrottleFilesDir}${yyyymmdd} $ThrottleFilesDir 
 echo "sync DONE"
}
		    ...
		    mkdir -p "${PNLDataPath}${server}"
		     ...
		     echo -e "StratNO Pnl, Numopen::$stratNo $totalPNL, $totalNumOpen\n"
                ...
                if [ $server == "IND14" ] || [ $server == "IND15" ] || [ $server == "IND19" ] || [ $server == "IND20" ] ; then
                   ...
                   products=`zcat $file | grep SIMRESULT | awk '{print $3,$5,$19;}' | tac | awk '!seen[$1]++' | sed "s#NSE_##g" | sed "s#_[^ ]* # #g" | awk '{pnl[$1]+=$2;msg[$1]+=$3;} END {for (key in pnl) {print "NSE_"key,pnl[key],msg[key];}}' | sort -n -k2`
