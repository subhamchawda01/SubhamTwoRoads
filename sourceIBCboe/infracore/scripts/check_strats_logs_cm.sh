...
do
    ...
    ssh ${server_to_ip_map[$server]} "/home/pengine/prod/live_scripts/check_strats_cash_servers_logs.sh"
    if [ $? -ne 0 ];
    then
...
cat $strat_mail_file | mailx -s "STRAT LOGS VERIFICATION CASH" -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in
