...
send_alert_and_exit() {
  msg=$1;
  echo "Message $msg"
  echo "" | mailx -s "Date $YYYYMMDD ${msg}" -r "${HOSTNAME}-${USER}<sanjeev.kumar@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in, hardik.dhakate@tworoads-trading.co.in, sanjay.ns@tworoads-trading.co.in, subham.chawda@tworoads-trading.co.in 
  exit;
}
...
[ $files_count -lt 1300 ] && send_alert_and_exit "CASH BAR DATA GENERATION  FILE COUNT IS LESS THAN 1300";
...
cd '/home/dvcinfra/'
pwd
#rsync -avz /NAS1/data/NSEBarData/CASH_BarData dvctrader@54.90.155.232:/NAS4/
#rsync -avz /NAS1/data/NSEBarData/CASH_BarData dvctrader@54.90.155.232:/NAS4/
/home/pengine/prod/live_scripts/sync_cash_bardata.sh

echo -e "BARDATACOUNT : $bardata_count\nNO_OF_PRODUCT : $no_of_prod_" | mailx -s " CASH DATA GEN successfully $YYYYMMDD" -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in, subham.chawda@tworoads-trading.co.in, infra_alerts@tworoads-trading.co.in
