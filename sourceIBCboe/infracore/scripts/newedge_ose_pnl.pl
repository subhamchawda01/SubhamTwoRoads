...
my $ne_file1_ = "/apps/data/MFGlobalTrades/MFGFiles/GMIST4_".$expected_date_.".csv" ;
...
my $ne_file2_ = "/apps/data/MFGlobalTrades/MFGFiles/GMIST4_".$next_date_.".csv" ;

my $prev_date_ = CalcPrevBusinessDay ( $expected_date_ ) ; # 

my %ne_exch_future_code_to_shortcode_ = ();
my %ne_exch_future_code_to_n2d_ = ();
...
my $closing_price_offset_ = 27 ; 
my $time_offset_ = 49; 
my $price_multiplier_offset_ = 91; 
...
for (my $i = 0; $i <= $#ne_trades1_; $i++) {
    ...
    if ( int ( $time_  ) > 130000 && int ( $time_ ) < 170000 )                   # yyyymmdd_ 
    {
#	print "SESSION 1 :: " ;
#	print $time_ , "\n" ;
    }
...
for (my $i = 0; $i <= $#ne_trades2_; $i++) {
    ...
    if ( int ( $time_  ) > 000000 && int ( $time_ ) < 100000 )    # yyyymmdd_ + 1 
    {
#	print "SESSION 2 :: " ;
#	print $time_ , "\n" ;
    }
...
if( $mail_body_ )
{
    ...
    print MAIL "To: kp\@circulumvite.com\n";
