...
for (my $location_ = 0; $location_ <= $#other_location_list_; $location_++) {
    ...
    my $exec_cmd_ = "taskset -c 20,21,22,23 $location_to_exec_prefix_{$this_location_} $receiver_exec_name_ $interface_ $ip_ $port_ > $local_file_location_ &";
...
for (my $location_ = 0; $location_ <= $#other_location_list_; $location_++) {
    ...
    my $exec_cmd_ = "taskset -c 20,21,22,23 $location_to_exec_prefix_{$this_location_} $sender_exec_name_ $ip_ $port_ 2 $sleep_interval_usec_ &";
