...
#  REPORT_EXEC=/home/pengine/prod/live_execs/ors_binary_reader_stats_multishm ;
  ...
  [ -f $REPORT_EXEC_OLD ] || print_msg_and_exit "REPORT_EXEC_OLD -> $REPORT_EXEC_OLD DOESN'T EXIST" ;

  fcount=`ls /NAS1/data/ORSData/ORSBCAST_MULTISHM/$yyyy/$mm/$dd | wc -l`;

  [ $fcount -gt 0 ] || print_msg_and_exit "NO DATAFILES PRESENT FOR REPORTING";

  echo "<th>DATE</th>" >> $REPORTING_FILE ;

  servers="IND14 IND15 IND16 IND17 IND18 IND19 IND20 IND22 IND23 IND24" ;
  ...
  grep -v "$yyyymmdd" $REPORT_SERVER_HTML_ALL_DATA | sort -t$'>' -nrk 3 > /tmp/latency_report.txt ;
...
#     if [ "$server" == "IND13" ] ; then 
#       $REPORT_EXEC_OLD $server $BASE_SACI $FREQ $yyyymmdd `ls /NAS1/data/ORSData/NSE/$yyyy/$mm/$dd/* | sed 's/\&/\\&/g'` ;
#     else 
      $REPORT_EXEC $server $BASE_SACI $FREQ $yyyymmdd `find /NAS1/data/ORSData/ORSBCAST_MULTISHM/$yyyy/$mm/$dd | sed 's/\&/\\&/g'` ;
#     fi

     REPORT_SERVER_WISE_HTML_ALL_DATA="/var/www/html/LatencyReports/"$server"_historical_records.txt" ;
     grep -v "$yyyymmdd" $REPORT_SERVER_WISE_HTML_ALL_DATA | sort -t$'>' -nrk 3 > /tmp/latency_report.txt
...
#MD
     serverlineprint="<tr><td>$yyyymmdd[MD]</td>";

     for data in `cat $server"_"$yyyymmdd"_cs_latency.txt"` ; do
       serverlineprint=`echo $serverlineprint"<td>"$data"</td>"` ;
     done 
...
#SHM1
     serverlineprint="<tr><td>$yyyymmdd[SHM1]</td>";

     for data in `cat $server"_"$yyyymmdd"_shm1_latency.txt"` ; do
       serverlineprint=`echo $serverlineprint"<td>"$data"</td>"` ;
     done 
...
#STRAT
     serverlineprint="<tr><td>$yyyymmdd[STRAT]</td>";

     for data in `cat $server"_"$yyyymmdd"_strat_latency.txt"` ; do
       serverlineprint=`echo $serverlineprint"<td>"$data"</td>"` ;
     done 
...
#SHM2
     serverlineprint="<tr><td>$yyyymmdd[SHM2]</td>";

     for data in `cat $server"_"$yyyymmdd"_shm2_latency.txt"` ; do
       serverlineprint=`echo $serverlineprint"<td>"$data"</td>"` ;
     done 
...
#ORS
     serverlineprint="<tr><td>$yyyymmdd[ORS]</td>";

     for data in `cat $server"_"$yyyymmdd"_ors_latency.txt"` ; do
       serverlineprint=`echo $serverlineprint"<td>"$data"</td>"` ;
     done 
