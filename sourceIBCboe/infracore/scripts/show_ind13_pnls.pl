...
require "$PSCRIPTS_DIR/get_number_of_working_days_BMF_between_two_dates.pl" ;
...
my $USAGE="$0 mode trade_type(H/M) YYYYMMDD pnl_type(TT/TI) Recovery/Normal(R/N)[dont_load_over_night]";  #TT : Tag Totals / TI: Tagwise Individual
...
#This function takes an array of internal execution tradefile lines and a flag to identify whether to add trades or remove remove them as arguments and updates the pnl maps.
	...
	for ( my $i = 0 ; $i <= $#ors_trades_file_lines_; $i ++ )
	{
		...
		{ # buy
			...
			$this_trade_pnl = -1 * $tsize_ * $tprice_ ;
		    }
		}
		else
		{
...
my $add_remove_flag = shift; #0 - Add the trade to the maps, #1 - Remove trades from the maps
	...
	for ( my $i = 0 ; $i <= $#ors_trades_file_lines_; $i ++ )
	{
		...
		if(( index ( $symbol_ , "NSE" ) == 0 )){
		    $this_trade_pnl -= ($tsize_ * $tprice_ * $symbol_to_commish_map_{$symbol_});
		    #Handling for considering trades of specific NSE Tags only.
			$symbol_to_total_commish_map_{$symbol_} +=  ($tsize_ * $tprice_ * $symbol_to_commish_map_{$symbol_});
...
sub LoadConfigFile {
    my $config="/home/pengine/prod/live_configs/ind13_pnl_config";
...
#Special handling for retail trades
    ...
    my $server=shift;
    my $tag_saci_qid_info_file;
	if($IsRecovery eq "R"){
		$tag_saci_qid_info_file="/spare/local/logs/pnl_data/hft/tag_pnl/saci_maps_hist/$server"."_qid_saci_tag_"."$input_date_";
	}
	else{
		$tag_saci_qid_info_file="/spare/local/logs/pnl_data/$trade_type_/tag_pnl/$server"."_qid_saci_tag";
	}
    if ($server eq "all"){
	if($IsRecovery eq "R"){
		$tag_saci_qid_info_file="/spare/local/logs/pnl_data/hft/tag_pnl/saci_maps_hist/qid_saci_tag_"."$input_date_" ;
	}
	else{
		$tag_saci_qid_info_file="/spare/local/logs/risk_logs/qid_saci_tag" ;
	}
...
while(1)
{
    LoadQidSaciTagsInfo ("all");
    LoadQidSaciTagsInfo ("unknown"); #Deliberately placed after all mapping so wrong console mappings can be overwritten
    RemapTags ();
    RestartHostIfAny();
...
#    PrintTagTotalPnls ('C');
 ...
 #   PrintInvalidTags();
	...
	if($PNL_SHOW_MODE eq 'TT'){
	    ...
	    my $eod_pnl_tag_file="/spare/local/logs/pnl_data/$trade_type_/tag_pnl/EODPnl/ind13_ors_pnl_$eod_date_".".txt";
	    my $eod_pnl_query_file="/spare/local/logs/pnl_data/$trade_type_/tag_pnl/EODPnl/ind13_ors_pnl_$eod_date_".".txt";
	    my $eod_pos_file="/spare/local/logs/pnl_data/$trade_type_/tag_pnl/EODPos/ind13_pos_$eod_date_".".txt";
...
#close open positions on latest trade price
	...
	my $commission_rate_ = $symbol_to_commish_map_{$exchange_symbol};
        ...
        if($exch_ eq "NSE"){
	    $commission_rate_ = $symbol_to_price_map_{$exchange_symbol} * $symbol_to_commish_map_{$exchange_symbol};
        ...
        $tag_shc_to_unrealpnl_map{$tag_shc} = $tag_shc_to_pos_map{$tag_shc} * $symbol_eod_price_ - abs ($tag_shc_to_pos_map{$tag_shc}) * $commission_rate_;

        $tag_to_pnl_map_{$tag} += $tag_shc_to_realpnl_map{$tag_shc} + $tag_shc_to_unrealpnl_map{$tag_shc};

	if($PNL_SHOW_MODE eq 'TI'){ #Handling for not displaying NSE products
	    ...
	    if( 0 == $line_number_ % 2 || $page_size_ < $MAX_PAGE_SIZE_ || $mode_ eq 'R') {
	    	printf "| POS: %6d | VOL: %6d \n", $tag_shc_to_pos_map{$tag_shc},$tag_shc_to_vol_map{$tag_shc}/$symbol_to_lots_map_{$exchange_symbol};
	    }
	    else{
		printf "| POS: %6d | VOL: %6d \t", $tag_shc_to_pos_map{$tag_shc},  $tag_shc_to_vol_map{$tag_shc}/$symbol_to_lots_map_{$exchange_symbol};
	    }
	}
...
sub PrintProductPnls{
	...
	if ( ( index ( $symbol_ , "NSE" ) == 0 ) ) {
	    $commission_rate_ = $symbol_to_price_map_{$symbol_} * $symbol_to_commish_map_{$symbol_};
	}
	$unreal_pnl_ = $symbol_to_realpnl_map_{$symbol_} + $symbol_to_pos_map_{$symbol_} * $symbol_eod_price_  - abs ($symbol_to_pos_map_{$symbol_}) * $commission_rate_;
	$line_number_ += 1 ;
	if($mode_  eq  'C'){
	    print color("BOLD");
...
#print "$symbol_1: $symbol_to_pos_map_{$symbol_1} \n";
			...
			$commission_rate_ = $symbol_to_price_map_{$symbol_1} * $symbol_to_commish_map_{$symbol_1};
		    ...
		    $exchange_to_unrealpnl_map_{$exchange} += $symbol_to_pos_map_{$symbol_1} * $symbol_eod_price_ - abs ($symbol_to_pos_map_{$symbol_1}) * $commission_rate_;
...
sub SetSecDef
{
	...
	my $commish_; my $exchange_name_; my $lots_;
...
{
    my $yesterday_date_ = CalcPrevBusinessDay ( $input_date_ );
	my $overnight_pos_file="/spare/local/logs/pnl_data/$trade_type_/tag_pnl/EODPos/ind13_pos_$yesterday_date_".".txt";
...
#Load Product positions for computing Exchange numbers
		    ...
		    $symbol_to_realpnl_map_{$symbol_}= -$symbol_to_pos_map_{$symbol_}* $symbol_eod_price_;
#Load Previous days positions to GBLHFT tag
		    ...
		    $tag_shc_to_vol_map{$tag_shc}=0;
		    $tag_shc_to_realpnl_map{$tag_shc}= -$tag_shc_to_pos_map{$tag_shc} * $symbol_eod_price_;
