...
for prod in `grep -an "ADDTRADINGSYMBOL" $ORS_FILE | awk -v line_app_no="$last_append_line" -F':' '{if($1>line_app_no) print $2 }' | awk '{print $4}' | sort | uniq | grep BSE `; do
...
if [ -s $SEC_MARGIN_ADDTS_ERROR_FILE ]; then
  ...
  if [ -s $SEC_MARGIN_ADDTS_ERROR_FILE ]; then
    ...
    /home/pengine/prod/live_scripts/bse_orsRejectHandling.sh margin $SEC_MARGIN_ADDTS_ERROR_FILE
...
if [ -s $RESETTING_ADDTS_FILE ]; then
  ...
  if [ -s $RESETTING_ADDTS_FILE ]; then
    ...
    /home/pengine/prod/live_scripts/bse_orsRejectHandling.sh resetting /tmp/verify_resetting_error
