...
if [ $# -ne 3 ] ; then
  echo "USAGE:<SCRIPT> <PRODUCT>[PFC] <VALUE> <OPERATION>[a/s/d/m]"
  exit
fi
...
exec_="/home/dvctrader/important/handling_Updates/cpp_new"
>${NEW_SYM_FILE}
...
echo "$exec_ /tmp/temp_ds_symbol.txt $VALUE $OP"
$exec_ /tmp/temp_ds_symbol.txt $VALUE $OP >/tmp/test_exc;
cat /tmp/test_exc | awk -F_ '{printf "%s\t%s_%s_%s_%s_%.2f\n",$1,$2,$3,$4,$5,$6;}' | awk -v strt=$last_num -v date_=$DATE -F "_" '{if($4>=date_){printf "NSE%d\tNSE_%s_%s_%s_%.2f\n",strt,$2,$3,$4,$5;strt++}}' >${NEW_SYM_FILE}
