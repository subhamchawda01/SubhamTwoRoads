...
#  fi
  ...
  do
     ...
     cd $REPORTING_DIR;
#     if [ $MODE == "INTRADAY" ]; then
       echo "$MODE : $REPORT_EXEC $server $BASE_SACI $FREQ $yyyymmdd find /NAS1/data/ORSData/ORSBCAST_MULTISHM/$yyyy/$mm/$dd"
       $REPORT_EXEC $server $BASE_SACI $FREQ $yyyymmdd live `find /NAS1/data/ORSData/ORSBCAST_MULTISHM/$yyyy/$mm/$dd | sed 's/\&/\\&/g'` ;   
#     elif [ $MODE == "NORMAL" ]; then
...
#PRODUCT WISE DETAILS     
     ...
     for files in `ls $server*_NSE*exchange_overall_latency.txt`; do
