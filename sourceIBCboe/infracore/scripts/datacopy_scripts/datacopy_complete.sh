...
ssh dvctrader@52.3.22.99 '/home/dvctrader/EOD_SCRIPTS/eod_jobs.sh';
sleep 10;

EOD_Instance=`ssh dvctrader@52.3.22.99 'ps aux | grep /home/dvctrader/EOD_SCRIPTS/eod_jobs.sh | grep -v grep | wc -l'`
while [ $EOD_Instance -gt 0 ]; 
do
  sleep 2;
  EOD_Instance=`ssh dvctrader@52.3.22.99 'ps aux | grep /home/dvctrader/EOD_SCRIPTS/eod_jobs.sh | grep -v grep | wc -l'`
done
ssh dvctrader@10.23.5.13 '/home/dvctrader/important/ratio_upload.sh' > ~/trash/sync.txt 
start_ratio_count_worker=`ssh dvctrader@52.3.22.99 "grep $today /spare/local/NseHftFiles/Ratio/StartRatio/NSE_*_FUT0  | wc -l"`;
end_ratio_count_worker=`ssh dvctrader@52.3.22.99 "grep $today /spare/local/NseHftFiles/Ratio/EndRatio/NSE_*_FUT0  | wc -l"`;
files_count=`ssh 52.3.22.99 "ls /NAS1/data/NSELoggedData/NSE/$yyyy/$mm/$dd/ | wc -l"` ;
...
(
  echo "To: raghunandan.sharma@tworoads-trading.co.in, hardik.dhakate@tworoads-trading.co.in, ravi.parikh@tworoads.co.in, uttkarsh.sarraf@tworoads.co.in, nishit.bhandari@tworoads-trading.co.in, sanjeev.kumar@tworoads-trading.co.in"
