...
do
	...
	if [ ! -f "$CONVERTED_DATA_DIR$basefilename" ] && [ ! -f "$CONVERTED_DATA_DIR$basefilename"".gz" ] ; then
		taskset -c $core_alloc $DATA_CONVERTER_EXEC $filename $YYYYMMDD $CONVERTED_DATA_DIR$basefilename >> $LOG_FILE 2>>$LOG_FILE &	#start this thread in background
		((count=count-1))
                ((core_alloc=core_alloc+1))

                if [ $core_alloc -eq 20 ] ; then
                  core_alloc=0;
                fi
