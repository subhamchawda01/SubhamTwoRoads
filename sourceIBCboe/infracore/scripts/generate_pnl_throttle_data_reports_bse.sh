...
servers="INDB12" ;
#servers="INDB12" ;
SourceDir="/home/dvcinfra/PNLProject/copy/"
ThrottleFilesDir="/home/dvcinfra/important/ThrottleProject/product_throttle_generated/"
Path="/home/dvcinfra/PNLProject/";
#Path="/home/dvcinfra/PNLProject/test/msgcount/";
PNLDataPath="/home/dvcinfra/PNLProject/PNLCount1/";
#PNLDataPath="/home/dvcinfra/PNLProject/test/msgcount/PNLCount1/";
REPORTS_DEST_DIR="/var/www/html/PNLReportsINDB/";
...
file_list="/home/dvcinfra/PNLProject/file_list.txt"
#file_list="/home/dvcinfra/PNLProject/test/msgcount/file_list.txt"

declare -A ServerMap=([INDB12]=400)
...
declare -a year_a=(2021 2022)

throttle_verify() {
  echo "verifying throttle $ThrottleFilesDir"
  [ ! -d ${ThrottleFilesDir}${yyyymmdd} ] && /home/pengine/prod/live_execs/mds_log_reader_overall_product_throttle "GEN_THROTTLE" "/NAS1/data/ORSData/ORSBCAST_MULTISHM/BSE/${yyyy}/${mm}/${dd}/" ${yyyymmdd} BSE >/home/dvcinfra/important/ThrottleProject/output_overall_product_throttle_bse
  echo "throttle verified"
}
...
Create_Daily_Top_Products_File () {
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_header.txt" | sed 's/PNL REPORTS/'$server' TOP GAINERS AND LOSERS For Strategy_ID '$curr_stratNo' [ '$curr_date' ] /g' > $curr_html_file ;
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_footer.txt" >> $curr_html_file;
...
Get_Daily_Top_Gainers_Losers () {
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_header.txt" | sed 's/PNL REPORTS/'$server' TOP GAINERS AND LOSERS Month [ '$mm' ] year ['$yyyy' ] Day [ '$dd' ] /g' > $Daily_Top_Gainer_Loser ;
        while read line 
        do
                curr_product=`echo $line | cut -d' ' -f 1`
                curr_pnl=`echo $line | cut -d' ' -f 2`
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_footer.txt" >> $Daily_Top_Gainer_Loser; 
...
Get_Month_Top_Gainers_Losers () {
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_header.txt" | sed 's/PNL REPORTS/'$server' TOP GAINERS AND LOSERS Month [ '$mm' ] year ['$yyyy' ] /g' > $Monthly_Top_Gainer_Loser ;
        while read line 
        do
                curr_product=`echo $line | cut -d' ' -f 1`
                curr_pnl=`echo $line | cut -d' ' -f 2`
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_footer.txt" >> $Monthly_Top_Gainer_Loser; 
...
Get_Year_Top_Gainers_Losers (){
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_header.txt" | sed 's/PNL REPORTS/'$server' TOP GAINERS AND LOSERS year ['$yyyy' ] /g' > $Year_Top_Gainer_Loser ;
        while read line 
        do
                curr_product=`echo $line | cut -d' ' -f 1`
                curr_pnl=`echo $line | cut -d' ' -f 2`
        ...
        cat "/home/dvcinfra/PNLProject/html/pnl_report_footer.txt" >> $Year_Top_Gainer_Loser;
...
Compute_Daily_PNL() {
 ...
 throttle_verify
 [ ! -d ${ThrottleFilesDir}${yyyymmdd} ] && echo "${ThrottleFilesDir}${yyyymmdd} FOR BSE NOT PRESENT IN 10.23.5.62"| mailx -s "ALERT : Failed to Generate PNL Report for $yyyymmdd" -r "${HOSTNAME}-${USER}<subham.chawda@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in hardik.dhakate@tworoads-trading.co.in infra_alerts@tworoads-trading.co.in && exit
  ...
  do
	...
	echo -e "for $server\n"
		...
		then
		    ...
		    mkdir -p "${PNLDataPath}${server}"
		     echo -e "before PNL\n"
                     totalPNL=`zcat $file | grep PORTFOLIO | awk 'END {print $3}'`
                     totalNumOpen=`zcat $file | grep PORTFOLIO | awk 'END {print $11}'`
		     echo -e "StratNO Pnl, Numopen::$stratNo $totalPNL, $totalNumOpen\n"
                ...
                if [ $server == "IND14" ] || [ $server == "IND15" ] || [ $server == "IND19" ] || [ $server == "IND20" ] ; then
                   ...
                   products=`zcat $file | grep SIMRESULT | awk '{print $3,$5,$19;}' | tac | awk '!seen[$1]++' | sed "s#BSE_##g" | sed "s#_[^ ]* # #g" | awk '{pnl[$1]+=$2;msg[$1]+=$3;} END {for (key in pnl) {print "BSE_"key,pnl[key],msg[key];}}' | sort -n -k2`
                   echo "$products" | grep -v '^$' >> $Daily_stratFile 
                   echo "$products" | grep -v '^$' >> $Monthly_stratFile 
                   echo "$products" | grep -v '^$' >> $Yearly_stratFile 
                   #products=`zcat $file | grep PORTRESULT | awk '{print $3,$5}' | tac | awk '!seen[$1]++' | sort -n -k2`
                else
                   echo "${server}:SIMRESULT:${file}"
                   products=`zcat $file | grep SIMRESULT | awk '{print $3,$5,$19}' | tac | awk '!seen[$1]++' | sort -n -k2`
                   echo "$products" | grep -v '^$' >> $Daily_stratFile
                   echo "$products" | grep -v '^$' >> $Monthly_stratFile
                   echo "$products" | grep -v '^$' >> $Yearly_stratFile 
                  #products=`zcat $file | grep SIMRESULT | awk '{print $3,$5}' | tac | awk '!seen[$1]++' | sort -n -k2`
                fi
                Get_Curr_Top_Gainers_Losers $* 
          done
          Compute_Daily_Throttle $*
...
Compute_Historical_Daily_PNL() {
 ...
 rm -rf $file_list $REPORTS_DEST_DIR"INDB"* $PNLDataPath $SERVER_REPORTING_FILE
 for i in "${year_a[@]}"
 do
   for j in "${month_a[@]}"
   do
