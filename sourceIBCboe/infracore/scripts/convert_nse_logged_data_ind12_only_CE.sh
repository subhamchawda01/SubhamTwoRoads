...
#some times mv fails if file are above 50k
  mv `ls | grep -v ORS_NSE | grep NSE_ | grep $date | grep B` ${data_dir}
  mv `ls | grep -v ORS_NSE | grep NSE_ | grep $date` ${data_dir}
  cd ${data_dir};
  count=`ls | wc -l`
...
#  [ -f ${running_status} ] && print_msg_and_exit "CANNOT START ANOTHER INSTANCE FILE  EXISTS : ${running_status}" ;
  ...
  status_sync=$?
  if [[ $status_sync -ne 0 ]]; then
	echo "Problem with IND13 sync please check manuall not running conversion  "
        echo ""| mailx -s "Problem with IND13 sync please check manuall not running conversion" -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in hardik.dhakate@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in
	exit 1;
  fi
...
#  [ $date == `date +"%Y%m%d"` ] && stop_nseshmwriter ;NO SHM IN IND12
...
#  mv * $data_dir & 
#  sleep 30; 
#  echo "Running convertion"
#  start_data_convertion CASH;
  
#  count_mv=`ls /spare/local/MDSlogs/GENERIC_NIFTY/ | wc -l`
#  echo "Current GENERIC_NIFTY Dir COUNT $count_mv"
#  while [[ $count_mv -gt 0 ]];do
#      sleep 40;
#      count_mv=`ls /spare/local/MDSlogs/GENERIC_NIFTY/ | wc -l`
#        echo "Count OF GENERIC NIFTY STILL NON ZERO..$count_mv WAITING."
#  done
...
#  start_data_convertion_space_opt PE;

  start_data_convertion_space_opt CE;   
  ...
  rsync -avz /spare/local/MDSlogs/${yyyy}/${mm}/${dd}/ 10.23.227.63:/spare/local/MDSlogs/${yyyy}/${mm}/${dd}
   ...
   if [[ $ind13_files -ne $ind13_conver ]] ;then
	echo ""| mailx -s "Problem with IND12 CE CONVERT or sync  COUNT13: $ind13_files convert13: $ind13_conver" -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in hardik.dhakate@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in 
  else
 	echo ""| mailx -s "IND13 CE convert COUNT13: $ind13_files convert12: $ind13_conver" -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in hardik.dhakate@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in
  fi
