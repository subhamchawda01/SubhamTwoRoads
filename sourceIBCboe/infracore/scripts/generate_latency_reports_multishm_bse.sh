...
#  REPORT_EXEC=/home/pengine/prod/live_execs/ors_binary_reader_stats_multishm ;
  ...
  do
     ...
     cd $REPORTING_DIR;
     echo "$REPORT_EXEC $server $BASE_SACI $FREQ $yyyymmdd `find /NAS1/data/ORSData/ORSBCAST_MULTISHM/BSE/$yyyy/$mm/$dd | sed 's/\&/\\&/g'`" ;
     $REPORT_EXEC $server $BASE_SACI $FREQ $yyyymmdd `find /NAS1/data/ORSData/ORSBCAST_MULTISHM/BSE/$yyyy/$mm/$dd | sed 's/\&/\\&/g'` ;
...
#PRODUCT WISE DETAILS     
     ...
     for files in `ls $server*_BSE*overall_latency.txt`; do
