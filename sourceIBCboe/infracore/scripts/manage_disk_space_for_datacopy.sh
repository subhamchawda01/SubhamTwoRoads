...
>$mail_alert_file
server_2="10.23.227.66"
master_server='10.23.5.66'
...
fail_alert_and_exit(){
    cat $mail_alert_file | mailx -s "Problem with $HOSTNAME: Not deleteing Data" -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in infra_alerts@tworoads-trading.co.in
        exit
}
...
echo 
echo "IND21 CHECKS"
ssh dvcinfra@$server_2 "/home/pengine/prod/live_scripts/manage_disk_space_for_datacopy_ind21.sh"
echo "IND21 DONE"
echo 
...
master_dir="/NAS1/data/NSELoggedData/NSE/${YYYY}/${MM}/${DD}"
mkdir -p $dir_tocheck
cd $dir_tocheck
chgrp infra *
echo "Syncing Data To Master $master_server"
echo "rsync -avz --no-perms -O ${dir_tocheck}/  $master_server:$master_dir"

rsync -avz --no-perms -O ${dir_tocheck}/  $master_server:$master_dir
...
rm NSE_N*
rm NSE_*
echo "" | mailx -s "Data Removed From MDSlogs for date $YYYYMMDD" -r "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>" raghunandan.sharma@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in infra_alerts@tworoads-trading.co.in
