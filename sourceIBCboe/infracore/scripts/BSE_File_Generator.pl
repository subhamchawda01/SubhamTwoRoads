...
my $USAGE = "$0 <bhavcopy.csv> <mktlots.csv> <strike_scheme.csv> <output_dated_contract_file> <mode , A-append> <appended_data_exchsymbol_file> <current_date> [expiry_date]";
...
my $default_lotsize_file_ = "/spare/local/tradeinfo/BSE_Files/Lotsizes/fo_lotsize_default.txt";
...
sub checkAndAdd
{
	...
	my $added_entry_string_ = "BSE".$highest_exchsymbol_offset_;
...
# /spare/local/tradeinfo/BSE_Files/StrikeSchemeFiles/sos_scheme_0616.csv : month "06" will be at 62nd index
...
my $prev_strike_scheme_file = "/spare/local/tradeinfo/BSE_Files/StrikeSchemeFiles/sos_scheme_".$prev_month_.$prev_year_.".csv";
...
my $datasource_exchsymbol_backup_file_name_ = "/spare/local/tradeinfo/BSE_Files/DataExchFile/datasource_exchsymbol.txt.".$unique_tag_;
...
#my @bfline_tokens_ = split(/,/, $bfline_ );
    #$bfline_tokens_[4] = $bfline_tokens_[4] * 1 ; 
#  print "Happy $bfline_tokens_[1] $bfline_tokens_[2] 123\n";
    if( ( $bfline_tokens_[1] eq "IF" || $bfline_tokens_[1] eq "IO" ) && $bfline_tokens_[4] > 0 )
    {
	$optname_string_ = sprintf( "BSE_%s_%s_%d_%.2f", $bfline_tokens_[2], $bfline_tokens_[5], formatDate( $bfline_tokens_[3] ), $bfline_tokens_[4] );	
	checkAndAdd( $optname_string_, $DEFILE );
    }

    #handles CD segment
    #handle FUTIRC and FUTCUR entries
    if( $bfline_tokens_[1] eq "FUTIRC" || $bfline_tokens_[1] eq "FUTCUR" ) # No handling required as currency not tradeded
	{
    ...
    elsif($bfline_tokens_[1] eq "IO" && ($bfline_tokens_[2] eq "BKX" || $bfline_tokens_[2] eq "BSX" ) )
    {
#   print "$bfline_ \n";
	$date_ = formatDate( $bfline_tokens_[3] );
            if($date_ <  $next_working_date_){
              print "IGNORING IO [ $bfline_tokens_[2] ] expiry date[ $date_ ] < nxt Working date[ $next_working_date_ ]\n";
              next;
            }
        ## Support for Weekly Options
        $key_ = "IO_".$bfline_tokens_[2]."_".$date_;
        $string_to_close_{ $key_ } = $bfline_tokens_[ 10 ];
        $string_to_type_{ $key_ } = "IO";
        $string_to_underlying_{ $key_ } = $bfline_tokens_[2];
        $string_to_month_{ $key_ } = int($date_/100);
        $string_to_date_{ $key_ } = $date_;
        $string_weekly_option_{ $key_ } = 1;
        #for NIFTY and BANKNIFTY we don't get #itm and step values in strike scheme file
        #we take fewer strikes than listed - data is from BSE website
        
        if($bfline_tokens_[2] eq "BSX") {
		      $string_to_itm_{ $key_ } = 20; #hft we want 6 each side with 500 as multiple
#$string_to_itm_{ $key_ } = 100; #hft we want 6 each side with 500 as multiple
          $string_to_step_{ $key_ } = 100;
        }
	elsif($bfline_tokens_[2] eq "BKX") {
                $string_to_itm_{ $key_ } = 20; #hft we want 6 each side with 500 as multiple
          	$string_to_step_{ $key_ } = 100; # prev 100
        }
        else {
          $string_to_itm_{ $key_ } = 20; #hft we want 6 each side with 500 as multiple
          $string_to_step_{ $key_ } = 100;
    ...
    elsif( ( $bfline_tokens_[1] eq "IF" && $bfline_tokens_[2] ne "BSX" && $bfline_tokens_[2] ne "BKX" )  || 
	( $bfline_tokens_[1] ne "IF" && $bfline_tokens_[1] ne "SF" ) )
    {     
        #due to different file format BhavCopy entries of CDS file will get here. // << not valid anymore
        print "ignoring these rows \n";
        next;
    }
    elsif( $bfline_tokens_[1] eq "IF" && ( $bfline_tokens_[2] eq "BKX" || $bfline_tokens_[2] eq "BSX" ) )
    {
		$date_ = formatDate( $bfline_tokens_[3] );
		if($date_ <  $next_working_date_){
              		print "IGNORING IF [ $bfline_tokens_[2] ] expiry date[ $date_ ] < nxt Working date[ $next_working_date_ ]\n";
              		next;
            	}

		$key_ = $bfline_tokens_[1]."_".$bfline_tokens_[2]."_".$date_;	
		$string_to_close_{ $key_ } = $bfline_tokens_[ 10 ];
		$string_to_type_{ $key_ } = "IF";
		$string_to_underlying_{ $key_ } = $bfline_tokens_[2];
		$string_to_month_{ $key_ } = int($date_/100);
		$string_to_date_{ $key_ } = $date_;

        my $weekly_option_key = $bfline_tokens_[1]."_".$bfline_tokens_[2]."_".$string_to_month_{ $key_ };
        print "KEY: $weekly_option_key VAL: weekly_option_string_to_close_{ $weekly_option_key } \n";
...
{
    if( $string_to_type_{ $string_ } eq "SF" || $string_to_type_{ $string_ } eq "IF" )
    {
...
printf FILE "\n# Options Contracts\n";
my %is_option_added_ = (); # [pjain] : This map is reuiqred to prevent same entries to go into BSE Contracts File multiple times.
foreach my $string_( keys %string_to_underlying_ )
{
    ...
    print "KeyCheck1 $string_ TYPE: $string_to_type_{ $string_ } Under: $string_to_underlying_{ $string_ }\n";
#print "Option Contract $string_to_type_{ $string_ }\n";
#   print "Underlyging $string_to_underlying_{ $string_ } \n";
    if( $string_to_type_{ $string_ } eq "IF" )
    {
        my $option_string_suffix_ = substr $string_, 2;
        my $option_string_ = "IO".$option_string_suffix_;
#        print "Entry From2 $option_string_\n";
        if(! exists $is_option_added_{ $option_string_ }){
            printf FILE "IO\t%s\t%.2f\t%d\t%.2f\t%d\t%d\t%.2f\t%.2f\t%d\n", $string_to_underlying_{ $string_ }, $string_to_close_{ $string_ }, $string_to_lotsize_{ $string_ }, 0.05, $string_to_date_{ $string_ }, $string_to_itm_{ $string_ }, $string_to_step_{ $string_ },$string_to_step_prev_{ $string_ },$string_to_itm_prev_{ $string_ };
            $is_option_added_{ $option_string_ } = 1;
        }
        
    }
    elsif( $string_to_type_{ $string_ } eq "SF" )
    {
	   printf FILE "SO\t%s\t%.2f\t%d\t%.2f\t%d\t%d\t%.2f\t%.2f\t%d\n", $string_to_underlying_{ $string_ }, $string_to_close_{ $string_ }, $string_to_lotsize_{ $string_ }, 0.05, $string_to_date_{ $string_ }, $string_to_itm_{ $string_ }, $string_to_step_{ $string_ }, $string_to_step_prev_{ $string_ },$string_to_itm_prev_{ $string_ };
    }
    ...
    elsif($string_to_type_{ $string_ } eq "IO" && ($string_to_underlying_{ $string_ } eq "BSX" || $string_to_underlying_{ $string_ } eq "BKX" ))
    {
        #Weekly Options
        my $weekly_option_key_current_mon_future = "IF_".$string_to_underlying_{ $string_ }."_".$string_to_month_{ $string_ };
#       print "Entry From1 $weekly_option_key_current_mon_future\n";
...
foreach my $string_( keys %string_to_underlying_ )
{
    if( not( exists $string_weekly_option_{ $string_ })){ #This check required for Weekly Options
        ...
        #add future contract
        $futname_string_ = "BSE_".$string_to_underlying_{ $string_ }."_FUT_".$string_to_date_{ $string_ };
        ...
        #add spread contract - note multiple calls are safe
        {
           ...
           if( $num_expiries_ >= 2 )
           {
               my $futname_spread_string_ = "BSE_".$string_to_underlying_{ $string_ }."_FUT_".$sorted_expiries_[0]."_".$sorted_expiries_[1];
    ...
    $optname_string_ = sprintf( "BSE_%s_CE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $atm_strike_1_ );
    checkAndAdd( $optname_string_, $DEFILE );
    
    #ATM PE for that expiry + underlying
    $optname_string_ = sprintf( "BSE_%s_PE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $atm_strike_1_ );
    checkAndAdd( $optname_string_, $DEFILE );
    
    #ATM CE for that expiry + underlying
    $optname_string_ = sprintf( "BSE_%s_CE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $atm_strike_2_ );
    checkAndAdd( $optname_string_, $DEFILE );
    
    #ATM PE for that expiry + underlying
    $optname_string_ = sprintf( "BSE_%s_PE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $atm_strike_2_ );
    checkAndAdd( $optname_string_, $DEFILE );
...
#    	print "ATM : $string_ $atm_strike_ ATM1: $atm_strike_1_ ATM2: $atm_strike_2_ $string_to_step_{ $string_ } : $string_to_step_prev_{ $string_ } OPT : $option_strike_ OPT1: $option_strike_1_ OPT2: $option_strike_2_\n";
	   ...
	   $optname_string_ = sprintf( "BSE_%s_CE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $option_strike_1_ );
	   checkAndAdd( $optname_string_, $DEFILE );
	   
	   $optname_string_ = sprintf( "BSE_%s_PE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $option_strike_1_ );
	   checkAndAdd( $optname_string_, $DEFILE );
        }
    }
    # For Prev Schema
...
#    	print "ATM : $string_ $atm_strike_ ATM1: $atm_strike_1_ ATM2: $atm_strike_2_ $string_to_step_{ $string_ } : $string_to_step_prev_{ $string_ } OPT : $option_strike_ OPT1: $option_strike_1_ OPT2: $option_strike_2_\n";
	   ...
	   $optname_string_ = sprintf( "BSE_%s_CE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $option_strike_2_ );
	   checkAndAdd( $optname_string_, $DEFILE );
	   
	   $optname_string_ = sprintf( "BSE_%s_PE_%d_%.2f", $string_to_underlying_{ $string_ }, $string_to_date_{ $string_ }, $option_strike_2_ );
	   checkAndAdd( $optname_string_, $DEFILE );
	}
    }
