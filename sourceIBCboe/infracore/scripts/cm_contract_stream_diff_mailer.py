...
HTML_REPORT_TITLE1 = "CM : STREAM INFO DIFFERENCE"
HTML_REPORT_TITLE2 = "CM : STREAM INFO REMOVED"
HTML_REPORT_TITLE3 = "CM : STREAM INFO ADDED"
HTML_REPORT_TEMPLATE = 'position_report_template.html'
HTML_OUT_FILE = '/home/dvctrader/trash/stream_diff_report.html'
#HTML_OUT_FILE = '/home/subham/subham/stream_diff_report.html'
...
def SendDiffReport(df_, df_old_, df_new_):
    if (not df_.empty) or (not df_new_.empty) or (not df_new_.empty):
        subject = '**********ALERT : CM NSE stream info difference**********'
...
def downloadFileFromFtp(ftp_host, ftp_username, ftp_passwd, ftp_filename):
    try:
        ...
        ftp.cwd('Common')
        ftp.cwd('tbt_masters')
        ftp.retrbinary('RETR cm_contract_stream_info.csv',
                       open(ftp_filename, 'wb').write)
    except:
        subject = 'ALERT : CM FAILED DOWNLOADING CONTRACT STREAM INFO 5.26'
        command = \
...
def main():
    ...
    ftp_passwd = 'FTPGUEST'
    stream_info_dir = '/spare/local/tradeinfo/NSE_Files/StreamInfo/Cash/'
#    stream_info_dir = '/home/subham/subham/stream_check_script/'
    ftp_filename = stream_info_dir + 'cm_contract_stream_info.csv'
    old_ftp_filename = stream_info_dir \
        + 'cm_contract_stream_info.csv_old'
    ...
    if ftp_file_new[0] != ftp_file_old[0]:
      ...
      subject = 'INFO : CM STREAM FILE UPDATED'
