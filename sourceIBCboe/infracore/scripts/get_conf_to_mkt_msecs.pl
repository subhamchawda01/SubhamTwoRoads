...
my @conf_lines_ = `$ORS_BIN_READER_EXEC $shortcode_ $yyyymmdd_ | grep " Conf "` ;

for ( my $i = 0 ; $i <= $#conf_lines_ ; $i ++ )
{
    my @conf_words_ = split ( ' ' , $conf_lines_ [ $i ] );

    my $t_int_px_ = $conf_words_ [ 6 ];
    my $t_buysell_ = $conf_words_ [ 8 ];
    my $t_time_ = $conf_words_ [ 11 ];
    my $t_saos_ = $conf_words_ [ 17 ];
    my $t_size_ = $conf_words_ [ 25 ]; chomp ( $t_size_ );

    if ( ! exists $saos_to_conf_time_ { $t_saos_ } )
    {
...
# For each confed saos, look after the conf. time in the mkt data
# to find an update which "most likely" contains our order.

foreach my $saos_ ( sort { $saos_to_conf_time_ { $a } <=> $saos_to_conf_time_ { $b } } 
		    keys %saos_to_conf_time_ )
{
    my $t_conf_time_ = $saos_to_conf_time_ { $saos_ };
    ...
    # starting 100 usecs before conf. time and upto 6 msecs after conf. time.
    my $t_search_time_start_ = $t_conf_time_ + 0.000100; # Might be too little for EUREX, but too much for others.
    my $t_search_time_end_ = $t_conf_time_ + 0.006000; # Might be too much for EUREX, but too little for others.
    ...
    my @t_conf_to_mkt_update_times_ = ( );
	...
	{ 
	    # Order count went up by 1, and the size increased by our order size.
	    # This could be our order.
	    push ( @t_conf_to_mkt_update_times_ , ( $t_time_ - $t_conf_time_ ) );
	}
    ...
    if ( $#t_conf_to_mkt_update_times_ >= 0 )
    {
	my $t_mean_ = GetAverage ( \@t_conf_to_mkt_update_times_ );
	my $t_median_ = GetMedianConst ( \@t_conf_to_mkt_update_times_ );
	...
	{
	    print "$t_conf_time_ $t_mean_ $t_median_ $saos_ $#t_conf_to_mkt_update_times_\n";
	}
    }
}
