...
AUDIT_LOG_GENERATE_EXEC=$HOME/LiveExec/bin/generate_newedge_audit_log 
EXCH_SYMBOL_EXEC=$HOME/LiveExec/bin/get_exchange_symbol 
...
for files in `ls | grep -v audittrail | grep -v zip`
do
...
totalfiles=`ls $this_dest_dir/*AuditTrail*$YYYYMMDD* | wc -l`

echo $totalfiles ;

rm -rf $TEMP_FILE_FOR_TRADES ;
...
cd incoming
put $this_dest_dir/*AuditTrail*$YYYYMMDD*
bye
