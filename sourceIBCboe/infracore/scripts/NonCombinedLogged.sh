...
#cleaup old data 
find /spare/local/MDSlogs/NonCombined/ -type f -mtime +3 -exec rm {} \;
	...
	then
		...
		LOGGER )      
		    ...
		    then 
	                 ...
	                 onload $MDS_EXEC --mode $MODE --exchange $EXCH --log_dir /spare/local/MDSlogs/NonCombined/$EXCH/ 2>$LOGFILE >> $COUTCERRFILE &
			 ...
			 then
			     ...
			     LD_PRELOAD=libvma.so $MDS_EXEC --mode $MODE --exchange $EXCH --log_dir /spare/local/MDSlogs/NonCombined/$EXCH/ 2>$LOGFILE >> $COUTCERRFILE &

			 else

                             $MDS_EXEC --mode $MODE --exchange $EXCH --log_dir /spare/local/MDSlogs/NonCombined/$EXCH/ 2>$LOGFILE >> $COUTCERRFILE &
