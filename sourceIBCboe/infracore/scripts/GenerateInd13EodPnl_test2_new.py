...
BULK_CONTRACT_SPECS = "/home/pengine/prod/live_execs/get_nse_commission"
...
def DumpPnls():
    ...
    for key in symbol_info_map:
        ...
        total_txn += symbol_info_map[key].trans_comm_add
...
def main():
    global date, exch_dir, total_txn #Update these using arguments to the string
    ...
    diff_date = curr_date - dt_date
    use_local_file = False
    if diff_date.days > 20:
      use_local_file = True
      print('USE LOCAL: ' + str(use_local_file))
    exch_dir = args.dir
    ...
    local_trade_dir = "/run/media/root/Elements/SERVERDATA/"
    for server in args.servers:
        ...
        local_tradefile_path = local_trade_dir + server[:5] + "/ORSlogs/"+config.get(server,'Exch')+"/"+config.get(server,                                                                                           "Profile")+"/trades."+str(
            date)
        ...
        if use_local_file:
          try:
            command='cp %s %s' % (local_tradefile_path, local_file_path)
            print('command: ' + command)
            BashExec(command)
            trade_files_.append(local_file_path)
            print(" Tradefile found on local server. Filename: "+ local_tradefile_path +"\n")
          except:
            print("ERROR: Tradefile not found. Filename: " + local_tradefile_path + "\n")
        else:
          while True:
              sys.stdout.flush()
              try:
                  ...
                  print(" Tradefile found. Filename: "+ remote_file_path +"\n")
                  break
              except (IOError):
                  print("ERROR: Tradefile not found. Filename: "+ remote_file_path +"\n")
                  break
              except:
                  print("ERROR: Server Connection Dropped. ServerIP: "+ server_ip +" Trying again" +"\n")

    '''
    add trade file to adjust pos
    '''
    print("PASS: GET TRADE FILE" +"\n")
    ...
    TRANS_ADD_SYM_FILE="/NAS1/data/MFGlobalTrades/ind_pnls/CM/trans_add_products/data_nifty_next." + str(date)
    if exch_dir == "CM": 
...
#Add symbols for neat trades
    try:
        ...
        next(neat_pos_file) #Skip first line as it contains headers
    ...
    backup_eod_to_nas()
    cleanup()
    print("TOTAL TRANSACTION ADDED: " + str(total_txn))
