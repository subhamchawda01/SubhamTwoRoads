...
ErrorMail(){
       (echo To: "raghunandan.sharma@tworoads-trading.co.in infra_alerts@tworoads-trading.co.in subham.chawda@tworoads-trading.co.in" ; echo From: "${HOSTNAME}-${USER}<subham.chawda@tworoads-trading.co.in>"; echo Subject: "FAILED Daily ORS Report for FO Products Generation $today_"; echo "Content-Type: text/html;";echo " ") | /usr/sbin/sendmail -t
...
mds_input_file="/home/dvcinfra/trash/ors_reply_mds_fo_input_file_${date_}"
mds_output_file="/home/dvcinfra/important/Generate_ors_detail/REPLY_NSE_OUTPUT_FO/ors_reply_mds_fo_output_file_${date_}"
datasource="/spare/local/tradeinfo/NSE_Files/datasource_exchsymbol.txt"
tmp_file1="/home/dvcinfra/trash/exch_2_ds_file_${date_}"
tmp_file2="/home/dvcinfra/trash/exch_sym_nse_mds_data_${date_}"
error_file_check="/home/dvcinfra/trash/error_ors_report_fo"
...
>$tmp_file1
for prod in `grep -v "Unknown Machine" $mds_output_file | awk '{print $1}'`; do
...
#  awk 'NR==FNR {sym[$2]=$1; next} {print sym[$1],$2,$3,$4,$5}' $tmp_file1 $ind13_mds_output_file >$tmp_file2
  ...
  local66_mds_input_file="/home/dvcinfra/trash/nse_file_list_${date_}"
  local66_mds_output_file="/home/dvcinfra/important/Generate_ors_detail/REPLY_NSE_OUTPUT_FO/mds_nse_output_file_${date_}"
  ...
  awk 'NR==FNR {sym[$2]=$1; next} {if ($1 in sym) {print sym[$1],$2,$3,$4,$5,$6}}' $tmp_file1 $local66_mds_output_file >$tmp_file2
#fi

awk 'NR==FNR {sym[$1]=$2" "$3" "$4" "$5" "$6; next} {if ($1 in sym) {print $0" "sym[$1]}}' $tmp_file2 $mds_output_file >$OutFileFO
