...
#      (echo To: "raghunandan.sharma@tworoads-trading.co.in" ; echo From: "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>"; echo Subject: "Daily ORS Report for Cash Products $date_"; echo "Content-Type: text/html;";cat ${mail_file}) | /usr/sbin/sendmail -t
      (echo To: "raghunandan.sharma@tworoads-trading.co.in nseall@tworoads.co.in" ; echo From: "${HOSTNAME}-${USER}<raghunandan.sharma@tworoads-trading.co.in>"; echo Subject: "Daily ORS Report for Cash Products $prev_day"; echo "Content-Type: text/html;";cat ${mail_file}) | /usr/sbin/sendmail -t
}
...
for sym in "${!month_product_cash[@]}"; do
    ...
    if [[ ${product_price[$sym]} -gt 1000 ]] ; then
        if [[ ${month_product_cash[$sym]} -gt 1500000 ]]; then # prev 1400000
      ...
      if [[ ${month_product_cash[$sym]} -gt 1500000 ]]; then #prev 800000
          if [[ $ratio -eq -1 ]] || [[ $ratio -gt 490 ]]; then
              echo "$sym ${month_product_cash[$sym]} ${month_exec_cash[$sym]} $ratio ${product_price[$sym]}" >> $tmp_file1
              echo "$sym ${month_product_cash[$sym]} ${month_exec_cash[$sym]} $ratio" >> $tmp_file3
          fi
...
do
  ssh dvctrader@${server_to_ip_map[$server]} "cat /home/dvctrader/ATHENA/run.sh | grep -v \"^#\"|grep -v START_RATIO | grep -v CM_VWAP |grep -v CONFIG_CM_NON_FO|grep LIVE | cut -d' ' -f2" >/tmp/live_file_path.txt
  ...
  do 
      ...
      do
          ...
          if grep -q "NSE_${prod} $server" $ignore_entry
          then
...
for i in 123707 123708 123709 123710 123711 ; do ssh dvctrader@10.23.227.81 "/home/pengine/prod/live_execs/user_msg --traderid $i --setmaxpos 1" ;done
...
for i in 123515 123516 123517 123518 123519; do ssh dvctrader@10.23.227.83 "/home/pengine/prod/live_execs/user_msg --traderid $i --setmaxpos 1" ; done
