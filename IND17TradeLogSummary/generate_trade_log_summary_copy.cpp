...
struct trade_log_data {
	int profit_count=0, loss_count=0, profit_count_wrt_limit=0, loss_count_wrt_limit=0, total_pnl=0, total_product_count=0;
};
...
trade_log_data InsertProductSummaryToFile(int &pnl_limit, std::string trade_filename) {
	...
	int loss_count=0;
	int profit_count=0;
	int loss_count_wrt_limit=0;
	int profit_count_wrt_limit=0;
	int total_pnl=0;
	...
	for(auto product : product_pnl) {
		++total_product_count;
		total_pnl += product.second;
		(product.second > 0) ? ++profit_count : ++loss_count;
		((pnl_limit + product.second) > 0) ? ++profit_count_wrt_limit : ++loss_count_wrt_limit;
	}
	//inserting data to trade summary file
	fout << trade_filename.substr(52) << "," << profit_count << "," << loss_count << "," << profit_count_wrt_limit << "," << loss_count_wrt_limit << "," << total_pnl << "," << total_product_count << "\n";
	fout.close();
	trade_log_data obj;
	obj.profit_count = profit_count;
	obj.loss_count = loss_count;
	obj.profit_count_wrt_limit = profit_count_wrt_limit;
	obj.loss_count_wrt_limit = loss_count_wrt_limit;
	obj.total_pnl = total_pnl;
	obj.total_product_count = total_product_count;
	//std::cout << trade_filename << "," << profit_count << "," << loss_count << "," << profit_count_wrt_limit << "," << loss_count_wrt_limit << "," << total_pnl << "," << total_product_count << "\n";
	return obj;
}
...
void InsertProductToMap(int pnl_limit) {
	...
	fout << "overall_summary" <<  "," << final_log_summary.profit_count << "," << final_log_summary.loss_count <<"," 
			<< final_log_summary.profit_count_wrt_limit << "," << final_log_summary.loss_count_wrt_limit << "," 
