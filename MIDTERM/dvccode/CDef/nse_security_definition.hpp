...
class NSESecurityDefinitions {
 ...
 public:
  ...
  static bool IsWeeklyOptionExpiry(const int yyyymmdd);
  ...
  static double GetNSECommission(std::string _shortcode_);
  ...
  static std::string GetShortcodeFromCanonical(const std::string& ticker_, int expiry_dt_, double strike_,
                                               bool is_call_);
  static void UpdateExchangeSymbol_map_shortcode();
