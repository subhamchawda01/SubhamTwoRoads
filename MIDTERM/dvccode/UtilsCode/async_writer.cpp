...
AsyncWriter::AsyncWriter(int len) : qlen(len), ptr(nullptr), continuelogging(0), allowaddition(1), worker(nullptr) {
  shm = ShmChannel::AllocateInMemory(len, &ptr);
  assert(ptr);
}
...
void AsyncWriter::start() {
  if (!continuelogging) {
    allowaddition = 0;
    continuelogging = 1;
    worker = new boost::thread(std::bind(&AsyncWriter::log_main, this));
  }
}
void AsyncWriter::stop() { continuelogging = false; }

void AsyncWriter::log_main() {
  Misc::AffineMe2Init();
