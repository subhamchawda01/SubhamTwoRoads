...
class MDSShmInterface {
 ...
 public:
  ...
  void InitializeOldSHM() {
    ...
    std::cout << "COMBINED_SHMID: " << shmid << std::endl;
    std::cout << "SHMKEY: " << key << std::endl;
  ...
  void WriteGenericStruct(HFSAT::MDS_MSG::GenericMDSMessage *_generic_mds_struct_, bool log_to_generic_enabled = true,
                          bool is_ors = false) {
    HFSAT::RuntimeProfiler::GetUniqueInstance(HFSAT::ProfilerType::kCOMBINEDSHMWRITER).End();
    const ProfilerTimeInfo &time_info =
        HFSAT::RuntimeProfiler::GetUniqueInstance(HFSAT::ProfilerType::kCOMBINEDSHMWRITER).GetTimeInfo();
