...
void NSESecurityDefinitions::PopulateTuesdayExpiryMap() {
  // 3 contracts outstanding in NSE  FINNIFTY AND MIDCPNIFTY
  for (uint32_t t_ctr_ = 0; t_ctr_ < 3; t_ctr_++) {
...
int NSESecurityDefinitions::ComputePreviousExpiry(const int t_date_, const std::string& _shortcode_) {
  if ( _shortcode_.find("NSE_FINNIFTY") != std::string::npos || _shortcode_.find("NSE_MIDCPNIFTY") != std::string::npos){
...
void NSESecurityDefinitions::PopulateTuesdayWeeklyOptionsExpiryMap() {
  ...
  for (uint32_t expiry_num = 0; expiry_num < 15; expiry_num++) {
    ...
    weekly_option_expirydate_2_contractnumber_tuesday_[weekly_option_contractnumber_2_expirydate_tuesday_[expiry_num]] = expiry_num;
//	std::cout <<"TUESDAY" << weekly_option_contractnumber_2_expirydate_tuesday_[expiry_num] <<" : " << expiry_num <<std::endl;
  }
}
...
int NSESecurityDefinitions::ComputeTuesdayNextExpiryWeeklyOptions(const int t_date_,
  ...
  if (t_expiry_date_ >= t_date_ && monthly_future_expiries.find(t_expiry_date_) == monthly_future_expiries.end()) {
    ...
    ymd = next_friday.year_month_day();
    return ComputeTuesdayNextExpiryWeeklyOptions(((ymd.year * 100 + ymd.month) * 100) + ymd.day, monthly_future_expiries);
  }
}

bool NSESecurityDefinitions::IsWeeklyOptionExpiry(const int yyyymmdd, char* underlying) {
  if (strcmp(underlying,"MIDCPNIFTY") == 0 || strcmp(underlying,"FINNIFTY") == 0)
...
bool NSESecurityDefinitions::LoadThisExpiryContracts(const int t_expiry_, const NSEInstrumentType_t t_inst_type_, char* underlying) {
  ...
  } else if (strcmp(underlying,"MIDCPNIFTY") == 0 || strcmp(underlying,"FINNIFTY") == 0) {
...
int NSESecurityDefinitions::GetContractNumberFromExpiry(const int t_date_, const NSEInstrumentType_t t_inst_type_, char* underlying) {
  if (t_inst_type_ == HFSAT::NSE_CURFUT || t_inst_type_ == HFSAT::NSE_CUROPT) {
...
int NSESecurityDefinitions::GetExpiryFromContractNumber(const uint32_t t_number_,
                                                        const NSEInstrumentType_t t_inst_type_, char* underlying) {
  if (t_inst_type_ == HFSAT::NSE_CURFUT || t_inst_type_ == HFSAT::NSE_CUROPT) {
...
int NSESecurityDefinitions::GetWeeklyContractNumberFromExpiry(const int t_date_, char* underlying) {
  if (strcmp(underlying,"MIDCPNIFTY") == 0 || strcmp(underlying,"FINNIFTY") == 0) {
       if (weekly_option_expirydate_2_contractnumber_tuesday_.find(t_date_) != weekly_option_expirydate_2_contractnumber_tuesday_.end()) {
...
double NSESecurityDefinitions::GetNSECommission(std::string _shortcode_) {
  ...
  if (t_commish_map_iter_ != shortcode_2_commissions_.end()) {
    ...
    if (t_type_map_iter_ != shortcode_2_type_.end()) {
      switch (t_type_map_iter_->second) {
        case HFSAT::NSE_IDXFUT:
        case HFSAT::NSE_STKFUT:
          t_commish_ = (0.0000025 * 1.14 + 0.000002 + 0.00005 + 0.000019 * 1.18 + 0.00001) *
                       GetContractMultiplier(t_type_map_iter_->second);  // Clearing + SEBI + STT + TXN + FUND + STAMP
          shortcode_2_commissions_[_shortcode_] = t_commish_;
          return t_commish_;

        case HFSAT::NSE_IDXOPT:
        case HFSAT::NSE_STKOPT:
          t_commish_ = (0.00025 * 1.14 + 0.000002 + 0.00025 + 0.00051 * 1.18 + 0.000015) *
                       GetContractMultiplier(t_type_map_iter_->second);  // txn charge can become less than this
          shortcode_2_commissions_[_shortcode_] = t_commish_;
          return t_commish_;

        case HFSAT::NSE_CURFUT:
          t_commish_ = (0.0000025 * 1.14 + 0.000002 + 0.000011 * 1.14 + 0.0000005 ) *  // txn charge can become less than this
        ...
        case HFSAT::NSE_EQ:
          t_commish_ =
              (0 + 0.000002 + 0.000125 + 0.0000325 * 1.14 + 0.000015) * GetContractMultiplier(t_type_map_iter_->second);
          shortcode_2_commissions_[_shortcode_] = t_commish_;
          return t_commish_;
        default:
...
void NSESecurityDefinitions::AddNSEContractSpecifications(ShortcodeContractSpecificationMap& t_contract_spec_map_) {
  ...
  for (t_veciter_ = inst_params_.begin(); t_veciter_ != inst_params_.end(); t_veciter_++) {
    ...
    // Only NIFTY and BANKNIFTY currently in IDXFUT
    if (t_param_->inst_type_ == HFSAT::NSE_STKFUT || t_param_->inst_type_ == HFSAT::NSE_IRFFUT ||
        ...
        (t_param_->inst_type_ == HFSAT::NSE_IDXFUT &&
         (strcmp(t_param_->underlying_, "NIFTY") == 0 || strcmp(t_param_->underlying_, "BANKNIFTY") == 0 
          || strcmp(t_param_->underlying_, "MIDCPNIFTY") == 0 || strcmp(t_param_->underlying_, "FINNIFTY") == 0))) {
      if (LoadThisExpiryContracts(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_)) {
        ...
        if (GetContractNumberFromExpiry(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_) == 1) {
          ...
          shortcode_2_expiry_date_[t_futname_oss_.str()] = GetExpiryFromContractNumber(0, t_param_->inst_type_, t_param_->underlying_);
        ...
        if (t_param_->inst_type_ == HFSAT::NSE_IRFFUT &&
          ...
          std::ostringstream t_futname_oss_;
          std::ostringstream t_futname_ess_;
          t_futname_oss_ << "NSE_GIND10YR_FUT" << GetContractNumberFromExpiry(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_);
        ...
        if (t_param_->inst_type_ == HFSAT::NSE_STKFUT &&
          ...
          t_cashname_oss_ << "NSE_" << t_param_->underlying_;
          ...
          shortcode_2_type_[t_cashname_oss_.str()] = HFSAT::NSE_EQ;
      ...
      if (LoadThisExpiryContracts(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_)) {
        ...
        if (strcmp(t_param_->underlying_, "NIFTY") == 0) {
          ...
          temp_step_value *= 1; // currently 100 (same as FINNIFTY)
        } else if (strcmp(t_param_->underlying_, "MIDCPNIFTY") == 0) {
          temp_step_value *= 1; // currently 100 (same as MIDCPNIFTY)
        }
        ...
        // atm put/call
        {
          ...
          if (IsWeeklyOptionExpiry(t_param_->expiry_, t_param_->underlying_)) {
            t_temp_optname_oss_ << t_optname_base_oss_.str() << "_P"
                                << GetWeeklyContractNumberFromExpiry(t_param_->expiry_,t_param_->underlying_) << "_A_W";
          } else {
            t_temp_optname_oss_ << t_optname_base_oss_.str() << "_P"
                                << GetContractNumberFromExpiry(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_) << "_A";
          }
          ...
          t_temp_optname_ess_.str(std::string());
          t_temp_optname_ess_ << t_optname_base_oss_.str() << "_CE_" << t_param_->expiry_ << "_" << std::setprecision(2)
                              << t_atm_strike_;
          shortcode_2_exchsymbol_[t_temp_optname_oss_.str()] =
              ConvertDataSourceNametoExchSymbolSecDef(t_temp_optname_ess_.str());
        ...
        // otm/itm options
        {
          // For OTM Call and ITM Put
          {
            ...
            while (t_cs_ctr_ <= t_param_->num_itm_) {
              ...
              t_cs_ctr_++;
              // The below block ensures that we are considering both strike steps in calculation the OTM_1, OTM_2 etc.
              if (fabs(option_strike_1_inc - option_strike_2_inc) < 0.0001) {  // $option_strike_1_ == $option_strike_2
          ...
          // For ITM Call and OTM Put
          {
            ...
            while (t_cs_ctr_ <= t_param_->num_itm_) {
              ...
              if (0 == (t_ctr_ % ratio)) {
                if (IsWeeklyOptionExpiry(t_param_->expiry_, t_param_->underlying_)) {
                  t_temp_optname_oss_ << t_optname_base_oss_.str() << "_P"
                                      << GetWeeklyContractNumberFromExpiry(t_param_->expiry_, t_param_->underlying_) << "_I"
                                      << t_ctr_ / ratio << "_W";
                } else {
                  t_temp_optname_oss_ << t_optname_base_oss_.str() << "_P"
                                      << GetContractNumberFromExpiry(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_) << "_I"
                                      << t_ctr_ / ratio;
                }
                ...
                if (IsWeeklyOptionExpiry(t_param_->expiry_, t_param_->underlying_)) {
                  t_temp_optname_oss_ << t_optname_base_oss_.str() << "_P"
                                      << GetWeeklyContractNumberFromExpiry(t_param_->expiry_, t_param_->underlying_) << "_I"
                                      << (t_ctr_ / ratio) << "_M" << (t_ctr_ % ratio) << "_W";
                } else {
                  t_temp_optname_oss_ << t_optname_base_oss_.str() << "_P"
                                      << GetContractNumberFromExpiry(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_) << "_I"
                                      << (t_ctr_ / ratio) << "_M" << (t_ctr_ % ratio);
                }
              }
              ...
              if (fabs(option_strike_1_dec - option_strike_2_dec) < 0.0001) {  // $option_strike_1_ == $option_strike_2
          ...
          for (otm_call_itm_put_itr_ = otm_call_itm_put_strike_price_set_.begin();
            ...
            if (0 == (t_ctr_ % ratio)) {
              if (IsWeeklyOptionExpiry(t_param_->expiry_, t_param_->underlying_)) {
                t_temp_optname_oss_ << t_optname_base_oss_.str() << "_C"
                                    << GetWeeklyContractNumberFromExpiry(t_param_->expiry_, t_param_->underlying_) << "_O"
                                    << t_ctr_ / ratio << "_W";
              } else {
                t_temp_optname_oss_ << t_optname_base_oss_.str() << "_C"
                                    << GetContractNumberFromExpiry(t_param_->expiry_, t_param_->inst_type_, t_param_->underlying_) << "_O"
                                    << t_ctr_ / ratio;
              }
  ...
  for (auto datasource_sym_ : non_zero_OI_) {
    ...
    if (exchsymbol_2_shortcode_.find(exch_sym_) == exchsymbol_2_shortcode_.end()) {
      ...
      if (ticker_ == "NIFTY" || ticker_ == "BANKNIFTY" || ticker_ == "FINNIFTY" || ticker_ == "MIDCPNIFTY") {
        t_contract_spec_map_[shc_.str()] =
            ContractSpecification(0.05, GetContractMultiplier(HFSAT::NSEInstrumentType_t::NSE_IDXOPT) *
        ...
        t_contract_spec_map_[shc_.str()] =
            ContractSpecification(0.05, GetContractMultiplier(HFSAT::NSEInstrumentType_t::NSE_STKOPT) *
                                            CurrencyConvertor::Convert(kCurrencyINR, kCurrencyUSD),
                                  kExchSourceNSE, lotsize_);
...
void NSESecurityDefinitions::AddEquityContractSpecifications(ShortcodeContractSpecificationMap& t_contract_spec_map_) {
  ...
  while (!nse_ref_file.eof()) {
    ...
    if (t_contract_spec_map_.find(shortcode) == t_contract_spec_map_.end()) {
      ...
      shortcode_2_price_limit_[shortcode] = new NSETradingLimit_t(-1, 100000000, -1, 0.05);
    ...
    if (nse_data_exch_file_.is_open()) {
      while (nse_data_exch_file_.good()) {
        ...
        if (tokens_.size() == 2 && tokens_[0][0] != '#') {
          ...
          char datasource_name_[NSE_DOTEX_OFFLINE_SYMBOL_LENGTH];
          strncpy(datasource_name_, tokens_[1], NSE_DOTEX_OFFLINE_SYMBOL_LENGTH);
...
/*  
  for (auto i : shortcode_2_exchsymbol_) {
    if (i.second == exch_sym_) {
      std::cout<<"1shortcode_2_exchsymbol_ " << i.first <<std::endl;
      return i.first;
    }
  }
  ...
  for (auto i : shortcode_2_exchsymbol_all_) {
    if (i.second == exch_sym_) {
      std::cout<<"2shortcode_2_exchsymbol_all_ " << i.first <<std::endl;
      return i.first;
    }
  }
*/
...
void NSESecurityDefinitions::UpdateExchangeSymbol_map_shortcode(){
  for (auto i : shortcode_2_exchsymbol_) {
    exchsymbol_2_shortcode_[i.second] = i.first;
  }
  for (auto i : shortcode_2_exchsymbol_all_) {
...
std::string NSESecurityDefinitions::GetShortCodeFromExchangeSymbol(std::string& exch_symbol) {
...
std::string NSESecurityDefinitions::GetShortCodeFromStrikePrice(double strike_price_,
  ...
  if ((step_value_ == 0) || (close_price_ == 0)) return NULL;
    ...
    if (nse_eq_file_.is_open()) {
      while (nse_eq_file_.good()) {
        ...
        if (tokens_.size() > 7) {
          ...
          shc_ = "NSE_" + shc_;
          if ((trimmed_str_ == "EQ") && (IsShortcode(shc_))) {
    ...
    if (nse_eq_file_.is_open()) {
      while (nse_eq_file_.good()) {
 	  ...
 	  std::string fut_shc_ = tokens_[3];
        	if ( fut_shc_.find("FINNIFTY") != std::string::npos || fut_shc_.find("MIDCPNIFTY") != std::string::npos ){
...
void NSESecurityDefinitions::PopulateOrderPriceRangeMapOpt(const int t_date_) {
  ...
  nse_eq_file_oss_ << NSE_ORDER_LIMIT_FILENAME_PREFIX << "/" << this_date.substr(0, 4) << "/" << this_date.substr(4, 2)
                   << "/" << this_date.substr(6, 2) << "/contract";
  nse_eq_file_name_ = nse_eq_file_oss_.str();
  UpdateExchangeSymbol_map_shortcode();
      ...
      while (nse_eq_file_.good()) {
        ...
        if (tokens_.size() > 68) {
	 	...
	 	current_month_ = (NSEMonth_t)((contractnumber_2_expirydate_tuesday_[0] % 10000) / 100);
            ...
            if ( fut_shc_.find("FINNIFTY") != std::string::npos || fut_shc_.find("MIDCPNIFTY") != std::string::npos ){
			shc_ =
                GetShortcodeFromCanonical(fut_shc_, contractnumber_2_expirydate_tuesday_[contract_number], strike_, is_call_);
