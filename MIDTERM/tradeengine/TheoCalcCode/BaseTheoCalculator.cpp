...
BaseTheoCalculator::BaseTheoCalculator(std::map<std::string, std::string>* _key_val_map, HFSAT::Watch& _watch_,
  ...
  if (NULL != secondary_smv_ && HFSAT::SecurityDefinitions::CheckIfContractSpecExists(secondary_smv_->shortcode(), watch_.YYYYMMDD())){
    ...
    DBGLOG_CLASS_FUNC_LINE_INFO << "SECONDARY : " << secondary_smv_->shortcode() << " LAST_CLOSE : " << HFSAT::NSESecurityDefinitions::GetClosePriceFromShortCode(secondary_smv_->shortcode()) << DBGLOG_ENDL_FLUSH ;
    secondary_last_close_int_price_ = secondary_smv_->GetIntPx(HFSAT::NSESecurityDefinitions::GetClosePriceFromShortCode(secondary_smv_->shortcode()));
  ...
  if (NULL != primary0_smv_ && HFSAT::SecurityDefinitions::CheckIfContractSpecExists(primary0_smv_->shortcode(), watch_.YYYYMMDD())){
    ...
    DBGLOG_CLASS_FUNC_LINE_INFO << "PRIMARY : " << primary0_smv_->shortcode() << " LAST_CLOSE : " << HFSAT::NSESecurityDefinitions::GetClosePriceFromShortCode(primary0_smv_->shortcode()) << DBGLOG_ENDL_FLUSH ;
    primary_last_close_int_price_ = primary0_smv_->GetIntPx(HFSAT::NSESecurityDefinitions::GetClosePriceFromShortCode(primary0_smv_->shortcode()));
...
void BaseTheoCalculator::LoadParams() {
  ...
  is_banned_prod_ = HFSAT::NSESecurityDefinitions::IsBannedProduct(ticker_name_);
  ...
  auto_getflat_near_circuit_factor_ = Parser::GetDouble(key_val_map_, "AUTO_GETFLAT_NEAR_CIRCUIT_FACTOR", 1);
