...
void Dimer::UpdateOrders() {
  ...
  if (order_count_*inv_trade_factor_ >= bad_ratio_limit_) {
    if (status_mask_ & BAD_FILL_RATIO_STATUS_SET) {
      DBGLOG_TIME_CLASS_FUNC << watch_.tv() << " " << identifier_  << " BAD FILL RATIO HIT LIMIT: " << bad_ratio_limit_ << " NUM TRADES: "  << 1/inv_trade_factor_  << " DISABLING DIMER " << DBGLOG_ENDL_FLUSH;
    }
