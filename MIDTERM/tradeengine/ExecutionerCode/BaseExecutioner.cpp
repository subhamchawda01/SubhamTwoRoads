...
void BaseExecutioner::ReloadConfig() {
  ...
  int prev_bad_ratio_limit_  = bad_ratio_limit_;
  LoadParams();
  ...
  if ((prev_bad_ratio_limit_ != bad_ratio_limit_) && (order_count_*inv_trade_factor_ < bad_ratio_limit_)) {
    if ((status_mask_ & BAD_FILL_RATIO_STATUS_SET) == 0) {
      DBGLOG_TIME_CLASS_FUNC << "BAD FILL RATIO HIT LIMIT: " << bad_ratio_limit_
                             << " not crossed EXEC RATIO: " << order_count_*inv_trade_factor_ << " ENABLING EXEC" << DBGLOG_ENDL_FLUSH;
    }
