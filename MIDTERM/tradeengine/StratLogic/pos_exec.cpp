...
int main(int argc, char **argv) {
  ...
  for (auto shc : source_shortcode_list_) {
    ...
    if (shc_to_channel_info_.find(shc) != shc_to_channel_info_.end()) {
      ...
      if (HFSAT::NSESecurityDefinitions::IsOption(shc)) {
    ...
    } else if (HFSAT::NSESecurityDefinitions::IsSpotIndex(shc) == true) {
      ...
      included_channels_.insert(channel);
                    ...
                    HFSAT::NSESecurityDefinitions::IsOption(shc)) {
//      is_fo_present_ = true;
  ...
  std::ostringstream cfn_oss;
  cfn_oss << "/spare/local/files/NSE/nse-tbt-mcast-" << progid_ << ".txt";

  channels_file.open(cfn_oss.str().c_str(), std::ofstream::out);

  if (!channels_file.is_open()) {
    std::cerr << "UNABLE TO OPEN STREAMS FILE FOR CREATING CHANNELS INFO : " << cfn_oss.str() << std::endl;
    std::exit(-1);
  }
  ...
  HFSAT::Utils::UDPDirectMultipleZocket &udp_direct_multiple_zockets =
      HFSAT::Utils::UDPDirectMultipleZocket::GetUniqueInstance();
  ...
  HFSAT::NSEMD::NSETBTRawMDHandler::GetUniqueInstance(dbglogger_, simple_live_dispatcher, nullptr, HFSAT::kRaw, "NSE",
                                                      true, combined_mds_messages_shm_processor, cfn_oss.str());
