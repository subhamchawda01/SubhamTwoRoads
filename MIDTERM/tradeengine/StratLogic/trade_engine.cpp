...
int main(int argc, char** argv) {
  ...
  if (argc < 6) {  // 5 is min of live and sim
    std::cerr
        << "expecting :\n"
        << " $tradeengineexec STRAT_LIVE_FILE TRADINGDATE START_TIME END_TIME PROGID [ADD_DBG_CODE CODE1 CODE2 ......]"
        << '\n';
  ...
  if (argc > 6) {
	...
	is_asm_filter_enable_ = true;
      }else{
        std::cerr
            << "expecting :\n"
            << " $tradeengineexec STRAT_LIVE_FILE TRADINGDATE START_TIME END_TIME PROGID IS_ASM_DISABLE[0/1] EXCH [ADD_DBG_CODE CODE1 CODE2 ......]" << '\n';
        HFSAT::ExitVerbose(HFSAT::kTradeInitCommandLineLessArgs);
      }
      if(argc > 7){
  ...
  if(is_asm_filter_enable_){
    std::string asm_filename_ = "/spare/local/tradeinfo/NSE_Files/ASMSecurities/short_term_asm.csv_" + std::string(argv[2]);
    ...
    if(asm_file_.is_open()){
      ...
      while(asm_file_.good()){
        ...
        if(shortcode_.length() >0){
          asm_symbols_list_.push_back("NSE_"+shortcode_);
        }
      }
    }else{
