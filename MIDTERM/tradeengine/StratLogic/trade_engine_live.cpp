...
//  }
  ...
  HFSAT::Utils::ExanicMultipleRx &exanic_multiple_zockets =
      HFSAT::Utils::ExanicMultipleRx::GetUniqueInstance();
  exanic_multiple_zockets.AddEventTimeoutNotifyListener(combined_mds_messages_shm_processor);
  ...
  std::cout<<" Running LiveDispatcher "<<std::endl;
#if USE_EXANIC_MERGE
//    exanic_multiple_zockets.RunLiveDispatcher();
  std::cout<<" RUNNING AS EXANIC " << std::endl;
  exanic_multiple_zockets.RunLiveDispatcherWithNotify();
#elif USE_UDP_DIRECT_SHM1_MERGE
  std::cout<<" RUNNING AS SOLARFLARE " << std::endl;
