...
class GapTheo(StratFramework):
	...
	def tradeGap(self,_prod,_counter):
		pos_ = math.floor(self.max_exposure_/self.prod_curr_posn_[_counter][Fields.close_.value])
		...
		elif self.current_time_ - self.om_.started_at_ == self.trade_bar_*self.granularity_:# and self.prod_curr_posn_[_counter][Fields.cascade_trigger.value] == True:
			if self.prod_curr_posn_[_counter][Fields.pos.value] > 0:
