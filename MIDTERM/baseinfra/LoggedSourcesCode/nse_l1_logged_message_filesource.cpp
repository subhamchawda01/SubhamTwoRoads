...
NSEL1LoggedMessageFileSource::NSEL1LoggedMessageFileSource(
  ...
  std::string t_nse_filename_ =
      NSEL1LoggedMessageFileNamer::GetName(t_exchange_symbol_, t_preevent_YYYYMMDD_, trading_location_file_read_);
...
void NSEL1LoggedMessageFileSource::SeekToFirstEventAfter(const ttime_t r_start_time_, bool& rw_hasevents_) {
  if (bulk_file_reader_.is_open()) {
    if (next_event_timestamp_.tv_sec == 0 && next_event_timestamp_.tv_usec == 0) {
      ...
      } while (available_len_ == sizeof(GenericL1DataStruct) && strcmp(next_event_.symbol, exchange_symbol_) != 0);
