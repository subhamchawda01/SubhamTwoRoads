...
BasePNL::BasePNL(DebugLogger& _dbglogger_, Watch& _watch_, 
                 SecurityMarketView& _dep_market_view_, int t_runtime_id_)
  ...
  if (dep_market_view_.exch_source() == kExchSourceNSE) {
    snprintf(numbered_secname_, 40, "%s.%d", dep_market_view_.shortcode().c_str(), runtime_id_);
  } else {
    snprintf(numbered_secname_, 40, "%s.%d", dep_market_view_.secname(), runtime_id_);
  }
