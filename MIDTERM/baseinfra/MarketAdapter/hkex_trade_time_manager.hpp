...
#ifndef BASE_MARKETADAPTER_HKEX_TRADE_TIME_MANAGER_H
#define BASE_MARKETADAPTER_HKEX_TRADE_TIME_MANAGER_H

#define HKEX_TRADE_TIME_INFO_FILE "/spare/local/files/HKEX/hkex-trd-hours.txt"
...
class HkexTradeTimeManager {
 private:
  int* start_times;
  int* end_times;
  int* first_break_start_times;
  int* first_break_end_times;
  int* second_break_start_times;
  int* second_break_end_times;
  int size;
  int yyyymmdd;

  HkexTradeTimeManager(const HkexTradeTimeManager&);

  HkexTradeTimeManager(HFSAT::SecurityNameIndexer& sec_indexer, int YYYYMMDD) {
    size = sec_indexer.GetNumSecurityId();
    start_times = (int*)calloc(size, sizeof(int));
    end_times = (int*)calloc(size, sizeof(int));
    yyyymmdd = YYYYMMDD;

    first_break_start_times = (int*)calloc(size, sizeof(int));
    first_break_end_times = (int*)calloc(size, sizeof(int));
  ...
  void parseTimeFile(HFSAT::SecurityNameIndexer& sec_indexer) {
    std::fstream file(HKEX_TRADE_TIME_INFO_FILE, std::ofstream::in);

    if (!file || !file.is_open()) {
      fprintf(stderr, "Could not open file %s in hkex_trade_time_manager\n", HKEX_TRADE_TIME_INFO_FILE);
      exit(1);
    }
    char line[1024];
    char* ticker;
    ...
    while (file.getline(line, sizeof(line))) {
      ...
      t1 = strtok(tmp3, ":");
      t2 = strtok(NULL, ":");
      int fbs = (atoi(t1) * 60 + atoi(t2)) * 60;

      t1 = strtok(tmp4, ":");
      t2 = strtok(NULL, ":");
      int fbe = (atoi(t1) * 60 + atoi(t2)) * 60;
      ...
      // std::cerr << " KP " << st << " " << et << " " << fbs << " " << fbe << " " << sbs << " " << sbe << "\n";
      ...
      if (sec_id >= 0) {
        ...
        first_break_start_times[sec_id] = fbs + offset;
        first_break_end_times[sec_id] = fbe + offset;
 ...
 public:
  static HkexTradeTimeManager& GetUniqueInstance(HFSAT::SecurityNameIndexer& sec_indexer, int YYYYMMDD = -1) {
    static HkexTradeTimeManager uniqueinstance_ = HkexTradeTimeManager(sec_indexer, YYYYMMDD);
  ...
  inline bool isValidTimeToTrade(int sec_id, int seconds_elapased_today) {
    ...
    return (start_times[sec_id] <= seconds_elapased_today && seconds_elapased_today <= end_times[sec_id] &&
            !(first_break_start_times[sec_id] <= seconds_elapased_today &&
              seconds_elapased_today < first_break_end_times[sec_id]) &&
...
#endif /* BASE_MARKETADAPTER_HKEX_TRADE_TIME_MANAGER_H */
