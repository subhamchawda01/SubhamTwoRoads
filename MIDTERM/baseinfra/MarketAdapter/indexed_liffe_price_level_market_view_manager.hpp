...
class IndexedLiffePriceLevelMarketViewManager : public PriceLevelGlobalListener,
 ...
 public:
  IndexedLiffePriceLevelMarketViewManager(DebugLogger& t_dbglogger_, const Watch& t_watch_,
  ...
  void OnPriceLevelDeleteFrom(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelDeleteFrom should never be called for Liffe .. exiting now \n";
    exit(1);
  }
  ...
  void OnPriceLevelDeleteThrough(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelDeleteThrough should never be called for Liffe .. exiting now \n";
    exit(1);
  }
  ...
  void OnPriceLevelOverlay(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelOverlay should never be called for Liffe .. exiting now \n";
    exit(1);
  }
};
}
