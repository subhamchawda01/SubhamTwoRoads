/**
    \file MarketAdapter/indexed_micex_market_view_manager.hpp
...
class IndexedMicexMarketViewManager : public PriceLevelGlobalListener,
 ...
 public:
  IndexedMicexMarketViewManager(DebugLogger& t_dbglogger_, const Watch& t_watch_,
  ...
  void OnPriceLevelDeleteFrom(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelDeleteFrom should never be called for MICEX .. exiting now \n";
    exit(1);
  }
  ...
  void OnPriceLevelDeleteThrough(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelDeleteThrough should never be called for MICEX .. exiting now \n";
    exit(1);
  }
  ...
  void OnPriceLevelOverlay(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelOverlay should never be called for MICEX .. exiting now \n";
    exit(1);
  }
};
}
