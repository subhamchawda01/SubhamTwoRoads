/**
    \file MarketAdapter/indexed_rts_market_view_manager.hpp
...
class IndexedRtsMarketViewManager : public PriceLevelGlobalListener,
 ...
 private:
  ...
  std::vector<TradeType_t> last_trade_aggressor_side_;
  std::vector<int> last_trade_int_price_;
  std::vector<int> last_l1_bid_del_int_price_;
  std::vector<int> last_l1_ask_del_int_price_;

 public:
  IndexedRtsMarketViewManager(DebugLogger& t_dbglogger_, const Watch& t_watch_,
  ...
  TradeType_t GetAggressorSide(int security_id, int int_price, TradeType_t orig_buysell);
  ...
  void OnPriceLevelDeleteFrom(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelDeleteFrom should never be called for RTS .. exiting now \n";
    exit(1);
  }
  ...
  void OnPriceLevelDeleteThrough(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelDeleteThrough should never be called for RTS .. exiting now \n";
    exit(1);
  }
  ...
  void OnPriceLevelOverlay(const unsigned int t_security_id_, const TradeType_t t_buysell_,
    ...
    std::cerr << " PriceLevelOverlay should never be called for RTS .. exiting now \n";
    exit(1);
  }
};
}
