...
ORSMessageStats::ORSMessageStats(DebugLogger& dbglogger, Watch& watch, SecurityMarketView* smv,
...
void ORSMessageStats::GenerateMktLatencies() {
  std::string shortcode = smv_->shortcode();
...
void ORSMessageStats::GenerateBMFMktLatencies() {
  ...
  const char* secname = smv_->secname();
...
void ORSMessageStats::GenerateEUREXMktLatencies() {
  ...
  size_t len = reader.read(&next_event, sizeof(EOBI_MDS::EOBICommonStruct));

  while (len == sizeof(EOBI_MDS::EOBICommonStruct)) {
    ...
    len = reader.read(&next_event, sizeof(EOBI_MDS::EOBICommonStruct));
  }
}
      ...
      switch (next_event.msg_type) {
        ...
        case ASX_MDS::kASXAdd: {
          ...
          if (it != exch_seq_to_ors_order_map_.end() && (int)next_event.data.add.size == it->second->size_ &&
              smv_->DblPxCompare(next_event.data.add.price, it->second->price_)) {
            it->second->mkt_time_ = next_event.time_;
          }
          break;
        }
      ...
      switch (next_event.msg_type) {
        ...
        case ASX_ITCH_MDS::kASXAdd: {
          ...
          if (it != exch_seq_to_ors_order_map_.end() && (int)next_event.add.size == it->second->size_ &&
              smv_->DblPxCompare(next_event.add.price, it->second->price_)) {
...
void ORSMessageStats::ProcessCxld(GenericORSReplyStruct& next_event) {
  ...
  it->second->cxl_time_ = next_event.time_set_by_server_;
}
...
void ORSMessageStats::ProcessExec(GenericORSReplyStruct& next_event) {
  ...
  ttime_t tdiff = next_event.time_set_by_server_ - it->second->cxlseq_time_;
  if (tdiff < normal_delay_) {
    return;
  }

  // giving some time for execution, so adding 20 usecs
  it->second->cxl_time_ = next_event.time_set_by_server_ + ttime_t(0, 20);
}
