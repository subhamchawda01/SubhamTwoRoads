...
struct BaseOrder {
  ...
  inline bool CanBeCanceled(const int watch_minus_cxl_wait_msecs_, bool is_cancellable_before_confirmation) {
    return (!(modified_ || canceled_) && size_remaining_ > 0 && !is_ioc_ &&
            (order_status_ == kORRType_Conf ||
             (order_status_ == kORRType_Seqd &&
              (is_cancellable_before_confirmation || watch_minus_cxl_wait_msecs_ >= seqd_msecs_)))); /* we are checking for CXL_WAIT if not confirmed already */
  }
