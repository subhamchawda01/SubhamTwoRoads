 ...
 public:
  CommonSMVSource(std::vector<std::string> source_shortcode_vec, int tradingdate_, bool live_trading_ = false)
      : source_shortcode_vec_(source_shortcode_vec),
        ...
        synth_market_view_vec_(),
        sec_name_indexer_(HFSAT::SecurityNameIndexer::GetUniqueInstance()),
        hybrid_security_manager_(new HFSAT::HybridSecurityManager(tradingdate_)),
  ...
  void SetSourceShortcodes(std::vector<std::string> &source_shortcode_vec) {
    HFSAT::SyntheticSecurityManager &t_synthetic_security_manager_ =
        HFSAT::SyntheticSecurityManager::GetUniqueInstance();

    for (auto i = 0u; i < source_shortcode_vec.size(); i++) {
      if (t_synthetic_security_manager_.IsSyntheticSecurity(source_shortcode_vec[i])) {
        std::vector<std::string> t_constituent_vec_ =
            t_synthetic_security_manager_.GetConstituentSHC(source_shortcode_vec[i]);
        for (unsigned int const_shc_idx_ = 0; const_shc_idx_ < t_constituent_vec_.size(); const_shc_idx_++) {
          HFSAT::VectorUtils::UniqueVectorAdd(source_shortcode_vec, t_constituent_vec_[const_shc_idx_]);
        }
      }
    }
    source_shortcode_vec_ = source_shortcode_vec;
  ...
  void InitializeMarketViewMgrs() {
    for (unsigned int secid = 0; secid < sec_name_indexer_.NumSecurityId(); secid++) {
      switch (sid_to_exch_source_map_[secid]) {
        ...
        case HFSAT::kExchSourceNSE: {
          if (!indexed_nse_market_view_manager2_) {
  ...
  void InitializeVariables(bool initilize_order_feed_data_ = false) {
    ...
    for (auto i = 0u; i < source_shortcode_vec_.size(); i++) {
      if (!sec_name_indexer_.HasString(source_shortcode_vec_[i])) {  // need to add this source to sec_name_indexer_
        ...
        if (synthetic_security_manager_.IsSyntheticSecurity(source_shortcode_vec_[i])) {
        ...
        if (hybrid_security_manager_->IsHybridSecurity(source_shortcode_vec_[i])) {
          const char *act_exchange_symbol_ = HFSAT::ExchangeSymbolManager::GetExchSymbol(
              hybrid_security_manager_->GetActualSecurityFromHybrid(source_shortcode_vec_[i]));
          HFSAT::HybridSecurityManager::AddHybridToActualExch(std::string(exchange_symbol_),
                                                              std::string(act_exchange_symbol_));
        }
      }
    }
    ...
    for (auto i = 0u; i < ors_shortcode_vec_.size(); i++) {
      if (!sec_name_indexer_.HasString(ors_shortcode_vec_[i])) {  // need to add this source to
        ...
        exchange_symbol_vec_.push_back(exchange_symbol_);
        sid_to_marketdata_needed_map_.push_back(false);
        if (hybrid_security_manager_->IsHybridSecurity(ors_shortcode_vec_[i])) {
          const char *act_exchange_symbol_ = HFSAT::ExchangeSymbolManager::GetExchSymbol(
              hybrid_security_manager_->GetActualSecurityFromHybrid(ors_shortcode_vec_[i]));
          HFSAT::HybridSecurityManager::AddHybridToActualExch(std::string(exchange_symbol_),
                                                              std::string(act_exchange_symbol_));
        }
      }
    ...
    for (auto i = 0u; i < source_shortcode_vec_.size(); i++) {
      if (synthetic_security_manager_.IsSyntheticSecurity(source_shortcode_vec_[i])) {
        HFSAT::SyntheticMarketView *t_synth_market_view_ =
            new HFSAT::SyntheticMarketView(dbglogger_, watch_, source_shortcode_vec_[i]);
        synth_market_view_vec_.push_back(t_synth_market_view_);
      }
    }
          ...
          if (sid_to_marketdata_needed_map_[secid]) {
            ...
            if (shortcode_asx_pf_data_filesource_map_.find(shortcode_) == shortcode_asx_pf_data_filesource_map_.end()) {
              ...
              shortcode_asx_pf_data_filesource_map_[shortcode_]->AddPriceLevelGlobalListener(
                  indexed_asx_market_view_manager_.get());
...
#undef MINUTES_TO_PREP
  ...
  void cleanup() {
    ...
    for (auto i = 0u; i < synth_market_view_vec_.size(); i++) {
      if (synth_market_view_vec_[i] != NULL) {
        delete (synth_market_view_vec_[i]);
      }
    }
    synth_market_view_vec_.clear();
