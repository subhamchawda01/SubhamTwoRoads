...
OptionObject::OptionObject(DebugLogger& _dbglogger_, const Watch& _watch_, std::string _option_name_)
  ...
  if (OptionType_t::CALL == (OptionType_t)NSESecurityDefinitions::GetOptionType(option_name_)) {
    is_call_q_ = 1;
  } else {
    is_call_q_ = -1;
  }

  strike_ = NSESecurityDefinitions::GetStrikePriceFromShortCode(option_name_);
  interest_rate_ = NSESecurityDefinitions::GetInterestRate(watch_.YYYYMMDD());

  const int expiry_utc_date_ = NSESecurityDefinitions::GetExpiryFromShortCode(option_name_);
