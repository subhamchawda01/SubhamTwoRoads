...
class CombinedMDSMessagesNSEProcessor {
 ...
 public:
  CombinedMDSMessagesNSEProcessor(DebugLogger& _dbglogger_, SecurityNameIndexer& _sec_name_indexer_)
    ...
    // NO RECOVERY RUNNING 
    memset((void*)security_id_to_recovery_complete_, 0, DEF_MAX_SEC_ID);
  ...
  bool RecoverLiveOrders(int32_t const& token, int32_t const& current_seq_num, int32_t const& sec_id, char segment) {
