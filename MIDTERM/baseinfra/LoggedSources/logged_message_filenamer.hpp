...
class LoggedMessageFileNamer {
 ...
 private:
  ...
  inline static bool test(std::string &file, unsigned int yyyymmdd, std::string &error_string_) {
    ...
    if ((strncmp(hostname, "ip-10-0-1", 9) == 0 && false/*HFSAT::FileUtils::ExistsAndReadable(IS_HS1_PRESENT)  */) || (work_dir != nullptr)) {
 ...
 public:
  ...
  static std::string GetName(std::string baseLogDir, const char *_exchange_symbol_,
    ...
    std::vector<TradingLocation_t> all_locs;
	  ...
	  // The fileNamer will not fall back & search the primary location in case it doesn't find the file in the trading location
...
//            (std::string(all_aws_disk_vec_[disk_counter_]) + std::string(AWS_CACHE_PREFIX) + std::string(baseLogDir)),
...
//
  ...
  inline static void SetSkipPrimaryLoc(bool skip_primary_loc){
  	skip_primary_location_ = skip_primary_loc;
  }
