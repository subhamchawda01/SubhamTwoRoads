/**
   \file Tests/dvctrade/MarketAdapter/hkex_mvm_tests.hpp
...
class HKEXMvmTests : public CppUnit::TestFixture {
  /**
   * General testing framework,
   * Currently It reads inputs from a file and processes accordingly
   */
  CPPUNIT_TEST_SUITE(HKEXMvmTests);
  CPPUNIT_TEST(TestHKEXMvm);
  ...
  void TestHKEXMvm(void);

  std::string exchange_;
  std::vector<std::vector<std::string> > hkex_instruction_list_;
