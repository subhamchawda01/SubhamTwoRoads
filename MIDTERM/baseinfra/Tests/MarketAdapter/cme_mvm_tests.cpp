/**
   \file Tests/dvctrade/MarketAdapter/cme_mvm_tests.cpp
...
#include "baseinfra/Tests/MarketAdapter/cme_mvm_tests.hpp"

namespace HFTEST {

void CMEMvmTests::setUp() {
  exchange_ = "CME";
...
void CMEMvmTests::TestCMEMvm() {
  auto& sec_name_indexer = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  auto cme_book_manager = common_smv_source_->indexed_cme_market_view_manager();
  ...
  for (auto& instruction : cme_instruction_list_) {
    ...
    if (instruction[1].compare("TESTCASE") == 0) {
      ...
      if (reset_book) cme_book_manager->ResetBook(security_id);
      ...
      cme_book_manager->OnPriceLevelNew(security_id, buysell, level_added, price, size, ordercount, intermediate);
      ...
      cme_book_manager->OnPriceLevelChange(security_id, buysell, level_changed, price, size, ordercount, intermediate);
      ...
      cme_book_manager->OnPriceLevelDelete(security_id, buysell, level_changed, price, intermediate);

    } else if (instruction[2].compare("OnPriceLevelDeleteSynthetic") == 0) {
      // Level Delete Message

      TradeType_t buysell = instruction[column_number++][0] == 'B' ? kTradeTypeBuy : kTradeTypeSell;
      column_number++;  // do not need level info in synthetic delete
      double price = atof(instruction[column_number++].c_str());
      bool intermediate = atoi(instruction[column_number++].c_str()) != 0;

      cme_book_manager->OnPriceLevelDeleteSynthetic(security_id, buysell, price, intermediate);
      ...
      cme_book_manager->OnTrade(security_id, price, size, buysell);
      ...
      cme_book_manager->OnPriceLevelOverlay(security_id, buysell, level_overlayed, price, size, ordercount,
                                            intermediate);
      ...
      cme_book_manager->OnPriceLevelDeleteThrough(security_id, buysell, max_level_deleted, intermediate);
      ...
      cme_book_manager->OnPriceLevelDeleteFrom(security_id, buysell, min_level_deleted, intermediate);
