...
#include "baseinfra/Tests/MarketAdapter/rts_mvm_tests.hpp"

namespace HFTEST {

void RTSMvmTests::setUp() {
  exchange_ = "RTS";
...
void RTSMvmTests::TestRTSMvm() {
  auto& sec_name_indexer = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  auto rts_book_manager = common_smv_source_->indexed_rts_market_view_manager();
  ...
  for (auto& instruction : cme_instruction_list_) {
    ...
    if (instruction[1].compare("TESTCASE") == 0) {
      ...
      if (reset_book) rts_book_manager->ResetBook(security_id);
      ...
      rts_book_manager->OnPriceLevelNew(security_id, buysell, level_added, price, size, ordercount, intermediate);
      ...
      rts_book_manager->OnPriceLevelChange(security_id, buysell, level_changed, price, size, ordercount, intermediate);
      ...
      rts_book_manager->OnPriceLevelDelete(security_id, buysell, level_changed, price, intermediate);
      ...
      rts_book_manager->OnTrade(security_id, price, size, buysell);
      ...
      rts_book_manager->OnPriceLevelOverlay(security_id, buysell, level_overlayed, price, size, ordercount,
                                            intermediate);
      ...
      rts_book_manager->OnPriceLevelDeleteThrough(security_id, buysell, max_level_deleted, intermediate);
      ...
      rts_book_manager->OnPriceLevelDeleteFrom(security_id, buysell, min_level_deleted, intermediate);
