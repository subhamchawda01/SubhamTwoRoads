/**
   \file Tests/dvctrade/MarketAdapter/ose_of_mvm_tests.hpp
...
class OSEOFMvmTests : public CppUnit::TestFixture {
  /**
   * General testing framework,
   * Currently It reads inputs from a file and processes accordingly
   */
  CPPUNIT_TEST_SUITE(OSEOFMvmTests);
  CPPUNIT_TEST(TestOSEOFMvm);
  ...
  void TestOSEOFMvm(void);

  std::string exchange_;
  std::vector<std::vector<std::string> > ose_of_instruction_list_;
