/**
   \file Tests/dvctrade/MarketAdapter/cfe_mvm_tests.cpp
...
#include "baseinfra/Tests/MarketAdapter/cfe_mvm_tests.hpp"

namespace HFTEST {

void CFEMvmTests::setUp() {
  exchange_ = "CFE";
...
void CFEMvmTests::TestCFEMvm() {
  auto& sec_name_indexer = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  auto cfe_book_manager = common_smv_source_->indexed_cfe_market_view_manager();
  ...
  for (auto& instruction : cme_instruction_list_) {
    ...
    if (instruction[1].compare("TESTCASE") == 0) {
      ...
      if (reset_book) cfe_book_manager->ResetBook(security_id);
      ...
      cfe_book_manager->OnPriceLevelNew(security_id, buysell, level_added, price, size, ordercount, intermediate);
      ...
      cfe_book_manager->OnPriceLevelChange(security_id, buysell, level_changed, price, size, ordercount, intermediate);
      ...
      cfe_book_manager->OnPriceLevelDelete(security_id, buysell, level_changed, price, intermediate);
      ...
      cfe_book_manager->OnTrade(security_id, price, size);
      ...
      cfe_book_manager->OnPriceLevelOverlay(security_id, buysell, level_overlayed, price, size, ordercount,
                                            intermediate);

    } else if (instruction[2].compare("OnSpreadTrade") == 0) {
      // All levels till given level are deleted ( including the given level)

      double price = atof(instruction[column_number++].c_str());
      int size = atoi(instruction[column_number++].c_str());

      cfe_book_manager->OnSpreadTrade(security_id, price, size);
