/**
   \file Tests/dvctrade/MarketAdapter/eobi_mvm_tests.hpp
...
class EOBIMvmTests : public CppUnit::TestFixture {
  /**
   * General testing framework,
   * Currently It reads inputs from a file and processes accordingly
   */
  CPPUNIT_TEST_SUITE(EOBIMvmTests);
  CPPUNIT_TEST(TestEOBIMvm);
  ...
  void TestEOBIMvm(void);

  std::string exchange_;
  std::vector<std::vector<std::string> > eobi_instruction_list_;
