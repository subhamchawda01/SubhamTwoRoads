...
#include "baseinfra/Tests/MarketAdapter/ose_of_mvm_tests.hpp"

namespace HFTEST {

void OSEOFMvmTests::setUp() {
  exchange_ = "OSEOF";

  std::vector<std::string> shortcode_list_;

  MvmTestsUtils::ReadInstructionFile(exchange_, shortcode_list_, ose_of_instruction_list_);
  common_smv_source_ = new CommonSMVSource(shortcode_list_, 20170710);

  // remove this later on when we completely scrape away PF
  common_smv_source_->SetOSEBookType(true);
  //////////////
...
void OSEOFMvmTests::TestOSEOFMvm() {
  auto& sec_name_indexer = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  auto ose_of_book_manager = common_smv_source_->indexed_ose_order_feed_market_view_manager();
  std::string current_test_name = "";

  std::cout << std::endl;

  for (auto& instruction : ose_of_instruction_list_) {
    //
    ...
    if (instruction[1].compare("TESTCASE") == 0) {
      ...
      if (reset_book) ose_of_book_manager->OnOrderResetBegin(security_id);
      ...
      ose_of_book_manager->OnOrderAdd(security_id, order_id, side, price, size, priority, intermediate);
      ...
      ose_of_book_manager->OnOrderModify(security_id, order_id, side, new_size, new_order_id, intermediate);
      ...
      ose_of_book_manager->OnOrderDelete(security_id, order_id, side, intermediate);
      ...
      ose_of_book_manager->OnOrderExec(security_id, order_id, side, price, size_exec, intermediate);

    } else if (instruction[2].compare("OnTradingStatus") == 0) {
      std::string status = instruction[column_number++];
      ose_of_book_manager->OnTradingStatus(security_id, status);
