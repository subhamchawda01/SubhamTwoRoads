/**
   \file Tests/dvctrade/MarketAdapter/ice_mvm_tests.cpp
...
#include "baseinfra/Tests/MarketAdapter/ice_mvm_tests.hpp"

namespace HFTEST {

void ICEMvmTests::setUp() {
  exchange_ = "ICE";
...
void ICEMvmTests::TestICEMvm() {
  auto& sec_name_indexer = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  auto ice_book_manager = common_smv_source_->indexed_ice_market_view_manager();
  ...
  for (auto& instruction : cme_instruction_list_) {
    ...
    if (instruction[1].compare("TESTCASE") == 0) {
      ...
      if (reset_book) ice_book_manager->ResetBook(security_id);
      ...
      ice_book_manager->OnPriceLevelNew(security_id, buysell, level_added, price, size, ordercount, intermediate);
      ...
      ice_book_manager->OnPriceLevelChange(security_id, buysell, level_changed, price, size, ordercount, intermediate);
      ...
      ice_book_manager->OnPriceLevelDelete(security_id, buysell, level_changed, price, intermediate);
      ...
      ice_book_manager->OnTrade(security_id, price, size, buysell);
      ...
      ice_book_manager->OnPriceLevelOverlay(security_id, buysell, level_overlayed, price, size, ordercount,
                                            intermediate);
      ...
      ice_book_manager->OnPriceLevelDeleteThrough(security_id, buysell, max_level_deleted, intermediate);

    } else if (instruction[2].compare("OnResetBegin") == 0) {
      // Resets the book

      ice_book_manager->OnResetBegin(security_id);

    } else if (instruction[2].compare("OnResetEnd") == 0) {
      // Doesn't affect the book at all

      ice_book_manager->OnResetEnd(security_id);
      ...
      ice_book_manager->OnPriceLevelDeleteFrom(security_id, buysell, min_level_deleted, intermediate);
