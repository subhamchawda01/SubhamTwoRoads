/**
   \file Tests/dvctrade/MarketAdapter/tmx_mvm_tests.hpp
...
class TMXMvmTests : public CppUnit::TestFixture {
  /**
   * General testing framework,
   * Currently It reads inputs from a file and processes accordingly
   */
  CPPUNIT_TEST_SUITE(TMXMvmTests);
  CPPUNIT_TEST(TestTMXMvm);
  ...
  void TestTMXMvm(void);

  std::string exchange_;
  std::vector<std::vector<std::string> > tmx_instruction_list_;
