/**
   \file Tests/dvctrade/MarketAdapter/nse_mvm_tests.hpp
...
class NSEMvmTests : public CppUnit::TestFixture {
  /**
   * General testing framework,
   * Currently It reads inputs from a file and processes accordingly
   */
  CPPUNIT_TEST_SUITE(NSEMvmTests);
  CPPUNIT_TEST(TestNSEMvm);
  ...
  void TestNSEMvm(void);

  std::string exchange_;
  std::vector<std::vector<std::string> > nse_instruction_list_;
