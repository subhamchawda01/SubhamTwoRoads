/**
   \file Tests/dvctrade/MarketAdapter/ose_pf_mvm_tests.cpp
...
#include "baseinfra/Tests/MarketAdapter/ose_pf_mvm_tests.hpp"

namespace HFTEST {

void OSEPFMvmTests::setUp() {
  exchange_ = "OSEPF";
...
void OSEPFMvmTests::TestOSEPFMvm() {
  auto& sec_name_indexer = HFSAT::SecurityNameIndexer::GetUniqueInstance();
  auto ose_book_manager = common_smv_source_->indexed_ose_price_feed_market_view_manager();
  ...
  for (auto& instruction : cme_instruction_list_) {
    ...
    if (instruction[1].compare("TESTCASE") == 0) {
      ...
      if (reset_book) ose_book_manager->ResetBook(security_id);
      ...
      ose_book_manager->OnPriceLevelNew(security_id, buysell, level_added, price, size, ordercount, intermediate);
      ...
      ose_book_manager->OnPriceLevelChange(security_id, buysell, level_changed, price, size, ordercount, intermediate);
      ...
      ose_book_manager->OnPriceLevelDelete(security_id, buysell, level_changed, price, intermediate);
      ...
      ose_book_manager->OnTrade(security_id, price, size, buysell);

    } else if (instruction[2].compare("OnPriceLevelOverlay") == 0) {
      // A level is overlayed with new price/size/order_count
      // Not being used in ose as of now

      TradeType_t buysell = instruction[column_number++][0] == 'B' ? kTradeTypeBuy : kTradeTypeSell;
      int level_overlayed = atoi(instruction[column_number++].c_str());
      double price = atof(instruction[column_number++].c_str());
      int size = atoi(instruction[column_number++].c_str());
      int ordercount = atoi(instruction[column_number++].c_str());
      bool intermediate = (atoi(instruction[column_number++].c_str()) != 0);

      ose_book_manager->OnPriceLevelOverlay(security_id, buysell, level_overlayed, price, size, ordercount,
                                            intermediate);

    } else if (instruction[2].compare("OnPriceLevelDeleteThrough") == 0) {
      // All levels till given level are deleted ( including the given level)
      // not being used in ose as of now

      TradeType_t buysell = instruction[column_number++][0] == 'B' ? kTradeTypeBuy : kTradeTypeSell;
      int max_level_deleted = atoi(instruction[column_number++].c_str());
      bool intermediate = (atoi(instruction[column_number++].c_str()) != 0);

      ose_book_manager->OnPriceLevelDeleteThrough(security_id, buysell, max_level_deleted, intermediate);

    } else if (instruction[2].compare("OnOffMarketTrade") == 0) {
      // Doesn't affect the book at all, just notifies the specific listeners

      TradeType_t buysell = instruction[column_number++][0] == 'B' ? kTradeTypeBuy : kTradeTypeSell;
      double price = atof(instruction[column_number++].c_str());
      int size = atoi(instruction[column_number++].c_str());

      ose_book_manager->OnOffMarketTrade(security_id, price, size, buysell);
      ...
      ose_book_manager->OnPriceLevelDeleteFrom(security_id, buysell, min_level_deleted, intermediate);
