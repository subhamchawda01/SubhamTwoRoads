// =====================================================================================
//
//       Filename:  mid_term_data_and_order_server.cpp
//
//    Description:
//
//        Version:  1.0
...
#include "midterm/MidTerm/live_algo_manager.hpp"
...
class DataAndOrderServerManager {
private:
  HFSAT::DebugLogger &dbglogger_;
  TradeBarGenerator &trade_bar_generator_;
  HFSAT::TCPClientSocket *tcp_client_socket_;
  MIDTERM::BaseTrader &live_trader_;
  MIDTERM::BaseAlgoManager &base_algo_manager_;
  MIDTERM::MidTermOrderManager &mid_term_order_manager_;

public:
  DataAndOrderServerManager(Mode operating_mode, HFSAT::DebugLogger &dbglogger,
                            HFSAT::Watch &watch)
      : dbglogger_(dbglogger),
        trade_bar_generator_(TradeBarGenerator::GetUniqueInstance(
            operating_mode, dbglogger, watch)),
        tcp_client_socket_(new HFSAT::TCPClientSocket()),
        live_trader_(MIDTERM::LiveTrader::GetUniqueInstance(
            dbglogger, watch, tcp_client_socket_)),
        base_algo_manager_(MIDTERM::LiveAlgoManager::GetUniqueInstance(
            operating_mode, dbglogger, watch, tcp_client_socket_)),
        mid_term_order_manager_(MIDTERM::MidTermOrderManager::GetUniqueInstance(
            operating_mode, dbglogger, watch, live_trader_,
            base_algo_manager_)) {
    // Runs tcp wait thread for ORS replies
    if (operating_mode == Mode::kNSELoggerMode)
      return;
    static_cast<MIDTERM::LiveAlgoManager &>(base_algo_manager_).run();
  }

  TradeBarGenerator *GetDataProcessingObject() { return &trade_bar_generator_; }

  MIDTERM::MidTermOrderManager *GetOrderProcessingObject() {
    return &mid_term_order_manager_;
  }

  MIDTERM::BaseAlgoManager *GetAlgoProcessingObject() {
    return &base_algo_manager_;
  }
...
DataAndOrderServerManager *GLOBAL_PTR_FOR_TERMINATION;
...
void print_usage_and_exit(char const *prog_name) {
  std::cout << "THIS IS THE MID_TERM DATA AND TRADE SERVER EXEC" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  std::set<std::string> filtered_products_set;
  ...
  for (auto shc_ : shc_list) {
    sec_name_indexer.AddString(HFSAT::ExchangeSymbolManager::GetExchSymbol(shc_), shc_);
  }
  ...
  t_temp_oss
      << "/spare/local/logs/alllogs/mid_term_data_and_order_server_options_"
      << trading_date << ".log";
  ...
  DataAndOrderServerManager data_and_order_server_manager(operating_mode,
                                                          dbglogger, watch);
  ...
  for (uint32_t sec_counter = 0; sec_counter < sec_name_indexer.NumSecurityId();
    ...
    HFSAT::SecurityMarketView *this_new_smv = new HFSAT::SecurityMarketView(
        ...
        sec_name_indexer.GetSecurityNameFromId(sec_counter), sec_counter,
        HFSAT::SecurityDefinitions::GetContractExchSource(
            sec_name_indexer.GetShortcodeFromId(sec_counter), trading_date),
    ...
    this_new_smv->subscribe_rawtradeprints(
        data_and_order_server_manager.GetDataProcessingObject());
    this_new_smv->subscribe_L1_Only(
        data_and_order_server_manager.GetDataProcessingObject());
    this_new_smv->subscribe_L1_Only(
        data_and_order_server_manager.GetOrderProcessingObject());
    sid_to_smv_ptr_map.push_back(this_new_smv);

    std::cout << "USING SYMBOL : "
              << sec_name_indexer.GetSecurityNameFromId(sec_counter) << '\t'
			  << sec_name_indexer.GetShortcodeFromId(sec_counter) << '\t'
			  << HFSAT::NSESecurityDefinitions::GetStrikePriceFromShortCodeGeneric(sec_name_indexer.GetShortcodeFromId(sec_counter)) << '\t'
              << std::endl;
  }
  if (operating_mode == Mode::kNSEServerMode) {
    data_and_order_server_manager.GetOrderProcessingObject()->Initialize(
        operating_mode);
  }
