...
class BookManager : public HFSAT::Thread,
 ...
 private:
  ...
  std::vector<HFSAT::NSETradingLimit_t *> t_trading_limit_vec_;
 ...
 public:
  BookManager(HFSAT::DebugLogger& dbglogger, HFSAT::Watch& watch, vector<std::string> const& _shortcode_vec_, std::vector<int> & _saci_vec_,
    ...
    for(uint32_t shc_indx_ = 0; shc_indx_ < shortcode_vec_.size(); shc_indx_++){
      ...
      t_trading_limit_vec_.push_back(HFSAT::NSESecurityDefinitions::GetTradingLimits(shortcode_vec_[shc_indx_]));
  ...
  double GetPrice(int rnd_sec_id_){
    ...
    //        HFSAT::NSESecurityDefinitions::GetTradingLimits(t_smv->shortcode());
  ...
  void thread_main() {
    ...
    for(uint32_t shc_indx_ = 0; shc_indx_ < shortcode_vec_.size(); shc_indx_++){
      ...
      this_exch_source = HFSAT::SecurityDefinitions::GetContractExchSource(shortcode_vec_[shc_indx_], tradingdate);
    ...
    for(uint32_t sec_id_ = 0; sec_id_ < sec_id_vec_.size(); sec_id_++){
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderNotFoundListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderSequencedListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderConfirmedListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderConfCxlReplacedListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderConfCxlReplaceRejectListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderCancelSequencedListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderCanceledListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderExecutedListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderRejectedListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderRejectedDueToFundsListener(sec_id_, this);
      combined_mds_messages_shm_processor_.GetORSReplyProcessor()->AddOrderInternallyMatchedListener(sec_id_, this);
    }
 ...
 public:
  ORSReplyManager(HFSAT::DebugLogger& dbglogger, HFSAT::Watch& watch, std::vector<std::string> const& _shortcode_vec_,
    ...
    for(uint32_t shc_indx_ = 0; shc_indx_ < _shortcode_vec_.size(); shc_indx_++){
      ...
      sec_id_vec_.push_back(sec_id);
      this_exch_source = HFSAT::SecurityDefinitions::GetContractExchSource(_shortcode_vec_[shc_indx_], tradingdate);
    }
...
void ModifyOrder(int rnd_sec_id_, BookManager& book_manager, HFSAT::BaseTrader* tr, HFSAT::FastPriceConvertor* fast_px_converter_,
  ...
  try {
    ...
    if (new_size_requested_ > (int)max_size) {
...
int main(int argc, char** argv) {
  ...
  for(uint32_t shc_indx_ = 0; shc_indx_ < shortcode_vec_.size(); shc_indx_++){
    ...
    if (std::string::npos != shortcode_vec_[shc_indx_].find("NSE_")) {
      ...
      if (std::string::npos != shortcode_vec_[shc_indx_].find("INR_") || std::string::npos != shortcode_vec_[shc_indx_].find("_GIND")) {
        ...
        if (std::string::npos != shortcode_vec_[shc_indx_].find("_FUT") || HFSAT::NSESecurityDefinitions::IsOption(shortcode_vec_[shc_indx_])) {
  ...
  /// create trader class
    ...
    if (exchange_source == HFSAT::kExchSourceICE) {
      HFSAT::Utils::ETIAlgoTagging::GetUniqueInstance("ICE").TagAlgo(trader_vec_[shc_indx_]->GetClientId(), "CONSOLE", "CONSOLE", "ICE");
    }
  } 
