...
if ( length(args) < 2 ) {
        stop ("USAGE : <script> <data> <history>\n");
}

data_file <- args[1];
di1_data <- read.table(args[1]);
di1_data <- as.matrix(di1_data);
num_rows <- dim ( di1_data ) [1] 
num_cols <- dim ( di1_data ) [2];
history <- as.numeric(args[2]);

pca_refresh_time <- history/10.0;

# we start trading after we have seen this much data
