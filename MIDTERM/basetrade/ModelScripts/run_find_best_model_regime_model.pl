...
my $input_regime_ind_list_file_ = "";
...
sub ReadConfigFile
{
    ...
    while ( my $thisline_ = <CONFIGFILEHANDLE> )
    {
	    ...
	    when("INPUT_REGIME_IND_LIST_FILE")
            {
		$input_regime_ind_list_file_ = $thisline_words_[0];
            }
    ...
    my $fbm_version_string_ = "v0:find_best_model_for_strategy_var_pert_formultmodel.pl";
...
sub RunFindBestModel
{
    ...
    my $num_installed_ = 0;
    my $num_regime_inds_ = `wc -l $input_regime_ind_list_file_ | awk '{print \$1}'`;
    chomp ( $num_regime_inds_ );
    $num_regime_inds_ = int( $num_regime_inds_ );
    for (my $i=0; $i<=$#input_strat_list_; $i++)
    {
      for ( my $j=1; $j<= $num_regime_inds_; $j++ ){
	...
	`mkdir -p $compare_dir_uninstalled_`; 
        ...
        my $this_output_model_file_ = $work_dir_."w_model_opt_".$j."_".$i;
        #my  $this_output_model_file_ = "/home/".$USER."/modelling/fbmfswork/LFR_0/tmp_model_".$i;
        my $this_output_strat_file_ = $work_dir_."w_opt_".$j."_".$i;
        my $debug_string_ = "D";
        if($debug_ == 0)
        {
            $debug_string_ = "N"
        ...
        {
            $exec_cmd_ = $MODELSCRIPTS_DIR."find_best_model_for_strategy_var_pert_formultmodel.pl $this_input_strat_file_ $this_output_model_file_ $input_regime_ind_list_file_ $j $last_trading_date_ $num_prev_days_ $change_fraction_ $max_number_iterations_ $search_algo_ $debug_string_ $cost_fn_ $min_vol_ $max_ttc_ $num_perturbations_ $retain_more_pert_ $training_day_inclusion_prob_ $training_days_type_ $eco_date_time_file_ $use_fake_faster_data_";
        }
        ...
        foreach (@output_lines_split_)
        {
            ...
            elsif($words_[0] eq "OrigOnTraining" || $words_[0] eq "BestOnTraining")
            {
        ...
        $exec_cmd_ = $MODELSCRIPTS_DIR."compare_opt_regime_model.pl $this_output_model_file_ $input_model_full_path_";
	...
	my $mail_file_ = "";
            ...
            if($install_strats_ == 0)
	    {
 		$input_model_dir_path_ = $install_dir_;
            ...
            my $opt_model_full_path_ = $input_model_dir_path_."/w_reg_".$run_string_."_".$input_model_name_;
            my $opt_strat_full_path_ = $input_strat_dir_path_."/w_reg_".$run_string_."_".$input_strat_name_;
            while(-e $opt_model_full_path_ || -e $opt_strat_full_path_)
            {
              $run_string_ = $run_string_.int(rand(9));
              $opt_model_full_path_ = $input_model_dir_path_."/w_reg_".$run_string_."_".$input_model_name_;
              $opt_strat_full_path_ = $input_strat_dir_path_."/w_reg_".$run_string_."_".$input_strat_name_;
            }
	    ...
	    $comp_file_ = $compare_dir_installed_."compreg_".$run_string_."_".$input_strat_name_;
	    $mail_file_ = $compare_dir_installed_."mailreg_".$run_string_."_".$input_strat_name_;
            $num_installed_ ++;
        ...
        else
        {
            ...
            my $input_strat_name_ = `basename $this_input_strat_file_`; chomp($input_strat_name_);
	    my $input_model_name_ = `basename $input_model_full_path_`; chomp($input_model_name_);
            my $opt_model_full_path_ = $compare_dir_uninstalled_."w_reg_un_".$run_string_."_".$input_model_name_;
            my $opt_strat_full_path_ = $compare_dir_uninstalled_."/w_reg_un_".$run_string_."_".$input_strat_name_;
	    while(-e $opt_model_full_path_ || -e $opt_strat_full_path_)
            {
              $run_string_ = $run_string_.int(rand(9));
              $opt_model_full_path_ = $compare_dir_uninstalled_."w_reg_un_".$run_string_."_".$input_model_name_;
              $opt_strat_full_path_ = $compare_dir_uninstalled_."/w_reg_un_".$run_string_."_".$input_strat_name_;
            }
            if ( $install_strats_ == 1 )
	    {
		`cp $this_output_model_file_  $opt_model_full_path_`;
		...
		`cp $this_output_strat_file_  $opt_strat_full_path_`;
            ...
            $comp_file_ = $compare_dir_uninstalled_."compreg_".$run_string_."_".$input_strat_name_;
            $mail_file_ = $compare_dir_uninstalled_."mailreg_".$run_string_."_".$input_strat_name_;
