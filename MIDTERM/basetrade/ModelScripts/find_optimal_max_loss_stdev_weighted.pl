...
require "$GENPERLLIB_DIR/array_ops.pl"; # CalcPrevWorkingDateMult

# start
my $USAGE="$0 SHORTCODE TIMEPERIOD [DAYS_TO_LOOK_BEHIND=60] [RATIO_OF_NUM_OF_MAX_LOSS_HITS=0.25] [NUM_TOP_LOSSES=5] [STRAT_NAME=ALL_INSTALLED] [ENDDATE=TODAY] [SKIP_DATES_FILE=INVALIDFILE] [results_dir=~/ec2_globalresults]";
...
if ( $#ARGV > 7 ) { $GLOBAL_RESULTS_DIR = $ARGV [ 8 ]; }
...
my %similarity_weights_ = ( );
GetVolatileSimilarityMap ( $shortcode_, $end_date_, $days_to_look_behind_, \%similarity_weights_ );
...
print DATESFHANDLE join("\n", keys %similarity_weights_)."\n";
close DATESFHANDLE;

my $start_date_ = min keys %similarity_weights_;
...
sub ChooseMaxLoss
{
  ...
  my %strat_to_max_loss_to_sum_pnl_ = ( );
  ...
  foreach my $strat_name_ ( @running_queries_ )
  {
    ...
    foreach my $result_line_ ( @ { $strat_to_results_ { $strat_name_ } } )
    {
      ...
      $min_pnl_list_{ $date } = $t_min_pnl_;
      $final_pnl_list_{ $date } = $t_final_pnl_;
    }

    my @max_losses_to_try_ = values %min_pnl_list_;
...
# Compute results for each value of max loss.
    foreach my $max_loss_ ( @max_losses_to_try_ )
    {
      ...
      $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } { $max_loss_ } = 0;
      $strat_to_max_loss_to_num_times_max_loss_hit_ { $strat_name_ } { $max_loss_ } = 0;

      foreach my $t_date_ ( keys %final_pnl_list_ )
      {
        my $t_min_pnl_ = $min_pnl_list_ { $t_date_ };
        my $t_final_pnl_ = $final_pnl_list_ { $t_date_ };

        if ( $t_min_pnl_ <= $max_loss_ )
        { 
# With this value of max loss , this query would hit max loss and stop
          $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } { $max_loss_ } += $similarity_weights_{ $t_date_ } * $max_loss_;
          $strat_to_max_loss_to_num_times_max_loss_hit_ { $strat_name_ } { $max_loss_ } += $similarity_weights_{ $t_date_ };
        }
        else
        { 
# Query would continue to trade till final pnl
          $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } { $max_loss_ } += $similarity_weights_{ $t_date_ } * $t_final_pnl_;
        }
      }
    ...
    printf ( "%s =>\n" , $strat_name_ );
    printf ( "%10s %10s %s\n" , "MAX_LOSS" , "MEAN_PNL" , "NUM_MAX_LOSS_HITS" );
    my $already_printed_ = 0;
    foreach my $max_loss_ ( sort { $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } { $b } <=> $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } { $a } }
        keys ( % { $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } } ) )
    {
      next if ( $ratio_ < $strat_to_max_loss_to_num_times_max_loss_hit_ {  $strat_name_ } { $max_loss_ } ) ;	    

      printf ( "%10.2f %10.2lf %2.2f\n" , abs ( $max_loss_ ) , $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } { $max_loss_ } , $strat_to_max_loss_to_num_times_max_loss_hit_ {  $strat_name_ } { $max_loss_ } );
      $already_printed_++;
...
# Now optimize max-loss across this strat set.
    ...
    {
      foreach my $strat_name_ ( keys ( %strat_to_max_loss_to_sum_pnl_ ) )
      {
        my $t_sum_pnl_ = 0;
        my $t_max_loss_diff_ = 999999999999999;
        my $t_num_times_max_loss_hit_ = 0;

        foreach my $s_max_loss_ ( keys ( % { $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } } ) )
        {
# Find the max-loss value "closest" to the one under consideration.
          if ( abs ( $max_loss_ - $s_max_loss_ ) < $t_max_loss_diff_ )
          {
            $t_sum_pnl_ = $strat_to_max_loss_to_sum_pnl_ { $strat_name_ } { $s_max_loss_ };
        ...
        $max_loss_to_sum_pnl_ { $max_loss_ } += $t_sum_pnl_;
    ...
    {
      next if ( @running_queries_ < $max_loss_to_num_times_max_loss_hit_ { $max_loss_ } ) ;
...
#  print $tradingdates_similarity_json_string_."\n";
  ...
  my $sum_weights_ = 0;
  foreach my $t_sim_weight_ ( values %$tradingdates_similarity_ref_ ) { $sum_weights_ += $t_sim_weight_; }
  foreach my $t_date_ ( keys %$tradingdates_similarity_ref_ ) { 
    $$tradingdates_similarity_ref_{ $t_date_ } = min ( $$tradingdates_similarity_ref_{ $t_date_ }, 0.25 * $sum_weights_ );
  }
  $sum_weights_ = 0;
  foreach my $t_sim_weight_ ( values %$tradingdates_similarity_ref_ ) { $sum_weights_ += $t_sim_weight_; }

  foreach my $t_date_ ( keys %$tradingdates_similarity_ref_ ) { 
    $$tradingdates_similarity_ref_{ $t_date_ } = $$tradingdates_similarity_ref_{ $t_date_ } / $sum_weights_;
  }
