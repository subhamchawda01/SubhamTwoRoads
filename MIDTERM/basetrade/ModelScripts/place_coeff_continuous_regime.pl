...
while ( my $iline_ = <ILIST> )
{
	...
	elsif ( $iwords_[0] eq "MODELMATH" )
	{
	    $ilist_pre_indicator_text_ .= "MODELMATH SELECTIVECONTINUOUS $iwords_[2]\n";
	}
	else {
...
#print "@indicator_trailing_text_\n";
my $ilist_indicator_text_ = "";
my $regime_params_ = "";
...
#	print "vals: $this_init_corr_\t$this_ts_\t$this_auto_corr_\n";
	my $outline_ = sprintf("INDICATOR %s %s\n", $this_coeff_, $indicator_trailing_text_[$this_index_]);
#	my $outline_ = "INDICATOR ".$this_coeff_." ".$indicator_trailing_text_[$this_index_]." # IC: $this_init_corr_ TS: $this_ts_ AC: $this_auto_corr_ \n" ;
	$ilist_indicator_text_ = $ilist_indicator_text_.$outline_;
	$model_size_++;
    }
    elsif ( $regwords_[0] eq "CREGIMECUT" ) { $regime_params_ = "@regwords_";}
    elsif ( $regwords_[0] eq "Factor" && int($regwords_[1]) > 1) {$ilist_indicator_text_= $ilist_indicator_text_."INDICATORINTERMEDIATE\n";}
...
my $ilist_regime_indicator_text_ = sprintf("REGIMEINDICATOR 1.00 %s \n%s\nINDICATORSTART\n", $indicator_trailing_text_[0], $regime_params_);
$ilist_pre_indicator_text_ = $ilist_pre_indicator_text_.$ilist_regime_indicator_text_;
$ilist_pre_indicator_text_ = $ilist_pre_indicator_text_.$ilist_indicator_text_;
...
if ( $model_size_ >0 && ($model_rsquared_ <= 0  || $model_corr_ == 0.0 ) )
{
    ...
    ## Mail Header
    print MAIL "To: puneet.singh\@tworoads.co.in\n";
