...
# \file ModelScripts/label_timed_data_rows_for_bad_periods.pl
#
...
# start 
my $USAGE="$0 TIMED_DATA_INPUT_FILENAME TIMED_DATA_OUTPUT_FILENAME SHORTCODE YYYYMMDD";
...
while ( my $inline_ = <INFILE> )
{
    ...
    if ( $bad_period_index_<= $#bad_periods_end_ && $t_msecs_ >= $bad_periods_start_ [ $bad_period_index_ ] && $t_msecs_ <= $bad_periods_end_ [ $bad_period_index_ ] )
    {
#print "$t_msecs_ | $bad_periods_end_[$bad_period_index_] #  $bad_period_index_ and $#bad_periods_end_ $is_bad_period_\n";
      $is_bad_period_ = "true";
    }

    if ( $is_bad_period_ )
    {
      print OUTFILE $inline_." 1 \n";
    }
    else
    {
      print OUTFILE $inline_." 0 \n";
    }
}
