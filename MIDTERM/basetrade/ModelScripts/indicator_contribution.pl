...
if ( $shortcode_ ne "NA" )
{
  ...
  $min_price_increment_ = `$exec_cmd_` ; chomp ( $min_price_increment_ ); # using single date in hope that min_inc doesnt change much
}
...
#	print $main_log_file_handle_ "$exec_cmd\n";
	...
	if ( -e $this_day_timed_data_filename_ )
	{
		...
		if ( $#dwords_ >= $#model_coeff_vec_ + 4 )
		{
			...
			{
                          my $t_ind_val_ = $dwords_[ $j + 4 ];
                          $t_ind_val_ = ( 1 / ( 1 + exp ( - $t_ind_val_ * $model_coeff_alpha_vec_[$j] ) ) - 0.5 ) * $model_coeff_beta_vec_[$j];
                          if(abs($t_ind_val_) > 10*$min_price_increment_) {$huge_vals_=1; last;} 
                          push(@t_ind_val_vec_, $t_ind_val_);
                          $this_model_value_ += $t_ind_val_;
			}		
                    ...
                    if(abs($this_model_value_) > 10*$min_price_increment_ || abs($this_model_value_) < 0.001*$min_price_increment_)  {$huge_vals_=1;}
...
printf "MODEL STDEV: %f L1NORM: %f FITERPER: %f\n",$current_model_stdev_,$current_model_l1norm_,$filtered_pts_ ;
