...
# NOTE: DATAGEN_DAY_FILTER has updated in generate_strategies.pl and has to ben similarly updated here
# Else it would throw ERROR
...
if ( $USER eq "ankit" || $USER eq "kputta" )
{
    $LIVE_BIN_DIR=$HOME_DIR."/".$REPO."_install/bin";
}
...
my $pnl_script_start_date_ = "";
my $pnl_script_num_lookback_days_ = 0;
my $pnl_script_volume_cutoff_ = 0;
my $work_dir_pnl_script_ = $work_dir_;
...
my $thin_book_timeperiod_ = "US";
my $thin_book_periods_only_ = 0;
my $highlow_corr_periods_only_ = 0; # Time in secs to find large price moves. Default = 120 secs
my @highlow_corr_periods_indeps_ = ( );
my @highlow_corr_periods_type_ = ( );
my $highlow_corr_period_type_ = "HIGH";
my $highlow_corr_period_indep_ = "ES_0"; # defaults setting but not expected to run in this manner
my @high_stdev_periods_indeps_ = ( );
my @high_stdev_periods_timeperiods_ = ( );
my @high_stdev_periods_lookaheads_ = ( );
my $high_stdev_period_indep_ = "ES_0";
my $high_stdev_period_timeperiod_ = "US";
my $high_stdev_period_lookahead_= "300";
...
sub LoadInstructionFile 
{
	...
	{ # $#instruction_line_words_ >= 0
		...
		{ # $current_instruction_ is valid
			...
			when ("LAST_PICKED_INDICATOR_INDEX")
			{
			    if (scalar(@indicator_list_file_vec_) > 0 && $LAST_PICKED_INDICATOR_INDEX == -1)  #hopes to see indicatorfilelist before this instruction
			    {
				$LAST_PICKED_INDICATOR_INDEX = int ( $instruction_line_words_[0] );
				if ( length ( $thisline_ ) == 4 )
				{
				    if ( $LAST_PICKED_INDICATOR_INDEX < scalar(@indicator_list_file_vec_) && $LAST_PICKED_INDICATOR_INDEX >0 )
				    {
					#print $main_log_file_handle $LAST_PICKED_INDICATOR_INDEX,"\n";
					seek ( INSTRUCTIONFILEHANDLE,- ( length($thisline_ )+1 ),1 );
					printf INSTRUCTIONFILEHANDLE ("%04d\n",$LAST_PICKED_INDICATOR_INDEX + 1);
					$LAST_PICKED_INDICATOR_INDEX = $LAST_PICKED_INDICATOR_INDEX + 1;
				    }
				    else
				    {
					#print $main_log_file_handle $LAST_PICKED_INDICATOR_INDEX,"\n";
			...
			{
			    print $main_log_file_handle_ "On reading DATAGEN_LOW_VOLUME_DAYS datagen_day_vec reset to empty\n";
			    @datagen_day_vec_ = ();
			    GetLowVolumeDaysForShortcode ( $shortcode_, \@datagen_day_vec_ );
			    if ( $#datagen_day_vec_ >= 3 )
			    {
				print $main_log_file_handle_ "Only model on low volume days:",join ( ' ', @datagen_day_vec_ ),"\n";

				if ( index ( $name_, "lv" ) != -1 )
				{ 
				    $name_ = "lv_".$name_; # change name to indicate lv
				}
			    }
			    else
			    {
			...
			{
			    print $main_log_file_handle_ "On reading DATAGEN_HIGH_STDEV_DAYS datagen_day_vec reset to empty\n";
			    @datagen_day_vec_ = ();
			    my $t_shortcode_ = $instruction_line_words_[0];			    
			    
                            push ( @high_stdev_periods_indeps_, $instruction_line_words_[0] );
                            push ( @high_stdev_periods_timeperiods_, $instruction_line_words_[1] );
			    push ( @high_stdev_periods_lookaheads_, $instruction_line_words_[2] );

                            if ( rand( 1 ) > 1 / ($#high_stdev_periods_indeps_ + 1 ) )
                            {
                                $high_stdev_period_indep_ = $instruction_line_words_[0];
                                $high_stdev_period_timeperiod_  = $instruction_line_words_[1];
				$high_stdev_period_lookahead_ = $instruction_line_words_[2];
                            }
			    
			    GetHighStdevDaysForShortcode ( $high_stdev_period_indep_, $high_stdev_period_timeperiod_, $high_stdev_period_lookahead_, \@datagen_day_vec_ );
			    
			    if ( $#datagen_day_vec_ >= 3 )
			    {	
				print $main_log_file_handle_ "Only model on high stdev days:", join ( " ", @datagen_day_vec_ ),"\n";
				
			    	$name_ = "hsd.".lc($high_stdev_period_indep_); #change name to indicate hsd for high std deviation
			    }
			    else
			    {
				@datagen_day_vec_ = ();
			...
			{
			    if ( $thin_book_periods_only_ <=0 && 
				 ( rand ( 1 ) > 0.5 ) ) # Take this with a 50% prob.
			    {
				$thin_book_timeperiod_ = $instruction_line_words_[0];
				$thin_book_periods_only_ = 1;
				$name_ = "thinbookp".$name_; # Change name to indicate thin book period strat.
			    }          
			}
			...
			when ( "HIGH_LOW_CORR_PERIODS" )
			{
			    if ( $#instruction_line_words_ >= 0 )
			    {
				$highlow_corr_periods_only_ = 1;
			        push ( @highlow_corr_periods_type_, $instruction_line_words_[0] );   			   			   
				push ( @highlow_corr_periods_indeps_, $instruction_line_words_[1] );
				
				if ( rand( 1 ) < 1 / ($#highlow_corr_periods_indeps_ + 1 ) )
				{
				    $highlow_corr_period_indep_ = $instruction_line_words_[1];
				    $highlow_corr_period_type_  = $instruction_line_words_[0];
				}	
				if ( $highlow_corr_period_type_ eq "LOW" )
				{
				    $name_ = "low.corr.".lc($highlow_corr_period_indep_);
				}
				else
				{
				    $name_ = "high.corr.".lc($highlow_corr_period_indep_);
				}
			    }
			}
			...
			when ( "PNL_SCRIPT_TRAIN_START_YYYYMMDD" )
			{
			    $pnl_script_start_date_ = GetIsoDateFromStrMin1 ( @instruction_line_words_ );
			}
			when ( "PNL_SCRIPT_NUM_LOOKBACK_DAYS" )
			{
			    $pnl_script_num_lookback_days_ = $instruction_line_words_[0];
			}
			when ( "PNL_SCRIPT_VOLUME_CUTOFF" )
			{
			    $pnl_script_volume_cutoff_ = $instruction_line_words_[0];
			}
			...
			when ("MAIL_ADDRESS") 
			{
			    ...
			    if ( $USER eq "kishenp" ) {
                                $mail_address_ = "kishenp\@tworoads.co.in";
                            }
			...
			when ( "AUTHOR" )
			{
			    $author_ = $instruction_line_words_ [ 0 ];
                        ...
                        when ( "GENERATE_MODELS_ON_RECENT_DAYS" )
                        {
                            $generate_models_on_recent_days_ = $instruction_line_words_ [ 0 ];
                        }
    ...
    if ( $generate_models_on_recent_days_ == 1 )
    {
	print $main_log_file_handle_ "generate_models_on_recent_days_ set, using TODAY-15 TODAY-1 for datagen and TODAY-40 TODAY-20 for sim\n";
        $datagen_start_yyyymmdd_ = GetIsoDateFromStrMin1 ( "TODAY-15" );
        $datagen_end_yyyymmdd_ = GetIsoDateFromStrMin1 ( "TODAY-1" );
        $trading_start_yyyymmdd_ = GetIsoDateFromStrMin1 ( "TODAY-40" );
        $trading_end_yyyymmdd_ = GetIsoDateFromStrMin1 ( "TODAY-20" );
    }
    ...
    {
	# Randomly pick a single max_ttc_to_allow_ instead
	# of running them all.
    ...
    if ( $INDICATOR_PICK_STRATEGY == -1 )
    {
	print INSTRUCTIONFILEHANDLE "\nINDICATOR_PICK_STRATEGY\n";
	$INDICATOR_PICK_STRATEGY = 0;
	print INSTRUCTIONFILEHANDLE "$INDICATOR_PICK_STRATEGY\n";
    }
    
    if ( $LAST_PICKED_INDICATOR_INDEX == -1 )
    {
	print INSTRUCTIONFILEHANDLE "\nLAST_PICKED_INDICATOR_INDEX\n";
	$LAST_PICKED_INDICATOR_INDEX = sprintf("%04d",0001);
	print INSTRUCTIONFILEHANDLE "$LAST_PICKED_INDICATOR_INDEX\n";
    }
    ...
    if ( $bad_periods_only_ == 1 || $thin_book_periods_only_ == 1 )
    {
	    ...
	    # && ( $apply_all_dep_filters_ == 0 ) )
	{
	    ...
	    @dep_based_filter_ = ( );
	    push ( @dep_based_filter_ , $t_random_dep_based_filter_ );
	}
	else
	{
    ...
    if ( $#dep_based_filter_ < 0 )
    {
	push ( @dep_based_filter_ , "fsg1" );
    }
    ...
    # check if multiple-dependant-predalgo (na_s4,na_m4) are present
    # in that case only keep them
    # and make sure regress_exec only has MULTLR
    if ( ( FindItemFromVec ( "na_s4", \@predalgo_ ) ) ||
	 ( FindItemFromVec ( "na_m4", \@predalgo_ ) ) )
    {
	my @permissible_predalgo_choices_ = ();
	for ( my $pa_idx_ = 0 ; $pa_idx_ <= $#permissible_predalgo_choices_ ; $pa_idx_ ++ )
	{
	    if ( ( $predalgo_ [ $pa_idx_ ] eq "na_s4" ) ||
		 ( $predalgo_ [ $pa_idx_ ] eq "na_m4" ) )
	    {
		push ( @permissible_predalgo_choices_, $predalgo_ [ $pa_idx_ ] ) ;
	    }
	}
	@predalgo_ = @permissible_predalgo_choices_ ;

	my @permissible_regress_exec_ = ();
	for ( my $re_idx_ = 0 ; $re_idx_ <= $#regress_exec_ ; $re_idx_ ++ )
	{
	    my $scalar_ref_re_ = $regress_exec_ [ $re_idx_ ];
	    if ( $$scalar_ref_re_[0] eq "MULTLR" )
	    {
		push ( @permissible_regress_exec_, $scalar_ref_re_ );
	    }
	}
	@regress_exec_ = @permissible_regress_exec_ ;
	if ( $#regress_exec_ < 0 )
	{
	    my $regress_exec_text_ = "MULTLR 4 0.01 0 0.85 18";
	    my @regress_exec_words_ = split ( ' ', $regress_exec_text_ );
	    push ( @regress_exec_ , [ @regress_exec_words_ ] );
	}
    }	 

    my $multlr_present_ = 0;
    for ( my $re_idx_ = 0 ; $re_idx_ <= $#regress_exec_ ; $re_idx_ ++ )
    {
        my $scalar_ref_re_ = $regress_exec_ [ $re_idx_ ];
        if ( $$scalar_ref_re_[0] eq "MULTLR" )
        {
            $multlr_present_ = 1; last;
        }
    }

#    if ( FindItemFromVec ( "MULTLR", \@regress_exec_words_ ) )
    if ( $multlr_present_ == 1 )
    {
	my @permissible_regress_exec_ = ();
	for ( my $re_idx_ = 0 ; $re_idx_ <= $#regress_exec_ ; $re_idx_ ++ )
	{
	...
	if ( $#regress_exec_ < 0 )
	{
	    my $regress_exec_text_ = "MULTLR 4 0.01 0 0.85 18 N";
	    my @regress_exec_words_ = split ( ' ', $regress_exec_text_ );
	    push ( @regress_exec_ , [ @regress_exec_words_ ] );
	}
...
sub SanityCheckInstructionFile 
{
    ...
    my @valid_filter_list_ = ("f0", "fst.5", "fst1", "fst2", "fsl2", "fsl3", "fsg.5", "fsg1", "fsg2", "fsr1_5", "fsr.5_3" , "flogit", "fv" );
	...
	{ # for each pred_duration
		...
		{ # generate reg data of this type
			...
			{   # reg_data_file missing 
				...
				{
				    $exec_cmd_="$LIVE_BIN_DIR/datagen $indicator_list_filename_ $tradingdate_ $datagen_start_hhmm_ $datagen_end_hhmm_ $unique_gsm_id_ $this_day_timed_data_filename_ $datagen_msecs_timeout_ $datagen_l1events_timeout_ $datagen_num_trades_timeout_ $to_print_on_economic_times_ ADD_DBG_CODE -1";
				}
				else
				{
				    $exec_cmd_="$LIVE_BIN_DIR/datagen $indicator_list_filename_ $tradingdate_ $datagen_start_hhmm_ $datagen_end_hhmm_ $unique_gsm_id_ $this_day_timed_data_filename_ $datagen_msecs_timeout_ $datagen_l1events_timeout_ $datagen_num_trades_timeout_ $to_print_on_economic_times_ $traded_ezone_ ADD_DBG_CODE -1";
				}
			    ...
			    if ( ( $large_price_move_periods_ == 1 ) && 
				 ( $this_td_file_generated == 1 ) )
			    {
			    ...
			    if ( ( $thin_book_periods_only_ == 1 ) && ( $this_td_file_generated == 1 ) )
                            {
                                # Only use periods where installed models performed badly.
                                my $this_day_thin_book_period_timed_data_filename_ = $this_day_timed_data_filename_."_thin_book_periods";

                                my $exec_cmd_ = "$MODELSCRIPTS_DIR/select_timed_data_rows_for_thin_book_periods.pl $this_day_timed_data_filename_ $this_day_thin_book_period_timed_data_filename_ $shortcode_ $thin_book_timeperiod_ $tradingdate_";

				print $main_log_file_handle_ "$exec_cmd_\n";
                                `$exec_cmd_`;

                                if ( ExistsWithSize ( $this_day_thin_book_period_timed_data_filename_ ) )
                                {
                                    print $main_log_file_handle_ "Adding $this_day_thin_book_period_timed_data_filename_ to be deleted later\n" ;
                                    push ( @intermediate_files_, $this_day_thin_book_period_timed_data_filename_ ); # if this is a new file we created then add it to set of files to delete at the end
                                }
                                else
                                {
                                    print $main_log_file_handle_ "Could not create non-zero-sized $this_day_thin_book_period_timed_data_filename_\n" ;
                                    print $main_log_file_handle_ "rm -f $this_day_thin_book_period_timed_data_filename_\n" ;
                                    `rm -f $this_day_thin_book_period_timed_data_filename_`;
                                }

                                $this_day_timed_data_filename_ = $this_day_thin_book_period_timed_data_filename_;
                            }   

			    
			    if ( ( $highlow_corr_periods_only_ == 1 ) && ( $this_td_file_generated == 1 ) )
			    {
				
				# Only use periods where the correlations with major leading products are high or low i.e, different from normal
				# For example using ES or 6E for DOL

                                my $this_day_highlow_corr_period_timed_data_filename_ = $this_day_timed_data_filename_."_highlow_corr_periods";

                                my $exec_cmd_ = "$MODELSCRIPTS_DIR/select_timed_data_rows_for_highlow_corr_periods.pl $this_day_timed_data_filename_ $this_day_highlow_corr_period_timed_data_filename_ $shortcode_ $highlow_corr_period_indep_ $highlow_corr_period_type_ $tradingdate_";

                                print $main_log_file_handle_ "$exec_cmd_\n";
                                `$exec_cmd_`;

                                if ( ExistsWithSize ( $this_day_highlow_corr_period_timed_data_filename_ ) )
                                {
                                    print $main_log_file_handle_ "Adding $this_day_highlow_corr_period_timed_data_filename_ to be deleted later\n" ;
                                    push ( @intermediate_files_, $this_day_highlow_corr_period_timed_data_filename_ ); # if this is a new file we created then add it to set of files to delete at the end
                                }
                                else
                                {
                                    print $main_log_file_handle_ "Could not create non-zero-sized $this_day_highlow_corr_period_timed_data_filename_\n" ;
                                    print $main_log_file_handle_ "rm -f $this_day_highlow_corr_period_timed_data_filename_\n" ;
                                    `rm -f $this_day_highlow_corr_period_timed_data_filename_`;
                                }

                                $this_day_timed_data_filename_ = $this_day_highlow_corr_period_timed_data_filename_;
...
#			  }
		...
		{ # now $this_reg_data_filename_ exists
			...
			else
			{
				...
				given ( $regtype_ ) 
				{
				    print $main_log_file_handle_ "Checking for retypes...found this ".$regtype_."\n";
				    when ( "SIMPNL" )
				    { # FSHLR 0.04 0 0 0.80 18 H/N
					
					print $main_log_file_handle_ "Inside SIMPNL\n";
					if ( $#$scalar_ref_ >= 2 ) 
					{					    
					    my $max_indep_correlation_ = $$scalar_ref_[1];
					    my $min_model_size_ = $$scalar_ref_[2];
					    my $max_model_size_ = $$scalar_ref_[3];

					    my $regress_dotted_string_ = join ( '_', ( $regtype_, $pnl_script_start_date_, $pnl_script_num_lookback_days_, $max_indep_correlation_, $min_model_size_, $max_model_size_ ) );
					    ...
					    my $t_strat_filename_ = GetStrategyFileForPnlScript ( $this_pred_duration_, $this_model_filename_ );
					    print $main_log_file_handle_ "StrategyFilename ".$t_strat_filename_."\n";

					    {
						my $exec_cmd="~/basetrade/ModelScripts/build_pnl_based_model.R $shortcode_ $this_filtered_reg_data_filename_ $t_strat_filename_ $indicator_list_filename_ $pnl_script_start_date_ $pnl_script_num_lookback_days_ $max_indep_correlation_  $min_model_size_ $max_model_size_ $pnl_script_volume_cutoff_ $work_dir_pnl_script_";
						my $result_lines_ = `$exec_cmd`;
						print $main_log_file_handle_ "$exec_cmd\n";
						print $main_log_file_handle_ $result_lines_."\n";
					...
					{
					    print $main_log_file_handle_ "SIMPNL line now needs 5 words. Ignoring ".join ( ' ', $scalar_ref_ )."\n";
					}
				    }
				    default
				    {
...
sub AddModelFileToList
{
	...
	if ( exists $duration_to_model_filevec_{$this_pred_duration_} )
	{
	    ...
	    if ( $found_ == 0 )
	    {
		push ( @$scalar_ref_, $this_model_filename_ ); # unique push
	    }
	}
	else
	{ # initialization
...
sub GetStrategyFileForPnlScript
{    
    my ($this_pred_duration_, $this_model_filename_ ) = @_;    

    my $strategy_progid_pnl_script_ = 1001;
    my $this_strategy_filename_ = "";
    if ( exists $duration_to_param_filevec_{$this_pred_duration_} )
    { # if at least one paramfile was specified for that duration

        my $scalar_ref_param_filevec_ = $duration_to_param_filevec_{$this_pred_duration_};
	my $this_param_file_index_ = int ( rand (1) * $#$scalar_ref_param_filevec_ );
	
	my $this_param_filename_ = $$scalar_ref_param_filevec_[$this_param_file_index_];
	
        $this_strategy_filename_ = NameStrategyFromModelAndParamIndex ( $name_, $this_model_filename_, $this_param_file_index_, $trading_start_hhmm_, $trading_end_hhmm_ );
        if ( $author_ && $author_ eq "dvctrader_p" )
        {
             my $temp_str_ = basename ( $this_param_filename_ );
             $this_strategy_filename_ = NameStrategyFromModelAndParamIndex ( $name_, $this_model_filename_, $temp_str_ , $trading_start_hhmm_, $trading_end_hhmm_ ) ;
        }
        {
             my $exec_cmd_ = "";
             $exec_cmd_ = "$MODELSCRIPTS_DIR/create_strategy_file.pl $this_strategy_filename_ $shortcode_ $strategyname_ $this_model_filename_ $this_param_filename_ $trading_start_hhmm_ $trading_end_hhmm_ $strategy_progid_pnl_script_";

             print $main_log_file_handle_ "$exec_cmd_\n";

             `$exec_cmd_`;

             $strategy_progid_pnl_script_++;
        }
     ...
     print $main_log_file_handle_ "StrategyFile in function ".$this_strategy_filename_."\n";
     return $this_strategy_filename_;
}
	...
	{ # foreach model_file generated for this duration
		...
		foreach my $this_model_filename_ ( CreateSubModelFiles ( $$scalar_ref_model_filevec_[$model_filevec_index_], "TODAY-10", "TODAY-5", $datagen_start_hhmm_, $datagen_end_hhmm_ ) )
		{
			...
			{ # foreach param_file input for this duration
				...
				{
				    $exec_cmd_ = "$MODELSCRIPTS_DIR/create_strategy_file.pl $this_strategy_filename_ $shortcode_ $strategyname_ $this_model_filename_ $this_param_filename_ $trading_start_hhmm_ $trading_end_hhmm_ $strategy_progid_ ";
				}
				else
				{
				    $exec_cmd_ = "$MODELSCRIPTS_DIR/create_strategy_file.pl $this_strategy_filename_ $shortcode_ $strategyname_ $this_model_filename_ $this_param_filename_ $trading_start_hhmm_ $trading_end_hhmm_ $strategy_progid_ $traded_ezone_";
				}
				print $main_log_file_handle_ "$exec_cmd_\n";
				`$exec_cmd_`;
				$strategy_progid_++;
...
sub RunSimulationOnCandidates
{
	...
	if ( FindItemFromVec ( $tradingdate_ , @trading_exclude_days_ ) )
	{
        ...
        if ( $use_insample_days_for_trading_ == 0 && FindItemFromVec ( $tradingdate_, @datagen_day_vec_ ) )
        {
            print $main_log_file_handle_ "prune out $tradingdate_ because present in datagen_day_vec \n";
            $tradingdate_ = CalcPrevWorkingDateMult ( $tradingdate_, 1 );
            next;
        }
...
#		close TSCF;
		...
		my %unique_id_to_pnlstats_map_=(); # stored to write extended results to the database
		{
		    ...
		    my $exec_cmd="$LIVE_BIN_DIR/sim_strategy SIM $temp_strategy_cat_file_ $unique_gsm_id_ $tradingdate_ $market_model_index_ ADD_DBG_CODE -1 2>$sim_strat_cerr_file_"; # using hyper optimistic market_model_index, added nologs argument
...
sub SummarizeLocalResultsAndChoose
{
	...
	## support for dynamic cutoffs based on current pool
	if ( $use_dynamic_cutoffs_ )
	{
	    ...
	    my $cut_off_rank_ = min(int($percentile_best_ * $num_strats_in_global_results_ ), 150);
	    ...
	    my $exec_cmd="$LIVE_BIN_DIR/summarize_strategy_results $shortcode_ $cstempfile_ $global_results_dir_path $trading_start_yyyymmdd_ $trading_end_yyyymmdd_ INVALIDFILE $historical_sort_algo_";
	    print $main_log_file_handle_ $exec_cmd."\n";
	    ...
	    if ( $#ttc_vec < $cut_off_rank_ ) { 
		$mail_body_ .= "Hey not sure how, but your cut-off($cut_off_rank_) is greater than arraysize($#ttc_vec), So skipping dynamic setting.\n" ;
	    }
	    if ( $#ttc_vec >= $cut_off_rank_ && $ttc_vec[$cut_off_rank_] ne "" && $ttc_vec[$cut_off_rank_] > $max_ttc_to_allow_ )
	    {
		$mail_body_ .= "Max ttc reset from $max_ttc_to_allow_ ";
		$max_ttc_to_allow_ =  $ttc_vec[$cut_off_rank_] ;
		$mail_body_ .= "to $max_ttc_to_allow_\n";
	    }
	    if ( $#vol_vec >= $cut_off_rank_ && $vol_vec[$cut_off_rank_] ne "" && $vol_vec[$cut_off_rank_] < $min_volume_to_allow_ )
	    {
		$mail_body_ .= "Min vol reset from $min_volume_to_allow_ ";
		$min_volume_to_allow_ = $vol_vec[$cut_off_rank_] ;
		$mail_body_ .= "to $min_volume_to_allow_\n";
	    }
	    if ( $#ppc_vec >= $cut_off_rank_ && $ppc_vec[$cut_off_rank_] ne "" && $ppc_vec[$cut_off_rank_] < $min_pnl_per_contract_to_allow_ )
	    {
		$mail_body_ .= "Min ppc reset from $min_pnl_per_contract_to_allow_ ";
		$min_pnl_per_contract_to_allow_ = $ppc_vec[$cut_off_rank_] ;
	...
	my @best_strat_statistics=`$LIVE_BIN_DIR/summarize_local_results_dir_and_choose_by_algo $historical_sort_algo_ 1 $min_num_files_to_choose_ $min_pnl_per_contract_to_allow_ $min_volume_to_allow_ $max_ttc_to_allow_ $max_message_count_ $local_results_base_dir`;
# debug: 
	my @EARTH_best_strat_statistics=`$LIVE_BIN_DIR/summarize_local_results_dir_and_choose_by_algo $historical_sort_algo_ 500 500 $min_pnl_per_contract_to_allow_ $min_volume_to_allow_ $max_ttc_to_allow_ $max_message_count_ $local_results_base_dir | sed -n '/EARTH/, /STATISTICS/ {p}' | sed '/STATISTICS/q;'`;
	print $main_log_file_handle_ "$exec_cmd\n";
