...
# start
my $USAGE="$0 shortcode timeperiod basepx strategyname param_file_with_permutations start_date end_date strategy_file_name [ regime_index ] [min_volume] [max_ttc] [sort_algo]";
...
sub ProcessListArgs
{
    ...
    if ( -e $strategy_file_name_ )
    {
      push ( @filtered_strats_in_dir_ , $strategy_file_name_ );
    }
    else
    {
    ...
    my @t_model_filevec_ = ();
    my $param_file_list_ = "INVALIDFILE" ;
	...
	my ( $t_model_filename_, $t_param_filename_ ) = GetModelAndParamFileNames ( $t_strategy_filename_ ) ;       
        $param_file_list_ = $t_param_filename_;
...
#params
    ...
    {
        @param_filevec_ = ParallelPermuteParamsRegime ( $regime_index_, $param_file_list_, $orig_param_filename_, $local_params_dir_ );
    }
...
sub RunSimulationOnCandidates
{
	...
	if ( ExistsWithSize ( $this_trades_filename_ ) )
	{
	    my $exec_cmd_ = $MODELSCRIPTS_DIR."/get_pnl_stats_2.pl $this_trades_filename_";
            if ( $strategyname_ eq "NikPricePairBasedAggressiveTrading" )
            {
                $exec_cmd_ = $MODELSCRIPTS_DIR."/get_pnl_stats_stir_2.pl $this_trades_filename_";
            }
