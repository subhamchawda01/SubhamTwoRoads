...
TestPerformance <- function ( strat_filename, num_strats, list_of_dates )
{
	...
	{
            sim_result_lines <- system (sprintf('~/basetrade_install/bin/sim_strategy SIM %s 22222 %s ADD_DBG_CODE -1 2>/dev/null',strat_filename,trading_date), intern = TRUE );
...
min_price_increment <- as.numeric( system( sprintf ( "~/basetrade_install/bin/get_min_price_increment %s %s", shortcode, start_date ), intern = TRUE ) );
...
TestPerformance ( strat_file, 1, list_of_outsample_days );
