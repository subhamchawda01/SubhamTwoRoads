...
my $MODELING_MRT_STRATS_DIR=$MODELING_BASE_DIR."/mrt_strats"; # this directory is used to store the chosen strategy files
...
# start 
my $USAGE="$0 shortcode timeperiod [TILL_DATE=TODAY-1] [NUM_PAST_DAYS=from_file] [IGNORE_INSAMPLE 0/1]";
...
my $ignore_insample_ = 0;
if ( $#ARGV >= 4 ) { $ignore_insample_ = $ARGV[4]; }
...
my $outsample_barrier_yyyymmdd_ = CalcPrevWorkingDateMult ( $trading_end_yyyymmdd_, ($num_days_past_ * 2 / 3) ) ;

my @all_strats_in_dir_;
if ( $shortcode_ eq "EQIALL") {
    @all_strats_in_dir_ = MakeStratVecFromDirInTpExcludingSets ( $MODELING_MRT_STRATS_DIR."/".$timeperiod_, "strats", @exclude_tp_dirs_ );
} else {
...
# if you detect a duplicate entry then do not add the next one and print
    ...
    if ( $ignore_insample_ > 0 || $last_insample_date_ <= $outsample_barrier_yyyymmdd_ ) #ignoring partial outsample check
...

{
    ...
    my $exec_cmd="$LIVE_BIN_DIR/summarize_strategy_results $shortcode_ $cstempfile_ $GLOBALRESULTSDBDIR $trading_start_yyyymmdd_ $trading_end_yyyymmdd_ INVALIDFILE 5";
    my @ssr_output_ = `$exec_cmd`;
    chomp ( @ssr_output_ );

    printf ( "STRATEGYFILEBASE  _fn_  pnl  pnl_stdev  volume  pnl_shrp  pnl_cons  pnl_median  ttc norm_ttc  min_adj_pnl PPT  S B A I MAXDD DD_ADJ_PNL ABS_POS MSG_COUNT GPR PNLBYMAXLOSS .. .. .. \n" );
    print join ( "\n", @ssr_output_ )."\n\n";
    `rm -f $cstempfile_`;
}
