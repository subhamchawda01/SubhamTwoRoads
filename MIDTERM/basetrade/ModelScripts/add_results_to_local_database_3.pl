...
my @param_filevec_ = ();
my @param_id_ = ();
open STRATEGYLISTFILE, "< $strategylistfile_" or PrintStacktraceAndDie ( "add_results_to_local_database_3.pl could not open strategylistfile $strategylistfile_\n" );
while ( my $thisstrategyfile_ = <STRATEGYLISTFILE> ) 
{
    ...
    if ( $#stratwords_ >= 0 )
    {
        ...
        my $this_param_id_ = $stratwords_[7];
        chomp($this_param_id_);
        my $this_param_basename_ = `basename $stratwords_[4]`; chomp ( $this_param_basename_ );
        push ( @param_filevec_, $this_param_basename_ ) ;
        push ( @param_id_, $this_param_id_ ) ;
    }
}
...
my $cnt = 0;
foreach my $strat_line(@param_filevec_)
{
	my $s_id_ =  $param_id_[ $cnt ] ;
	$cnt ++;
