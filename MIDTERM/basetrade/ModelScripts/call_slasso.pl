...
if ( ExistsWithSize ( $filtered_reg_data_filename_ ) )
{
	...
	#my $exec_cmd_ = "cat $filtered_reg_data_filename_ |  awk -v r=$downsample_ratio_ '{ if ( ( NR % r) == 0 ) { print \$0 }}' > $reduced_reg_data_filename_" ;#terribly inefficent way to yank rth line from file
	...
	`$exec_cmd_`;
	$exec_cmd_ = "$MODELSCRIPTS_DIR/build_slasso_model.R $final_reg_data_filename_ $max_model_size_ $regression_output_filename_ $hash_correlations_filename_"; 
    }
    else
    {
	$exec_cmd_ = "$MODELSCRIPTS_DIR/build_slasso_model.R $final_reg_data_filename_ $max_model_size_ $regression_output_filename_";
    }
