...
my $date_ = `date +\%Y\%m\%d`;
$date_ = `date -d @\`head -n1 $tradesfilename_ | awk '{ print \$1}'\` +\%Y\%m\%d 2>/dev/null`;
chomp($date_);
...
while ( my $inline_ = <PNLFILEHANDLE2> )
{
    ...
    if ( ( $#pnlwords_ >= 20 ) && ! ($pnlwords_[0] eq "STATS:") && ! ($pnlwords_[0] eq "SIMRESULT" ) && ! ($pnlwords_[0] eq "PNLSAMPLES" ) ) 
    {
	...
	if ( ! ( exists $shcname_to_indfilehandle_map_{$numbered_shcname_} ) )
	{
	    ...
	    $shcname_to_prev_iv_map_{$numbered_shcname_} = 0;	    
	...
	$shcname_to_total_pnl_map_{$numbered_shcname_} += $shcname_to_prev_position_map_{$numbered_shcname_} *
	    ($pnlwords_[25] - $shcname_to_prev_iv_map_{$numbered_shcname_}) *
	    ($pnlwords_[22]) * $n2d_;
	...
	# common across
	$pnlwords_[4] = abs($pnlwords_[22] * $pnlwords_[4]);	


	#changing other two items to run pnl_stats scripts
	$pnlwords_[6] = $pnlwords_[22] * $pnlwords_[6];
	...
	#changing other two
	$pnlwords_[6] += $underlying_to_prev_position_map_{$numbered_underlying_} - $shcname_to_prev_iv_map_{$numbered_shcname_}* $shcname_to_prev_position_map_{$numbered_shcname_};
	...
	$shcname_to_prev_iv_map_{$numbered_shcname_} = $pnlwords_[25];
...
sub StatsForI    
{
	...
	print join(' ', $number_, $shc_."_VEGA", $stats2_[0], int($stats2_[1]/$shcname_to_lotsize_map_{$numbered_shcname_}*100)/100, $shc_2_simresult_struct_{$numbered_shcname_}, $inline1_);
	print "\n";
    }
}
...
sub StatsForU
{
	...
	my @stats2_ = split(' ', $inline2_);
	print join(' ', $number_, $shc_."_VEGA", $stats2_[0], int($stats2_[1]/$underlying_to_lotsize_map_{$numbered_underlying_}*100)/100, $underlying_2_simresult_struct_{$numbered_underlying_}, $inline1_);
	print "\n";
    }
}
...
sub StatsForA
{
	...
	my @stats2_ = split(' ', $inline2_);
	print join(' ', $number_, "ALL_VEGA", $stats2_[0], $stats2_[1], $id_2_simresult_struct_{$number_}, $inline1_);
	print "\n";
    }
}
