...
data <- as.matrix(data);
num_deps <- as.numeric(args[2]);
num_deps_plus1 <- num_deps + 1;
x <- data[,num_deps_plus1:ncol(data)]; 
y <- data[,1:num_deps];
remove(data);
...
# using lasso penalty
selected_indicators <- rep(-1,num_deps*as.numeric(args[3]));
selected_indicators_weights <- rep(-1,num_deps*as.numeric(args[3]));
ind_n_weights_all <- cbind(selected_indicators, selected_indicators_weights);
#ncol(x);
for (i in 1:num_deps)
{
	y_cap <- y[,i];
initial_corrs <- cor(y_cap,x);
...
while(flag==TRUE)
{
	fit <- cv.glmnet(x,y_cap,family="gaussian",alpha=1,intercept=FALSE,exclude=excl); 
	#print(fit);
	...
	error <-  fit$cvm [indx] ;
	selected_indicators_tmp <- which(coeffs!=0) ;
	selected_indicators_weights_tmp <- coeffs[ which(coeffs!=0 ) ] ;
...
ind_n_weights_tmp <- cbind ( selected_indicators_tmp, selected_indicators_weights_tmp );
for (j in 1:nrow(ind_n_weights_tmp))
{
for (k in 1:nrow(ind_n_weights_all))
{
if ( ind_n_weights_all[k,1] == -1 )
{
ind_n_weights_all[k,1] = ind_n_weights_tmp[j,1];
ind_n_weights_all[k,2] = ind_n_weights_tmp[j,2]/sd(y_cap);
break;
} else if ( ind_n_weights_all[k,1] == ind_n_weights_tmp[j,1] )
{
if ( abs(ind_n_weights_tmp[j,2])/sd(y_cap) > abs(ind_n_weights_all[k,2]) ) {
ind_n_weights_all[k,2] = ind_n_weights_tmp[j,2]/sd(y_cap) ;
}
break;
}
...
#print(ind_n_weights_tmp);
#print(ind_n_weights_all);
}
ind_n_weights_all <- ind_n_weights_all[ ind_n_weights_all[,1]!=-1,];
ind_n_weights_all <- ind_n_weights_all[ order(-abs(ind_n_weights_all[,2])), ];
ind_n_weights_all <- ind_n_weights_all[ 1:as.numeric(args[3]), ];
# print classification error
#print(ind_n_weights_all);
#print(error);
#print(min(fit$cvm)) ;
write.table(ind_n_weights_all, args[4],col.names=F,row.names=F);
