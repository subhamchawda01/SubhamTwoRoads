...
# \file  ModelScripts/find_best_model_for_strategy_var_pert_single_indicator.pl
#
...
my $USAGE="$0 original_strat_file input_strat_file output_model last_trading_date num_prev_days change_fraction max_number_iterations [algo=gradalpha3] [D] [sortalgo] [min_volume] [max_ttc] [num_perturbations=2] [retain_more_pert(0|1)=0] [training_period_fraction_=0.7] [use_fake_faster_data_]";
if ( $#ARGV < 6 ) { print $USAGE."\n"; exit ( 0 ); }

my $original_strat_file_ =  $ARGV [ 0 ];
my $strat_file_ = $ARGV [ 1 ];
my $output_model_filename_ = $ARGV[2];
my $last_trading_date_ = GetIsoDateFromStrMin1 ( $ARGV[3] ) ;
my $num_days_ = max ( 1, int($ARGV [ 4 ]) );
my $change_ = $ARGV [ 5 ];
my $max_number_of_iterations_ = max ( 1, int($ARGV [ 6 ]) );
my $calc_weights_algo_ = "gradalpha3"; 
if ( $#ARGV >= 7 ) { $calc_weights_algo_ = $ARGV[7]; } # BEST / GRAD
...
if ( $#ARGV >= 8 )
{
    if ( index ( $ARGV [ 8 ] , "D" ) != -1 ) { $DEBUG = 1; print "DEBUG MODE\n"; }
}
...
{
    $use_volume_bound_=1;
    $volume_min_train_=int($ARGV[10]);
}
if ( $#ARGV >=11 )
{
    $use_ttc_bound_=1;
    $ttc_max_train_=int($ARGV[11]);
}

if ( $#ARGV >=12 )
{
    $num_perturbations_=int($ARGV[12]);
}

if ( $#ARGV >=13 )
{
    $retain_more_pert_ = int($ARGV[13]);
}

if ( $#ARGV >=14 )
{
    my $t_training_period_fraction_ = ($ARGV[14])*1;
...
if ( $#ARGV >=15 )
{
    $use_fake_faster_data_ = int($ARGV[15]);
}
...
#$main_log_file_handle_->printf ( "Orig on training : %d %d %d\n", $orig_pnl_, $orig_vol_, $orig_ttc_ );
#printf ( "Best on training: %d %d %d\n", $best_seen_pnl_, $best_seen_vol_, $best_seen_ttc_ );
#$main_log_file_handle_->printf ( "Best on training : %d %d %d\n", $best_seen_pnl_, $best_seen_vol_, $best_seen_ttc_ );

#my @orig_indicator_weight_vec_not_added_ = @orig_indicator_weight_vec_ ;
#if($#orig_indicator_weight_vec_not_added_ >= 0)
#{
#	$orig_indicator_weight_vec_not_added_[$#orig_indicator_weight_vec_not_added_] = 0.0;
#}
...
#push (@indicator_weight_vec_vec_crossval_, \@orig_indicator_weight_vec_not_added_);
#push (@indicator_weight_vec_vec_crossval_, \@best_seen_indicator_weight_vec_);
#CalcPnls (\@indicator_weight_vec_vec_crossval_, \@stats_vec_crossval_, \@volume_vec_crossval_, \@ttc_vec_crossval_, $iter_count_, \@ttc_vol_check_crossval_, "insample", \@training_date_vec_);
...
my $t_out_strat_file_ = $work_dir_."/w_opt_strat";;
my $exec_cmd_ = "cat $original_strat_file_ | awk -vnmf=$output_model_filename_ '{\$4 = nmf; print}' > $t_out_strat_file_";
`$exec_cmd_`;
push(@strat_list_, $t_out_strat_file_);

#TRAINING STATS 
my @score_vec_train_ = ();
my @vol_vec_train_ = ();
my @ttc_vec_train_ = ();
my $stats_string_train_ = CalcScores( \@strat_list_, \@score_vec_train_, \@vol_vec_train_, \@ttc_vec_train_, \@training_date_vec_, "train" );
printf ("Original On Training %d %d %d\n",$score_vec_train_[0], $vol_vec_train_[0], $ttc_vec_train_[0]);
$main_log_file_handle_->printf ("Original On Training: %d %d %d\n",$score_vec_train_[0], $vol_vec_train_[0], $ttc_vec_train_[0]);
printf ( "Original with new indicator before pert on Training %d %d %d\n", $orig_pnl_, $orig_vol_, $orig_ttc_ );
$main_log_file_handle_->printf ( "Original with new indicator before pert on Training : %d %d %d\n", $orig_pnl_, $orig_vol_, $orig_ttc_ );
printf ("Best with new indicator on Training %d %d %d\n",$score_vec_train_[1], $vol_vec_train_[1], $ttc_vec_train_[1]);
$main_log_file_handle_->printf ("Best with new indicator on Training: %d %d %d\n",$score_vec_train_[1], $vol_vec_train_[1], $ttc_vec_train_[1]);

#OUTSAMPLE STATS
my @score_vec_outsample_ = ();
my @vol_vec_outsample_ = ();
my @ttc_vec_outsample_ = ();
my $stats_string_outsample_ = CalcScores( \@strat_list_, \@score_vec_outsample_, \@vol_vec_outsample_, \@ttc_vec_outsample_, \@outofsample_date_vec_, "outsample" );
printf ("OriginalOnOutsample %d %d %d\n",$score_vec_outsample_[0], $vol_vec_outsample_[0], $ttc_vec_outsample_[0]);
$main_log_file_handle_->printf ("OriginalOnOutsample %d %d %d\n",$score_vec_outsample_[0], $vol_vec_outsample_[0], $ttc_vec_outsample_[0]);
printf ("BestOnOutsample %d %d %d\n",$score_vec_outsample_[1], $vol_vec_outsample_[1], $ttc_vec_outsample_[1]);
$main_log_file_handle_->printf ("BestOnOutsample %d %d %d\n",$score_vec_outsample_[1], $vol_vec_outsample_[1], $ttc_vec_outsample_[1]);
printf ("OutsamplePerformaceSummary\n%s\n", $stats_string_outsample_);
