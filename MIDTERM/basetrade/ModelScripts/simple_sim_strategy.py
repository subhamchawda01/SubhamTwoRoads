...
def ExecLogic(sumvars_, paramset_):
    ...
    elif info_.exec_logic_type_:
        if (info_.mkt_sizew_price_ + sumvars_ - info_.bid_price_ < paramset_.bid_keep_ and (info_.bid_price_ in info_.bid_price_to_order_.keys())):
            del info_.bid_price_to_order_[info_.bid_price_]
        elif (info_.mkt_sizew_price_ + sumvars_ - info_.bid_price_ >= paramset_.bid_place_ and (info_.bid_price_ not in info_.bid_price_to_order_.keys()) and paramset_.bs_ > 0):
            if info_.dbg_:
                print "SendBidOrder: " + str(info_.bid_price_) + " " + str(info_.uts_)
            SendOrder(info_.bid_price_, info_.uts_, 1)
        if (info_.ask_price_ - sumvars_ - info_.mkt_sizewprice_ < paramset_.ask_keep_ and (info_.ask_price_ in info_.ask_price_to_order_.keys())):
            del info_.ask_price_to_order_[info_.ask_price_]
        elif (info_.ask_price_ - sumvars_ - info_.mkt_sizewprice_ >= paramset_.ask_place_ and (info_.ask_price_ not in info_.ask_price_to_order_.keys()) and paramset_.as_ > 0):
            if info_.dbg_:
                print "SendAskOrder: " + str(info_.ask_price_) + " " + str(info_.uts_) + " sv: " + str(sumvars_) + " " + "thres:" + str(-1 * paramset_.ask_place_)
            SendOrder(info_.ask_price_, info_.uts_, -1)
...
def main(mkt_data_filename_, reg_data_filename_):
    ...
    lines_ = open(mkt_data_filename_, "r")
    reg_data_ = open(reg_data_filename_, "r")
    #min_reg_data_timestamp_ = reg_data_[0][0]

    index_to_begin_from_ = 0  # numpy.searchsorted(lines_[:,0],min_reg_data_timestamp_)
    reg_data_idx_ = 0

    with open(mkt_data_filename_) as file_:
        for line_ in file_:
            line_words_ = line_.split()  # lines_[idx]
            mkt_mfm_ = float(line_words_[0])
            info_.time_ = line_words_[0]
            isTrade = False

            if info_.dbg_:
                print "mkt_mfm_" + str(mkt_mfm_)
            if mkt_mfm_ > mfm_ and not getflat_:
                for regline_ in reg_data_:
                    reg_line_words_ = regline_.split()
                    mfm_ = float(reg_line_words_[0])
                    sumvars_ = float(reg_line_words_[1])
                    if info_.dbg_:
                        print "this mfm_ " + str(mfm_) + " " + str(sumvars_)
                    if (mkt_mfm_ > mfm_):
                        last_mfm_ = mfm_
                        last_sumvars_ = sumvars_
                        continue
                    elif (mkt_mfm_ <= mfm_):
                        break
                    elif (mkt_mfm_ < mfm_ and mkt_mfm_ > last_mfm_):
                        break
                    elif (mkt_mfm_ == mfm_):
                        last_sumvars = sumvars_
                        break
                else:
                    getflat_ = True

            if info_.dbg_:
                print "LINE " + str(line_words_)
            ...
            if (bid_ask_read_ and not getflat_):
                ...
                info_.mkt_sizew_price_ = (info_.bid_price_ * info_.ask_size_ + info_.bid_size_ *
                                          info_.ask_price_) / (float)(info_.ask_size_ + info_.bid_size_)
                info_.mid_price_ = (info_.bid_price_ + info_.ask_price_) / 2.0
                ...
                if (info_.pos_ == 0):
                    ...
                    info_.pnl_ = info_.pnl_ - (info_.pos_ * (info_.bid_price_ * info_.ask_size_ +
                                                             info_.bid_size_ * info_.ask_price_) / (float)(info_.ask_size_ + info_.bid_size_))
                    info_.volume_ = info_.volume_ + abs(info_.pos_)
                    info_.pos_ = 0
                    return
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) >= 7):
        info_.exec_logic_type_ = sys.argv[6]
    ...
    exec_cmd_ = BIN_DIR + "get_contract_specs " + info_.shortcode_ + " " + str(info_.tradingdate_) + " " + "N2D"
    out = subprocess.Popen(exec_cmd_, shell=True, stdout=subprocess.PIPE)
    n2d_ = out.communicate()[0].strip().split()[1]
    print "SIMRESULT " + str(-1 * info_.total_pnl_ * float(n2d_)) + " " + str(info_.volume_) + " 0 0 0 0 "
