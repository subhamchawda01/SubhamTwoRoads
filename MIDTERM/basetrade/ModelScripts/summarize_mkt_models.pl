...
# \file ModelScripts/summarize_mkt_models.pl
#
...
# start 
my $USAGE = "$0 SHORTCODE START_DATE END_DATE FBMMPID [SUMMARIZATION_CRITERA=SFINALPNL]";
...
my $work_dir_ = $FBMMP_WORK_DIR.$fbscp_id_;
...
sub DisplaySummarizedResults
{
    my $t_head_ = sprintf ( "%60s %9s %9s %9s %9s %9s %9s %9s %9s %10s %10s\n" , 
			    "MKTMODEL" , "PNLMEAN" , "PNLMEDIAN" , "PNLSTDEV" , "FINALPNL" ,
			    "VOLMEAN" , "VOLMEDIAN" , "VOLSTDEV" , "TRADES" , "SPNLMEDIAN" , "SFINALPNL" );
