...
# start
my $USAGE="$0 shortcode timeperiod basepx strategyname param_file_with_permutations start_date end_date";

sub GetPriceTypeFromCode 
{
    my $px_code_ = shift;
    my @StandardPxTypes = ( "Midprice", "MktSizeWPrice", "MktSinusoidal", "OrderWPrice", "OfflineMixMMS" , "ALL" );
...
print "$shortcode_ $timeperiod_ $basepx_pxtype_ $strategyname_\n";
...
sub RunSimulationOnCandidates
{
	...
	else 
	{
	    ...
	    # run sim_strategy, store output in @trade_init_output_lines_
	    # print lines in @trade_init_output_lines_ that have the word "SIMRESULT" into $temp_results_list_file_.
...
#		close TSCF;
		...
		my @trade_init_output_lines_=(); # stored to seive out the SIMRESULT lines
		my %unique_id_to_pnlstats_map_=(); # stored to write extended results to the database
		{
		    ...
		    print $main_log_file_handle_ "$exec_cmd\n";
		    @trade_init_output_lines_=`$exec_cmd`;
		...
		for ( my $t_trade_init_output_lines_index_ = 0, my $psindex_ = 0; $t_trade_init_output_lines_index_ <= $#trade_init_output_lines_; $t_trade_init_output_lines_index_ ++ )
		{
		    if ( $trade_init_output_lines_[$t_trade_init_output_lines_index_] =~ /SIMRESULT/ )
		    { # SIMRESULT pnl volume
			my @rwords_ = split ( ' ', $trade_init_output_lines_[$t_trade_init_output_lines_index_] );
...
#			    print STDERR $unique_id_to_pnlstats_map_{$unique_sim_id_}."\n";
