...
# \file ModelScripts/select_rows_with_dep_lt_max.pl
#
...
#
# This script takes :
# R_REG_DATA_INPUT_FILENAME  MAX_STDEV_MULTIPLIER  RW_REG_DATA_OUTPUT_FILENAME
...
# start 
my $USAGE="$0  R_REG_DATA_INPUT_FILENAME  MAX_STDEV_MULTIPLIER  RW_REG_DATA_OUTPUT_FILENAME [NUM_COLS = 1]";

if ( $#ARGV < 2 ) { print $USAGE."\n"; exit ( 0 ); }
my $r_reg_data_input_filename_ = $ARGV[0];
my $max_stdev_multiplier_ = $ARGV[1];
...
while ( my $inline_ = <INFILE> ) 
{
	...
	{ # online computation of mean and stdev
	    ...
	    my $var_dep_ = max ( 0, ( ( $l2_dep_sum_[$i] - ( $l1_dep_sum_[$i] * $l1_dep_sum_[$i] / $l0_dep_sum_[$i] ) ) / ($l0_dep_sum_[$i] - 1) ) ) ;
	    if ( $var_dep_ > 0.00000000000001 )
	    {
		my $stdev_dep_ = sqrt ( $var_dep_ ) ;
		my $abs_dev_px_ = abs ( $words_[$i] - $mean_dep_ );
		if ( $abs_dev_px_ > ( $max_stdev_multiplier_ * $stdev_dep_ ) )
