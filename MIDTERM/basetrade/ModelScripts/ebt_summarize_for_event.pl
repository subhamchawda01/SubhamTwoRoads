...
if ($strat_filename_ eq "POOL"){
  my $list_strat_command_="ls /home/dvctrader/modelling/strats/*/EBT/$ezone_string_/* |sort |xargs -I % basename %";
  @strategies_=`$list_strat_command_`;
  chomp(@strategies_);
}
elsif($strat_filename_ eq "STAGED"){
    my $list_strat_command_="ls /home/dvctrader/modelling/staged_strats/*/EBT/$ezone_string_/* |sort |xargs -I % basename %";
    @strategies_=`$list_strat_command_`;
    chomp(@strategies_);
...
foreach my $index_ (@enumeration)
{
    ...
    my $shortcode_=`awk '{print \$2}' $full_strat_file_path_`;
    chomp($shortcode_);
