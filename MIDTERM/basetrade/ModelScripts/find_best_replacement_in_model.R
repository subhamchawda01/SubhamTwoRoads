...
TestPerformanceOriginal <- function ( strat_filename, list_of_dates )
{
	...
	cat ( sprintf ( "perl %s %s %s %s %s %s %s\n" , run_sim_script_ , shortcode , input_strat_list_filename_ , output_results_list_filename_ , tradingdate_list_filename_ , work_dir , use_fake_faster_data_) );
	system ( sprintf ( "perl %s %s %s %s %s %s %s" , run_sim_script_ , shortcode , input_strat_list_filename_ , output_results_list_filename_ , tradingdate_list_filename_ , work_dir , use_fake_faster_data_) );

	for ( trading_date in list_of_dates )
	{
#	   cat ( sprintf ( "grep \"%s %s \" %s | awk '{ print $NF; }'\n" , strat_filename , trading_date , output_results_list_filename_ ) );
	   output_results_file_ <- system ( sprintf ( "grep \"%s %s \" %s | awk '{ print $NF; }'\n" , strat_filename , trading_date , output_results_list_filename_ ) , intern = TRUE , ignore.stderr = FALSE );

#	   cat ( sprintf ( "cat %s\n" , output_results_file_ ) );
	   sim_result_lines <- system ( sprintf ( "cat %s" , output_results_file_ ) , intern = TRUE , ignore.stderr = FALSE );

	   if ( length ( sim_result_lines ) >= 1 )
	   {
		sim_result_words <- unlist(strsplit(sim_result_lines, "\\ " ));
...
GetDataForRegression <- function ()
{
	...
	run_datagen_script <- paste ( MODELSCRIPTS_DIR , "run_datagen_and_generate_reg_data.pl" , sep = "" );
	cat ( sprintf ( "perl %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n" ,
			...
			pred_duration , pred_algo , catted_regdata_filename , work_dir , use_fake_faster_data_, sampling_shortcode_str_ ) );
...
if ( length(args) < 17 ) {
        stop ("USAGE : <script> <shortcode> <strat_file> <model_file> <ilist> <start_date> <num_days_lookback> <start_time> <end_time> <dgen_msecs> <dgen_l1events> <dgen_trades> <to_print_on_eco> <pred_duration> <pred_algo> <filter> <work_dir> <output_aim_file> [use_fake_faster_data_(0/1) = 1] [sampling_shortcode_string]\n");
}
...
use_fake_faster_data_ <- 1;
if (length(args)>=18)
{
    use_fake_faster_data_ <- args[18]
}
if (length(args)>19)
{
    for(i in c(19:length(args)))
    {
         sampling_shortcode_[i-18] <- args[i];
    }
...
num_outsample_days <- 0.01 * num_days_lookback;
