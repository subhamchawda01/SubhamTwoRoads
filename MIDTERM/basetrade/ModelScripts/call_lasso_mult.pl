...
# start 
my $USAGE="$0  filtered_reg_data_filename_ num_dependents_ max_model_size_ regression_output_filename_ [indicator_list_filename_ (if checking for hash correlations)]";

if ( $#ARGV < 3 ) { print $USAGE."\n"; exit ( 0 ); }
my $filtered_reg_data_filename_ = $ARGV[0];
my $num_dependents_ = $ARGV[1];
...
if ( ExistsWithSize ( $filtered_reg_data_filename_ ) )
{
    ...
    if( $#ARGV > 3 )
    {
	my $indicator_list_filename_ = $ARGV[4];
	...
	`$exec_cmd_`;
	$exec_cmd_ = "$MODELSCRIPTS_DIR/build_lasso_model_multY.R $final_reg_data_filename_ $num_dependents_ $max_model_size_ $regression_output_filename_ $hash_correlations_filename_"; 
    }
    else
    {
	$exec_cmd_ = "$MODELSCRIPTS_DIR/build_lasso_model_multY.R $final_reg_data_filename_ $num_dependents_ $max_model_size_ $regression_output_filename_";
    }
