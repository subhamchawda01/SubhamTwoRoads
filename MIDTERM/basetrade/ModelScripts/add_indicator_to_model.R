...
GetOriginalModelCoeffs <- function ()
{
		...
		beta <- c();
        	...
        	{
                  if ( (substring(model_text[i],1,1) != "#") && grepl(pattern="INDICATOR ", x = model_text[i] ) ) 
                  {
...
if ( length(args) < 17 ) {
        stop ("USAGE : <script> <shortcode> <strat_file> <model_file> <ilist> <start_date> <num_days_lookback> <start_time> <end_time> <dgen_msecs> <dgen_l1events> <dgen_trades> <to_print_on_eco> <pred_duration> <pred_algo> <filter> <work_dir> <output_aim_file> [use_fake_faster_data_(0/1) = 1] [sampling_shortcode_str_]\n");
}
cat (length(args),"\n");
num_models <- 0;
shortcode <- args[1];
