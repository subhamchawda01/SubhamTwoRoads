...
# \file ModelScripts/place_coeffs_in_logit_model_new.pl
#
...
while ( my $regline_ = <REGOUTFILE> )
{
    ...
    my ($this_index_, $this_theta_1_, $this_theta_2_, $this_theta_3_ )=(-1,-1,-1,-1);
    if ( ( $#regwords_ >= 4 ) &&
	 ( $regwords_[0] eq "OutCoeff" ) )
    {
	$this_index_ = int($regwords_[1]);
	$this_theta_1_ = $regwords_[2];
	$this_theta_2_ = $regwords_[3];
	$this_theta_3_ = $regwords_[4];
	
	if ( $this_theta_1_ * $this_theta_2_ * $this_theta_3_ != 0 )
	{
	   my $outline_ = "INDICATOR $this_theta_1_:$this_theta_2_:$this_theta_3_ ".$indicator_trailing_text_[$this_index_]."\n";
	   $ilist_pre_indicator_text_ = $ilist_pre_indicator_text_.$outline_;
	   $model_size_++;
	}
    }
    elsif ( ( $#regwords_ >= 3 ) &&
	 ( $regwords_[0] eq "OutConst" ) )
    {
	$this_theta_1_ = $regwords_[1];
	$this_theta_2_ = $regwords_[2];
	$this_theta_3_ = $regwords_[3];
	
	if ( $this_theta_1_ * $this_theta_2_ * $this_theta_3_ != 0 )
	{
	   my $outline_ = "INTERCEPT $this_theta_1_:$this_theta_2_:$this_theta_3_ "."\n";
	   $ilist_pre_indicator_text_ = $ilist_pre_indicator_text_.$outline_;
	   $model_size_++;
	}
    }
