...
my $start_time_ = `date`; chomp ( $start_time_ ) ;
my $model_index_ = 1 ; 
...
sub ReadConfigFile
{
    ...
    while ( my $thisline_ = <CONFIGFILEHANDLE> )
    {
        ...
        given($current_instruction_)
        {
            ...
            when ( "MODEL_INDEX" )
            {
                $model_index_ = $thisline_words_[0 ];
            }
            default
            {
            }
...
sub RunFindBestModel
{
	...
	`mkdir -p $compare_dir_uninstalled_`; 
        ...
        {
            $exec_cmd_ = $MODELSCRIPTS_DIR."find_best_model_for_strategy_random_dates.pl $this_input_strat_file_ $this_output_model_file_ $last_trading_date_ $num_prev_days_ $change_fraction_ $batch_size_ $debug_string_ $cost_fn_ $use_fake_faster_data_ $model_index_";
        }
        else
        {
            $exec_cmd_ = $MODELSCRIPTS_DIR."find_best_model_for_strategy_var_pert_stir.pl $this_input_strat_file_ $this_output_model_file_ $last_trading_date_ $num_prev_days_ $change_fraction_ $max_number_iterations_ $search_algo_ $debug_string_ $cost_fn_ $min_vol_ $max_ttc_ $num_perturbations_ $retain_more_pert_ $training_day_inclusion_prob_ $training_days_type_ $eco_date_time_file_ $use_fake_faster_data_ $model_index_";
        }
...
sub GetModelFromStrat
{
    ...
    my $strat_out_ = `cat $t_strat_`; chomp ( $strat_out_ ) ;
    my @strat_words_ = split ( ' ', $strat_out_ ) ;
    my $im_strat_ = "no_strat_";
    if ( $#strat_words_ >= 2) 
    {
        $im_strat_ = $strat_words_[1];
    }
    
    my @output_ = `cat $im_strat_`;
    chomp ( @output_ );
    my @strat_output_ = split ( ' ' , $output_ [ $model_index_ ] ); chomp ( @strat_output_ );
    my $t_model_ = $strat_output_ [ 2 ];
