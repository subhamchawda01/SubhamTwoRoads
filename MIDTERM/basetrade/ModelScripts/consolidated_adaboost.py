    ...
    with open(sys.argv[3]) as file:
        for line in file:
            ...
            for i in range(0, min(granularity_level, int(line[0])) - 1):
                granularity_range.append((granularity_begin, granularity_end))
                num_new_entries_ += 1
            ...
            granularity_range.append((granularity_begin, granularity_end))
            num_new_entries_ += 1
            current_begin_ = granularity_end

    file.close()
    Y = np.zeros((num_new_entries_,))
    X = np.zeros((num_new_entries_, t_data_.shape[1] - 1))
    # replace each block with a representatiive element.currently it is the mean
    for idx, period in zip(xrange(num_new_entries_), granularity_range):
        X[idx] = np.mean(t_data_[period[0]:period[1], 1:t_data_.shape[1]], axis=0)
        Y[idx] = np.mean(t_data_[period[0]:period[1], 0], axis=0)
...
def TrainWithConsolidatedBoosting(model, loss_threshold):
    global weights
    ...
    while t_avg_loss_ < loss_threshold and steps < 50:
        ...
        beta.append(t_beta)
        if sys.argv[1] == "CONSOLIDATEDBOOSTING":
...
if len(sys.argv) < 9:
    print "Usage:<script><CONSOLIDATEDTREEBOOSTING/CONSOLIDATEDBOOSTING><regdatafilename><datefile><max_depth/alpha_RR><loss_function><max_indicators_size><modelfilename><granularity(1000 to 10000)>[CVTest][max_allowed_error][statusfile]"
    exit(1)
...
if granularity_level < 500:
    granularity_level = 500
elif granularity_level > 2500:
    granularity_level = 2500

ReadFileAndConsolidate()
...
if sys.argv[1] == "CONSOLIDATEDTREEBOOSTING":
    model = DecisionTreeRegressor(splitter='random', max_depth=int(sys.argv[4]))
    model.fit(X, Y)
    steps = TrainWithConsolidatedBoosting(model, 0.5)
...
if len(sys.argv) > 10:  # evaulate the model
