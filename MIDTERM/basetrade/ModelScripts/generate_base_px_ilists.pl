...
if ( $USER eq "sghosh" || $USER eq "ravi" || $USER eq "kputta" || $USER eq "ankit" )
{
    $LIVE_BIN_DIR = $HOME_DIR."/".$REPO."_install/bin";
}

my $USAGE = "$0 USER BASE-PRICE-1 BASE-PRICE-2 PREDALGO FILTER ILIST_PREFIX SHORTCODE TIMEPERIOD STARTDATE ENDDATE CUTOFFCORR MAKEINDMEASURE NUMINDICATORS DURATION1 DURATION2 DURATION3 ... -e exlucde_ilist [-hv]";
...
my $make_ilist_exec_ = $LIVE_BIN_DIR."/make_indicator_list_extended";
my $filter_corr_file_exec_ = $LIVE_BIN_DIR."/filter_indicators";
...
my @ilist_day_vec_ = ();
if ( int($use_high_volume_days_) == 1 )
{
    GetHighVolumeDaysForShortcode ( $shortcode_, \@ilist_day_vec_ );    

    for ( my $i=0; $i <= $#ilist_day_vec_ ; $i ++ )
    {
	if ( $ilist_day_vec_[$i] >  GetIsoDateFromStrMin1 ( $end_date_ ) || $ilist_day_vec_[$i] < GetIsoDateFromStrMin1 ( $start_date_ ) )
	{
	   $ilist_day_vec_[$i] = -1;
	}	
    }	
...
foreach my $t_file ( @files_to_remove_ )
{
    `rm $t_file` ;
}
...
sub GeneratePrimaryIlists
{
	...
	if ( $filter_ )
	{
	    $indicator_corr_filename_ = "/NAS1/indicatorwork/".$shortcode_."_".$t_pred_duration_."_".$filter_."_".$pred_algo_."_".$timeperiod_."_".$base_px_1_."_".$base_px_2_."/indicator_corr_record_file.txt.gz";
	}
	else
	{
	    $indicator_corr_filename_ = "/NAS1/indicatorwork/".$shortcode_."_".$t_pred_duration_."_".$pred_algo_."_".$timeperiod_."_".$base_px_1_."_".$base_px_2_."/indicator_corr_record_file.txt.gz";
	}
	...
	if ( $count == 0  )
	{
	    ...
	    if ( $filter_ )
	    {
		$t_indicator_corr_filename_ = "/NAS1/indicatorwork/".$shortcode_."_".$t_pred_duration_."_".$filter_."_".$pred_algo_."_".$timeperiod_."_".$base_px_1_."_".$base_px_2_."/indicator_corr_record_file.txt";
	    }
	    else
	    {
		$t_indicator_corr_filename_ = "/NAS1/indicatorwork/".$shortcode_."_".$t_pred_duration_."_".$pred_algo_."_".$timeperiod_."_".$base_px_1_."_".$base_px_2_."/indicator_corr_record_file.txt";
	    }
	...
	{
	    # apply filter _ exclude
	    my $t_indicator_corr_filename_ = "";	    
	    if ( $filter_ )
	    {
		$t_indicator_corr_filename_ = "$this_work_dir_/".$shortcode_."_".$t_pred_duration_."_".$filter_."_".$pred_algo_."_".$timeperiod_."_".$base_px_1_."_".$base_px_2_."_indicator_corr_record_file.txt";
	    }
	    else
	    {
		$t_indicator_corr_filename_ = "$this_work_dir_/".$shortcode_."_".$t_pred_duration_."_".$pred_algo_."_".$timeperiod_."_".$base_px_1_."_".$base_px_2_."_indicator_corr_record_file.txt";
	    }

	    my $f_cmd_ = $filter_corr_file_exec_." ".$shortcode_."_".$timeperiod_." -c ".$indicator_corr_filename_." ".$exclude_filename_." >".$t_indicator_corr_filename_." 2>/dev/null" ;
	    print $f_cmd_."\n";
	    `$f_cmd_`;
	    $indicator_corr_filename_ = $t_indicator_corr_filename_ ;
	    push ( @files_to_remove_ , $t_indicator_corr_filename_ ) ;
	    # end __exclude
	    $exec_cmd_ = $make_ilist_exec_." ".$shortcode_." ".$indicator_corr_filename_." ".$base_px_1_." ".$base_px_2_." ".$start_date_." ".$end_date_." ".$corr_cutoff_." -a ".$measure_;
	}	
	else
	{
	    $exec_cmd_ = $make_ilist_exec_." ".$shortcode_." ".$indicator_corr_filename_." ".$base_px_1_." ".$base_px_2_." ".$start_date_." ".$end_date_." ".$corr_cutoff_." -a ".$measure_;
	...
	if ( $use_high_volume_days_ == 0 && $#ilist_day_vec_ >= 5 ) # atleast 5 days.
	{
	    $exec_cmd_ = $exec_cmd_." -d";
	    for ( my $i = 0 ; $i <= $#ilist_day_vec_;  $i++ )
	    {
		if ( $ilist_day_vec_[$i] > 0 )
		{
		   $exec_cmd_ = $exec_cmd_." ".$ilist_day_vec_[$i];
		}
	    }
	}
	
	$exec_cmd_ = $exec_cmd_." 2>/dev/null";
	...
	if ( $#exec_output_ <  min ( 10 , 0.25 * ( $num_indicators_ ) ) )
	{
	    foreach my $t_file ( @files_to_remove_ )
	    {
		`rm $t_file` ;
