...
#       +91 80 4190 3551
#
# this script is intended to work for type 5 configs however users may use it for other type configs as well

# use strict; # Why has this been commented here?
use strict;
use Getopt::Long;
use Pod::Usage;
use warnings;
...
sub PrintMat ; #prints a matrix (used for debugging)
...
# start
my $USAGE="$0 param_file_with_permutations start_date/dateslist_file end_date/-1 config_name [ distributed = 0 ] [ event_string(INVALID) ] [ regime_index(starts with 1/-1) ] [ skip_days_file(INVALIDFILE) ] [AvgPrice NA 0 16/STDEV NA 0.2 0.3/VOL NA 0.3 HIGH]";
...
##############get the list of days we want to param permute on###############
#
my $is_valid_config_ = `$HOME_DIR/$REPO/walkforward/is_valid_config.py -c $config_name_`; chomp($is_valid_config_);
if($is_valid_config_ < 0 )
{
  print "Seems like the config is not valid exiting...\n"; exit(0);
}

my $startday_strat_ = `$HOME_DIR/$REPO/walkforward/print_strat_from_config.py -c $config_name_ -d $trading_start_yyyymmdd_`; chomp($startday_strat_);
my @startday_strat_arr_ = split ' ', $startday_strat_;
my $trading_start_hhmm_  = $startday_strat_arr_[5];
my $trading_end_hhmm_ = $startday_strat_arr_[6];
my $shortcode_ = $startday_strat_arr_[1];
my $strategyname_ = $startday_strat_arr_[2];
my $t_param_filename_ = $startday_strat_arr_[4];
...
##############read config and fill hash maps##################
# read the config and make the hash maps first
# the key of the map is the date. The idea is any of the strategy constituents can vary 
my %shortcode_map_ = ();
my %strategyname_map_ = ();
my %model_filename_map_ = ();
my %param_filename_map_ = ();
my %trading_start_hhmm_map_ = ();
my %trading_end_hhmm_map_ = ();
my %strategy_line_to_date_map_ = ();
my %strategy_line_to_name_map_ = ();

foreach my $tradingdate_ ( @trading_days_vec_ ) {
  my $thisday_strat_ = `$HOME_DIR/$REPO/walkforward/print_strat_from_config.py -c $config_name_ -d $tradingdate_`; chomp($thisday_strat_);
  my @thisday_strat_arr_ = split ' ', $thisday_strat_;
  $shortcode_map_{$tradingdate_} 	= $thisday_strat_arr_[1];
  $strategyname_map_{$tradingdate_} 	= $thisday_strat_arr_[2];
  $model_filename_map_{$tradingdate_} 	= $thisday_strat_arr_[3];
  $param_filename_map_{$tradingdate_} 	= $thisday_strat_arr_[4];
  $trading_start_hhmm_map_{$tradingdate_}  = $thisday_strat_arr_[5];
  $trading_end_hhmm_map_{$tradingdate_} = $thisday_strat_arr_[6];
}
##############read config and fill hash maps##################

my $traded_ezone_ = GetStratTradedEzone ( $config_name_ );
...
print "$strategyname_ $config_name_ $work_dir_ \n";
...
sub MakeStrategyFiles 
{
  ...
  foreach my $tradingdate_ ( @trading_days_vec_ ) {
    ...
    print $main_log_file_handle_ "This date = $tradingdate_\n";    

    my $strategy_progid_ = 1001;
    my $this_shortcode_ 	= $shortcode_map_{$tradingdate_};
    my $this_strategyname_ 	= $strategyname_map_{$tradingdate_};
    my $this_model_filename_ 	= $model_filename_map_{$tradingdate_};
    my $this_trading_start_hhmm_= $trading_start_hhmm_map_{$tradingdate_};
    my $this_trading_end_hhmm_ 	= $trading_end_hhmm_map_{$tradingdate_};
    ...
    my @this_date_strategy_filevec_ = ();
...
#Copy param file to a shared location (for distributed runs)
      ...
      `mkdir -p $local_strats_dir_/$tradingdate_`;
      my $this_strategy_filename_ = $local_strats_dir_."/$tradingdate_/".$this_strategy_filebase_;

      my $exec_cmd = "$MODELSCRIPTS_DIR/create_strategy_file.pl $this_strategy_filename_ $this_shortcode_ $this_strategyname_ $this_model_filename_ $this_param_filename_ $this_trading_start_hhmm_ $this_trading_end_hhmm_ $strategy_progid_";
...
# uniqueness of progid ensures that we can run them in sim together
      ...
      if ( ExistsWithSize (  $this_strategy_filename_ ) ) {
        push ( @this_date_strategy_filevec_, $this_strategy_filename_ ); 
        $strat2param_ { $this_strategy_filebase_ } = $this_param_filename_ ;
        
        my $strat_line = `cat $this_strategy_filename_ | head -n1`; chomp($strat_line);
        $strategy_line_to_name_map_{$strat_line} = $this_strategy_filebase_;
        push(@{$strategy_line_to_date_map_{$strat_line}}, $tradingdate_);
      }
     
    }
    push ( @strategy_filevec_, \@this_date_strategy_filevec_ ); 
  }
}
...
sub RunSimulationOnCandidates
{
  ...
  if ( $using_grid_){
  	my $artifacts_dir = ComputePnlGridStratDateVec($work_dir_, \%strategy_line_to_date_map_, \%strategy_line_to_name_map_);
  	CopyFromArtifactsToLocalDir($artifacts_dir, $local_results_base_dir);
...
# independent from each other and can be safely run in parallel.
  ...
  my $tradingdate_ = $trading_end_yyyymmdd_;
  my $strategy_mat_ = \@strategy_filevec_;
  for ( my $index_ = 0; $index_ <= $#trading_days_vec_; $index_ ++ ) {
    my $tradingdate_ = $trading_days_vec_[$index_];

    next if ( $tradingdate_ ~~ @skip_days_ );

    my $temp_strategy_list_file_index_ = 0;
    my $this_date_stratarr_ref_ = $$strategy_mat_[$index_];  
    for ( my $strategy_filevec_index_ = 0 ; $strategy_filevec_index_ <= $#$this_date_stratarr_ref_ ; ) {
      ...
      for ( my $num_files_ = 0 ; $num_files_ < $MAX_STRAT_FILES_IN_ONE_SIM && $strategy_filevec_index_ <= $#$this_date_stratarr_ref_ ; $num_files_ ++ ) {
        my $this_strategy_filename_ = $$this_date_stratarr_ref_ [ $strategy_filevec_index_ ];
        #print "$this_strategy_filename_\n";
...
# Get a concurrency safe id.
      ...
      if ( ! $distributed_ ) {
        ...
        $exec_cmd_ = "$DISTRIBUTED_EXECUTOR $max_sim_timeout $temp_strategy_output_file_ $celery_channel_name $shortcode_ bash -c \"sim_output=\\`$LIVE_BIN_DIR/sim_strategy SIM $temporary_strat_location $unique_sim_id_ $tradingdate_ $market_model_index_ ADD_DBG_CODE -1 2>&1 | tr '\\n' '\\002'\\`; pnl_stats=\\`$MODELSCRIPTS_DIR/get_pnl_stats_2.pl $this_trades_filename_ 2>&1 | sed 's/^/PNLSTATS /g' | tr '\\n' '\\002'\\`; echo -n \\\$sim_output\\\$'\\n'\\\$pnl_stats | tr '\\002' '\\n'; \"";
