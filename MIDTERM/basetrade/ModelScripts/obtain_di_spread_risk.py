        ...
        if open_position_ == 0:
            ...
            for i in range(len(legs)):
                for j in range(i + 1, len(legs)):
                    sp_pnl_.append(abs(element[j] * n2d_[j] - last_prices_[j] - dv01_[j] /
                                       dv01_[i] * (element[i] * n2d_[i] - last_prices_[i])))
...
for i in range(len(legs)):
    for j in range(i + 1, len(legs)):
        print legs[j] + "-" + legs[i], np.mean(sp_series_for_analysis[idx_])
        idx_ = idx_ + 1
