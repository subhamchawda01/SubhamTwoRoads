...
#  }
  ...
  my $exec_cmd="$LIVE_BIN_DIR/summarize_strategy_results \"\"  $temp_stratlist_filename_ $results_dir_ $trading_start_yyyymmdd_ $trading_end_yyyymmdd_ $skip_dates_file_ $sort_algo_ $max_loss_per_uts_ $specific_dates_file_ $shorter_output_";
...
# start 
my $USAGE="$0 shortcode/ALL strat_dir results_dir start_date end_date [skip_dates_file or INVALIDFILE] [sortalgo=kCNAPnlAdjAverage] [MaxLossPerUts(0 no_maxloss|-ve param.maxloss)=0] [specific_dates_file or specific_dates_with_weights_file_ or INVALIDFILE] [shorter_output=1] [all_shc_=0] [stats_mode_ = 0/1/2]";
...
my @all_strats_in_dir_ = MakeStratVecFromDirectory($strat_dir_);
...
if(!$all_shc_) {
  ...
  print "SHORTCODE PNL STDEV VOL SHARPE PNL_CONSERVATIVE PNL_MEDIAN TTC TTC_VOL PNLMINADJ PPC NBL BL A I DD DDPNL ABS MSG G2PR PNL/LOSS MIN OTL OPEN MKTPERC PPT\n\n";
  ...
  if($shortcode_ eq "ALL")
  {
    ...
    @list_shc_  = GetListOfShcFromUnderlying($full_strat_path_,"NSE_".$shortcode_."_FUT0");
