...
require "$GENPERLLIB_DIR/make_strat_vec_from_dir_in_tp_maxpred_excluding_sets.pl"; # MakeStratVecFromDirInTpMaxPredExcludingSets
...
my $max_preddur_ = 2;
my @all_strats_in_dir_ = MakeStratVecFromDirInTpMaxPredExcludingSets ( $MODELING_STRATS_DIR."/".$shortcode_, $timeperiod_, $max_preddur_, @exclude_tp_dirs_ );
#print join ( "\n", @all_strats_in_dir_ );
...
if ( $#chosen_strats_ + 1 < $max_num_to_choose_ )
{
    ...
    my @ssr_output_ = `$LIVE_BIN_DIR/summarize_strategy_results $shortcode_ $cstempfile_ $GLOBALRESULTSDBDIR $trading_start_yyyymmdd_ $trading_end_yyyymmdd_`;
    chomp ( @ssr_output_ );
    `rm -f $cstempfile_`;
	...
	if ( $#ssr_words_ >= 14 )
	{
# STRATEGYFILEBASE _fn_ pnl pnl_stdev volume pnl_shrp cons_pnl pnl_median ttc pnl_zs avg_min_max_pnl ppt S B A\n" );
