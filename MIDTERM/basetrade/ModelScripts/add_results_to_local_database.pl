...
while ( my $thisstrategyfile_ = <STRATEGYLISTFILE> )
{
	...
	#print $stratwords_[0]."\n";
	my $this_strat_id_ = "";
	my $strat_word_cnt_ = 0;
	my $this_strat_basename_ = "";
        #my $this_strategyname_ = `head -1 $stratwords_[0] | awk '{print \$3}'`;
	my ( $shortcode_ , $this_strategyname_ ) = GetTradingExec ( $stratwords_[0] ) ;
        #chomp($this_strategyname_);
  	if($stratwords_[0] eq "STRUCTURED_STRATEGYLINE" )
        {
                $this_strat_id_ = `head -1 $stratwords_[1] | awk '{print \$NF}' ` ;
                $strat_word_cnt_ = `head -1 $stratwords_[1] | wc -w ` ;
                chomp($this_strat_id_);
                $this_strat_basename_ = `basename $stratwords_[1]`; chomp ( $this_strat_basename_ );
        }
        elsif ( index($this_strategyname_, "Event") != -1 )
        {
                $this_strat_id_ = `head -1 $stratwords_[0] | awk '{print \$8}' `;
                $strat_word_cnt_ = `head -1 $stratwords_[0] | wc -w `;
                chomp($this_strat_id_);
                $this_strat_basename_ = `basename $stratwords_[0]`; chomp ( $this_strat_basename_ );            
        }
        elsif ( $this_strategyname_ eq "OptionsTrading" )
        {
                $this_strat_id_ = `head -1 $stratwords_[0] | awk '{print \$5}' `;
                $strat_word_cnt_ = `head -1 $stratwords_[0] | wc -w `;
                chomp($this_strat_id_);
                $this_strat_basename_ = `basename $stratwords_[0]`; chomp ( $this_strat_basename_ );            
        }
        ...
        {
                $this_strat_id_ = `head -1 $stratwords_[0] | awk '{print \$NF}' `;
                $strat_word_cnt_ = `head -1 $stratwords_[0] | wc -w `;
                chomp($this_strat_id_);
                $this_strat_basename_ = `basename $stratwords_[0]`; chomp ( $this_strat_basename_ );
        }
        ...
        push ( @strategy_id_, $this_strat_id_ );
    }
}
...
my $cnt = 0;
foreach my$strat_line(@strategy_filevec_)
{
	my $s_id_ =  $strategy_id_[ $cnt ] ;
	$cnt ++;
