...
#Shared folder for running distributed instances of sim_strategy
my $shared_work_dir_ = $celery_shared_location.$work_dir_;
`mkdir -p $shared_work_dir_ `; #make_chared_work_dir for distributed executions
...
#Creating model files in shared directory to enable distributed runs across machines
    my $t_model_filename_ = $shared_work_dir_."/".$t_model_filename_base_;
...
#changing timings as per the event
    ...
    my $temp_output_log = $TRADELOG_DIR."/output_log.".$tradingdate_.".".int($unique_sim_id_);
    my $temporary_strat_location = $celery_shared_location.$this_strat_filename_; #Copy the strat file to a shared location, so that it is accessible to celery workers
    push (@temp_strat_files_to_delete, $temporary_strat_location); #Push to vector so that we can delete this later
    my $copy_cmd = "mkdir -p `dirname $temporary_strat_location`; cp $this_strat_filename_ $temporary_strat_location";
    `$copy_cmd`;
#Find the best worker pool to assign the task to
    my $celery_channel_name = GetBestWorkerPool($base_shortcode_);
    my $exec_cmd_ = "$DISTRIBUTED_EXECUTOR $max_sim_timeout $t_sim_res_filename_ $celery_channel_name $base_shortcode_ bash -c \"sim_output=\\`$LIVE_BIN_DIR/sim_strategy SIM $temporary_strat_location $unique_sim_id_ $tradingdate_ $market_model_index_ 0 0.0 $use_fake_faster_data_ ADD_DBG_CODE -1 2>&1 | tr '\\n' '\\002'\\`; pnl_stats=\\`$MODELSCRIPTS_DIR/get_pnl_stats_2.pl $this_tradesfilename_ 2>&1 | sed 's/^/PNLSTATS /g' | tr '\\n' '\\002'\\`; echo -n \\\$sim_output > $temp_output_log; echo -n \\\$pnl_stats >> $temp_output_log; cat $temp_output_log | tr '\\002' '\\n';\"";
  ...
  #All sim runs complete => lets remove the temporary strat files
  for (my $temp_strat_idx = 0; $temp_strat_idx <= $#temp_strat_files_to_delete; $temp_strat_idx++)
  {
    my $remove_command = "rm -rf $temp_strat_files_to_delete[$temp_strat_idx]";
    `$remove_command`;
  }
...
#print "iter_count_ ".$iter_count_." command_index_ ".$command_index_."\n";
    ...
    {	
      ...
      @all_output_lines_=`cat $t_sim_res_filename_`;
    }

    #Distribute the output lines to sim_strategy_output_lines_ and pnlstats_output_lines_
    for ( my $output_line_index = 0; $output_line_index <= $#all_output_lines_; $output_line_index++ )
    {
      if ( $all_output_lines_[$output_line_index] =~ /PNLSTATS/ )
      {
        # This line was printed by get_pnl_stats (on remote machine)
        push(@pnlstats_output_lines_, $all_output_lines_[$output_line_index]);
      }
      else
      {
        # This line was printed by sim_strategy (stdout/stderr)
        push(@sim_strategy_output_lines_, $all_output_lines_[$output_line_index]);
      }
    }

    #Get pnl stats from query's trades file
    if ( $#pnlstats_output_lines_ >= 0 )
    {
      for ( my $t_pnlstats_output_lines_index_ = 0 ; $t_pnlstats_output_lines_index_ <= $#pnlstats_output_lines_; $t_pnlstats_output_lines_index_ ++ )
      {
        ...
        if( $#rwords_ >= 2 )
        {
          ...
          splice ( @rwords_, 0, 2 ); # remove the first two words since it is "PNLSTATS" and unique_sim_id_
