...
my $USAGE="$0 strategylistfile this_day_resultsfile tradingdate local_results_base_dir prod_index_";
...
while ( my $thisstrategyfile_ = <STRATEGYLISTFILE> ) 
{
    ...
    if ( $#stratwords_ >= 0 )
    {
        ...
        my $this_model_id_ = $stratwords_[2];
        chomp($this_model_id_);
        my $strat_im_name_ = $stratwords_[1];
        open IM_STRAT,"< $strat_im_name_ " or PrintStackTraceAndDie ( " Could not open im_file $strat_im_name_ \n");
        my @im_stratlines_ = <IM_STRAT> ;
        my $index_ = 0 ;
        while ( $index_ <= $#im_stratlines_ ) 
        {
            if ( $index_ == $prod_index_ )
            {
                my @strat_line_words_ = split ( ' ', $im_stratlines_[$index_]) ;
                my $this_model_basename_ = `basename $strat_line_words_[2]`; chomp ( $this_model_basename_ ) ;
