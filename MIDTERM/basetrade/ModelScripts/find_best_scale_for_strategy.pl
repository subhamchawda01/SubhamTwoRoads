...
# \file ModelScripts/find_best_scale_for_strategy.pl
#
...
my $USAGE="$0 input_strat_file output_model last_trading_date num_prev_days change_fraction(between -0.2 to +0.2) [D]";
...
my $num_perturbations_ = 10;
my $final_scale_factor_ = 1.0;

my $prob_day_selected_ = 0.51;

if ( $#ARGV >= 5 )
{
    if ( index ( $ARGV [ 5 ] , "D" ) != -1 ) { $DEBUG = 1; print "DEBUG MODE\n"; }
}
...
push ( @change_fraction_vec_, 1 ); # orig
for ( my $i = 1; $i <= $num_perturbations_ ; $i ++ )
{
    push ( @change_fraction_vec_, ( 1 + $change_ ) ** $i ) ;
    push ( @change_fraction_vec_, ( 1 - $change_ ) ** $i ) ;
}
# print "DEBUG CFV: ".join ( ' ', @change_fraction_vec_ )."\n";
	...
	( $improvement_made_ == 1 ) )
{
    ...
    my @pnl_vec_ = ( );
    my @volume_vec_ = ( );
    @training_date_vec_ = shuffle ( @training_date_vec_ ) ; # adding shuffle so that we can add randomization to CalcPnls
...
=pod
    ...
    if ( ( $best_seen_vol_ > 0 ) && 
	 ( GetMyScore ( $best_seen_pnl_, 1 ) > GetMyScore ( $pnl_vec_[$best_index_], ( $volume_vec_[$best_index_] / $best_seen_vol_ ) ) ) )
    { # last time we had seen a best that is better than this time
	$main_log_file_handle_->printf ( "Reset weights. We had taken a bad step current best pnl %d, score %.2f < old best pnl %d score %.2f\n", GetMyScore ( $pnl_vec_[$best_index_], ( $volume_vec_[$best_index_] / $best_seen_vol_ ) ), $pnl_vec_[$best_index_], $best_seen_pnl_, GetMyScore ( $best_seen_pnl_, 1 ) ) ;
...
=cut
	...
	if ( $best_index_ <= $#change_fraction_vec_ )
	{
	    $main_log_file_handle_->printf ( "Improvement made. change fraction = %f\n", $change_fraction_vec_[$best_index_] ) ;
	}

	if ( ( $best_seen_vol_ <= 0 ) ||
	     ( GetMyScore ( $best_seen_pnl_, 1 ) < GetMyScore ( $pnl_vec_[$best_index_], ( $volume_vec_[$best_index_] / $best_seen_vol_ ) ) ) )
	{ # updating best_seen
	    # if best_index_ > 0 , this should always be true
		...
		$final_scale_factor_ = $final_scale_factor_ * $change_fraction_vec_[$best_index_];
	    }
	}

	my $tref_ = $indicator_weight_vec_vec_[$best_index_];
	@best_indicator_weight_vec_ = @$tref_ ; # set to best
...
# break out of loop if nothign significant
	$main_log_file_handle_->printf ( "Breaking out of loop since no improvement\n" ) ;
	last;
    }
}
...
sub ReadModelFile
{
	...
	if($model_words_[0] eq "INDICATOR")
	{
	    ...
	    {
		my @t_siglr_coeffs_ = split ':', $model_words_[1];
		push( @siglr_alpha_vec_, $t_siglr_coeffs_[0] ); 
		push( @best_indicator_weight_vec_, $t_siglr_coeffs_[1] + 0 );
	    }
	    else
	    {		
...
sub GenerateWeightVecs
{
    ...
    for (my $j = 1; $j <= $#change_fraction_vec_; $j++)
    {
	    my @t_indicator_weight_vec_ = @this_indicator_weight_vec_ ;
	    for ( my $i_w_idx_ = 0 ; $i_w_idx_ <= $#t_indicator_weight_vec_ ; $i_w_idx_ ++ )
	      {
		$t_indicator_weight_vec_[$i_w_idx_] = ( $change_fraction_vec_[$j] ) * $this_indicator_weight_vec_ [$i_w_idx_] ;
	      }
...
sub GetMyScore
{
    my ( $t_pnl_, $t_volume_ratio_ ) = @_;
    return ( $t_pnl_ * sqrt ( $t_volume_ratio_ ) ) ; # pnl * sqrt ( volume[i]/volume[0] )
}
...
sub GetBestIndex
{
    ...
    {
	push ( @sorting_value_vec_, GetMyScore ( $$pnl_vec_ref_[$i], $$volume_vec_ref_[$i]/$$volume_vec_ref_[0] ) ) ; 
