...
if ( $USER eq "ankit" || $USER eq "anshul" || $USER eq "diwakar")
{
...
# start
my $USAGE="$0 param_file_with_permutations start_date end_date strategy_file_name [ event_string(INVALID) ] [ regime_index(starts with 1) ] [ directory_path(if you want work directory to be given) ]";
...
my $event_string_ = "INVALID";
if ( $#ARGV > 3 )
{
  $event_string_ = $ARGV[4];
}

my $regime_index_ = -1;
if ( $#ARGV > 4 && $ARGV[5] > 0 ) {
  $regime_index_ = $ARGV[5];
}
...
if ( $#event_dates_ >= 0 )
{
  $traded_ezone_ =  GetStratTradedEzone ( $strategy_file_name_ );
}
    ...
    if ( ExistsWithSize ( $this_model_filename_ ) )
    {
      ...
      { # For each param
        ...
        if ( ExistsWithSize ( $this_param_filename_ ) )
        {
          ...
          if ( $#event_dates_ >= 0 )
          {
            $exec_cmd="$MODELSCRIPTS_DIR/create_strategy_file.pl $this_strategy_filename_ $shortcode_ $strategyname_ $this_model_filename_ $this_param_filename_ $trading_start_hhmm_ $trading_end_hhmm_ $strategy_progid_ $traded_ezone_";
          }
...
# independent from each other and can be safely run in parallel.
  ...
  for ( my $i = 0 ; $i < $max_days_at_a_time_ ; $i ++ )
  {
    ...
    if ( $#event_dates_ >= 0 && ( not ( $tradingdate_ ~~ @event_dates_ ) ) )
    {
      $tradingdate_ = CalcPrevWorkingDateMult ( $tradingdate_, 1 );
      next;
    }
