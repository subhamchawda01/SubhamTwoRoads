...
while ( my $regline_ = <REGOUTFILE> )
{
    ...
    my ($this_index_, $this_alpha_, $this_beta_ )=(-1,-1,-1);
    if ( ( $#regwords_ >= 2 ) &&
	 ( $regwords_[0] eq "OutCoeff" ) )
    {
	$this_index_ = int($regwords_[1]);
	$this_alpha_ = $regwords_[2];
	$this_beta_ = $regwords_[3];
	
	if ( $this_beta_ * $this_alpha_ != 0 )
	{
	   my $outline_ = "INDICATOR $this_alpha_:$this_beta_ ".$indicator_trailing_text_[$this_index_]."\n";
...
if ( $model_size_ > 0 ) {
    ...
    close OUTMODEL;
    `sed -i 's/LINEAR/SIGLR/g' $output_model_filename_`;
}
else {
    print "-1" ;
}
