...
# \file ModelScripts/find_best_market_model_permute.pl
#
...
sub SanityCheckMarketModelPermutations;
sub PruneRedundantMarketModels;
sub InstallMarketModelPermutations;
...
sub UninstallMarketModelPermutations;
...
sub GetMarketModelFromStratFileName;
...
my $FBMMP_WORK_DIR = $SPARE_HOME."FBMMP/";
...
my $ORIGINAL_MKT_MODEL_FILE_LOCATION = $HOME_DIR."/".$REPO."/SysInfo/TradingInfo/MarketModelInfo";
my $ORIGINAL_MKT_MODEL_FILE = $ORIGINAL_MKT_MODEL_FILE_LOCATION."/market_model_info.txt";
...
# start
my $USAGE="$0 SHORTCODE TIMEPERIOD START_DATE END_DATE MKTMODEL_PERMUATION_FILE [ PLOT=1 ]";
...
my $mktmodel_permutation_file_ = $ARGV [ 4 ]; # Permutation being a misnomer , it's a list of mkt_models
...
my $ACCURATE_ECO_EVENTS_FILE = $HOME_DIR."/".$REPO."/SysInfo/TradingInfo/SimConfig/".$shortcode_."/accurate_merged_eco_2012_processed.txt";

if ( ! ( -d $FBMMP_WORK_DIR ) ) { `mkdir -p $FBMMP_WORK_DIR`; }

my $lock_file_ = $FBMMP_WORK_DIR."f_b_m_m_p.lock";
...
my $work_dir_ = $FBMMP_WORK_DIR.$unique_gsm_id_; 
for ( my $i = 0 ; $i < 30 ; $i ++ )
{
    if ( -d $work_dir_ )
    {
	...
	$work_dir_ = $FBMMP_WORK_DIR.$unique_gsm_id_; 
    }
    else
    {
	last;
...
my $total_mkt_models_ = SanityCheckMarketModelPermutations ( );
PruneRedundantMarketModels ( );
InstallMarketModelPermutations ( );
...
my %mkt_model_to_simreal_results_ = ( );
...
UninstallMarketModelPermutations ( );
...
sub GetParamsAndModels
{
    ...
    my $ACCURATE_SIM_REAL_SCRIPT = $SCRIPTS_DIR."/see_accurate_sim_real_for_prod_nograph.sh";
...
sub SanityCheckMarketModelPermutations
{
    print $main_log_file_handle_ "# SanityCheckMarketModelPermutations\n";

    open ( MKT_MODEL_FILE , "<" , $mktmodel_permutation_file_ ) or PrintStacktraceAndDie ( "Could not open $mktmodel_permutation_file_" );
    my @mkt_model_lines_ = <MKT_MODEL_FILE>; chomp ( @mkt_model_lines_ );
    close ( MKT_MODEL_FILE );

    my $t_total_mkt_models_ = $#mkt_model_lines_;

    foreach my $mkt_model_line_ ( @mkt_model_lines_ )
    {
	my @mkt_model_words_ = split ( ' ' , $mkt_model_line_ );

	if ( $#mkt_model_words_ < 4 )
	{
	    print $main_log_file_handle_ "# SanityCheckMarketModelPermutations : Malformed line $mkt_model_line_\n";
	    PrintStacktraceAndDie ( "# SanityCheckMarketModelPermutations : Malformed line $mkt_model_line_" );
	}

	my $t_shortcode_ = $mkt_model_words_ [ 0 ];

	if ( $t_shortcode_ ne $shortcode_ )
	{
	    print $main_log_file_handle_ "# SanityCheckMarketModelPermutations : Line $mkt_model_line_ for different shortcode than $shortcode_\n";
	    PrintStacktraceAndDie ( "# SanityCheckMarketModelPermutations : Line $mkt_model_line_ for different shortcode than $shortcode_" );
	}
    }

    return $t_total_mkt_models_;
}
...
sub InstallMarketModelPermutations
{
    print $main_log_file_handle_ "# InstallMarketModelPermutations\n";

    my $exec_cmd_ = "cp $ORIGINAL_MKT_MODEL_FILE $work_dir_";
    print $main_log_file_handle_ "# InstallMarketModelPermutations : ".$exec_cmd_."\n";
    `$exec_cmd_`;

    $exec_cmd_ = "cp $mktmodel_permutation_file_ $ORIGINAL_MKT_MODEL_FILE";
    print $main_log_file_handle_ "# InstallMarketModelPermutations : ".$exec_cmd_."\n";
    `$exec_cmd_`;
...
sub UninstallMarketModelPermutations
{
    print $main_log_file_handle_ "# UninstallMarketModelPermutations\n";

    my $t_original_mktmodel_file_ = $work_dir_."/market_model_info.txt";
    my $exec_cmd_ = "cp $t_original_mktmodel_file_ $ORIGINAL_MKT_MODEL_FILE";

    print $main_log_file_handle_ "# UninstallMarketModelPermutations : $exec_cmd_\n";
    `$exec_cmd_`;
...
sub CountTotalSimStratsToBeRun
{
	...
	{
	    for ( my $mkt_model_index_ = 0 ; $mkt_model_index_ <= $total_mkt_models_ ; $mkt_model_index_ ++ )
	    {
...
sub GenerateStrategyFilesAndRunSimulations
{
    print $main_log_file_handle_ "# GenerateStrategyFilesAndRunSimulations MAX_SIM_STRATEGY_IN_PARALLEL=".$MAX_SIM_STRATEGY_IN_PARALLEL."\n";

    my $strategy_progid_ = 6601;

    foreach my $date_ ( sort { $date_to_strat_file_list_ { $a } <=> $date_to_strat_file_list_ { $b } }
			keys %date_to_strat_file_list_ )
    {
	my @parallel_strat_file_list_ = ( );
	my @parallel_prog_id_list_ = ( );
	my @parallel_mkt_model_list_ = ( );
	...
	for ( my $t_strat_file_index_ = 0 ; $t_strat_file_index_ <= $#t_strat_file_list_ ; $t_strat_file_index_ ++ )
	{
	    ...
	    for ( my $mkt_model_index_ = 0 ; $mkt_model_index_ <= $total_mkt_models_ ; $mkt_model_index_ ++ )
	    {
		my $mm_str_ = sprintf ( "%03d" , $mkt_model_index_ );
		my $t_strategy_file_name_ = $local_strats_dir_."/strat_dt.".$date_."_stime.".$t_start_time_."_sfi.".$t_strat_file_index_."_mmi.".$mm_str_."_progid.".$strategy_progid_;

		my $old_sim_id_ = CreateStratFile ( $strat_file_line_ , $t_strategy_file_name_ , $strategy_progid_ );
		...
		push ( @parallel_strat_file_list_ , $t_strategy_file_name_ );
		push ( @parallel_prog_id_list_ , $strategy_progid_ );
		push ( @parallel_mkt_model_list_ , $mkt_model_index_ );

		$strategy_progid_ ++;
	    }
	}

	# Break into blocks of MAX_SIM_STRATEGY_IN_PARALLEL together and run all for today.
	for ( my $i = 0 ; $i <= $#parallel_prog_id_list_ ; $i ++ )
	{
	    my $t_prog_id_ = $parallel_prog_id_list_ [ $i ];
	    my $t_real_id_ = $date_to_sim_id_to_real_id_ { $date_ } { $t_prog_id_ };
	    my $t_mkt_model_ = $parallel_mkt_model_list_ [ $i ];
	    my $t_strat_file_ = $parallel_strat_file_list_ [ $i ];

	    my $t_sim_log_file_ = $SIM_LOG_LOCATION."log.".$date_.".".$t_prog_id_;
	    my $t_sim_trades_file_ = $SIM_TRADES_LOCATION."trades.".$date_.".".$t_prog_id_;

	    if ( -e $t_sim_trades_file_ ) { `rm -f $t_sim_trades_file_`; }
	    if ( -e $t_sim_log_file_ ) { `rm -f $t_sim_log_file_`; }

	    my $exec_cmd_ = "$SIM_STRATEGY_EXEC SIM $t_strat_file_ $t_prog_id_ $date_ $t_mkt_model_ ADD_DBG_CODE -1 >/dev/null 2>&1 &";

	    print $main_log_file_handle_ "# $exec_cmd_\n";
	    print $main_log_file_handle_ "  $t_strat_file_ REALID= $t_real_id_\n";

	    system ( $exec_cmd_ );
	    $total_num_sim_strategies_already_run_ ++;

	    if ( ( $i + 1 ) % $MAX_SIM_STRATEGY_IN_PARALLEL == 0 )
	    {
	...
	for ( my $i = 0 ; $i <= $#parallel_prog_id_list_ ; $i ++ )
	{
	    ...
	    my $t_real_trades_file_ = $REAL_TRADES_LOCATION."trades.".$date_.".".$t_real_id_;

	    if ( ExistsWithSize ( $t_real_trades_file_ ) && ExistsWithSize ( $t_sim_trades_file_ ) )
	    {
		$strat_file_to_simreal_results_ { $t_strat_file_ } = GetSimrealResults ( $t_real_trades_file_ , $t_sim_trades_file_ );

		print $main_log_file_handle_ "# SIMREAL-RESULT $t_strat_file_=".$strat_file_to_simreal_results_ { $t_strat_file_ }."\n";
	    }
...
sub CreateStratFile
{
    my ( $strat_file_line_ , $strat_file_name_ , $strat_progid_ ) = @_;
    ...
    my $old_sim_id_ = $strat_file_words_ [ $#strat_file_words_ ];
...
sub AddResultsToLocalDatabase
{
    ...
    my %date_to_mkt_model_to_simreal_results_ = ( );
...
sub GetMarketModelFromStratFileName
{
    ...
    my $mkt_model_ = 0;
    if ( index ( $strat_file_name_ , "mmi" ) >= 0 &&
	 index ( $strat_file_name_ , "progid" ) >= 0 )
    {
	$mkt_model_ = substr ( $strat_file_name_ , index ( $strat_file_name_ , "mmi" ) + 4 , 3 );
    }
