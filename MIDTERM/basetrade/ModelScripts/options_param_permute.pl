...
require "$GENPERLLIB_DIR/get_market_model.pl"; # market model and use accurate flags
...
if ( $#ARGV == 5  )
{
    $MAX_CORES_TO_USE_IN_PARALLEL = $ARGV[5];
}
...
($market_model_, $use_accurate_) = GetMarketModel($strategy_file_name_);

print "Using following market model and accurate flags ".$market_model_." ".$use_accurate_."\n";
...
sub MakeStrategyFiles 
{
  ...
  for (my $model_file_index_ = 0; $model_file_index_ <= $#model_filevec_ ; $model_file_index_++, $strategy_progid_++) 
  {
      ...
      if ( ExistsWithSize ( $this_model_filename_ ) )
      {
          ...
          my $exec_cmd="$MODELSCRIPTS_DIR/create_options_strategyfile.pl $this_strategy_filename_ $strategyname_ $this_model_filename_ $ivadaptor_ $trading_start_hhmm_ $trading_end_hhmm_ $strategy_progid_ $market_model_ $use_accurate_";
          `$exec_cmd`;
...
# we need to change stratid used inside each of strategyline
    ...
    {
	my $strategy_idx_ = 0;
	my $catted_strategy_filename_ = $work_dir_."/catted_strat_".$strategy_idx_;
	...
	while ( $strategy_idx_ < scalar ( @strategy_filevec_ ) )
	{
		...
		$catted_strategy_filename_ = $work_dir_."/catted_strat_".$strategy_idx_;
...
sub RunSimulationOnCandidates
{
	...
	my $resultsfilename_ = $local_results_base_dir."/".$tradingdateyyyy_."/".$tradingdatemm_."/".$tradingdatedd_."/results_database.txt";
	...
	{
	    for ( my $core_index_ = 0; $core_index_ < $MAX_CORES_TO_USE_IN_PARALLEL; $core_index_++ )
	    {
		...
		push(@resultline_filenames_, $this_resultline_filename_);
		push(@intermediate_files_, $this_resultline_filename_);
		...
		$catted_file_index_++;
		$unique_sim_id_++;
		if ( $catted_file_index_ >= scalar (@clubbed_strategy_filevec_) )
		{
		    last;
		}
		#print $main_log_file_handle_ "running ".$pid_."\n";
	    }
	    while ( wait() > 0 ) {
	    }
	}
	for ( my $resultline_idx_ = 0; $resultline_idx_ < scalar ( @resultline_filenames_ ); $resultline_idx_++ )
	{
	    #2897 NSE_SBIN_C0_O1 106.909108 190 0 100 0 0 3.34 87 697 0 3 64 0.06 0.52 -137 203 150 9730 9958 697 0 0 1 14 29 0.0191 0.0527
	    open RLH, "< $resultline_filenames_[$resultline_idx_]" or PrintStacktraceAndDie ( "Could not open file $resultline_filenames_[$resultline_idx_] for reading" );
		...
		my $stratname_ = $strategy_filemap_{$strat_id_}."_".$shc_;
		my $result_line_ = join(' ', @resultline_words_);

		open ( RESULTSDBFILE, ">> $resultsfilename_" );
		flock ( RESULTSDBFILE, LOCK_EX );
		print RESULTSDBFILE $stratname_, " ", $tradingdate_, " ", $result_line_, "\n" ;
		close ( RESULTSDBFILE );	
	    }
	}
    }
