# \file GenPerlLib/break_strat_file_for_periods_2.pl
#
...
sub BreakStratFileForPeriods2
{
  my ($top_directory_, $basepx_pxtype_, $temp_strat_dir, $trading_start_time, $trading_end_time ) = @_;
  ...
  foreach  my $temp_strat_ ( @strategy_filevec_ )
  {
   ...
   #print "$orig_start_time $orig_end_time $trading_start_time $trading_end_time\n";  	
    	my $new_strat = $temp_strat_dir."$this_strat_basename_" ;
    	`cp $temp_strat_ $new_strat` ;
    	`sed -i 's/ $orig_start_time $orig_end_time / $trading_start_time $trading_end_time /g' $new_strat` ;
    #}
  }
}
