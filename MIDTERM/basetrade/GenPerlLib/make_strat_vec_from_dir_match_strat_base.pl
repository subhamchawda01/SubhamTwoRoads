# \file GenPerlLib/make_strat_vec_from_dir_match_strat_base.pl
#
...
sub MakeStratVecFromDirMatchStratBase
{
  my ($top_directory_, $strategy_type_, $basepx_pxtype_ ) = @_;
  ...
  if ( -d $top_directory_ )
  {
	  ...
	  for my $dir_item_ (@t_list_)
	  {
	      ...
	      if ( ( -f $strat_file_path_ ) &&
		   ...
		   ( ( $basepx_pxtype_ eq "ALL" ) || ( lc ( GetBasepxStratFirstModel ( $strat_file_path_ ) ) eq lc ( $basepx_pxtype_ ) ) ) )
	      {
	      ...
	      push @ret_abs_file_paths_, MakeStratVecFromDirMatchStratBase ("$top_directory_/$dir_item_", $strategy_type_ ) if -d "$top_directory_/$dir_item_";
	  }
      }
  }
  @ret_abs_file_paths_;
