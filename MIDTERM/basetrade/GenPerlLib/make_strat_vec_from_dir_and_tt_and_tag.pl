...
require "$GENPERLLIB_DIR/make_strat_vec_from_dir_matchtag.pl"; # MakeStratVecFromDirMatchTag
...
sub MakeStratVecFromDirAndTTAndTag
{
    my $dirname_ = shift;
    ...
    if ( $#words_ == 1 )
    {
        ...
        my $st_start_limit_ = TimDiff($st_ ,-1*$allowed_gap_in_tt_); 
        my $st_end_limit_ = TimDiff($st_ ,$allowed_gap_in_tt_);
        
        my $et_start_limit_ = TimDiff($et_ ,-1*$allowed_gap_in_tt_); 
        my $et_end_limit_ = TimDiff($et_ ,$allowed_gap_in_tt_);
        
        #print "$st_ $st_start_limit_ $st_end_limit_ $et_ $et_start_limit_ $et_end_limit_\n";
        my @abs_file_paths_ = ();
        push @abs_file_paths_, MakeStratVecFromDirMatchTag ( "$dirname_", $pool_tag_ );
