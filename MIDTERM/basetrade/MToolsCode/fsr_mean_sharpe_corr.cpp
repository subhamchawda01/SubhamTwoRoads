/**
   \file MToolsCode/fsr_mean_sharpe_corr.cpp
...
void FSR_Mean_Sharpe_Corr(std::vector<double>& train_data_dependant_,
  ...
  if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
    ...
    if (must_add_next_k_indeps_ > 0) {
      ...
      double max_abs_median_correlation_ = 0;
      for (unsigned int indep_index_ = 0; indep_index_ < train_data_independants_.size(); indep_index_++) {
        if (included_[indep_index_] == 0) {  // still eligible to be selected
          double this_median_correlation_ = GetNoMeanCorrelationProdMeanSharpe(
              train_data_dependant_, train_data_independants_[indep_index_], num_folds_);

          if (this_median_correlation_ * initial_correlations_[indep_index_] <
              0) {  // the sign of correlation has changed from initial correlation
            included_[indep_index_] = -1;
            continue;
          }

          if (fabs(this_median_correlation_) <= kMinCorrelationIncrement) {  // pruning independants that drop below
                                                                             // kMinCorrelationIncrement threshold along
                                                                             // the way
            included_[indep_index_] = -1;
            continue;
          }
          ...
          if ((chosen_index_ == train_data_independants_.size()) ||
              (fabs(this_median_correlation_) >
               max_abs_median_correlation_)) {  // store index and value of current best option
            chosen_index_ = indep_index_;
            max_abs_median_correlation_ = fabs(this_median_correlation_);
          }
        }
      }
    }
    ...
    if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
      ...
      FSR_Mean_Sharpe_Corr(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, included_,
                           finmodel_, num_folds_, kMinCorrelationIncrement, kMaxIndepCorrelation, initial_correlations_,
                           max_model_size_);
