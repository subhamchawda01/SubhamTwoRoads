/**
   \file MToolsCode/fshlr_so_nomeandata.cpp
...
void FSHLR_SO_NoMeanData(std::vector<double>& train_data_dependant_,
  ...
  if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
    ...
    if (must_add_next_k_indeps_ > 0) {
      ...
      double max_fabs_shlr_correlation_ = 0;
      ...
      for (unsigned int indep_index_ = 0; indep_index_ < train_data_independants_.size(); indep_index_++) {
        if (included_[indep_index_] == 0) {  // still eligible to be selected
          ...
          double t_thresh_value_ = thresh_factor_ * sqrt(sum_square_independants_[indep_index_] /
                                                         (double)((train_data_independants_[indep_index_]).size() - 1));
          double this_shlr_correlation_ =
              GetHVCorrelationNoMean(train_data_dependant_, train_data_independants_[indep_index_], t_thresh_value_);
          double this_fabs_shlr_correlation_ = fabs(this_shlr_correlation_);

          if ((this_shlr_correlation_ * this_correlation_) <
              0) {  // pruning independants that have hvcorr and corr of different sign
            included_[indep_index_] = -1;
            continue;
          }
          if (ignore_zeros_ == true) {
            ...
            if ((chosen_index_ == kInvalidArrayIndex) ||
                (this_fabs_shlr_correlation_ >
                 max_fabs_shlr_correlation_)) {  // store index and value of current best option
              chosen_index_ = indep_index_;
              max_fabs_shlr_correlation_ = this_fabs_shlr_correlation_;
            }
          }
        }
      }
    ...
    // SHLR coeff for nomean data = Sum_j_lines_with_(fabs(x_ci_j) > stdev(x_ci)) [ ( x_ci_j * y_j ) / Sum_j ( x_ci_j *
    // x_ci_j ) ]
    // which is the same as SLR Coeff for nomean data if the dataset is restricted to samples where the fabs_indep_value
    // >= stdev
    double t_thresh_value_ = thresh_factor_ * sqrt(sum_square_independants_[chosen_index_] /
                                                   (double)((train_data_independants_[chosen_index_]).size() - 1));
    double dep_chosen_coeff_ =
        GetSHLRCoeffNoMean(train_data_dependant_, train_data_independants_[chosen_index_], t_thresh_value_);
    ...
    if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
      ...
      FSHLR_SO_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, included_,
                          finmodel_, kMinCorrelationIncrement, kMaxIndepCorrelation, initial_correlations_,
                          max_model_size_, thresh_factor_, train_data_independants_original_, tstat_cutoff_);
