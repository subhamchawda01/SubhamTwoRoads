...
void FSHDVLR_SO_NoMeanData(std::vector<double>& train_data_dependant_,
  ...
  if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
    ...
    if (RSS_ >= sum_square_dependant_) {  // Gained nothing by adding this indicator...
      ...
      // std::cerr<<"Gained Nothing by adding this indicator. Chosen_index: " << chosen_index_ << " RSS_: "<< RSS_ << "
      // sum_square_dependant_: " << sum_square_dependant_ << std::endl ;
      included_[chosen_index_] = -1;
      VectorUtils::ScaledVectorAddition(train_data_dependant_, train_data_independants_[chosen_index_],
                                        +dep_chosen_coeff_);
      // std::cerr<< "SHDLRCoeff: "<< dep_chosen_coeff_ << " FSLRCoeff: " << GetSLRCoeffNoMean ( train_data_dependant_,
      // train_data_independants_ [ chosen_index_ ] ) << std::endl;
    } else {
    ...
    if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
      ...
      FSHDVLR_SO_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, included_,
