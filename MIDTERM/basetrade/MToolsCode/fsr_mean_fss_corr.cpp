/**
   \file MToolsCode/fsr_mean_fss_corr.cpp
...
void FSR_Mean_FSS_Corr(std::vector<double>& train_data_dependant_,
  ...
  if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
    ...
    if (must_add_next_k_indeps_ > 0) {
      ...
      for (unsigned int indep_index_ = 0; indep_index_ < train_data_independants_.size(); indep_index_++) {
        if (included_[indep_index_] == 0) {  // still eligible to be selected
          double this_median_correlation_ = GetNoMeanCorrelationProdMeanFracSameSign(
    ...
    if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
      ...
      FSR_Mean_FSS_Corr(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, included_, finmodel_,
