/**
   \file MToolsCode/fsrlm_nomeandata.cpp
...
void FSRLM_NoMeanData(std::vector<double>& train_data_dependant_,
  ...
  if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
    ...
    double dep_chosen_coeff_ =
        GetMedianNoMeanCorrelation(train_data_dependant_, train_data_independants_[chosen_index_], num_folds_) *
    ...
    if (NextNotIncluded(included_) != kInvalidArrayIndex) {  // there are still indicators eligible to be selected
      ...
      FSRLM_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, included_, finmodel_,
