...
/// call FSRLM ( on data that is mean zero to save needless mean calcing operations )
/// FSRLM selects the indicator (X_i) that has the highest median correlation with the residual,
...
int main(int argc, char** argv) {
  ...
  if (argc >= 11) {
    HFSAT::MTOOLS_UTILS::loadIndicatorHistoricalCorrelations(argv[10], historical_indicator_correlations_);
  ...
  if (stdev_orig_dependant_ > 0.00000001) {
    FSRLM_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_, finmodel_,
                     num_folds_, min_correlation_, max_indep_correlation_, initial_correlations_, max_model_size_);
  }

  std::vector<double> prev_computed_tstats;  // temporary, just for logging, TODO remove me
