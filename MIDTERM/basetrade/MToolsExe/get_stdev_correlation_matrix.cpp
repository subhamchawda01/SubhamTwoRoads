/**
    \file MToolsExe/get_stdev_correlation_matrix.cpp
...
int main(int argc, char** argv) {
  ...
  std::vector<double> stdev_vec_(t_in_matrix_.size(), 0);
  covar_matrix_.SetSqrtDiagonal(stdev_vec_);

  HFSAT::MatrixUtils::CovarToCorrMatrix(covar_matrix_);

  std::cout << stdev_vec_[0];
  for (unsigned int i = 1; i < stdev_vec_.size(); i++) {
    std::cout << ' ' << stdev_vec_[i];
  }
  std::cout << std::endl;
