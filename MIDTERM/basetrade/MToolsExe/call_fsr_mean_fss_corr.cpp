/**
    \file MToolsExe/call_fsr_mean_fss_corr.cpp
...
/// Exec used to call Forward Stagewise Robust Linear Regression
...
/// call FSR_Mean_FSS_Corr ( on data that is mean zero to save needless mean calcing operations )
/// FSR_Mean_FSS_Corr selects the indicator (X_i) that has the highest meean * abs(sharpe) correlation with the
/// residual,
/// removes effect from residual (Y) by alpha * X_i, where alpha = median(corr(Y,X_i),nfolds) * std(Y)/std(X_i)
///
int main(int argc, char** argv) {
  ...
  if (argc < 9) {
    std::cerr << "USAGE: " << argv[0]
              << " input_reg_data_file_name  nfolds  min_correlation  first_indep_is_weight  "
                 "mult_include_first_k_independants  max_indep_correlation  regression_output_filename  max_model_size "
                 "avoid_sharpe_check_filename [indicator_correlations_]" << std::endl;
    exit(0);
  }
  ...
  max_model_size_ = std::min(30u, std::max(1u, (unsigned int)atoi(argv[8])));
  if (argc >= 10) {
    avoid_high_sharpe_check_vector_filename_ = argv[9];
  }

  std::vector<double> ilist_indicator_correlations_;  //-1 for negative, 0 for no info or actually ZERO, 1 for positive
  std::vector<int> semantic_indicator_correlations_;
  if (argc >= 11) {
    HFSAT::MTOOLS_UTILS::loadIndicatorSignsFromIlist(argv[10], ilist_indicator_correlations_);
    HFSAT::MTOOLS_UTILS::loadIndicatorSemantics(argv[10], semantic_indicator_correlations_);
  }

  bool check_semantic_correlations_ = true;
  if (argc >= 12 && *argv[11] == 'N') {
    std::cout << "muting semantic correlations\n";
  ...
  if ((stdev_orig_dependant_ <= 0) || (fabs(mean_orig_dependant_) > (kMaxSharpeDependant * stdev_orig_dependant_))) {
    ...
    if (std::isnan(sharpe_)) {
      ...
      std::cout << " Sharpe of the dependant is " << sharpe_ << " more than " << kMaxSharpeDependant
  ...
  if (stdev_orig_dependant_ > 0.00000001) {
  ...
  std::vector<std::vector<double> > train_data_independants_copy_ = train_data_independants_;
  HFSAT::CovMatInv& covMat = HFSAT::CovMatInv::GetUniqueInstance(train_data_independants_);
    ...
    if ((in_included_[indep_index_] == 0) &&
      ...
      if (stdev_orig_indep_[indep_index_] > 0) {
        ...
        if (std::isnan(sharpe_)) {
          ...
          std::cout << " Sharpe of indep ( " << indep_index_ << " ) is " << sharpe_ << " more than "
  ...
  HFSAT::MTOOLS_UTILS::filterIndicatorsBasedOnHistoricalCorrelations(
      train_data_independants_.size(), initial_correlations_, min_correlation_, ilist_indicator_correlations_,
      in_included_, false);
  ...
  if (stdev_orig_dependant_ > 0.00000001) {
    FSR_Mean_FSS_Corr(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_, finmodel_,
                      num_folds_, min_correlation_, max_indep_correlation_, initial_correlations_, max_model_size_);
  }

  std::vector<double> prev_computed_tstats;  // tmporary, just for logging, TODO remove me
  // compute final tstat_
  {
    for (size_t i = 0; i < finmodel_.size(); ++i) covMat.AddIndex(finmodel_[i].origindex_);
    std::vector<double> err_mat_diag = covMat.inv_mat.diagonal();

    double RSS_ = VectorUtils::CalcL2Norm(train_data_dependant_);
    double n_2_ = train_data_dependant_.size() - 2;  // train_data_dependant_.size() - model_size (degrees of freedom)
    double n_1_ = train_data_dependant_.size() - 1;
    double denom_ = sqrt((RSS_ / n_2_) / n_1_);
    for (auto i = 0u; i < finmodel_.size(); i++) {
      finmodel_[i].tstat_ = finmodel_[i].coeff_ / sqrt((RSS_ / n_2_) * err_mat_diag[i]);
      prev_computed_tstats.push_back(finmodel_[i].coeff_ / denom_);
    }
  }
  ...
  for (auto i = 0u; i < finmodel_.size(); i++) {
    ...
    regression_output_file_
        << "OutCoeff " << finmodel_[i].origindex_ << ' ' << stdev_readjusted_coeff_ << " InitCorrelation "
        << initial_correlations_[finmodel_[i].origindex_] << " Tstat " << finmodel_[i].tstat_ << " AutoCorrelation "
        << indep_auto_corrs_[i] << " Heuristic_Tstat "
        << prev_computed_tstats[i]  // a hack. TODO remove this line after we have logged and seen some typical values
        << " TstatSelect " << finmodel_[i].tstat_residual_dependant_ << std::endl;
  }
