...
/// call FSHLR ( on data that is mean zero to save needless mean calcing operations )
int main(int argc, char** argv) {
  ...
  double stdev_thresh_factor_ = 1.0;
  ...
  if (argc >= 10) {
    HFSAT::MTOOLS_UTILS::loadIndicatorSignsFromIlist(argv[9], ilist_indicator_correlations_);
  ...
  if (argc >= 12 && *argv[11] == 'Y') {
    std::cerr << "Ignoring zeros for indicator selection\n";
    ignore_zeros_ = true;
  }
  // read data
  ...
  FSHLR_SO_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_, finmodel_,
                      ...
                      stdev_thresh_factor_, train_data_independants_original_, tstat_thresh_, ignore_zeros_);
  ...
  model_correlation_HDV_ = GetModelHVCorrelationNoMean(train_data_orig_dependant_, train_data_dependant_,
                                                       train_data_independants_, finmodel_, 1.0);
