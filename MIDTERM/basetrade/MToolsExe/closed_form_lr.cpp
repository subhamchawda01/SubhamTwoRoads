...
int main(int argc, char** argv) {
  ...
  if (argc < 5) {
    std::cerr << argv[0] << " input_reg_data_file_name  input_variable_selection_filename  is_first_indep_weight  "
                            "regression_output_filename  " << std::endl;
    exit(0);
  }

  input_reg_data_file_name_ = argv[1];
  input_variable_selection_filename_ = argv[2];
  is_first_indep_weight_ = (atoi(argv[3]) != 0);
  ...
  {  // computing norm_reg_coeffs_ = ( X' * X )^-1 * ( X' * y )
    ...
    std::cout << covar_matrix_.ToString() << std::endl;
