...
int main(int argc, char** argv) {
  ...
  dep_outlier_percentile_ = (std::max(0.1, std::min(100.0, atof(argv[3]))) / 100.00);
  indep_outlier_percentile_ = (std::max(0.1, std::min(100.0, atof(argv[4]))) / 100.00);
  ...
  unsigned int dep_max_acceptable_index_ = (int)(std::min(round((dep_outlier_percentile_)*train_data_dependant_.size()),
                                                          train_data_dependant_.size() - 1.0));

  unsigned int indep_min_acceptable_index_ = (int)round((1 - indep_outlier_percentile_) * train_data_dependant_.size());
  unsigned int indep_max_acceptable_index_ = (int)(std::min(
      round((indep_outlier_percentile_)*train_data_dependant_.size()), train_data_dependant_.size() - 1.0));
  ...
  // read data
  {
    ...
    while (infile_.good()) {
          ...
          (int)(tokens_.size()))  // make sure this line has the number of words we are expecting
      {
        ...
        if ((this_dep_value_ > dep_min_acceptable_value_) && (this_dep_value_ < dep_max_acceptable_value_)) {
          for (int i = 0; i < num_independants_; i++) {
            ...
            if ((this_indep_value_ < indep_min_acceptable_value_[i]) ||
                (this_indep_value_ > indep_max_acceptable_value_[i])) {
              okay = false;
              break;
            }
          }
