/**
    \file MToolsExe/call_fsr_mean_sharpe_corr.cpp
...
/// call FSR_Mean_Sharpe_Corr ( on data that is mean zero to save needless mean calcing operations )
/// FSR_Mean_Sharpe_Corr selects the indicator (X_i) that has the highest meean * abs(sharpe) correlation with the
/// residual,
...
int main(int argc, char** argv) {
  ...
  if (stdev_orig_dependant_ > 0.00000001) {
    FSR_Mean_Sharpe_Corr(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_,
                         finmodel_, num_folds_, min_correlation_, max_indep_correlation_, initial_correlations_,
                         max_model_size_);
  }
