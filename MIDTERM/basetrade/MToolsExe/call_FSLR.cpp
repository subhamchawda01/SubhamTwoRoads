...
int main(int argc, char** argv) {
  ...
  if (argc < 8) {
    std::cerr << "USAGE: " << argv[0]
              << " input_reg_data_file_name  min_correlation  first_indep_is_weight  mult_include_first_k_independants "
                 ...
                 "ilist_filename_ [check_semantic_correlations_(Y/N)=Y] ignore_zeros(true/false)" << std::endl;
    exit(0);
  }
  ...
  if (argc >= 10) {
    // HFSAT::MTOOLS_UTILS::loadIndicatorHashCorrelations ( argv[9], historical_indicator_correlations_ );
    HFSAT::MTOOLS_UTILS::loadIndicatorSignsFromIlist(argv[9], ilist_indicator_correlations_);  // loading from file
    HFSAT::MTOOLS_UTILS::loadIndicatorSemantics(argv[9], semantic_indicator_correlations_);
  }  // also loads up semantic correlations
  ...
  if (DEBUG == 1) {
    std::vector<std::string> indicator_;
    if (HFSAT::FileUtils::ExistsAndReadable(argv[9])) {
      std::ifstream ilist_infile_;
      ilist_infile_.open(argv[9], std::ifstream::in);
      if (!ilist_infile_.is_open()) {
        std::cerr << " input indicator list file " << argv[9] << " cannot be opened " << std::endl;
      ...
      while (ilist_infile_.good()) {
        ...
        if (ilist_infile_.gcount() > 0) {
          ...
          if (strcmp(tokens_[0], "INDICATOR") == 0) {
            indicator_.push_back(std::string(readline_buffer_, kIListLineBufferLen));
          }
        }
      }
      ilist_infile_.close();
    ...
    for (auto i = 0u; i < semantic_indicator_correlations_.size(); i++) {
      if (semantic_indicator_correlations_[i] != (initial_correlations_[i] / fabs(initial_correlations_[i]))) {
        std::cerr << indicator_[i] << "\t" << semantic_indicator_correlations_[i] << "\t" << initial_correlations_[i]
                  << "\n";
      }
    }
  }
  ...
  // to add an idnicator
  // now this also makes sure signs of historical correlations is same as initial correlations
  ...
  FSLR_SO_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_, finmodel_,
