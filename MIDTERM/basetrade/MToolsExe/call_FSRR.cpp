...
/// call FSRR ( on data that is normalized since ridge regression is defined this way )
int main(int argc, char** argv) {
  ...
  if (argc < 9) {
    std::cerr << "USAGE: " << argv[0]
              << " input_reg_data_file_name  regularization_coeff  min_correlation  first_indep_is_weight  "
                 "mult_include_first_k_independants  max_indep_correlation regression_output_filename  max_model_size "
                 "avoid_sharpe_check_filename [historical_indicator_correlations_]" << std::endl;
    exit(0);
  }
  ...
  if (argc >= 13 && *argv[12] == 'Y') {
    std::cout << "selecting indicators via correaltion excluding zeros\n";
    ignore_zeros_ = true;
  }
  ...
  if (NextNotIncluded(in_included_) == kInvalidArrayIndex) {
    std::cout << "No valid independant left after eliminating high sharpe ones" << std::endl;
  ...
  for (auto i = 0u; i < coeff_dependancy_matrix_.size(); i++) {
  ...
  FSRR_SO_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_, finmodel_,
                     coeff_dependancy_matrix_, min_correlation_, regularization_coeff_, max_indep_correlation_,
                     initial_correlations_, max_model_size_, train_data_independants_original_, tstat_thresh_,
                     ignore_zeros_);
