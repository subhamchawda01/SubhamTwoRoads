/**
    \file MToolsExe/call_fslrsq_nm.cpp
...
/// call FSLRSQ_NM ( on data that is mean zero to save needless mean calcing operations )
int main(int argc, char** argv) {
  ...
  if (argc < 8) {
    std::cerr << "USAGE: " << argv[0]
              << " input_reg_data_file_name  min_correlation  first_indep_is_weight  mult_include_first_k_independants "
                 " sort_criterion  regression_output_filename  max_model_size avoid_sharpe_check_filename "
  ...
  if (argc >= 10) {
    HFSAT::MTOOLS_UTILS::LoadHistoricalIndicatorCorrelations(argv[9], historical_indicator_correlations_);
  }

  double min_rsquared_increment_ = min_correlation_ * min_correlation_;
  ...
  // to add an idnicator
  HFSAT::MTOOLS_UTILS::FilterIndicatorsBasedOnInitialCorrelation(train_data_independants_.size(), initial_correlations_,
                                                                 min_correlation_, historical_indicator_correlations_,
                                                                 in_included_);
  ...
  // initialize
  std::vector<double> max_abs_correlations_sel_(train_data_independants_.size(), 0);

  FSLRSQ_NM(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_, finmodel_,
            min_rsquared_increment_, sort_criterion_, max_abs_correlations_sel_, initial_correlations_, max_model_size_,
