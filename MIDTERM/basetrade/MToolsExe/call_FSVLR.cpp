...
/// call FSVLR ( on data that is mean zero to save needless mean calcing operations )
int main(int argc, char** argv) {
  ...
  double stdev_thresh_factor_ = 2.0;
  ...
  if (argc < 9) {
    std::cerr << "USAGE: " << argv[0]
              << " input_reg_data_file_name  stdev_thresh_factor  min_correlation  first_indep_is_weight  "
  ...
  infilename_ = argv[1];
  stdev_thresh_factor_ = std::max(1.0, atof(argv[2]));
  ...
  FSHLR_SO_NoMeanData(train_data_dependant_, train_data_independants_, must_add_next_k_indeps_, in_included_, finmodel_,
                      min_correlation_, max_indep_correlation_, initial_correlations_, max_model_size_,
                      stdev_thresh_factor_, train_data_independants_original_, tstat_thresh_);
  ...
  // computing auto correlation for indeps
  ...
  double model_correlation_HDV_ = 0;
  ...
  model_correlation_HDV_ = GetModelHVCorrelationNoMean(train_data_orig_dependant_, train_data_dependant_,
                                                       train_data_independants_, finmodel_, stdev_thresh_factor_);
  ...
  regression_output_file_ << "HDVCorr " << model_correlation_HDV_ << std::endl;
