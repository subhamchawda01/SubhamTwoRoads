/**
   \file MToolsExe/summarize_local_results_dir_and_choose.cpp
...
void termination_handler(int signum) {
  if (signum == SIGSEGV) {  // On a segfault inform , so this is picked up and fixed.
    ...
    std::string email_string_ =
        "summarize_local_results_dir_and_choose_by_algo received sigsegv on " + std::string(hostname_) + "\n";
...
/// This exec will look at all the result_files in the directory provided
...
int main(int argc, char** argv) {
  ...
  if (argc < 8) {
    std::cerr << argv[0] << " sortalgo min_num_files_to_choose_ num_files_to_choose_ min_ppt min_volume max_ttc "
                            "max_msgs results_directory [start_date] [end_date]" << std::endl;
    exit(0);
  }
  ...
  max_messages_to_allow_ = atoi(argv[7]);
  std::string local_results_directory_ = argv[8];
  int start_date = 19700101;
  int end_date = 21210101;

  if (argc > 9) {
    start_date = atoi(argv[9]);
    if (argc > 10) {
      end_date = atoi(argv[10]);
    }
  }

  FileUtils::GetFilePaths(local_results_directory_, resultsfilename_vec_);
    ...
    for (auto i = 0u; i < resultsfilename_vec_.size(); i++) {
      ...
      while (infile_.good()) {
        ...
        if (tokens_.size() >= 4) {  // filename date pnl volume supporting% bestlevel% agg% [ Average_Abs_Pos
          ...
          if (result_line_.yyyymmdd_ > start_date && result_line_.yyyymmdd_ < end_date) {
  ...
  for (ResultLineSetVecIter_t rlsviter_ = result_set_vec_left_to_choose_.begin();
    ...
    if ((rlsviter_->median_volume_normalized_average_time_to_close_trades_ > max_ttc_to_allow_) ||
        ...
        ((max_messages_to_allow_ != -1) && (rlsviter_->average_msg_count_ > max_messages_to_allow_))) {
  ...
  for (auto i = 0u; i < result_set_vec_chosen_.size(); i++) {
    ...
    printf(
        "STATISTICS %d %d %d %.2f %d %d %d %d %.2f %d %d %d %d %d %.2f %d %d %d %d %.2f %.2f %d %.2f %.2f %.2f %2f %d "
        "%d %.2f\n",
