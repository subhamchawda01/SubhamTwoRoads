...
int main(int argc, char** argv) {
  ...
  std::vector<double> periodic_stdevs_ = common_mds_info_util->GetMappedStdev();
  std::vector<int> timestamps_ = common_mds_info_util->GetTimeStamps();

  if (periodic_stdevs_.size() == timestamps_.size()) {
    for (unsigned i = 0; i < timestamps_.size(); i++) {
      std::cout << timestamps_[i] << " " << periodic_stdevs_[i] << std::endl;
    }
  }
}
