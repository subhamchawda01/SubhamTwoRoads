...
#ifndef BASE_TOOLS_E5PROCESSINGNEW_HPP
#define BASE_TOOLS_E5PROCESSINGNEW_HPP

namespace HFSAT {
class E5Processing_new : public Processing {
 protected:
 ...
 public:
  E5Processing_new(const std::string& input_data_filename_, SimpleLineProcessor& _simple_line_processor_,
  ...
  void Process(int _base_msecs_, int _base_events_, bool& were_files_reset_, int& _last_good_offset_, bool& _found_,
...

    {
      ...
      while (input_data_file_pred2_.good()) {
        ...
        int pred2_events = atoi(pred2_tokens_[0]);
      ...
      while (input_data_file_pred3_.good()) {
        ...
        int pred3_events = atoi(pred3_tokens_[0]);
      ...
      while (input_data_file_pred4_.good()) {
        ...
        int pred4_events = atoi(pred4_tokens_[0]);
        if (pred4_events > (base_events + msecs2_to_predict_)) {
          _found_ = true;
      ...
      while (input_data_file_pred5_.good()) {
        ...
        int pred5_events = atoi(pred5_tokens_[0]);
        if (pred5_events > (base_events + msecs2_to_predict_)) {
          _found_ = true;
