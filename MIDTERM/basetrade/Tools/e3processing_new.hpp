...
#ifndef BASE_TOOLS_E3PROCESSINGNEW_HPP
#define BASE_TOOLS_E3PROCESSINGNEW_HPP

namespace HFSAT {
class E3Processing_new : public Processing {
 protected:
  std::ifstream input_data_file_pred1_, input_data_file_pred2_, input_data_file_pred3_;
  int msecs1_to_predict_, msecs2_to_predict_, msecs3_to_predict_;
  double nfac1_, nfac2_, nfac3_;

 public:
  E3Processing_new(const std::string& input_data_filename_, SimpleLineProcessor& _simple_line_processor_,
    ...
    msecs1_to_predict_ = std::max(1, (msecs_to_predict_ * 1) / 3);
    msecs2_to_predict_ = std::max(1, (msecs_to_predict_ * 2) / 3);
    msecs3_to_predict_ = msecs_to_predict_;
    nfac1_ = 4.0 / 9.0;
    nfac2_ = 3.0 / 9.0;
    nfac3_ = 2.0 / 9.0;
  }
  void Process(int _base_msecs_, int _base_events_, bool& were_files_reset_, int& _last_good_offset_, bool& _found_,
...

    {
      ...
      while (input_data_file_pred1_.good()) {
        ...
        int pred1_events = atoi(pred1_tokens_[1]);
        if (pred1_events > (base_events + msecs1_to_predict_)) {
          _found_ = true;
      ...
      while (input_data_file_pred2_.good()) {
        ...
        int pred2_events = atoi(pred2_tokens_[1]);
        if (pred2_events > (base_events + msecs2_to_predict_)) {
          _found_ = true;
      ...
      while (input_data_file_pred3_.good()) {
        ...
        int pred3_events = atoi(pred3_tokens_[1]);
        if (pred3_events > (base_events + msecs3_to_predict_)) {
          _found_ = true;
      ...
      if (!_found_) return;
      double factor = trade_vol_reader == NULL ? 1.0 : trade_vol_reader->getTradedVolRatioAtTime(_base_msecs_ / 1000);
