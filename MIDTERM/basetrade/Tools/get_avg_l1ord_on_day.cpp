 ...
 public:
  ...
  void OnMarketUpdate(const unsigned int _security_id_, const HFSAT::MarketUpdateInfo &_market_update_info_) {
    if (watch_.tv().tv_sec % 86400 > start_unix_time_ && watch_.tv().tv_sec % 86400 < end_unix_time_) {
      if (this_smv_.bestask_ordercount() > 0 && this_smv_.bestbid_size() > 0) {
        total_size_ += (this_smv_.bestask_ordercount() + this_smv_.bestbid_ordercount()) / 2;
        total_count_++;
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  if (test_book_.total_count_ < 1) {
    ...
    std::cout << _this_shortcode_ << ' ' << print_secname_ << ' ' << (test_book_.total_size_ / test_book_.total_count_)
              << std::endl;
