...
class SimpleBAwithMarket : public HFSAT::SecurityMarketViewChangeListener,
 ...
 public:
  SimpleBAwithMarket(HFSAT::SecurityMarketView& this_svm_t, HFSAT::DebugLogger& dbglogger_t, HFSAT::Watch& watch_t,
                     ...
                     std::map<std::pair<std::string, std::string>, std::vector<PnlCheckStruct>>& r_pnl_check_map_, int _min_order_size_)
...
long double SimpleBAwithMarket::GetTradedVolume() {
   return self_traded_volume_;
}
...
void SimpleBAwithMarket::DumpHeader() {
  ...
  oebu_info_outstream << "jsonstr= [{ "
                      ...
                      << " \"TotalTradedValue\" : \" "
                      << (total_traded_value / LAKH_DIVISOR) << "\","
                      << " \"OurTotalTradedValue\" : \" "
                      << (our_total_traded_value / LAKH_DIVISOR)
                      << "\"}]";
...
void SimpleBAwithMarket::DefLoop(unsigned int this_sid_number, const int _trade_px_,
  ...
  if (send_pnl_alert_) {
    ...
    for (unsigned int ii = 0;ii < shortcode_timeperiod_to_pnl_check_[std::make_pair(this_smv_.market_update_info_.shortcode_, timeofday_)].size();ii++) {
      PnlCheckStruct* pnl_check_struct_ =
          &shortcode_timeperiod_to_pnl_check_[std::make_pair(this_smv_.market_update_info_.shortcode_, timeofday_)][ii];
      if (pnl_check_struct_->pnl_check_ != 0) {
        if (pnl_check_struct_->recent_pnl_ - (int)prom_pnl_->total_pnl() > pnl_check_struct_->pnl_check_) {
  ...
  */
  ...
  long current_exposure_t = 0;
  if ( this_smv_.mid_price() < 0 || this_smv_.bid_price(t_level_) < 0 || this_smv_.ask_price(t_level_) < 0 ) {
    current_exposure_t = (long int)(p_prom_order_manager_->global_position() * current_traded_price_) / LAKH_DIVISOR;
  }
  else {
    current_exposure_t = (long int)(p_prom_order_manager_->global_position() * this_smv_.mid_price()) / LAKH_DIVISOR;
  }
  ...
  if (m_m_levels >= 1 && std::min(this_smv_.NumBidLevels(), this_smv_.NumAskLevels()) >= 1) {
    switch (this_smv_.secname()[0]) {
      ...
      default: {
        ...
        if (this_smv_.this_smv_exch_source_ == HFSAT::kExchSourceMICEX ||
         ...
         /*
          ...
          //dbglogger_ << "Secname, TV, MV:: " << secname.c_str() << ", " << p_prom_order_manager_->get_executed_totalsize() << ", " << (double)(traded_volume_) << "\n";
          //dbglogger_ << "Secname, curr_px, midpx, pos:: " << secname.c_str() << ", " << current_traded_price_ << ", " << this_smv_.mid_price() << ", " << p_prom_order_manager_->global_position() << "\n";
          tempstring << "[";
	  ...
	  tempstring << start << (double)p_prom_order_manager_->global_position() / min_order_size_<< end;

          if(this_smv_.mid_price() < 0 || this_smv_.bid_price(t_level_) < 0 
			  || this_smv_.ask_price(t_level_) < 0) {
            tempstring << start << (int)(p_prom_order_manager_->global_position() * current_traded_price_) << end;
          }
	  else{
            tempstring << start << (int)(p_prom_order_manager_->global_position() * this_smv_.mid_price()) << end;
          }

          tempstring << start << self_traded_volume_ / 100000 << end;
          tempstring << start << (double)traded_volume_/100000 << end;
          tempstring << start
	  ...
	  if ( false == underlying_to_prom_order_manager_map.empty() ) {
	    ...
	    HFSAT::PerishableStringTokenizer::ConstStringTokenizer(secname.c_str(), "_", tokens_);
            ...
            size_t tokens_vec_size = tokens_.size();
            if ( std::string(tokens_[tokens_vec_size - 1]) == "W" ) {
              sstream << tokens_[0] << "_W_COMB";
            }
            else {
	    ...
	    for (UnderlyingToSimpleBABookMapCIter itr = result3.first; itr != result3.second; ++itr) {
	      aggregate_traded_volume += itr->second->GetTradedVolume();
            ...
            tempstring1 << "["
                        ...
                        << start << "" << end
                        << start << "" << end
                        << start << aggregate_pnl << end
                        << start << aggregate_pnl_diff << end
                        << start << aggregate_global_pos << end
                        << start << "" << end
                        << start << aggregate_traded_volume/ LAKH_DIVISOR << end
                        << start << "" << end
                        << start << "" << end
                        << start << "" << end
                        << start << "" << end
...
int main(int argc, char** argv) {
  ...
  if (argc < 7) {
    std::cerr << "Usage : <INPUT_PROD_FILE> <MKT_INFO_JSON_FILE> <OEBU_INFO_JSON_FILE> <MACHINE> <OEBU_TYPE [N/C]> <EXPIRY_NUM>"
  ...
  // setup DebugLogger
  {
    ...
    t_temp_oss_ << "/spare/local/logs/alllogs/our_extended_ba_mkt_book." << machine << "." << HFSAT::DateTime::GetCurrentIsoDateLocal();
  ...
  if (argc > 4 && !strncmp(argv[4], "LOG", strlen("LOG"))) {
    ...
    t_temp_oss_ << "/spare/local/logs/alllogs/our_extended_ba_mkt_book_binary_log."
                << machine << "." << HFSAT::DateTime::GetCurrentIsoDateLocal();
    std::string logfilename_ = t_temp_oss_.str();
...
// ================================================================
 ...
 // Since NSE requires some files to load which may/may not be present on a given server
  ...
  t_oss << "/spare/local/logs/alllogs/our_extended_BULK." << machine << ".log";
  ...
  for (unsigned int ii = 0; ii < (unsigned int)sec_list_vec.size(); ii++) {
    ...
    if ( tokens_.size() > 2 && 
      ...
      min_order_size =  HFSAT::SecurityDefinitions::GetContractMinOrderSize(ss1.str(), HFSAT::DateTime::GetCurrentIsoDateLocal());
    }
    ...
    simple_ba_price_with_market_price_list_.push_back(
        new SimpleBAwithMarket(*(shortcode_smv_map_.GetSecurityMarketView(_this_shortcode_)), dbglogger_, watch_,
                               ...
                               p_prom_pnl_map_[_this_shortcode_], p_binary_log_, shortcode_timeperiod_to_pnl_check_, min_order_size));
    ...
    if (combine_shortcodes_info && tokens_.size() >= 3) {
       ...
       if ( std::string(tokens_[tokens_.size() - 1]) == "W") {
         sstream << tokens_[1] << "_COMB"  << "_W";
       }
       else {
         sstream << tokens_[1] << "_COMB";
       }
  ...
  HFSAT::AllocateCPUUtils::GetUniqueInstance().AllocateCPUOrExit("our_extended_bidask_mkt_book_util_nse_web_multishm_all");
