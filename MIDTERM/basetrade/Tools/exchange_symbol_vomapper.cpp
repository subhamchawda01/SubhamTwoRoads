...
int main(int argc, char** argv) {
  ...
  if (vfile.is_open()) {
    ...
    for (unsigned int ii = 0; ii < (unsigned int)sec_list_vec.size(); ii++) {
      ...
      if (sec_list_vec[ii].compare("DI") == 0) {
        std::vector<int> vol_vec;
        std::vector<std::string> max_symbol_vec;
        unsigned int kMaxNumbersToList = 7;
        // List first kMaxNumbersToList max vols contract in case of DI

        symMapper.SearchMaxNVolumeforBaseSymbol(sec_list_vec[ii], kMaxNumbersToList, vol_vec, max_symbol_vec);

        // array bound fix
        if (max_symbol_vec.size() != kMaxNumbersToList || vol_vec.size() != kMaxNumbersToList) continue;

        for (unsigned int jj = 0; jj < kMaxNumbersToList; jj++) {
          if (vol_vec[jj] > 0 && max_symbol_vec[jj].compare(0, 13, "ZEROMAXVOLUME") != 0) {
            std::stringstream ss;
            ss << sec_list_vec[ii] << "_" << jj;
            vfile << ss.str() << "\t" << max_symbol_vec[jj] << "\t" << vol_vec[jj] << "\n";
          }
        }
