/**
   ...
   \author: (c) Copyright Two Roads Technological Solutions Pvt Ltd 2011
    Address:
         ...
         +91 80 4190 3551
*/
#ifndef BASE_TOOLS_M4PROCESSING_HPP
#define BASE_TOOLS_M4PROCESSING_HPP

#include "basetrade/Tools/timed_data_to_reg_data_common.hpp"

namespace HFSAT {

// bit more uniform ... 5 pointers
void M4Processing(const std::string& input_data_filename_, SimpleLineProcessor& simple_line_processor_,
                  const bool is_returns_based_, const int counters_to_predict_,
                  DailyTradeVolumeReader* trade_vol_reader, bool print_time_) {
  ...
  int event_count8_to_predict_ = counters_to_predict_;
  int event_count7_to_predict_ = std::max(1, (event_count8_to_predict_ / 2));
  int event_count6_to_predict_ = std::max(1, (event_count7_to_predict_ / 2));
  int event_count5_to_predict_ = std::max(1, (event_count6_to_predict_ / 2));
  int event_count4_to_predict_ = std::max(1, (event_count5_to_predict_ / 2));
  int event_count3_to_predict_ = std::max(1, (event_count4_to_predict_ / 2));
  int event_count2_to_predict_ = std::max(1, (event_count3_to_predict_ / 2));
  int event_count1_to_predict_ = std::max(1, (event_count2_to_predict_ / 2));
  ...
  if (input_data_file_base_.is_open() && input_data_file_pred1_.is_open() && input_data_file_pred2_.is_open() &&
    ...
    while (input_data_file_base_.good()) {
      ...
      found = false;
      // now find the first line at or after input_data_file_pred4_ such that event_count_from_midnight >=
      // (base_event_count + event_count4_to_predict_ )
      while (input_data_file_pred4_.good()) {
        ...
        unsigned long long pred4_event_count = atol(pred4_tokens_[1]);
        if (pred4_event_count > (base_event_count + event_count4_to_predict_)) {
          found = true;
      ...
      found = false;
      // now find the first line at or after input_data_file_pred5_ such that event_count_from_midnight >=
      // (base_event_count + event_count5_to_predict_ )
      while (input_data_file_pred5_.good()) {
        ...
        unsigned long long pred5_event_count = atol(pred5_tokens_[1]);
        if (pred5_event_count > (base_event_count + event_count5_to_predict_)) {
          found = true;
      ...
      found = false;
      // now find the first line at or after input_data_file_pred6_ such that event_count_from_midnight >=
      // (base_event_count + event_count6_to_predict_ )
      while (input_data_file_pred6_.good()) {
        ...
        unsigned long long pred6_event_count = atol(pred6_tokens_[1]);
        if (pred6_event_count > (base_event_count + event_count6_to_predict_)) {
          found = true;
      ...
      found = false;
      // now find the first line at or after input_data_file_pred7_ such that event_count_from_midnight >=
      // (base_event_count + event_count7_to_predict_ )
      while (input_data_file_pred7_.good()) {
        ...
        unsigned long long pred7_event_count = atol(pred7_tokens_[1]);
        if (pred7_event_count > (base_event_count + event_count7_to_predict_)) {
          found = true;
      ...
      found = false;
      // now find the first line at or after input_data_file_pred8_ such that event_count_from_midnight >=
      // (base_event_count + event_count8_to_predict_ )
      while (input_data_file_pred8_.good()) {
        ...
        unsigned long long pred8_event_count = atol(pred8_tokens_[1]);
        if (pred8_event_count > (base_event_count + event_count8_to_predict_)) {
          found = true;
