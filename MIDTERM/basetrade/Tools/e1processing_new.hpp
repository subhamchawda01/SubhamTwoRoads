...
#ifndef BASE_TOOLS_E1PROCESSINGNEW_HPP
#define BASE_TOOLS_E1PROCESSINGNEW_HPP

namespace HFSAT {
class E1Processing_new : public Processing {
 protected:
  std::ifstream input_data_file_pred_;

 public:
  E1Processing_new(const std::string& input_data_filename_, SimpleLineProcessor& _simple_line_processor_,
  ...
  void Process(int _base_msecs_, int _base_events_, bool& were_files_reset_, int& _last_good_offset_, bool& _found_,
    ...
    if (input_data_file_pred_.is_open()) {
      ...
      while (input_data_file_pred_.good()) {
        ...
        int pred_events = atoi(pred_tokens_[0]);
        if (pred_events > (base_events + msecs_to_predict_)) {
          _found_ = true;
          ...
          double factor =
              trade_vol_reader == NULL ? 1.0 : trade_vol_reader->getTradedVolRatioAtTime(_base_msecs_ / 1000);
