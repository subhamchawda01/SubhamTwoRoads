...
int main(int argc, char** argv) {
  ...
  std::string ors_data_file_;
  if (argc < 4) {
    std::cout << "Usage : SHORTCODE YYYYMMDD FILENAME" << std::endl;
    std::cout << "Usage : SHORTCODE YYYYMMDD FILENAME SUMMARY " << std::endl;
    std::cout << "Usage : SHORTCODE YYYYMMDD FILENAME PLOTTABLE " << std::endl;
    exit(0);
  } else {
    shortcode_ = std::string(argv[1]);
    yyyymmdd_ = atoi(argv[2]);
    ors_data_file_ = std::string(argv[3]);
    if (argc >= 5) {
      if (!strcmp("SUMMARY", argv[4]))
        seqd_conf_summary_ = true;
      else if (!strcmp("PLOTTABLE", argv[4]))
  ...
  common_logger.processMsgRecvd(ors_data_file_);
}
