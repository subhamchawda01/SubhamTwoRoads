...
int main(int argc, char** argv) {
  ...
  HFSAT::PcaWeightsManager::SetUniqueInstance(date);
  ...
  if (HFSAT::SecurityDefinitions::GetUniqueInstance(date).IsValidContract(shc.c_str())) {
    ...
    // could be portfolio
    std::vector<std::string> portfolio_shortcodes;
    HFSAT::PcaWeightsManager::GetUniqueInstance().GetPortfolioShortCodeVec(shc, portfolio_shortcodes);
    for (auto i = 0u; i < portfolio_shortcodes.size(); ++i) {
      bool hasData = checkForData(portfolio_shortcodes[i], date);
      if (!hasData) {
        std::cout << "0\n";
        return 0;
      }
