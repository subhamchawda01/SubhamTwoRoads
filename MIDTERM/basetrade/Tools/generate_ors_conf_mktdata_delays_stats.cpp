...
class ORSBinReader {
 public:
  ORSBinReader(std::string _shortcode_, int _yyyymmdd_, std::vector<unsigned long>& _mds_delta_events_time_,
               std::vector<struct DeltaInfo>& _mds_delta_packets_)
      : shortcode_(_shortcode_),
        yyyymmdd_(_yyyymmdd_),
        mds_delta_events_time_(_mds_delta_events_time_),
        mds_delta_packet_(_mds_delta_packets_),
        last_vector_index_for_mds_event_(0) {}

  void MatchMarketUpdateForDelta(HFSAT::ttime_t delta_time_, double delta_price_, HFSAT::TradeType_t buysell_,
                                 int qty_) {
    ...
    for (; last_vector_index_for_mds_event_ < (int)mds_delta_events_time_.size(); last_vector_index_for_mds_event_++) {
      int this_tsec_ = delta_time_.tv_sec;
      int this_usec_ = delta_time_.tv_usec;
      ...
      if (this_qty_diff_ < 0) {  // just not taking into account might actually be our order
        last_delta_price_ = mds_delta_packet_[last_vector_index_for_mds_event_].delta_price_;
      ...
      if (unmatched_) {
        if (mds_delta_events_time_[last_vector_index_for_mds_event_] - this_delta_time_ > MAX_DELAY_LIMIT) {
          ...
          std::cerr << " Our Conf : " << (unsigned long)this_delta_time_ << " " << this_delta_time_ << " "
                    << (int)buysell_ << " " << qty_ << "\t";
      ...
      if (((int)(mds_delta_packet_[last_vector_index_for_mds_event_].delta_price_ * 1000) ==
        ...
        double ors_delta_time_ = (this_delta_time_) / (double)(1000000);
        double mds_delta_time_ = (mds_delta_events_time_[last_vector_index_for_mds_event_]) / (double)(1000000);
        unsigned int delay_usec_ = delta_market_update_time_;

        std::cerr << " Our Conf : " << (unsigned long)this_delta_time_ << " " << delta_price_ << " " << (int)buysell_
                  << " " << qty_ << "\t";
        std::cerr << " Exch Delta : " << mds_delta_events_time_[last_vector_index_for_mds_event_] << " "
                  << mds_delta_packet_[last_vector_index_for_mds_event_].delta_price_ << " "
                  << "0"
                  << " " << mds_delta_packet_[last_vector_index_for_mds_event_].delta_qty_ << "\t";
        std::cerr << " Delay :  " << delta_market_update_time_ << "\n";

        mds_delta_packet_[last_vector_index_for_mds_event_].delta_qty_ -= qty_;

        printf("%11.6f %11.6f %u\n", ors_delta_time_, mds_delta_time_, delay_usec_);
        ...
        std::cerr << " Unmatched Our Conf : " << (unsigned long)this_delta_time_ << " " << delta_price_ << " "
                  << (int)buysell_ << " " << qty_ << "\n";
      }
      last_delta_price_ = mds_delta_packet_[last_vector_index_for_mds_event_].delta_price_;
      last_delta_size_ = mds_delta_packet_[last_vector_index_for_mds_event_].delta_qty_;
  ...
  void dumpStats() {
    ...
    std::sort(delta_mktupdate_delay_vec_.begin(), delta_mktupdate_delay_vec_.end());

    for (auto i = 0u; i < delta_mktupdate_delay_vec_.size(); i++) {
      total_time_ += delta_mktupdate_delay_vec_[i];
    }

    std::cout << " Sample Size : " << delta_mktupdate_delay_vec_.size() << " "
              << " 10 % : " << delta_mktupdate_delay_vec_[delta_mktupdate_delay_vec_.size() * 10 / 100] << " "
              << " 50 % : " << delta_mktupdate_delay_vec_[delta_mktupdate_delay_vec_.size() / 2] << " "
              << " 75 % : " << delta_mktupdate_delay_vec_[delta_mktupdate_delay_vec_.size() * 3 / 4] << " "
              << " 90 % : " << delta_mktupdate_delay_vec_[delta_mktupdate_delay_vec_.size() * 9 / 10] << " "
              << " 95 % : " << delta_mktupdate_delay_vec_[delta_mktupdate_delay_vec_.size() * 95 / 100] << " "
              << " 99 % : " << delta_mktupdate_delay_vec_[delta_mktupdate_delay_vec_.size() * 99 / 100] << " "
              << " Mean : " << (unsigned long)(total_time_ / delta_mktupdate_delay_vec_.size()) << "\n";
  }
  ...
  void processMsgRecvd() {
    ...
    std::cerr << " Total Delta Events : " << mds_delta_packet_.size() << "\n";
    ...
    if (bulk_file_reader_.is_open()) {
      while (true) {
        ...
        if (reply_struct.orr_type_ == HFSAT::kORRType_Conf) {
          HFSAT::ttime_t this_delta_time_ = reply_struct.time_set_by_server_;
          double delta_price_ = reply_struct.price_;
          HFSAT::TradeType_t buysell_ = reply_struct.buysell_;
          int delta_size_ = reply_struct.size_executed_;

          MatchMarketUpdateForDelta(this_delta_time_, delta_price_, buysell_, delta_size_);
        }
      }
    ...
    switch (exch_src_) {
      ...
      case HFSAT::kExchSourceHONGKONG: {
        std::string t_hkex_file_name_ =
            HFSAT::HKEXLoggedMessageFileNamer::GetName(t_exchange_symbol_, tradingdate_, trading_location_file_read_);
      ...
      case HFSAT::kExchSourceHKOMDPF: {
        std::string t_hkomd_file_name_ = HFSAT::HKOMDCPFLoggedMessagefileNamer::GetName(
            t_exchange_symbol_, tradingdate_, trading_location_file_read_);
        HKOMD_MDS::HKOMDPFCommonStruct next_event_;
        bulk_file_reader_.open(t_hkomd_file_name_);
        if (!bulk_file_reader_.is_open()) {
          std::cerr << " Could not open : " << t_hkomd_file_name_ << " to Compute Accurate Conf to ORS data \n";
          return;
        }
        while (true) {
          ...
          if (available_len_ >= sizeof(HKOMD_MDS::HKOMDPFCommonStruct)) {
            if (next_event_.msg_ == HKOMD_MDS::HKOMD_PF_DELTA) {
              ...
              mds_delta_events_time_.push_back(this_time_);
              struct DeltaInfo this_delta_info_;
              this_delta_info_.delta_price_ = next_event_.data_.delta_.price_;
              this_delta_info_.delta_qty_ = next_event_.data_.delta_.quantity_;
              this_delta_info_.buysell_ =
                  (0 == next_event_.data_.delta_.side_) ? HFSAT::kTradeTypeBuy : HFSAT::kTradeTypeSell;
              mds_delta_packet_.push_back(this_delta_info_);
            }
          } else {
            break;
...
int main(int argc, char** argv) {
  ...
  std::vector<unsigned long> mds_delta_events_time_;
  std::vector<struct DeltaInfo> mds_delta_events_vec_;

  MDSTradeEvents::ReadMDSTradeEvents(shortcode_, tradingdate_, mds_delta_events_time_, mds_delta_events_vec_);
  ORSBinReader ors_bin_reader_(shortcode_, tradingdate_, mds_delta_events_time_, mds_delta_events_vec_);
