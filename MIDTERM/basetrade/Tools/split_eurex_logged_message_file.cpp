...
int main(int argc, char** argv) {
  ...
  // get timeval corresponding to tradingdate_ midnight ( CET )
  HFSAT::ttime_t base_tv_(HFSAT::DateTime::GetTimeMidnightCET(tradingdate_), 0);
  ...
  while (bulk_file_reader_.is_open()) {
    ...
    bulk_file_reader_.read(&next_event_, sizeof(EUREX_MDS::EUREXCommonStruct));
    ...
    switch (next_event_.msg_) {
      case EUREX_MDS::EUREX_DELTA: {
        memcpy(char16_symbol_, next_event_.data_.eurex_dels_.contract_, EUREX_MDS_CONTRACT_TEXT_SIZE);
        ...
        p_bulk_file_writer_->Write(&next_event_, sizeof(EUREX_MDS::EUREXCommonStruct));
        p_bulk_file_writer_->CheckToFlushBuffer();

      } break;
      case EUREX_MDS::EUREX_TRADE: {
        memcpy(char16_symbol_, next_event_.data_.eurex_trds_.contract_, EUREX_MDS_CONTRACT_TEXT_SIZE);
