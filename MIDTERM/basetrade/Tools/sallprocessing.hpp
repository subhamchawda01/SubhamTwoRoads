/**
   ...
   \author: (c) Copyright Two Roads Technological Solutions Pvt Ltd 2011
    Address:
         ...
         +91 80 4190 3551
*/
#ifndef BASE_TOOLS_SALLPROCESSING_HPP
#define BASE_TOOLS_SALLPROCESSING_HPP

#include "basetrade/Tools/timed_data_to_reg_data_common.hpp"

namespace HFSAT {

void SAllProcessing(const std::string& input_data_filename_, SimpleLineProcessor& simple_line_processor_,
                    const bool is_returns_based_, const int msecs_to_predict_,
  ...
  int msecs8_to_predict_ = 512;  // Kept constant for the time being
  ...
  if (input_data_file_base_.is_open() && input_data_file_pred1_.is_open() && input_data_file_pred2_.is_open() &&
    ...
    while (input_data_file_base_.good()) {
      ...
      if (found) {  // need not check "found" again since break would have been called otherwise
        ...
        // print multiple dependent columns
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred1_value_) / base_value_)
                                                           : (factor * pred1_value_));
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred2_value_) / base_value_)
                                                           : (factor * pred2_value_));
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred3_value_) / base_value_)
                                                           : (factor * pred3_value_));
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred4_value_) / base_value_)
                                                           : (factor * pred4_value_));
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred5_value_) / base_value_)
                                                           : (factor * pred5_value_));
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred6_value_) / base_value_)
                                                           : (factor * pred6_value_));
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred7_value_) / base_value_)
                                                           : (factor * pred7_value_));
        simple_line_processor_.AddWord((is_returns_based_) ? (factor * (pred8_value_) / base_value_)
                                                           : (factor * pred8_value_));
