...
int main(int argc, char** argv) {
  if (argc < 3) {
    ...
    std::cout << "Output : Span+Exposure" << std::endl;
    exit(0);
  }
  ...
  if (HFSAT::FileUtils::ExistsAndReadable(filename)) {
	...
	if (tokens.size() > 1) {
         ...
         // std::cout << shortcode << std::endl;
          ...
          std::string underlying = shortcode;
  ...
  for(it=underlying_to_shortcode_position_vec_pair.begin();it!=underlying_to_shortcode_position_vec_pair.end();it++){
	...
	std::vector<int> position_vec  = it->second.second;
  	...
  	exposure_margin = nse_span_calc.GetExposureMargin(shortcode_vec, position_vec, GetInstrumentFromShortcode(underlying));
  
  	std::cout << underlying<<" "<<span_risk+exposure_margin<<std::endl;
  }
  return 0;
}
