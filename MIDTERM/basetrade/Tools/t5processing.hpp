/**
   ...
   \author: (c) Copyright Two Roads Technological Solutions Pvt Ltd 2011
    Address:
         ...
         +91 80 4190 3551
*/
#ifndef BASE_TOOLS_T5PROCESSING_HPP
#define BASE_TOOLS_T5PROCESSING_HPP

#include "basetrade/Tools/timed_data_to_reg_data_common.hpp"

namespace HFSAT {

void T5Processing(const std::string& input_data_filename_, SimpleLineProcessor& simple_line_processor_,
  ...
  int msecs1_to_predict_ = std::max(1, (msecs_to_predict_ * 1) / 5);
  int msecs2_to_predict_ = std::max(1, (msecs_to_predict_ * 2) / 5);
  int msecs3_to_predict_ = std::max(1, (msecs_to_predict_ * 3) / 5);
  int msecs4_to_predict_ = std::max(1, (msecs_to_predict_ * 4) / 5);
  int msecs5_to_predict_ = msecs_to_predict_;
