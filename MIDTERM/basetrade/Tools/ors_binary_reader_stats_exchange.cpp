    ...
    if ( bulk_file_reader.is_open () ) {
      while ( true ) {
       ...
       HFSAT::GenericORSReplyStruct reply_struct;
        ...
        size_t read_length = bulk_file_reader.read(reinterpret_cast<char*>(&reply_struct), sizeof(HFSAT::GenericORSReplyStruct));
        if (read_length < sizeof(HFSAT::GenericORSReplyStruct)) break ;
...
class OverallExchangeStatsLive {
...
int main ( int argc, char *argv[] )
{
  ...
  if ( argc < 6 ) {
    std::cerr << "USAGE : <server> <saci-base> <srv-freq> <date> <live(optional)> <file/s>" << std::endl ;
    std::exit(-1);
  }
  ...
  if (type == "live") {
    std::cout << "live_struct" << std::endl;
    for ( int32_t counter = 6 ; counter < argc ; counter++ ) {
      files_list_vec.push_back(argv[counter]) ;
      FileStatsLive statslive;
      statslive.ComputeAndDumpStats(argv[counter]) ;
    }

    OverallExchangeStatsLive overall_exchange_stats_live;
    overall_exchange_stats_live.ComputeAndDumpStats () ;
  }
  else {
    std::cout << "nolive_struct" << std::endl;
