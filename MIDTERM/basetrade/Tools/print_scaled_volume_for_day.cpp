...
class TestVolume : public HFSAT::RecentScaledVolumeListener {
  HFSAT::SecurityMarketView* this_smv_;
  HFSAT::DebugLogger& dbglogger_;
  HFSAT::Watch& watch_;
  int duration_;
  HFSAT::RecentScaledVolumeCalculator& scaled_volume_calculator_;
 ...
 public:
  TestVolume(HFSAT::SecurityMarketView* _this_smv_, HFSAT::DebugLogger& _dbglogger_, HFSAT::Watch& _watch_,
      ...
      : this_smv_(_this_smv_),
        ...
        scaled_volume_calculator_(
            *(HFSAT::RecentScaledVolumeCalculator::GetUniqueInstance(dbglogger_, watch_, *this_smv_, duration_))) {
    scaled_volume_calculator_.AddRecentScaledVolumeListener(1, this);
  }
  void OnScaledVolumeUpdate(const unsigned int r_security_id_, const double& r_new_scaled_volume_) {
    if (r_security_id_ == 1) std::cout << watch_.tv() << " " << r_new_scaled_volume_ << std::endl;
  }
};
class TestVolumePort : public HFSAT::RecentScaledVolumeListener {
  std::string this_port_;
  HFSAT::DebugLogger& dbglogger_;
  HFSAT::Watch& watch_;
  int duration_;
  HFSAT::RecentScaledVolumeCalculatorPort& scaled_volume_calculator_;
 ...
 public:
  TestVolumePort(std::string _this_port_, HFSAT::DebugLogger& _dbglogger_, HFSAT::Watch& _watch_, int _duration_)
      : this_port_(_this_port_),
        ...
        duration_(_duration_),
        scaled_volume_calculator_(
            *(HFSAT::RecentScaledVolumeCalculatorPort::GetUniqueInstance(dbglogger_, watch_, this_port_, duration_))) {
    scaled_volume_calculator_.AddRecentScaledVolumeListener(1, this);
  }
  void OnScaledVolumeUpdate(const unsigned int r_security_id_, const double& r_new_scaled_volume_) {
    if (r_security_id_ == 1) std::cout << watch_.tv() << " " << r_new_scaled_volume_ << std::endl;
