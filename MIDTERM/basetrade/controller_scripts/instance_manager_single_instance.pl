...
#to check if the task is already running
      ...
      `awk 'NR==1{store=\$0;next}1;END{print store}' $jobq_file > $jobq_file_backup ; mv $jobq_file_backup $jobq_file`;      #Takes the task to the end of queue if it is was already running
    }
  }
  return "";
}
