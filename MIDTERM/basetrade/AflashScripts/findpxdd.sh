...
if [ $# -lt 7 ]; then echo "$0 <shortcode> <event_dat_file> <event_time> <start_time> <end_time> <fld_path> <tm_dur_min>"; exit; fi
...
for dt in `tail -n +2 $event_datf | cut -d, -f1`; do
  ...
  echo ~/basetrade_install/bin/datagen $shc_px_ilist $dt $stt $ett $uniqid $tmpdat 200 0 0 1 ; 
  ~/basetrade_install/bin/datagen $shc_px_ilist $dt $stt $ett $uniqid $tmpdat 200 0 0 1 ; 
...
scaleR=$HOME"/basetrade/AflashScripts/findPriceDD.R";
outF=$wdir2"/pxdd.dat";
