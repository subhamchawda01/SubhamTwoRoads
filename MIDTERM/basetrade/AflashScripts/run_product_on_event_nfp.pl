  ...
  foreach my $cline_ ( @config_file_lines_ )
  {
    ...
    if ( ! $current_param_ ) {
      ...
      given ( $current_param_ ) {
        ...
        when ( "REVISED_EVENT_NAMES" ) {
          $cline_ =~ s/^\s+|\s+$//g;
          if ( ! FindItemFromVec ( $cline_, @event_names_ ) ) {
            push ( @event_names_, $cline_ );
          }
          $revised_events_{ $cline_ } = 1;
        }
        when ( "EVENT_ID" ) {
...
sub LoadEventMapFXS
{
  ...
  print "Revised Datum: ".$_.":".$revised_event_name_to_datumid_{$_}."\n" foreach keys %revised_event_name_to_datumid_;
  print "Revised Scale: ".$_.":".$revised_event_name_to_scale_{$_}."\n" foreach keys %revised_event_name_to_scale_;
}
...
sub GenerateEventData
{
  ...
  my @evlines_ = `cat $evdat_file_tmp_`; chomp ( @evlines_ );
  my $header_ = shift @evlines_;

  my @dates_ = ( );
  my %date_to_time_ = ( );
  my %date_to_estimate_ = ( );
  my %date_to_release_ = ( );
  my %date_to_prior_ = ( );
  my %date_to_revised_ = ( );
  my %date_to_latency_ = ( );
  foreach my $line_ ( @evlines_ ) {
    ...
    my $date_ = $tokens_[0];
    $date_to_time_{ $date_ } = $tokens_[1];
    $date_to_estimate_{ $date_ } = $tokens_[2] if $#tokens_ >= 2;
    $date_to_release_{ $date_ } = $tokens_[3] if $#tokens_ >= 3;
    $date_to_prior_{ $date_ } = $tokens_[4] if $#tokens_ >= 4;
    $date_to_revised_{ $date_ } = $tokens_[5] if $#tokens_ >= 5;
    push ( @dates_, $date_ );
  }
  @dates_ = grep { $_ > 20140901 } @dates_;
  ...
  foreach my $date_ ( @dates_ ) {
    ...
    next if ($ev_values_[0] / 1000 < 0 || $ev_values_[0] / 1000 > 300);

    $date_to_latency_{ $date_ } = $time_ - GetUnixtimeFromUTC( $date_, $date_to_time_{ $date_ } );
    $date_to_release_{ $date_ } = $ev_values_[0] / 1000;
    $date_to_revised_{ $date_ } = $ev_values_[1] / 1000;
    if ( defined $last_mm_ && defined $last_release_ && $last_mm_ % 12 == $mm_-1 ) {
      $date_to_prior_{ $date_ } = $last_release_ / 1000;
    }
    $last_mm_ = $mm_;
    $last_release_ = $ev_values_[0];
  }

  print "Latency: ".$_.": ".$date_to_latency_{ $_ }."\n" foreach keys %date_to_latency_;
  ...
  print DHANDLE $header_."\n";
  foreach ( @dates_ ) {
    print DHANDLE $_.",".$date_to_time_{$_}.",".$date_to_estimate_{$_}.",".$date_to_release_{$_}.",".$date_to_prior_{$_}.",".$date_to_revised_{$_}."\n";
  }
  close DHANDLE;
}
...
sub FindProductBetas
{
  ...
  my $time_min_bef_ = $time_minutes_ - 20;
  ...
  my %shc_to_pxchange_ = ( );
  foreach my $shc_ ( @shortcode_list_ ) {
    my $gendatg_cmd_ = $AFLASHSCRIPTS_DIR."/findpxch.sh $shc_ $evdat_file_ $event_time_ $time_bef_  $time_aft_ ".$working_dir_." 2 5 10 20";
    print $gendatg_cmd_."\n";
  ...
  my @revised_idx_ = grep { exists $revised_events_{ $event_names_[ $_-1 ] } } 1..(1+$#event_names_);
  my @revised_event_names_ = grep { exists $revised_events_{ $_ } } @event_names_;

  my @evid_list_ = map { $event_name_to_datumid_{ $_ } } @event_names_;
  my @rev_evid_list_ = map { $revised_event_name_to_datumid_{ $_ } } @revised_event_names_;
  push ( @evid_list_, @rev_evid_list_ );

  my @evscale_list_ = map { $event_name_to_scale_{ $_ } } @event_names_;
  my @rev_evscale_list_ = map { $revised_event_name_to_scale_{ $_ } } @revised_event_names_;
  push ( @evscale_list_, @rev_evscale_list_ );

  print "Revised EventNames: ".join("\n", @revised_event_names_)."\n";

  foreach my $shc_ ( @shortcode_list_ ) {
    my @percentiles_ = (0.75, $aggressive_threshold_);
    my $findscale_cmd_ = $AFLASHSCRIPTS_DIR."/findScale.R $evdat_file_ $shc_ ".$shc_to_pxchange_{ $shc_ }." ".join(",", @percentiles_)." ".join(",", @revised_idx_);
