...
sub GenerateEventData
{
  ...
  if ( $event_names_[0] eq "EIA Crude Oil Stocks change" ) {
    $exec_cmd_ = $AFLASHSCRIPTS_DIR."/generate_event_dat_crude.R $evdat_file_ $event_time_";
  } else {
    $exec_cmd_ = $AFLASHSCRIPTS_DIR."/generate_event_dat.R $db_ $evdat_file_ $event_time_ ".join(" ", map { "\"$_\"" } @event_names_);
  }
  print $exec_cmd_."\n";
  `$exec_cmd_`;

  if ( defined $skip_dates_file_ ) {
    my @datvec_ = `cat $evdat_file_`; chomp( @datvec_ );

    my @skip_dates_ = `cat $skip_dates_file_`; chomp ( @skip_dates_ );
    my %skip_dates_map_ = map { $_ => 1 } @skip_dates_;

    my @datvec_filt_ = grep { ! defined $skip_dates_map_{ (split(',', $_))[0] } } @datvec_;
