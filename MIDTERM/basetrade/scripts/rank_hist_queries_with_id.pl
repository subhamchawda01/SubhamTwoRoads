...
# \file scripts/rank_hist_queries_with_id.pl
#
...
my $end_yyyymmdd_ = GetIsoDateFromStr ( $ARGV[3] );
...
for ( my $i = 0 ; $i <= $#result_lines_; $i ++ )
{
	...
	if ( $volume_ > 0 )
	{
	    ...
	    my $t_stratbase_id_ = $stratbase_parts_[$#stratbase_parts_];

	    if ( ( $#stratbase_parts_ >= 1 )
		 && ( ( $stratbase_parts_ [ $#stratbase_parts_ ] > 0 ) && 
		      ( $stratbase_parts_ [ $#stratbase_parts_ ] < 100000 ) ) )
	    {
		...
		$stratbase2stratid_{$stratbase_} = $t_stratbase_id_;
	    }
...
{
    printf "%d %d %d %s %d\n", $stratbase2pnlsum_{$stratbase_}, $stratbase2volsum_{$stratbase_}, $stratbase2stratid_{$stratbase_}, $stratbase_, $stratbase2count_{$stratbase_};
}
