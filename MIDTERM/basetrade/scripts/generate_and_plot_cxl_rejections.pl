...
# \file scripts/generate_and_plot_cxl_rejections.pl
#
...
my $GET_GRP_EXEC = $LIVE_BIN_DIR."/get_cxl_rejection_periods";
...
my $exec_cmd_ = "$GET_GRP_EXEC $shortcode_ $yyyymmdd_";
if ( $USER eq "sghosh" && $to_mail_ == 1 )
{ # Most likely a cron job.
    my $exec_cmd_ = "$GET_GRP_EXEC $shortcode_ $yyyymmdd_ /home/sghosh/master/temp";
...
if ( -e $output_filename_ )
{
    ...
    my $t_all_file_name_ = $temp_dir_."/grp.".$shortcode_.".".$yyyymmdd_;
    `grep \"GetTimeToNumCxlRejections\" $output_filename_ | awk '{ print \$2\" \"\$3; }' > $t_all_file_name_`;

    if ( $to_plot_ == 1 )
    {
	`$PLOT_SCRIPT $t_all_file_name_ 2 $shortcode_.$yyyymmdd_.grp NL`;
    }

    if ( $to_mail_ == 1 )
    {
	my $t_output_png_ = $temp_dir_."/plot.".$shortcode_.".".$yyyymmdd_.".png";
	`$SAVEPLOT_SCRIPT $t_all_file_name_ 2 $shortcode_.$yyyymmdd_.grp NL`;
    }
