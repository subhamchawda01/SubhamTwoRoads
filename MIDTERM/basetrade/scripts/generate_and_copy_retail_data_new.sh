...
`rm -rf $tmp_dir`;
if [ -s $logfile ]; then
  zcat $logfile | tr '\n' '\r' | sed -e 's/\r / /g' | tr '\r' '\n' | grep "RETAILTradeMessage" | awk '{print $4,$6,$10,$8,$12}' | column -t > $tmp_file;
#zgrep "ProcessORSReply  Struct" $logfile  | awk '{bs="B"; if($21==1){bs="S"}; print $9 ,$5, $27, bs , $7;}' | column -t > $tmp_file; 
  if [ -s $tmp_file ]; then 
    ...
    chmod 755 -R $tmp_dir;
    chmod 755 $tmp_file;
    scp $tmp_file dvcinfra@10.23.74.41:/apps/data/Retail_Trades/ ;
    scp -r $tmp_dir dvcinfra@10.23.74.41:/apps/data/RETAILLoggedData/ ;
  fi;     
fi; 
`rm -rf $tmp_file`;
`rm -rf $tmp_dir`;
