...
# \file scripts/sort_abs_wzscore.pl
#
#
...
if ( $opt_fromfile == 1 )
{
    open TXTFILEHANDLE, "< $txtfile_" or PrintStacktraceAndDie ( "sort_abs_wzscore.pl could not open $txtfile_\n" );
	...
	if ( $#txt_words_ >= $stdev_key_index_ )
	{
		...
		$new_txt_line_with_key_->key_value_ ( abs ( $txt_words_[$key_index_] * ( $txt_words_[$key_index_] / ( max ( 0.01, $txt_words_[$stdev_key_index_] ) ) ) ) ) ;
