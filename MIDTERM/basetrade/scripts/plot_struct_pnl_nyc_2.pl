...
if ( ( -e "$LIVE_BIN_DIR/get_utc_hhmm_str" ) &&
     ( ( $tradingdate_ > 20110101 ) &&
       ( $tradingdate_ < 20140101 ) ) )
{ 
...
while ( my $inline_ = <PNLFILEHANDLE> )
{
	...
	my $numbered_secname_ = "ALL"; # used for pnl
	if ( ( ! ( $arbit_secname_ ) ) &&
	     ( $all_file_needed_ ) )
	{
