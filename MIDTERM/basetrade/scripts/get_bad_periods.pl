...
my $USAGE = "$0 shortcode_1 shortcode_2 shortcode_3 ... shortcode_n output_files_dir days_to_look_behind";
...
for ( my $shortcode_ = 0 ; $shortcode_ <= $#shortcode_list_ ; $shortcode_ ++ ) 
{
	...
	$date_to_bad_periods_ { $current_yyyymmdd_ } = GetBadMfmForShortcodeForDate ( $shortcode_list_ [ $shortcode_ ] , $current_yyyymmdd_ );
    ...
    {
	my $bad_period_file_name_ = $output_files_dir_."/"."bad_periods.".$shortcode_list_ [ $shortcode_ ];
...
#	print "Using precomputed bad-periods for $shortcode_ $yyyymmdd_\n";
    ...
    my $query_id_eu_ = GetRealQueryIdPrefixForShortcodeTimePeriod ( $shortcode_ , "EU" );
    my $query_id_us_ = GetRealQueryIdPrefixForShortcodeTimePeriod ( $shortcode_ , "US" );

    my $t_trades_files_eu_ = "/NAS1/logs/QueryTrades/".$t_yyyy_."/".$t_mm_."/".$t_dd_."/trades.".$yyyymmdd_.".".$query_id_eu_."??";
    my $t_trades_files_us_ = "/NAS1/logs/QueryTrades/".$t_yyyy_."/".$t_mm_."/".$t_dd_."/trades.".$yyyymmdd_.".".$query_id_us_."??";

    my @trades_file_list_ = `ls $t_trades_files_eu_ $t_trades_files_us_ 2>/dev/null`;
