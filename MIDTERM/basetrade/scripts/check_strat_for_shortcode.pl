...
if ( $#ARGV < 1 )
{
  print "USAGE: <script> strat_basename shortcode\n";
  exit(1);
}
...
my $strat_path_ = $strat_name_;
if ( ! -s $strat_path_ )
{
  my $exec_cmd_ = "ls /home/dvctrader/modelling/*strats/*/*/$strat_name_";
  $strat_path_ = `$exec_cmd_`;  chomp($strat_path_);
}

my $model_path_ = `cat $strat_path_ \| awk '{print \$4}'`;
...
while ( my $model_line_ = <MODEL_FILE> )
{
	...
	if ($prod_exch_ eq "" ){
	    ...
	    {
		if($port_constituent eq $shortcode_)
		{
		    printf("Reject at %s \n",$model_words_[3]);
		    close ( MODEL_FILE );
		    exit(0);
	...
	{
	    if($model_words_[3] eq $shortcode_)
	    {
		printf("Reject at %s \n",$model_words_[3]);
		close ( MODEL_FILE );
		exit(0);
	...
	if ( $model_words_[4] =~ /[A-Za-z]/ && !($model_words_[4] =~ /Midprice|MktSinusoidal|MktSizeWPrice|OrderWPrice|TradeWPrice|OfflineMixMMS/ ) && ! ($model_words_[4] =~ /^[-+]?[0-9]*\.?[0-9]+$/) )
	{
	    ...
	    {
		if($model_words_[4] eq $shortcode_)
		{
		    printf("Reject at %s \n",$model_words_[4]);
		    close ( MODEL_FILE );
		    exit(0);
