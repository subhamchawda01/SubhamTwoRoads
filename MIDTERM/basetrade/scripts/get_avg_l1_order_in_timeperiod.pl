...
my $GET_L1_EVENTS_EXEC = $BIN_DIR."/get_avg_l1ord_on_day";
my $GET_L1_SIZE_EXEC = $BIN_DIR."/get_avg_l1sz_on_day";
...
my $USAGE="$0 shortcode start_date end_date start_time end_time printavg_ord_sz [ N ]";
...
my $print_avg_order_size_ = "N";
if ( $#ARGV >= 5 )
{
  $print_avg_order_size_ = $ARGV [ 5 ] ;
}
...
for ( my $date_ = $end_yyyymmdd_ ; $date_ >= $start_yyyymmdd_ ; )
{
    ...
    my $get_l1_events_exec_cmd_ = $GET_L1_EVENTS_EXEC." ".$shortcode_." ".$date_." ".$start_time_." ".$end_time_;
	...
	if ( $t_l1_ > 0 )
	{
          if ( $print_avg_order_size_ eq "Y" )
          {
              my $get_l1_sz_exec_cmd_ = $GET_L1_SIZE_EXEC." ".$shortcode_." ".$date_." ".$start_time_." ".$end_time_;
              my $get_l1_sz_output_ = `$get_l1_sz_exec_cmd_ 2>/dev/null`; chomp ( $get_l1_sz_output_ );
              my @t_l1_sz_out_words_ = split ( ' ' , $get_l1_sz_output_ );
              if ( $#t_l1_sz_out_words_ >= 2 )
              {
                my $t_l1_size_ = $t_l1_sz_out_words_[ $#t_l1_sz_out_words_ ];
                if ( $t_l1_size_ > 0 )
                {
                  my $av_ord_ = sprintf "%0.3f", $t_l1_size_/$t_l1_;
                  push ( @l1_, $av_ord_ );
                  $exch_symbol_ = $t_l1_sz_out_words_ [ 1 ];
                  print "$date_ $shortcode_ $av_ord_ \n";
                }
