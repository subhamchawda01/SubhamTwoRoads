...
if [[ -s $STRATPATH ]]; then $HOME/basetrade/scripts/printModel.pl $STRATPATH;
else 
  STRATPATH=`$HOME/basetrade/scripts/print_pruned_strat_from_base.sh $STRATFILENAME 2>/dev/null`;
  if [[ -s $STRATPATH ]]; then $HOME/basetrade/scripts/printModel.pl $STRATPATH;
