...
# this script is an extension of scripts/get_regime_parameter.sh, given an ilist with regimes it finds the correlations for the indicators splitting it into lower and higher regimes for multiple pred durations; internally it calls get_regdata_multiple.R unlike get_regime_parameter.sh which calls get_regdata.R; otherwise all the functionalities are same to get_regime_parameter.sh
...
reg_data_script="/home/dvctrader/basetrade/scripts/get_regdata_multiple.R";
...
num_regime=`cat $2 | grep -v "#INDICATOR " | grep "INDICATOR " | wc -l` ; #counts lines with INDICATOR in the ilist"
num_regime=`expr $num_regime - $num_indicator`; #gets the number of regimes "

#append all the regime indicators to reg_indicators
reg_indicators=();
...
$reg_data_script $1 $2 $3 $4 $5 $6 $datagen_msecs $events $trades $print_on_eco $pred_durations $pred_algo $filter $work_dir >/dev/null
#./get_regdata.R $1 $2 $3 $4 $5 $6 1000 0 0 1 900 na_e3 fsg1 res/

cd $work_dir

#get pred_duration array 
pred_duration_array=$(echo $pred_durations | tr "," "\n");

#for every predduartion we get the filtered_regdata_filename cut all the indicator columns to temp and run get_dep_corr to get the original correlations for each indicator
#then for each regime it splits the indicator according to the regime indicator value and gets the correaltion for the indicator for each regime
for pred_duration in $pred_duration_array
do
    echo "Pred_duration : $pred_duration";
    delimeter="_";
    filtered_regdata_filename="filtered_regdata_filename_$pred_duration$delimeter$pred_algo$delimeter$filter"
