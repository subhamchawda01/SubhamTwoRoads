...
my $USAGE="$0 SHORTCODE TIMEPERIOD [MIN_VOLUME=0] [MAX_TTC=inf] [STRAT_FILE_NAME=w_strategy_ilist_BR_DOL_0_US_Mkt_Mkt_J0.noeu_8_na_e3_20111206_20120123_EST_800_EST_1400_500_0_0_fsr.5_3_FSHLR_0.01_0_0_0.7.tt_EST_700_EST_1400.pfi_3]";
...
#my @comparison_durations_ = ( 4 , 24 , 90, 210 );
my @comparison_durations_ = ( 0 , 4, 20, 90 );

if ( $USER eq "sghosh" || $USER eq "ravi" )
{
    @comparison_durations_ = ( 0 , 2 , 5 , 7 , 10 , 15 , 20 , 25 , 40 , 70 , 90 , 100 );
}
...
my @running_queries_ = `$SEE_STATS_OF_RUNNING_QUERIES $trading_location_ $shortcode_ 2 $time_period_`;
...
foreach my $duration_ ( @comparison_durations_ )
{
    ...
    my @t_ss_stats_ = `$SS $shortcode_ $time_period_ $duration_ TODAY-1 1 | awk '{ if ( \$2 >= $min_volume_ && \$7 <= $max_ttc_ ) { print \$0; } }' | $SORT_MULTCOL_SQRT 1 2`;
    my @ss_stats_ = reverse ( @t_ss_stats_ );
    my @t_ss_noc_stats_ = `$SS_NOC $shortcode_ $time_period_ $duration_ TODAY-1 1 | awk '{ if ( \$2 >= $min_volume_ && \$7 <= $max_ttc_ ) { print \$0; } }' | $SORT_MULTCOL_SQRT 1 2`;
    my @ss_noc_stats_ = reverse ( @t_ss_noc_stats_ );
	...
	if ( ( $strat_file_name_ ne " " && $strat_file_name_ eq $t_running_strat_name_ ) || 
	     ( $strat_file_name_ eq " " ) )
	{
	    $should_print_ = 1;
	}
