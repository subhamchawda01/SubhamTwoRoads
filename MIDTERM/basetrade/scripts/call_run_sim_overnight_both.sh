...
if [ "$NUMRUNSIMS" -lt "13" ] ; then
	...
	$HOME/basetrade/scripts/call_run_sim_overnight_recent.sh
    fi
    NUML1SIMS=`pidof -x call_run_sim_overnight_longer_1.sh |wc -w`;
    if [ "$NUML1SIMS" -lt "7" ] ; then 
	$HOME/basetrade/scripts/call_run_sim_overnight_longer_1.sh
    fi
    NUML2SIMS=`pidof -x call_run_sim_overnight_longer_2.sh | wc -w`;
    if [ "$NUML2SIMS" -lt "7" ] ; then
	$HOME/basetrade/scripts/call_run_sim_overnight_longer_2.sh
    fi
...
SUBJECT: run_simulations.pl seems to have stuck.

Only $NUMSIMSTRATEGY sim_straegy's are running while $NUMRUNSIMS run_simulations.pl are running. 
