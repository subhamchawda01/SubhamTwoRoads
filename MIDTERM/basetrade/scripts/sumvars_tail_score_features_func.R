...
get_indicator_tail_score_1 = function(df, pred_val_ind, indicator_ind, sum_vars, feature_vec_ind, sd_fac, thres, indicator_name = "",feature_vec_names = rep(" " ,length( feature_vec_ind ) ), print_features = TRUE ){
  source("~/basetrade/scripts/r_utils.R");
 
  indicator_val = df[,indicator_ind];
  
  
  positive_ind = which( df[,pred_val_ind]*sum_vars > 0 );
  negative_ind = which( df[,pred_val_ind]*sum_vars <= 0 );  
    
    
  tail_ind = get_tails( sum_vars, sd_fac  );
  
  x = sum_vars[ tail_ind ]*df[ tail_ind, pred_val_ind ];
  
  positive_tail_ind = which( x > 0);
  negative_tail_ind = which ( x <= 0);
  
  total_score = mean ( indicator_val / sum_vars ) ;
  positive_score =  mean( indicator_val[positive_ind ] / sum_vars [ positive_ind ]  ) ;  
  negative_score =  mean( indicator_val[negative_ind ] / sum_vars [ negative_ind ]  ) ;

  total_cor = cor_m( indicator_val , df[,pred_val_ind] ) ;
  positive_cor =  cor_m( indicator_val[positive_ind ] , df [ positive_ind, pred_val_ind ]  ) ;
  negative_cor = cor_m( indicator_val[negative_ind ], df [ negative_ind, pred_val_ind ]  ) ;


  tail_score =  mean( indicator_val[ tail_ind ] / sum_vars [ tail_ind ] ) ;
  tail_positive_score = mean( indicator_val[ tail_ind ][ positive_tail_ind ] / sum_vars[ tail_ind ][ positive_tail_ind ]  ) ;  
  tail_negative_score = mean( indicator_val[ tail_ind ][ negative_tail_ind ] / sum_vars[ tail_ind ][ negative_tail_ind ]  ) ;

  tail_cor =  cor_m( indicator_val[ tail_ind ] , df [ tail_ind, pred_val_ind ] ) ;
  tail_positive_cor = cor_m( indicator_val[ tail_ind ][ positive_tail_ind ] , df[ tail_ind, pred_val_ind ][ positive_tail_ind ]  ) ;
  tail_negative_cor = cor_m( indicator_val[ tail_ind ][ negative_tail_ind ] , df[ tail_ind, pred_val_ind ][ negative_tail_ind ]  ) ;
 ...
 if( print_features ) {
  ...
  cat(c("total_contribution", total_score , "total_positive_contribution", positive_score, "total_negative_contribution", negative_score  ) );
  cat ( "\n");
  cat(c("total_cor", total_cor , "total_positive_cor", positive_cor, "total_negative_cor", negative_cor  ) );
  cat ( "\n");
  cat(c("tail_contribution", tail_score , "tail_positive_contribution", tail_positive_score, "tail_negative_contribution", tail_negative_score  ) );
  cat ("\n");
  cat(c("tail_cor", tail_cor , "tail_positive_cor", tail_positive_cor, "tail_negative_cor", tail_negative_cor  ) );
  cat ("\n");
  cat ( "\n");
...
get_ilist_tail_score = function (  model_file,feature_ilist, begin_date, end_date, begin_time, end_time, pred_dur, thres = 0, 
  ...
  sum_vars =  rowSums(weighted_indicators_data);  
  
  ind =which( abs(sum_vars) > delta_y_cutoff );
  pred_data = pred_data[ind, ];
  ...
  positive_ind = which( pred_data[,2]*sum_vars > 0 );
  negative_ind = which( pred_data[,2]*sum_vars <= 0 );


  tail_ind = get_tails( sum_vars, sd_fac  );

  x = sum_vars[ tail_ind ]*pred_data[ tail_ind, 2 ];

  positive_tail_ind = which( x > 0);
  negative_tail_ind = which ( x <= 0);

  total_cor = cor_m( sum_vars, pred_data[, 2 ] )   ;
  tail_cor = cor_m( sum_vars[tail_ind], pred_data[ tail_ind, 2 ] )   ;
  ...
  cat("SUM_VARS_STATS" )
  cat( "\n" );
  cat ( c( "total_cor" , total_cor, "tail_cor", tail_cor ) ) 
  cat ( "\n" );
  cat(c( "fraction_of_positive_contribution", ( length(positive_ind) / length(sum_vars ) ) ,"fraction_of_negative_contribution", ( length(negative_ind) / length(sum_vars ) )   ) );
  cat ( "\n" )  
  cat(c("fraction_of_tail", length(tail_ind)/length(sum_vars), "fraction_of_positive_tail", ( length(positive_tail_ind) / length(tail_ind ) ) ,"fraction_of_negative_tail", ( length(negative_tail_ind) / length(tail_ind ) )   ) );
  cat ( "\n" );
  cat ("===================================================================================================================================");
  cat ("\n");
  ...
  for ( i in 3:(3+num_indicators -1 ) ) {
    if( i == (3+num_indicators -1 ) ) { print_features = ( TRUE && feature_file_exists ) ; }
    get_indicator_tail_score_1 (pred_data, 2, i, sum_vars, feature_ind_vec, sd_fac, thres, indicator_name = indicator_names[i-2], feature_names, print_features  );
