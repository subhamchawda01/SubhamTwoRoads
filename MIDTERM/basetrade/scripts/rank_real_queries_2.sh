...
for LOGFILENAME in $TRADELOGDIR/log.$YYYYMMDD.$PREFIX??.gz $TRADELOGDIR/log.$YYYYMMDD.$PREFIX?? ; 
do 
    if [ -e $LOGFILENAME ] ; 
    then 
	REALID=`echo $LOGFILENAME | awk -F\. '{print $3}'`; 
	STRATFILE=`less $LOGFILENAME | head -1`; 
	STRATFILEBASE=`basename $STRATFILE`;
	TRADESFILENAME=$TRADELOGDIR/trades.$YYYYMMDD.$REALID ;
	...
	echo $FinalPNL $FinalVolume $STRATFILEBASE ; 
    fi ; 
done
