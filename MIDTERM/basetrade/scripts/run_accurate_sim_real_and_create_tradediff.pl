...
my $GENERATE_TRADE_DIFF = $SCRIPTS_DIR."/generate_trades_file_diffs.pl";


my $print_strat_exec_cmd = 0;
my $usage_ = "run_accurate_sim_real.pl DATE UNIQUE_ID OUT_FILE SIMCONFIG[INVALID_FILE] [REMOVE_STRAT_FILES=1] [STRAT_FILE_DIR] [PARAM_FILE_DIR] [MODEL_FILE_DIR]";
...
my $diff_out_file_name_ = $ARGV[2];
my $sim_config_file_name_ = "";

if ( $#ARGV > 2 )
{

  if ( index ($ARGV[3], "INVALID") >= 0 )  
  { 
    $sim_config_file_name_ = "";
  }
  else 
  { 
    $sim_config_file_name_ = $ARGV [ 3 ];
  }
  if ( $#ARGV > 3 ) { $to_remove_strat_files_ = $ARGV [ 4 ];}
...
{ # Generate local strategy, model & param file.
	...
	if (index ($query_log_file_lines_[$strategy_line_], "STRATEGYLINE") >= 0) {
	    ...
	    $sim_id_ = "$strategy_info_words_[7]"."$t_" ;
            ...
            $query_id_to_stratline_map_ { $strategy_info_words_[7] } = "$strategy_info_words_[0] $strategy_info_words_[1] $strategy_info_words_[2] $t_model_file_name_ $t_param_file_name_ $sim_start_time_ $sim_end_time_ $strategy_info_words_[7] $sim_config_file_name_ \n"; 
            }
...
{ # Find real pnl & vol.
	...
	if ($#trades_words_ >= 15) {
	    ...
	    $real_pnl_ = $t_pnl_;
            if ( abs ( $real_pnl_ )  > $max_abs_real_pnl_ )
            {
              $max_abs_real_pnl_ = abs ( $real_pnl_ );
            }
    ...
    print "MAXABSREALPNL $max_abs_real_pnl_ \n";
}
...
#calculate the pnl diff stats for sim and real
#
my $generate_trade_diff_exec_cmd_ = $GENERATE_TRADE_DIFF." "."/spare/local/logs/tradelogs/trades.".$yyyymmdd_.".".$sim_id_." ".$trades_file_name_." ".$diff_out_file_name_;
if ( $USER eq "diwakar" ) 
{
  print $generate_trade_diff_exec_cmd_."\n";
}

my $exec_out_ = `$generate_trade_diff_exec_cmd_`;

my $min_max_sim_ = `cat /spare/local/logs/tradelogs/trades.$yyyymmdd_.$sim_id_ | grep -v PNLSAMPLES | awk 'BEGIN{getline;min=max=\$9} NF>9 { max=(max>\$9)?max:\$9; min=(min>\$9)?\$9:min} END {print min, max}'`;
my @min_max_sim_vec_ = split( ' ', $min_max_sim_ );
my $max_abs_sim_pnl_ = 0;
if ( abs ( $min_max_sim_vec_[0] ) > abs ( $min_max_sim_vec_[1] ) )
{
  $max_abs_sim_pnl_ = $min_max_sim_vec_ [0] ;
}
else
{
  $max_abs_sim_pnl_ = $min_max_sim_vec_ [1] ;
}
print "MAXABSSIMPNL $max_abs_sim_pnl_ \n";
