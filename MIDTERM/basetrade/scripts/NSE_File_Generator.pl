...
if ( $#ARGV == 6 ){
	$this_month_expiry_date_ = $ARGV[6];
...
while( my $bfline_ = <FILE> )
{
    ...
    if( $bfline_tokens_[1] eq "OPTSTK" || $bfline_tokens_[1] eq "OPTIDX" )
    {
    ...
    elsif($bfline_tokens_[1] eq "OPTIDX" && $bfline_tokens_[2] eq "BANKNIFTY")
    {
    ...
    elsif( ( $bfline_tokens_[1] eq "FUTIDX" && $bfline_tokens_[2] ne "NIFTY" && $bfline_tokens_[2] ne "BANKNIFTY" ) || 
    ...
    elsif( $bfline_tokens_[1] eq "FUTIDX" && ( $bfline_tokens_[2] eq "NIFTY" || $bfline_tokens_[2] eq "BANKNIFTY" ) )
    {
		$date_ = formatDate( $bfline_tokens_[3] );
		$key_ = $bfline_tokens_[1]."_".$bfline_tokens_[2]."_".$date_;	
		$string_to_close_{ $key_ } = $bfline_tokens_[ 10 ];
    ...
    elsif($bfline_tokens_[2] eq "BANKNIFTY")
    {
      ...
      $string_to_itm_{ $key_ } = 16; #hft we want 6 each side with 500 as multiple
      $string_to_step_{ $key_ } = 100;
    }else 
    {
...
foreach my $string_( keys %string_to_underlying_ )
{
    ...
    elsif($string_to_type_{ $string_ } eq "IDXOPT" && $string_to_underlying_{ $string_ } eq "BANKNIFTY")
    {
        #Weekly Options
        #[pjain] : Currently the code is written assuming that weekly options is only supported for BANKNIFTY.
        my $weekly_option_key_current_mon_future = "FUTIDX_BANKNIFTY_".$string_to_month_{ $string_ };
