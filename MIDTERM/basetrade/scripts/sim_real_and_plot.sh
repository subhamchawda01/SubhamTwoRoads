...
USAGE="$0 PID YYYYMMDD [MKT_MODEL=-1] [TRADEDATE=$YYYYMMDD]";
if [ $# -lt 2 ] ; 
then 
    echo $USAGE;
    exit;
...
REALLOGFILE=/NAS1/logs/QueryLogs/$NEWDIR/log.$YYYYMMDD.$PID.gz
REALTRADESFILE=/NAS1/logs/QueryTrades/$NEWDIR/trades.$YYYYMMDD.$PID
...
#	SIM_EXEC=~/LiveExec/bin/sim_strategy ;
	...
	echo    $SIM_EXEC SIM $SIMSTRATFILENAME $SIMID $TRADEDATE $MKT_MODEL ADD_DBG_CODE -1
	$SIM_EXEC SIM $SIMSTRATFILENAME $SIMID $TRADEDATE $MKT_MODEL ADD_DBG_CODE -1
	rm -f /spare/local/logs/tradelogs/log.$TRADEDATE.$SIMID
	
	SIMTRADESFILE=/spare/local/logs/tradelogs/trades.$TRADEDATE.$SIMID
	if [ -e $SIMTRADESFILE ] ;then
	    ...
	    if [ $TRADEDATE -ne $YYYYMMDD ] ; then
		~/basetrade/scripts/plot_trades_pnl_nyc_2_all.sh $SIMTRADESFILE # $REALTRADESFILE
	    else
		~/basetrade/scripts/plot_trades_pnl_nyc_2_all.sh $SIMTRADESFILE $REALTRADESFILE
	    fi
	    rm -f $SIMTRADESFILE
