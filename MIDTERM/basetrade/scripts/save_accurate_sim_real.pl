...
my $usage_ = "save_accurate_sim_real.pl DATE UNIQUE_ID [REMOVE_STRAT_FILES=1] [STRAT_FILE_DIR] [PARAM_FILE_DIR] [MODEL_FILE_DIR]";
...
{ # Run SIM Strategy and get SIM pnl & vol.
    ...
    my $exec_cmd="$sim_exec_ SIM $sim_strat_file_name_ $sim_id_ $yyyymmdd_ $mkt_model_ ADD_DBG_CODE -1";

    if ( $print_strat_exec_cmd || $USER eq "sghosh" || $USER eq "dvctrader" )
    {
	print "$exec_cmd\n";
    }
...
if ( $USER eq "sghosh" )
{
    ...
    my $plot_cmd_ = $SCRIPTS_DIR."/saveplot_multifile_cols.pl /spare/local/logs/tradelogs/trades.$yyyymmdd_.$sim_id_ 9 $yyyymmdd_.$sim_id_ WL $trades_file_name_ 9 $yyyymmdd_.$unique_id_ WL";
    ...
    my $output_filename_ = $HOME_DIR."/master/temp/".$yyyymmdd_.".".$dep_shortcode_.".".$unique_id_.".png";
    $plot_cmd_ = $plot_cmd_." ".$output_filename_;
