...
my @pred_durations_ = (2,10,30,60,120,300,600, 900, 1800, 3600);
...
`$cmd_`;
$cmd_ = "echo \"INDICATOR 1.00 L1Price $dep_shortcode_ MktSizeWPrice\" >> $indicator_list_";
`$cmd_`;
...
{
	$stdev_file_ = "/spare/local/tradeinfo/datageninfo/returns_stdev_$dep_shortcode_"."_$time_period_";
}
...
for ( my $days_ = $days_to_look_behind_ ; $days_ != 0 ; $days_ -- ) 
{
   ...
   foreach my $duration_ (@pred_durations_)
   {
	...
	`$cmd_`;
	$cmd_ = "cat $tmp_reg_data_filename_ | awk '{sum1+=(\$1/\$2); sum2+=((\$1 * \$1)/(\$2*\$2));}END{ print sqrt(sum2/NR - (sum1*sum1)/(NR*NR)) }'";
