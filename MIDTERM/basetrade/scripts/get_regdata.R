...
## match order of dates to order intra day time series
list_of_insample_days <<- rev(list_of_insample_days)
}


GetDataForRegression <- function (list_of_days)
{
	...
	catted_regdata_filename_withdates <- paste( work_dir, "catted_regdata_outfile_withdates", sep="" );

	for ( trading_date in list_of_days )
	{	    
        ...
        aggregation_command <- "cat %s >> %s"
        aggregation_command_withdates <- paste0("cat %s | awk '{print $0,",trading_date,"}' >> %s")



        system ( sprintf ( aggregation_command, t_regdata_outfile, catted_regdata_filename ) );
        system ( sprintf ( aggregation_command_withdates, t_regdata_outfile, catted_regdata_filename_withdates ) );
        ...
        system ( sprintf ( "%s %s %s %s %s %s", apply_dep_filter_script, shortcode, catted_regdata_filename_withdates, filter, filtered_regdata_filename_withdates, start_date ) );
...
if ( length(args) < 14 ) {
        stop ("USAGE : <script> <shortcode> <ilist> <start_date> <num_days_lookback> <start_time> <end_time> <dgen_msecs> <dgen_l1events> <dgen_trades> <to_print_on_eco> <pred_duration> <pred_algo> <filter> <work_dir> <compute_sum_vars[0/1] = 0> <list of dates to get data for e.g. '20170428 20170429' > \n");
}
...
filtered_regdata_filename_withdates <- paste(work_dir, "filtered_regdata_filename_withdates", sep="");
...
# if list of dates if specified use that instead
if(length(args) > 15)
{
  list_of_days <- strsplit(args[16]," ")[[1]] ;
} else { GetListOfDates(); list_of_days <- list_of_insample_days ; }
...
GetDataForRegression (list_of_days);
