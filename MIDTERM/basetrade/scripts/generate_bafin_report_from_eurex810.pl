...
my $USAGE = "$0 date shortcode <output_dir>";
...
my $output_filename_  = "$output_dir/bfr_".$shc_."_".$yyyymmdd_;
...
if ( -r $newedge_file_ )
{
    $nexp_ =  `awk -F\, '{ if ( \$24 == \"\\\"27\\\"\"  && \$25 == \"\\\"$newedge_symbol_{ $shc_}\\\"\" ) print \$23 ;  }'  $newedge_file_  | uniq`;
    chomp ( $nexp_ ) ;
    $nexp_ =~ s/\"//g;
}
else
{
    $msg = $msg."couldn't find the newedge file or it is unreadable, calculating expiry using get_eurex_product_expiry\n";
}
...
{
    my $result_ =  `wc -l $output_filename_`;
    $msg = $msg. "no of transactions: ".$result_;
}
else
{
