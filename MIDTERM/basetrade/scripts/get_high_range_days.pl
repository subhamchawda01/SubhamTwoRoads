...
#VX_1 VX201603 L1Events 2.19373e+06 AvgL1Size 91.8856 NumTrds 6893 Volume 24888 AvgTrdSz 3.61062 HTrdPx 24.5 LTrdPx 22.35 TrdSizeWPrice 23.6701
	...
	my @high_tr_px_ = split (' ', $output_[6]);
	my @low_tr_px_ = split (' ', $output_[7]);
	if ( $#high_tr_px_ < 1 || $#low_tr_px_ < 1 )
	{
	    $date_to_info_{$current_yyyymmdd_} = "-1";
	    $current_yyyymmdd_ = CalcPrevWorkingDateMult ($current_yyyymmdd_, 1);
	    next;
	...
	my $atp_ = $high_tr_px_[1] - $low_tr_px_[1] ; chomp ($atp_);
