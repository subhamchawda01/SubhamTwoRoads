...
# \file scripts/call_run_sim_overnight_perdir_longer_stir.pl
#
...
my $MODELING_STRATS_DIR=$MODELING_BASE_DIR."/stir_strats"; # this directory is used to store the chosen strategy files
...
my $USAGE="$0 shortcode [skip_recent_days=4] [num_past_days=70] [compute_individual_res=0]";
my $is_lock_created_ = 0;

my $skip_recent_days_ = 4;
my $num_working_days_ = 70;
my $compute_individual_stats_ = 0 ;

if ( $#ARGV < 0 ) { print $USAGE."\n"; exit ( 0 ); }
my $shortcode_ = $ARGV[ 0 ];
if ( $#ARGV >= 1 ) { $skip_recent_days_ = $ARGV[ 1 ]; }
if ( $#ARGV >= 2 ) { $num_working_days_ = $ARGV[ 2 ]; }
if ( $#ARGV >= 3 ) { $compute_individual_stats_ = $ARGV[ 3 ]; } 
...
if ( -d $top_directory_ )
{
	my $exec_cmd = "$MODELSCRIPTS_DIR/run_simulations_stir_2.pl $shortcode_ $top_directory_/ $t_start_date_ $t_end_date_ $compute_individual_stats_";
	print $exec_cmd,"\n";
	`$exec_cmd`;
