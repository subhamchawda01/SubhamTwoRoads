...
# \file scripts/generate_and_plot_seqd_to_conf.pl
#
...
my $GET_STC_EXEC = $LIVE_BIN_DIR."/get_seqd_to_conf";
...
my $exec_cmd_ = "$GET_STC_EXEC $shortcode_ $yyyymmdd_";
if ( $USER eq "sghosh" && $to_mail_ == 1 )
{ # Most likely a cron job.
    my $exec_cmd_ = "$GET_STC_EXEC $shortcode_ $yyyymmdd_ /home/sghosh/master/temp";
...
if ( -e $output_filename_ )
{
    ...
    my $t_all_file_name_ = $temp_dir_."/stc.".$shortcode_.".".$yyyymmdd_;
    `grep \"GetSeqdToConfTimes\" $output_filename_ | awk '{ print \$2\" \"( \$3 * 1000.0 ); }' > $t_all_file_name_`;

    if ( $to_plot_ == 1 )
    {
	`$PLOT_SCRIPT $t_all_file_name_ 2 $shortcode_.$yyyymmdd_.stc NL`;
    }

    if ( $to_mail_ == 1 )
    {
	my $t_output_png_ = $temp_dir_."/plot.".$shortcode_.".".$yyyymmdd_.".png";
	`$SAVEPLOT_SCRIPT $t_all_file_name_ 2 $shortcode_.$yyyymmdd_.stc NL`;
    }
