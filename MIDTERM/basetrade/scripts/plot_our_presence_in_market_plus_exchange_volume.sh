...
export PATH=/apps/gcc_versions/gdb7_7_gcc4_9/bin/gdb:$PATH # for gdb 4.9
...
case $PLOT_OPTION in
	...
	GNUPLOT_CMD='set title "Our Presence in Market( VolumesRatio ) Plus arket Volume"; set xlabel "Time ( UTC )"; set ylabel "Our Volumes To Exch Volumes Ratio(%)"; set y2label "Total Market Volume"; set xdata time; set timefmt "%s"; set ytics nomirror ; set y2tics nomirror ; plot "'$OUR_PRESENCE_IN_MARKET_STATS_FILE'" u '$END_TIME_OFFSET':($'$OUR_PERI_VOL_OFFSET'/$'$EXCH_PERI_VOL_OFFSET')*100 t "Volume Ratio(%)" w lines, "" u '$END_TIME_OFFSET':'$EXCH_PERI_VOL_OFFSET' axes x1y2 w steps t "EV"'
