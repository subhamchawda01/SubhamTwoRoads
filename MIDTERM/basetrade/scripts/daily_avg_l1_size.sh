...
do
    ...
    if [ -e $symbol_file_ ]; then
      ...
      echo $symbol
        ...
        $EXEC $symbol $YYYYMMDD | awk -v d="$YYYYMMDD" '{ if($3 != 0) print $1" "d" "$3;}' |  cat - $DEST_DIR"/"$symbol | sort -nk2 -r | awk 'BEGIN{nextdate = 20991212}{if(nextdate > $2){print $1" "$2" "$3}; nextdate = $2}' > $DEST_DIR"/temp_$symbol$id_";
