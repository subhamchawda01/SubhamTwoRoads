...
my $USAGE = "$0 SHORTCODE STRAT_DIR/STRAT_LIST DATE NUM_DAYS [START_TIME] [END_TIME] [ DETAILED[0/1] ]";
...
my $num_days_ = $ARGV[3];
my $start_hhmm_ = "0000";
if ( $#ARGV > 3 ) { $start_hhmm_ = $ARGV[4]; }
my $end_hhmm_ = "4800";
if ( $#ARGV > 4 ) { $end_hhmm_ = $ARGV[5]; }
my $detailed_ = 0;
if ( $#ARGV > 5 ) { $detailed_ = $ARGV[6]; }
...
for my $strat_base_name_ ( @strats_base_){
  if (IsValidConfig($strat_base_name_)){
    $fetch_from_wf_db_ = 1;
    last;
  }
}
...
FetchPnlSamplesStrats ( $shortcode_, \@strats_base_, \@dates_vec_, \%sample_pnls_strats_vec_, $start_hhmm_, $end_hhmm_, $fetch_from_wf_db_);
...
for my $t_strat_ ( @strats_sorted_ )
{
  if ( ! $detailed_ ) {
    ...
    foreach my $t_date_ ( sort { $a <=> $b } keys %sample_pnl_series_ ) {
      my $date_pnl_str_ = $t_date_." ";
      foreach my $t_slot_ ( sort { $a <=> $b } keys %{ $sample_pnl_series_{ $t_date_ } } ) {
        $date_pnl_str_ .= $t_slot_.":".$sample_pnl_series_{ $t_date_ }{ $t_slot_ }." ";
      } 
      print $date_pnl_str_."\n"; 
    }
    my $date_pnl_str_ =  "STATISTICS ";
    foreach my $t_slot_ ( sort { $a <=> $b } keys %slot_pnls_ ) {
