...
# \file scripts/call_run_sim_overnight_perdir_longer_staged.pl
#
...
my $LOCKFILE=$HOME_DIR."/locks/call_run_sim_overnight_longer_staged_".$shortcode_."_".$skip_recent_days_."_".$num_working_days_.".lock";

my $hostname_ = `hostname`;
if ( index ( $hostname_ , "ip-10-0" ) >= 0 )
{ # AWS
    $LOCKFILE="/mnt/sdf/locks/call_run_sim_overnight_longer_staged_".$shortcode_."_".$skip_recent_days_."_".$num_working_days_.".lock";
}
...
my $t_end_date_ = $last_date_with_data_ ;
	...
	if (opendir my $dh, $top_directory_)
	{
		...
		{
		    my $exec_cmd = "$MODELSCRIPTS_DIR/run_simulations.pl $shortcode_ $top_directory_/$dir_item_ $t_start_date_ $t_end_date_ ";
		    `$exec_cmd`;
