...
#HK servers to copy to NY11 
if [ "$HOSTNAME" == "SDV-HK-SRV11" ] || [ "$HOSTNAME" == "SDV-HK-SRV12" ] 
then
...
do 
	...
	PID=`crontab -l | grep $STRATFILEPATH | awk '{print $8}'`;
	...
	if [ -e $LOGFILEPATH ] ; then
		...
		if [ -e $MODELFILEPATH ] ; then
		    rsync $MODELFILEPATH $USER@$DEST_SERVER:$DEST_LOG_LOC/;
		fi
	    done
		...
		if [ -e $PARAMFILEPATH ] ; then
		    rsync $PARAMFILEPATH $USER@$DEST_SERVER:$DEST_LOG_LOC/;
		fi
	    done
		...
		#echo $ESTSTARTTIME;
		rsync $TEMPSTRATFILEPATH $USER@$DEST_SERVER:$DEST_LOG_LOC/;
	    fi
	fi
    fi
done 
