...
DI_strip_shortcodes <- c("BR_DI_0", "BR_DI_1", "BR_DI_2", "BR_DI_3", "BR_DI_4", "BR_DI_5", "BR_DI_6", "BR_DI_7", "BR_DI_8" );
...
GetDV01 <- function ( )
{
        ...
        {
                shc <- DI_strip_shortcodes[i];
                t_dv01 <- system ( sprintf ( "~/basetrade_install/bin/p2y %s %s", shc, current_date ), intern=TRUE );
		cat ( t_dv01, "\n");
...
GetTerms <- function ( )
{
	...
	{
		shc <- DI_strip_shortcodes[i];
		t_term <- system ( sprintf ( "~/basetrade_install/bin/get_di_term %s %s", current_date, shc ), intern = TRUE ); 
