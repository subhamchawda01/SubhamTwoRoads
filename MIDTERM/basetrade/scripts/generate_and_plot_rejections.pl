...
# \file scripts/generate_and_plot_rejections.pl
#
...
my $GET_GRP_EXEC = $LIVE_BIN_DIR."/get_rejection_periods";
...
if ( -e $output_filename_ )
{
    ...
    `grep \"GetTimeToNumRejections\" $output_filename_ | awk '{ print \$2\" \"\$3; }' > $t_all_file_name_`;
