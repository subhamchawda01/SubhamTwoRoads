...
~/basetrade/ModelScripts/summarize_staged_strats_for_day_no_outsample_check.pl $SHORTCODE $TIMEPERIOD $TRADING_END_YYYYMMDD $NUM_PAST_DAYS INVALIDFILE $folder_name| awk '{ if ( $2 == "_fn_" ) printf "%5s %7s %4s %5s %5s %s\n" , $3 , $5 , $9 , $12 , $17 , $2 ; else if ( NF > 15 ) { printf "%5d %7d %4d %+4.2f %5d %s\n" , $3 , $5 , $9 , $12 , $17 , $2 } }' | sort -k $COL -g
