...
PRINT_STRAT_FROM_BASE_EXEC=`$GETEXEC_PY -s print_strat_from_base.sh`
...
if [ $CONFIGID -gt 0 ]; then
  if [ $# -eq 2 ]; then
    ...
    MODEL=`echo $STRAT | awk '{print $4}'`;
    if [[ -f "$MODEL" ]]; then cat $MODEL; fi
  else
    $PROCESSCONFIG_PY -c $STRATFILENAME -m VIEWMODEL; 
  fi;
else
  if [[ -s $STRATPATH ]]; then
