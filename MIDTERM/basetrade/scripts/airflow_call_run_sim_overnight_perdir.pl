...
my $USAGE="$0 shortcode [start-date] [num_past_days=5] [time-period]";
...
if ( $#ARGV >= 3 ) { $time_period_ = $ARGV [ 3 ] ; }

my $last_date_with_data_ = `date +%Y%m%d`; chomp ( $last_date_with_data_ );
if ( $#ARGV >= 1 ) {  $last_date_with_data_ =  $ARGV [ 1 ] ; } 

my $LOCKFILE=$HOME_DIR."/locks/airflow_call_run_sim_overnight_".$shortcode_."_".$last_date_with_data_."_".$num_working_days_."_".$time_period_.".lock";

my $hostname_ = `hostname`;
if ( index ( $hostname_ , "ip-10-0" ) >= 0 )
{ # AWS
    $LOCKFILE="/mnt/sdf/locks/airflow_call_run_sim_overnight_".$shortcode_."_".$last_date_with_data_."_".$num_working_days_."_".$time_period_.".lock";
}
