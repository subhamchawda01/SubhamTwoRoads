...
my $usage_ = "run_accurate_sim_real.pl DATE STARTTIME ENDTIME UNIQUE_ID [REMOVE_STRAT_FILES=1] [STRAT_FILE_DIR] [PARAM_FILE_DIR] [MODEL_FILE_DIR]";
...
my $starttime_ = $ARGV[1]; chomp ($starttime_);
my $endtime_ = $ARGV[2]; chomp ($endtime_);
my $unique_id_ = $ARGV[3]; chomp ($unique_id_);
...
if ( $#ARGV > 1 )
{
    $to_remove_strat_files_ = $ARGV [ 4 ];
    if ( $#ARGV > 4 ) { $t_strat_file_dir_ = $ARGV [ 5 ]; }
    if ( $#ARGV > 5 ) { $t_param_file_dir_ = $ARGV [ 6 ]; }
    if ( $#ARGV > 6 ) { $t_model_file_dir_ = $ARGV [ 7 ]; }
...
{ # Generate local strategy, model & param file.
	...
	if ( index ( $query_log_file_lines_[$strategy_line_], "STRATEGYLINE" ) >= 0 ) {
	    ...
	    {
		# Print out the strategy name.
		print $sim_strat_file_name_."\n";
	    }

	    $sim_strat_file_name_ = $t_strat_file_dir_."/".$sim_strat_file_name_;
...
#    my $sim_exec_=$HOME_DIR."/LiveExec/bin/sim_strategy";
    ...
    my $exec_cmd="$sim_exec_ SIM $sim_strat_file_name_ $sim_id_ $yyyymmdd_ $mkt_model_ 1800 0.0 0 ADD_DBG_CODE -1";
    print $exec_cmd ;

    if ( $print_strat_exec_cmd || $USER eq "sghosh" || $USER eq "dvctrader" || $USER eq "rahul" || $USER eq "ravi" )
    {
	print "$exec_cmd\n";
    }
...
my $trades_by_time_exec_=$BIN_DIR."/get_trades_within_time_frame" ;
...
#if ( $USER eq "ravi" || $USER eq "sghosh" ) 
if ( $USER eq "sghosh" ) 
{
    ...
    my $temp_out_real_trades_="/tmp/trades_temp_real" ;
    my $temp_out_sim_trades_="/tmp/trades_temp_sim" ;

    my $trades_by_time_exec_cmd="$trades_by_time_exec_ $trades_file_name_ $yyyymmdd_ $starttime_ $endtime_ $temp_out_real_trades_" ;

    `$trades_by_time_exec_cmd` ;

    my $sim_trades_="/spare/local/logs/tradelogs/trades.$yyyymmdd_.$sim_id_" ;

    $trades_by_time_exec_cmd="$trades_by_time_exec_ $sim_trades_ $yyyymmdd_ $starttime_ $endtime_ $temp_out_sim_trades_" ;

    `$trades_by_time_exec_cmd` ;

    $plot_cmd_ = "$PLOT_PNL_DIFF_SCRIPT $temp_out_real_trades_ $temp_out_sim_trades_";
    `$plot_cmd_`;

    `rm -rf $temp_out_real_trades_`;
    `rm -rf $temp_out_sim_trades_`;
}
else
{

    my $temp_out_real_trades_="/tmp/trades_temp_real.$yyyymmdd_.99991111" ;
    my $temp_out_sim_trades_="/tmp/trades_temp_sim.$yyyymmdd_.99991111" ;
    ...
    `$SCRIPTS_DIR/plot_trades_pnl_nyc_2_all.sh $temp_out_sim_trades_ $temp_out_real_trades_`;
