...
sudo mount /media/disk-ephemeral1
sudo mount /media/disk-ephemeral2
...
sudo mkdir -p /media/disk-ephemeral1/ec2_globalresults
...
#for user in `ls -l ../ | grep -v total | grep -v dvctrader | grep -v ec2-user | awk '{print $NF}'`; do sudo mount -t nfs 52.87.81.158:/media/ephemeral30/$user /home/$user; done

#For updateing ld config cache
sudo ldconfig
#Run dvctrader's component
sudo runuser -l dvctrader -c '/mnt/sdf/startup_settings_autoscaling.sh > /home/dvctrader/startup_err.txt'
