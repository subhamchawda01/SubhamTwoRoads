/**
   \file InitLogic/base_data_gen.cpp
...
void termination_handler(int signum) {
  ...
  if (signum == SIGSEGV || signum == SIGILL ||
    ...
    std::string email_address_ = "";
    {
      ...
      t_oss_ << "datagen received " << SimpleSignalString(signum) << " on " << hostname_ << newline << newline;
    ...
    if (send_slack_notification) {
      ...
      } else if (!strncmp(getenv("USER"), "kishenp", strlen("kishenp"))) {
        email_address_ = "kishenp@circulumvite.com";
      } else if (!strncmp(getenv("USER"), "hagarwal", strlen("hagarwal"))) {
        email_address_ = "hrishav.agarwal@tworoads.co.in";
      } else if (!strncmp(getenv("USER"), "chandan", strlen("chandan"))) {
        email_address_ = "chandan.kumar@tworoads.co.in";
      } else {  // Not sure if others want to receive these emails.
        email_address_ = "nseall@tworoads.co.in";
      }

      HFSAT::Email email_;
      email_.setSubject(subject_string_);
...
void CheckDiskSpace(std::string output_filename_, int argc, char** argv) {
  // Check for disk space only if Datagen is not called for STATS or STATS_SAMPLES or PNL_BASED_STATS
  if (output_filename_.compare("STATS") != 0 && output_filename_.compare("STATS_SAMPLES") != 0 &&
      output_filename_.compare("PNL_BASED_STATS") != 0) {
    unsigned long int space_left = HFSAT::FileUtils::GetAvailableSpace(output_filename_);
    if (space_left < 2 * 1024 * 1024 * 1024L)  // 2GB
    {
      // Do not change this output to std::cerr below ,
      // it is used at locations where datagen gets called from
      // to bypass subsequent calls to datagen.
      std::cerr << "Less Space for Datagen: " << space_left << std::endl;

      HFSAT::Email e;
      e.setSubject("Less Space for DataGen");
      char hostname_[128];
      hostname_[127] = '\0';
      gethostname(hostname_, 127);
      e.addRecepient("nseall@tworoads.co.in");
      e.addSender("nseall@tworoads.co.in");
      e.content_stream << "datagen called on host " << hostname_ << "<br/>";
      e.content_stream << "space available at location " << output_filename_ << " " << space_left << "<br/>";
      e.content_stream << "command_execution attempted :";
      for (int i = 0; i < argc; ++i) {
        e.content_stream << " " << argv[i];
      }
      e.content_stream << "<br/>"
                       << "Result: Aborted due to low disk space\n";
      e.sendMail();
      exit(1);
    }
  }
...
int main(int argc, char** argv) {
  ...
  // Check disk space. Whether to allow datagen to run or not.
  CheckDiskSpace(output_filename_, argc, argv);
  ...
  // go through all source_shortcodes and check for NSE, LoadNSESecurityDefinitions only if NSE shortcode is required
  for (auto i = 0u; i < source_shortcode_vec_.size(); i++) {
    if (strncmp(source_shortcode_vec_[i].c_str(), "NSE", 3) == 0) {
  ...
  HFSAT::MarketUpdateManager& market_update_manager_ = *(HFSAT::MarketUpdateManager::GetUniqueInstance(
      dbglogger_, watch_, sec_name_indexer_, sid_to_smv_ptr_map, tradingdate_));
  ...
  if (stats_only_) {
    indicator_logger_ = HFSAT::ModelCreator::CreateIndicatorStats(
        ...
        use_sample_shortcodes_, sample_shc_smv_vec_, c3_required_cutoffs_, regime_mode_to_print_, samples_print_);
  } else if (type_ == 1) {
