/**
    \file IndicatorsCode/curve_adjusted_price.cpp
...
void CurveAdjustedPrice::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
CurveAdjustedPrice* CurveAdjustedPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                          const std::vector<const char*>& r_tokens_,
                                                          PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 7u) {
    ExitVerbose(kExitErrorCodeGeneral, "CurveAdjustedPrice needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 CurveAdjustedPrice BR_DI1N_0 BR_DI_0 BR_DI_1 PRICE_TYPE HALFLIFE
...
CurveAdjustedPrice* CurveAdjustedPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, CurveAdjustedPrice*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new CurveAdjustedPrice(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                               _indep1_market_view_, _indep2_market_view_, t_price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

CurveAdjustedPrice::CurveAdjustedPrice(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void CurveAdjustedPrice::OnMarketUpdate(const unsigned int _security_id_,
...
void CurveAdjustedPrice::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void CurveAdjustedPrice::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
void CurveAdjustedPrice::InitializeValues() {
  current_dep_price_ = 0;
