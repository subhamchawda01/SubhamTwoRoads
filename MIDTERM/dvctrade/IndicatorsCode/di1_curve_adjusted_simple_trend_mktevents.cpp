/**
    \file IndicatorsCode/di1_curve_adjusted_simple_trend_mktevents.cpp
...
void DI1CurveAdjustedSimpleTrendMktEvents::CollectShortCodes(
...
DI1CurveAdjustedSimpleTrendMktEvents* DI1CurveAdjustedSimpleTrendMktEvents::GetUniqueInstance(
  ...
  // INDICATOR 1.00 DI1CurveAdjustedSimpleTrendMktEvents DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
DI1CurveAdjustedSimpleTrendMktEvents* DI1CurveAdjustedSimpleTrendMktEvents::GetUniqueInstance(
  ...
  static std::map<std::string, DI1CurveAdjustedSimpleTrendMktEvents*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new DI1CurveAdjustedSimpleTrendMktEvents(
...
DI1CurveAdjustedSimpleTrendMktEvents::DI1CurveAdjustedSimpleTrendMktEvents(
...
void DI1CurveAdjustedSimpleTrendMktEvents::OnIndicatorUpdate(const unsigned int& indicator_index_,
...
void DI1CurveAdjustedSimpleTrendMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DI1CurveAdjustedSimpleTrendMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool DI1CurveAdjustedSimpleTrendMktEvents::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void DI1CurveAdjustedSimpleTrendMktEvents::InitializeValues() { indicator_value_ = 0; }
}
