/**
    \file IndicatorsCode/volume_pca_deviation_pairs_port_mktevents.cpp
...
void VolumePCADeviationPairsPortMktEvents::CollectShortCodes(
...
VolumePCADeviationPairsPortMktEvents* VolumePCADeviationPairsPortMktEvents::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::vector<const char*>& r_tokens_,
    PriceType_t _basepx_pxtype_) {
  if (r_tokens_.size() < 7) {
    std::cerr << "VolumePCADeviationPairsPortMktEvents syntax incorrect! given text:";
    for (auto i = 0u; i < r_tokens_.size(); i++) {
      std::cerr << " " << r_tokens_[i];
    }
    std::cerr << std::endl;
    std::cerr << "Expected syntax: INDICATOR  _this_weight_  VolumePCADeviationPairsPortMktEvents  _dep_market_view_  "
...
VolumePCADeviationPairsPortMktEvents* VolumePCADeviationPairsPortMktEvents::GetUniqueInstance(
    ...
    const std::string& _portfolio_descriptor_shortcode_, const double _num_events_halflife_,
  ...
  static std::map<std::string, VolumePCADeviationPairsPortMktEvents*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
    ...
    if (principal_eigen_vec.size() < 1) {
      std::cerr << "VolumePCADeviationPairsPortMktEvents: No eigen vectors loaded for "
                << (_portfolio_descriptor_shortcode_) << " in PcaWeightsManager " << std::endl;
      exit(1);
    }

    concise_indicator_description_map_[concise_indicator_description_] = new VolumePCADeviationPairsPortMktEvents(
        _dbglogger_, _watch_, concise_indicator_description_, _dep_market_view_, _portfolio_descriptor_shortcode_,
        _num_events_halflife_, principal_eigen_vec[0u].eigenvec_components_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

VolumePCADeviationPairsPortMktEvents::VolumePCADeviationPairsPortMktEvents(
    ...
    const double _num_events_halflife_, const std::vector<double>& t_eigen_components_, const PriceType_t _price_type_)
  ...
  if (dep_index_in_portfolio_ < 0) {
    ...
    DBGLOG_DUMP;
    std::cerr << "VolumePCADeviationPairsPortMktEvents: since " << dep_index_in_portfolio_
              << " not in PortFolio. Dep:" << r_dep_market_view_.shortcode()
    ...
    for (auto i = 0u; i < indep_market_view_vec_.size(); i++) {
      ...
      SimpleTrendMktEvents* _dep_indicator_ = SimpleTrendMktEvents::GetUniqueInstance(
          t_dbglogger_, r_watch_, *(indep_market_view_vec_[i]), _num_events_halflife_, _price_type_);
...
void VolumePCADeviationPairsPortMktEvents::WhyNotReady() {
  if (!is_ready_) {
...
void VolumePCADeviationPairsPortMktEvents::OnIndicatorUpdate(const unsigned int& _indicator_index_,
...
void VolumePCADeviationPairsPortMktEvents::OnVolumeUpdate(unsigned int t_index_, double r_new_volume_value_) {
...
void VolumePCADeviationPairsPortMktEvents::InitializeValues(int _index_) {
