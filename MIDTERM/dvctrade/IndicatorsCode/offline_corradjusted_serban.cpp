/**
    \file IndicatorsCode/offline_corradjusted_serban.cpp
...
void OfflineCorradjustedSerban::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OfflineCorradjustedSerban* OfflineCorradjustedSerban::GetUniqueInstance(DebugLogger& t_dbglogger_,
...
OfflineCorradjustedSerban* OfflineCorradjustedSerban::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineCorradjustedSerban*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new OfflineCorradjustedSerban(
...
OfflineCorradjustedSerban::OfflineCorradjustedSerban(
...
void OfflineCorradjustedSerban::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
  ...
  if (!is_ready_) {
    ...
    if (!data_interrupted_) {
      ...
      // std::cerr << "Trend: " << short_term_indep_trend_ << " " << indep_trend_momentum_factor_ *
      // current_projection_multiplier_ * short_term_indep_trend_ << " Reversion: " << current_projected_spread_ << " "
      // << spread_reversion_factor_ * (  current_projected_spread_ ) << " Momentum: " << (current_projected_spread_ -
      // moving_avg_spread_ ) <<  " " << moving_avg_spread_ << " " <<  -spread_trend_momentum_factor_ *
      // (current_projected_spread_ - moving_avg_spread_ ) << "\n";
      NotifyIndicatorListeners(indicator_value_);
    }
  }
}

void OfflineCorradjustedSerban::WhyNotReady() {
  if (!is_ready_) {
...
void OfflineCorradjustedSerban::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineCorradjustedSerban::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void OfflineCorradjustedSerban::InitializeValues() {
...
void OfflineCorradjustedSerban::UpdateLRInfo() {
