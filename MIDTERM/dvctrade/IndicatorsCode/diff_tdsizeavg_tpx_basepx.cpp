/**
    \file IndicatorsCode/diff_tdsizeavg_tpx_basepx.cpp
...
void DiffTDSizeAvgTPxBasepx::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                               std::vector<std::string>& _ors_source_needed_vec_,
                                               const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

DiffTDSizeAvgTPxBasepx* DiffTDSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                  const std::vector<const char*>& r_tokens_,
...
DiffTDSizeAvgTPxBasepx* DiffTDSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, DiffTDSizeAvgTPxBasepx*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DiffTDSizeAvgTPxBasepx(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                   _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

DiffTDSizeAvgTPxBasepx::DiffTDSizeAvgTPxBasepx(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void DiffTDSizeAvgTPxBasepx::OnMarketUpdate(const unsigned int _security_id_,
...
#define MIN_SIGNIFICANT_SUM_SZ_TRADED 1.00
  ...
  if (time_decayed_trade_info_manager_.sumsz_ >= MIN_SIGNIFICANT_SUM_SZ_TRADED) {
    indicator_value_ = (time_decayed_trade_info_manager_.sumpxsz_ / time_decayed_trade_info_manager_.sumsz_) -
...
void DiffTDSizeAvgTPxBasepx::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
                                          const MarketUpdateInfo& cr_market_update_info_) {
  // need to compare against a low value since otherwise there would be weird values as the denominator recedes in value
  if (time_decayed_trade_info_manager_.sumsz_ >= MIN_SIGNIFICANT_SUM_SZ_TRADED) {
    indicator_value_ = (time_decayed_trade_info_manager_.sumpxsz_ / time_decayed_trade_info_manager_.sumsz_) -
...
void DiffTDSizeAvgTPxBasepx::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DiffTDSizeAvgTPxBasepx::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
