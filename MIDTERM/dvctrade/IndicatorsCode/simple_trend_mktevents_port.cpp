...
void SimpleTrendMktEventsPort::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                                 std::vector<std::string>& _ors_source_needed_vec_,
                                                 const std::vector<const char*>& r_tokens_) {
  IndicatorUtil::AddPortfolioShortCodeVec((std::string)r_tokens_[3], _shortcodes_affecting_this_indicator_);
}

SimpleTrendMktEventsPort* SimpleTrendMktEventsPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                      const std::vector<const char*>& r_tokens_,
                                                                      PriceType_t _basepx_pxtype_) {
  if (r_tokens_.size() < 6) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight SimpleTrendMktEventsPort _indep_market_view_ _num_events_/_fractional_seconds_ <T> "
                "_price_type_ ");
...
SimpleTrendMktEventsPort* SimpleTrendMktEventsPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, SimpleTrendMktEventsPort*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new SimpleTrendMktEventsPort(t_dbglogger_, r_watch_, concise_indicator_description_,
                                     _portfolio_descriptor_shortcode_, _num_events_halflife_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SimpleTrendMktEventsPort::SimpleTrendMktEventsPort(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void SimpleTrendMktEventsPort::OnPortfolioPriceChange(double _new_price_) {
...
void SimpleTrendMktEventsPort::OnPortfolioPriceReset(double t_new_portfolio_price_, double t_old_portfolio_price_,
...
void SimpleTrendMktEventsPort::SetTimeDecayWeights() {
  decay_page_factor_ = MathUtils::CalcDecayFactor(trend_history_num_events_halflife_);
  inv_decay_sum_ = (1 - decay_page_factor_);
}
void SimpleTrendMktEventsPort::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void SimpleTrendMktEventsPort::OnMarketDataResumed(const unsigned int _security_id_) { data_interrupted_ = false; }
}
