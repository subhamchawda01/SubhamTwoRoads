...
void BollingerBand::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                      std::vector<std::string>& _ors_source_needed_vec_,
                                      const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

BollingerBand* BollingerBand::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                const std::vector<const char*>& r_tokens_,
                                                PriceType_t _basepx_pxtype_) {
  // INDICATOR _this_weight_ _indicator_string_ _dep_market_view_ _fractional_seconds_ _price_type_
...
BollingerBand* BollingerBand::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                ...
                                                double _multiplication_factor_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << _dep_market_view_.secname() << ' ' << _fractional_seconds_ << ' '
              << PriceType_t_To_String(_price_type_) << _multiplication_factor_;
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, BollingerBand*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new BollingerBand(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                          _fractional_seconds_, _price_type_, _multiplication_factor_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

BollingerBand::BollingerBand(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                             const std::string& concise_indicator_description_,
                             const SecurityMarketView& _dep_market_view_, double _fractional_seconds_,
                             PriceType_t _price_type_, double _multiplication_factor_)
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      stdev_value_(0.01),
      multiplication_factor_(_multiplication_factor_),
...
{
  stdev_value_ = PcaWeightsManager::GetUniqueInstance().GetShortcodeStdevs(dep_market_view_.shortcode());
...
void BollingerBand::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& cr_market_update_info_) {
  ...
  if (!is_ready_) {
    ...
    // indicator_value_ = unadjusted_trend_ - multiplication_factor_ * stdev_value_ ;
    if (fabs(unadjusted_trend_) > fabs(multiplication_factor_ * stdev_value_)) {
      if (unadjusted_trend_ > 0)
        indicator_value_ = unadjusted_trend_ - multiplication_factor_ * stdev_value_;
      else
        indicator_value_ = unadjusted_trend_ + multiplication_factor_ * stdev_value_;
    } else {
    ...
    // std::cout << " UnAsjusted tredn: " << unadjusted_trend_ << " curr_price_ " << current_indep_price_ << "
    // moving_avg_ " << moving_avg_price_ << std::endl;
    // indicator_value_ = unadjusted_trend_;
...
void BollingerBand::InitializeValues() {
...
void BollingerBand::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void BollingerBand::OnMarketDataResumed(const unsigned int _security_id_) {
  if (dep_market_view_.security_id() == _security_id_) {
    InitializeValues();
