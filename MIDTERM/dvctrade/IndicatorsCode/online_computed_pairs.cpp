...
void OnlineComputedPairs::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OnlineComputedPairs* OnlineComputedPairs::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                            const std::vector<const char*>& r_tokens_,
...
OnlineComputedPairs* OnlineComputedPairs::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, OnlineComputedPairs*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OnlineComputedPairs(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                _indep_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OnlineComputedPairs::OnlineComputedPairs(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OnlineComputedPairs::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    ...
    if (watch_.msecs_from_midnight() - last_new_page_msecs_ <
      ...
      if (num_pages_to_add_ >= (int)decay_vector_.size()) {  // if way too many pages added ... like we took a nap
        ...
        if (num_pages_to_add_ == 1) {  // exactly 1 page added so optimizations possible
          // moving_avg_dep_price_ = ( current_dep_price_ * inv_decay_sum_ ) + ( moving_avg_dep_price_ *
          // decay_page_factor_ ) ;
          // moving_avg_indep_price_ = ( current_indep_price_ * inv_decay_sum_ ) + ( moving_avg_indep_price_ *
          // decay_page_factor_ ) ;
          ...
          // moving_avg_dep_price_ = ( current_dep_price_ * inv_decay_sum_ ) + ( last_dep_price_recorded_ *
          // inv_decay_sum_ * decay_vector_sums_ [ ( num_pages_to_add_ - 1 ) ] ) + ( moving_avg_dep_price_ *
          // decay_vector_ [ num_pages_to_add_ ] ) ;
          // moving_avg_indep_price_ = ( current_indep_price_ * inv_decay_sum_ ) + ( last_indep_price_recorded_ *
          // inv_decay_sum_ * decay_vector_sums_ [ ( num_pages_to_add_ - 1 ) ] ) + ( moving_avg_indep_price_ *
          // decay_vector_ [ num_pages_to_add_ ] ) ;
          moving_avg_dep_indep_price_ = (current_dep_price_ * current_indep_price_ * inv_decay_sum_) +
    ...
    if (moving_avg_indep_indep_price_ > 1.00) {  // added to prevent nan
      // Main Differnece from OnlineComputedPair
...
void OnlineComputedPairs::InitializeValues() {
...
void OnlineComputedPairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OnlineComputedPairs::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
