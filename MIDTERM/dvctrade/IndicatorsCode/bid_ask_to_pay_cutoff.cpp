...
void BidAskToPayCutoff::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                          std::vector<std::string>& _ors_source_needed_vec_,
                                          const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

BidAskToPayCutoff* BidAskToPayCutoff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        const std::vector<const char*>& r_tokens_,
                                                        PriceType_t _basepx_type_) {
  // INDICATOR _this_weight_ _indicator_string_ _indep_market_view_ _num_levels_ _size_to_seek_ _cutoff_increments_
  // _price_type_
  PriceType_t t_price_type_ = kPriceTypeMax;
  if (r_tokens_.size() < 8) {
    if (r_tokens_.size() < 7) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "BidAskToPayCutoff incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_market_view_ _num_levels_ _size_to_seek_ _cutoff_increments_ _price_type_ ");
    } else {
      t_dbglogger_ << "BidAskToPayCutoff incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                      "_indep_market_view_ _num_levels_ _size_to_seek_ _cutoff_increments_ _price_type_ "
                   << "\n";
      t_dbglogger_.CheckToFlushBuffer();
      ;
      t_price_type_ = _basepx_type_;
    }
  } else {
  ...
  return GetUniqueInstance(
      ...
      std::max(0, atoi(r_tokens_[4])), std::max(0, atoi(r_tokens_[5])), std::max(0, atoi(r_tokens_[6])), t_price_type_);
}

BidAskToPayCutoff* BidAskToPayCutoff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        SecurityMarketView& _indep_market_view_,
                                                        unsigned int _num_levels_, unsigned int _size_to_seek_,
                                                        unsigned int _cutoff_increments_, PriceType_t _price_type_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << _indep_market_view_.secname() << ' ' << _num_levels_ << ' ' << _size_to_seek_
              << ' ' << _cutoff_increments_ << ' ' << PriceType_t_To_String(_price_type_);

  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, BidAskToPayCutoff*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new BidAskToPayCutoff(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_levels_,
                              _size_to_seek_, _cutoff_increments_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

BidAskToPayCutoff::BidAskToPayCutoff(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                     const std::string& concise_indicator_description_,
                                     SecurityMarketView& t_indep_market_view_, unsigned int _num_levels_,
                                     unsigned int _size_to_seek_, unsigned int _cutoff_increments_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      cutoff_increments_(_cutoff_increments_) {
  t_indep_market_view_.subscribe_price_type(this, kPriceTypeMidprice);  // KP : not sure why need this subscription ?
...
void BidAskToPayCutoff::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    ...
    if (cr_market_update_info_.spread_increments_ > (int)cutoff_increments_) {
      indicator_value_ = 0;
    }
      ...
      int bid_size_left_to_seek_ = size_to_seek_;
      int ask_size_left_to_seek_ = size_to_seek_;
...
void BidAskToPayCutoff::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void BidAskToPayCutoff::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
