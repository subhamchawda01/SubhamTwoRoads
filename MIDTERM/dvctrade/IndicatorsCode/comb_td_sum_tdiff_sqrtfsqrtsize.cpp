/**
    \file IndicatorsCode/comb_td_sum_tdiff_sqrtfsqrtsize.cpp
...
void CombTDSumTDiffSqrtFSqrtSize::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                                    std::vector<std::string>& _ors_source_needed_vec_,
                                                    const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

CombTDSumTDiffSqrtFSqrtSize* CombTDSumTDiffSqrtFSqrtSize::GetUniqueInstance(DebugLogger& t_dbglogger_,
...
CombTDSumTDiffSqrtFSqrtSize* CombTDSumTDiffSqrtFSqrtSize::GetUniqueInstance(DebugLogger& t_dbglogger_,
  ...
  static std::map<std::string, CombTDSumTDiffSqrtFSqrtSize*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new CombTDSumTDiffSqrtFSqrtSize(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                        _fractional_st_seconds_, _fractional_lt_seconds_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

CombTDSumTDiffSqrtFSqrtSize::CombTDSumTDiffSqrtFSqrtSize(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  p_st_indicator_ =
      TDSumTDiffSqrtFSqrtSize::GetUniqueInstance(t_dbglogger_, r_watch_, indep_market_view_, _fractional_st_seconds_);
  ...
  p_lt_indicator_ =
      TDSumTDiffSqrtFSqrtSize::GetUniqueInstance(t_dbglogger_, r_watch_, indep_market_view_, _fractional_lt_seconds_);
  p_lt_indicator_->add_indicator_listener(1, this, (1.00 / ratio_lt_st_));
}

void CombTDSumTDiffSqrtFSqrtSize::WhyNotReady() {
  if (!is_ready_) {
...
void CombTDSumTDiffSqrtFSqrtSize::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
  if (!is_ready_) {
    is_ready_ = true;
...
void CombTDSumTDiffSqrtFSqrtSize::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void CombTDSumTDiffSqrtFSqrtSize::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
