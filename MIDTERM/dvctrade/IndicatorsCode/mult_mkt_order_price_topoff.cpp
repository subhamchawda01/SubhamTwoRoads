/**
    \file IndicatorsCode/mult_mkt_order_price_topoff.cpp
...
void MultMktOrderPriceTopOff::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                                std::vector<std::string>& _ors_source_needed_vec_,
                                                const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

MultMktOrderPriceTopOff* MultMktOrderPriceTopOff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                    const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    if (r_tokens_.size() < 6) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "MultMktOrderPriceTopOff incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ ");
    } else {
      t_dbglogger_ << "MultMktOrderPriceTopOff incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                      "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ "
                   << "\n";
...
MultMktOrderPriceTopOff* MultMktOrderPriceTopOff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, MultMktOrderPriceTopOff*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new MultMktOrderPriceTopOff(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                    _num_levels_, _decay_factor_, stdev_duration, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

MultMktOrderPriceTopOff::MultMktOrderPriceTopOff(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                 const std::string& concise_indicator_description_,
                                                 SecurityMarketView& _indep_market_view_, unsigned int _num_levels_,
                                                 double _decay_factor_, double stdev_duration, PriceType_t _price_type_)
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      indep_market_view_(_indep_market_view_),
      price_type_(_price_type_),
      num_levels_(std::max(1u, _num_levels_)),
      decay_factor_(_decay_factor_),
      decay_vector_(std::max(24u, (5 * 2u * _num_levels_ + 8u)), 0),
...
void MultMktOrderPriceTopOff::OnStdevRatioUpdate(const unsigned int index_to_send,
...
void MultMktOrderPriceTopOff::OnMarketUpdate(const unsigned int _security_id_,
  ...
  } else  //  if( ! data_interrupted_ )
  {
    ...
    double sum_bid_price_order_ = 0;
    double sum_bid_order_ = 0;
    double sum_ask_price_order_ = 0;
    double sum_ask_order_ = 0;

    unsigned int new_num_levels = (unsigned)(current_dep_stdev_ratio_ * num_levels_);
    ...
    if (indep_market_view_.market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
      ...
      for (unsigned int
        ...
        // In Indexed Book
        // bid_int_price_level=base_bid_index_ -
        // t_current_bid_index_ ( since levels
        // are 1 min_price_increment_ away )
        ...
        double this_bid_ordercount_ = this_bid_level_.limit_ordercount_;
        double this_bid_price_ = this_bid_level_.limit_price_;
        sum_bid_price_order_ += this_bid_decay_factor_ * this_bid_ordercount_ * this_bid_price_;
        sum_bid_order_ += this_bid_decay_factor_ * this_bid_ordercount_;
      ...
      for (unsigned int
        ...
        double this_ask_ordercount_ = this_ask_level_.limit_ordercount_;
        double this_ask_price_ = this_ask_level_.limit_price_;
        sum_ask_price_order_ += this_ask_decay_factor_ * this_ask_ordercount_ * this_ask_price_;
        sum_ask_order_ += this_ask_decay_factor_ * this_ask_ordercount_;
      ...
      for (unsigned int i = 1; i < new_num_levels && i < indep_market_view_.bidlevels_size(); i++) {
        ...
        double this_bid_decay_factor_ = decay_vector_[this_bid_int_price_level_];
        sum_bid_price_order_ +=
            this_bid_decay_factor_ * indep_market_view_.bid_order(i) * indep_market_view_.bid_price(i);
        sum_bid_order_ += this_bid_decay_factor_ * indep_market_view_.bid_order(i);
      }
      ...
      for (unsigned int i = 1; i < new_num_levels && i < indep_market_view_.asklevels_size(); i++) {
        ...
        double this_ask_decay_factor_ = decay_vector_[this_ask_int_price_level_];
        sum_ask_price_order_ +=
            this_ask_decay_factor_ * indep_market_view_.ask_order(i) * indep_market_view_.ask_price(i);
        sum_ask_order_ += this_ask_decay_factor_ * indep_market_view_.ask_order(i);
      }
    }

    if ((sum_bid_order_ < 0.01) || (sum_ask_order_ < 0.01)) {
      indicator_value_ = 0;
    } else {
      double adjusted_price_ = (((sum_bid_price_order_ * sum_ask_order_ / sum_bid_order_) +
                                 (sum_ask_price_order_ * sum_bid_order_ / sum_ask_order_)) /
                                (sum_bid_order_ + sum_ask_order_));
...
void MultMktOrderPriceTopOff::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void MultMktOrderPriceTopOff::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
