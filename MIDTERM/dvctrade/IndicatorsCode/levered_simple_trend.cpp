/**
    \file IndicatorsCode/levered_simple_trend.cpp
...
void LeveredSimpleTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
LeveredSimpleTrend* LeveredSimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                          const std::vector<const char*>& r_tokens_,
                                                          PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 7u) {
    ExitVerbose(kExitErrorCodeGeneral, "LeveredSimpleTrend needs 7 tokens");
    return NULL;
  }

  // INDICATOR 1.00 LeveredSimpleTrend DEP INDEP1 HALFLIFE PRICE_TYPE
...
LeveredSimpleTrend* LeveredSimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, LeveredSimpleTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new LeveredSimpleTrend(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
...
LeveredSimpleTrend::LeveredSimpleTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void LeveredSimpleTrend::OnIndicatorUpdate(const unsigned int& indicator_index_, const double& new_value_) {
  if (!is_ready_) {
...
void LeveredSimpleTrend::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void LeveredSimpleTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    indep_interrupted_ = false;
  } else if (dep_market_view_.security_id() == _security_id_) {
    dep_interrupted_ = false;
...
bool LeveredSimpleTrend::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void LeveredSimpleTrend::InitializeValues() { indicator_value_ = 0; }
}
