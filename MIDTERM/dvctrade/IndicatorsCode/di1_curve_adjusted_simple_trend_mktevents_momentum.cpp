/**
    \file IndicatorsCode/di1_curve_adjusted_simple_trend_mktevents_momentum.cpp
...
void DI1CurveAdjustedSimpleTrendMktEventsMomentum::CollectShortCodes(
...
DI1CurveAdjustedSimpleTrendMktEventsMomentum* DI1CurveAdjustedSimpleTrendMktEventsMomentum::GetUniqueInstance(
  ...
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "DI1CurveAdjustedSimpleTrendMktevents needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 DI1CurveAdjustedSimpleTrendMktEventsMomentum DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
DI1CurveAdjustedSimpleTrendMktEventsMomentum* DI1CurveAdjustedSimpleTrendMktEventsMomentum::GetUniqueInstance(
    ...
    double _num_events_halflife_, PriceType_t t_price_type_) {
  std::ostringstream t_temp_oss_;

  t_temp_oss_ << VarName() << ' ' << _dep_market_view_.secname() << ' ' << _indep1_market_view_.secname() << ' '
              << _indep2_market_view_.secname() << ' ' << _num_events_halflife_ << ' ' << t_price_type_;

  std::string concise_indicator_description_(t_temp_oss_.str());
  static std::map<std::string, DI1CurveAdjustedSimpleTrendMktEventsMomentum*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DI1CurveAdjustedSimpleTrendMktEventsMomentum(t_dbglogger_, r_watch_, concise_indicator_description_,
                                                         _dep_market_view_, _indep1_market_view_, _indep2_market_view_,
...
DI1CurveAdjustedSimpleTrendMktEventsMomentum::DI1CurveAdjustedSimpleTrendMktEventsMomentum(
    ...
    const SecurityMarketView& _indep2_market_view_, double _num_events_halflife_, PriceType_t _price_type_)
    : IndicatorListener(),
...
void DI1CurveAdjustedSimpleTrendMktEventsMomentum::OnIndicatorUpdate(const unsigned int& indicator_index_,
...
void DI1CurveAdjustedSimpleTrendMktEventsMomentum::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DI1CurveAdjustedSimpleTrendMktEventsMomentum::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool DI1CurveAdjustedSimpleTrendMktEventsMomentum::AreAllReady() {
  return VectorUtils::CheckAllForValue(is_ready_vec_, true);
}

void DI1CurveAdjustedSimpleTrendMktEventsMomentum::InitializeValues() { indicator_value_ = 0; }
}
