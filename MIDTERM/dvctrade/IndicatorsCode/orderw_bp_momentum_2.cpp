/**
    \file IndicatorsCode/orderw_bp_momentum_2.cpp
...
void OrderWBPMomentum2::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                          std::vector<std::string>& _ors_source_needed_vec_,
                                          const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

OrderWBPMomentum2* OrderWBPMomentum2::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "OrderWBPMomentum2 incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
...
OrderWBPMomentum2* OrderWBPMomentum2::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, OrderWBPMomentum2*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OrderWBPMomentum2(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_levels_,
                              _level_decay_factor_, _fractional_seconds_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OrderWBPMomentum2::OrderWBPMomentum2(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OrderWBPMomentum2::OnPLNew(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
  ...
  if (t_new_ordercount_ > 1) return;
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      if (t_level_added_ < num_levels_ && t_new_ordercount_ > 1) {
        bidpressure_ += t_new_ordercount_ * level_decay_vector_[std::max(0, t_level_added_)];
    ...
    case kTradeTypeSell: {
      if (t_level_added_ < num_levels_ && t_new_ordercount_ > 1) {
        askpressure_ += t_new_ordercount_ * level_decay_vector_[std::max(0, t_level_added_)];
...
void OrderWBPMomentum2::OnPLDelete(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
                                   ...
                                   const bool t_is_intermediate_message_) {
  if (t_old_ordercount_ > 1) return;
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      if (t_level_removed_ < num_levels_ && t_old_ordercount_ > 1) {
        bidpressure_ -= t_old_ordercount_ * level_decay_vector_[std::max(0, t_level_removed_)];
    ...
    case kTradeTypeSell: {
      if (t_level_removed_ < num_levels_ && t_old_ordercount_ > 1) {
        askpressure_ -= t_old_ordercount_ * level_decay_vector_[std::max(0, t_level_removed_)];
...
void OrderWBPMomentum2::OnPLChange(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      if (t_level_changed_ < num_levels_ &&
          (t_old_ordercount_ == t_new_ordercount_ + 1 || t_old_ordercount_ == t_new_ordercount_ - 1)) {
        bidpressure_ += (t_new_ordercount_ - t_old_ordercount_) * level_decay_vector_[std::max(0, t_level_changed_)];
    ...
    case kTradeTypeSell: {
      if (t_level_changed_ < num_levels_ &&
          (t_old_ordercount_ == t_new_ordercount_ + 1 || t_old_ordercount_ == t_new_ordercount_ - 1)) {
        askpressure_ += (t_new_ordercount_ - t_old_ordercount_) * level_decay_vector_[std::max(0, t_level_changed_)];
...
void OrderWBPMomentum2::OnMarketUpdate(const unsigned int _security_id_,
                                       const MarketUpdateInfo& cr_market_update_info_) {
  if (!is_ready_) {
    if (indep_market_view_.is_ready_complex(2) && bidpressure_ != 0 && askpressure_ != 0) {
      is_ready_ = true;
      indicator_value_ = 0;
...
void OrderWBPMomentum2::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
  data_interrupted_ = true;
  indicator_value_ = 0;
  NotifyIndicatorListeners(indicator_value_);
}

void OrderWBPMomentum2::OnMarketDataResumed(const unsigned int _security_id_) { data_interrupted_ = false; }
}
