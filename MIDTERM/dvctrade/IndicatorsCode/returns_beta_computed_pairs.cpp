/**
    \file IndicatorsCode/returns_beta_computed_pairs.cpp
...
void ReturnsBetaComputedPairs::CollectShortCodes(std::vector<std::string> &_shortcodes_affecting_this_indicator_,
...
ReturnsBetaComputedPairs *ReturnsBetaComputedPairs::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
                                                                      const std::vector<const char *> &r_tokens_,
...
ReturnsBetaComputedPairs *ReturnsBetaComputedPairs::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
                                                                      ...
                                                                      double t_beta_change_factor_,
                                                                      PriceType_t t_price_type_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << t_dep_market_view_.secname() << ' ' << t_indep_market_view_.secname() << ' '
              << t_fractional_seconds_ << ' ' << t_beta_change_factor_ << ' ' << t_price_type_;
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, ReturnsBetaComputedPairs *> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new ReturnsBetaComputedPairs(t_dbglogger_, r_watch_, concise_indicator_description_, t_dep_market_view_,
                                     t_indep_market_view_, t_fractional_seconds_, t_beta_change_factor_, t_price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ReturnsBetaComputedPairs::ReturnsBetaComputedPairs(DebugLogger &t_dbglogger_, const Watch &r_watch_,
                                                   const std::string &concise_indicator_description_,
                                                   SecurityMarketView &t_dep_market_view_,
                                                   SecurityMarketView &t_indep_market_view_,
                                                   double t_fractional_seconds_, double t_beta_change_factor_,
                                                   PriceType_t t_price_type_)
...
void ReturnsBetaComputedPairs::OnIndicatorUpdate(const unsigned int &t_indicator_index_,
...
void ReturnsBetaComputedPairs::WhyNotReady() {
  if (!is_ready_) {
...
void ReturnsBetaComputedPairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void ReturnsBetaComputedPairs::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    indicator_value_ = 0;
...
bool ReturnsBetaComputedPairs::InitializeBetaValues() {
