...
void BidAskToPayGeneralizedAllLvls::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                                      std::vector<std::string>& _ors_source_needed_vec_,
                                                      const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

BidAskToPayGeneralizedAllLvls* BidAskToPayGeneralizedAllLvls::GetUniqueInstance(
  ...
  if (r_tokens_.size() < 6) {
    if (r_tokens_.size() < 5) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "BidAskToPayGeneralizedAllLvls incorrect syntax. Should be INDICATOR _this_weight_ "
                  "_indicator_string_ _indep_market_view_ _num_levels_ _size_to_seek_/_orders_to_seek_ _price_type_ ");
    } else {
      t_dbglogger_ << " BidAskToPayGeneralizedAllLvls incorrect syntax. Should be INDICATOR _this_weight_ "
                      "_indicator_string_ _indep_market_view_ _num_levels_ _size_to_seek_/_orders_to_seek_ "
                      "_price_type_ "
                   << "\n";
  ...
  return GetUniqueInstance(t_dbglogger_, r_watch_,
                           *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[3])),
                           std::max(0, atoi(r_tokens_[4])), t_price_type_);
}

BidAskToPayGeneralizedAllLvls* BidAskToPayGeneralizedAllLvls::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                                const Watch& r_watch_,
                                                                                SecurityMarketView& _indep_market_view_,
                                                                                unsigned int _quantity_to_seek_,
                                                                                PriceType_t _price_type_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << _indep_market_view_.secname() << ' ' << _quantity_to_seek_ << ' '
              << PriceType_t_To_String(_price_type_);
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, BidAskToPayGeneralizedAllLvls*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new BidAskToPayGeneralizedAllLvls(
        t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _quantity_to_seek_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

BidAskToPayGeneralizedAllLvls::BidAskToPayGeneralizedAllLvls(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void BidAskToPayGeneralizedAllLvls::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    ...
    if (indep_market_view_.market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
      ...
      } while (bid_level_ < indep_market_view_.bidlevels_size());
      ...
      } while (ask_level_ < indep_market_view_.asklevels_size());
    }

#define MIN_SIGNIFICANT_FACTOR 1
    if ((sum_bid_quantity_ <= MIN_SIGNIFICANT_FACTOR) || (sum_ask_quantity_ <= MIN_SIGNIFICANT_FACTOR)) {
      // this means there are no valid entries in that side since oterwise even top level makes this sum >= 1.0
      indicator_value_ = 0;
    } else {
...
void BidAskToPayGeneralizedAllLvls::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void BidAskToPayGeneralizedAllLvls::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    data_interrupted_ = false;
  }
}

void BidAskToPayGeneralizedAllLvls::OnMarketStatusChange(const unsigned int _security_id_,
