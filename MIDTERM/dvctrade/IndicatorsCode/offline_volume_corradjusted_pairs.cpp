/**
   \file IndicatorsCode/offline_volume_corradjusted_pairs.cpp
...
void OfflineVolumeCorradjustedPairs::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OfflineVolumeCorradjustedPairs* OfflineVolumeCorradjustedPairs::GetUniqueInstance(
...
OfflineVolumeCorradjustedPairs* OfflineVolumeCorradjustedPairs::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineVolumeCorradjustedPairs*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OfflineVolumeCorradjustedPairs(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                           _indep_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OfflineVolumeCorradjustedPairs::OfflineVolumeCorradjustedPairs(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (lrdb_absent_) {
    ...
    RecentVolumeRatioCalculator* p_rvrc_ = RecentVolumeRatioCalculator::GetUniqueInstance(
        t_dbglogger_, r_watch_, t_dep_market_view_, t_indep_market_view_, 3600.0);
...
void OfflineVolumeCorradjustedPairs::OnIndicatorUpdate(const unsigned int& _indicator_index_,
  ...
  if (!is_ready_) {
    ...
    // bad: how to avoid this check? good: on indicatorupdate is only called if p_indep_indicator and p_dep_indicator
    // are defined, and so good to use pointer
    if (p_indep_indicator_->IsDataInterrupted()) {
      indicator_value_ = 0;
    }

    else if (_indicator_index_ == 1u) {
...
void OfflineVolumeCorradjustedPairs::PrintDebugInfo() const {
  std::cerr << " IV " << indicator_value_ << " RVR  " << recent_volume_ratio_ << " IPT_t "
            << indep_price_trend_ / indep_market_view_.min_price_increment() << " CPM "
            << current_projection_multiplier_ << " CPT " << current_projected_trend_ << " DPT " << dep_price_trend_
            << std::endl;
}

// market_interrupt_listener interface

void OfflineVolumeCorradjustedPairs::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineVolumeCorradjustedPairs::UpdateLRInfo() {
...
void OfflineVolumeCorradjustedPairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineVolumeCorradjustedPairs::OnMarketDataResumed(const unsigned int _security_id_) {
  InitializeValues();
