/**
    \file IndicatorsCode/offline_corradjusted_pairs_port.cpp
...
void OfflineCorradjustedPairsPort::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OfflineCorradjustedPairsPort* OfflineCorradjustedPairsPort::GetUniqueInstance(DebugLogger& t_dbglogger_,
...
OfflineCorradjustedPairsPort* OfflineCorradjustedPairsPort::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineCorradjustedPairsPort*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OfflineCorradjustedPairsPort(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                         _portfolio_descriptor_shortcode_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OfflineCorradjustedPairsPort::OfflineCorradjustedPairsPort(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OfflineCorradjustedPairsPort::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
  if (!is_ready_) {
#if EQUITY_INDICATORS_ALWAYS_READY
...
void OfflineCorradjustedPairsPort::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineCorradjustedPairsPort::UpdateLRInfo() {
...
void OfflineCorradjustedPairsPort::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineCorradjustedPairsPort::OnMarketDataResumed(const unsigned int _security_id_) {
  InitializeValues();
