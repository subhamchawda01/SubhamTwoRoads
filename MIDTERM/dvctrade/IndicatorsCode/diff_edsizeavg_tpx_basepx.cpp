/**
    \file IndicatorsCode/diff_edsizeavg_tpx_basepx.cpp
...
void DiffEDSizeAvgTPxBasepx::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                               std::vector<std::string>& _ors_source_needed_vec_,
                                               const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

DiffEDSizeAvgTPxBasepx* DiffEDSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                  const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 6) {
    if (r_tokens_.size() < 5) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs,
                  " DiffEDSizeAvgTPxBasePx, Incorrect syntax, Should b INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_makret_view_ _num_trades_halflife_ _price_type_");
    } else {
      t_dbglogger_ << "DiffEDSizeAvgTPxBasePx, Incorrect syntax, Should b INDICATOR _this_weight_ _indicator_string_ "
                      "_indep_makret_view_ _num_trades_halflife_ _price_type_"
                   << "\n";
...
DiffEDSizeAvgTPxBasepx* DiffEDSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, DiffEDSizeAvgTPxBasepx*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DiffEDSizeAvgTPxBasepx(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                   _num_trades_halflife_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

DiffEDSizeAvgTPxBasepx::DiffEDSizeAvgTPxBasepx(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  event_decayed_trade_info_manager_.compute_sumpxsz();
  event_decayed_trade_info_manager_.compute_sumsz();
  indep_market_view_.subscribe_tradeprints(this);
}

void DiffEDSizeAvgTPxBasepx::OnMarketUpdate(const unsigned int _security_id_,
                                            const MarketUpdateInfo& cr_market_update_info_) {
#define MIN_SIGNIFICANT_SUM_SZ_TRADED 0.10
  // need to compare against a low value since otherwise there would be weird values as the denominator recedes in value
  if (event_decayed_trade_info_manager_.sumsz_ >= MIN_SIGNIFICANT_SUM_SZ_TRADED) {
    indicator_value_ = (event_decayed_trade_info_manager_.sumpxsz_ / event_decayed_trade_info_manager_.sumsz_) -
...
void DiffEDSizeAvgTPxBasepx::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
                                          const MarketUpdateInfo& cr_market_update_info_) {
#define MIN_SIGNIFICANT_SUM_SZ_TRADED 0.10
  // need to compare against a low value since otherwise there would be weird values as the denominator recedes in value
  if (event_decayed_trade_info_manager_.sumsz_ >= MIN_SIGNIFICANT_SUM_SZ_TRADED) {
...
void DiffEDSizeAvgTPxBasepx::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DiffEDSizeAvgTPxBasepx::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
