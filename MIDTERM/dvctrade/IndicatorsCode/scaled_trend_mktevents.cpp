/**
    \file IndicatorsCode/scaled_trend_mktevents.cpp
...
void ScaledTrendMktEvents::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                             std::vector<std::string>& _ors_source_needed_vec_,
                                             const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

ScaledTrendMktEvents* ScaledTrendMktEvents::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                              const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 6) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight ScaledTrendMktEvents _indep_market_view_ _num_events_/_fractional_seconds_ <T> "
                "_price_type_ ");
...
ScaledTrendMktEvents* ScaledTrendMktEvents::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, ScaledTrendMktEvents*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new ScaledTrendMktEvents(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                 _num_events_halflife_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ScaledTrendMktEvents::ScaledTrendMktEvents(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void ScaledTrendMktEvents::OnMarketUpdate(const unsigned int _security_id_,
...
void ScaledTrendMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void ScaledTrendMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
