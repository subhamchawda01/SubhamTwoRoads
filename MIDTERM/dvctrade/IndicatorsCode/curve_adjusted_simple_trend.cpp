...
void CurveAdjustedSimpleTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
CurveAdjustedSimpleTrend* CurveAdjustedSimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                      const std::vector<const char*>& r_tokens_,
                                                                      PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "CurveAdjustedSimpleTrend needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 CurveAdjustedSimpleTrend DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
CurveAdjustedSimpleTrend* CurveAdjustedSimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, CurveAdjustedSimpleTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new CurveAdjustedSimpleTrend(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
...
CurveAdjustedSimpleTrend::CurveAdjustedSimpleTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void CurveAdjustedSimpleTrend::OnIndicatorUpdate(const unsigned int& indicator_index_, const double& new_value_) {
  if (!is_ready_) {
...
void CurveAdjustedSimpleTrend::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void CurveAdjustedSimpleTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool CurveAdjustedSimpleTrend::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void CurveAdjustedSimpleTrend::InitializeValues() { indicator_value_ = 0; }
}
