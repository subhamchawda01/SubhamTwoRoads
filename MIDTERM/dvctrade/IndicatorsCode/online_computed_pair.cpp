/**
    \file IndicatorsCode/online_computed_pair.cpp
...
void OnlineComputedPair::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OnlineComputedPair* OnlineComputedPair::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                          const std::vector<const char*>& r_tokens_,
...
OnlineComputedPair* OnlineComputedPair::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, OnlineComputedPair*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OnlineComputedPair(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                               _indep_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OnlineComputedPair::OnlineComputedPair(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OnlineComputedPair::WhyNotReady() {
  if (!is_ready_) {
...
void OnlineComputedPair::OnMarketUpdate(const unsigned int _security_id_,
...
// SMV is not ready
  ...
  if (!is_ready_) {
    ...
    if (moving_avg_indep_indep_price_ > 1.00) {  // added to prevent nan
      ...
      // main Difference from OnlineComputedPairs
...
void OnlineComputedPair::InitializeValues() {
...
void OnlineComputedPair::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OnlineComputedPair::OnMarketDataResumed(const unsigned int _security_id_) {
