/**
    \file IndicatorsCode/di1_curve_adjusted_simple_trend.cpp
...
void DI1CurveAdjustedSimpleTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
DI1CurveAdjustedSimpleTrend* DI1CurveAdjustedSimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                            const Watch& r_watch_,
                                                                            const std::vector<const char*>& r_tokens_,
                                                                            PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "DI1CurveAdjustedSimpleTrend needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 DI1CurveAdjustedSimpleTrend DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
DI1CurveAdjustedSimpleTrend* DI1CurveAdjustedSimpleTrend::GetUniqueInstance(
  ...
  static std::map<std::string, DI1CurveAdjustedSimpleTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DI1CurveAdjustedSimpleTrend(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                        _indep1_market_view_, _indep2_market_view_, _fractional_secs_, t_price_type_);
  }

  return concise_indicator_description_map_[concise_indicator_description_];
}

DI1CurveAdjustedSimpleTrend::DI1CurveAdjustedSimpleTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                         ...
                                                         double _fractional_secs_, PriceType_t _price_type_)
    : IndicatorListener(),
  ...
  //    double alpha1_ =  ( ( indep2_term_ - dep_term_ )  / ( indep2_term_ - indep1_term_ ) ) ;
  //    double alpha2_ =  ( - ( indep1_term_ - dep_term_ ) / ( indep2_term_ - indep1_term_ ) ) ;

  dep_trendI_ =
      SimpleTrend::GetUniqueInstance(t_dbglogger_, r_watch_, dep_market_view_.shortcode(), _fractional_secs_, _price_type_);
  indep1_trendI_ =
...
void DI1CurveAdjustedSimpleTrend::OnIndicatorUpdate(const unsigned int& indicator_index_, const double& new_value_) {
  if (!is_ready_) {
...
void DI1CurveAdjustedSimpleTrend::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DI1CurveAdjustedSimpleTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool DI1CurveAdjustedSimpleTrend::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void DI1CurveAdjustedSimpleTrend::InitializeValues() { indicator_value_ = 0; }
}
