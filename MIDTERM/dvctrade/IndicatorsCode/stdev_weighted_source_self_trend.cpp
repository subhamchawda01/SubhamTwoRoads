/**
    \file IndicatorsCode/stdev_weighted_source_self_trend.cpp
...
void StdevWeightedSourceSelfTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
StdevWeightedSourceSelfTrend* StdevWeightedSourceSelfTrend::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                              const Watch& r_watch_,
                                                                              const std::vector<const char*>& r_tokens_,
                                                                              PriceType_t _basepx_pxtype_) {
  // INDICATOR _this_weight_ _indicator_string_ t_dep_market_view_ t_indep_market_view_ t_stdev_fractional_seconds_
  // t_stdev_lower_bound_ t_stdev_upper_bound_ t_fractional_seconds_ t_price_type_
...
StdevWeightedSourceSelfTrend* StdevWeightedSourceSelfTrend::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, SecurityMarketView& t_dep_market_view_,
    SecurityMarketView& t_indep_market_view_, double t_stdev_fractional_seconds_, double t_stdev_lower_bound_,
    double t_stdev_upper_bound_, double t_fractional_seconds_, PriceType_t t_price_type_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << t_dep_market_view_.secname() << ' ' << t_indep_market_view_.secname() << ' '
              << t_stdev_fractional_seconds_ << ' ' << t_stdev_lower_bound_ << ' ' << t_stdev_upper_bound_ << ' '
              << t_fractional_seconds_ << ' ' << t_price_type_;
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, StdevWeightedSourceSelfTrend*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new StdevWeightedSourceSelfTrend(
        t_dbglogger_, r_watch_, concise_indicator_description_, t_dep_market_view_, t_indep_market_view_,
        t_stdev_fractional_seconds_, t_stdev_lower_bound_, t_stdev_upper_bound_, t_fractional_seconds_, t_price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

StdevWeightedSourceSelfTrend::StdevWeightedSourceSelfTrend(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::string& concise_indicator_description_,
    SecurityMarketView& t_dep_market_view_, SecurityMarketView& t_indep_market_view_, double _stdev_fractional_seconds_,
    double _stdev_lower_bound_, double _stdev_upper_bound_, double _fractional_seconds_, PriceType_t t_price_type_)
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      current_stdev_factor_(0),
      hist_stdev_(0),
      stdev_lower_bound_(_stdev_lower_bound_),
      stdev_upper_bound_(_stdev_upper_bound_),
      stdev_norm_factor_(1 / (_stdev_upper_bound_ - _stdev_lower_bound_)),
      p_slow_stdev_calculator_(NULL) {
  watch_.subscribe_BigTimePeriod(this);  // for UpdateLRInfo and updating volume adjustment

  bool lrdb_absent_ = false;

  p_slow_stdev_calculator_ = SlowStdevCalculator::GetUniqueInstance(
      t_dbglogger_, r_watch_, indep_market_view_.shortcode(), _stdev_fractional_seconds_ * 1000u, 0);
  p_slow_stdev_calculator_->AddSlowStdevCalculatorListener(this);
  ...
  if (lrdb_absent_) {
    ...
    p_dep_indicator_ = SimpleTrend::GetUniqueInstance(t_dbglogger_, r_watch_, t_dep_market_view_.shortcode(),
                                                      _fractional_seconds_, t_price_type_);
...
void StdevWeightedSourceSelfTrend::OnStdevUpdate(const unsigned int _security_id_, const double& _new_stdev_value_) {
  double t_stdev_ratio_ = std::min(stdev_upper_bound_, std::max(stdev_lower_bound_, (_new_stdev_value_ / hist_stdev_)));
  current_stdev_factor_ = stdev_norm_factor_ * t_stdev_ratio_;
}

void StdevWeightedSourceSelfTrend::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
  ...
  if (!is_ready_) {
    ...
    if (!data_interrupted_) {
      ...
      indicator_value_ =
          current_stdev_factor_ * dep_price_trend_ + (1 - current_stdev_factor_) * current_projected_trend_;
...
void StdevWeightedSourceSelfTrend::WhyNotReady() {
  if (!is_ready_) {
...
void StdevWeightedSourceSelfTrend::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void StdevWeightedSourceSelfTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void StdevWeightedSourceSelfTrend::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
  hist_stdev_ = SampleDataUtil::GetAvgForPeriod(indep_market_view_.shortcode(), watch_.YYYYMMDD(),
                                                NUM_DAYS_HISTORY_STDEV, trading_start_mfm_, trading_end_mfm_, "STDEV");
}

void StdevWeightedSourceSelfTrend::UpdateLRInfo() {
