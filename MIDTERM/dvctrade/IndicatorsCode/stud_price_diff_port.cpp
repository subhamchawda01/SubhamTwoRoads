/**
    \file IndicatorsCode/stud_price_diff_port.cpp
...
void StudPriceDiffPort::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
StudPriceDiffPort* StudPriceDiffPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight StudPriceDiffPort _dep_market_view_ _portfolio_ _fractional_seconds_ _price_type_ ");
  }
  if (r_tokens_.size() == 7) {
...
StudPriceDiffPort* StudPriceDiffPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, StudPriceDiffPort*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new StudPriceDiffPort(t_dbglogger_, r_watch_, concise_indicator_description_, _shortcode_,
                              _portfolio_descriptor_shortcode_, _fractional_seconds_, _lrdb_sign_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

StudPriceDiffPort::StudPriceDiffPort(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  SlowStdevCalculator* std_1 =
      SlowStdevCalculator::GetUniqueInstance(t_dbglogger_, r_watch_, _shortcode_, 200u * 1000u, 0.5);
  if (std_1 != NULL) {
    std_1->AddSlowStdevCalculatorListener(this);
    std_1->add_unweighted_indicator_listener(1, this);
  }
  SlowStdevCalculatorPort* std_2 = SlowStdevCalculatorPort::GetUniqueInstance(
      t_dbglogger_, r_watch_, _portfolio_descriptor_shortcode_, 200u * 1000u, 0.5);
  if (std_2 != NULL) {
    std_2->AddSlowStdevCalculatorPortListener(this);
  }
...
void StudPriceDiffPort::OnStdevUpdate(const unsigned int _security_id_, const double& _new_stdev_value_) {
  stdev_dep_ = _new_stdev_value_;
}

void StudPriceDiffPort::OnStdevUpdate(const double& _new_stdev_value_) { stdev_indep_ = _new_stdev_value_; }

void StudPriceDiffPort::WhyNotReady() {
  if (!is_ready_) {
...
void StudPriceDiffPort::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
...
void StudPriceDiffPort::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
...
void StudPriceDiffPort::OnPortfolioPriceChange(double _new_price_) {
  current_indep_price_ = _new_price_;

  if (!is_ready_) {
    if (is_dep_portfolio_) {
      if (dep_portfolio_->is_ready() && indep_portfolio_price_->is_ready()) {
        is_ready_ = true;
        InitializeValues();
      } else {
...
void StudPriceDiffPort::UpdateComputedVariables() {
  if (watch_.msecs_from_midnight() - last_new_page_msecs_ < page_width_msecs_) {
...
void StudPriceDiffPort::OnPortfolioPriceReset(double t_new_portfolio_price_, double t_old_portfolio_price_,
...
void StudPriceDiffPort::InitializeValues() {
...
void StudPriceDiffPort::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void StudPriceDiffPort::OnMarketDataResumed(const unsigned int _security_id_) {
