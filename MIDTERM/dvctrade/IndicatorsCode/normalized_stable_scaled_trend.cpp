/**
    \file IndicatorsCode/normalized_stable_scaled_trend.cpp
...
void NormalizedStableScaledTrend::CollectShortCodes(std::vector<std::string>& t_shortcodes_affecting_this_indicator_,
...
NormalizedStableScaledTrend* NormalizedStableScaledTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        const std::vector<const char*>& r_tokens_,
...
NormalizedStableScaledTrend* NormalizedStableScaledTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  // static std::map<std::string, NormalizedStableScaledTrend*> concise_indicator_description_map_;
  if (global_concise_indicator_description_map_.find(concise_indicator_description_) ==
    ...
    global_concise_indicator_description_map_[concise_indicator_description_] = new NormalizedStableScaledTrend(
        t_dbglogger_, r_watch_, concise_indicator_description_, _shortcode_, _fractional_seconds_, _price_type_);
  }
  return dynamic_cast<NormalizedStableScaledTrend*>(global_concise_indicator_description_map_[concise_indicator_description_]);
}

NormalizedStableScaledTrend::NormalizedStableScaledTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  // Using last 60 days ss trend average value to normalize the indicator value across different set of trading days
  // Adding norm support only for shortcodes now, TODO for portfolio
  if ( !is_indep_portfolio_ ) {
    ss_trend_norm_factor_ = HFSAT::SampleDataUtil::GetAvgForPeriod(
            std::string(indep_market_view_->shortcode()), r_watch_.YYYYMMDD(), 60, std::string("SSTREND"));
  } 
  /* if (!indep_market_view_.subscribe_price_type(this, _price_type_)) {
     PriceType_t t_error_price_type_ = _price_type_;
     std::cerr << typeid(*this).name() << ':' << __func__ << ':' << __LINE__ << ' ' << concise_indicator_description_
...
void NormalizedStableScaledTrend::OnStdevUpdate(const unsigned int _security_id_, const double& _new_stdev_value_) {
  if (is_indep_portfolio_) {
...
void NormalizedStableScaledTrend::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
...
void NormalizedStableScaledTrend::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
...
void NormalizedStableScaledTrend::UpdateComputedVariables() {
  ...
  indicator_value_ = indicator_value_ / ss_trend_norm_factor_;

  NotifyIndicatorListeners(indicator_value_);
}

void NormalizedStableScaledTrend::InitializeValues() {
...
void NormalizedStableScaledTrend::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void NormalizedStableScaledTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (!is_indep_portfolio_ && indep_market_view_->security_id() == _security_id_) {
    InitializeValues();
