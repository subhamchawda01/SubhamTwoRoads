/**
    \file IndicatorsCode/mult_mkt_complex_order_price.cpp
...
void MultMktComplexOrderPrice::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                                 std::vector<std::string>& _ors_source_needed_vec_,
                                                 const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

MultMktComplexOrderPrice* MultMktComplexOrderPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                      const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    if (r_tokens_.size() < 6) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "MultMktComplexOrderPrice incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ ");
    } else {
      t_dbglogger_ << "MultMktComplexOrderPrice incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                      "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ "
                   << "\n";
...
MultMktComplexOrderPrice* MultMktComplexOrderPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, MultMktComplexOrderPrice*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new MultMktComplexOrderPrice(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                     _num_levels_, _decay_factor_, _price_type_, _stdev_duration_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

MultMktComplexOrderPrice::MultMktComplexOrderPrice(DebugLogger& t_dbglogger_, const Watch& r_watch_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      decay_vector_(std::max(24u, (2u * _num_levels_ + 8)), 0),
  ...
  if (book_info_struct_ == NULL) {
    std::cerr << "Error getting book_info_struct for MultMktComplexOrderPrice: " << _num_levels_ << " "
              << _decay_factor_ << std::endl;
  }
...
void MultMktComplexOrderPrice::OnMarketUpdate(const unsigned int _security_id_,
  ...
  } else  // if( ! data_interrupted_ )
  {
    ...
    if ((book_info_struct_->sum_bid_factor_order_ <= 0) || (book_info_struct_->sum_ask_factor_order_ <= 0)) {
      indicator_value_ = 0;
    } else {
      double adjusted_price_ =
          (2 * indep_market_view_.mid_price()) -
          ((book_info_struct_->sum_bid_factor_price_order_ + book_info_struct_->sum_ask_factor_price_order_) /
...
void MultMktComplexOrderPrice::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void MultMktComplexOrderPrice::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
