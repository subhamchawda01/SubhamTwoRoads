/**
    \file IndicatorsCode/sizew_bp_momentum_2.cpp
...
void SizeWBPMomentum2::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                         std::vector<std::string>& _ors_source_needed_vec_,
                                         const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

SizeWBPMomentum2* SizeWBPMomentum2::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                      const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "SizeWBPMomentum2 incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
...
SizeWBPMomentum2* SizeWBPMomentum2::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, SizeWBPMomentum2*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new SizeWBPMomentum2(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_levels_,
                             _level_decay_factor_, _fractional_seconds_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SizeWBPMomentum2::SizeWBPMomentum2(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void SizeWBPMomentum2::OnPLNew(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
...
void SizeWBPMomentum2::OnPLDelete(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
...
void SizeWBPMomentum2::OnPLChange(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
  ...
  switch (t_buysell_) {
    case kTradeTypeBuy: {
      if (t_level_changed_ < num_levels_ &&
          (t_new_ordercount_ <= t_old_ordercount_ + 1 && t_new_ordercount_ >= t_old_ordercount_ - 1)) {
...
void SizeWBPMomentum2::OnMarketUpdate(const unsigned int _security_id_,
...
void SizeWBPMomentum2::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void SizeWBPMomentum2::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
