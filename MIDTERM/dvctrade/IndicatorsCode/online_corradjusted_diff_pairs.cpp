...
void OnlineCorradjustedDiffPairs::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OnlineCorradjustedDiffPairs* OnlineCorradjustedDiffPairs::GetUniqueInstance(DebugLogger& t_dbglogger_,
...
OnlineCorradjustedDiffPairs* OnlineCorradjustedDiffPairs::GetUniqueInstance(
  ...
  static std::map<std::string, OnlineCorradjustedDiffPairs*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OnlineCorradjustedDiffPairs(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                        _indep_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OnlineCorradjustedDiffPairs::OnlineCorradjustedDiffPairs(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OnlineCorradjustedDiffPairs::OnMarketUpdate(const unsigned int t_security_id_,
  ...
  if (!is_ready_) {
    ...
    current_price_diff_ = current_indep_price_ - (moving_avg_indep_ / moving_avg_dep_) * current_dep_price_;
    ...
    if (watch_.msecs_from_midnight() - last_new_page_msecs_ <
      ...
      moving_avg_diff_ += inv_decay_sum_ * (current_price_diff_ - last_price_diff_);
      ...
      if (num_pages_to_add_ >= (int)decay_vector_.size()) {  // if way too many pages added ... like we took a nap
        ...
        if (num_pages_to_add_ == 1) {  // exactly 1 page added so optimizations possible
          moving_avg_diff_ = (current_price_diff_ * inv_decay_sum_) + (moving_avg_diff_ * decay_page_factor_);
          ...
          moving_avg_diff_ = (current_price_diff_ * inv_decay_sum_) +
...
void OnlineCorradjustedDiffPairs::InitializeValues() {
  moving_avg_diff_ = current_price_diff_;
  last_price_diff_ = current_price_diff_;
...
void OnlineCorradjustedDiffPairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OnlineCorradjustedDiffPairs::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
