/**
    \file IndicatorsCode/offline_computed_serban_port.cpp
...
void OfflineComputedSerbanPort::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OfflineComputedSerbanPort* OfflineComputedSerbanPort::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                        const Watch& r_watch_,
                                                                        const std::vector<const char*>& r_tokens_,
                                                                        PriceType_t _basepx_pxtype_) {
  // INDICATOR _this_weight_ _indicator_string_ t_dep_market_view_ t_portfolio_descriptor_shortcode_
  ...
  return GetUniqueInstance(
      ...
      std::string(r_tokens_[4]), atof(r_tokens_[5]), atof(r_tokens_[6]), atof(r_tokens_[7]), atof(r_tokens_[8]),
      atof(r_tokens_[9]), atof(r_tokens_[10]), StringToPriceType_t(r_tokens_[11]));
}

OfflineComputedSerbanPort* OfflineComputedSerbanPort::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, SecurityMarketView& t_dep_market_view_,
    const std::string& t_portfolio_descriptor_shortcode_, double t_fractional_seconds_,
  ...
  static std::map<std::string, OfflineComputedSerbanPort*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new OfflineComputedSerbanPort(
        t_dbglogger_, r_watch_, concise_indicator_description_, t_dep_market_view_, t_portfolio_descriptor_shortcode_,
        t_fractional_seconds_, t_indep_trend_momentum_fractional_seconds_, t_indep_trend_momentum_factor_,
        t_spread_reversion_factor_, t_spread_trend_momentum_fractional_seconds_, t_spread_trend_momentum_factor_,
        t_price_type_);
  }
...
OfflineComputedSerbanPort::OfflineComputedSerbanPort(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::string& concise_indicator_description_,
    SecurityMarketView& t_dep_market_view_, const std::string& t_portfolio_descriptor_shortcode_,
    double t_fractional_seconds_, double t_indep_trend_momentum_fractional_seconds_,
    double t_indep_trend_momentum_factor_, double t_spread_reversion_factor_,
    double t_spread_trend_momentum_fractional_seconds_, double t_spread_trend_momentum_factor_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      short_term_indep_trend_indicator_(
          SimpleTrendPort::GetUniqueInstance(t_dbglogger_, r_watch_, t_portfolio_descriptor_shortcode_,
                                             t_indep_trend_momentum_fractional_seconds_, t_price_type_)),
      ...
      spread_trend_history_msecs_(std::max(20, (int)round(1000 * t_spread_trend_momentum_fractional_seconds_))),
  ...
  if (lrdb_absent_) {
    ...
    p_indep_indicator_ = SimpleTrendPort::GetUniqueInstance(t_dbglogger_, r_watch_, t_portfolio_descriptor_shortcode_,
...
void OfflineComputedSerbanPort::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
  ...
  if (!is_ready_) {
    ...
    if (!data_interrupted_) {
      ...
      indicator_value_ = (indep_trend_momentum_factor_ * current_projection_multiplier_ * short_term_indep_trend_) +
                         (spread_reversion_factor_ * -current_projected_trend_) +
...
void OfflineComputedSerbanPort::WhyNotReady() {
  if (!is_ready_) {
    ...
    if (!(p_indep_indicator_->IsIndicatorReady())) {
      p_indep_indicator_->WhyNotReady();
    }
  }
}

void OfflineComputedSerbanPort::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineComputedSerbanPort::UpdateLRInfo() {
