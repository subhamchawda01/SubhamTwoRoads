/**
    \file IndicatorsCode/diff_trsizeavg_tpx_basepx.cpp
...
void DiffTRSizeAvgTPxBasepx::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                               std::vector<std::string>& _ors_source_needed_vec_,
                                               const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

DiffTRSizeAvgTPxBasepx* DiffTRSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                  const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 6) {
    if (r_tokens_.size() < 5) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  " DiffTRSizeAvegTPxBasepx Incorrect Syntax. Correct syntax would b  INDICATOR _this_weight_ "
                  "_indicator_string_ _indep_market_view_ _num_trades_halflife_ _price_type_");
    } else {
      t_dbglogger_ << " DiffTRSizeAvgTPxBasepx Incorrect Syntax. Correct syntax would b  INDICATOR _this_weight_ "
                      "_indicator_string_ _indep_market_view_ _num_trades_halflife_ _price_type_ "
                   << "\n";
...
DiffTRSizeAvgTPxBasepx* DiffTRSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, DiffTRSizeAvgTPxBasepx*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DiffTRSizeAvgTPxBasepx(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                   _num_trades_halflife_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

DiffTRSizeAvgTPxBasepx::DiffTRSizeAvgTPxBasepx(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void DiffTRSizeAvgTPxBasepx::OnMarketUpdate(const unsigned int _security_id_,
...
#define MIN_SIGNIFICANT_SUM_SZ_TRADED 0.10
  ...
  if (trade_decayed_trade_info_manager_.sumsz_ >= MIN_SIGNIFICANT_SUM_SZ_TRADED) {
...
void DiffTRSizeAvgTPxBasepx::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
                                          const MarketUpdateInfo& cr_market_update_info_) {
#define MIN_SIGNIFICANT_SUM_SZ_TRADED 0.10
  // need to compare against a low value since otherwise there would be weird values as the denominator recedes in value
  if (!data_interrupted_ && trade_decayed_trade_info_manager_.sumsz_ >= MIN_SIGNIFICANT_SUM_SZ_TRADED) {
    indicator_value_ = (trade_decayed_trade_info_manager_.sumpxsz_ / trade_decayed_trade_info_manager_.sumsz_) -
                       SecurityMarketView::GetPriceFromType(price_type_, cr_market_update_info_);
  } else {
    indicator_value_ = 0;
...
void DiffTRSizeAvgTPxBasepx::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DiffTRSizeAvgTPxBasepx::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
