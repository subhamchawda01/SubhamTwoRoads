/**
    \file IndicatorsCode/di1_levered_simple_trend.cpp
...
void DI1LeveredSimpleTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
DI1LeveredSimpleTrend* DI1LeveredSimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                const std::vector<const char*>& r_tokens_,
                                                                PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 7u) {
    ExitVerbose(kExitErrorCodeGeneral, "DI1LeveredSimpleTrend needs 7 tokens");
    return NULL;
  }

  // INDICATOR 1.00 DI1LeveredSimpleTrend DEP INDEP1 HALFLIFE PRICE_TYPE
...
DI1LeveredSimpleTrend* DI1LeveredSimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  t_temp_oss_ << VarName() << ' ' << _dep_market_view_.secname() << ' ' << _indep_market_view_.secname() << ' '
              << _fractional_secs_ << ' ' << t_price_type_;

  std::string concise_indicator_description_(t_temp_oss_.str());
  static std::map<std::string, DI1LeveredSimpleTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DI1LeveredSimpleTrend(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                  _indep_market_view_, _fractional_secs_, t_price_type_);
  }

  return concise_indicator_description_map_[concise_indicator_description_];
}

DI1LeveredSimpleTrend::DI1LeveredSimpleTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                             const std::string& concise_indicator_description_,
                                             const SecurityMarketView& _dep_market_view_,
                                             const SecurityMarketView& _indep_market_view_, double _fractional_secs_,
  ...
  double alpha_ = 1;  // this is now simple trend TODO ( find weight )
  ...
  indep_trendI_ =
      SimpleTrend::GetUniqueInstance(t_dbglogger_, r_watch_, indep_market_view_.shortcode(), _fractional_secs_, _price_type_);
...
void DI1LeveredSimpleTrend::OnIndicatorUpdate(const unsigned int& indicator_index_, const double& new_value_) {
  if (!is_ready_) {
...
void DI1LeveredSimpleTrend::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DI1LeveredSimpleTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    indep_interrupted_ = false;
  } else if (dep_market_view_.security_id() == _security_id_) {
    dep_interrupted_ = false;
...
bool DI1LeveredSimpleTrend::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void DI1LeveredSimpleTrend::InitializeValues() { indicator_value_ = 0; }
}
