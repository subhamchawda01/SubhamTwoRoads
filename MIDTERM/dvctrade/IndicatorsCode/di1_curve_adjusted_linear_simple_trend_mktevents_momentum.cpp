/**
    \file IndicatorsCode/di1_curve_adjusted_linear_simple_trend_mktevents_momentum.cpp
...
void DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::CollectShortCodes(
...
DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                      const std::vector<const char*>& r_tokens_,
  ...
  // INDICATOR 1.00 DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum(
            t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_, _indep1_market_view_,
            _indep2_market_view_, _num_events_halflife_, t_price_type_);
  }

  return concise_indicator_description_map_[concise_indicator_description_];
}

DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum(
...
void DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::OnIndicatorUpdate(const unsigned int& indicator_index_,
...
void DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::AreAllReady() {
  return VectorUtils::CheckAllForValue(is_ready_vec_, true);
}

void DI1CurveAdjustedLinearSimpleTrendMktEventsMomentum::InitializeValues() { indicator_value_ = 0; }
}
