/**
    \file IndicatorsCode/curve_adjusted_simple_trend_mktevents_momentum.cpp
...
void CurveAdjustedSimpleTrendMktEventsMomentum::CollectShortCodes(
...
CurveAdjustedSimpleTrendMktEventsMomentum* CurveAdjustedSimpleTrendMktEventsMomentum::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::vector<const char*>& r_tokens_,
    PriceType_t _base_price_type_) {

  // INDICATOR 1.00 CurveAdjustedSimpleTrendMktEventsMomentum DEP INDEP1 INDEP2 HALFLIFE <T> PRICE_TYPE
  ShortcodeSecurityMarketViewMap::StaticCheckValid(r_tokens_[4]);  // dep
  ...
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "INDICATOR weight CurveAdjustedSimpleTrendMktEventsMomentum DEP INDEP1 INDEP2 _num_events_/_fractional_seconds_ <T> "
                "_price_type_ ");
    return NULL;
...
CurveAdjustedSimpleTrendMktEventsMomentum* CurveAdjustedSimpleTrendMktEventsMomentum::GetUniqueInstance(
  ...
  static std::map<std::string, CurveAdjustedSimpleTrendMktEventsMomentum*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new CurveAdjustedSimpleTrendMktEventsMomentum(
...
CurveAdjustedSimpleTrendMktEventsMomentum::CurveAdjustedSimpleTrendMktEventsMomentum(
...
void CurveAdjustedSimpleTrendMktEventsMomentum::OnIndicatorUpdate(const unsigned int& indicator_index_,
...
void CurveAdjustedSimpleTrendMktEventsMomentum::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void CurveAdjustedSimpleTrendMktEventsMomentum::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool CurveAdjustedSimpleTrendMktEventsMomentum::AreAllReady() {
  return VectorUtils::CheckAllForValue(is_ready_vec_, true);
}

void CurveAdjustedSimpleTrendMktEventsMomentum::InitializeValues() { indicator_value_ = 0; }
}
