/**
    \file IndicatorsCode/offline_computed_pairs_mktevents2.cpp
...
void OfflineComputedPairsMktEvents2::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OfflineComputedPairsMktEvents2* OfflineComputedPairsMktEvents2::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::vector<const char*>& r_tokens_,
    PriceType_t _basepx_pxtype_) {
  // INDICATOR _this_weight_ _indicator_string_ _dep_shortcode_ _indep_market_view_
  // _num_events_halflife_/_fractional_seconds_ <T> _price_type_
  ShortcodeSecurityMarketViewMap::StaticCheckValid(r_tokens_[3]);
  ShortcodeSecurityMarketViewMap::StaticCheckValid(r_tokens_[4]);

  if (r_tokens_.size() < 7) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight OfflineComputedPairsMktEvents2 _dep_market_view_ _indep_market_view_ "
                "_fractional_seconds_/_num_events_ <E> _price_type_ ");
    return nullptr;
  } else if (r_tokens_.size() == 7 || std::string(r_tokens_[7]).compare("#") == 0 ||
             std::string(r_tokens_[6]).compare("E") != 0) {
    double avg_l1_events_per_sec_ = HFSAT::SampleDataUtil::GetAvgForPeriod(
        std::string(r_tokens_[4]), r_watch_.YYYYMMDD(), 60, std::string("L1EVPerSec"));
    ...
    return GetUniqueInstance(
        ...
        *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[4])),
        (unsigned int)std::max(1.00, avg_l1_events_per_sec_ * atof(r_tokens_[5])), StringToPriceType_t(r_tokens_[6]));
  } else {
    return GetUniqueInstance(t_dbglogger_, r_watch_,
                             *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[3])),
                             *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[4])),
                             (unsigned int)std::max(1, atoi(r_tokens_[5])), StringToPriceType_t(r_tokens_[7]));
  }
}
...
OfflineComputedPairsMktEvents2* OfflineComputedPairsMktEvents2::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineComputedPairsMktEvents2*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OfflineComputedPairsMktEvents2(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                           _indep_market_view_, _num_events_halflife_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OfflineComputedPairsMktEvents2::OfflineComputedPairsMktEvents2(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (lrdb_absent_) {
    ...
    p_dep_indicator_ = SimpleTrendIndepMktEvents::GetUniqueInstance(
        t_dbglogger_, r_watch_, t_dep_market_view_, t_indep_market_view_, _num_events_halflife_, _price_type_);
    p_dep_indicator_->add_unweighted_indicator_listener(1u, this);

    p_indep_indicator_ = SimpleTrendMktEvents::GetUniqueInstance(t_dbglogger_, r_watch_, t_indep_market_view_,
                                                                 _num_events_halflife_, _price_type_);
    p_indep_indicator_->add_unweighted_indicator_listener(2u, this);
  }
}

void OfflineComputedPairsMktEvents2::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
...
void OfflineComputedPairsMktEvents2::WhyNotReady() {
  if (!is_ready_) {
...
void OfflineComputedPairsMktEvents2::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineComputedPairsMktEvents2::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void OfflineComputedPairsMktEvents2::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineComputedPairsMktEvents2::UpdateLRInfo() {
