/**
    \file IndicatorsCode/offline_computed_smart_cutoff_pairs.cpp
...
void OfflineComputedSmartCutoffPairs::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OfflineComputedSmartCutoffPairs* OfflineComputedSmartCutoffPairs::GetUniqueInstance(
  ...
  // _stdev_duration_ _divergence_factor_ _price_type_
  ShortcodeSecurityMarketViewMap::StaticCheckValid(r_tokens_[3]);
  ShortcodeSecurityMarketViewMap::StaticCheckValid(r_tokens_[4]);

  double t_stdev_duration_ = 900;
  double t_divergence_factor_ = 1.0;
  PriceType_t t_price_type_ = _basepx_pxtype_;
  if (r_tokens_.size() >= 9) {
    t_stdev_duration_ = atof(r_tokens_[6]);
    t_divergence_factor_ = atof(r_tokens_[7]);
    t_price_type_ = StringToPriceType_t(r_tokens_[8]);
  } else if (r_tokens_.size() >= 8) {
    t_divergence_factor_ = atof(r_tokens_[6]);
    ...
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight OfflineComputedSmartCutoffPairs _dep_market_view_ _indep_market_view_ "
                "t_fractional_seconds_ _stdev_duration_  _divergence_factor_ _price_type_");
  }
  return GetUniqueInstance(t_dbglogger_, r_watch_,
                           *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[3])),
                           *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[4])),
                           atof(r_tokens_[5]), t_stdev_duration_, t_divergence_factor_, t_price_type_);
}

OfflineComputedSmartCutoffPairs* OfflineComputedSmartCutoffPairs::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, SecurityMarketView& t_dep_market_view_,
    SecurityMarketView& t_indep_market_view_, double t_fractional_seconds_, double _stdev_duration_,
    double _divergence_factor_, PriceType_t t_price_type_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << t_dep_market_view_.secname() << ' ' << t_indep_market_view_.secname() << ' '
              << t_fractional_seconds_ << ' ' << _stdev_duration_ << ' ' << _divergence_factor_ << ' ' << t_price_type_;
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, OfflineComputedSmartCutoffPairs*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new OfflineComputedSmartCutoffPairs(
...
OfflineComputedSmartCutoffPairs::OfflineComputedSmartCutoffPairs(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                 const std::string& concise_indicator_description_,
                                                                 SecurityMarketView& t_dep_market_view_,
                                                                 SecurityMarketView& t_indep_market_view_,
                                                                 double t_fractional_seconds_, double _stdev_duration_,
                                                                 double _divergence_factor_, PriceType_t t_price_type_)
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      stdev_trend_(0),
      divergence_factor_(_divergence_factor_),
  ...
  if (lrdb_absent_) {
    ...
    p_stdev_trend_indicator_ = SlowStdevTrendCalculator::GetUniqueInstance(
        t_dbglogger_, r_watch_, t_dep_market_view_.shortcode(), _stdev_duration_, t_fractional_seconds_, t_price_type_);

    if (p_stdev_trend_indicator_ == NULL) {
      indicator_value_ = 0;
      is_ready_ = true;
      std::cerr << __FUNCTION__ << " Failed for Dep StdevTrendCalculator " << t_dep_market_view_.shortcode() << " "
                << t_fractional_seconds_ << " " << PriceType_t_To_String(t_price_type_) << std::endl;
      return;
    }
    ...
    p_stdev_trend_indicator_->add_unweighted_indicator_listener(3u, this);
  }
}

void OfflineComputedSmartCutoffPairs::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
  ...
  if (!is_ready_) {
    ...
    switch (t_indicator_index_) {
      ...
      case 3u: {
        stdev_trend_ = t_new_indicator_value_;
      } break;
    }

    if (fabs(current_projected_trend_) <= fabs(dep_price_trend_)) {
      indicator_value_ = 0;
    } else if (((indep_price_trend_ * dep_price_trend_ < 0 && lrdb_sign_) ||
                (indep_price_trend_ * dep_price_trend_ > 0 && !lrdb_sign_)) &&
               fabs(current_projected_trend_ - dep_price_trend_) > divergence_factor_ * stdev_trend_) {
      indicator_value_ = 0;
    } else {
...
void OfflineComputedSmartCutoffPairs::WhyNotReady() {
  if (!is_ready_) {
...
void OfflineComputedSmartCutoffPairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineComputedSmartCutoffPairs::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void OfflineComputedSmartCutoffPairs::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineComputedSmartCutoffPairs::UpdateLRInfo() {
