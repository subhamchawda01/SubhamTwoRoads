/**
    \file IndicatorsCode/levered_simple_trend_mktevents.cpp
...
void LeveredSimpleTrendMktEvents::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
LeveredSimpleTrendMktEvents* LeveredSimpleTrendMktEvents::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                            const Watch& r_watch_,
                                                                            const std::vector<const char*>& r_tokens_,
                                                                            PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 7u) {
    ExitVerbose(kExitErrorCodeGeneral, "LeveredSimpleTrendMktEvents needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 LeveredSimpleTrendMktEvents DEP INDEP HALFLIFE PRICE_TYPE
...
LeveredSimpleTrendMktEvents* LeveredSimpleTrendMktEvents::GetUniqueInstance(
  ...
  static std::map<std::string, LeveredSimpleTrendMktEvents*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new LeveredSimpleTrendMktEvents(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
...
LeveredSimpleTrendMktEvents::LeveredSimpleTrendMktEvents(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void LeveredSimpleTrendMktEvents::OnIndicatorUpdate(const unsigned int& indicator_index_, const double& new_value_) {
  if (!is_ready_) {
...
void LeveredSimpleTrendMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void LeveredSimpleTrendMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    indep_interrupted_ = false;
  } else if (dep_market_view_.security_id() == _security_id_) {
    dep_interrupted_ = false;
...
bool LeveredSimpleTrendMktEvents::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void LeveredSimpleTrendMktEvents::InitializeValues() { indicator_value_ = 0; }
}
