/**
    \file IndicatorsCode/ors_self_exec_recent_orders.cpp
...
void ORSSelfExecRecentOrders::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
ORSSelfExecRecentOrders* ORSSelfExecRecentOrders::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                    const std::vector<const char*>& r_tokens_,
                                                                    PriceType_t _basepx_pxtype_) {
  // INDICATOR _this_weight_ _indicator_string_ _indep_market_view_ _factor_reduced_
  if (r_tokens_.size() < 5) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "ORSSelfExecRecentOrders incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
...
ORSSelfExecRecentOrders* ORSSelfExecRecentOrders::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                    ...
                                                                    double _factor_reduced_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << _indep_market_view_.secname() << ' ' << _factor_reduced_;
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, ORSSelfExecRecentOrders*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new ORSSelfExecRecentOrders(
        t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _factor_reduced_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ORSSelfExecRecentOrders::ORSSelfExecRecentOrders(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void ORSSelfExecRecentOrders::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
...
void ORSSelfExecRecentOrders::OrderExecuted(
    const int t_server_assigned_client_id_, const int _client_assigned_order_sequence_,
    const int _server_assigned_order_sequence_, const unsigned int _security_id_, const double r_trade_price_,
  ...
  if (r_buysell_ == kTradeTypeBuy) {
    if (r_int_price_ < indep_market_view_.market_update_info_.bestbid_int_price_) {
      indicator_value_ = r_trade_price_ - indep_market_view_.market_update_info_.mkt_size_weighted_price_;
    } else {
      ...
      if (size_to_be_removed_ < _size_executed_ ||
        ...
        if (indep_market_view_.spread_increments() > 1) {
          ...
          if ((recent_bid_orders_map_.find(_server_assigned_order_sequence_) != recent_bid_orders_map_.end()) &&
...
void ORSSelfExecRecentOrders::OrderConfirmed(
    const int t_server_assigned_client_id_, const int _client_assigned_order_sequence_,
...
void ORSSelfExecRecentOrders::OrderORSConfirmed(const int t_server_assigned_client_id_,
                                                const int _client_assigned_order_sequence_,
                                                const int _server_assigned_order_sequence_,
                                                const unsigned int _security_id_, const double r_order_price_,
                                                const TradeType_t r_buysell_, const int r_size_remaining_,
                                                const int r_size_executed_, const int r_int_price_,
...
void ORSSelfExecRecentOrders::OrderCanceled(
    const int t_server_assigned_client_id_, const int _client_assigned_order_sequence_,
    const int _server_assigned_order_sequence_, const unsigned int _security_id_, const double r_order_price_,
    const TradeType_t r_buysell_, const int r_size_remaining_, const int _client_position_, const int _global_position_,
    const int r_order_int_price_, const int32_t server_assigned_message_sequence, const uint64_t exchange_order_id,
...
void ORSSelfExecRecentOrders::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void ORSSelfExecRecentOrders::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
