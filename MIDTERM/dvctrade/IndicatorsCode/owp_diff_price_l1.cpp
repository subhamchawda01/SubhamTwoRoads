...
void OwpDiffPriceL1::CollectShortCodes(std::vector<std::string> &_shortcodes_affecting_this_indicator_,
                                       std::vector<std::string> &_ors_source_needed_vec_,
                                       const std::vector<const char *> &r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

OwpDiffPriceL1 *OwpDiffPriceL1::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
                                                  const std::vector<const char *> &r_tokens_,
  ...
  if (r_tokens_.size() < 6) {
    if (r_tokens_.size() < 5) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "OwpDiffPriceL1 incorrect syntax should b Indicator _this_weight_ _indicator_string_ "
                  "_idnep_market_view _fractional_seconds_ _price_type_");
    } else {
      t_dbglogger_ << "OwpDiffPriceL1 incorrect syntax should b Indicator _this_weight_ _indicator_string_ "
                      "_idnep_market_view _fractional_seconds_ _price_type_"
                   << "\n";
...
OwpDiffPriceL1 *OwpDiffPriceL1::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
  ...
  static std::map<std::string, OwpDiffPriceL1 *> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OwpDiffPriceL1(t_dbglogger_, r_watch_, concise_indicator_description_, t_indep_market_view_,
                           _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OwpDiffPriceL1::OwpDiffPriceL1(DebugLogger &t_dbglogger_, const Watch &r_watch_,
...
void OwpDiffPriceL1::OnTimePeriodUpdate(const int num_pages_to_add_) {
...
void OwpDiffPriceL1::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo &_trade_print_info_,
                                  const MarketUpdateInfo &_market_update_info_) {
  OnMarketUpdate(_security_id_, _market_update_info_);
}

void OwpDiffPriceL1::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo &_market_update_info_) {
    ...
    if (!is_ready_) {
      ...
      if (first_time_) {
        ...
        if (_market_update_info_.spread_increments_ > 1) {
          ...
          if (level_shift_ == true && time_decayed_trade_info_manager_.onelvlsumsz_ == 0) {
...
void OwpDiffPriceL1::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void OwpDiffPriceL1::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
...
void OwpDiffPriceL1::Initialize() {
