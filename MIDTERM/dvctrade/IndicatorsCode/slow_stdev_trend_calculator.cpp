...
namespace HFSAT {
void SlowStdevTrendCalculator::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
  ...
  if (IndicatorUtil::IsPortfolioShortcode(t_source_shortcode_)) {
    ...
    IndicatorUtil::CollectShortcodeOrPortfolio(_shortcodes_affecting_this_indicator_, _ors_source_needed_vec_,
                                               r_tokens_);
  }
}

SlowStdevTrendCalculator* SlowStdevTrendCalculator::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                      const std::vector<const char*>& r_tokens_,
...
SlowStdevTrendCalculator* SlowStdevTrendCalculator::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, SlowStdevTrendCalculator*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new SlowStdevTrendCalculator(
...
SlowStdevTrendCalculator::SlowStdevTrendCalculator(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                   const std::string& concise_indicator_description_,
                                                   std::string _source_shortcode_,
                                                   const unsigned int _indicator_duration_,
                                                   const unsigned int t_trend_history_secs_, PriceType_t _t_price_type_,
                                                   const double t_min_stdev_value_factor_)
  ...
  if (IndicatorUtil::IsPortfolioShortcode(_source_shortcode_)) {
    ...
    ShortcodeSecurityMarketViewMap::StaticCheckValidShortCodeOrPortWithExit(_source_shortcode_);
    if (ShortcodeSecurityMarketViewMap::StaticCheckValidWithoutExit(_source_shortcode_)) {
    ...
    } else if (ShortcodeSecurityMarketViewMap::StaticCheckValidPortWithoutExit(_source_shortcode_)) {
      double min_price_increment =
          IndicatorUtil::GetMinPriceIncrementForPricePortfolio(_source_shortcode_, r_watch_.YYYYMMDD());
      min_unbiased_l2_norm_ =
          min_price_increment * min_price_increment * t_min_stdev_value_factor_ * t_min_stdev_value_factor_;
      indep_trend_indicator_ = SimpleTrend::GetUniqueInstance(t_dbglogger_, r_watch_, _source_shortcode_,
                                                              t_trend_history_secs_, _t_price_type_);
    }
  }
  indep_trend_indicator_->add_unweighted_indicator_listener(0, this);
}
void SlowStdevTrendCalculator::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
...
void SlowStdevTrendCalculator::InitializeValues() {
...
void SlowStdevTrendCalculator::OnMarketDataInterrupted(const unsigned int _security_id_,
                                                       const int msecs_since_last_receive_) {
  if (ShortcodeSecurityMarketViewMap::StaticCheckValidWithoutExit(source_shortcode_)) data_interrupted_ = true;
}

void SlowStdevTrendCalculator::OnMarketDataResumed(const unsigned int _security_id_) {
  InitializeValues();
