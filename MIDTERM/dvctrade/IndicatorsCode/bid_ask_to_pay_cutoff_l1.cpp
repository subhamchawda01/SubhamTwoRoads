...
void BidAskToPayCutoffL1::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                            std::vector<std::string>& _ors_source_needed_vec_,
                                            const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

BidAskToPayCutoffL1* BidAskToPayCutoffL1::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                            const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 8) {
    if (r_tokens_.size() < 7) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "BidAskToPayCutoffL1 incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_market_view_ _num_levels_ _size_to_seek_ _cutoff_increments_ _price_type_ ");
    } else {
      t_dbglogger_ << " BidAskToPayCutoffL1 incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
...
BidAskToPayCutoffL1* BidAskToPayCutoffL1::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                            ...
                                                            unsigned int _cutoff_increments_,
  ...
  static std::map<std::string, BidAskToPayCutoffL1*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new BidAskToPayCutoffL1(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                _num_levels_, _size_to_seek_, _cutoff_increments_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

BidAskToPayCutoffL1::BidAskToPayCutoffL1(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void BidAskToPayCutoffL1::OnMarketUpdate(const unsigned int _security_id_,
...
void BidAskToPayCutoffL1::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void BidAskToPayCutoffL1::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
