/**
    \file IndicatorsCode/online_ratio_calculator.cpp
...
void OnlineRatioCalculator::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OnlineRatioCalculator* OnlineRatioCalculator::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                            const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    if (r_tokens_.size() < 6) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "OnlineRatioCalculator incorrect syntax should b INDICATOR _this_weight_ _indicator_string_ "
                  "_dep_market_view_ _indep_market_view_ _fractional_seconds_ _price_type_");
    } else {
      t_dbglogger_ << "OnlineRatioCalculator incorrect syntax should b INDICATOR _this_weight_ _indicator_string_ "
                      "_dep_market_view_ _indep_market_view_ _fractional_seconds_ _price_type_"
                   << "\n";
  ...
  return GetUniqueInstance(t_dbglogger_, r_watch_,
                           ...
                           *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[4])), atof(r_tokens_[5]), t_price_type_);
}
...
OnlineRatioCalculator* OnlineRatioCalculator::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, OnlineRatioCalculator*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new OnlineRatioCalculator(
        t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_, _indep_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OnlineRatioCalculator::OnlineRatioCalculator(DebugLogger& t_dbglogger_, const Watch& r_watch_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      moving_avg_ratio_(0.0),
      last_ratio_recorded_(0.00),
      current_ratio_(0.00),
...
void OnlineRatioCalculator::WhyNotReady() {
  if (!is_ready_) {
...
void OnlineRatioCalculator::OnMarketUpdate(const unsigned int _security_id_,
                                         const MarketUpdateInfo& cr_market_update_info_) {
  if (_security_id_ == dep_market_view_.security_id()) {
    current_dep_price_ = SecurityMarketView::GetPriceFromType(basepx_pxtype_, cr_market_update_info_);
  } else {
...
// SMV is not ready
  ...
  if (!is_ready_) {
	...
	current_ratio_ = current_dep_price_ / current_indep_price_ ;
	if (watch_.msecs_from_midnight() - last_new_page_msecs_ < page_width_msecs_) {
	  moving_avg_ratio_ += inv_decay_sum_ * (current_ratio_ - last_ratio_recorded_);
	} else {
   	  ...
   	  if (num_pages_to_add_ >= (int)decay_vector_.size()) {
   	    ...
   	    if (num_pages_to_add_ == 1) {
   	      moving_avg_ratio_ = (current_ratio_ * inv_decay_sum_) + (moving_avg_ratio_ * decay_vector_[1]);
   	    } else {  // num_pages_to_add_ >= 2 < decay_vector_.size ( )
   	      moving_avg_ratio_ = (current_ratio_ * inv_decay_sum_) +
   	    		  (last_ratio_recorded_ * inv_decay_sum_ * decay_vector_sums_[(num_pages_to_add_ - 1)]) +
   	    		  (moving_avg_ratio_ * decay_vector_[num_pages_to_add_]);
   	    }
   	    last_new_page_msecs_ += (num_pages_to_add_ * page_width_msecs_);
   	  }
	}

	last_ratio_recorded_ = current_ratio_;
	indicator_value_ = moving_avg_ratio_;
        NotifyIndicatorListeners(indicator_value_);
  }
}

void OnlineRatioCalculator::InitializeValues() {
	moving_avg_ratio_ = current_ratio_;
	last_ratio_recorded_ = current_ratio_;
	indicator_value_ = current_ratio_;
	last_new_page_msecs_ = watch_.msecs_from_midnight() - watch_.msecs_from_midnight() % page_width_msecs_;
}

void OnlineRatioCalculator::set_basepx_pxtype(SecurityMarketView& t_dep_market_view_, PriceType_t _basepx_pxtype_) {
  CommonIndicator::set_basepx_pxtype(t_dep_market_view_, _basepx_pxtype_);
...
void OnlineRatioCalculator::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OnlineRatioCalculator::OnMarketDataResumed(const unsigned int _security_id_) {
