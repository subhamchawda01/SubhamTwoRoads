/**
    \file IndicatorsCode/curve_adjusted_simple_trend_momentum.cpp
...
void CurveAdjustedSimpleTrendMomentum::CollectShortCodes(
...
CurveAdjustedSimpleTrendMomentum* CurveAdjustedSimpleTrendMomentum::GetUniqueInstance(
  ...
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "CurveAdjustedSimpleTrendMomentum needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 CurveAdjustedSimpleTrendMomentum DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
CurveAdjustedSimpleTrendMomentum* CurveAdjustedSimpleTrendMomentum::GetUniqueInstance(
  ...
  static std::map<std::string, CurveAdjustedSimpleTrendMomentum*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new CurveAdjustedSimpleTrendMomentum(
...
CurveAdjustedSimpleTrendMomentum::CurveAdjustedSimpleTrendMomentum(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void CurveAdjustedSimpleTrendMomentum::OnIndicatorUpdate(const unsigned int& indicator_index_,
...
void CurveAdjustedSimpleTrendMomentum::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void CurveAdjustedSimpleTrendMomentum::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool CurveAdjustedSimpleTrendMomentum::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void CurveAdjustedSimpleTrendMomentum::InitializeValues() { indicator_value_ = 0; }
}
