/**
    \file IndicatorsCode/stable_ask_spot_future_spread.cpp
...
void StableAskSpotFutureSpread::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
StableAskSpotFutureSpread* StableAskSpotFutureSpread::GetUniqueInstance(DebugLogger& t_dbglogger_,
  ...
  if (r_tokens_.size() < 5) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight StableAskSpotFutureSpread _fut_market_view_ _spot_market_view_ ");
  }
...
StableAskSpotFutureSpread* StableAskSpotFutureSpread::GetUniqueInstance(DebugLogger& t_dbglogger_,
  ...
  static std::map<std::string, StableAskSpotFutureSpread*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new StableAskSpotFutureSpread(
        t_dbglogger_, r_watch_, concise_indicator_description_, _fut_market_view_, _spot_market_view_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

StableAskSpotFutureSpread::StableAskSpotFutureSpread(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (!fut_market_view_.subscribe_price_type(this, kPriceTypeStableBidPrice) ||
      !spot_market_view_.subscribe_price_type(this, kPriceTypeStableAskPrice)) {
    PriceType_t t_error_price_type_ = kPriceTypeStableAskPrice;
    std::cerr << typeid(*this).name() << ':' << __func__ << ':' << __LINE__ << ' ' << concise_indicator_description_
              << " passed " << t_error_price_type_ << std::endl;
  }
}

void StableAskSpotFutureSpread::WhyNotReady() {
  if (!is_ready_) {
...
void StableAskSpotFutureSpread::OnMarketUpdate(const unsigned int _security_id_,
                                               const MarketUpdateInfo& _market_update_info_) {
  if (_security_id_ == fut_market_view_.security_id()) {
    current_fut_price_ = SecurityMarketView::GetPriceFromType(kPriceTypeStableBidPrice, _market_update_info_) / 1000.0;
  } else if (_security_id_ == spot_market_view_.security_id()) {
    current_spot_price_ = SecurityMarketView::GetPriceFromType(kPriceTypeStableAskPrice, _market_update_info_);
  } else {
    return;
  }
...
void StableAskSpotFutureSpread::InitializeValues() { indicator_value_ = 0; }

// market_interrupt_listener interface
void StableAskSpotFutureSpread::OnMarketDataInterrupted(const unsigned int _security_id_,
                                                        const int msecs_since_last_receive_) {
  // Emitting the same value is the best thing to do here....so doing nothing
}

void StableAskSpotFutureSpread::OnMarketDataResumed(const unsigned int _security_id_) {
