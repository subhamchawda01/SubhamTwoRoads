/**
    \file IndicatorsCode/regime_online_offline_corr.cpp
...
void RegimeOnlineOfflineCorr::CollectShortCodes(std::vector<std::string>& r_shortcodes_affecting_this_indicator_,
...
RegimeOnlineOfflineCorr* RegimeOnlineOfflineCorr::GetUniqueInstance(DebugLogger& r_dbglogger_, const Watch& r_watch_,
                                                                    const std::vector<const char*>& r_tokens_,
                                                                    PriceType_t _basepx_pxtype_) {
  if (r_tokens_.size() < 8) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, r_dbglogger_,
                "INDICATOR weight RegimeOnlineOfflineCorr _dep_market_view_ _source_code_ _beta_secs_ _threshold_ "
                "_tolerance_ ");
  }
...
RegimeOnlineOfflineCorr* RegimeOnlineOfflineCorr::GetUniqueInstance(DebugLogger& r_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, RegimeOnlineOfflineCorr*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new RegimeOnlineOfflineCorr(r_dbglogger_, r_watch_, concise_indicator_description_, r_dep_market_view_,
                                    r_source_shortcode_, _beta_secs_, t_threshold_, t_tolerance_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

RegimeOnlineOfflineCorr::RegimeOnlineOfflineCorr(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void RegimeOnlineOfflineCorr::WhyNotReady() {
  if (!is_ready_) {
...
void RegimeOnlineOfflineCorr::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
  ...
  if (beta_updated_ && stdev_ratio_updated_) {
    if (!is_ready_) {
      ...
      if (indicator_value_ == 1 && corr_ratio_ >= threshold_ + tolerance_) {
        indicator_value_ = 2;
      } else if (indicator_value_ == 2 && corr_ratio_ <= threshold_ - tolerance_) {
        indicator_value_ = 1;
      }
...
void RegimeOnlineOfflineCorr::InitializeValues() {
  indicator_value_ = 1;
  UpdateLRInfo();
...
void RegimeOnlineOfflineCorr::UpdateLRInfo() {
