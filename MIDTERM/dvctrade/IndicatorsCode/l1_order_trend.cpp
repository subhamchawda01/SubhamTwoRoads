/**
    \file IndicatorsCode/l1_size_per_order_trend.cpp
...
void L1OrderTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                     std::vector<std::string>& _ors_source_needed_vec_,
                                     const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

L1OrderTrend* L1OrderTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
L1OrderTrend* L1OrderTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, L1OrderTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new L1OrderTrend(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                         _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

L1OrderTrend::L1OrderTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      moving_avg_l1_order_(0),
      last_l1_order_recorded_(0),
      current_l1_order_(0) {
...
void L1OrderTrend::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
  current_l1_order_ = (_market_update_info_.bestask_ordercount_ + _market_update_info_.bestbid_ordercount_) / 2;
  ...
  if (!is_ready_) {
    ...
    if (watch_.msecs_from_midnight() - last_new_page_msecs_ < page_width_msecs_) {
      moving_avg_l1_order_ += inv_decay_sum_ * (current_l1_order_ - last_l1_order_recorded_);
    } else {
      ...
      if (num_pages_to_add_ >= (int)decay_vector_.size()) {
        ...
        if (num_pages_to_add_ == 1) {
          moving_avg_l1_order_ = (current_l1_order_ * inv_decay_sum_) + (moving_avg_l1_order_ * decay_vector_[1]);
        } else {  // num_pages_to_add_ >= 2 < decay_vector_.size ( )
          moving_avg_l1_order_ = (current_l1_order_ * inv_decay_sum_) + (last_l1_order_recorded_ * inv_decay_sum_ *
                                                                         decay_vector_sums_[(num_pages_to_add_ - 1)]) +
                                 (moving_avg_l1_order_ * decay_vector_[num_pages_to_add_]);
        }
        last_new_page_msecs_ += (num_pages_to_add_ * page_width_msecs_);
      }
    }

    last_l1_order_recorded_ = current_l1_order_;
    indicator_value_ = moving_avg_l1_order_;
...
void L1OrderTrend::InitializeValues() {
  moving_avg_l1_order_ = current_l1_order_;
  last_l1_order_recorded_ = current_l1_order_;
...
void L1OrderTrend::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void L1OrderTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
