...
void EventOwpDiffPriceL1::CollectShortCodes(std::vector<std::string> &_shortcodes_affecting_this_indicator_,
                                            std::vector<std::string> &_ors_source_needed_vec_,
                                            const std::vector<const char *> &r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

EventOwpDiffPriceL1 *EventOwpDiffPriceL1::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
                                                            const std::vector<const char *> &r_tokens_,
  ...
  if (r_tokens_.size() < 6) {
    if (r_tokens_.size() < 5) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "   EventOwpDiffPriceL1 Incorrect Syntax. Correct syntax would b  INDICATOR _this_weight_ "
                  "_indicator_string_ _indep_market_view_ _fractional_seconds_ _price_type_");
    } else {
      t_dbglogger_ << "   EventOwpDiffPriceL1 Incorrect Syntax. Correct syntax would b  INDICATOR _this_weight_ "
                      "_indicator_string_ _indep_market_view_ _fractional_seconds_ _price_type_ "
                   << "\n";
...
EventOwpDiffPriceL1 *EventOwpDiffPriceL1::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
  ...
  static std::map<std::string, EventOwpDiffPriceL1 *> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new EventOwpDiffPriceL1(t_dbglogger_, r_watch_, concise_indicator_description_, t_indep_market_view_,
                                _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

EventOwpDiffPriceL1::EventOwpDiffPriceL1(DebugLogger &t_dbglogger_, const Watch &r_watch_,
  ...
  // used below
  if (!t_indep_market_view_.subscribe_price_type(this, kPriceTypeOrderWPrice)) {
    PriceType_t t_error_price_type_ = _price_type_;
    std::cerr << typeid(*this).name() << ':' << __func__ << ':' << __LINE__ << ' ' << " passed " << t_error_price_type_
              << std::endl;
  }
  ...
  // used below
  if (!t_indep_market_view_.subscribe_price_type(this, kPriceTypeMktSizeWPrice)) {
    PriceType_t t_error_price_type_ = _price_type_;
    std::cerr << typeid(*this).name() << ':' << __func__ << ':' << __LINE__ << ' ' << " passed " << t_error_price_type_
              << std::endl;
  }
...
void EventOwpDiffPriceL1::OnTimePeriodUpdate(const int num_pages_to_add_) {
...
void EventOwpDiffPriceL1::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo &_trade_print_info_,
                                       const MarketUpdateInfo &_market_update_info_) {
  OnMarketUpdate(_security_id_, _market_update_info_);
}

void EventOwpDiffPriceL1::OnMarketUpdate(const unsigned int _security_id_,
                                         const MarketUpdateInfo &_market_update_info_) {
    ...
    if (!is_ready_) {
      ...
      if (first_time_) {
        ...
        if ((last_bid_int_price_ != _market_update_info_.bestbid_int_price_) ||
          ...
          last_level_shift_events_ = indep_market_view_.l1events();
        ...
        if (_market_update_info_.spread_increments_ > 1) {
          ...
          // use OrderWPrice till 3*avg_l1_events_per_sec if a new level is formed
          if (indep_market_view_.l1events() - last_level_shift_events_ < 3 * avg_l1_events_per_sec_) {
...
void EventOwpDiffPriceL1::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void EventOwpDiffPriceL1::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
...
void EventOwpDiffPriceL1::Initialize() {
    ...
    while (t_l1_book_avg_infile_.good()) {
      ...
      if (tokens_.size() == 6) {
        ...
        if ((indep_market_view_.shortcode().compare(dep_indep_portfolio_code_) == 0) &&
          ...
          avg_l1_events_per_sec_ = atof(tokens_[4]);
