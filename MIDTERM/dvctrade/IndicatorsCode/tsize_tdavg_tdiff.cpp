/**
    \file IndicatorsCode/tsize_tdavg_tdiff.cpp
...
void TSizeTDAvgTDiff::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                        std::vector<std::string>& _ors_source_needed_vec_,
                                        const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

TSizeTDAvgTDiff* TSizeTDAvgTDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                    const std::vector<const char*>& r_tokens_,
...
TSizeTDAvgTDiff* TSizeTDAvgTDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, TSizeTDAvgTDiff*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new TSizeTDAvgTDiff(
        t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _fractional_seconds_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

TSizeTDAvgTDiff::TSizeTDAvgTDiff(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void TSizeTDAvgTDiff::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
#define MIN_SIGNIFICANT_SUM_SZ_TRADED 0.10

  if (time_decayed_trade_info_manager_.sumsz_ > MIN_SIGNIFICANT_SUM_SZ_TRADED) {
    indicator_value_ = time_decayed_trade_info_manager_.sumtdiffsz_ / time_decayed_trade_info_manager_.sumsz_;
  } else {
    indicator_value_ = 0;
  }
...
void TSizeTDAvgTDiff::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
...
void TSizeTDAvgTDiff::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void TSizeTDAvgTDiff::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
