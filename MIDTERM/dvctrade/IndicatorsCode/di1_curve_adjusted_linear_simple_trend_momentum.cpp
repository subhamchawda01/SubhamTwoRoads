/**
    \file IndicatorsCode/di1_curve_linear_adjusted_simple_trend_momentum.cpp
...
void DI1CurveAdjustedLinearSimpleTrendMomentum::CollectShortCodes(
...
DI1CurveAdjustedLinearSimpleTrendMomentum* DI1CurveAdjustedLinearSimpleTrendMomentum::GetUniqueInstance(
  ...
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "DI1CurveAdjustedLinearSimpleTrendMomentum needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 DI1CurveAdjustedLinearSimpleTrendMomentum DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
DI1CurveAdjustedLinearSimpleTrendMomentum* DI1CurveAdjustedLinearSimpleTrendMomentum::GetUniqueInstance(
  ...
  static std::map<std::string, DI1CurveAdjustedLinearSimpleTrendMomentum*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new DI1CurveAdjustedLinearSimpleTrendMomentum(
...
DI1CurveAdjustedLinearSimpleTrendMomentum::DI1CurveAdjustedLinearSimpleTrendMomentum(
...
void DI1CurveAdjustedLinearSimpleTrendMomentum::OnIndicatorUpdate(const unsigned int& indicator_index_,
...
void DI1CurveAdjustedLinearSimpleTrendMomentum::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DI1CurveAdjustedLinearSimpleTrendMomentum::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool DI1CurveAdjustedLinearSimpleTrendMomentum::AreAllReady() {
  return VectorUtils::CheckAllForValue(is_ready_vec_, true);
}

void DI1CurveAdjustedLinearSimpleTrendMomentum::InitializeValues() { indicator_value_ = 0; }
}
