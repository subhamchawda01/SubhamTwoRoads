/**
    \file IndicatorsCode/mean_normalized_diff.cpp
...
void MeanNormalizedDiff::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
MeanNormalizedDiff* MeanNormalizedDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                          const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    ExitVerbose(
        ...
        "INDICATOR weight MeanNormalizedDiff _dep_market_view_ _indep_market_view_ _fractional_seconds_ _price_type_ ");
  }
...
MeanNormalizedDiff* MeanNormalizedDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, MeanNormalizedDiff*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new MeanNormalizedDiff(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                               _indep_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

MeanNormalizedDiff::MeanNormalizedDiff(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void MeanNormalizedDiff::WhyNotReady() {
  if (!is_ready_) {
...
void MeanNormalizedDiff::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    ...
    indicator_value_ = current_indep_price_ * (moving_avg_dep_ / moving_avg_indep_) - current_dep_price_;
    NotifyIndicatorListeners(indicator_value_);
  }
}

void MeanNormalizedDiff::InitializeValues() {
...
void MeanNormalizedDiff::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void MeanNormalizedDiff::OnMarketDataResumed(const unsigned int _security_id_) {
