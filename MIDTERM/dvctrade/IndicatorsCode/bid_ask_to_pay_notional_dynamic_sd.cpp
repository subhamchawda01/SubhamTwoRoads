...
void BidAskToPayNotionalDynamicSD::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                                     std::vector<std::string>& _ors_source_needed_vec_,
                                                     const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

BidAskToPayNotionalDynamicSD* BidAskToPayNotionalDynamicSD::GetUniqueInstance(DebugLogger& t_dbglogger_,
  ...
  if (r_tokens_.size() < 8) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                ...
                "_indep_market_view_ _num_levels_ _size_to_seek_ _fractional_seconds_ _price_type_ ");
  } else {
  ...
  return GetUniqueInstance(
      ...
      std::max(0, atoi(r_tokens_[4])), std::max(0, atoi(r_tokens_[5])), atof(r_tokens_[6]), t_price_type_);
}

BidAskToPayNotionalDynamicSD* BidAskToPayNotionalDynamicSD::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, SecurityMarketView& _indep_market_view_,
    unsigned int _num_levels_, unsigned int _size_to_seek_, double _fractional_seconds_, PriceType_t _price_type_) {
  ...
  static std::map<std::string, BidAskToPayNotionalDynamicSD*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new BidAskToPayNotionalDynamicSD(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                         _num_levels_, _size_to_seek_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

BidAskToPayNotionalDynamicSD::BidAskToPayNotionalDynamicSD(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  int idx = 0;
  stdev_ratio_calculator_ =
      StdevRatioCalculator::GetUniqueInstance(t_dbglogger_, r_watch_, t_indep_market_view_, _fractional_seconds_, 0);
  if (stdev_ratio_calculator_ != nullptr) {
    stdev_ratio_calculator_->AddStdevRatioListener(idx, this);
  }
}

void BidAskToPayNotionalDynamicSD::OnStdevRatioUpdate(const unsigned int index_to_send_,
                                                      const double& r_new_scaled_stdev_value_) {
  if (!is_ready_) {
    if (stdev_ratio_calculator_->IsIndicatorReady()) {
      is_ready_ = true;
    }
  }
  if (index_to_send_ == 0u) {
    stdev_ratio_ = r_new_scaled_stdev_value_;
  }
}

void BidAskToPayNotionalDynamicSD::OnMarketUpdate(const unsigned int _security_id_,
                                                  const MarketUpdateInfo& cr_market_update_info_) {
  if (!is_ready_) {
    if (indep_market_view_.is_ready_complex(2) && stdev_ratio_calculator_->IsIndicatorReady()) {
      is_ready_ = true;
      indicator_value_ = 0;
    ...
    // std::cout << volume_ratio_ << "\n";
    double notional_bid_size_left_to_seek_ = size_to_seek_ * stdev_ratio_;
    double notional_ask_size_left_to_seek_ = size_to_seek_ * stdev_ratio_;
    unsigned int num_levels_new_ = (int)ceil(num_levels_ * stdev_ratio_);

    // std::cout << stdev_ratio_ << "\n";
    ...
    if (indep_market_view_.market_update_info_.temporary_bool_checking_if_this_is_an_indexed_book_) {
      ...
      } while (bid_levels_so_far_ < num_levels_new_ && t_current_bid_index_ > 0);
      ...
      } while ((ask_levels_so_far_ < num_levels_new_) && (t_current_ask_index_ > 0));
    } else {
      ...
      } while (bid_levels_so_far_ < num_levels_new_ && bid_level_ < indep_market_view_.bidlevels_size());
      ...
      } while (ask_levels_so_far_ < num_levels_new_ && ask_level_ < indep_market_view_.asklevels_size());
    }
...
#define MIN_SIGNIFICANT_FACTOR 1
    if ((sum_bid_size_ <= MIN_SIGNIFICANT_FACTOR) ||
      ...
      double unconstrained_proj_price_ = (sum_bid_px_size_ / sum_bid_size_ + sum_ask_px_size_ / sum_ask_size_) / 2.0;
      double proj_price_ =
...
void BidAskToPayNotionalDynamicSD::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void BidAskToPayNotionalDynamicSD::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
