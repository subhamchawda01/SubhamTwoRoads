/**
    \file IndicatorsCode/book_size_diff.cpp
...
void BookSizeDiff::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                     std::vector<std::string>& _ors_source_needed_vec_,
                                     const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

BookSizeDiff* BookSizeDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (r_tokens_.size() < 6) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "BookSizeDiff incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
...
BookSizeDiff* BookSizeDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, BookSizeDiff*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new BookSizeDiff(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_levels_,
                         _decay_factor_, _stdev_duration_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

BookSizeDiff::BookSizeDiff(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (book_info_struct_ == NULL) {
    std::cerr << "Error getting book_info_struct for BookSizeDiff: " << _num_levels_ << " " << _decay_factor_
              << std::endl;
  }
...
void BookSizeDiff::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& cr_market_update_info_) {
  if (!is_ready_) {
...
#define MIN_SIGNIFICANT_FACTOR 0.10
    if ((book_info_struct_->sum_bid_factor_ <= MIN_SIGNIFICANT_FACTOR) ||
      ...
      indicator_value_ = (book_info_struct_->sum_bid_factor_size_ / book_info_struct_->sum_bid_factor_) -
                         (book_info_struct_->sum_ask_factor_size_ / book_info_struct_->sum_ask_factor_);
    }
...
void BookSizeDiff::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void BookSizeDiff::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
