/**
  \file IndicatorsCode/level_size.cpp
...
void LevelSize::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                  std::vector<std::string>& _ors_source_needed_vec_,
                                  const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

LevelSize* LevelSize::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                        const std::vector<const char*>& r_tokens_, bool _is_ask) {
  if (r_tokens_.size() < 6) {
    std::cerr << "Invalid Args to LevelSize\n";
    exit(1);
  }
...
LevelSize* LevelSize::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, LevelSize*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new LevelSize(
        t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, book_level_, _is_ask);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

LevelSize::LevelSize(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                     const std::string& concise_indicator_description_, SecurityMarketView& _indep_market_view_,
                     int _book_level_, bool is_ask)
...
void LevelSize::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
  if (!is_ready_) {
    ...
    if (!_is_ask) {
      indicator_value_ = indep_market_view_.bid_size(book_level_);
    } else {
      indicator_value_ = indep_market_view_.ask_size(book_level_);
    }
...
void LevelSize::InitializeValues() { indicator_value_ = 0; }

// market_interrupt_listener interface
void LevelSize::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void LevelSize::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
