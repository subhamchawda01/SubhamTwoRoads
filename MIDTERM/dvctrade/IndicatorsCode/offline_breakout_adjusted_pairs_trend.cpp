...
void OfflineBreakoutAdjustedPairsTrend::CollectShortCodes(
...
OfflineBreakoutAdjustedPairsTrend* OfflineBreakoutAdjustedPairsTrend::GetUniqueInstance(
...
OfflineBreakoutAdjustedPairsTrend* OfflineBreakoutAdjustedPairsTrend::GetUniqueInstance(
  ...
  t_temp_oss_ << VarName() << ' ' << _dep_market_view_.secname() << ' ' << _indep_market_view_.secname() << ' '
              << _fractional_seconds_ << ' ' << t_trend_weight_ << ' ' << PriceType_t_To_String(_price_type_);
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, OfflineBreakoutAdjustedPairsTrend*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new OfflineBreakoutAdjustedPairsTrend(
...
OfflineBreakoutAdjustedPairsTrend::OfflineBreakoutAdjustedPairsTrend(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::string& concise_indicator_description_,
    const SecurityMarketView& t_dep_market_view_, const SecurityMarketView& t_indep_market_view_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      p_dep_indicator_brk_(nullptr),
      p_indep_indicator_brk_(nullptr) {
  watch_.subscribe_FifteenSecondPeriod(this);  // for UpdateLRInfo and updating trend adjustment
  // watch_.subscribe_BigTimePeriod ( this ); // for UpdateLRInfo and updating trend adjustment
  // std::cout<<"Values being "<<dep_mean_l1_norm_<<" "<<indep_mean_l1_norm_<<std::endl;
  ...
  if (lrdb_absent_) {
    ...
    if (p_dep_indicator_brk_ == nullptr) {
      indicator_value_ = 0;
      is_ready_ = true;
      return;
    }
    ...
    p_indep_indicator_brk_ = SimpleTrend::GetUniqueInstance(t_dbglogger_, r_watch_, t_indep_market_view_.shortcode(),
                                                            BREAKOUT_TREND_SECONDS, kPriceTypeMktSizeWPrice);
    if (p_indep_indicator_brk_ == nullptr) {
      indicator_value_ = 0;
      is_ready_ = true;
      return;
    }
...
void OfflineBreakoutAdjustedPairsTrend::OnTimePeriodUpdate(const int num_pages_to_add_) {
  UpdateLRInfo();
  dep_l1_norm_ratio_ = std::abs(dep_price_trend_brk_) / dep_mean_l1_norm_;
  indep_l1_norm_ratio_ = std::abs(indep_price_trend_brk_) / indep_mean_l1_norm_;
  if (indep_volume_ratio_ > indep_l1_norm_ratio_) {
    max_ratio_indep_ = indep_volume_ratio_;
    max_ratio_dep_ = dep_volume_ratio_;
  } else {
    max_ratio_indep_ = indep_l1_norm_ratio_;
    max_ratio_dep_ = dep_l1_norm_ratio_;
  }
  if (max_ratio_indep_ < 0.8) {
    if (max_ratio_dep_ > 1.0) {
      pred_mode_ = 0u;  // full dt
    } else {
      pred_mode_ = 3u;  // just 0
    }
  } else {
    if (max_ratio_indep_ > max_ratio_dep_) {
      pred_mode_ = 1u;  // full offline
    } else {
      if (max_ratio_indep_ > 0.8 * max_ratio_dep_) {
...
void OfflineBreakoutAdjustedPairsTrend::OnVolumeRatioUpdate(const unsigned int r_security_id_,
...
void OfflineBreakoutAdjustedPairsTrend::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
...
void OfflineBreakoutAdjustedPairsTrend::WhyNotReady() {
  if (!is_ready_) {
...
void OfflineBreakoutAdjustedPairsTrend::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineBreakoutAdjustedPairsTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void OfflineBreakoutAdjustedPairsTrend::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineBreakoutAdjustedPairsTrend::UpdateLRInfo() {
