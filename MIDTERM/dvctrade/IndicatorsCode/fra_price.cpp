/**
    \file IndicatorsCode/fra_price.cpp
...
void FRAPrice::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
FRAPrice* FRAPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                const std::vector<const char*>& r_tokens_,
                                                                PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 7u) {
    ExitVerbose(kExitErrorCodeGeneral, "FRAPrice needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 FRAPrice BR_DI1N_0 BR_DI_0 BR_DI_1 PRICE_TYPE HALFLIFE
...
FRAPrice* FRAPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, FRAPrice*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new FRAPrice(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                  _indep1_market_view_, _indep2_market_view_, t_price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

FRAPrice::FRAPrice(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void FRAPrice::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (indep1_market_view_.security_id() == _security_id_) {
    ...
    inv_pu_indep1_ = ComputeInvPU(current_indep1_price_, indep1_term_);
    ...
    inv_pu_indep2_ = ComputeInvPU(current_indep2_price_, indep2_term_);
    ...
    inv_pu_dep_ = ComputeInvPU(current_dep_price_, dep_term_);
  }
  ...
  if (!is_ready_) {
    ...
    current_predicted_price_ = ComputeSlideFRA();
    indicator_value_ = current_predicted_price_;
    NotifyIndicatorListeners(indicator_value_);
  }
}

// market_interrupt_listener interface
void FRAPrice::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void FRAPrice::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
void FRAPrice::InitializeValues() {
  current_dep_price_ = 0;
...
double FRAPrice::ComputeInvPU(double _price_, int _term_) {
  double t_inv_pu_ = 0;
  t_inv_pu_ = pow(1 + _price_/100, _term_/252.0);
  return t_inv_pu_;
}

double FRAPrice::ComputeSlideFRA() {
  double t_fra_price_;
  double t_fwd_rate_dep_indep1_ = 100 *(pow ( inv_pu_dep_/ inv_pu_indep1_, 252.0 / (indep1_term_ - dep_term_)) - 1);
  double t_fwd_rate_indep2_indep1_ = 100 *(pow ( inv_pu_indep1_/ inv_pu_indep2_, 252.0 / (indep2_term_ - indep1_term_)) - 1);
  t_fra_price_ = t_fwd_rate_indep2_indep1_ - t_fwd_rate_dep_indep1_;
  return t_fra_price_;
}
}
