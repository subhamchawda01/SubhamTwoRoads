/**
    \file IndicatorsCode/di_pricing_indicator_mktevents.cpp
...
DIPricingIndicatorMktEvents* DIPricingIndicatorMktEvents::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                            const Watch& r_watch_,
                                                                            const std::vector<const char*>& r_tokens_,
                                                                            PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "DIPricingIndicatorMktEvents needs 10 tokens");
    return NULL;
  }
...
DIPricingIndicatorMktEvents* DIPricingIndicatorMktEvents::GetUniqueInstance(
    ...
    unsigned int _num_events_halflife_, PriceType_t _t_pxtype_)

{
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << t_dep_market_view_.secname() << ' ' << t_indep_1_market_view_.secname() << ' '
              << t_indep_2_market_view_.secname() << ' ' << _num_events_halflife_ << ' ' << _t_pxtype_;
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, DIPricingIndicatorMktEvents*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new DIPricingIndicatorMktEvents(
        t_dbglogger_, r_watch_, concise_indicator_description_, t_dep_market_view_, t_indep_1_market_view_,
        t_indep_2_market_view_, _num_events_halflife_, _t_pxtype_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

void DIPricingIndicatorMktEvents::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
void DIPricingIndicatorMktEvents::OnMarketUpdate(const unsigned int _security_id_,
...
inline void DIPricingIndicatorMktEvents::OnTradePrint(const unsigned int _security_id_,
                                                      const TradePrintInfo& _trade_print_info_,
                                                      const MarketUpdateInfo& _market_update_info_) {
  OnMarketUpdate(_security_id_, _market_update_info_);
}

inline void DIPricingIndicatorMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
                                                                 const int msecs_since_last_receive_) {}
inline void DIPricingIndicatorMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {}

void DIPricingIndicatorMktEvents::WhyNotReady() {
  if (!is_ready_) {
    if (!dep_ready_) {
...
void DIPricingIndicatorMktEvents::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
  if (!is_ready_) {
...
inline void DIPricingIndicatorMktEvents::OnIndicatorUpdate(const unsigned int& _indicator_index_,
                                                           const double& new_value_decrease_,
                                                           const double& new_value_nochange_,
                                                           const double& new_value_increase_) {
  return;
}

DIPricingIndicatorMktEvents::DIPricingIndicatorMktEvents(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  indep_1_p_ = indep_1_p_ / dep_p_;  // small optimization to prevent division on each update
  indep_2_p_ = indep_2_p_ / dep_p_;

  indep_1_trend_indicator_ = SimpleTrendMktEvents::GetUniqueInstance(t_dbglogger_, r_watch_, indep_1_market_view_,
                                                                     _num_events_halflife_, t_pxtype_);
  indep_2_trend_indicator_ = SimpleTrendMktEvents::GetUniqueInstance(t_dbglogger_, r_watch_, indep_2_market_view_,
                                                                     _num_events_halflife_, t_pxtype_);
