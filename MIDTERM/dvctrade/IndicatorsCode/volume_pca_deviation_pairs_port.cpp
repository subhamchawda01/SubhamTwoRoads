...
void VolumePCADeviationPairsPort::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
VolumePCADeviationPairsPort* VolumePCADeviationPairsPort::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                            const Watch& r_watch_,
                                                                            const std::vector<const char*>& r_tokens_,
                                                                            PriceType_t _basepx_pxtype_) {
  if (r_tokens_.size() < 7) {
    std::cerr << "VolumePCADeviationPairsPort syntax incorrect! given text:";
    for (auto i = 0u; i < r_tokens_.size(); i++) {
      std::cerr << " " << r_tokens_[i];
    }
    std::cerr << std::endl;
    std::cerr << "Expected syntax: INDICATOR  _this_weight_  VolumePCADeviationPairsPort  _dep_market_view_  "
...
VolumePCADeviationPairsPort* VolumePCADeviationPairsPort::GetUniqueInstance(
  ...
  static std::map<std::string, VolumePCADeviationPairsPort*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
    ...
    if (principal_eigen_vec.size() < 1) {
      std::cerr << "VolumePCADeviationPairsPort: No eigen vectors loaded for " << (_portfolio_descriptor_shortcode_)
                << " in PcaWeightsManager " << std::endl;
      exit(1);
    }

    concise_indicator_description_map_[concise_indicator_description_] = new VolumePCADeviationPairsPort(
...
VolumePCADeviationPairsPort::VolumePCADeviationPairsPort(
  ...
  if (dep_index_in_portfolio_ < 0) {
    ...
    DBGLOG_DUMP;
    std::cerr << "VolumePCADeviationPairsPort: since " << dep_index_in_portfolio_
              << " not in PortFolio. Dep:" << r_dep_market_view_.shortcode()
...
void VolumePCADeviationPairsPort::WhyNotReady() {
  if (!is_ready_) {
...
void VolumePCADeviationPairsPort::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
...
void VolumePCADeviationPairsPort::OnVolumeUpdate(unsigned int t_index_, double r_new_volume_value_) {
...
void VolumePCADeviationPairsPort::InitializeValues(int _index_) {
