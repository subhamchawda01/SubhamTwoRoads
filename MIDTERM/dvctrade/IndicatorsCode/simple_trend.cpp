...
void SimpleTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
SimpleTrend* SimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
SimpleTrend* SimpleTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_, std::string _shortcode_,
  ...
  static std::map<std::string, SimpleTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new SimpleTrend(
        t_dbglogger_, r_watch_, concise_indicator_description_, _shortcode_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SimpleTrend::SimpleTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void SimpleTrend::WhyNotReady() {
  if (!is_ready_) {
    if (is_indep_portfolio_) {
      if (!indep_portfolio_->is_ready()) {
        DBGLOG_TIME_CLASS << shortcode_ << " is_ready_ = false " << DBGLOG_ENDL_FLUSH;
        DBGLOG_DUMP;
      }
    } else {
...
void SimpleTrend::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
...
void SimpleTrend::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
...
void SimpleTrend::UpdateComputedVariables() {
...
void SimpleTrend::InitializeValues() {
...
void SimpleTrend::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void SimpleTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (!is_indep_portfolio_ && indep_market_view_->security_id() == _security_id_) {
    InitializeValues();
