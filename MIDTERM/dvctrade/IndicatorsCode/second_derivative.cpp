/**
    \file IndicatorsCode/second_derivative.cpp
...
void SecondDerivative::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                         std::vector<std::string>& _ors_source_needed_vec_,
                                         const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

SecondDerivative* SecondDerivative::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                      const std::vector<const char*>& r_tokens_,
...
SecondDerivative* SecondDerivative::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, SecondDerivative*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new SecondDerivative(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                             _fractional_st_seconds_, _fractional_lt_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SecondDerivative::SecondDerivative(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  indicator_vec_.push_back(p_st_indicator_);

  double ratio_lt_st_ = std::max(1.00, (_fractional_lt_seconds_ / _fractional_st_seconds_));
  ...
  indicator_vec_.push_back(p_lt_indicator_);
}

void SecondDerivative::WhyNotReady() {
  if (!is_ready_) {
...
void SecondDerivative::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
  if (!is_ready_) {
    is_ready_ = true;
...
void SecondDerivative::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
  data_interrupted_ = true;
  indicator_value_ = 0;
  NotifyIndicatorListeners(indicator_value_);
}

void SecondDerivative::OnMarketDataResumed(const unsigned int _security_id_) { data_interrupted_ = false; }
}
