...
void VolumeRatioRegime::CollectShortCodes(std::vector<std::string>& t_shortcodes_affecting_this_indicator_,
...
VolumeRatioRegime* VolumeRatioRegime::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        const std::vector<const char*>& r_tokens_,
                                                        PriceType_t _basepx_pxtype_) {
  // INDICATOR _this_weight_ _indicator_string_ _dep_market_view_ _indep_market_view_ _fractional_seconds_
  // _switch_threshold_ _price_type_
  if (r_tokens_.size() < 8) {
    ExitVerbose(kExitErrorCodeGeneral,
                " VolumeRatioRegime: Incomplete Argument\n INDICATOR _this_weight_ _indicator_string_ "
                "_dep_market_view_ _indep_market_view_ _fractional_seconds_ _switch_threshold_  _price_type_");
  }
...
VolumeRatioRegime* VolumeRatioRegime::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        ...
                                                        const std::string& indep_name, double _fractional_seconds_,
                                                        double _switch_threshold_, PriceType_t _price_type_) {
  ...
  static std::map<std::string, VolumeRatioRegime*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new VolumeRatioRegime(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_, indep_name,
                              _fractional_seconds_, _switch_threshold_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

VolumeRatioRegime::VolumeRatioRegime(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                     const std::string& concise_indicator_description_,
                                     SecurityMarketView& _dep_market_view_, const std::string& indep_name,
                                     double _fractional_seconds_, double switch_threshold, PriceType_t _price_type_)
  ...
  dep_volume_ratio_calculator_ =
      VolumeRatioCalculator::GetUniqueInstance(t_dbglogger_, watch_, _dep_market_view_, _fractional_seconds_);
  if (dep_volume_ratio_calculator_ != nullptr) {
    dep_volume_ratio_calculator_->AddVolumeRatioListener(idx, this);
  }
  ...
  if (IndicatorUtil::IsPortfolioShortcode(indep_name)) {
    ...
    indep_volume_ratio_calculator_port_ =
        VolumeRatioCalculatorPort2::GetUniqueInstance(dbglogger_, watch_, indep_name, _fractional_seconds_);
    if (indep_volume_ratio_calculator_port_ != nullptr) {
      indep_volume_ratio_calculator_port_->AddVolumeRatioListener(idx, this);
    }
  } else {
    ...
    indep_volume_ratio_calculator_ = VolumeRatioCalculator::GetUniqueInstance(
        t_dbglogger_, watch_, *indep_market_view_vec_[0], _fractional_seconds_);
    if (indep_volume_ratio_calculator_ != nullptr) {
      indep_volume_ratio_calculator_->AddVolumeRatioListener(idx, this);
    }
  }
...
void VolumeRatioRegime::OnTimePeriodUpdate(const int num_pages_to_add_) { ComputeRegime(); }

void VolumeRatioRegime::OnVolumeRatioUpdate(const unsigned int index_to_send_,
...
void VolumeRatioRegime::ComputeRegime() {
...
void VolumeRatioRegime::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
  indicator_value_ = 3u;
}

void VolumeRatioRegime::OnMarketDataResumed(const unsigned int _security_id_) { indicator_value_ = 3u; }
}
