/**
    \file IndicatorsCode/mult_mkt_complex_price.cpp
...
void MultMktComplexPrice::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                            std::vector<std::string>& _ors_source_needed_vec_,
                                            const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

MultMktComplexPrice* MultMktComplexPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                            const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    if (r_tokens_.size() < 6) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "MultMktComplexPrice incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ ");
    } else {
      t_dbglogger_ << "MultMktComplexPrice incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                      "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ "
                   << "\n";
...
MultMktComplexPrice* MultMktComplexPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, MultMktComplexPrice*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new MultMktComplexPrice(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                _num_levels_, _decay_factor_, _price_type_, _stdev_duration_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

MultMktComplexPrice::MultMktComplexPrice(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (book_info_struct_ == NULL) {
    std::cerr << "Error getting book_info_struct for MultMktComplexPrice: " << _num_levels_ << " " << _decay_factor_
              << std::endl;
  }
...
void MultMktComplexPrice::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    ...
    if ((book_info_struct_->sum_bid_factor_size_ <= 0) || (book_info_struct_->sum_ask_factor_size_ <= 0)) {
      ...
      double adjusted_price_ =
          ...
          ((book_info_struct_->sum_bid_factor_price_size_ + book_info_struct_->sum_ask_factor_price_size_) /
...
void MultMktComplexPrice::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void MultMktComplexPrice::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
