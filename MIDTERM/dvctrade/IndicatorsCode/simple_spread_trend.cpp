/**
    \file IndicatorsCode/simple_spread_trend.cpp
...
void SimpleSpreadTrend::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                          std::vector<std::string>& _ors_source_needed_vec_,
                                          const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

SimpleSpreadTrend* SimpleSpreadTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        const std::vector<const char*>& r_tokens_,
...
SimpleSpreadTrend* SimpleSpreadTrend::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, SimpleSpreadTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new SimpleSpreadTrend(t_dbglogger_, r_watch_, concise_indicator_description_, _indep1_market_view_,
                              _indep2_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SimpleSpreadTrend::SimpleSpreadTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                     const std::string& concise_indicator_description_,
                                     const SecurityMarketView& _indep1_market_view_,
                                     const SecurityMarketView& _indep2_market_view_, double _fractional_seconds_,
  ...
  if (!indep1_market_view_.subscribe_price_type(this, _price_type_) ||
      !indep2_market_view_.subscribe_price_type(this, _price_type_)) {
...
void SimpleSpreadTrend::WhyNotReady() {
  if (!is_ready_) {
...
void SimpleSpreadTrend::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
  ...
  current_spread_ = current_indep2_price_ - current_indep1_price_;
  ...
  if (!is_ready_) {
    ...
    if (watch_.msecs_from_midnight() - last_new_page_msecs_ < page_width_msecs_) {
      moving_avg_spread_ += inv_decay_sum_ * (current_spread_ - last_spread_recorded_);
    } else {
      ...
      if (num_pages_to_add_ >= (int)decay_vector_.size()) {
        ...
        if (num_pages_to_add_ == 1) {
          moving_avg_spread_ = (current_spread_ * inv_decay_sum_) + (moving_avg_spread_ * decay_vector_[1]);
        } else {  // num_pages_to_add_ >= 2 < decay_vector_.size ( )
          moving_avg_spread_ = (current_spread_ * inv_decay_sum_) +
    ...
    last_spread_recorded_ = current_spread_;
    indicator_value_ = (current_spread_ - moving_avg_spread_);
...
void SimpleSpreadTrend::InitializeValues() {
  moving_avg_spread_ = current_spread_;
  last_spread_recorded_ = current_spread_;
...
void SimpleSpreadTrend::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
  if (indep1_market_view_.security_id() == _security_id_ || indep2_market_view_.security_id() == _security_id_) {
...
void SimpleSpreadTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_ && indep2_market_view_.security_id() == _security_id_) {
    InitializeValues();
