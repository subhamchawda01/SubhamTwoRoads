/**
   \file IndicatorsCode/book_order_diff.cpp
...
void BookOrderDiff::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                      std::vector<std::string>& _ors_source_needed_vec_,
                                      const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

BookOrderDiff* BookOrderDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 6) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "BookOrderDiff incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
...
BookOrderDiff* BookOrderDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, BookOrderDiff*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new BookOrderDiff(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_levels_,
                          _decay_factor_, _stdev_duration_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

BookOrderDiff::BookOrderDiff(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (book_info_struct_ == NULL) {
    std::cerr << "Error getting book_info_struct for BookOrderDiff: " << _num_levels_ << " " << _decay_factor_
              << std::endl;
  }
...
void BookOrderDiff::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& cr_market_update_info_) {
  if (!is_ready_) {
...
#define MIN_SIGNIFICANT_FACTOR 1.00
    if ((book_info_struct_->sum_bid_factor_ <= MIN_SIGNIFICANT_FACTOR) ||
      ...
      indicator_value_ = (book_info_struct_->sum_bid_factor_order_ / book_info_struct_->sum_bid_factor_) -
...
void BookOrderDiff::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void BookOrderDiff::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
