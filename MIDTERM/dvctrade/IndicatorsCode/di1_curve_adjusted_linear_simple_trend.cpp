/**
    \file IndicatorsCode/di1_curve_linear_adjusted_simple_trend.cpp
...
void DI1CurveAdjustedLinearSimpleTrend::CollectShortCodes(
...
DI1CurveAdjustedLinearSimpleTrend* DI1CurveAdjustedLinearSimpleTrend::GetUniqueInstance(
  ...
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "DI1CurveAdjustedLinearSimpleTrend needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 DI1CurveAdjustedLinearSimpleTrend DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
DI1CurveAdjustedLinearSimpleTrend* DI1CurveAdjustedLinearSimpleTrend::GetUniqueInstance(
  ...
  static std::map<std::string, DI1CurveAdjustedLinearSimpleTrend*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new DI1CurveAdjustedLinearSimpleTrend(
...
DI1CurveAdjustedLinearSimpleTrend::DI1CurveAdjustedLinearSimpleTrend(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void DI1CurveAdjustedLinearSimpleTrend::OnIndicatorUpdate(const unsigned int& indicator_index_,
...
void DI1CurveAdjustedLinearSimpleTrend::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DI1CurveAdjustedLinearSimpleTrend::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool DI1CurveAdjustedLinearSimpleTrend::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void DI1CurveAdjustedLinearSimpleTrend::InitializeValues() { indicator_value_ = 0; }
}
