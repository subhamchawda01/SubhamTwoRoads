/**
    \file IndicatorsCode/offline_computed_returns_cutoff_pairs_port_port.cpp
...
void OfflineComputedReturnsCutoffPairsPort::CollectShortCodes(
...
OfflineComputedReturnsCutoffPairsPort* OfflineComputedReturnsCutoffPairsPort::GetUniqueInstance(
...
OfflineComputedReturnsCutoffPairsPort* OfflineComputedReturnsCutoffPairsPort::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineComputedReturnsCutoffPairsPort*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new OfflineComputedReturnsCutoffPairsPort(
...
OfflineComputedReturnsCutoffPairsPort::OfflineComputedReturnsCutoffPairsPort(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::string& concise_indicator_description_,
    SecurityMarketView& t_dep_market_view_, std::string t_portfolio_descriptor_shortcode_, double t_fractional_seconds_,
...
void OfflineComputedReturnsCutoffPairsPort::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
  ...
  if (!is_ready_) {
    ...
    if (!data_interrupted_) {
      ...
      if (fabs(current_projected_trend_) > fabs(dep_price_trend_)) {
        indicator_value_ =
            (current_projected_trend_ - dep_price_trend_) * dep_market_view_.price_from_type(price_type_);
      } else {
        indicator_value_ = 0;
      }
...
void OfflineComputedReturnsCutoffPairsPort::WhyNotReady() {
  if (!is_ready_) {
...
void OfflineComputedReturnsCutoffPairsPort::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineComputedReturnsCutoffPairsPort::OnMarketDataResumed(const unsigned int _security_id_) {
  data_interrupted_ = false;
  InitializeValues();
}

void OfflineComputedReturnsCutoffPairsPort::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineComputedReturnsCutoffPairsPort::UpdateLRInfo() {
