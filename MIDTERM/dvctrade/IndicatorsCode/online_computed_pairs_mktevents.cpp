/**
    \file IndicatorsCode/online_computed_pairs_mktevents.cpp
...
void OnlineComputedPairsMktEvents::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OnlineComputedPairsMktEvents* OnlineComputedPairsMktEvents::GetUniqueInstance(DebugLogger& t_dbglogger_,
...
OnlineComputedPairsMktEvents* OnlineComputedPairsMktEvents::GetUniqueInstance(
  ...
  static std::map<std::string, OnlineComputedPairsMktEvents*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OnlineComputedPairsMktEvents(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                         _indep_market_view_, _num_events_halflife_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OnlineComputedPairsMktEvents::OnlineComputedPairsMktEvents(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OnlineComputedPairsMktEvents::OnMarketUpdate(const unsigned int _security_id_,
...
// SMV is not ready
  ...
  if (!is_ready_) {
    ...
    if (moving_avg_indep_indep_price_ > 1.00) {  // added to prevent nan
      ...
      // Main Difference from OnlineComputedPairMktEvents
...
void OnlineComputedPairsMktEvents::InitializeValues() {
...
void OnlineComputedPairsMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OnlineComputedPairsMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
