...
void MovingAvgTradeSize::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                           std::vector<std::string>& _ors_source_needed_vec_,
                                           const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

MovingAvgTradeSize* MovingAvgTradeSize::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                          const std::vector<const char*>& r_tokens_,
...
MovingAvgTradeSize* MovingAvgTradeSize::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, MovingAvgTradeSize*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new MovingAvgTradeSize(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                               _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

MovingAvgTradeSize::MovingAvgTradeSize(DebugLogger& t_dbglogger_, const Watch& r_watch_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      moving_avg_trade_size_(0),
      last_trade_size_recorded_(0),
...
void MovingAvgTradeSize::WhyNotReady() {
  if (!is_ready_) {
...
void MovingAvgTradeSize::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
  ...
  if (!is_ready_) {
    ...
    if (watch_.msecs_from_midnight() - last_new_page_msecs_ < page_width_msecs_) {
      moving_avg_trade_size_ += inv_decay_sum_ * (current_trade_size_ - last_trade_size_recorded_);
    } else {
      ...
      if (num_pages_to_add_ >= (int)decay_vector_.size()) {
        ...
        if (num_pages_to_add_ == 1) {
          moving_avg_trade_size_ = (current_trade_size_ * inv_decay_sum_) + (moving_avg_trade_size_ * decay_vector_[1]);
        } else {  // num_pages_to_add_ >= 2 < decay_vector_.size ( )
          moving_avg_trade_size_ =
              (current_trade_size_ * inv_decay_sum_) +
              (last_trade_size_recorded_ * inv_decay_sum_ * decay_vector_sums_[(num_pages_to_add_ - 1)]) +
              (moving_avg_trade_size_ * decay_vector_[num_pages_to_add_]);
        }
        last_new_page_msecs_ += (num_pages_to_add_ * page_width_msecs_);
      }
    }

    last_trade_size_recorded_ = current_trade_size_;
    indicator_value_ = moving_avg_trade_size_;
...
void MovingAvgTradeSize::InitializeValues() {
  moving_avg_trade_size_ = current_trade_size_;
  last_trade_size_recorded_ = current_trade_size_;
...
void MovingAvgTradeSize::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void MovingAvgTradeSize::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
