/**
    \file IndicatorsCode/mult_mid_order_price.cpp
...
void MultMidOrderPrice::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                          std::vector<std::string>& _ors_source_needed_vec_,
                                          const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

MultMidOrderPrice* MultMidOrderPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                        const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    if (r_tokens_.size() < 6) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "MultMidOrderPrice incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ ");
    } else {
      t_dbglogger_ << "MultMidOrderPrice incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                      "_indep_market_view_ _num_levels_ _decay_factor_ _price_type_ "
                   << "\n";
...
MultMidOrderPrice* MultMidOrderPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, MultMidOrderPrice*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new MultMidOrderPrice(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_levels_,
                              _decay_factor_, _price_type_, _stdev_duration_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

MultMidOrderPrice::MultMidOrderPrice(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (book_info_struct_ == NULL) {
    std::cerr << "Error getting book_info_struct for MultMidOrderPrice: " << _num_levels_ << " " << _decay_factor_
              << std::endl;
  }
...
void MultMidOrderPrice::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    if (indep_market_view_.is_ready() &&
      ...
      // std::cout<<indep_market_view_.is_ready()<<" "<<indep_market_view_.spread_increments()<<std::endl;
    }
...
#define MIN_SIGNIFICANT_SUM_ORDER 0.99
    if ((book_info_struct_->sum_bid_factor_order_ <= MIN_SIGNIFICANT_SUM_ORDER) ||
        (book_info_struct_->sum_ask_factor_order_ <= MIN_SIGNIFICANT_SUM_ORDER)) {
      indicator_value_ = 0;
    } else {
      double adjusted_price_ =
          (((book_info_struct_->sum_bid_factor_price_order_ / book_info_struct_->sum_bid_factor_order_) +
            (book_info_struct_->sum_ask_factor_price_order_ / book_info_struct_->sum_ask_factor_order_)) /
           2.0);
      indicator_value_ = adjusted_price_ - SecurityMarketView::GetPriceFromType(price_type_, cr_market_update_info_);
    }
#undef MIN_SIGNIFICANT_SUM_ORDER
...
void MultMidOrderPrice::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void MultMidOrderPrice::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
