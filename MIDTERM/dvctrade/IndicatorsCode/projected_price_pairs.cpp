/**
    \file IndicatorsCode/projected_price_pairs.cpp
...
void ProjectedPricePairs::CollectShortCodes(std::vector<std::string> &_shortcodes_affecting_this_indicator_,
...
ProjectedPricePairs *ProjectedPricePairs::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
                                                            const std::vector<const char *> &r_tokens_,
...
ProjectedPricePairs *ProjectedPricePairs::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
  ...
  static std::map<std::string, ProjectedPricePairs *> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
    ...
    concise_indicator_description_map_[concise_indicator_description_] =
        new ProjectedPricePairs(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                t_indep_market_view_vec_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ProjectedPricePairs::ProjectedPricePairs(DebugLogger &t_dbglogger_, const Watch &r_watch_,
...
void ProjectedPricePairs::OnStdevUpdate(const unsigned int _security_id_, const double &_new_stdev_value_) {
  int update_idx_ = security_id_to_idx_map_[_security_id_];
  volatility_factor_vec_[update_idx_] = _new_stdev_value_;
}

void ProjectedPricePairs::OnMarketUpdate(const unsigned int t_security_id_,
  ...
  */
  ...
  if (is_ready_ && !data_interrupted_) {
    ...
    if (update_idx_ == 0) {
      ...
      double t_vol_factor_ = volatility_factor_vec_[0] / volatility_factor_vec_[update_idx_];
      // proj_price = moving_dep_price_ + (curr_source_price_-moving_source_price_) * (scale)
      projected_bid_price_ = moving_avg_bid_price_vec_[0] +
                             (current_bid_price_ - moving_avg_bid_price_vec_[update_idx_]) * (t_vol_factor_);
      projected_ask_price_ = moving_avg_ask_price_vec_[0] +
                             (current_ask_price_ - moving_avg_ask_price_vec_[update_idx_]) * (t_vol_factor_);
    }
        ...
        (last_best_bid_idx_ >= 0))  // if its an indep update, then we can just check if it beats the current proj price
    {
      ...
      for (int i = 1; i < int(last_bid_price_vec_.size()); i++) {
        double t_projected_price_ = moving_avg_bid_price_vec_[0] +
                                    (last_bid_price_vec_[i] - moving_avg_bid_price_vec_[i]) *
                                        (volatility_factor_vec_[0] / volatility_factor_vec_[i]);
        ...
        (last_best_ask_idx_ >= 0))  // if its an indep update, then we can just check if it beats the current proj price
    {
      ...
      for (int i = 1; i < int(last_ask_price_vec_.size()); i++) {
        double t_projected_price_ = moving_avg_ask_price_vec_[0] +
                                    (last_ask_price_vec_[i] - moving_avg_ask_price_vec_[i]) *
                                        (volatility_factor_vec_[0] / volatility_factor_vec_[i]);
...
void ProjectedPricePairs::InitializeValues(int _update_idx_) {
  if (_update_idx_ == 0) {
...
void ProjectedPricePairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void ProjectedPricePairs::OnMarketDataResumed(const unsigned int _security_id_) {
  if (dep_market_view_.security_id() == _security_id_) {
...
bool ProjectedPricePairs::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }
}
