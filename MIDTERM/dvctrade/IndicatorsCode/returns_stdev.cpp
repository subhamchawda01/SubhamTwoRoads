/**
    \file IndicatorsCode/returns_stdev.cpp
...
void ReturnsStdev::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                     std::vector<std::string>& _ors_source_needed_vec_,
                                     const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

ReturnsStdev* ReturnsStdev::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
ReturnsStdev* ReturnsStdev::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                              ...
                                              double _stdev_fractional_seconds_, PriceType_t _price_type_) {
  ...
  static std::map<std::string, ReturnsStdev*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new ReturnsStdev(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                         _stdev_fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ReturnsStdev::ReturnsStdev(DebugLogger& t_dbglogger_, const Watch& r_watch_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      last_price_recorded_(0.0),
      current_indep_price_(0.0),
      moving_avg_price_(0.0),
      current_stdev_(0.0) {
  trend_history_msecs_ = std::max(20, (int)round(1000 * _stdev_fractional_seconds_));
...
void ReturnsStdev::OnStdevUpdate(const unsigned int _security_id_, const double& _new_stdev_value_) {
  current_stdev_ = _new_stdev_value_;
}

void ReturnsStdev::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
  ...
  if (!is_ready_) {
    ...
    indicator_value_ = current_stdev_ / moving_avg_price_;
    NotifyIndicatorListeners(indicator_value_);
  }
}

void ReturnsStdev::InitializeValues() {
...
void ReturnsStdev::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void ReturnsStdev::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
