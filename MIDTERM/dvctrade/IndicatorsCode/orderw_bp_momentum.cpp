/**
    \file IndicatorsCode/orderw_bp_momentum.cpp
...
void OrderWBPMomentum::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                         std::vector<std::string>& _ors_source_needed_vec_,
                                         const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

OrderWBPMomentum* OrderWBPMomentum::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                      const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() < 7) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "OrderWBPMomentum incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
                "_indep_market_view_ _num_levels_ _level_decay_factor_");
  }
  return GetUniqueInstance(t_dbglogger_, r_watch_,
                           *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(r_tokens_[3])),
                           std::max(0, atoi(r_tokens_[4])), atof(r_tokens_[5]), atof(r_tokens_[6]), _basepx_pxtype_);
}
...
OrderWBPMomentum* OrderWBPMomentum::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  t_temp_oss_ << VarName() << ' ' << _indep_market_view_.secname() << ' ' << _num_levels_ << ' ' << _level_decay_factor_
              << ' ' << _fractional_seconds_ << " " << PriceType_t_To_String(_basepx_pxtype_) << " "
              << PriceType_t_To_String(_basepx_pxtype_);

  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, OrderWBPMomentum*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OrderWBPMomentum(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_levels_,
                             _level_decay_factor_, _fractional_seconds_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OrderWBPMomentum::OrderWBPMomentum(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OrderWBPMomentum::OnPLNew(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
...
void OrderWBPMomentum::OnPLDelete(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
...
void OrderWBPMomentum::OnPLChange(const unsigned int t_security_id_, const MarketUpdateInfo& r_market_update_info_,
...
void OrderWBPMomentum::OnMarketUpdate(const unsigned int _security_id_,
...
void OrderWBPMomentum::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
  data_interrupted_ = true;
  indicator_value_ = 0;
  NotifyIndicatorListeners(indicator_value_);
}

void OrderWBPMomentum::OnMarketDataResumed(const unsigned int _security_id_) { data_interrupted_ = false; }
}
