...
void ORSSelfExec::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
ORSSelfExec* ORSSelfExec::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (r_tokens_.size() < 5) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "ORSSelfExec incorrect syntax. Should be INDICATOR _this_weight_ _indicator_string_ "
...
ORSSelfExec* ORSSelfExec::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, ORSSelfExec*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new ORSSelfExec(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _factor_reduced_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ORSSelfExec::ORSSelfExec(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void ORSSelfExec::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& cr_market_update_info_) {
  if (!is_ready_) {
...
void ORSSelfExec::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
...
void ORSSelfExec::OrderExecuted(const int t_server_assigned_client_id_, const int _client_assigned_order_sequence_,
...
void ORSSelfExec::OnMarketDataInterrupted(const unsigned int _security_id_, const int msecs_since_last_receive_) {
...
void ORSSelfExec::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
