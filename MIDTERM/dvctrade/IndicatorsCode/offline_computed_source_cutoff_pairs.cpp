/**
    \file IndicatorsCode/offline_computed_source_cutoff_pairs.cpp
...
void OfflineComputedSourceCutoffPairs::CollectShortCodes(
...
OfflineComputedSourceCutoffPairs* OfflineComputedSourceCutoffPairs::GetUniqueInstance(
  ...
  if (r_tokens_.size() < 7) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight OfflineComputedSourceCutoffPairs _dep_market_view_ _indep_market_view_ "
...
OfflineComputedSourceCutoffPairs* OfflineComputedSourceCutoffPairs::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineComputedSourceCutoffPairs*> concise_indicator_description_map_;
  ...
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
    ...
    concise_indicator_description_map_[concise_indicator_description_] = new OfflineComputedSourceCutoffPairs(
...
OfflineComputedSourceCutoffPairs::OfflineComputedSourceCutoffPairs(
  ...
  if (lrdb_absent_) {
    ...
    p_indep_trend_indicator_ = SimpleTrend::GetUniqueInstance(t_dbglogger_, r_watch_, t_indep_market_view_.shortcode(),
                                                              t_trend_fractional_seconds_, t_price_type_);
    if (p_indep_trend_indicator_ == NULL) {
      indicator_value_ = 0;
      is_ready_ = true;
      std::cerr << __FUNCTION__ << " Failed for InDep MktView " << t_indep_market_view_.shortcode() << " "
...
void OfflineComputedSourceCutoffPairs::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
  ...
  if (!is_ready_) {
    ...
    switch (t_indicator_index_) {
      ...
      case 3u: {
        indep_cutoff_price_trend_ = t_new_indicator_value_ * trend_duration_ratio_;
...
void OfflineComputedSourceCutoffPairs::WhyNotReady() {
  if (!is_ready_) {
...
void OfflineComputedSourceCutoffPairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineComputedSourceCutoffPairs::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void OfflineComputedSourceCutoffPairs::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineComputedSourceCutoffPairs::UpdateLRInfo() {
