/**
    \file IndicatorsCode/online_computed_pairs_port.cpp
...
void OnlineComputedPairsPort::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OnlineComputedPairsPort* OnlineComputedPairsPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                    const std::vector<const char*>& r_tokens_,
...
OnlineComputedPairsPort* OnlineComputedPairsPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, OnlineComputedPairsPort*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OnlineComputedPairsPort(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                    _portfolio_descriptor_shortcode_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OnlineComputedPairsPort::OnlineComputedPairsPort(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void OnlineComputedPairsPort::OnPortfolioPriceChange(double _new_price_) {
...
void OnlineComputedPairsPort::OnMarketUpdate(
...
void OnlineComputedPairsPort::UpdateComputedVariables() {
  ...
  if (moving_avg_indep_indep_price_ > 1.00) {  // added to prevent nan
    ...
    // Main Differnece from OnlineComputedPairPort
...
void OnlineComputedPairsPort::OnPortfolioPriceReset(double t_new_portfolio_price_, double t_old_portfolio_price_,
...
void OnlineComputedPairsPort::InitializeValues() {
...
void OnlineComputedPairsPort::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OnlineComputedPairsPort::OnMarketDataResumed(const unsigned int _security_id_) {
  InitializeValues();
  data_interrupted_ = false;
}
}
