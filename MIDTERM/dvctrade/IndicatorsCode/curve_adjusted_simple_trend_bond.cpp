...
void CurveAdjustedSimpleTrendBond::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
CurveAdjustedSimpleTrendBond* CurveAdjustedSimpleTrendBond::GetUniqueInstance(DebugLogger& t_dbglogger_,
                                                                              const Watch& r_watch_,
                                                                              const std::vector<const char*>& r_tokens_,
                                                                              PriceType_t _base_price_type_) {
  if (r_tokens_.size() < 8u) {
    ExitVerbose(kExitErrorCodeGeneral, "CurveAdjustedSimpleTrendBond needs 8 tokens");
    return NULL;
  }

  // INDICATOR 1.00 CurveAdjustedSimpleTrendBond DEP INDEP1 INDEP2 HALFLIFE PRICE_TYPE
...
CurveAdjustedSimpleTrendBond* CurveAdjustedSimpleTrendBond::GetUniqueInstance(
  ...
  static std::map<std::string, CurveAdjustedSimpleTrendBond*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new CurveAdjustedSimpleTrendBond(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
...
CurveAdjustedSimpleTrendBond::CurveAdjustedSimpleTrendBond(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  indep1_trendI_->add_indicator_listener(1, this, 1.0);
  indep2_trendI_->add_indicator_listener(2, this, 1.0);
...
void CurveAdjustedSimpleTrendBond::OnIndicatorUpdate(const unsigned int& indicator_index_, const double& new_value_) {
  if (!is_ready_) {
...
void CurveAdjustedSimpleTrendBond::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void CurveAdjustedSimpleTrendBond::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep1_market_view_.security_id() == _security_id_) {
    indep1_interrupted_ = false;
  } else if (indep2_market_view_.security_id() == _security_id_) {
    indep2_interrupted_ = false;
...
bool CurveAdjustedSimpleTrendBond::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }

void CurveAdjustedSimpleTrendBond::InitializeValues() {
  indicator_value_ = 0;
  double _alpha1_ = 0.0;
  double _alpha2_ = 0.0;
  ...
  if (weight_vec_.empty()) {
    weight_vec_.push_back(-1);
    weight_vec_.push_back(_alpha1_);
    weight_vec_.push_back(_alpha2_);
  } else {
    weight_vec_[0] = -1;
    weight_vec_[1] = _alpha1_;
    weight_vec_[2] = _alpha2_;
  }
}
}
