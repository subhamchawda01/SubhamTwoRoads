/**
    \file IndicatorsCode/projected_pricetype_pairs.cpp
...
void ProjectedPriceTypePairs::CollectShortCodes(std::vector<std::string> &_shortcodes_affecting_this_indicator_,
...
ProjectedPriceTypePairs *ProjectedPriceTypePairs::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
                                                                    const std::vector<const char *> &r_tokens_,
...
ProjectedPriceTypePairs *ProjectedPriceTypePairs::GetUniqueInstance(DebugLogger &t_dbglogger_, const Watch &r_watch_,
  ...
  static std::map<std::string, ProjectedPriceTypePairs *> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
    ...
    concise_indicator_description_map_[concise_indicator_description_] =
        new ProjectedPriceTypePairs(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                    t_indep_market_view_vec_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ProjectedPriceTypePairs::ProjectedPriceTypePairs(DebugLogger &t_dbglogger_, const Watch &r_watch_,
...
void ProjectedPriceTypePairs::OnStdevUpdate(const unsigned int _security_id_, const double &_new_stdev_value_) {
  int update_idx_ = security_id_to_idx_map_[_security_id_];
  volatility_factor_vec_[update_idx_] = _new_stdev_value_;
}

void ProjectedPriceTypePairs::OnMarketUpdate(const unsigned int t_security_id_,
  ...
  */
  ...
  if (is_ready_ && !data_interrupted_) {
    ...
    update_idx_ = 1;
    double projected_price_ = moving_avg_price_vec_[0] +
                              (last_price_vec_[update_idx_] - moving_avg_price_vec_[update_idx_]) *
                                  (volatility_factor_vec_[0] / volatility_factor_vec_[update_idx_]);
...
void ProjectedPriceTypePairs::InitializeValues(int _update_idx_) {
  if (_update_idx_ == 0) {
...
void ProjectedPriceTypePairs::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void ProjectedPriceTypePairs::OnMarketDataResumed(const unsigned int _security_id_) {
  if (dep_market_view_.security_id() == _security_id_) {
...
bool ProjectedPriceTypePairs::AreAllReady() { return VectorUtils::CheckAllForValue(is_ready_vec_, true); }
}
