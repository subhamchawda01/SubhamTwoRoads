/**
    \file IndicatorsCode/sizew_diff_tdsizeavg_tpx_basepx.cpp
...
void SizeWDiffTDSizeAvgTPxBasepx::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                                    std::vector<std::string>& _ors_source_needed_vec_,
                                                    const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

SizeWDiffTDSizeAvgTPxBasepx* SizeWDiffTDSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_,
  ...
  if (r_tokens_.size() < 6) {
    if (r_tokens_.size() < 5) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  "SizeWDiffTDSizeAvgTPxBasepx incorrect syntax should b INDICATOR _this_weight_ _indicator_string_ "
                  "_indep_market_view_ _fractional_seconds_ _price_type_");
    } else {
      t_dbglogger_ << "SizeWDiffTDSizeAvgTPxBasepx incorrect syntax should b INDICATOR _this_weight_ "
...
SizeWDiffTDSizeAvgTPxBasepx* SizeWDiffTDSizeAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_,
  ...
  static std::map<std::string, SizeWDiffTDSizeAvgTPxBasepx*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new SizeWDiffTDSizeAvgTPxBasepx(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                        _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SizeWDiffTDSizeAvgTPxBasepx::SizeWDiffTDSizeAvgTPxBasepx(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void SizeWDiffTDSizeAvgTPxBasepx::OnMarketUpdate(const unsigned int _security_id_,
                                                 const MarketUpdateInfo& cr_market_update_info_) {
#define MIN_SIGNIFICANT_SUM_SZ_TRADED 1.0
  ...
  if (time_decayed_trade_info_manager_.sumsz_ >= MIN_SIGNIFICANT_SUM_SZ_TRADED) {
    indicator_value_ = sqrt(time_decayed_trade_info_manager_.sumsz_) *
                       ((time_decayed_trade_info_manager_.sumpxsz_ / time_decayed_trade_info_manager_.sumsz_) -
...
void SizeWDiffTDSizeAvgTPxBasepx::OnTradePrint(const unsigned int _security_id_,
...
void SizeWDiffTDSizeAvgTPxBasepx::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void SizeWDiffTDSizeAvgTPxBasepx::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
