/**
    \file IndicatorsCode/offline_computed_volume_adjusted_pairs_mktevents.cpp
...
void OfflineComputedVolumeAdjustedPairsMktEvents::CollectShortCodes(
...
OfflineComputedVolumeAdjustedPairsMktEvents* OfflineComputedVolumeAdjustedPairsMktEvents::GetUniqueInstance(
...
OfflineComputedVolumeAdjustedPairsMktEvents* OfflineComputedVolumeAdjustedPairsMktEvents::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineComputedVolumeAdjustedPairsMktEvents*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OfflineComputedVolumeAdjustedPairsMktEvents(t_dbglogger_, r_watch_, concise_indicator_description_,
                                                        _dep_market_view_, _indep_market_view_, _num_events_halflife_,
                                                        _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OfflineComputedVolumeAdjustedPairsMktEvents::OfflineComputedVolumeAdjustedPairsMktEvents(
  ...
  if (lrdb_absent_) {
    ...
    RecentVolumeRatioCalculator* p_rvrc_ = RecentVolumeRatioCalculator::GetUniqueInstance(
        t_dbglogger_, r_watch_, _dep_market_view_, _indep_market_view_, 3600);
...
void OfflineComputedVolumeAdjustedPairsMktEvents::OnIndicatorUpdate(const unsigned int& _indicator_index_,
...
void OfflineComputedVolumeAdjustedPairsMktEvents::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineComputedVolumeAdjustedPairsMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineComputedVolumeAdjustedPairsMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void OfflineComputedVolumeAdjustedPairsMktEvents::UpdateLRInfo() {
