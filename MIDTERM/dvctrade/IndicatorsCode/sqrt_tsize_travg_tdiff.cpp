/**
    \file IndicatorsCode/sqrt_tsize_travg_tdiff.cpp
...
void SqrtTSizeTRAvgTDiff::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                            std::vector<std::string>& _ors_source_needed_vec_,
                                            const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

SqrtTSizeTRAvgTDiff* SqrtTSizeTRAvgTDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                            const std::vector<const char*>& r_tokens_,
...
SqrtTSizeTRAvgTDiff* SqrtTSizeTRAvgTDiff::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, SqrtTSizeTRAvgTDiff*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new SqrtTSizeTRAvgTDiff(
        t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_, _num_trades_halflife_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SqrtTSizeTRAvgTDiff::SqrtTSizeTRAvgTDiff(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  trade_decayed_trade_info_manager_.compute_sumtdiffsqrtsz();
  trade_decayed_trade_info_manager_.compute_sumsqrtsz();
  _indep_market_view_.subscribe_tradeprints(this);
}

void SqrtTSizeTRAvgTDiff::OnMarketUpdate(const unsigned int _security_id_,
...
#define MIN_SIGNIFICANT_SUM_SQRT_SZ_TRADED 0.10
  ...
  if (trade_decayed_trade_info_manager_.sumsqrtsz_ > MIN_SIGNIFICANT_SUM_SQRT_SZ_TRADED) {
    indicator_value_ = trade_decayed_trade_info_manager_.sumtdiffsqrtsz_ / trade_decayed_trade_info_manager_.sumsqrtsz_;
  } else {
    indicator_value_ = 0;
  }
...
void SqrtTSizeTRAvgTDiff::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
                                       const MarketUpdateInfo& _market_update_info_) {
#define MIN_SIGNIFICANT_SUM_SQRT_SZ_TRADED 0.10
...
void SqrtTSizeTRAvgTDiff::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void SqrtTSizeTRAvgTDiff::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
