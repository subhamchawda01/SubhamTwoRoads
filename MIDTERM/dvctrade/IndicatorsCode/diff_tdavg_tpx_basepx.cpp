/**
    \file IndicatorsCode/diff_tdavg_tpx_basepx.cpp
...
void DiffTDAvgTPxBasepx::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                           std::vector<std::string>& _ors_source_needed_vec_,
                                           const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

DiffTDAvgTPxBasepx* DiffTDAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                          const std::vector<const char*>& r_tokens_,
                                                          PriceType_t _basepx_pxtype_) {
  // INDICATOR _this_weight_ _indicator_string_ _indep_market_view_ _fractional_seconds_ price_type_
  PriceType_t t_price_type_ = kPriceTypeMax;
  if (r_tokens_.size() < 6) {
    if (r_tokens_.size() < 5) {
      ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                  " DiffTDAvgTPxBasepx Incorrect Syntax. Correct syntax would b  INDICATOR _this_weight_ "
                  "_indicator_string_ _indep_market_view_ _fractional_seconds_ price_type_");
    } else {
      t_dbglogger_ << "DiffTDAvgTPxBasepx Incorrect Syntax. Correct syntax would b  INDICATOR _this_weight_ "
                      "_indicator_string_ _indep_market_view_ _fractional_seconds_ price_type_"
                   << "\n";
...
DiffTDAvgTPxBasepx* DiffTDAvgTPxBasepx::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, DiffTDAvgTPxBasepx*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new DiffTDAvgTPxBasepx(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                               _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

DiffTDAvgTPxBasepx::DiffTDAvgTPxBasepx(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void DiffTDAvgTPxBasepx::OnMarketUpdate(const unsigned int _security_id_,
...
#define MIN_SIGNIFICANT_SUM_COEFFS 0.50
  ...
  if (time_decayed_trade_info_manager_.sumcoeffs_ > MIN_SIGNIFICANT_SUM_COEFFS) {
    if (time_decayed_trade_info_manager_.sumcoeffs_ <
        1) {  // if the trades are pretty old then make this indicator weaker
      indicator_value_ = (time_decayed_trade_info_manager_.sumpx_ -
                          (time_decayed_trade_info_manager_.sumcoeffs_ *
                           SecurityMarketView::GetPriceFromType(price_type_, cr_market_update_info_)));
    } else {
      indicator_value_ = (time_decayed_trade_info_manager_.sumpx_ / time_decayed_trade_info_manager_.sumcoeffs_) -
                         SecurityMarketView::GetPriceFromType(price_type_, cr_market_update_info_);
    }
  } else {
    indicator_value_ = 0;
...
void DiffTDAvgTPxBasepx::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
                                      const MarketUpdateInfo& cr_market_update_info_) {
  // need to compare against a low value since otherwise there would be weird values as the denominator recedes in value
  if (!data_interrupted_ && time_decayed_trade_info_manager_.sumcoeffs_ > MIN_SIGNIFICANT_SUM_COEFFS) {
    if (time_decayed_trade_info_manager_.sumcoeffs_ <
        1) {  // if the trades are pretty old then make this indicator weaker
      indicator_value_ = (time_decayed_trade_info_manager_.sumpx_ -
                          (time_decayed_trade_info_manager_.sumcoeffs_ *
...
void DiffTDAvgTPxBasepx::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void DiffTDAvgTPxBasepx::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
