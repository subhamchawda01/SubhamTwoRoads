/**
    \file IndicatorsCode/stdev_adjusted_returns_diff_norm.cpp
...
void StdevAdjustedReturnsDiffNorm::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
StdevAdjustedReturnsDiffNorm* StdevAdjustedReturnsDiffNorm::GetUniqueInstance(DebugLogger& t_dbglogger_,
  ...
  if (r_tokens_.size() < 8) {
    ExitVerbose(
        ...
        "INDICATOR weight StdevAdjustedReturnsDiffNorm _dep_market_view_ _indep_market_view_ _fractional_seconds_ "
...
StdevAdjustedReturnsDiffNorm* StdevAdjustedReturnsDiffNorm::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const SecurityMarketView& _dep_market_view_,
    const SecurityMarketView& _indep_market_view_, double _indicator_duration_, double _stdev_duration_,
    PriceType_t _price_type_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << _dep_market_view_.secname() << ' ' << _indep_market_view_.secname() << ' '
              << _indicator_duration_ << ' ' << _stdev_duration_ << ' ' << PriceType_t_To_String(_price_type_);
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, StdevAdjustedReturnsDiffNorm*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new StdevAdjustedReturnsDiffNorm(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                         _indep_market_view_, _indicator_duration_, _stdev_duration_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

StdevAdjustedReturnsDiffNorm::StdevAdjustedReturnsDiffNorm(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  dep_stdev_trend_calculator_ = SlowStdevReturnsCalculator::GetUniqueInstance(
      t_dbglogger_, r_watch_, dep_market_view_.shortcode(), t_stdev_duration_, _indicator_duration_, _price_type_);
  indep_stdev_trend_calculator_ = SlowStdevReturnsCalculator::GetUniqueInstance(
      t_dbglogger_, r_watch_, indep_market_view_.shortcode(), t_stdev_duration_, _indicator_duration_, _price_type_);
...
void StdevAdjustedReturnsDiffNorm::OnIndicatorUpdate(const unsigned int& _indicator_index_,
...
void StdevAdjustedReturnsDiffNorm::WhyNotReady() {
  if (!is_ready_) {
...
void StdevAdjustedReturnsDiffNorm::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    ...
    if (moving_avg_dep_ != 0 && moving_avg_indep_ != 0) {
      ...
      // normalizing the indicator_value by dep_stdev to make it normalized - similar to scaled trend for simple trend
      indicator_value_ = lrdb_sign_ * (returns_indep_ / stdev_indep_) - (returns_dep_ / stdev_dep_);
    } else {
      indicator_value_ = 0;
    }
...
void StdevAdjustedReturnsDiffNorm::InitializeValues() {
...
void StdevAdjustedReturnsDiffNorm::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void StdevAdjustedReturnsDiffNorm::OnMarketDataResumed(const unsigned int _security_id_) {
