...
void SpreadImpliedVolPrice::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                              std::vector<std::string>& _ors_source_needed_vec_,
                                              const std::vector<const char*>& r_tokens_) {
  std::string dep_shortcode = (std::string)r_tokens_[3];
  std::vector<std::string> all_shortcodes = SpreadImpliedVolPrice::GetShortCodes(dep_shortcode);
...
SpreadImpliedVolPrice* SpreadImpliedVolPrice::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                const std::vector<const char*>& r_tokens_,
  ...
  std::vector<std::string> all_shortcodes = SpreadImpliedVolPrice::GetShortCodes((std::string)r_tokens_[3]);
...
SpreadImpliedVolPrice* SpreadImpliedVolPrice::GetUniqueInstance(
    DebugLogger& t_dbglogger_, const Watch& r_watch_, const std::vector<SecurityMarketView*> _all_market_views_,
  ...
  static std::map<std::string, SpreadImpliedVolPrice*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new SpreadImpliedVolPrice(
        t_dbglogger_, r_watch_, concise_indicator_description_, _all_market_views_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SpreadImpliedVolPrice::SpreadImpliedVolPrice(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void SpreadImpliedVolPrice::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
  if (num_updated_ != all_market_views_.size()) {
    ...
    if (_indicator_index_ == all_market_views_.size() - 1) {
      current_sp_price_ = _new_value_;
    }

    if (num_updated_ == all_market_views_.size()) {
      last_price_vec_[all_market_views_.size() - 1] = _new_value_ + last_price_vec_[all_market_views_.size() - 2];
      curr_price_vec_[all_market_views_.size() - 1] = _new_value_ + curr_price_vec_[all_market_views_.size() - 2];

      curr_vx_spot_ = SpreadImpliedVolPrice::GetVXSpot(curr_price_vec_[0], curr_price_vec_[1]);
    ...
    if (_indicator_index_ == all_market_views_.size() - 1) {
      curr_price_vec_[_indicator_index_] = _new_value_ + curr_price_vec_[all_market_views_.size() - 2];
      current_sp_price_ = _new_value_;
    }
    ...
    temp_1 =
        temp_1 - ((std::pow(curr_vx_spot_, 0.5) * ttm_0_ / 252.0) + (std::pow(curr_price_vec_[0], 0.5) * 22 / 252));
    temp_1 = std::pow(temp_1 / (22.0 / 252.0), 0.5);
    indicator_value_ = temp_1 - current_sp_price_;
    NotifyIndicatorListeners(indicator_value_);
  }
}

void SpreadImpliedVolPrice::InitializeValues() { indicator_value_ = 0; }

// market_interrupt_listener interface

void SpreadImpliedVolPrice::OnMarketDataResumed(const unsigned int _security_id_) {
...
std::vector<std::string> SpreadImpliedVolPrice::GetShortCodes(std::string dep_shortcode) {
  ...
  all_shortcodes.push_back(dep_shortcode);

  return (all_shortcodes);
}

double SpreadImpliedVolPrice::GetVXSpot(double p_0, double p_1) {
