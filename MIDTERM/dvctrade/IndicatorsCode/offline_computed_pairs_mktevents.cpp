/**
    \file IndicatorsCode/offline_computed_pairs_mktevents.cpp
...
void OfflineComputedPairsMktEvents::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
OfflineComputedPairsMktEvents* OfflineComputedPairsMktEvents::GetUniqueInstance(
  ...
  if (r_tokens_.size() < 7) {
    ExitVerbose(kModelCreationIndicatorLineLessArgs, t_dbglogger_,
                "INDICATOR weight OfflineComputedPairsMktEvents _dep_market_view_ _indep_market_view_ "
                "_num_events_/_fractional_seconds_ <T> _price_type_ ");
    return nullptr;
  } else if (r_tokens_.size() == 7 || std::string(r_tokens_[7]).compare("#") == 0 ||
             std::string(r_tokens_[6]).compare("T") != 0) {
    ...
    double avg_l1_events_per_sec_ = HFSAT::SampleDataUtil::GetAvgForPeriod(
        std::string(r_tokens_[3]), r_watch_.YYYYMMDD(), 60, global_trading_start_mfm_, global_trading_end_mfm_,
        std::string("L1EVPerSec"));
...
OfflineComputedPairsMktEvents* OfflineComputedPairsMktEvents::GetUniqueInstance(
  ...
  static std::map<std::string, OfflineComputedPairsMktEvents*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OfflineComputedPairsMktEvents(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                          _indep_market_view_, _num_events_halflife_, _price_type_, _use_time_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

OfflineComputedPairsMktEvents::OfflineComputedPairsMktEvents(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  if (lrdb_absent_) {
    ...
    p_dep_indicator_ = SimpleTrendMktEvents::GetUniqueInstance(t_dbglogger_, r_watch_, t_dep_market_view_,
    ...
    if (_use_time_) {
      double dep_avg_l1_events_per_sec_ =
          HFSAT::SampleDataUtil::GetAvgForPeriod(t_dep_market_view_.shortcode(), watch_.YYYYMMDD(), 60,
                                                 trading_start_mfm_, trading_end_mfm_, std::string("L1EVPerSec"));
      double indep_avg_l1_events_per_sec_ =
          HFSAT::SampleDataUtil::GetAvgForPeriod(t_indep_market_view_.shortcode(), watch_.YYYYMMDD(), 60,
                                                 trading_start_mfm_, trading_end_mfm_, std::string("L1EVPerSec"));
...
void OfflineComputedPairsMktEvents::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
...
void OfflineComputedPairsMktEvents::WhyNotReady() {
  if (!is_ready_) {
...
void OfflineComputedPairsMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OfflineComputedPairsMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {
  if (_security_id_ == indep_market_view_.security_id()) {
    InitializeValues();
...
void OfflineComputedPairsMktEvents::InitializeValues() {
  indicator_value_ = 0;
  UpdateLRInfo();
}

void OfflineComputedPairsMktEvents::UpdateLRInfo() {
