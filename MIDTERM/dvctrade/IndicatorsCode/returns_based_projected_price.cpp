/**
    \file IndicatorsCode/return_based_projected_price.cpp
...
void ReturnsBasedProjectedPrice::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
ReturnsBasedProjectedPrice* ReturnsBasedProjectedPrice::GetUniqueInstance(DebugLogger& t_dbglogger_,
...
ReturnsBasedProjectedPrice* ReturnsBasedProjectedPrice::GetUniqueInstance(
  ...
  static std::map<std::string, ReturnsBasedProjectedPrice*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new ReturnsBasedProjectedPrice(t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_,
                                       _indep_market_view_, _fractional_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

ReturnsBasedProjectedPrice::ReturnsBasedProjectedPrice(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  HFSAT::IndicatorUtil::GetOfflineReturnsRatio(dep_market_view_.shortcode(), indep_market_view_.shortcode(),
                                               offline_returns_ratio_);
}

void ReturnsBasedProjectedPrice::WhyNotReady() {
  if (!is_ready_) {
...
void ReturnsBasedProjectedPrice::OnMarketUpdate(const unsigned int _security_id_,
  ...
  if (!is_ready_) {
    ...
    if (_security_id_ == dep_market_view_.security_id()) {
      if (watch_.msecs_from_midnight() - last_new_page_msecs_ < page_width_msecs_ &&
          _security_id_ == dep_market_view_.security_id()) {
        ...
        if (num_pages_to_add_ >= (int)decay_vector_.size()) {
          ...
          if (num_pages_to_add_ == 1) {
            moving_avg_dep_price_ = (current_dep_price_ * inv_decay_sum_) + (moving_avg_dep_price_ * decay_vector_[1]);
    ...
    if (_security_id_ == indep_market_view_.security_id()) {
      if (watch_.msecs_from_midnight() - last_new_page_msecs_ < page_width_msecs_ &&
          _security_id_ == indep_market_view_.security_id()) {
        ...
        if (num_pages_to_add_ >= (int)decay_vector_.size()) {
          ...
          if (num_pages_to_add_ == 1) {
            moving_avg_indep_price_ =
                (current_indep_price_ * inv_decay_sum_) + (moving_avg_indep_price_ * decay_vector_[1]);
          } else {  // num_pages_to_add_ >= 2 < decay_vector_.size ( )
            moving_avg_indep_price_ =
    ...
    double t_indep_return_ = (current_indep_price_ - moving_avg_indep_price_) / moving_avg_indep_price_;

    indicator_value_ = moving_avg_dep_price_ * (offline_returns_ratio_ * t_indep_return_ + 1);

    NotifyIndicatorListeners(indicator_value_);
  }
}

void ReturnsBasedProjectedPrice::InitializeValues() {
...
void ReturnsBasedProjectedPrice::OnMarketDataInterrupted(const unsigned int _security_id_,
                                                         const int msecs_since_last_receive_) {
  if (dep_market_view_.security_id() == _security_id_ || indep_market_view_.security_id() == _security_id_) {
...
void ReturnsBasedProjectedPrice::OnMarketDataResumed(const unsigned int _security_id_) {
  if (dep_market_view_.security_id() == _security_id_ || indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
