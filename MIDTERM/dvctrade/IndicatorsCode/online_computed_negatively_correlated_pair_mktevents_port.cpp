/**
    \file IndicatorsCode/online_computed_negatively_correlated_pair_mktevents_port.cpp
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::CollectShortCodes(
...
OnlineComputedNegativelyCorrelatedPairMktEventsPort*
OnlineComputedNegativelyCorrelatedPairMktEventsPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                       const std::vector<const char*>& r_tokens_,
...
OnlineComputedNegativelyCorrelatedPairMktEventsPort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, OnlineComputedNegativelyCorrelatedPairMktEventsPort*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new OnlineComputedNegativelyCorrelatedPairMktEventsPort(t_dbglogger_, r_watch_, concise_indicator_description_,
...
OnlineComputedNegativelyCorrelatedPairMktEventsPort::OnlineComputedNegativelyCorrelatedPairMktEventsPort(
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::OnPortfolioPriceChange(double t_new_portfolio_price_) {
  ...
  if (!is_ready_) {
    if (dep_market_view_.is_ready_complex(2) && indep_portfolio_price__->is_ready()) {
      is_ready_ = true;
      twice_initial_indep_price_ = 2 * t_new_portfolio_price_;
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::OnPortfolioPriceReset(double t_new_portfolio_price_,
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::OnMarketUpdate(
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::UpdateComputedVariables() {
  ...
  if (moving_avg_indep_indep_price_ > 1.00) {  // added to prevent nan
    ...
    // Main difference from OnlineComputedNegativelyCorrelatedPairsMktEventsPort
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::InitializeValues() {
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void OnlineComputedNegativelyCorrelatedPairMktEventsPort::OnMarketDataResumed(const unsigned int _security_id_) {
  InitializeValues();
  data_interrupted_ = false;
}
}
