/**
    \file IndicatorsCode/simple_trend_mktevents.cpp
...
void SimpleTrendMktEvents::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
                                             std::vector<std::string>& _ors_source_needed_vec_,
                                             const std::vector<const char*>& r_tokens_) {
  VectorUtils::UniqueVectorAdd(_shortcodes_affecting_this_indicator_, (std::string)r_tokens_[3]);
}

SimpleTrendMktEvents* SimpleTrendMktEvents::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                              const std::vector<const char*>& r_tokens_,
...
SimpleTrendMktEvents* SimpleTrendMktEvents::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
  ...
  static std::map<std::string, SimpleTrendMktEvents*> concise_indicator_description_map_;
  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] =
        new SimpleTrendMktEvents(t_dbglogger_, r_watch_, concise_indicator_description_, _indep_market_view_,
                                 _num_events_halflife_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

SimpleTrendMktEvents::SimpleTrendMktEvents(DebugLogger& t_dbglogger_, const Watch& r_watch_,
...
void SimpleTrendMktEvents::OnMarketUpdate(const unsigned int _security_id_,
...
void SimpleTrendMktEvents::InitializeValues() {
  indicator_value_ = 0;
  moving_avg_price_ = current_indep_price_;
}
// market_interrupt_listener interface
void SimpleTrendMktEvents::OnMarketDataInterrupted(const unsigned int _security_id_,
...
void SimpleTrendMktEvents::OnMarketDataResumed(const unsigned int _security_id_) {
  if (indep_market_view_.security_id() == _security_id_) {
    InitializeValues();
