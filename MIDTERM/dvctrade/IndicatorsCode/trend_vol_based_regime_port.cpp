...
void TrendVolBasedRegimePort::CollectShortCodes(std::vector<std::string>& _shortcodes_affecting_this_indicator_,
...
TrendVolBasedRegimePort* TrendVolBasedRegimePort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                    const std::vector<const char*>& r_tokens_,
  ...
  if (r_tokens_.size() >= 8) {
    return GetUniqueInstance(
        ...
        (std::string)r_tokens_[4], atof(r_tokens_[5]), atoi(r_tokens_[6]), StringToPriceType_t(r_tokens_[7]));
  } else {
    return NULL;
  }
}
...
TrendVolBasedRegimePort* TrendVolBasedRegimePort::GetUniqueInstance(DebugLogger& t_dbglogger_, const Watch& r_watch_,
                                                                    ...
                                                                    int t_volume_measure_seconds_,
                                                                    PriceType_t _price_type_) {
  std::ostringstream t_temp_oss_;
  t_temp_oss_ << VarName() << ' ' << _dep_market_view_.secname() << ' ' << t_portfolio_descriptor_shortcode_ << ' '
              << _fractional_seconds_ << ' ' << t_volume_measure_seconds_ << ' ' << PriceType_t_To_String(_price_type_);
  std::string concise_indicator_description_(t_temp_oss_.str());

  static std::map<std::string, TrendVolBasedRegimePort*> concise_indicator_description_map_;

  if (concise_indicator_description_map_.find(concise_indicator_description_) ==
      concise_indicator_description_map_.end()) {
    concise_indicator_description_map_[concise_indicator_description_] = new TrendVolBasedRegimePort(
        t_dbglogger_, r_watch_, concise_indicator_description_, _dep_market_view_, t_portfolio_descriptor_shortcode_,
        _fractional_seconds_, t_volume_measure_seconds_, _price_type_);
  }
  return concise_indicator_description_map_[concise_indicator_description_];
}

TrendVolBasedRegimePort::TrendVolBasedRegimePort(DebugLogger& t_dbglogger_, const Watch& r_watch_,
    ...
    : CommonIndicator(t_dbglogger_, r_watch_, concise_indicator_description_),
      ...
      indep_portfolio_price_(
          *(PCAPortPrice::GetUniqueInstance(t_dbglogger_, r_watch_, t_portfolio_descriptor_shortcode_, _price_type_))),
      ...
      indep_volume_ratio_calculator_(*(VolumeRatioCalculatorPort::GetUniqueInstance(
          t_dbglogger_, r_watch_, t_portfolio_descriptor_shortcode_, t_volume_measure_seconds_))),
      ...
      indep_mean_l1_norm_(LoadMeanL1NormPort(r_watch_.YYYYMMDD(), t_portfolio_descriptor_shortcode_, NUM_DAYS_HISTORY)),
  ...
  p_indep_indicator_brk_ = SimpleTrendPort::GetUniqueInstance(t_dbglogger_, r_watch_, t_portfolio_descriptor_shortcode_,
                                                              BREAKOUT_TREND_SECONDS, kPriceTypeMktSizeWPrice);
  ...
  p_dep_indicator_brk_->add_unweighted_indicator_listener(1u, this);
  p_indep_indicator_brk_->add_unweighted_indicator_listener(2u, this);

  dep_volume_ratio_calculator_.AddVolumeRatioListener(1, this);
  indep_volume_ratio_calculator_.AddVolumeRatioListener(2, this);
}

void TrendVolBasedRegimePort::OnTimePeriodUpdate(const int num_pages_to_add_) {
...
void TrendVolBasedRegimePort::OnVolumeRatioUpdate(const unsigned int r_security_id_,
...
void TrendVolBasedRegimePort::OnIndicatorUpdate(const unsigned int& t_indicator_index_,
...
void TrendVolBasedRegimePort::WhyNotReady() {
  if (!is_ready_) {
...
void TrendVolBasedRegimePort::InitializeValues() { indicator_value_ = 0; }
}
