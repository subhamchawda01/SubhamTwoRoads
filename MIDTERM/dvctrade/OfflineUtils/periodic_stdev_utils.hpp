...
inline float LoadMeanTrendStdev(int _YYYYMMDD, const std::string& symbol, int num_trading_days_) {
  ...
  if (total_days_found_ > 0) {
    return sum_stdev_ / total_days_found_;
  } else {
...
inline float LoadMeanStdevPort(const std::string& portfolio_descriptor_shortcode_, int _YYYYMMDD, int num_trading_days_,
  ...
  for (unsigned int ii = 0u; ii < shortcode_vec_.size(); ii++) {
    ...
    weighted_stdev_ += t_thisweight_ * SampleDataUtil::GetAvgForPeriod(shortcode_vec_[ii], _YYYYMMDD, num_trading_days_,
                                                                       trading_start_mfm, trading_end_mfm, "STDEV");
  }
  return weighted_stdev_;
}
}
