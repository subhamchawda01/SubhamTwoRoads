/**
   \file ExecLogicCode/price_based_trading.cpp
...
PriceBasedTrading::PriceBasedTrading(DebugLogger& _dbglogger_, const Watch& _watch_,
...
void PriceBasedTrading::TradingLogic() {
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
        ((target_price_ - best_nonself_bid_price_ >=
         ...
         (!dep_market_view_.trade_update_implied_quote())))  // Don't place new orders when the quote is stale
    {
      top_bid_place_ = true;
      top_bid_keep_ = true;
    } else {
  ...
  if (!placed_bids_this_round_) {  // only come here if no aggressive or improve bid orders sent this cycle
    if (top_bid_place_) {
                                                                                            ...
                                                                                            // if the spread is too wide
      {
        ...
        {
          DBGLOG_TIME << "SendTrade B of " << current_tradevarset_.l1bid_trade_size_ << " @ " << best_nonself_bid_price_
                      << " ebp_t: "
      ...
      if (!top_bid_keep_) {  // cancel all orders at best_nonself_bid_price_
        ...
        if (canceled_size_ > 0) {
          ...
          if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
          {
            DBGLOG_TIME << "Canceled B of " << canceled_size_ << " EqAbove " << best_nonself_bid_price_ << " ebp_t: "
  ...
  if (!placed_asks_this_round_) {
    if (top_ask_place_) {
                                                                                            ...
                                                                                            // if the spread is too wide
      {
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
        {
          DBGLOG_TIME << "SendTrade S of " << current_tradevarset_.l1ask_trade_size_ << " @ " << best_nonself_ask_price_
                      << " eap_t: "
      ...
      if (!top_ask_keep_) {
        ...
        if (canceled_size_ > 0) {
          ...
          if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
          {
            DBGLOG_TIME << "Canceled S of " << canceled_size_ << " EqAbove " << best_nonself_ask_price_ << " eap_t: "
    ...
    if (dep_market_view_.spread_increments() > 1 && !ask_improve_keep_) {
      ...
      if (cancelled_size_ > 0) {
        canceled_asks_this_round_ = true;
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          DBGLOG_TIME_CLASS_FUNC << " Cancelled Improve Ask above price: " << best_nonself_ask_int_price_ << " eap_t: "
                                 << (best_nonself_ask_price_ - target_price_ + bestask_queue_hysterisis_) /
                                        dep_market_view_.min_price_increment()
                                 << " thresh_t: "
...
void PriceBasedTrading::PrintFullStatus() {
