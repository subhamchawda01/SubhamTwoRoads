...
PriceBasedAggressiveTrading::PriceBasedAggressiveTrading(
...
void PriceBasedAggressiveTrading::TradingLogic() {
  ...
  if (param_set_.read_dat_weightage_fraction_) {
    dat_bid_reduce_l1bias_ =
        param_set_.dat_weightage_fraction_ * (target_price_ - targetbias_numbers_ - best_nonself_bid_price_);

    dat_ask_reduce_l1bias_ =
        param_set_.dat_weightage_fraction_ * (best_nonself_ask_price_ - target_price_ + targetbias_numbers_);
  }
  ...
  if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
    ...
    our_ask_orders_ = order_manager_.SumAskSizeUnconfirmedEqAboveIntPrice(best_nonself_ask_int_price_) +
                      order_manager_.SumAskSizeConfirmedEqAboveIntPrice(best_nonself_ask_int_price_);
    DBGLOG_TIME_CLASS_FUNC << " Shc: " << dep_market_view_.shortcode() << " BidOrders: " << our_bid_orders_
                           << " AskOrders: " << our_ask_orders_ << " Pos: " << my_position_ << " # "
                           << effective_bid_position_ << " # " << effective_ask_position_
                           << " Thresh: " << current_bid_tradevarset_.l1bid_place_ << " # "
                           << current_ask_tradevarset_.l1ask_place_ << " # " << current_bid_tradevarset_.l1bid_keep_
                           << " # " << current_ask_tradevarset_.l1ask_keep_ << " Tgt Px: " << target_price_
                           << " best bid: " << best_nonself_bid_price_ << " best ask: " << best_nonself_ask_price_
                           << DBGLOG_ENDL_FLUSH;
  }
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
        ...
        ((target_price_ - best_nonself_bid_price_ - dat_bid_reduce_l1bias_ >=
          current_bid_tradevarset_.l1bid_place_ - l1_bias_ -
              (param_set_.dat_weightage_fraction_ / 2) * dep_market_view_.spread_increments() *
                  dep_market_view_.min_price_increment() -
              trade_bias_buy_ + cancel_bias_buy_ - l1_order_bias_ - short_positioning_bias_ +
      ...
      if (watch_.msecs_from_midnight() - last_agg_buy_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ((target_price_ - best_nonself_ask_price_ >=
             ...
             (watch_.msecs_from_midnight() - last_bigtrades_bid_aggress_msecs_ <=
              param_set_
                  .bigtrades_aggress_cooloff_interval_)) &&  // only LIFT offer if the margin of buying here i.e. (
      ...
      if ((target_price_ - best_nonself_bid_price_ + bestbid_queue_hysterisis_ - dat_bid_reduce_l1bias_) >=
              current_bid_keep_tradevarset_.l1bid_keep_ - l1_bid_trade_bias_ -
                  (param_set_.dat_weightage_fraction_ / 2) * dep_market_view_.spread_increments() *
                      dep_market_view_.min_price_increment() -
                  trade_bias_buy_ + cancel_bias_buy_ - l1_bias_ - l1_order_bias_ - short_positioning_bias_ +
                  param_set_.spread_increase_ * (dep_market_view_.spread_increments() - 1) &&
          implied_mkt_bid_flag_) {
        top_bid_keep_ = true;
      } else {
    ...
    if (((dep_market_view_.spread_increments() > 1) &&
         (target_price_ - best_nonself_bid_price_ - dat_bid_reduce_l1bias_) >=
             current_tradevarset_.l1bid_improve_keep_ - trade_bias_buy_ + cancel_bias_buy_ - l1_bias_ - l1_order_bias_ -
                 (param_set_.dat_weightage_fraction_ / 2) * dep_market_view_.spread_increments() *
                     dep_market_view_.min_price_increment() -
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        ...
        (watch_.msecs_from_midnight() - last_bigtrades_ask_aggress_msecs_ <=
         param_set_.bigtrades_aggress_cooloff_interval_) ||
        ((best_nonself_ask_price_ - target_price_ - dat_ask_reduce_l1bias_ >=
          current_ask_tradevarset_.l1ask_place_ - l1_bias_ - l1_order_bias_ -
              (param_set_.dat_weightage_fraction_ / 2) * dep_market_view_.spread_increments() *
                  dep_market_view_.min_price_increment() -
              trade_bias_sell_ + cancel_bias_sell_ - long_positioning_bias_ +
              param_set_.spread_increase_ * (dep_market_view_.spread_increments() - 1)) &&
         (best_nonself_ask_size_ > param_set_.min_size_to_join_) && implied_mkt_ask_flag_)) {
      top_ask_place_ = true;
      top_ask_keep_ = true;
      ...
      if ((best_nonself_ask_price_ - target_price_ + bestask_queue_hysterisis_ - dat_ask_reduce_l1bias_) >=
              current_ask_keep_tradevarset_.l1ask_keep_ - l1_ask_trade_bias_ - trade_bias_sell_ + cancel_bias_sell_ -
                  l1_bias_ - l1_order_bias_ -
                  (param_set_.dat_weightage_fraction_ / 2) * dep_market_view_.spread_increments() *
                      dep_market_view_.min_price_increment() -
                  long_positioning_bias_ + param_set_.spread_increase_ * (dep_market_view_.spread_increments() - 1) &&
          implied_mkt_ask_flag_) {  // but with place in line effect enough to keep the live order there
        top_ask_keep_ = true;
      } else {
        top_ask_keep_ = false;
      }
    ...
    if ((dep_market_view_.spread_increments() > 1) &&
        ((best_nonself_ask_price_ - target_price_ - dat_ask_reduce_l1bias_) >=
         current_tradevarset_.l1ask_improve_keep_ - trade_bias_sell_ + cancel_bias_sell_ - l1_bias_ -
             (param_set_.dat_weightage_fraction_ / 2) * dep_market_view_.spread_increments() *
                 dep_market_view_.min_price_increment() -
             l1_order_bias_ - long_positioning_bias_ +
             param_set_.spread_increase_ * (dep_market_view_.spread_increments() - 1))) {
  ...
  if (top_ask_lift_) {
    // only place aggressive bid orders when there is no active unconfirmed order at or above
      ...
      if (allowance_for_aggressive_buy_ >= 0) {
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
        {
          DBGLOG_TIME_CLASS_FUNC << " Shc: " << dep_market_view_.shortcode() << "Sending aggressive B at px "
                                 << best_nonself_ask_price_ << " position " << my_position_ << DBGLOG_ENDL_FLUSH;
    ...
    if (top_bid_place_) {
      ...
      if (!top_bid_keep_) {  // cancel all orders at best_nonself_bid_price_
        ...
        if (canceled_size_ > 0) {
          canceled_bids_this_round_ = true;
          if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
            DBGLOG_TIME_CLASS_FUNC << " Shc: " << dep_market_view_.shortcode() << "Canceled B of " << canceled_size_
    ...
    if ((order_manager_.SumAskSizeUnconfirmedEqAboveIntPrice(best_nonself_ask_int_price_ - 1) == 0) &&
      ...
      if (allowance_for_aggressive_sell_ >= 0) {
        if (allowance_for_aggressive_sell_ > _canceled_size_) {
          ...
          _canceled_size_ += order_manager_.CancelAsksFromFar(aggress_size);
        }
      } else {
        // Place new order
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          DBGLOG_TIME_CLASS_FUNC << " Shc: " << dep_market_view_.shortcode() << "Sending aggressive S at px "
                                 << best_nonself_bid_price_ << " position " << my_position_ << DBGLOG_ENDL_FLUSH;
          p_base_model_math_->ShowIndicatorValues();
        }
      }
    } else {
  ...
  if ((!placed_asks_this_round_) && (top_ask_improve_)) {
    // only place Ask Improve orders when there is no active unconfirmed order and no confirmed orders above the
    // best
    // price
    if ((order_manager_.SumAskSizeUnconfirmedEqAboveIntPrice(best_nonself_ask_int_price_ - 1) == 0) &&
        (order_manager_.SumAskSizeConfirmedAboveIntPrice(best_nonself_ask_int_price_) == 0)) {
      ...
      if (-my_position_ + order_manager_.SumAskSizes() + improve_size + current_tradevarset_.l1ask_trade_size_ >
    ...
    if (top_ask_place_) {
      ...
      if (!top_ask_keep_) {
        ...
        if (canceled_size_ > 0) {
          canceled_asks_this_round_ = true;
          if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
            DBGLOG_TIME_CLASS_FUNC << " Shc: " << dep_market_view_.shortcode() << "Canceled S of " << canceled_size_
                                   << " EqAbove " << best_nonself_ask_price_ << " SizeToCancel "
                                   << (-our_ask_orders_ + my_position_) << " eap_t: "
                                   << (best_nonself_ask_price_ - target_price_ + bestask_queue_hysterisis_) /
                                          dep_market_view_.min_price_increment() << " thresh_t: "
...
void PriceBasedAggressiveTrading::PrintFullStatus() {
  DBGLOG_TIME << " Shc: " << dep_market_view_.shortcode() << "tgt: " << target_price_ << " mkt "
