...
void EquityTrading2::SetComputeReturns() {
  for (unsigned i = 0; i < product_vec_.size(); i++) {
    SimpleReturns *t_returns_indicator_ = SimpleReturns::GetUniqueInstance(
        dbglogger_, watch_, *dep_market_view_vec_[i], common_paramset_.stdev_duration_, kPriceTypeMktSizeWPrice);
    if (t_returns_indicator_ != NULL) {
      t_returns_indicator_->add_unweighted_indicator_listener(product_vec_.size() + i, this);
    }
  }

  {
    // Gettting historical returns stdev

    std::ifstream historical_returns_stdev_file_;
    std::string filename_ = "/home/spare/local/tradeinfo/PCAInfo/shortcode_returns_stdev_DEFAULT.txt";

    historical_returns_stdev_file_.open(filename_.c_str(), std::ifstream::in);

    if (historical_returns_stdev_file_.is_open()) {
      while (historical_returns_stdev_file_.good()) {
        // # #shortcode #2s #10s #30s #60s #120s #300s #600s #900s #1800s #3600s
        const int kL1AvgBufferLen = 1024;
        char readline_buffer_[kL1AvgBufferLen];
        bzero(readline_buffer_, kL1AvgBufferLen);
        historical_returns_stdev_file_.getline(readline_buffer_, kL1AvgBufferLen);
        ...
        for (unsigned i = 0; i < product_vec_.size(); i++) {
          if (products_being_traded_[i].compare(tokens_[1]) == 0) {
            product_vec_[i]->hist_retuns_stdev_ = (atof(tokens_[9]) + atof(tokens_[10])) / 2;
          }
        }
      }
    }
    historical_returns_stdev_file_.close();
  }
  SetSectorIndex();
}

void EquityTrading2::SetSectorIndex() {
  SimpleReturns *t_returns_indicator_ = SimpleReturns::GetUniqueInstance(
      dbglogger_, watch_, *index_market_view_, common_paramset_.stdev_duration_, kPriceTypeMktSizeWPrice);
  if (t_returns_indicator_ != NULL) {
    t_returns_indicator_->add_unweighted_indicator_listener(2 * product_vec_.size(), this);
  }

  std::vector<std::string> sector_vec_;
  for (unsigned i = 0; i < product_vec_.size(); i++) {
    std::string sectorport_ = GetSectorForBMFStock(products_being_traded_[i]);
    bool found_ = false;
    for (unsigned j = 0; j < sector_vec_.size(); j++) {
      if (sector_vec_[j].compare(sectorport_) == 0) {
        found_ = true;
        prod_index_to_sector_index_[i] = j;
      }
    }

    if (not found_) {
      SimpleReturnsPort *t_returns_indicator_ = SimpleReturnsPort::GetUniqueInstance(
          dbglogger_, watch_, sectorport_, common_paramset_.stdev_duration_, kPriceTypeMktSizeWPrice);
      if (t_returns_indicator_ != NULL) {
        t_returns_indicator_->add_unweighted_indicator_listener(2 * product_vec_.size() + sector_vec_.size() + 1, this);
      }
      prod_index_to_sector_index_[i] = sector_vec_.size();
      sector_vec_.push_back(sectorport_);
    }
    DBGLOG_TIME_CLASS_FUNC_LINE << " shc: " << products_being_traded_[i] << " port: " << sectorport_ << " idx_to_idx_ "
                                << prod_index_to_sector_index_[i] << DBGLOG_ENDL_FLUSH;
  }
  sector_ = sector_vec_;
  sector_returns_.resize(sector_vec_.size(), 0.0);
}

void EquityTrading2::DatTradingLogic(int _product_index_) {
  current_product_ = product_vec_[_product_index_];
  my_risk_ = (int)current_product_->beta_adjusted_position_;
  targetbias_numbers_ = targetbias_numbers_vec_[_product_index_];
  ...
  if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
    DBGLOG_TIME_CLASS_FUNC_LINE << " TargetBiasNumber: " << targetbias_numbers_
  ...
  if (current_global_tradevarset_.l1bid_trade_size_ > 0) {
    ...
    if ((current_product_->last_buy_msecs_ > 0) &&
      ...
      if ((best_nonself_bid_size_ > t_paramset_->safe_distance_) ||
          ((targetbias_numbers_ + ((p_dep_market_view_->spread_increments() > current_product_->moving_avg_spread_)
        ...
        if (watch_.msecs_from_midnight() - last_agg_buy_msecs_ > t_paramset_->agg_cooloff_interval_) {
          ...
          if ((t_paramset_->allowed_to_aggress_) &&
              ...
              (targetbias_numbers_ >= current_global_tradevarset_.l1bid_aggressive_) &&
...

          {
            ...
            if ((t_paramset_->allowed_to_improve_) && (my_risk_ <= t_paramset_->max_position_to_bidimprove_) &&
                ...
                (targetbias_numbers_ >= current_global_tradevarset_.l1bid_improve_) &&
                ((current_product_->last_buy_msecs_ <= 0) ||
                 (watch_.msecs_from_midnight() - current_product_->last_buy_msecs_ >= t_paramset_->cooloff_interval_))
                // TODO_OPT : later change setting of current_product_->last_buy_int_price_ to include px_band_
                ) {
        ...
        if ((targetbias_numbers_ + ((p_dep_market_view_->spread_increments() > current_product_->moving_avg_spread_)
                                        ? (t_paramset_->high_spread_allowance_)
                                        : 0.0) +
    ...
    if ((best_nonself_ask_size_ > t_paramset_->safe_distance_) ||
      ...
      if (watch_.msecs_from_midnight() - current_product_->last_agg_sell_msecs_ > t_paramset_->agg_cooloff_interval_) {
        ...
        if ((t_paramset_->allowed_to_aggress_) && /* external control on aggressing */
            ...
            (-targetbias_numbers_ >= current_global_tradevarset_.l1ask_aggressive_) &&
            ((current_product_->last_sell_msecs_ <= 0) ||
             (watch_.msecs_from_midnight() - current_product_->last_sell_msecs_ >= t_paramset_->cooloff_interval_))
...

        {
          ...
          if ((t_paramset_->allowed_to_improve_) && (my_risk_ >= t_paramset_->min_position_to_askimprove_) &&
              ...
              (targetbias_numbers_ >= current_global_tradevarset_.l1ask_improve_) &&
              ((current_product_->last_sell_msecs_ <= 0) ||
               (watch_.msecs_from_midnight() - current_product_->last_sell_msecs_ >= t_paramset_->cooloff_interval_))
              // TODO_OPT : later change setting of, current_product_->last_sell_int_price_ to include px_band_
              ) {
      ...
      if (cancelled_size_ > 0) {
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          DBGLOG_TIME_CLASS_FUNC << " Cancelled Improve Bid orders Above: " << best_nonself_bid_int_price_
                                 << " Position: " << my_risk_
                                 << " tgt_bias: " << targetbias_numbers_ / p_dep_market_view_->min_price_increment()
                                 << " thresh_t: "
...
void EquityTrading2::PbatTradingLogic(int _product_index_) {
  ...
  my_risk_ = (int)current_product_->beta_adjusted_position_;
  targetbias_numbers_ = targetbias_numbers_vec_[_product_index_];
  target_price_ = target_price_vec_[_product_index_];
      ...
      if (watch_.msecs_from_midnight() - current_product_->last_agg_buy_msecs_ > t_paramset_->agg_cooloff_interval_) {
        ...
        if ((t_paramset_->allowed_to_aggress_) &&
          ...
          if ((t_paramset_->allowed_to_improve_) && (my_risk_ <= t_paramset_->max_position_to_bidimprove_) &&
              // Don't improve bid when my_position_ is already decently long
              (p_dep_market_view_->spread_increments() >= t_paramset_->min_int_spread_to_improve_) &&
              (target_price_ - best_nonself_bid_price_ - p_dep_market_view_->min_price_increment() >=
      ...
      if (watch_.msecs_from_midnight() - current_product_->last_agg_sell_msecs_ > t_paramset_->agg_cooloff_interval_) {
...

        {
          ...
          if ((t_paramset_->allowed_to_improve_) &&
              ...
              (best_nonself_ask_int_price_ - best_nonself_bid_int_price_ >= t_paramset_->min_int_spread_to_improve_) &&
              (best_nonself_ask_price_ - target_price_ - p_dep_market_view_->min_price_increment() >=
...
void EquityTrading2::CallPlaceCancelNonBestLevels(int _product_index_) { PlaceCancelNonBestLevels(_product_index_); }

void EquityTrading2::PlaceCancelNonBestLevels(int _product_index_) {
...
void EquityTrading2::SetComputeTresholds() {
  l1_size_indicator_vec_.resize(product_vec_.size(), NULL);
  l1_order_indicator_vec_.resize(product_vec_.size(), NULL);
  l1_bias_vec_.resize(product_vec_.size(), 0);
  l1_order_bias_vec_.resize(product_vec_.size(), 0);
  short_positioning_bias_vec_.resize(product_vec_.size(), 0);
  long_positioning_bias_vec_.resize(product_vec_.size(), 0);

  for (unsigned i = 0; i < product_vec_.size(); i++) {
    current_product_ = product_vec_[i];
    ParamSet *t_paramset_ = prod_paramset_vec_[i];
    SecurityMarketView *p_dep_market_view_ = dep_market_view_vec_[i];
    ...
    if (t_paramset_->read_max_min_diff_ || t_paramset_->read_high_uts_factor_) {
      ...
      l1_size_indicator_vec_[i] = L1SizeTrend::GetUniqueInstance(
          dbglogger_, watch_,
    ...
    if (t_paramset_->read_max_min_diff_) {
      HFSAT::ExecLogicUtils::GetL1SizeBound(p_dep_market_view_->shortcode(), current_product_->l1_size_lower_bound_,
                                            current_product_->l1_size_upper_bound_, watch_,
                                            current_product_->trading_start_utc_mfm_,
                                            current_product_->trading_end_utc_mfm_, dbglogger_);
      if (current_product_->l1_size_upper_bound_ - current_product_->l1_size_lower_bound_ >= 1.0) {
        current_product_->l1_norm_factor_ =
            1.0 / (current_product_->l1_size_upper_bound_ - current_product_->l1_size_lower_bound_);
      } else {
        current_product_->l1_norm_factor_ = 0;
      }
    ...
    if (t_paramset_->read_max_min_diff_order_) {
      ...
      HFSAT::ExecLogicUtils::GetL1OrderBound(p_dep_market_view_->shortcode(), current_product_->l1_order_lower_bound_,
                                             current_product_->l1_order_upper_bound_, watch_,
                                             current_product_->trading_start_utc_mfm_,
                                             current_product_->trading_end_utc_mfm_, dbglogger_);
      if (current_product_->l1_order_upper_bound_ - current_product_->l1_order_lower_bound_ >= 1.0) {
        current_product_->l1_order_norm_factor_ =
            1.0 / (current_product_->l1_order_upper_bound_ - current_product_->l1_order_lower_bound_);
      } else {
        current_product_->l1_order_norm_factor_ = 0.0;
      }

      l1_order_indicator_vec_[i] = L1OrderTrend::GetUniqueInstance(
          dbglogger_, watch_,
          *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(p_dep_market_view_->shortcode())),
          t_fractional_seconds_, kPriceTypeMktSizeWPrice);
    ...
    if (t_paramset_->read_scale_max_pos_) {
      ...
      double t_trading_volume_expected_ = HFSAT::ExecLogicUtils::GetMURVolBound(
          ...
          current_product_->trading_start_utc_mfm_, current_product_->trading_end_utc_mfm_);
...
void EquityTrading2::ProcessTimePeriodUpdate(int num_pages_to_add_) {
  if (!initialized_) {
    Initialize();
  }
  if (order_placing_logic_ != kDirectionalAggressiveTrading && order_placing_logic_ != kPriceBasedAggressiveTrading) {
