/**
   \file ExecLogicCode/price_based_spread_trading.cpp
...
PriceBasedSpreadTrading::PriceBasedSpreadTrading(
    ...
    : BaseTrading(_dbglogger_, _watch_, _dep_market_view_, _order_manager_, _paramfilename_, _livetrading_,
      ...
      spread_market_view_(_spread_trading_manager_.GetSpreadMarketView()) {
  spread_trading_manager_.AddStmListener(this);
}

void PriceBasedSpreadTrading::TradingLogic() {
  ...
  // best_nonself_bid_size_ = spread_trading_manager_.bestbid_size();
  // best_nonself_ask_size_ = spread_trading_manager_.bestask_size();

  double t_spread_target_price_ =
      target_price_;  // PriceBasedSpreadTrading have target price of spread instead of smv_ target price
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    double bidside_bias_ =
        (t_spread_target_price_ - spread_market_view_.price_from_type(kPriceTypeSpreadAggLegBidPrice));
    ...
    if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
        ((bidside_bias_ >= current_tradevarset_.l1bid_place_ - l1_bias_ - l1_order_bias_ - short_positioning_bias_ +
      ...
      if (watch_.msecs_from_midnight() - last_agg_buy_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ((t_spread_target_price_ - spread_market_view_.bestask_price()) >=
          ...
          if ((param_set_.allowed_to_improve_) &&
              ...
              (bidside_bias_ >= current_tradevarset_.l1bid_improve_) &&
              ((last_buy_msecs_ <= 0) ||
               (watch_.msecs_from_midnight() - last_buy_msecs_ >= param_set_.cooloff_interval_) ||
      ...
      if ((bidside_bias_ + bestbid_queue_hysterisis_) >=
    ...
    if (((dep_market_view_.spread_increments() > 1) &&
         (bidside_bias_) >= current_tradevarset_.l1bid_improve_keep_ - l1_bias_ - l1_order_bias_ -
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    double askside_bias_ =
        (spread_market_view_.price_from_type(kPriceTypeSpreadAggLegAskPrice) - t_spread_target_price_);
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        ((askside_bias_ >= current_tradevarset_.l1ask_place_ - l1_bias_ - l1_order_bias_ - long_positioning_bias_ +
                               param_set_.spread_increase_ * (dep_market_view_.spread_increments() - 1)) &&
      ...
      if (watch_.msecs_from_midnight() - last_agg_sell_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ...
            ((spread_market_view_.bestbid_price() - t_spread_target_price_) >=
...

        {
          ...
          if ((param_set_.allowed_to_improve_) &&
              ...
              (dep_market_view_.spread_increments() >= param_set_.min_int_spread_to_improve_) &&
              (askside_bias_ >= current_tradevarset_.l1ask_improve_) &&
              ((last_sell_msecs_ <= 0) ||
               (watch_.msecs_from_midnight() - last_sell_msecs_ >= param_set_.cooloff_interval_) ||
      ...
      if ((askside_bias_ + bestask_queue_hysterisis_) >=
    ...
    if ((dep_market_view_.spread_increments() > 1) &&
        ((askside_bias_) >= current_tradevarset_.l1ask_improve_keep_ - l1_bias_ - l1_order_bias_ -
...
void PriceBasedSpreadTrading::OnPositionChange(int t_new_position_, int position_diff_,
                                               const unsigned int _security_id_) {
  BaseTrading::OnPositionChange(t_new_position_, position_diff_, _security_id_);
  spread_trading_manager_.OnPositionUpdate(_security_id_, my_position_);
}
...
bool PriceBasedSpreadTrading::UpdateTarget(double _target_price_, double _targetbias_numbers_) {
  if (!is_ready_) {  // checking whether we are post start time
    // if dependant is ready
    // and if so, then if the signal is between bid and offer
    if ((watch_.msecs_from_midnight() > trading_start_utc_mfm_) && (spread_market_view_.is_ready()) &&
        ((_target_price_ >= spread_market_view_.bestbid_price()) &&
         (_target_price_ <= spread_market_view_.bestask_price()))) {
      is_ready_ = true;
      DBGLOG_TIME_CLASS_FUNC << "SACI: " << order_manager_.server_assigned_client_id_ << " got ready!"
                             << DBGLOG_ENDL_FLUSH;
...
void PriceBasedSpreadTrading::PrintFullStatus() {
