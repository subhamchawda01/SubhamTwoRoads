/**
    \file ExecLogicCode/implied_price_pair_aggressive_trading.cpp
...
void ImpliedPricePairAggressiveTrading::CollectORSShortCodes(DebugLogger& _dbglogger_,
...
ImpliedPricePairAggressiveTrading::ImpliedPricePairAggressiveTrading(
...
void ImpliedPricePairAggressiveTrading::TradingLogic() {
  for (unsigned int i = 0; i < indep_mkt_view_vec_.size(); i++) {
    if (!indep_mkt_view_vec_[i]->is_ready()) {
      return;
    }
  }
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    // Based on nkm updates, always place if best bid offers a better fill than nkm ( ose )
    if ((((target_price_ - best_nonself_bid_price_ >= current_tradevarset_.l1bid_place_ - l1_bias_ - l1_order_bias_) ||
          ...
          (param_set_.sgx_market_making_ && my_position_ - param_set_.unit_trade_size_ < -param_set_.max_position_)) &&
      ...
      if (watch_.msecs_from_midnight() - last_agg_buy_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ...
            // only LIFT offer if the margin of buying here i.e. ( target_price_ -
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      // don't place any best ask in sgx_nk if best ask is lower than that in nkm
      (!param_set_.implied_price_based_cancellation_ ||
    ...
    // Based on nkm updates, always place if best ask offers a better fill than nkm ( ose )
    if ((((best_nonself_ask_price_ - target_price_ >= current_tradevarset_.l1ask_place_ - l1_bias_ - l1_order_bias_) ||
          (param_set_.implied_price_based_place_ &&
      ...
      if (((best_nonself_ask_price_ - target_price_ + bestask_queue_hysterisis_) >=
           current_tradevarset_.l1ask_keep_ - l1_bias_ - l1_order_bias_) ||
          (param_set_.implied_mkt_price_based_place_ &&
            ...
            implied_ask_price_ +
                (param_set_.implied_place_threshold_ -
                 param_set_.place_keep_diff_ * dep_market_view_.min_price_increment())))) {  // but with place in line
                                                                                             // effect enough to keep
                                                                                             // the live order there
        top_ask_keep_ = true;
      } else {
        top_ask_keep_ = false;
      }
  ...
  // TODO Add support aggressive orders in case prices are very off
...
void ImpliedPricePairAggressiveTrading::PrintFullStatus() {
