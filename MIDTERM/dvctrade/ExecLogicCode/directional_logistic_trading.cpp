/**
    \file ExecLogicCode/directional_logistic_trading.cpp
...
DirectionalLogisticTrading::DirectionalLogisticTrading(
...
void DirectionalLogisticTrading::TradingLogic() {
  int t_my_position_ = my_position_;
  int max_pos_possible_ = std::max(param_set_.max_position_, param_set_.worst_case_position_);
  if (t_my_position_ < 0) {
    t_my_position_ = std::max(t_my_position_, -1 * max_pos_possible_);
  } else {
    t_my_position_ = std::min(t_my_position_, max_pos_possible_);
  }
  t_my_position_ = t_my_position_ + max_pos_possible_;
  if (t_my_position_ < 0) {
    std::cerr << "-ve index for logistic thresh_maps : " << t_my_position_ << std::endl;
    exit(1);
  }
  // std::cout<<"my_position_ : " << my_position_ << std::endl;
  // std::cout<<"bid_place_ : " << pos_to_thresh_bid_place_[my_position_] << std::endl;
  // std::cout<<"ask_place_ : " << pos_to_thresh_ask_place_[my_position_] << std::endl;
  ...
  if (current_tradevarset_.l1bid_trade_size_ > 0) {
    ...
    if ((last_buy_msecs_ > 0) && (watch_.msecs_from_midnight() - last_buy_msecs_ < param_set_.cooloff_interval_) &&
      ...
      if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
          ((prob_decrease_ < prob_increase_ && prob_increase_ > pos_to_thresh_bid_place_[t_my_position_]) &&
        ...
        if (watch_.msecs_from_midnight() - last_agg_buy_msecs_ > param_set_.agg_cooloff_interval_) {
          ...
          {
            // TODO figure out a way for aggressive trades
            // top_ask_lift_ = true ;
        ...
        if (prob_decrease_ < prob_increase_ && prob_increase_ > pos_to_thresh_bid_keep_[t_my_position_]) {
          top_bid_keep_ = true;
        } else {
          top_bid_keep_ = false;
        }
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        ((prob_decrease_ > prob_increase_ && prob_decrease_ > pos_to_thresh_ask_place_[t_my_position_]) &&
      ...
      if (prob_decrease_ > prob_increase_ &&
          prob_decrease_ > pos_to_thresh_ask_keep_[t_my_position_]) {  // but with place in line effect enough to keep
...
void DirectionalLogisticTrading::PrintFullStatus() {
