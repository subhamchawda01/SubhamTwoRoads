/**
    \file ExecLogicCode/sgx_nk_price_pair_aggressive_trading.cpp
...
void SGXNKPricePairAggressiveTrading::CollectORSShortCodes(DebugLogger& _dbglogger_,
...
SGXNKPricePairAggressiveTrading::SGXNKPricePairAggressiveTrading(
	...
	nkm_market_view_.subscribe_price_type(this, HFSAT::kPriceTypeMktSizeWPrice);
	nkm_market_view_.subscribe_tradeprints(this);
}

void SGXNKPricePairAggressiveTrading::TradingLogic() {
  if (!nkm_market_view_.is_ready()) {
    return;
  }
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      ( best_nonself_bid_price_ <= nkm_market_view_.market_update_info_.bestbid_price_ ) &&
      ...
      if (watch_.msecs_from_midnight() - last_agg_buy_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ((target_price_ - best_nonself_ask_price_ >= current_tradevarset_.l1bid_aggressive_ ) ||
            		( best_nonself_ask_price_ < nkm_market_view_.market_update_info_.bestbid_price_ ) ) &&
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      ( best_nonself_ask_price_ >= nkm_market_view_.market_update_info_.bestask_price_ ) &&
      // first check for cooloff_interval
      ...
      if (watch_.msecs_from_midnight() - last_agg_sell_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ...
            ((best_nonself_bid_price_ - target_price_ >= current_tradevarset_.l1ask_aggressive_ ) ||
            		( best_nonself_bid_price_ > nkm_market_view_.market_update_info_.bestask_price_ ) ) &&
            ((last_sell_msecs_ <= 0) ||
...
void SGXNKPricePairAggressiveTrading::PrintFullStatus() {
