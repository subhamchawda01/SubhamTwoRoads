/**
    \file ExecLogicCode/price_based_security_aggressive_trading.cpp
...
void PriceBasedSecurityAggressiveTrading::CollectORSShortCodes(DebugLogger& _dbglogger_,
                                                               const std::string& r_dep_shortcode_,
                                                               std::vector<std::string>& source_shortcode_vec_,
                                                               std::vector<std::string>& ors_source_needed_vec_) {
  HFSAT::RiskManager::CollectORSShortCodes(_dbglogger_, r_dep_shortcode_, ors_source_needed_vec_);
}

PriceBasedSecurityAggressiveTrading::PriceBasedSecurityAggressiveTrading(
...
void PriceBasedSecurityAggressiveTrading::TradingLogic() {
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
        ((target_price_ - best_nonself_bid_price_ >= current_bid_tradevarset_.l1bid_place_ - l1_bias_) &&
      ...
      if ((target_price_ - best_nonself_bid_price_ + bestbid_queue_hysterisis_) >=
          current_bid_keep_tradevarset_.l1bid_keep_ - l1_bias_ - l1_bid_trade_bias_) {
        top_bid_keep_ = true;
      } else {
        top_bid_keep_ = false;
      }
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        ((best_nonself_ask_price_ - target_price_ >= current_ask_tradevarset_.l1ask_place_ - l1_bias_) &&
      ...
      if ((best_nonself_ask_price_ - target_price_ + bestask_queue_hysterisis_) >=
          current_ask_keep_tradevarset_.l1ask_keep_ - l1_bias_ -
              l1_ask_trade_bias_) {  // but with place in line effect enough to keep the live order there
        top_ask_keep_ = true;
      } else {
        top_ask_keep_ = false;
      }
         ...
         param_set_.worst_case_position_ )
                    {
                      ...
                      if ( -my_position_ + order_manager_.SumAskSizes ( ) + current_tradevarset_.l1bid_trade_size_ -
         _canceled_size_ > param_set_.worst_case_position_ )
                        {
...
void PriceBasedSecurityAggressiveTrading::PrintFullStatus() {
  DBGLOG_TIME << "tgt: " << target_price_ << " mkt " << best_nonself_bid_price_ << " X " << best_nonself_bid_size_
              ...
              << " minimal_risk: " << current_risk_mapped_to_product_position_ << DBGLOG_ENDL_FLUSH;
}
}
