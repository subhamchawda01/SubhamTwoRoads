/**
   \file ExecLogicCode/implied_directional_agrressive_trading.cpp
...
void ImpliedDirectionalAggressiveTrading::CollectORSShortCodes(DebugLogger& _dbglogger_,
  ...
  HFSAT::VectorUtils::UniqueVectorAdd(
      source_shortcode_vec_, HFSAT::ImpliedPriceCalculator::GetIndepShortcodesForImpliedPrice(r_dep_shortcode_));
}

ImpliedDirectionalAggressiveTrading::ImpliedDirectionalAggressiveTrading(
    DebugLogger& _dbglogger_, const Watch& _watch_, const SecurityMarketView& _dep_market_view_,
    const std::vector<SecurityMarketView*> _indep_mkt_view_vec_, SmartOrderManager& _order_manager_,
    ...
    : BaseTrading(_dbglogger_, _watch_, _dep_market_view_, _order_manager_, _paramfilename_, _livetrading_,
      ...
      indep_mkt_view_vec_(_indep_mkt_view_vec_),
      implied_bid_price_(-1000.0),
      implied_ask_price_(-1000.0),
      implied_bid_size_(-1000),
      implied_ask_size_(-1000),
      implied_mkt_price_(-1000.0) {
  implied_price_calculator_ =
      new ImpliedPriceCalculator(_dbglogger_, _watch_, _dep_market_view_, param_set_.implied_price_calculation_);

  for (unsigned int i = 0; i < indep_mkt_view_vec_.size(); i++) {
    indep_mkt_view_vec_[i]->subscribe_price_type(this, HFSAT::kPriceTypeMktSizeWPrice);
    indep_mkt_view_vec_[i]->subscribe_tradeprints(this);
  }

  param_set_.implied_cancel_threshold_ *= dep_market_view_.min_price_increment();
  param_set_.implied_place_threshold_ *= dep_market_view_.min_price_increment();
  param_set_.implied_agg_threshold_ *= dep_market_view_.min_price_increment();
  param_set_.implied_improve_threshold_ *= dep_market_view_.min_price_increment();
  param_set_.implied_mkt_cancel_threshold_ *= dep_market_view_.min_price_increment();
  param_set_.implied_mkt_place_threshold_ *= dep_market_view_.min_price_increment();
}

void ImpliedDirectionalAggressiveTrading::TradingLogic() {
  ...
  implied_price_calculator_->GetImpliedPrice(indep_mkt_view_vec_, implied_bid_price_, implied_ask_price_,
                                             implied_bid_size_, implied_ask_size_, implied_mkt_price_);
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      // don't place any best bids in sgx_nk if best bid is higher than that in nkm
      (!param_set_.implied_price_based_cancellation_ ||
       (best_nonself_bid_price_ <= implied_bid_price_ + param_set_.implied_cancel_threshold_)) &&
      (!param_set_.implied_mkt_price_based_cancellation_ ||
       (best_nonself_bid_price_ < implied_mkt_price_ + param_set_.implied_mkt_cancel_threshold_)) &&
    ...
    if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
        // place in sgx_nk if the best bid is lower than that in nkm
        ...
        (param_set_.implied_price_based_place_ &&
         (best_nonself_bid_price_ < implied_bid_price_ - param_set_.implied_place_threshold_)) ||
        (param_set_.implied_mkt_price_based_place_ &&
         (best_nonself_bid_price_ < implied_mkt_price_ - param_set_.implied_mkt_place_threshold_)) ||
        ...
        if ((param_set_.allowed_to_aggress_) && /* external control on aggressing */
            (  // If the buy trades on the source crosses aggress threshold then it will aggress
                ...
                ((targetbias_numbers_ >= current_tradevarset_.l1bid_aggressive_) ||
                 // only LIFT offer if the margin of buying exceeds the
                 // threshold current_tradevarset_.l1bid_aggressive_
                 (param_set_.implied_price_based_agg_ &&
                  (best_nonself_ask_price_ < implied_bid_price_ - param_set_.implied_agg_threshold_)))) &&
...

        {
          ...
          if ((param_set_.allowed_to_improve_) &&
              ...
              ((targetbias_numbers_ >= current_tradevarset_.l1bid_improve_) ||
               // Improving the bid based on the implied price from the leading contract
               // TODO : Make it <=
               (param_set_.implied_price_based_improve_ &&
                (best_nonself_bid_price_ + dep_market_view_.min_price_increment() <
                 implied_bid_price_ - param_set_.implied_improve_threshold_)) ||
      ...
      if (((targetbias_numbers_ + ((dep_market_view_.spread_increments() > moving_avg_dep_bidask_spread_)
          ...
          (param_set_.implied_price_based_place_ &&
           (best_nonself_bid_price_ <
            implied_bid_price_ - (param_set_.implied_place_threshold_ -
                                  param_set_.place_keep_diff_ * dep_market_view_.min_price_increment()))) ||
          (param_set_.implied_mkt_price_based_place_ &&
           (best_nonself_bid_price_ <
            implied_mkt_price_ - (param_set_.implied_mkt_place_threshold_ -
                                  param_set_.place_keep_diff_ * dep_market_view_.min_price_increment())))) {
        top_bid_keep_ = true;
      } else {
        top_bid_keep_ = false;
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      // don't place any best asks in sgx_nk if best ask is lower than that in nkm
      (!param_set_.implied_price_based_cancellation_ ||
       (best_nonself_ask_price_ >= implied_ask_price_ - param_set_.implied_cancel_threshold_)) &&
      (!param_set_.implied_mkt_price_based_cancellation_ ||
       (best_nonself_ask_price_ > implied_mkt_price_ - param_set_.implied_mkt_cancel_threshold_)) &&
      // in case we are using big trades cancel then check if the cooloff has passed
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        // place in sgx_nk if the best ask is higher than that in nkm
        ...
        (watch_.msecs_from_midnight() - last_bigtrades_ask_aggress_msecs_ <=
         param_set_.bigtrades_aggress_cooloff_interval_) ||
        (param_set_.implied_price_based_place_ &&
         (best_nonself_ask_price_ > implied_ask_price_ + param_set_.implied_place_threshold_)) ||
        (param_set_.implied_mkt_price_based_place_ &&
         (best_nonself_ask_price_ > implied_mkt_price_ + param_set_.implied_mkt_place_threshold_)) ||
        (((-targetbias_numbers_ + ((dep_market_view_.spread_increments() > moving_avg_dep_bidask_spread_)
                                       ? (param_set_.high_spread_allowance_)
      ...
      if (watch_.msecs_from_midnight() - last_agg_sell_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) && /* external control on aggressing */
            ...
            (dep_market_view_.spread_increments() <= param_set_.max_int_spread_to_cross_) &&
            // only HIT bid if the margin of selling exceeds the
            // threshold current_tradevarset_.l1ask_aggressive_
            (  // If the sell trades on the source crosses aggress threshold then it will aggress
                ...
                ((-targetbias_numbers_ >= current_tradevarset_.l1ask_aggressive_) ||
                 (param_set_.implied_price_based_agg_ &&
                  (best_nonself_bid_price_ > implied_ask_price_ + param_set_.implied_agg_threshold_)))) &&
            ((last_sell_msecs_ <= 0) ||
             (watch_.msecs_from_midnight() - last_sell_msecs_ >= param_set_.cooloff_interval_) ||
             // TODO : later change setting of last_sell_int_price_ to include px_band_
...

        {
          ...
          if ((param_set_.allowed_to_improve_) &&
              ...
              (  // If the sell trades on the source crosses aggress threshold then it will try to improve the best
                  ...
                  // Improving the ask based on the implied price from the leading contract
                  // TODO : Make it >=
                  (param_set_.implied_price_based_improve_ &&
                   (best_nonself_ask_price_ - dep_market_view_.min_price_increment() >
                    implied_ask_price_ + param_set_.implied_improve_threshold_))) &&
              ((last_sell_msecs_ <= 0) ||
               (watch_.msecs_from_midnight() - last_sell_msecs_ >= param_set_.cooloff_interval_) ||
      ...
      if (((-targetbias_numbers_ + ((dep_market_view_.spread_increments() > moving_avg_dep_bidask_spread_)
                                        ? (param_set_.high_spread_allowance_)
                                        : 0.0) +
            bestask_queue_hysterisis_) >= current_ask_keep_tradevarset_.l1ask_keep_ - trade_bias_sell_ +
                                              cancel_bias_sell_ - l1_bias_ - l1_order_bias_ - long_positioning_bias_) ||
          (param_set_.implied_mkt_price_based_place_ &&
          ...
          (param_set_.implied_price_based_place_ &&
           (best_nonself_ask_price_ >
            (implied_ask_price_ +
             (param_set_.implied_place_threshold_ -
              param_set_.place_keep_diff_ * dep_market_view_.min_price_increment()))))) {  // but with place in line
                                                                                           // effect enough to keep the
                                                                                           // live order there
        top_ask_keep_ = true;
      } else {
        top_ask_keep_ = false;
...
void ImpliedDirectionalAggressiveTrading::OnMarketUpdate(const unsigned int security_id,
...
void ImpliedDirectionalAggressiveTrading::OnTradePrint(const unsigned int security_id,
                                                       const TradePrintInfo& trade_print_info,
                                                       const MarketUpdateInfo& market_update_info) {
  if (security_id == dep_market_view_.security_id()) {
...
void ImpliedDirectionalAggressiveTrading::CallTradingLogic() {
  if (is_ready_) {
...
void ImpliedDirectionalAggressiveTrading::PrintFullStatus() {
