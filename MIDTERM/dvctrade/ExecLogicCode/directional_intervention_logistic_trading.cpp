/**
   \file ExecLogicCode/directional_intervention_logistic_trading.cpp
...
DirectionalInterventionLogisticTrading::DirectionalInterventionLogisticTrading(
...
bool DirectionalInterventionLogisticTrading::IsLargeBidOrderPresent() {
...
bool DirectionalInterventionLogisticTrading::IsLargeAskOrderPresent() {
  for (auto i = 0u; i < 2u; ++i) {
...
bool DirectionalInterventionLogisticTrading::AreSafeMarketConditions() {
...
bool DirectionalInterventionLogisticTrading::AreNonStandardMarketConditions() {
...
void DirectionalInterventionLogisticTrading::UpdateNonStandardMarketConditionVars(
    const MarketUpdateInfo& _market_update_info_) {
  // removed costly check since by the fact that we don't have this strategy name for other securities
  // this is only for DOL
  // if ( dep_market_view_.shortcode ( ).compare ( "BR_DOL_0" ) )
...
void DirectionalInterventionLogisticTrading::OnTimePeriodUpdate(const int num_pages_to_add_) {
...
void DirectionalInterventionLogisticTrading::TradingLogic() {
  ...
  if ((current_tradevarset_.l1bid_trade_size_ > 0) &&
    ...
    if ((last_buy_msecs_ > 0) && (watch_.msecs_from_midnight() - last_buy_msecs_ < param_set_.cooloff_interval_) &&
      ...
      if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
          ((prob_increase_ > prob_decrease_ && prob_increase_ > pos_to_thresh_bid_place_[my_position_]) &&
        ...
        if (prob_increase_ > prob_decrease_ && prob_increase_ > pos_to_thresh_bid_keep_[my_position_]) {
          top_bid_keep_ = true;
        } else {
          top_bid_keep_ = false;
        }
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        ((prob_decrease_ > prob_increase_ && prob_decrease_ > pos_to_thresh_ask_place_[my_position_]) &&
      ...
      if (prob_decrease_ > prob_increase_ &&
          prob_decrease_ > pos_to_thresh_ask_keep_[my_position_]) {  // but with place in line effect enough to keep the
                                                                     // live order there
        top_ask_keep_ = true;
      } else {
        top_ask_keep_ = false;
  ...
  if (top_bid_hit_) {
    ...
    if ((order_manager_.SumAskSizeUnconfirmedEqAboveIntPrice(best_nonself_ask_int_price_ - 1) == 0) &&
      ...
      if (-my_position_ + order_manager_.SumAskSizes() + current_tradevarset_.l1ask_trade_size_ >=
        ...
        if (-my_position_ + order_manager_.SumAskSizes() + current_tradevarset_.l1bid_trade_size_ - _canceled_size_ >
            param_set_.worst_case_position_) {
...
void DirectionalInterventionLogisticTrading::PrintFullStatus() {
...
void DirectionalInterventionLogisticTrading::OnMarketUpdate(const unsigned int _security_id_,
                                                            const MarketUpdateInfo& _market_update_info_) {
  NonSelfMarketUpdate();
