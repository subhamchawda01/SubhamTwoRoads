/**
    \file ExecLogicCode/directional_pair_agrressive_trading.cpp
...
DirectionalPairAggressiveTrading::DirectionalPairAggressiveTrading(
...
void DirectionalPairAggressiveTrading::TradingLogic() {
  ...
  if (current_tradevarset_.l1bid_trade_size_ > 0) {
    ...
    if ((last_buy_msecs_ > 0) && (watch_.msecs_from_midnight() - last_buy_msecs_ < param_set_.cooloff_interval_) &&
      ...
      if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
          ((indep_market_view_.market_update_info_.bestbid_int_price_ >=
            (best_nonself_bid_int_price_ + param_set_.indep_safe_ticks_)) &&
           (indep_market_view_.market_update_info_.bestbid_size_ >= param_set_.indep_safe_size_)) ||
          ((targetbias_numbers_ +
  ...
  if (  // check if we have any allowance to place orders at top level
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        ((indep_market_view_.market_update_info_.bestask_int_price_ <=
          (best_nonself_ask_int_price_ - param_set_.indep_safe_ticks_)) &&
         (indep_market_view_.market_update_info_.bestask_size_ >= param_set_.indep_safe_size_)) ||
        (((-targetbias_numbers_ +
           ((dep_market_view_.spread_increments() > 1) ? (param_set_.high_spread_allowance_) : 0.0)) >=
...
void DirectionalPairAggressiveTrading::PrintFullStatus() {
