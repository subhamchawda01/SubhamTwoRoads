...
EventDirectionalAggressiveTrading::EventDirectionalAggressiveTrading(
    ...
    : BaseTrading(_dbglogger_, _watch_, _dep_market_view_, _order_manager_, _paramfilename_, _livetrading_,
      ...
      evtrading_start_mfm_(0),
      source_shortcode_(""),
      source_shortcode_price_median_price_movement_(0.0),
      scale_risk_based_on_source_(false),
      source_price_move_computed_(false),
      start_computing_source_price_(false),
      decrease_risk_(false),
      product_risk_decided_(false),
      min_risk_(false),
      max_source_price_(-100000),
      min_source_price_(100000),
      current_px_(0.0),
      source_shortcode_market_view_(NULL),
  ...
  for (unsigned int i = 0; i < economic_events_manager_.traded_events_of_the_day().size(); i++) {
    traded_events_of_the_day_.push_back(economic_events_manager_.traded_events_of_the_day()[i]);
  }
  // get traded ezone for this event
  //  traded_events_of_the_day_ = economic_events_manager_.GetTradedEventOfDay();
  is_event_based_ = true;
  double val_decay_factor_ = 1;
  if (param_set_.af_event_halflife_secs_ > 0) {
    val_decay_factor_ = MathUtils::CalcDecayFactor((int)param_set_.af_event_halflife_secs_);
  }
  ...
  // Inititalize the source price movement map
  DBGLOG_TIME << "The min risk param: " << param_set_.use_min_risk_ << DBGLOG_ENDL_FLUSH;
  DBGLOG_TIME << "The source stdev scaling param: " << param_set_.source_stdev_risk_scaling_ << DBGLOG_ENDL_FLUSH;

  if (param_set_.source_stdev_risk_scaling_ != "") {
    // I have to scale the risk based on the source price move
    scale_risk_based_on_source_ = true;
    // Source short code to use for scaling the risk
    source_shortcode_ = param_set_.source_stdev_risk_scaling_;
    DBGLOG_TIME << "The source shortcode is :  " << source_shortcode_ << DBGLOG_ENDL_FLUSH;
    // Get the smv of the source shortcode
    source_shortcode_market_view_ = (ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(source_shortcode_));
    // If the smv is found then go on with scaling risk
    if (source_shortcode_market_view_) {
      SecurityMarketView& source_shortcode_market_view_ =
          *(ShortcodeSecurityMarketViewMap::StaticGetSecurityMarketView(source_shortcode_));
      source_price_indicator_ =
          SimplePriceType::GetUniqueInstance(dbglogger_, watch_, source_shortcode_market_view_, kPriceTypeMidprice);
      // Initialize the source price map

      // Initialize the price decay trade info manager
      dep_decayed_trade_info_manager_ =
          TimeDecayedTradeInfoManager::GetUniqueInstance(_dbglogger_, _watch_, source_shortcode_market_view_, 5.0);

      dep_decayed_trade_info_manager_->compute_sumpxsz();
      dep_decayed_trade_info_manager_->compute_sumsz();

      double source_min_price_increment_ =
          HFSAT::SecurityDefinitions::GetContractMinPriceIncrement(source_shortcode_, watch_.YYYYMMDD());

      EventDirectionalAggressiveTrading::InititalizeSourcePriceMap(source_shortcode_, source_min_price_increment_);
      // Get the source price move
      // If the event ezone is not found in the events happening today
      if (traded_events_of_the_day_.size() != 0) {
        source_shortcode_price_median_price_movement_ =
            GetSourcePriceMedian(std::string(traded_events_of_the_day_[0].event_text_), source_shortcode_);
        DBGLOG_TIME << "The source price movement for the event:" << traded_events_of_the_day_[0].event_text_ << " is "
                    << source_shortcode_price_median_price_movement_ << DBGLOG_ENDL_FLUSH;
      } else {
        scale_risk_based_on_source_ = false;
      }
    } else {
      DBGLOG_TIME << "The smv for the source shortcode not found hence the risk is unchanged! " << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "The symbol for source shortcode is: " << source_shortcode_ << DBGLOG_ENDL_FLUSH;
      scale_risk_based_on_source_ = false;
    }
  }
...
void EventDirectionalAggressiveTrading::ProcessTimePeriodUpdate(const int num_pages_to_add_) {
  ...
  if (scale_risk_based_on_source_) {
    if (traded_events_of_the_day_[0].event_mfm_ < watch_.msecs_from_midnight() &&
        start_computing_source_price_ == false) {
      DBGLOG_TIME << "Start computing price movement from now: " << watch_.UTCTimeString();
      start_computing_source_price_ = true;
    }

    // check whether price is to be computed for the source shortcode

    if (traded_events_of_the_day_[0].event_mfm_ + 9500 < watch_.msecs_from_midnight() &&
        source_price_move_computed_ == false) {
      DBGLOG_TIME << "Event mfm: " << traded_events_of_the_day_[0].event_mfm_ << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "Watch time in msec from midnight: " << watch_.msecs_from_midnight() << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "Source price move computed" << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "Time at which the source price move has been computed: " << watch_.UTCTimeString()
                  << DBGLOG_ENDL_FLUSH;
      source_price_move_computed_ = true;
    }

    bool indicator_ready_ = true;
    // if (scale_risk_based_on_source_ && source_shortcode_market_view_!=NULL && source_price_indicator_!=NULL &&
    // source_price_move_computed_==false){
    if (source_shortcode_market_view_ != NULL && source_price_move_computed_ == false &&
        start_computing_source_price_ == true) {
      current_px_ = source_price_indicator_->indicator_value(indicator_ready_);

      if (current_px_ >= max_source_price_ && current_px_ != 0) {
        max_source_price_ = current_px_;
      }

      if (current_px_ <= min_source_price_ && current_px_ != 0) {
        min_source_price_ = source_price_indicator_->indicator_value(indicator_ready_);
      }
    }

    // decide whether to decrease the risk or not
    if (source_price_move_computed_ && product_risk_decided_ == false) {
      DBGLOG_TIME << "The max price in the period:  " << max_source_price_ << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "The min price in the period:  " << min_source_price_ << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "The median price movement of the source is:  " << source_shortcode_price_median_price_movement_
                  << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "The absolute movement of the source in this event  "
                  << (std::abs(max_source_price_ - min_source_price_)) << DBGLOG_ENDL_FLUSH;
      DBGLOG_TIME << "The absolute movement of the source in this event  "
                  << (std::abs(max_source_price_ - min_source_price_)) << DBGLOG_ENDL_FLUSH;
      if (std::abs(max_source_price_ - min_source_price_) < source_shortcode_price_median_price_movement_) {
        decrease_risk_ = true;
        DBGLOG_TIME << "Risk has been decreased " << DBGLOG_ENDL_FLUSH;
      } else {
        DBGLOG_TIME << "Risk is unchanged " << DBGLOG_ENDL_FLUSH;
      }
      product_risk_decided_ = true;
      DBGLOG_TIME << "Product risk decided: " << product_risk_decided_ << DBGLOG_ENDL_FLUSH;
    }

    // if the product risk is decreased then update the paramset

    if ((product_risk_decided_ && decrease_risk_) ||
        (param_set_.use_min_risk_ == 1 && min_risk_ == false && decrease_risk_)) {
      for (auto i = 0u; i < param_set_vec_.size(); i++) {
        if (param_set_.use_min_risk_ == 1 && decrease_risk_ == true) {
          double new_uts_val_ = 1;
          double max_pos_scaling_factor_ = (new_uts_val_ / param_set_vec_[i].unit_trade_size_);
          int new_max_position_ = (param_set_.max_position_ * max_pos_scaling_factor_);
          param_set_vec_[i].unit_trade_size_ = new_uts_val_;
          param_set_vec_[i].max_position_ = new_max_position_;
          param_set_vec_[i].max_position_original_ = param_set_vec_[i].max_position_;
          min_risk_ = true;
        } else {
          double new_uts_val_ = fmax(1, std::floor(param_set_.unit_trade_size_ / 2));
          double max_pos_scaling_factor_ = (new_uts_val_ / param_set_vec_[i].unit_trade_size_);
          int new_max_position_ = (param_set_.max_position_ * max_pos_scaling_factor_);
          param_set_vec_[i].unit_trade_size_ = new_uts_val_;
          param_set_vec_[i].max_position_ = new_max_position_;
        ...
        //		  param_set_vec_[i].worst_case_position_ = std::max(1,param_set_.worst_case_position_ / 2);

        DBGLOG_TIME << "New uts after decreasing risk: " << param_set_vec_[i].unit_trade_size_ << DBGLOG_ENDL_FLUSH;
        DBGLOG_TIME << "New max position after decreasing risk: " << param_set_vec_[i].max_position_
                    << DBGLOG_ENDL_FLUSH;
      }
      // Since there is only 1 param in EBT hence i can access the param_vec at 0th position and update its value in the
      // paramset
      // So here i am using the param values stored in the first value in the param_set_vec_
      // If ever regime param is being used in EBT, then this code would have to change
      param_set_.unit_trade_size_ = param_set_vec_[0].unit_trade_size_;
      param_set_.max_position_ = param_set_vec_[0].max_position_;
      param_set_.max_position_original_ = param_set_vec_[0].max_position_;
      //	  param_set_.worst_case_position_ = param_set_vec_[0].worst_case_position_;
  ...
  if (getflat_due_to_non_tradable_events_) {
    if (currently_tradable_ && applicable_severity_ < severity_to_getflat_on_) {
      ...
      evtrading_start_mfm_ = watch_.msecs_from_midnight();
...
void EventDirectionalAggressiveTrading::TradingLogic() {
  // setting top level directives
  top_bid_place_ = false;
  top_bid_keep_ = false;
  top_bid_improve_ = false;
  top_ask_lift_ = false;
  // decay threshold as a function of time
  int array_decay_index_ = (int)((watch_.msecs_from_midnight() - evtrading_start_mfm_)) / 1000;
  float present_decay_thres_ = std::max(0.3, val_decay_vec_[array_decay_index_]);
    ...
    if (last_buy_msecs_ <= 0 || (watch_.msecs_from_midnight() - last_buy_msecs_ >= param_set_.cooloff_interval_)) {
      ...
      if ((best_nonself_bid_size_ > param_set_.safe_distance_) ||
          ((targetbias_numbers_ +
            ...
            current_tradevarset_.l1bid_place_ * present_decay_thres_ - trade_bias_buy_ + cancel_bias_buy_) &&
        ...
        if ((targetbias_numbers_ +
            ...
            current_tradevarset_.l1bid_keep_ * present_decay_thres_ - trade_bias_buy_ + cancel_bias_buy_) {
          top_bid_keep_ = true;
        } else {
          top_bid_keep_ = false;
        }
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      // first check for cooloff_interval
      ((last_sell_msecs_ <= 0) ||
       (watch_.msecs_from_midnight() - last_sell_msecs_ >=
        param_set_.cooloff_interval_))  // later change setting of last_sell_int_price_ to include px_band_
      ) {
    ...
    if ((best_nonself_ask_size_ > param_set_.safe_distance_) ||
        (((-targetbias_numbers_ +
           ((dep_market_view_.spread_increments() > 1) ? (param_set_.high_spread_allowance_) : 0.0)) >=
          (current_tradevarset_.l1ask_place_ * present_decay_thres_) - trade_bias_sell_ + cancel_bias_sell_) &&
      ...
      if ((-targetbias_numbers_ +
          ...
          current_tradevarset_.l1ask_keep_ * present_decay_thres_ - trade_bias_sell_ +
              cancel_bias_sell_) {  // but with place in line effect enough to keep the live order there
        top_ask_keep_ = true;
      } else {
        top_ask_keep_ = false;
      }
      ...
      //       param_set_.worst_case_position_ ) ) ) // TODO ... maintain a sum_bid_placed in ordermanager
      {
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
        {
          DBGLOG_TIME_CLASS_FUNC << "SendTrade B of " << current_tradevarset_.l1bid_trade_size_ << " @ "
                                 ...
                                 << " thresh_t: "
                                 << (current_tradevarset_.l1bid_place_ * present_decay_thres_) /
        ...
        if (canceled_size_ > 0) {
          ...
          if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
          {
            DBGLOG_TIME << "Canceled B of " << canceled_size_ << " @ " << best_nonself_bid_price_
                        << " tgt_bias: " << targetbias_numbers_ / dep_market_view_.min_price_increment()
                        << " thresh_t: "
                        << (current_tradevarset_.l1bid_keep_ * present_decay_thres_) /
                               dep_market_view_.min_price_increment() << " tMktSz: " << best_nonself_bid_size_
                        << DBGLOG_ENDL_FLUSH;
          }
        }
      }
      ...
      //       param_set_.worst_case_position_ ) ) ) // TODO ... maintain a sum_ask_placed in ordermanager
      {
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
        {
          DBGLOG_TIME_CLASS_FUNC << "SendTrade S of " << current_tradevarset_.l1ask_trade_size_ << " @ "
                                 ...
                                 << " tgt_bias: " << -targetbias_numbers_ / dep_market_view_.min_price_increment()
                                 << " thresh_t: "
                                 << (current_tradevarset_.l1ask_place_ * present_decay_thres_) /
                                        dep_market_view_.min_price_increment()
        ...
        if (canceled_size_ > 0) {
          ...
          if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
          {
            DBGLOG_TIME << "Canceled S of " << canceled_size_ << " @ " << best_nonself_ask_price_
                        << " tgt_bias: " << -targetbias_numbers_ / dep_market_view_.min_price_increment()
                        << " thresh_t: "
                        << (current_tradevarset_.l1ask_keep_ * present_decay_thres_) /
                               dep_market_view_.min_price_increment() << " tMktSz: " << best_nonself_ask_size_
                        << DBGLOG_ENDL_FLUSH;
          }
        }
      }
...
double EventDirectionalAggressiveTrading::GetSourcePriceMedian(std::string event_string, std::string source_string) {
  // here the event string is the event description
  // create the key for the event
  std::string event_source_key = event_string + "_" + source_string;
  double source_median_movement_ = 0.0;
  DBGLOG_TIME << "The event source key was: " << event_source_key << DBGLOG_ENDL_FLUSH;
  if (event_source_price_map_.find(event_source_key) != event_source_price_map_.end()) {
    source_median_movement_ = event_source_price_map_[event_source_key];
  } else {
    DBGLOG_TIME << "The event source key not found " << DBGLOG_ENDL_FLUSH;
    DBGLOG_TIME << "The event source key was: " << event_source_key << DBGLOG_ENDL_FLUSH;
    source_median_movement_ = 0.0;
  }
  DBGLOG_TIME << "The median price movement returned for the event is: " << source_median_movement_
              << DBGLOG_ENDL_FLUSH;
  return source_median_movement_;
}

void EventDirectionalAggressiveTrading::PrintFullStatus() {
