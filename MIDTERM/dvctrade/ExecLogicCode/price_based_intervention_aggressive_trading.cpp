/**
    \file ExecLogicCode/price_based_intervention_aggressive_trading.cpp
...
PriceBasedInterventionAggressiveTrading::PriceBasedInterventionAggressiveTrading(
...
bool PriceBasedInterventionAggressiveTrading::IsLargeBidOrderPresent() {
  for (auto i = 0u; i < 2; ++i) {
...
bool PriceBasedInterventionAggressiveTrading::IsLargeAskOrderPresent() {
  for (auto i = 0u; i < 2; ++i) {
...
bool PriceBasedInterventionAggressiveTrading::AreSafeMarketConditions() {
...
bool PriceBasedInterventionAggressiveTrading::AreNonStandardMarketConditions() {
  if (dep_market_view_.shortcode().compare("BR_DOL_0")) {  // Currently just for DOL.
    return false;
  }
...
void PriceBasedInterventionAggressiveTrading::UpdateNonStandardMarketConditionVars(
    const MarketUpdateInfo& _market_update_info_) {
  if (dep_market_view_.shortcode().compare("BR_DOL_0")) {  // Currently just for DOL.
    return;
  }
...
void PriceBasedInterventionAggressiveTrading::OnTimePeriodUpdate(const int num_pages_to_add_) {
...
void PriceBasedInterventionAggressiveTrading::TradingLogic() {
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      // will place a bid if there isnt a large ask order or my position is not too long
    ...
    // if either the book is very thick
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      // will place an ask if there isnt a large bid order or my position is not too short
    ...
    if ((order_manager_.SumBidSizeUnconfirmedEqAboveIntPrice(best_nonself_bid_int_price_ + 1) == 0) &&
      ...
      if (param_set_.use_new_aggress_logic_) {
        allowance_for_aggressive_buy_ = my_position_ + order_manager_.SumBidSizes() +
                                        current_tradevarset_.l1bid_trade_size_ - param_set_.max_position_;
      }
...
void PriceBasedInterventionAggressiveTrading::PrintFullStatus() {
  DBGLOG_TIME << "tgt: " << target_price_ << " mkt " << best_nonself_bid_price_ << " X " << best_nonself_bid_size_
              ...
              << " opnl : " << order_manager_.base_pnl().opentrade_unrealized_pnl() << DBGLOG_ENDL_FLUSH;
}

void PriceBasedInterventionAggressiveTrading::OnMarketUpdate(const unsigned int _security_id_,
                                                             const MarketUpdateInfo& _market_update_info_) {
  NonSelfMarketUpdate();
