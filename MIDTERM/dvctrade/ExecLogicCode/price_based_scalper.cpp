/**
   \file ExecLogicCode/price_based_scalper.cpp
...
PriceBasedScalper::PriceBasedScalper(DebugLogger& _dbglogger_, const Watch& _watch_,
...
void PriceBasedScalper::TradingLogic() {
  ...
  /*    ///modify target price based on pnl - continuous mode symmetric logic
 ///analogous to stoploss values
 if ( order_manager_.base_pnl( ).opentrade_unrealized_pnl( ) > 0 )
 {
 if ( my_position_ > 0 )
 {
 target_price_ -= param_set_.pclose_factor_ * (  order_manager_.base_pnl( ).opentrade_unrealized_pnl( )/
 ...
 param_set_.max_position_ );
  ...
  if (current_tradevarset_.l1bid_trade_size_ >
    ...
    if (target_price_ - best_nonself_bid_price_ >=
  ...
  if (current_tradevarset_.l1ask_trade_size_ >
    ...
    if (best_nonself_ask_price_ - target_price_ >=
        ...
        ((last_bigtrades_ask_cancel_msecs_ <= 0) ||
         (watch_.msecs_from_midnight() - last_bigtrades_ask_cancel_msecs_ >=
          param_set_.bigtrades_cooloff_interval_)))  ///-1 for consistency with earlier semantics
    {
      top_ask_place_ = true;
      top_ask_keep_ = true;
    } else {
  ...
  if (!placed_bids_this_round_) {  // only come here if no aggressive or improve bid orders sent this cycle
    if (top_bid_place_) {
      ...
      if ((param_set_.place_multiple_orders_ && CanPlaceNextOrder(best_nonself_bid_int_price_, kTradeTypeBuy) &&
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          dbglogger_ << watch_.tv() << ' ' << typeid(*this).name() << ':' << __func__ << " SendTrade B of "
                     ...
                     << " thresh_t: " << current_bid_tradevarset_.l1bid_place_ / dep_market_view_.min_price_increment()
      ...
      // Iteration here is useless
      {  // cancel all orders at best_nonself_bid_price_

        int cancelled_size_ = 0;
        if (param_set_.place_multiple_orders_) {
          cancelled_size_ =
              order_manager_.CancelBidsEqAboveIntPrice(best_nonself_bid_int_price_, param_set_.unit_trade_size_);
        } else {
          cancelled_size_ = order_manager_.CancelBidsEqAboveIntPrice(best_nonself_bid_int_price_);
        }

        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          dbglogger_ << watch_.tv() << ' ' << typeid(*this).name() << ':' << __func__
                     << " CancelTrade B EqAbove: " << best_nonself_bid_price_ << " Size: " << cancelled_size_
                     << " ebp_t: "
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          dbglogger_ << watch_.tv() << ' ' << typeid(*this).name() << ':' << __func__
                     << " not cancelling because top_bid_keep = " << (top_bid_keep_ ? 'Y' : 'N')
                     << " and confirmed order array size is "
                     << order_manager_.intpx_2_sum_bid_confirmed(best_nonself_bid_int_price_) << " at price  "
                     << best_nonself_bid_int_price_ << '\n';
          dbglogger_.CheckToFlushBuffer();
        }
      }
    ...
    if ((dep_market_view_.spread_increments() > 1) && !bid_improve_keep_) {
      ...
      if (cancelled_size_ > 0) {
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          DBGLOG_TIME_CLASS_FUNC << " Cancelled Improve Bids above price: " << best_nonself_bid_int_price_
                                 << " Size: " << cancelled_size_ << " ebp_t: "
                                 << (target_price_ - best_nonself_bid_price_ + bestbid_queue_hysterisis_) /
                                        dep_market_view_.min_price_increment() << " thresh_t: "
  ...
  if (!placed_asks_this_round_) {
    if (top_ask_place_) {
      ...
      if ((param_set_.place_multiple_orders_ && CanPlaceNextOrder(best_nonself_ask_int_price_, kTradeTypeSell) &&
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          dbglogger_ << watch_.tv() << ' ' << typeid(*this).name() << ':' << __func__ << " SendTrade S of "
                     ...
                     << " thresh_t: " << current_ask_tradevarset_.l1ask_place_ / dep_market_view_.min_price_increment()
      ...
      if (!top_ask_keep_)  //&& order_manager_.SumAskSizeConfirmedEqAboveIntPrice( best_nonself_ask_int_price_ ) > 0 )
      {                    // cancel all orders at best_nonself_ask_price_
        ...
        if (param_set_.place_multiple_orders_) {
          ...
          cancelled_size_ = order_manager_.CancelAsksEqAboveIntPrice(best_nonself_ask_int_price_);
        }

        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          dbglogger_ << watch_.tv() << ' ' << typeid(*this).name() << ':' << __func__ << " CancelTrade EqAbove "
                     << best_nonself_ask_price_ << " Size: " << cancelled_size_ << " eap_t: "
        ...
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          dbglogger_ << watch_.tv() << ' ' << typeid(*this).name() << ':' << __func__
                     << " not cancelling because top_ask_keep = " << (top_ask_keep_ ? 'Y' : 'N')
                     << " and confirmed order array size is "
                     << order_manager_.intpx_2_sum_ask_confirmed(best_nonself_ask_int_price_) << " at price  "
...
void PriceBasedScalper::PrintFullStatus() {
