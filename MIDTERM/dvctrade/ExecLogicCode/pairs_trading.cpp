...
PairsTrading::PairsTrading(DebugLogger& _dbglogger_, const Watch& _watch_, const SecurityMarketView& _dep_market_view_,
                           SmartOrderManager& _order_manager_, MultBasePNL* _mult_base_pnl_,
                           const std::string& _trading_shortcode_, const std::string& _common_paramfilename_,
    ...
    : BaseTrading(_dbglogger_, _watch_, _dep_market_view_, _order_manager_, _common_paramfilename_, _livetrading_,
      ...
      trading_shortcode_(_trading_shortcode_),
  ...
  pair_trade_folio_ = HFSAT::SimpleTrend::GetUniqueInstance(dbglogger_, watch_, trading_shortcode_, atof("100"),
                                                            StringToPriceType_t("OfflineMixMMS"));
  pair_trade_folio_->add_unweighted_indicator_listener(1729, this);
  // pair_trade_folio_->is_ready();
}

void PairsTrading::SetOrderPlacingLogic(const StrategyType _strategy_type_) { order_placing_logic_ = _strategy_type_; }

void PairsTrading::AddProductToTrade(SecurityMarketView* _dep_market_view_, SmartOrderManager* _smart_order_manager_,
  ...
  t_ins_->shortcode_ = _dep_market_view_->shortcode();
  ...
  GetEZVecForShortcode(_dep_market_view_->shortcode(), _trading_start_mfm_, ezone_vec_);

  //( t_trading_end_utc_mfm_ - 1 ) to avoid events overlapping period for EU and US
  GetEZVecForShortcode(dep_market_view_.shortcode(), _trading_end_mfm_ - 1, ezone_vec_);

  severity_to_getflat_on_ = common_paramset_.severity_to_getflat_on_;
  ...
  trend_product_ = HFSAT::SimpleTrend::GetUniqueInstance(dbglogger_, watch_, _dep_market_view_->shortcode(),
                                                         atof("100"), StringToPriceType_t("OfflineMixMMS"));
  trend_product_->add_unweighted_indicator_listener(10000 + total_products_trading_ - 1, this);
  ...
  SlowStdevCalculator* t_stdev_ =
      SlowStdevCalculator::GetUniqueInstance(dbglogger_, watch_, _dep_market_view_->shortcode(), 100u * 1000u, 0.01);
  ...
  _model_math_->GetBasePx(_price_type_);
  L1Price* t_l1px = L1Price::GetUniqueInstance(dbglogger_, watch_, *_dep_market_view_, _price_type_);
  t_l1px->add_unweighted_indicator_listener(base_px_offset_ + total_products_trading_ - 1, this);
  ...
  SetComputeTresholds(t_ins_, paramset_, _dep_market_view_);
  t_ins_->hist_dollar_adjusted_change_stdev_ =
      SampleDataUtil::GetAvgForPeriod(_dep_market_view_->shortcode(), watch_.YYYYMMDD(), 250, trading_start_utc_mfm_,
                                      trading_end_utc_mfm_, std::string("STDEV")) *
      SecurityDefinitions::GetContractNumbersToDollars(_dep_market_view_->shortcode(), watch_.YYYYMMDD());
  product_vec_.push_back(t_ins_);
}

void PairsTrading::BuildTradeVarSets(int _product_index_) {
...
void PairsTrading::BuildConstantTradeVarSets(int _product_index_) {
...
void PairsTrading::TradingLogic(int _product_index_) {
  ...
  while (my_risk_ + tot_buy_placed_ < (int)std::min(t_paramset_->max_position_, t_paramset_->worst_case_position_)) {
     ...
     * Though this is most probably make a bound on prices which we quote to be aligned with the prices where we
     * quoted
     * first time
...
char PairsTrading::GetOrderLevelIndicator(TradeType_t order_side, int int_order_px, int _product_index_) {
...
void PairsTrading::OnExec(const int t_new_position_, const int _exec_quantity_, const TradeType_t _buysell_,
  ...
  for (unsigned int prod_ind_ = 0; prod_ind_ < product_vec_.size(); prod_ind_++) {
    if ((int)prod_ind_ != index_) {
      // order_manager_vec_[prod_ind_]->CancelAllOrders();
      ;
    }
  }
}

void PairsTrading::OnCancelReject(const TradeType_t _buysell_, const double _price_, const int r_int_price_,
...
double PairsTrading::ModifyTargetPx(double _target_price_, int _modelmath_index_) {
  double _stdev_constituent_ = stdev_vec_[_modelmath_index_];
  double _stdev_portfolio_ = stdev_vec_[0];
  double _new_target_price_ = _target_price_ * (_stdev_constituent_ / _stdev_portfolio_);
  return _new_target_price_;
}

double PairsTrading::ModifyTargetBias(double _targetbias_numbers_, int _modelmath_index_) {
  double _stdev_constituent_ = stdev_vec_[_modelmath_index_];
  double _stdev_portfolio_ = stdev_vec_[0];
  double _new_targetbias_numbers_ = _targetbias_numbers_ * (_stdev_constituent_ / _stdev_portfolio_);
  return _new_targetbias_numbers_;
}

double PairsTrading::MeanReversionAlpha(int _modelmath_index_) {
  //  double _basePx_constituent = base_px_vec_[_modelmath_index_];
  double _basePx_constituent_trend_ = base_px_simpletrend_vec_[_modelmath_index_];
  double _stdev_constituent_ = stdev_vec_[_modelmath_index_];
  double _stdev_portfolio_ = stdev_vec_[0];
  double deviation_from_portfolio_ =
      trend_portfolio_price_ * (_stdev_constituent_ / _stdev_portfolio_) - _basePx_constituent_trend_;

  if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
    DBGLOG_TIME_CLASS_FUNC_LINE << " Shortcode_Index " << _modelmath_index_ << " " << deviation_from_portfolio_ << " "
                                << _basePx_constituent_trend_ << " " << trend_portfolio_price_ << " "
                                << _stdev_constituent_ << " " << _stdev_portfolio_ << DBGLOG_ENDL_FLUSH;
  }

  if (common_paramset_.skip_mean_reversion_on_indep_) {
    if (_basePx_constituent_trend_ * deviation_from_portfolio_ < 0 &&
        fabs(_basePx_constituent_trend_) >
            common_paramset_.mean_reversion_skip_factor_ * fabs(deviation_from_portfolio_)) {
      deviation_from_portfolio_ = 0;
      if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
        DBGLOG_TIME_CLASS_FUNC_LINE << " Shortcode_Index " << _modelmath_index_ << " Setting to 0" << DBGLOG_ENDL_FLUSH;
      }
    }
  }

  // if (_modelmath_index_ == 0)
  // return 0;
  //  double deviation_from_portfolio_NIY_ =
  //      trend_portfolio_price_ * stdev_vec_[0] / stdev_vec_[0] - base_px_simpletrend_vec_[0];
  //  double deviation_from_portfolio_NKD_ =
  //      trend_portfolio_price_ * stdev_vec_[1] / stdev_vec_[0] - base_px_simpletrend_vec_[1];
  // std::cout<< deviation_from_portfolio_NIY_ << " " << deviation_from_portfolio_NKD_<<"
  // "<<base_px_simpletrend_vec_[0]<<" "<<base_px_simpletrend_vec_[1]<<" "<<trend_portfolio_price_<<"
  // "<<stdev_vec_[0]<<" "<<stdev_vec_[1]<<" "<<trend_portfolio_price_ * stdev_vec_[1] / stdev_vec_[0]
  //          << std::endl;

  // std::cout << "DeviationPortfolio " << deviation_from_portfolio_NIY_ << " " << deviation_from_portfolio_NKD_
  //          << std::endl;

  return deviation_from_portfolio_;
  // else
  // return 0;
}

bool PairsTrading::UpdateTarget(double _target_price_, double _targetbias_numbers_, int _modelmath_index_) {
  if (!initialized_) {
    Initialize();
  }

  /*
   * we would be getting update with just one modelmath_index
   */

  if (!is_ready_) {
    bool t_is_ready = true;
    t_is_ready = model_math_vec_[_modelmath_index_]->is_ready();

    if (watch_.msecs_from_midnight() > current_product_->trading_start_utc_mfm_ && watch_.YYYYMMDD() == tradingdate_ &&
        t_is_ready) {
      ...
      for (auto i = 0u; i < dep_market_view_vec_.size(); i++) {
        // Compute the target_bias for this security

        double this_prod_target_bias = ModifyTargetBias(_targetbias_numbers_, i);
        if (!is_ready_vec_[i] &&
            this_prod_target_bias <
                dep_market_view_vec_[i]->spread_increments() * dep_market_view_vec_[i]->min_price_increment() &&
            dep_market_view_vec_[i]->is_ready()) {
          is_ready_vec_[i] = true;
          DBGLOG_TIME_CLASS_FUNC << "SACI: " << order_manager_vec_[i]->server_assigned_client_id_
                                 << " got ready! shc: " << dep_market_view_vec_[i]->shortcode() << DBGLOG_ENDL_FLUSH;
          model_math_vec_[_modelmath_index_]->ShowIndicatorValues();
          TradeVarSetLogic(i);
        }

        t_is_ready = t_is_ready && is_ready_vec_[i];
      }
    ...
    if ((!external_freeze_trading_) && (!freeze_due_to_exchange_stage_) && (!freeze_due_to_funds_reject_)) {
      ...
      /*
       * Since we are getting update from one model only,
       * we need to call trading logic for all securities in this iteration only
       */

      for (auto index = 0; index < total_products_trading_; index++) {
        current_product_ = product_vec_[index];

        // Compute target_bias for this product
        //_target_price_ = ModifyTargetPx(_target_price_, _modelmath_index_);
        double new_targetbias_numbers = ModifyTargetBias(_targetbias_numbers_, index);
        double mean_revesion_bias_numbers = 0;
        if (common_paramset_.use_mean_reversion_) {
          mean_revesion_bias_numbers = MeanReversionAlpha(index);
        }

        double base_price = base_px_vec_[index];
        // std::cout<<index<<" "<<new_targetbias_numbers<<" "<<mean_revesion_bias_numbers<<endl;
        double new_target_price = base_price + new_targetbias_numbers + mean_revesion_bias_numbers;

        if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
          DBGLOG_CLASS_FUNC_LINE << " tgtpr: " << new_target_price << " tgtbias: " << new_targetbias_numbers
                                 << " basepx: " << base_price << " is_ready: " << is_ready_ << " "
                                 << current_product_->trading_start_utc_mfm_ << DBGLOG_ENDL_FLUSH;
        }

        target_price_vec_[index] = new_target_price;
        targetbias_numbers_vec_[index] = new_targetbias_numbers;

        if ((!should_be_getting_flat_) && !should_be_getting_flat_vec_[index] && current_product_->is_trading_today_ &&
            (current_product_->throttle_manager_->allowed_through_throttle(watch_.msecs_from_midnight()))) {
          // call appropriate trading-logic
          if (order_placing_logic_ == kPriceBasedVolTrading) {
            TradingLogic(index);
          } else if (order_placing_logic_ == kDirectionalAggressiveTrading) {
            DatTradingLogic(index);
          } else if (order_placing_logic_ == kPriceBasedAggressiveTrading) {
            PbatTradingLogic(index);
          } else {
            TradingLogic(index);
          }
          // handle nonbest level orders
          CallPlaceCancelNonBestLevels(index);
        } else if (should_be_getting_flat_) {
          // Get-flat on all products
        ...
        } else if (should_be_getting_flat_vec_[index] || !current_product_->is_trading_today_) {
          // GetFlatOnIndividualProducts
          GetFlatTradingLogic(index);
          if (livetrading_ || (dbglogger_.CheckLoggingLevel(TRADING_INFO))) {
            LogFullStatus();
          }
        }
...
void PairsTrading::OnMarketUpdate(const unsigned int _security_id_, const MarketUpdateInfo& _market_update_info_) {
  if (!initialized_) {
    Initialize();
  }
...
void PairsTrading::OnTradePrint(const unsigned int _security_id_, const TradePrintInfo& _trade_print_info_,
...
void PairsTrading::NonSelfMarketUpdate(int _product_index_) {
...
void PairsTrading::OnMarketStatusChange(const unsigned int _security_id_, const MktStatus_t _new_market_status_) {
  if (!initialized_) {
    Initialize();
  }
...
void PairsTrading::OnStdevUpdate(const unsigned int security_id_, const double& _new_stdev_val_) {
  product_vec_[sec_id_to_index_[security_id_]]->m_stdev_ = (common_paramset_.stdev_fact_ * _new_stdev_val_);
  stdev_vec_[sec_id_to_index_[security_id_]] = _new_stdev_val_;
  /*
...
void PairsTrading::OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
  ...
  } else if (_indicator_index_ == 1729) {
    trend_portfolio_price_ = _new_value_;
  } else if (_indicator_index_ < (unsigned)(10000 + total_products_trading_)) {
    base_px_simpletrend_vec_[_indicator_index_ - 10000] = _new_value_;
  } else if (_indicator_index_ < (unsigned)(100000 + total_products_trading_)) {
    base_px_vec_[_indicator_index_ - base_px_offset_] = _new_value_;
  }
}

void PairsTrading::OnVolumeRatioUpdate(const unsigned int _security_index_, const double& r_new_volume_ratio_) {
...
void PairsTrading::GetFlatTradingLogic(int _product_index_) {
  current_product_ = product_vec_[_product_index_];

  if (current_product_->getflat_aggressive_)
...
void PairsTrading::NormalGetFlatTradingLogic(int _product_index_) {
  current_product_ = product_vec_[_product_index_];
...
void PairsTrading::AggressiveGetFlatTradingLogic(int _product_index_) {
  current_product_ = product_vec_[_product_index_];
...
void PairsTrading::UnusualTradingStatusGetFlatTradingLogic(int _product_index_) {
...
void PairsTrading::OnPositionChange(int t_new_position_, int position_diff_, const unsigned int _security_id_) {
...
void PairsTrading::UpdateSecurityWiseRisk(int new_position, int old_position, int prod_index) {
  if (common_paramset_.compute_notional_risk_) {
    // Compute the notional based risk wherever required.
    // Mostly needed by Equities
  } else {
    DBGLOG_TIME_CLASS_FUNC << " Pos: ";
    for (unsigned i = 0; i < position_vec_.size(); i++) {
      // Position of security t_index is converted into position of security i
      position_vec_[i] += ((new_position - old_position) * (product_vec_[prod_index]->hist_dollar_adjusted_change_stdev_) /
                           product_vec_[i]->hist_dollar_adjusted_change_stdev_);
      dbglogger_ << " " << products_being_traded_[i] << " Risk: " << position_vec_[i] << " ";
    }
    dbglogger_ << DBGLOG_ENDL_FLUSH;
  }
}

void PairsTrading::UpdateBetaPosition(const unsigned int sec_id_, int _new_position_) {
  ...
  double t_old_beta_risk_ = position_vec_[0];

  UpdateSecurityWiseRisk(_new_position_, product_vec_[t_index_]->position_, t_index_);
  ...
  for (int i = 0; i < total_products_trading_; i++) {
    ...
    product_vec_[i]->beta_adjusted_position_ =
        MathUtils::GetFlooredMultipleOf(self_pos_projection_factor_ * product_vec_[i]->position_ +
                                            (1 - self_pos_projection_factor_) * (position_vec_[i]),
  ...
  UpdateOpenUnrealizedPNL(t_old_beta_risk_, position_vec_[0]);
  mult_base_pnl_->UpdateTotalRisk(position_vec_[0]);
}

void PairsTrading::UpdateProductPosition(int sec_id_, int _new_position_) {
...
inline void PairsTrading::UpdateVolumeAdjustedMaxPos(int _product_index_) {
  if (prod_paramset_vec_[_product_index_]->read_scale_max_pos_) {
...
void PairsTrading::TradeVarSetLogic(int _product_index_) {
  ...
  if (t_map_pos_increment_ > 1)  // only when param_set_.max_position_ > MAX_POS_MAP_SIZE
  {
    if (t_my_risk_ > 0) {
      ...
      double size_to_compare = t_current_global_tradevarset_.l1bid_trade_size_;
      if (
          // If current size exceeds global risk
          (size_to_compare + position_vec_[_product_index_] > max_risk_vec_[_product_index_]) ||
          // Or if current size exceeds 5 times self risk, ideally we can plainly ignore this
          (t_current_global_tradevarset_.l1bid_trade_size_ + t_position_ >
           int(3 * prod_paramset_vec_[_product_index_]->max_position_))) {
      ...
      current_position_tradevarset_map_index_ -= std::min(MAX_POS_MAP_SIZE, (abs(t_my_risk_) / t_map_pos_increment_));
      t_current_global_tradevarset_ = t_position_tradevarset_map_[current_position_tradevarset_map_index_];
      double size_to_compare = t_current_global_tradevarset_.l1ask_trade_size_;
      if (
          // If current size exceeds global risk
          (size_to_compare - position_vec_[_product_index_] > max_risk_vec_[_product_index_]) ||
          // Or if current size exceeds 5 times self risk, ideally we can plainly ignore this
          (t_current_global_tradevarset_.l1ask_trade_size_ - t_position_ >
           int(3 * prod_paramset_vec_[_product_index_]->max_position_))) {
  ...
  if (dbglogger_.CheckLoggingLevel(TRADING_INFO)) {
    DBGLOG_TIME_CLASS_FUNC << " L1BidTrdSize: " << t_current_global_tradevarset_.l1bid_trade_size_
                           << " L1AskTrdSize: " << t_current_global_tradevarset_.l1ask_trade_size_ << " "
                           << position_vec_[_product_index_] << " " << max_risk_vec_[_product_index_]
                           << DBGLOG_ENDL_FLUSH;
  }
  current_global_tradevarset_vec_[_product_index_] = t_current_global_tradevarset_;
}

void PairsTrading::ProcessGetFlat() {
  if (!initialized_) {
    Initialize();
  }
...
bool PairsTrading::ShouldBeGettingFlat() {
  // Global controls
...
inline void PairsTrading::OnControlUpdate(const ControlMessage& _control_message_, const char* symbol,
                                          const int trader_id) {
  if (!initialized_) {
    Initialize();
  }
...
inline double PairsTrading::total_pnl() { return total_pnl_; }

inline void PairsTrading::UpdatePNL(int _total_pnl_) {
  total_pnl_ = _total_pnl_;
  open_unrealized_pnl_ = total_pnl_ - realized_pnl_;
}

inline void PairsTrading::UpdateOpenUnrealizedPNL(double last_pc1_risk_, double current_pc1_risk_) {
...
bool PairsTrading::IsHittingMaxLoss() {
...
bool PairsTrading::IsHittingOpentradeLoss() {
  if (open_unrealized_pnl_ < -max_opentrade_loss_) {
...
void PairsTrading::GetProductListToGetFlatMultOrder() {
...
void PairsTrading::Initialize() {
  DBGLOG_CLASS_FUNC << " Stopping for EZones:";
  ...
  stdev_vec_.resize(total_products_trading_, 0.0);
  base_px_vec_.resize(total_products_trading_, 0.0);
  base_px_simpletrend_vec_.resize(total_products_trading_, 0.0);
  position_vec_.resize(total_products_trading_, 0.0);

  max_risk_vec_.resize(total_products_trading_, 0.0);
  for (unsigned i = 0; i < product_vec_.size(); i++) {
    // Max risk is computed in terms of ith product
    for (unsigned j = 0; j < product_vec_.size(); j++) {
      max_risk_vec_[i] += (prod_paramset_vec_[j]->max_position_ * product_vec_[j]->hist_dollar_adjusted_change_stdev_) /
                          product_vec_[i]->hist_dollar_adjusted_change_stdev_;
    }
    DBGLOG_FUNC << " MaxRisK: " << max_risk_vec_[i] << " Contract: " << product_vec_[i]->shortcode_
                << DBGLOG_ENDL_FLUSH;
  }
...
void PairsTrading::InitializeBetaValues() {
  // Change LRDB
  ...
  for (unsigned i = 0; i < products_being_traded_.size(); i++) {
    offline_beta_[products_being_traded_.size()][i] =
        lrdb_vec_[i]->GetLRCoeff("INVALIDSHC", products_being_traded_[i]).lr_coeff_;
  ...
  for (unsigned i = 0; i < products_being_traded_.size(); i++) {
    offline_beta_[i][products_being_traded_.size()] =
        lrdb_vec_[i]->GetLRCoeff(products_being_traded_[i], "INVALIDSHC").lr_coeff_;
...
*/
  ...
  for (auto i = 0u; i < products_being_traded_.size(); i++) {
    betas_[i] = offline_beta_[0][i];
  }
}

void PairsTrading::PrintFullStatus(int _product_index_) {
...
void PairsTrading::LogFullStatus() {
  if (zero_logging_mode_ && !override_zero_logging_mode_once_for_external_cmd_) {
    return;
  }
...
void PairsTrading::ReportResults(HFSAT::BulkFileWriter& trades_writer_, bool _conservative_close_) {
  ...
  for (unsigned i = 0; i < order_manager_vec_.size(); i++) {
    if (order_manager_vec_[i] != nullptr) {
      ...
      if (livetrading_) {
        dbglogger_ << "EOD_MSG_COUNT: " << runtime_id_ << " " << dep_market_view_vec_[i]->secname() << " "
                   << (t_order_manager_.SendOrderCount() + t_order_manager_.CxlOrderCount() +
                       t_order_manager_.ModifyOrderCount()) << "\n";
      } else {
        trades_writer_ << "EOD_MSG_COUNT: " << runtime_id_ << " " << dep_market_view_vec_[i]->secname() << " "
                       << (t_order_manager_.SendOrderCount() + t_order_manager_.CxlOrderCount() +
                           t_order_manager_.ModifyOrderCount()) << "\n";
      }
    }
...
void PairsTrading::SendMail(std::string _mail_content_, std::string _mail_subject_) {
  HFSAT::Email email_;
...
void PairsTrading::get_positions(std::vector<std::string>& _instrument_vec_, std::vector<int>& _position_vec_) {
  for (unsigned i = 0; i < products_being_traded_.size(); i++) {
