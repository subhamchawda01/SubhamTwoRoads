/**
    \file ExecLogicCode/sgx_nifty_price_pair_aggressive_trading.cpp
...
void SGXNiftyPricePairAggressiveTrading::CollectORSShortCodes(DebugLogger& _dbglogger_,
...
SGXNiftyPricePairAggressiveTrading::SGXNiftyPricePairAggressiveTrading(
    ...
    : BaseTrading(_dbglogger_, _watch_, _dep_market_view_, _order_manager_, _paramfilename_, _livetrading_,
      ...
      nse_nifty_market_view_(_nse_nifty_market_view_) {
	nse_nifty_market_view_.subscribe_price_type(this, HFSAT::kPriceTypeMktSizeWPrice);
	nse_nifty_market_view_.subscribe_tradeprints(this);
}

void SGXNiftyPricePairAggressiveTrading::TradingLogic() {
  if (!nse_nifty_market_view_.is_ready()) {
    return;
  }
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      ( best_nonself_bid_price_ <= nse_nifty_market_view_.market_update_info_.bestbid_price_ ) &&
    ...
    if ((((target_price_ - best_nonself_bid_price_ >= current_tradevarset_.l1bid_place_ - l1_bias_ - l1_order_bias_) ||
    		( best_nonself_bid_price_ < nse_nifty_market_view_.market_update_info_.bestbid_price_ ) ) &&
         (best_nonself_bid_size_ > param_set_.min_size_to_join_) &&  // Don't place any orders at a level if less than X size there
      ...
      if (watch_.msecs_from_midnight() - last_agg_buy_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ((target_price_ - best_nonself_ask_price_ >= current_tradevarset_.l1bid_aggressive_ ) ||
            		( best_nonself_ask_price_ < nse_nifty_market_view_.market_update_info_.bestbid_price_ ) ) &&
  ...
  if (  // check if we have any allowance to place orders at top level
      ...
      ( best_nonself_ask_price_ >= nse_nifty_market_view_.market_update_info_.bestask_price_ ) &&
      // first check for cooloff_interval
      ...
      if (watch_.msecs_from_midnight() - last_agg_sell_msecs_ > param_set_.agg_cooloff_interval_) {
        ...
        if ((param_set_.allowed_to_aggress_) &&  // external control on aggressing
            ...
            ((best_nonself_bid_price_ - target_price_ >= current_tradevarset_.l1ask_aggressive_ ) ||
            		( best_nonself_bid_price_ > nse_nifty_market_view_.market_update_info_.bestask_price_ ) ) &&
            ((last_sell_msecs_ <= 0) ||
  ...
  if (top_ask_lift_) {
    ...
    if ((order_manager_.SumBidSizeUnconfirmedEqAboveIntPrice(best_nonself_bid_int_price_ + 1) == 0) &&
      ...
      if (allowance_for_aggressive_buy_ >= 0) {
        ...
        {
          DBGLOG_TIME_CLASS_FUNC << "Sending aggressive B at px " << best_nonself_ask_price_ << " position " << my_position_
                                 << DBGLOG_ENDL_FLUSH;
        }
      }
    } else {
  ...
  if (top_bid_hit_) {
    ...
    if (((order_manager_.SumAskSizeUnconfirmedEqAboveIntPrice(best_nonself_ask_int_price_ - 1) == 0) &&
      ...
      if (allowance_for_aggressive_sell_ >= 0) {
        ...
        // Place new order
        order_manager_.SendTrade(best_nonself_bid_price_, best_nonself_bid_int_price_, current_tradevarset_.l1ask_trade_size_,
                                 kTradeTypeSell, 'A');
        placed_asks_this_round_ = true;
        last_agg_sell_msecs_ = watch_.msecs_from_midnight();
        if (dbglogger_.CheckLoggingLevel(TRADING_INFO))  // zero logging
        {
          DBGLOG_TIME_CLASS_FUNC << "Sending aggressive S at px " << best_nonself_bid_price_ << " position " << my_position_
                                 << DBGLOG_ENDL_FLUSH;
        }
      }
    } else {
...
void SGXNiftyPricePairAggressiveTrading::PrintFullStatus() {
