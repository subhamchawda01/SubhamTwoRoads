...
class KVectorContextImpl {
 public:
  ...
  explicit KVectorContextImpl(std::string elemDelim = " ", std::string startDelim = std::string(),
                              std::string endDelim = std::string(), unsigned prec = 4)
      : elemDelim_(elemDelim), startDelim_(startDelim), endDelim_(endDelim), precision_(prec), width_(8 + prec) {
    std::string ws(" \t\n");
...
inline KVector<T, BEG, DBG>::KVector(const KVector& v)
    : vimpl_(v.vimpl_), v_((v.size() != 0) ? &vimpl_[0] - BEG : 0), n_(v.n_) {}
...
inline T& KVector<T, BEG, DBG>::operator()(K_UINT_32 i) {
  if (DBG) {
    if (i < BEG || i >= n_ + BEG) {
      KOSTRINGSTREAM oss;
      oss << "Trying to access element " << i << " not included in [" << BEG << ", " << n_ + BEG - 1
          << "]." KEND_OF_STREAM;
...
template <typename T, K_UINT_32 BEG, bool DBG>
inline KVector<T, BEG, DBG>& KVector<T, BEG, DBG>::operator=(const KVector& v) {
  KVector temp(v);
  swap(temp);
  return *this;
}
...
template <typename T, K_UINT_32 BEG, bool DBG>
inline std::istream& operator>>(std::istream& is, KVector<T, BEG, DBG>& v) {
  v.get(is);
  return is;
}
...
template <typename T, K_UINT_32 BEG, bool DBG>
inline std::ostream& operator<<(std::ostream& os, const KVector<T, BEG, DBG>& v) {
  v.put(os);
  return os;
}
}
