...
class KMatrixContextImpl {
 public:
  ...
  KMatrixContextImpl(std::string elemDelim = " ", std::string rowDelim = "\n", std::string startDelim = std::string(),
                     std::string endDelim = std::string(), unsigned prec = 4)
      : elemDelim_(elemDelim),
        rowDelim_(rowDelim),
        startDelim_(startDelim),
        endDelim_(endDelim),
        precision_(prec),
        width_(8 + prec) {
    std::string ws(" \t\n");
    skipElemDelim_ = (elemDelim_.find_first_not_of(ws) != std::string::npos);
    skipRowDelim_ = (rowDelim_.find_first_not_of(ws) != std::string::npos);
    skipStartDelim_ = (startDelim_.find_first_not_of(ws) != std::string::npos);
    skipEndDelim_ = (endDelim_.find_first_not_of(ws) != std::string::npos);
  }
...
template <typename T, K_UINT_32 BEG, bool DBG>
inline T& KMatrix<T, BEG, DBG>::operator()(K_UINT_32 i, K_UINT_32 j) {
  if (DBG) {
    if (i < BEG || i >= m_ + BEG || j < BEG || j >= n_ + BEG) {
      KOSTRINGSTREAM oss;
      oss << "Trying to access element (" << i << ", " << j << ") not included in [" << BEG << ", " << m_ + BEG - 1
...
//!   bounds and <tt>DBG == true</tt>.
template <typename T, K_UINT_32 BEG, bool DBG>
inline const T& KMatrix<T, BEG, DBG>::operator()(K_UINT_32 i, K_UINT_32 j) const {
  if (DBG) {
    if (i < BEG || i >= m_ + BEG || j < BEG || j >= n_ + BEG) {
      KOSTRINGSTREAM oss;
      oss << "Trying to access element (" << i << ", " << j << ") not included in [" << BEG << ", " << m_ + BEG - 1
...
template <typename T, K_UINT_32 BEG, bool DBG>
inline KMatrix<T, BEG, DBG>& KMatrix<T, BEG, DBG>::operator=(const KMatrix& M) {
  KMatrix temp(M);
  swap(temp);
  return *this;
}
...
template <typename T, K_UINT_32 BEG, bool DBG>
inline void KMatrix<T, BEG, DBG>::assign(K_UINT_32 m, K_UINT_32 n, const T* v) {
  KMatrix temp(m, n, v);
  swap(temp);
}
...
template <typename T, K_UINT_32 BEG, bool DBG>
inline std::ostream& operator<<(std::ostream& os, const KMatrix<T, BEG, DBG>& M) {
  M.put(os);
  return os;
}
}
