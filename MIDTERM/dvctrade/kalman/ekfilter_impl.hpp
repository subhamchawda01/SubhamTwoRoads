...
template <typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
EKFilter<T, BEG, OQ, OVR, DBG>::EKFilter(K_UINT_32 n_, K_UINT_32 nu_, K_UINT_32 nw_, K_UINT_32 m_, K_UINT_32 nv_)
    : flags(0) {
...
template <typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
void EKFilter<T, BEG, OQ, OVR, DBG>::setDim(K_UINT_32 n_, K_UINT_32 nu_, K_UINT_32 nw_, K_UINT_32 m_, K_UINT_32 nv_) {
  setSizeX(n_);
  setSizeU(nu_);
  setSizeW(nw_);
  setSizeZ(m_);
    ...
    if (flags & (KALMAN_W_MODIFIED | KALMAN_Q_MODIFIED)) {
      for (i = BEG; i < n + BEG; ++i) {
        for (j = BEG; j < nw + BEG; ++j) {
          W_(i, j) = W(i, j);
          for (k = BEG; k < j; ++k) W_(i, j) += W(i, k) * Q(j, k);
        }
      }
    }
...
void EKFilter<T, BEG, OQ, OVR, DBG>::measureUpdateStep(const Vector& z_) {
  ...
  for (i = BEG; i < m + BEG; ++i) dz(i) = z_(i) - z(i);
  ...
  if (OVR) {
    ...
    if (flags & (KALMAN_V_MODIFIED | KALMAN_R_MODIFIED)) {
      for (i = BEG; i < m + BEG; ++i) R_(i, i) = V(i, i) * V(i, i) * R(i, i);
    }
    ...
    if (flags & (KALMAN_V_MODIFIED | KALMAN_R_MODIFIED)) {  // calculate R_
      ...
      for (i = BEG; i < m + BEG; ++i) {
        ...
        for (j = BEG; j < nv + BEG; ++j) {
          _x(j) = T(0.0);
          for (k = BEG; k < nv + BEG; ++k) _x(j) += V(i, k) * R(k, j);
        }

        // R_(i,:) = (V*R*V')(i,:) = (V*R)(i,:) * V'
        for (j = BEG; j < m + BEG; ++j) {
          R_(i, j) = T(0.0);
          for (k = BEG; k < nv + BEG; ++k) R_(i, j) += _x(k) * V(j, k);
        }
      }
    ...
    if (flags & (KALMAN_H_MODIFIED | KALMAN_V_MODIFIED | KALMAN_R_MODIFIED)) {  // calculate H_
      ...
      for (i = BEG; i < m + BEG; ++i) {
        for (j = BEG; j < n + BEG; ++j) {
          H_(i, j) = H(i, j);
          for (k = i + 1; k < m + BEG; ++k) H_(i, j) += R_(k, i) * H(k, j);
        }
      }
    }
    ...
    for (i = BEG; i < m + BEG; ++i) {
      _x(i) = dz(i);
      for (k = i + 1; k < m + BEG; ++k) _x(i) += R_(k, i) * dz(k);
    }
  ...
  _x = T(0.0);
  for (i = BEG; i < m + BEG; ++i) {
    for (j = BEG; j < n + BEG; ++j) a(j) = H(i, j);

    measureUpdate(dz(i), R_(i, i));
  }
  for (i = BEG; i < n + BEG; ++i) x(i) += _x(i);
...
void EKFilter<T, BEG, OQ, OVR, DBG>::sizeUpdate() {
  ...
  if (flags & KALMAN_P_MODIFIED) {
    ...
    U.resize(n, nn);
    for (K_UINT_32 i = BEG; i < n + BEG; ++i)
      for (K_UINT_32 j = BEG; j < n + BEG; ++j) U(i, j) = _P(i, j);
    ...
    _P.resize(n, nn);
    for (K_UINT_32 i = BEG; i < n + BEG; ++i)
      for (K_UINT_32 j = i; j < n + BEG; ++j) _P(i, j) = U(i, j);
    U.swap(_P);
  }
...
void EKFilter<T, BEG, OQ, OVR, DBG>::factor(Matrix& P_) {
  ...
  for (j = N - 1 + BEG; j > BEG; --j) {
    ...
    for (k = BEG; k < j; ++k) {
      beta = P_(k, j);
      P_(k, j) = alpha * beta;
      for (i = BEG; i <= k; ++i) P_(i, k) -= beta * P_(i, j);
    }
  }
}
...
void EKFilter<T, BEG, OQ, OVR, DBG>::upperInvert(Matrix& P_) {
  ...
  for (i = N - 2 + BEG; i != (K_UINT_32)(BEG - 1); --i) {  // intended overflow if BEG==0
    for (k = i + 1; k < N + BEG; ++k) {
      val = P_(i, k);
      for (j = i + 1; j <= k - 1; ++j) val += P_(i, j) * P_(k, j);
      P_(k, i) = -val;
    }
  }
}
...
void EKFilter<T, BEG, OQ, OVR, DBG>::timeUpdate() {
  ...
  for (j = n - 1 + BEG; j > BEG; --j) {
    for (i = BEG; i <= j; ++i) d(i) = U(i, j);
    for (i = BEG; i < n + BEG; ++i) {
      U(i, j) = A(i, j);
      for (k = BEG; k < j; ++k) U(i, j) += A(i, k) * d(k);
    }
  }

  d(BEG) = U(BEG, BEG);
  for (j = BEG; j < n + BEG; ++j) U(j, BEG) = A(j, BEG);

  // d(n+1:nn) = q
  // U(:,n+1:nn) = G
  for (i = BEG; i < nw + BEG; ++i) {
    d(i + n) = Q(i, i);
    for (j = BEG; j < n + BEG; ++j) U(j, i + n) = W(j, i);
  }
  ...
  for (j = n - 1 + BEG; j != (K_UINT_32)(BEG - 1); --j) {  // intended overflow if BEG==0
    ...
    for (k = BEG; k < j; ++k) {
      sigma = T(0.0);
      for (i = BEG; i < nn + BEG; ++i) sigma += U(k, i) * a(i);
      sigma *= dinv;
      for (i = BEG; i < nn + BEG; ++i) U(k, i) -= sigma * v(i);
      U(j, k) = sigma;
    }
  }
...
void EKFilter<T, BEG, OQ, OVR, DBG>::measureUpdate(T dz, T r) {
  ...
  // dz = dz - Hdx
  for (j = BEG; j < n + BEG; ++j) dz -= a(j) * _x(j);
  ...
  // dx = dx + K(dz - Hdx)
  dz *= gamma;
  for (j = BEG; j < n + BEG; ++j) _x(j) += d(j) * dz;
}
