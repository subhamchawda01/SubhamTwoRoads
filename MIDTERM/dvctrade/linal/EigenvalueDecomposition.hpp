/**
    \file EigenvalueDecomposition.hpp
...
class EigenvalueDecomposition {
  ...
  */
  ...
  void tred2(std::vector<double>& d, std::vector<double>& e) {
  ...
  void tql2(std::vector<double>& d, std::vector<double>& e) {
  ...
  void hqr2(std::vector<double>& d, std::vector<double>& e) {
      ...
      if (q == 0) {
        ...
        for (int i = n - 2; i >= 0; i--) {
          ...
          if (e[i] < 0.0) {
            ...
            t = std::max(fabs(H[i][n - 1]), fabs(H[i][n]));
            if ((eps * t) * t > 1) {
              for (int j = i; j <= n; j++) {
                H[j][n - 1] = H[j][n - 1] / t;
                H[j][n] = H[j][n] / t;
  ...
  */
 public:
  EigenvalueDecomposition(const Matrix& Arg, Matrix& V_, std::vector<double>& d, std::vector<double>& e) {
    ...
    for (int j = 0; (j < n) & issymmetric; j++) {
      for (int i = 0; (i < n) & issymmetric; i++) {
        issymmetric = Arg.get(i, j) == Arg.get(j, i);
      }
    }
