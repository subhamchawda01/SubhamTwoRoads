...
class SingularValueDecomposition {
  ...
  */
  SingularValueDecomposition(const Matrix& Arg, Matrix& U, Matrix& S, Matrix& V) {
      ...
      switch (kase) {
        ...
        case 3: {
          ...
          double scale = std::max(
              std::max(std::max(std::max(fabs(s[p - 1]), fabs(s[p - 2])), fabs(e[p - 2])), fabs(s[k])), fabs(e[k]));
    ...
    // set the values in the diagonal matrix S
    S.create(n, n);
    for (int i = 0; i < n; i++) {
      S(i, i) = s[i];
    }
  ...
  */
  std::vector<double> getSingularValues() { return s; }
  ...
  */
  ...
  @return     max(S)/min(S)
  */
  double cond() { return s[0] / s[std::min(m, n) - 1]; }
