...
class TrendVolBasedRegime : public IndicatorListener,
 ...
 protected:
  ...
  double dep_price_trend_brk_;
  double indep_price_trend_brk_;
  ...
  double max_ratio_dep_;
  double max_ratio_indep_;

  unsigned int pred_mode_;

  SimpleTrend* p_dep_indicator_;
  SimpleTrend* p_indep_indicator_;
  SimpleTrend* p_dep_indicator_brk_;
  SimpleTrend* p_indep_indicator_brk_;

 protected:
  TrendVolBasedRegime(DebugLogger& _dbglogger_, const Watch& _watch_, const std::string& concise_indicator_description_,
                      const SecurityMarketView& _dep_market_view_, const SecurityMarketView& _indep_market_view_,
                      double _fractional_seconds_, int t_volume_measure_seconds_, PriceType_t _price_type_);
 ...
 public:
  ...
  static TrendVolBasedRegime* GetUniqueInstance(DebugLogger& _dbglogger_, const Watch& _watch_,
  ...
  static std::string VarName() { return "TrendVolBasedRegime"; }

  void WhyNotReady();

  inline void SubscribeDataInterrupts(MarketUpdateManager& market_update_manager_) {
    if (p_dep_indicator_brk_ != NULL) {
      market_update_manager_.AddMarketDataInterruptedListener(p_dep_indicator_brk_);
    }
    if (p_indep_indicator_brk_ != NULL) {
      market_update_manager_.AddMarketDataInterruptedListener(p_indep_indicator_brk_);
    }
  }
 ...
 protected:
  ...
  /// From price changes, we compute the coefficient and corerlation through Single Linear Regression
  /// < lr_coeff_, lr_correlation_ > = GetSLRCoeffCorrelation ( dep_price_change_vector_, indep_price_change_vector_ )
  /// For TrendVolBasedRegime lr_coeff_/fabs(lr_correlation_) is sort of the lr_coeff_ as it would be if indep and dep
  /// are either perfectly correlated or
