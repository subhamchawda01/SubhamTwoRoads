/**
    ...
    \author: (c) Copyright Two Roads Technological Solutions Pvt Ltd 2011
     Address:
         ...
         +91 80 4190 3551
 */
#ifndef BASE_INDICATORS_OFFLINE_RETURNS_LRDB_H
#define BASE_INDICATORS_OFFLINE_RETURNS_LRDB_H
...
class OfflineReturnsLRDB {
 protected:
  ...
  static OfflineReturnsLRDB* uniqueinstance_;

  // functions
 protected:
  OfflineReturnsLRDB(DebugLogger& _dbglogger_, const Watch& _watch_, const std::string& _dep_shortcode_);

 public:
  static OfflineReturnsLRDB& GetUniqueInstance(DebugLogger& _dbglogger_, const Watch& _watch_,
                                               const std::string& _dep_shortcode_) {
    if (uniqueinstance_ == nullptr) {
      uniqueinstance_ = new OfflineReturnsLRDB(_dbglogger_, _watch_, _dep_shortcode_);
    }
 ...
 protected:
  ...
  unsigned int LRDBVariationCheck(std::string dep_indep_portfolio_code_, std::string tag_);
