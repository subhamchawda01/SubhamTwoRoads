/**
    \file ExecLogic/sgx_nifty_price_pair_aggressive_trading.hpp
    ...
    \author: (c) Copyright Two Roads Technological Solutions Pvt Ltd 2011
     Address:
         ...
         +91 80 4190 3551
 */
#ifndef BASE_EXECLOGIC_SGX_NIFTY_PRICE_PAIR_AGGRESSIVE_TRADING_H
#define BASE_EXECLOGIC_SGX_NIFTY_PRICE_PAIR_AGGRESSIVE_TRADING_H
...
class SGXNiftyPricePairAggressiveTrading : public virtual BaseTrading {
 public:
 ...
 protected:
 public:
  SGXNiftyPricePairAggressiveTrading(DebugLogger& _dbglogger_, const Watch& _watch_,
  ...
  ~SGXNiftyPricePairAggressiveTrading(){};  ///< destructor not made virtual ... please do so when making child classes

  static std::string StrategyName() {
    return "SGXNiftyPricePairAggressiveTrading";
  }  ///< used in trade_init to check which strategy is to be initialized

 protected:
  const SecurityMarketView& nse_nifty_market_view_;
  ...
  void OnMarketUpdate(const unsigned int security_id, const MarketUpdateInfo& market_update_info) {
    ...
    if (( security_id_ == dep_market_view_.security_id() ) || ( security_id_ == nse_nifty_market_view_.security_id()))
    {
      CallTradingLogic();
    }
  }
  ...
  void OnTradePrint(const unsigned int security_id, const TradePrintInfo& trade_print_info,
    ...
    if ((security_id == dep_market_view_.security_id() ) || ( security_id == nse_nifty_market_view_.security_id())) {
      CallTradingLogic();
    }
  }
...
#endif  // BASE_EXECLOGIC_SGX_NIFTY_PRICE_PAIR_AGGRESSIVE_TRADING_H
