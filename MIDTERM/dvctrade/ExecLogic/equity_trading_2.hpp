...
namespace HFSAT {
class EquityTrading2 : public virtual BaseTrading,
                       public VolumeRatioListener,
                       public IndicatorListener,
                       public MultBasePNLListener {
 protected:
 public:
  EquityTrading2(DebugLogger& _dbglogger_, const Watch& _watch_, const SecurityMarketView& _dep_market_view_,
                 SmartOrderManager& _order_manager_, MultBasePNL* _mult_base_pnl_, const std::string& _paramfilename_,
  ...
  ~EquityTrading2() {}
  static std::vector<std::string> LoadProductList();
  static std::string StrategyName() {
    return "EquityTrading2";
  ...
  SecurityMarketView* index_market_view_;
  SecurityNameIndexer& sec_name_indexer_;
  ...
  std::vector<OfflineReturnsRetLRDB*> retlrdb_vec_;
  ...
  std::vector<std::vector<double> > offline_ret_beta_;
  std::vector<double> stdevratio_with_index_;
  std::vector<int> sec_id_to_index_;
  std::vector<int> prod_index_to_sector_index_;
  std::vector<int> prod_map_pos_increment_;
  std::vector<int> ls_position_;
  std::vector<int> target_position_;
  std::vector<double> sector_returns_;
  ...
  double index_returns_;
  double total_index_risk_;
  ...
  std::vector<double> betas_with_sector_;
  double beta_adjusted_notional_risk_;
  double max_global_beta_adjusted_notional_risk_;
  double self_pos_projection_factor_;
  double max_notionl_risk_;
  int counter_;
  int counter2_ = 0;
 ...
 protected:
  ...
  void SetComputeReturns();
  void SetSectorIndex();
  void SetComputeTresholds();
 ...
 public:
  ...
  void MultOrderGetFlatTradingLogic(int _product_index_);
