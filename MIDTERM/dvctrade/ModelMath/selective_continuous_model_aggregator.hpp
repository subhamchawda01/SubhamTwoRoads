// =====================================================================================
//
//       Filename:  selective_continuous_model_aggregator.hpp
//
...
namespace HFSAT {
class SelectiveContinuousModelAggregator : public BaseModelMath,
 ...
 protected:
  ...
  unsigned int num_regimes_;
  std::vector<double> regime_cutoffs_;
  std::vector<double> regime_index_prob_;

 public:
  SelectiveContinuousModelAggregator(DebugLogger& _dbglogger_, const Watch& _watch_,
                                     const std::string& _model_file_name_, SecurityMarketView& _dep_market_view_,
                                     PriceType_t _dep_baseprice_type_, bool _is_returns_based_)
    ...
    sum_vars_vec_.clear();
    regime_index_prob_.clear();
    regime_cutoffs_.clear();
  ...
  virtual ~SelectiveContinuousModelAggregator() {}

  inline void SetNumRegimes(unsigned int regimes_no_) { num_regimes_ = regimes_no_; }

  inline void SetRegimesCutoff(std::vector<double> cutoff_vec_) {
    for (auto i = 0u; i < cutoff_vec_.size(); i++) {
      regime_cutoffs_.push_back(cutoff_vec_[i]);
    }
  }
  ...
  inline void OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
    if (_indicator_index_ == 0) {
      regime_index_prob_.clear();
      for (unsigned int j = 0; j < num_regimes_; j++) {
        if (j == 0) {
          if (_new_value_ <= regime_cutoffs_[j]) {
            regime_index_prob_.push_back(1);
          } else if (_new_value_ > regime_cutoffs_[j] && _new_value_ <= regime_cutoffs_[j + 1]) {
            regime_index_prob_.push_back((regime_cutoffs_[j + 1] - _new_value_) /
                                         (regime_cutoffs_[j + 1] - regime_cutoffs_[j]));
          } else {
            regime_index_prob_.push_back(0);
          }
        } else if (j == (num_regimes_ - 1)) {
          if (_new_value_ > regime_cutoffs_[j]) {
            regime_index_prob_.push_back(1);
          } else if (_new_value_ >= regime_cutoffs_[j - 1] && _new_value_ < regime_cutoffs_[j]) {
            regime_index_prob_.push_back((_new_value_ - regime_cutoffs_[j - 1]) /
                                         (regime_cutoffs_[j] - regime_cutoffs_[j - 1]));
          } else {
            regime_index_prob_.push_back(0);
          }
        } else {
          if (_new_value_ >= regime_cutoffs_[j - 1] && _new_value_ < regime_cutoffs_[j]) {
            regime_index_prob_.push_back((_new_value_ - regime_cutoffs_[j - 1]) /
                                         (regime_cutoffs_[j] - regime_cutoffs_[j - 1]));
          } else if (_new_value_ > regime_cutoffs_[j] && _new_value_ <= regime_cutoffs_[j + 1]) {
            regime_index_prob_.push_back((regime_cutoffs_[j + 1] - _new_value_) /
  ...
  void ShowIndicatorValues() {
    if (dbglogger_.CheckLoggingLevel(DBG_MODEL_ERROR)) {
      ...
      // dbglogger_ << " Current Regime: " << regime_index_prob_ << " of " <<
      // indicator_vec_[0]->concise_indicator_description()
      //           << DBGLOG_ENDL_FLUSH;
  ...
  void ShowIndicatorValuesToScreen() {
    ...
    std::cout << "\n=================================================================" << std::endl;
    // std::cout << " Current Regime: " << regime_index_prob_ << " of " <<
    // indicator_vec_[0]->concise_indicator_description()
    //           << std::endl;
    for (unsigned int j = 0; j < num_models_; j++) {
      std::cout << "MODEL: " << j << "\n";
 ...
 protected:
  ...
  void CalculateSumVars() {
    ...
    if (regime_index_prob_.empty()) {
      ...
      for (auto i = 0u; i < sum_vars_vec_.size(); i++) {
        if (regime_index_prob_[i] > 0) {
          sum_vars_ += sum_vars_vec_[i] * regime_index_prob_[i];
        }
