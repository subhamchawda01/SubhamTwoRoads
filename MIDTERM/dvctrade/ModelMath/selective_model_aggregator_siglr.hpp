...
namespace HFSAT {
class SelectiveModelAggregatorSiglr : public BaseModelMath,
 ...
 protected:
  ...
  int regime_index_;
  std::vector<std::vector<double>> ind_idx_to_alpha_vec_;
  std::vector<std::vector<double>> ind_idx_to_beta_vec_;
  std::vector<std::vector<unsigned int>> ind_idx_to_presence_vec_;

 public:
  SelectiveModelAggregatorSiglr(DebugLogger& _dbglogger_, const Watch& _watch_, const std::string& _model_file_name_,
      ...
      : BaseModelMath(_dbglogger_, _watch_, _model_file_name_),
        ...
        last_updated_baseprice_(0.0),
        last_propagated_target_price_(0.0),
        model_intercept_(0.0),
        sum_vars_(0.0),
        num_models_(0.0),
        regime_index_(0) {
    sum_vars_vec_.clear();
  ...
  virtual ~SelectiveModelAggregatorSiglr() {}
  ...
  inline void OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
    if (_indicator_index_ == 0) {
      ...
      if (!is_ready_) {
        ...
        for (unsigned j = 0; j < ind_idx_to_presence_vec_[_indicator_index_].size(); j++) {
          double this_ind_diff_ =
              ind_idx_to_beta_vec_[_indicator_index_][j] *
                  (1 / (1 + exp(-_new_value_ * ind_idx_to_alpha_vec_[_indicator_index_][j])) - 0.5) -
              ind_idx_to_beta_vec_[_indicator_index_][j] *
                  (1 / (1 + exp(-prev_value_vec_[_indicator_index_] * ind_idx_to_alpha_vec_[_indicator_index_][j])) -
                   0.5);
          ...
          sum_vars_vec_[ind_idx_to_presence_vec_[_indicator_index_][j]] += this_ind_diff_;
        }
  ...
  void AddIndicator(CommonIndicator* p_this_indicator_, const double& _this_alpha_, const double& _this_beta_,
    ...
    if (p_this_indicator_ != NULL) {
      // look if we are listening to this indicator already
      // ind_idx is the id for unique indicator
      int ind_idx_ = -1;
      ...
      // this is a new indicator across all the regimes
      if (ind_idx_ < 0) {
        ...
        ind_idx_to_alpha_vec_.push_back(std::vector<double>(1, _this_alpha_));
        ind_idx_to_beta_vec_.push_back(std::vector<double>(1, _this_beta_));
        ind_idx_to_presence_vec_.push_back(std::vector<unsigned int>(1, num_models_));

        p_this_indicator_->add_unweighted_indicator_listener(indicator_vec_.size(), this);
        indicator_vec_.push_back(p_this_indicator_);
      }
      // we have this already but not in the same regime
      else if (!VectorUtils::LinearSearchValue(ind_idx_to_presence_vec_[ind_idx_], num_models_)) {
        ind_idx_to_alpha_vec_[ind_idx_].push_back(_this_alpha_);
        ind_idx_to_beta_vec_[ind_idx_].push_back(_this_beta_);
        ind_idx_to_presence_vec_[ind_idx_].push_back(num_models_);
      } else {
  ...
  void ShowIndicatorValues() {
    if (dbglogger_.CheckLoggingLevel(DBG_MODEL_ERROR)) {
      ...
      for (unsigned int j = 0; j <= num_models_; j++) {
        ...
        for (auto i = 0u; i < indicator_vec_.size(); i++) {
          if (VectorUtils::LinearSearchValue(ind_idx_to_presence_vec_[i], j)) {
  ...
  void DumpIndicatorValues() {
    ...
    for (unsigned int j = 0; j < num_models_; j++) {
      ...
      for (auto i = 0u; i < indicator_vec_.size(); i++) {
        if (VectorUtils::LinearSearchValue(ind_idx_to_presence_vec_[i], j)) {
          dbglogger_ << " value: "
                     << ind_idx_to_presence_vec_[i][j] * (prev_value_vec_[i] / dep_market_view_.min_price_increment())
