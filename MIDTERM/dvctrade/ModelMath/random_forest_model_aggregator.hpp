...
+91 80 4190 3551
*/
#ifndef BASE_MODELMATH_RANDOM_FOREST_MODEL_AGGREGATOR_H
#define BASE_MODELMATH_RANDOM_FOREST_MODEL_AGGREGATOR_H
...
class RandomForestModelAggregator : public BaseModelMath {
 protected:
  ...
  std::vector<unsigned> last_node_index_vec_;
 ...
 public:
  RandomForestModelAggregator(DebugLogger& _dbglogger_, const Watch& _watch_, const std::string& _model_filename_,
      ...
      : BaseModelMath(_dbglogger_, _watch_, _model_filename_),
        ...
        indicator_based_nodes_vec_(),
        indicator_based_tree_vec_(),
        num_trees_(0),
        normalization_factor_(0.0),
        tree_vec_(),
        tree_predictions_(),
        dep_baseprice_type_(_dep_baseprice_type_),
        is_returns_based_(false),
  ...
  virtual ~RandomForestModelAggregator() {}
  ...
  inline void OnIndicatorUpdate(const unsigned int& _indicator_index_, const double& _new_value_) {
    if (!is_ready_) {
      ...
      // following is not needed in random forest models atleat in this form
      /*
               if ( fabs ( _new_value_ - prev_value_vec_ [ _indicator_index_ ] ) > 10 *
         dep_market_view_.min_price_increment ( ) )
               ...
               << " ntgt: " << ( dep_market_view_.price_from_type ( dep_baseprice_type_ ) + sum_vars_ + ( _new_value_ -
         prev_value_vec_ [ _indicator_index_ ] ) )
...
*/
      /*                for ( auto i = 0u; i < indicator_based_nodes_vec_[_indicator_index_].size(); i++ )
                                                                              {
                                                                              if ( _new_value_ <
         indicator_based_nodes_vec_ [ _indicator_index_ ][i]->split_indicator_value_ )
                                                                              {
                                                                              indicator_based_nodes_vec_ [
         _indicator_index_ ][i]->node_answer_ = true;
                                                                              }
                                                                              else
                                                                              {
                                                                              indicator_based_nodes_vec_ [
         _indicator_index_ ][i]->node_answer_ = false;

                                                                              }
                                                                              } */

      unsigned start_node_index_ = 0;

      std::vector<TreeLine*> node_vec_ = indicator_based_nodes_vec_[_indicator_index_];
      unsigned last_node_index_ = node_vec_.size() - 1;
      bool found_ = false;
      while (!found_) {
        unsigned mid_index_ = (last_node_index_ + start_node_index_) / 2;
        if (last_node_index_ == start_node_index_ || mid_index_ == start_node_index_) {
          found_ = true;
          start_node_index_ = last_node_index_;
        }
        if (_new_value_ <= node_vec_[start_node_index_]->split_indicator_value_) {
          found_ = true;
          last_node_index_ = start_node_index_;
        } else if (_new_value_ >= node_vec_[last_node_index_]->split_indicator_value_) {
          found_ = true;
          start_node_index_ = std::max(last_node_index_ + 1, unsigned(node_vec_.size() - 1));
        } else if (_new_value_ == node_vec_[mid_index_]->split_indicator_value_) {
          start_node_index_ = mid_index_;
          while (mid_index_ > 0 && (_new_value_ == node_vec_[mid_index_]->split_indicator_value_)) {
            node_vec_[mid_index_]->node_answer_ = true;
            mid_index_--;
          }
          found_ = true;
        } else if (_new_value_ < node_vec_[mid_index_]->split_indicator_value_) {
          last_node_index_ = mid_index_;
        } else {
          start_node_index_ = mid_index_;
        }
      }

      if (start_node_index_ == last_node_index_vec_[_indicator_index_]) {
        // no need for computation;
      } else {
        if (start_node_index_ < last_node_index_vec_[_indicator_index_]) {
          for (unsigned i = start_node_index_; i < last_node_index_vec_[_indicator_index_]; i++) {
            node_vec_[i]->node_answer_ = true;
          }
        } else {
          for (unsigned i = last_node_index_vec_[_indicator_index_]; i < start_node_index_; i++) {
            node_vec_[i]->node_answer_ = false;
          }
        }
        last_node_index_vec_[_indicator_index_] = start_node_index_;
        prev_value_vec_[_indicator_index_] = _new_value_;
        for (unsigned int i = 1; i < indicator_based_tree_vec_[_indicator_index_].size(); i++) {
          int current_index_ = 0;
          TreeLine* this_line_ = tree_vec_[indicator_based_tree_vec_[_indicator_index_][i]][current_index_];
          while (!this_line_->is_leaf_) {
            ...
            this_line_ = tree_vec_[indicator_based_tree_vec_[_indicator_index_][i]][current_index_];
          }
          sum_vars_ = sum_vars_ - tree_predictions_[indicator_based_tree_vec_[_indicator_index_][i]];
          tree_predictions_[indicator_based_tree_vec_[_indicator_index_][i]] = this_line_->prediction_;
          sum_vars_ = sum_vars_ + tree_predictions_[indicator_based_tree_vec_[_indicator_index_][i]];
        }
      }
  ...
  void AddTreeLine(const int tree_index_, const int tree_line_index_, const int t_indicator_index_,
    ...
    if (t_indicator_index_ >= 0) {
      ...
      VectorUtils::UniqueVectorAdd(indicator_based_tree_vec_[t_indicator_index_], tree_index_);
    }
  }
  ...
  void AddIndicator(CommonIndicator* p_this_indicator_, bool _readiness_required_) {
    if (p_this_indicator_ != NULL) {
      ...
      last_node_index_vec_.push_back(0);
    } else {
  ...
  void FinishCreation() {
    ...
    for (auto i = 0u; i < indicator_based_nodes_vec_.size(); i++) {
      std::sort(indicator_based_nodes_vec_[i].begin(), indicator_based_nodes_vec_[i].end(), TreeNodeComparision);
    }
  }

  void InterpretModelParameters(const std::vector<const char*> _tokens_) {
    num_trees_ = atoi(_tokens_[1]);
    normalization_factor_ = 1.0 / num_trees_;
 ...
 protected:
  ...
  inline void CalcAndPropagate() {
    if (is_ready_) {
      ...
      double new_target_bias_ = sum_vars_ * normalization_factor_;
      if (use_mid_price_base_) {
...
#endif  // BASE_MODELMATH_RANDOM_FOREST_MODEL_AGGREGATOR_H
