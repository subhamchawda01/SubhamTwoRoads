/**
        \file ModelMath/siglr_model_aggregator.hpp
...
+91 80 4190 3551
*/
#ifndef BASE_MODELMATH_SIGLR_MODEL_AGGREGATOR_H
#define BASE_MODELMATH_SIGLR_MODEL_AGGREGATOR_H
...
namespace HFSAT {
class SIGLRModelAggregator : public BaseModelMath {
 protected:
 ...
 public:
  SIGLRModelAggregator(DebugLogger& _dbglogger_, const Watch& _watch_, const std::string& _model_filename_,
                       SecurityMarketView& _dep_market_view_, PriceType_t _dep_baseprice_type_, bool _is_returns_based_)
  ...
  void ShowIndicatorValues() {
    if (dbglogger_.CheckLoggingLevel(DBG_MODEL_ERROR)) {
      ...
      for (auto i = 0u; i < indicator_vec_.size(); i++) {
        dbglogger_ << " values: " << (prev_value_vec_[i] / dep_market_view_.min_price_increment()) << " of "
                   << indicator_vec_[i]->concise_indicator_description() << DBGLOG_ENDL_FLUSH;
      }
...
#endif  // BASE_MODELMATH_SIGLR_MODEL_AGGREGATOR_H
