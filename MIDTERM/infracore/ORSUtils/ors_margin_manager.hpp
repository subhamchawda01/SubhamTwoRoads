...
class ORSMarginManager {
 private:
  ...
  ORSMarginManager(HFSAT::DebugLogger& logger)
      : dbglogger_(logger),
 ...
 public:
  static ORSMarginManager& GetUniqueInstance(HFSAT::DebugLogger& logger) {
    static ORSMarginManager unique_instance(logger);
  ...
  void SetGrossMarginCheck(double gross_margin_check) {
    ...
    DBGLOG_CLASS_FUNC_LINE_INFO << "GROSS MARGIN CHECK SETUP AT : " << gross_margin_check_
                                << " MARGIN_CHECK_ENFORCED ? " << IsMarginChecksInPlace() << DBGLOG_ENDL_DUMP;
    alert_sent_ = false;
  }
  ...
  void SetNetMarginCheck(double net_margin_check) {
    ...
    DBGLOG_CLASS_FUNC_LINE_INFO << "GROSS MARGIN CHECK SETUP AT : " << net_margin_check_ << " MARGIN_CHECK_ENFORCED ? "
                                << IsMarginChecksInPlace() << DBGLOG_ENDL_DUMP;
    alert_sent_ = false;
  }
  ...
  void OnOrderExec(int32_t security_id) {
    ...
    if (false == alert_sent_) {
      if ((gross_margin_used_ >= (gross_margin_check_ * 0.9)) || (std::fabs(net_margin_used_) >= (net_margin_check_ * 0.9))) {
        HFSAT::Email e;
        e.setSubject("NSE-MARGIN ALERT");
        e.addRecepient(
      ...
      if ((gross_margin_used_ <= (gross_margin_check_ * 0.6)) && (std::fabs(net_margin_used_) <= (net_margin_check_ * 0.6))) {
        alert_sent_ = false;
      }
    }
  }
