...
void DumpCombinedWriterProfileStats() {
  std::string summary = HFSAT::CpucycleProfiler::GetUniqueInstance().GetCpucycleSummaryStringUsecs();
...
void sighandler(int signum) {
  ...
  HFSAT::RuntimeProfiler::ResetUniqueInstance(HFSAT::ProfilerType::kCOMBINEDSHMWRITER);
...
int main(int argc, char** argv) {
  ...
  HFSAT::RuntimeProfiler::GetUniqueInstance(HFSAT::ProfilerType::kCOMBINEDSHMWRITER);
...
//   HFSAT::CpucycleProfiler::GetUniqueInstance().SetTag(4, "Total Time");
  ...
  for (auto exch_ : exch_processing_list_) {
    ...
    if (exch_ == "NTP") {
      ...
      // Explicitly set Combined Control as Primary Source -> updated to Secondary Source
