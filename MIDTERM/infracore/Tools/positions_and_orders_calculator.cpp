...
void PositionAndOrderCalculator::OrderExecuted(
  ...
  if (r_buysell_ == kTradeTypeBuy) {
    position_mgr_.AddBuyTrade(_security_id_, t_server_assigned_client_id_, size_now_executed, false);
    position_mgr_.DecBidSize(_security_id_, size_now_executed, 0, 0);
  } else if (r_buysell_ == kTradeTypeSell) {
    position_mgr_.AddSellTrade(_security_id_, t_server_assigned_client_id_, size_now_executed, false);
