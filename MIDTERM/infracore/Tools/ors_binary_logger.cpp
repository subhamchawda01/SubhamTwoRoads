...
class ORSBinLogger {
 public:
  ...
  void processMsgRecvd() {
    while (true) {
      int num_bytes = socket_->ReadN(sizeof(HFSAT::GenericORSReplyStructLive), (void*)&generic_ors_reply_struct_);

      // This is for the ors data coming from a different server
      generic_ors_reply_struct_.time_set_by_server_ = HFSAT::GetTimeOfDay();
 ...
 private:
  ...
  std::string ip_;
  int port_;
  HFSAT::GenericORSReplyStructLive generic_ors_reply_struct_;
