...
void termination_handler(int signal_num_) { exit(0); }
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    int received_size_ = abc_.ReadN(sizeof(HFSAT::ttime_t), &time_);
    ...
    if (no_packets_recv_ == 10 * 300) {  // 10 packets per sec, 5 mins.
