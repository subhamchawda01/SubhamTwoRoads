...
NSEEngine::NSEEngine(HFSAT::ORS::Settings &settings, HFSAT::DebugLogger &dbglogger, std::string output_log_dir,
    ...
    : HFSAT::ORS::BaseEngine(settings, dbglogger),
      ...
      last_send_time_(),
      nse_session_socket_(new TcpClientSocketWithLogging(
          true, pWriter, pReader, output_log_dir + "/audit." + HFSAT::DateTime::GetCurrentIsoDateLocalAsString())),
      nse_gr_socket_(new TCPClientSocket()),
      read_offset_(-1),
  ...
  std::string username = settings.getValue("UserName");
  std::string password = settings.getValue("Password");
  std::string new_password = settings.getValue("NewPassword", "        ");
  ...
  if (HFSAT::DaysBetweenDates(last_pw_change_date, HFSAT::DateTime::GetCurrentIsoDateUTC()) >= 7) {
    ...
    dbglogger << "Changing Password to " << new_password << "\n";
    dbglogger.DumpCurrentBuffer();

    // Updating to latest value to write back to config file
    settings.setValue("PasswordChangeDate", HFSAT::DateTime::GetCurrentIsoDateLocalAsString());
    settings.setValue("Password", new_password);
  ...
  nse_msgs_handler_->new_order_->SetPreLoadedOrderEntryRequestFields(user_id_, broker_id_.c_str(), nnf_, algo_id_, pan_.c_str(), branch_id_);
  nse_msgs_handler_->cancel_order_->SetPreLoadedOrderCancelRequestFields(user_id_, broker_id_.c_str(), nnf_, algo_id_, pan_.c_str(), branch_id_);
  nse_msgs_handler_->modify_order_->SetPreLoadedOrderModifyRequestFields(user_id_, broker_id_.c_str(), nnf_, algo_id_, pan_.c_str(), branch_id_);
  nse_msgs_handler_->spread_new_order_.SetPreLoadedOrderEntryRequestFields(user_id_, broker_id_.c_str(), nnf_, branch_id_);
...
void NSEEngine::Connect() {
  ...
  for (auto ip : nse_gateway_ip_list_) {
    ...
    route_cmd = "sudo ip route add " + nse_gateway_ip_ + "/32 via " + tap_gateway_ + " dev " + tap_interface_ + " src " + tap_ip_ ;
    system(route_cmd.c_str());
...
void NSEEngine::GRRequest() {
  ...
  int32_t write_length =
      nse_gr_socket_->WriteN(nse_msgs_handler_->gr_request_->GetGRRequestMsgLength(),
  ...
  int32_t read_length =
        nse_gr_socket_->ReadN(1024, buffer);
...
void NSEEngine::BoxLoginRequest() {
  ...
  route_cmd = "sudo ip route add " + trading_ip_ + "/32 via " + tap_gateway_ + " dev " + tap_interface_ + " src " + tap_ip_ ;
  system(route_cmd.c_str());
  ...
  nse_gr_socket_->DisConnect();
  nse_session_socket_->Connect(trading_ip_, trading_port_);

  next_message_sequnece_++;
  nse_msgs_handler_->boxlogin_request_->SetDynamicBoxLoginRequestsFieldsAndUpdateChecksum(next_message_sequnece_, session_key_);

  last_send_time_ = time(NULL);
  int32_t write_length =
      nse_session_socket_->WriteN(nse_msgs_handler_->boxlogin_request_->GetBoxLoginRequestMsgLength(),
                                  (void *)nse_msgs_handler_->boxlogin_request_->GetBoxLoginRequestBuffer());
...
void NSEEngine::Login() {
  ...
  last_send_time_ = time(NULL);
  int32_t write_length =
      nse_session_socket_->WriteN(nse_msgs_handler_->logon_request_->GetLogonRequestMsgLength(),
                                  (void *)nse_msgs_handler_->logon_request_->GetLogonRequestBuffer());

  std::cout << " SENT : " << write_length << " " << std::endl ;
  printHexString(nse_msgs_handler_->logon_request_->GetLogonRequestBuffer(), nse_msgs_handler_->logon_request_->GetLogonRequestMsgLength());
...
//  IsOnloadMsgWarmSuppoted(nse_session_socket_->socket_file_descriptor());
...
void NSEEngine::Logout() {
  ...
  int32_t write_length = nse_session_socket_->WriteN(
      NSE_LOGOFF_REQUEST_LENGTH, (void *)nse_msgs_handler_->logout_request_.GetLogoffRequestBuffer());
...
void NSEEngine::SendSystemInformationRequest() {
  ...
  int32_t write_length =
      nse_session_socket_->WriteN(nse_msgs_handler_->system_info_request_->GetSystemInfoMsgLength(),
                                  (void *)nse_msgs_handler_->system_info_request_->GetSystemInfoRequestBuffer());
...
void NSEEngine::SendUpdateLocalDBRequest(st_market_status &st_mkt_status, st_ex_market_status &st_ex_mkt_status,
  ...
  int32_t write_length = nse_session_socket_->WriteN(
      nse_msgs_handler_->update_local_db_request_->GetUpdateLocalDatabaseRequestLength(),
      (void *)nse_msgs_handler_->update_local_db_request_->GetUpdateLocalDatabaseRequestBuffer());
...
inline void NSEEngine::SendOrder(HFSAT::ORS::Order *order, InstrumentDesc *inst_desc) {
  ...
  nse_msgs_handler_->new_order_->SetDynamicOrderEntryRequestFields(
      next_message_sequnece_, buy_sell, order->size_remaining_, order->size_disclosed_, price,
      order->server_assigned_order_sequence_, order->is_ioc, inst_desc, is_mkt_order_, is_pre_open_);

  int32_t write_length =
      nse_session_socket_->WriteN(nse_msgs_handler_->new_order_->GetOrderEntryMsgLength(),
  ...
  printHexString((char *)nse_msgs_handler_->new_order_->GetOrderEntryRequestBuffer(),
                 nse_msgs_handler_->new_order_->GetOrderEntryMsgLength());
#endif

  if (write_length < (int32_t)nse_msgs_handler_->new_order_->GetOrderEntryMsgLength()) {
    DBGLOG_CLASS_FUNC_LINE_FATAL << "FAILD TO WRITE COMPLETE MESSAGE TO EXCHANGE SOCKET" << DBGLOG_ENDL_FLUSH;
    DBGLOG_DUMP;
    exit(-1);
  }
...
inline void NSEEngine::CancelOrder(HFSAT::ORS::Order *order, InstrumentDesc *inst_desc) {
  ...
  nse_msgs_handler_->cancel_order_->SetDynamicOrderCancelRequestFields(
      next_message_sequnece_, (int64_t)order->exch_assigned_seq_, order->server_assigned_order_sequence_,
      order->entry_dt_, order->last_mod_dt_, buy_sell, order->size_remaining_, order->size_disclosed_, price,
      inst_desc, order->last_activity_reference_, is_pre_open_);

  int32_t write_length =
      nse_session_socket_->WriteN(nse_msgs_handler_->cancel_order_->GetOrderCancelMsgLength(),
                                  (void *)nse_msgs_handler_->cancel_order_->GetOrderCancelRequestBuffer());

#if NSE_DEBUG_INFO_CM
  std::cout << "Cancel Order : " << order->server_assigned_order_sequence_ << std::endl;
  printHexString((char *)nse_msgs_handler_->cancel_order_->GetOrderCancelRequestBuffer(),
                 nse_msgs_handler_->cancel_order_->GetOrderCancelMsgLength());
#endif

  if (write_length < (int32_t)nse_msgs_handler_->cancel_order_->GetOrderCancelMsgLength()) {
    DBGLOG_CLASS_FUNC_LINE_FATAL << "FAILD TO WRITE COMPLETE MESSAGE TO EXCHANGE SOCKET" << DBGLOG_ENDL_FLUSH;
    DBGLOG_DUMP;
    exit(-1);
  }
...
inline void NSEEngine::OptModifyOrder(HFSAT::ORS::Order *order, int32_t inst_id, ORS::Order *orig_order) {
  ...
  std::cout << "sizeof(OrderPriceModifyRequest):" << sizeof(OrderPriceModifyRequest) << std::endl;
  printHexString(pr_modify_order_.GetOrderPriceModifyRequestBuffer(), sizeof(OrderPriceModifyRequest));
#endif
  //-------------------Price Modification-----------------------

  pr_modify_order_.SetDynamicOrderEntryRequestFields(next_message_sequnece_, (int64_t)order->exch_assigned_seq_,
                                                     order->server_assigned_order_sequence_, order->last_mod_dt_,
                                                     buy_sell, order->size_remaining_, order->last_activity_reference_, 																	  price, inst_id);
  int32_t write_length = nse_session_socket_->WriteN(sizeof(OrderPriceModifyRequest),
                                                     (void *)pr_modify_order_.GetOrderPriceModifyRequestBuffer());

  if (write_length < (int32_t)sizeof(OrderPriceModifyRequest)) {
    DBGLOG_CLASS_FUNC_LINE_FATAL << "FAILD TO WRITE COMPLETE MESSAGE TO EXCHANGE SOCKET" << DBGLOG_ENDL_FLUSH;
    DBGLOG_DUMP;
    exit(-1);
  }
...
void NSEEngine::ModifyOrder(HFSAT::ORS::Order *order, InstrumentDesc *inst_desc, ORS::Order *orig_order) {
  ...
  nse_msgs_handler_->modify_order_->SetDynamicOrderModifyRequestFields(
      next_message_sequnece_, (int64_t)order->exch_assigned_seq_, order->server_assigned_order_sequence_,
      order->entry_dt_, order->last_mod_dt_, buy_sell, order->size_remaining_, order->size_disclosed_, price,
      order->size_executed_, inst_desc, order->last_activity_reference_, is_pre_open_);

  int32_t write_length =
      nse_session_socket_->WriteN(nse_msgs_handler_->modify_order_->GetOrderModifyMsgLength(),
                                  (void *)nse_msgs_handler_->modify_order_->GetOrderModifyRequestBuffer());


#if NSE_DEBUG_INFO_CM
  std::cout << "Cancel Order : " << order->server_assigned_order_sequence_ << std::endl;
  printHexString((char *)nse_msgs_handler_->modify_order_->GetOrderModifyRequestBuffer(),
                 nse_msgs_handler_->modify_order_->GetOrderModifyMsgLength());
#endif

  if (write_length < (int32_t)nse_msgs_handler_->modify_order_->GetOrderModifyMsgLength()) {
    DBGLOG_CLASS_FUNC_LINE_FATAL << "FAILD TO WRITE COMPLETE MESSAGE TO EXCHANGE SOCKET" << DBGLOG_ENDL_FLUSH;
    DBGLOG_DUMP;
    exit(-1);
  }
...
void NSEEngine::AdjustSymbol(const char *) {}
...
void NSEEngine::ProcessFakeSend(HFSAT::ORS::Order *ord, ORQType_t type) {
  ...
  nse_msgs_handler_->new_order_->SetDynamicOrderEntryRequestFields(0, 0, 0, 0, 0, 0, false, inst_desc, false, false);
  // TODO: CM exchange is disconnecting us on sending these orders with seq no set to 0: Abhishek
//  if (is_msg_warm_supported_) {
//    send(nse_session_socket_->socket_file_descriptor(),
//         (void *)nse_msgs_handler_->new_order_->GetOrderEntryRequestBuffer(),
//         nse_msgs_handler_->new_order_->GetOrderEntryMsgLength(), ONLOAD_MSG_WARM);
//  }
}
...
void NSEEngine::SendHeartbeatReply() {
  ...
  int write_length = nse_session_socket_->WriteN(NSE_HEARTBEAT_LENGTH,
                                                 (void *)nse_msgs_handler_->heartbeat_request_.GetHeartbeatBuffer());

#if NSE_DEBUG_INFO
  std::cout << "Sending HeartBeat: \n";
  printHexString(heartbeat_request_.GetHeartbeatBuffer(), NSE_HEARTBEAT_LENGTH);
#endif
...
//#if NSE_DEBUG_INFO
  ...
  printHexString(msg_ptr - (NSE_PACKET_REQUEST_LENGTH + NSE_RESPONSE_MESSAGE_HEADER_LENGTH), NSE_BoxLogin_REQUEST_LENGTH);
//#endif
...
void NSEEngine::ProcessLogonFailure(const char *msg_ptr, int16_t t_error_code) {
  ...
  int32_t error_msg_offset = nse_msgs_handler_->logon_response_->GetLogonErrorMsgOffset();
  int32_t error_msg_length = nse_msgs_handler_->logon_response_->GetLogonErrorMsgLength();
  // Prints the login error msg
  for (int i = 0; i < error_msg_length; i++) {
    dbglogger_ << msg_ptr[error_msg_offset + i];
  }
  dbglogger_ << "\n";
...
uint32_t NSEEngine::ProcessExchangeResponse(char *nse_msg_buffer, const uint32_t &msg_length) {
  ...
  while (length_to_be_processed > 0) {
    ...
    switch (processed_response_header_->transaction_code) {
      ...
      case GR_REQUEST_OUT: {
        DBGLOG_CLASS_FUNC_LINE_INFO << "ERROR CODE : " << error_code << " TC : " << processed_response_header_->transaction_code << DBGLOG_ENDL_FLUSH;                     
      ...
      case ORDER_ERROR: {
        DBGLOG_CLASS_FUNC_LINE_INFO << "Received Order Reject : Reason : " << processed_response_header_->error_code
                                    << "\n" << DBGLOG_ENDL_FLUSH;
        ProcessOrderRejectionNonTR(msg_ptr, error_code);
      } break;
      ...
      case BATCH_ORDER_CANCEL: {
		...
		p_engine_listener_->OnBatchCxlAlert(user_id_);
	}
...
inline int32_t NSEEngine::onInputAvailable(int32_t _socket_) {
  ...
  if (nse_sec_def_.IsSpreadExchSymbol(exchange_symbol)) {  // spread symbol added
    ...
    if (std::string("INVALID") == data_symbol_1 || std::string("INVALID") == data_symbol_2) {
      DBGLOG_CLASS_FUNC_LINE_FATAL << "FAILED TO ADD SYMBOL STRING AS DATASYMBOL FOR ONE OF THE TWO/BOTH INTERNAL "
                                      "SYMBOL WAS INVALID DATA SYMBOL1 : " << data_symbol_1
                                   << " DATA SYMBOL2 : " << data_symbol_2;
      DBGLOG_DUMP;
    ...
    int32_t strike_price =
        (int32_t)(ref_data_loader.GetNSERefData(nse_segment_type_)[token].strike_price * price_multiplier_);
...
void NSEEngine::CancelOrder(ORS::Order *order) {
  CancelOrder(order, &container_.inst_desc_[order->security_id_]);
}
  ...
  try {
    ...
    if ((tokens_.size() >= 2) && (strcmp(tokens_[0], "CONTROLCOMMAND") == 0)) {
      if (strcmp(tokens_[2], "SwitchToPostMarketSetting") == 0) {
        ...
        is_mkt_order_ = false ;
        is_pre_open_ = false ;
      }else if (strcmp(tokens_[2], "BroadcastForceCancelSAOS") == 0) { 
        if( tokens_.size() < 4 ){
          DBGLOG_CLASS_FUNC_LINE_ERROR << "NOT ENOUGH ARGS PROVIDED, EXPECTED 4 ARGUMENTS, PROVIDED :" << tokens_.size() << DBGLOG_ENDL_FLUSH ;
        }else{
          DBGLOG_CLASS_FUNC_LINE_INFO << "BroadcastFroceCancelSAOS  Called" << DBGLOG_ENDL_FLUSH;
          p_engine_listener_->ForceCancelBroadcastSAOS(tokens_[3]);
          DBGLOG_CLASS_FUNC_LINE_INFO << "BroadcastFroceCancelSAOS  End" << DBGLOG_ENDL_FLUSH;
        }

      }else if (strcmp(tokens_[2], "BroadcastForceTradeSAOS") == 0) {
        if( tokens_.size() < 4 ){
          DBGLOG_CLASS_FUNC_LINE_ERROR << "NOT ENOUGH ARGS PROVIDED, EXPECTED 4 ARGUMENTS, PROVIDED :" << tokens_.size() << DBGLOG_ENDL_FLUSH ;
        }else{
          DBGLOG_CLASS_FUNC_LINE_INFO << "BroadcastFroceTradeSAOS  Called" << DBGLOG_ENDL_FLUSH;
          p_engine_listener_->ForceTradeBroadcastSAOS(tokens_[3]);
          DBGLOG_CLASS_FUNC_LINE_INFO << "BroadcastFroceTradeSAOS  End" << DBGLOG_ENDL_FLUSH;
        }

      } else if (strcmp(tokens_[2], "BroadcastForceRejectSAOS") == 0) {
        if( tokens_.size() < 4 ){
          DBGLOG_CLASS_FUNC_LINE_ERROR << "NOT ENOUGH ARGS PROVIDED, EXPECTED 4 ARGUMENTS, PROVIDED :" << tokens_.size() << DBGLOG_ENDL_FLUSH ;
        }else{
          DBGLOG_CLASS_FUNC_LINE_INFO << "BroadcastFroceRejectSAOS  Called" << DBGLOG_ENDL_FLUSH;
          p_engine_listener_->ForceRejectBroadcastSAOS(tokens_[3]);
          DBGLOG_CLASS_FUNC_LINE_INFO << "BroadcastFroceRejectSAOS  End" << DBGLOG_ENDL_FLUSH;
        }
