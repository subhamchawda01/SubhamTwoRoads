// =====================================================================================
//
//       Filename:  SystemInformationCashMarket.hpp
//
//    Description:
//
//        Version:  1.0
...
/*
 * Defines sysinfo request semantics for cash market
 * Extends generic  NSESystemInfoRequest  template for NSE trimmed structure.
 */

class NSESystemInfoRequestCashMarket : public NSESystemInfoRequest {
 public:
  void InitializeStaticFields() {
    //================================ @ Packet

    // Total Length Of The Packet To Be Sent
    SetPacketLength(NSE_CM_SYSTEMINFO_REQUEST_LENGTH);
    ...
    *((int16_t*)(msg_ptr + NSE_REQUEST_MESSAGE_HEADER_MESSAGELENGTH_OFFSET)) =
        hton16(NSE_CM_SYSTEMINFO_REQUEST_LENGTH - NSE_PACKET_REQUEST_LENGTH);

    //================================= @ SystemInfoRequest
  }

  NSESystemInfoRequestCashMarket() {
    system_info_request_buffer_ = (char*)calloc(NSE_CM_SYSTEMINFO_REQUEST_LENGTH, sizeof(char));
 ...
 public:
  inline void SetDynamicSystemInfoRequestFieldsAndUpdateChecksum(int32_t const& packet_sequence_number) {
    ...
    HFSAT::MD5::MD5((unsigned char*)(msg_ptr + NSE_REQUEST_MESSAGE_HEADER_TRANSACTION_CODE_OFFSET),
                    NSE_CM_SYSTEMINFO_REQUEST_LENGTH - NSE_PACKET_REQUEST_LENGTH,
                    (unsigned int*)(msg_ptr + NSE_PACKET_CHECKSUM_OFFSET));
  }
  int32_t GetSystemInfoMsgLength() { return NSE_CM_SYSTEMINFO_REQUEST_LENGTH; }
};
}
}
