...
//        hton16(1);
 ...
 public:
  ...
  inline void SetPan(char const *pan) {
    memcpy((void *)(msg_ptr + NSE_PACKET_REQUEST_LENGTH + NSE_CM_ORDER_MODCXL_REQUEST_PAN_OFFSET ), pan, NSE_CM_ORDER_MODCXL_REQUEST_PAN_LENGTH); 
  }
  ...
  inline void SetLastActivityReference(int64_t &last_activity_ref) {
    *((int64_t *)(msg_ptr + NSE_PACKET_REQUEST_LENGTH + NSE_CM_ORDER_MODCXL_REQUEST_LASTACTIVITYREFERENCE_OFFSET)) = hton64(last_activity_ref);
  }

  inline void AddPreOpen() final {
    *((int16_t *)(msg_ptr + NSE_PACKET_REQUEST_LENGTH + NSE_CM_ORDER_MODCXL_REQUEST_STORDERFLAGS_STRUCT_OFFSET)) |=
        hton16((int16_t)(1 << 3));  // ioc order
  }

  inline void SetAlgoId(int32_t const & algo_id){
    *((int32_t *)(msg_ptr + NSE_PACKET_REQUEST_LENGTH + NSE_CM_ORDER_MODCXL_REQUEST_ALGOID_OFFSET)) = hton32(algo_id);
  }

  void SetDynamicOrderCancelRequestFields(int32_t const &packet_sequence_number, int64_t const &order_num,
                                          int32_t const &saos, int32_t const &entry_date, int32_t const &last_date,
                                          int16_t const &buy_sell, int32_t const &volume,
                                          int32_t const &disclosed_volume, int32_t const &price, InstrumentDesc *inst_desc, 
...
class OrderModifyRequestCashMarket : public OrderModifyRequest {
 ...
 public:
  ...
  // For broker’s own order, this field should be set to the broker code.
  inline void SetAccountNumber(char const *broker_id) {
    std::cout << "setacc " << broker_id << std::endl;
    memcpy((void *)(msg_ptr + NSE_PACKET_REQUEST_LENGTH + NSE_CM_ORDER_MODCXL_REQUEST_ACCOUNTNUMBER_OFFSET),
           (void *)broker_id, NSE_CM_ORDER_MODCXL_REQUEST_BROKERID_LENGTH);
  }
  ...
  inline void SetLastActivityReference(int64_t &last_activity_ref) {
    *((int32_t *)(msg_ptr + NSE_PACKET_REQUEST_LENGTH + NSE_CM_ORDER_MODCXL_REQUEST_LASTACTIVITYREFERENCE_OFFSET)) = hton64(last_activity_ref);
  }
 ...
 public:
  ...
  void SetDynamicOrderModifyRequestFields(int32_t const &packet_sequence_number, int64_t const &order_num, int32_t saos,
                                          int32_t const &entry_date, int32_t const &last_date, int16_t const &buy_sell,
                                          int32_t const &volume, int32_t const &disclosed_volume, int32_t const &price,
                                          int32_t const &traded_volume, InstrumentDesc *inst_desc, int64_t last_activity_reference_,
