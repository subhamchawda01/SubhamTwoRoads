// =====================================================================================
//
//       Filename:  MessageDownloadRequest.hpp
//
//    Description:
//
//        Version:  1.0
//        Created:  06/29/2015 10:21:08 PM
//       Revision:  none
//       Compiler:  g++
//
...
#define NSE_MESSAGEDOWNLOAD_REQUEST_SEQUENCENUMBER_OFFSET \
  (NSE_REQUEST_START_OFFSET + NSE_REQUEST_MESSAGE_HEADER_LENGTH + NSE_PACKET_REQUEST_LENGTH)
#define NSE_MESSAGEDOWNLOAD_REQUEST_SEQUENCENUMBER_LENGTH sizeof(double)

#define NSE_MESSAGEDOWNLOAD_REQUEST_LENGTH \
  (NSE_PACKET_REQUEST_LENGTH + NSE_REQUEST_MESSAGE_HEADER_LENGTH + NSE_MESSAGEDOWNLOAD_REQUEST_SEQUENCENUMBER_LENGTH)

namespace HFSAT {
namespace NSE {

class NSEMessageDownloadRequest {
 private:
  char message_download_request_buffer[NSE_MESSAGEDOWNLOAD_REQUEST_LENGTH];
  char const *msg_ptr;
 ...
 private:
  void InitializeStaticFields() {
    ...
    SetPacketLength(NSE_MESSAGEDOWNLOAD_REQUEST_LENGTH);
    ...
    SetMessageHeaderTransactionCode(DOWNLOAD_REQUEST);
    ...
    *((int16_t *)(msg_ptr + NSE_REQUEST_MESSAGE_HEADER_MESSAGELENGTH_OFFSET)) =
        hton16(NSE_MESSAGEDOWNLOAD_REQUEST_LENGTH - NSE_PACKET_REQUEST_LENGTH);

    //================================= @ MessageDownloadRequest

    // Request All Messages Since Start Of Day
    *((int32_t *)(msg_ptr + NSE_MESSAGEDOWNLOAD_REQUEST_SEQUENCENUMBER_OFFSET)) = 0;
  }

 public:
  NSEMessageDownloadRequest() : msg_ptr(message_download_request_buffer + NSE_REQUEST_START_OFFSET) {
    // Initialize MessageDownload Request Buffer
    memset((void *)message_download_request_buffer, 0, NSE_MESSAGEDOWNLOAD_REQUEST_LENGTH);
 ...
 public:
  inline void SetPreLoadedMessageDownloadRequestFields(int32_t const &user_id) { SetMessageHeaderTraderId(user_id); }

  inline void SetDynamicMessageDownloadRequestFields(int32_t const &packet_sequence_number) {
    ...
    HFSAT::MD5::MD5((unsigned char *)(msg_ptr + NSE_REQUEST_MESSAGE_HEADER_TRANSACTION_CODE_OFFSET),
                    NSE_MESSAGEDOWNLOAD_REQUEST_SEQUENCENUMBER_LENGTH - NSE_PACKET_REQUEST_LENGTH,
                    (unsigned char *)(msg_ptr + NSE_PACKET_CHECKSUM_OFFSET));
  }

  char const *GetMessageDownloadRequestBuffer() const { return message_download_request_buffer; }
};
}
}
