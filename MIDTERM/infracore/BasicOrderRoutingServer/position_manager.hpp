...
class PositionManager {
 ...
 public:
  ...
  // Will Ensure Combined Checks
  inline int32_t GetProcessedSecurityToEnforceMarginChecks(const int32_t& _security_id_) const {
    return ((-1 == security_id_to_pos_info_[_security_id_].combined_security_id)
                ? _security_id_
                : security_id_to_pos_info_[_security_id_].combined_security_id);
  }

  inline double GetSecurityWeightForOrderSize(const int32_t& _security_id_) const {
    return ((-1 == security_id_to_pos_info_[_security_id_].combined_security_id)
                ? 1.00
                : security_id_to_pos_info_[_security_id_].combined_bucket_weight_factor);
  }

  inline void AddSecurityToCombinedPool(const int32_t& _security_id_, const int32_t& _combined_security_id_,
                                        const double& _security_weight_, const bool& _is_security_spread_) {
    // TODO SendEmail, Didn't Know If Same Security Wants to be added in 2 combined bucket
    if (-1 != security_id_to_pos_info_[_security_id_].combined_security_id ||
        _combined_security_id_ >= DEF_MAX_SEC_ID) {
      return;
    }

    security_id_to_pos_info_[_security_id_].combined_security_id = _combined_security_id_;
    security_id_to_pos_info_[_security_id_].combined_bucket_weight_factor = _security_weight_;
  }
  ...
  inline void AddBuyTrade(const unsigned int _security_id_, const int _server_assigned_client_id_,
                          const int _bid_trade_size_, bool ignore_from_global) {
    PositionInfo& pos_info = security_id_to_pos_info_[_security_id_];

    if (!ignore_from_global) {
      __sync_add_and_fetch(&pos_info.position, _bid_trade_size_);
    }

    // Update Combined Security As Well
    if (-1 != pos_info.combined_security_id) {
      __sync_add_and_fetch(&security_id_to_pos_info_[pos_info.combined_security_id].position,
                           (int32_t)(_bid_trade_size_ * pos_info.combined_bucket_weight_factor));
    }
  ...
  inline void AddSellTrade(const unsigned int _security_id_, const int _server_assigned_client_id_,
                           const int _ask_trade_size_, bool ignore_from_global) {
    PositionInfo& pos_info = security_id_to_pos_info_[_security_id_];
    ...
    // Update Combined Security As Well
    if (-1 != pos_info.combined_security_id) {
      __sync_sub_and_fetch(&security_id_to_pos_info_[pos_info.combined_security_id].position,
                           (int32_t)(_ask_trade_size_ * pos_info.combined_bucket_weight_factor));
    }
  ...
  inline void AddBidSize(const unsigned int _security_id_, const int _additional_bid_size_, int ordered_vol,
    ...
    // Combined Security
    if (-1 != pos_info.combined_security_id) {
      __sync_add_and_fetch(&security_id_to_pos_info_[pos_info.combined_security_id].bid_size,
                           (int32_t)(_additional_bid_size_ * pos_info.combined_bucket_weight_factor));
    }
  }
  ...
  inline void DecBidSize(const unsigned int _security_id_, const int _reduced_bid_size_, int ordered_vol,
    ...
    if (-1 != pos_info.combined_security_id) {
      __sync_sub_and_fetch(&security_id_to_pos_info_[pos_info.combined_security_id].bid_size,
                           (int32_t)(_reduced_bid_size_ * pos_info.combined_bucket_weight_factor));
    }
  }
  ...
  inline void AddAskSize(const unsigned int _security_id_, const int _additional_ask_size_, int ordered_vol,
    ...
    if (-1 != pos_info.combined_security_id) {
      __sync_add_and_fetch(&security_id_to_pos_info_[pos_info.combined_security_id].ask_size,
                           (int32_t)(_additional_ask_size_ * pos_info.combined_bucket_weight_factor));
    }
  }
  ...
  inline void DecAskSize(const unsigned int _security_id_, const int _reduced_ask_size_, int ordered_vol,
    ...
    if (-1 != pos_info.combined_security_id) {
      __sync_sub_and_fetch(&security_id_to_pos_info_[pos_info.combined_security_id].ask_size,
                           (int32_t)(_reduced_ask_size_ * pos_info.combined_bucket_weight_factor));
    }
  }
 ...
 private:
  PositionManager() : position_filename_(""), order_manager_(OrderManager::GetUniqueInstance()) {
    ...
    for (int i = 0; i < DEF_MAX_SEC_ID; i++) {
      ...
      //-1 denotes it's not a combined security
      security_id_to_pos_info_[i].combined_security_id = -1;
      // Default Weight is 1.00
      security_id_to_pos_info_[i].combined_bucket_weight_factor = 1.00;
