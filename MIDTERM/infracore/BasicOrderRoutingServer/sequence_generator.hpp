 ...
 public:
  ...
  inline unsigned int GetNextSequence() {
    if (cap_on_sequence_ && (sequence_ >= MAX_ORDER_ID)) {
      (void)__sync_and_and_fetch(&sequence_, 0);  // Reset to 0
    }
    return __sync_add_and_fetch(&sequence_, 1);
  }
  ...
  inline unsigned int GetNextSequence(int jump_seq) {
    ...
    return __sync_add_and_fetch(&sequence_, jump_seq);
  }

  inline void RevertToPreviousSequence() {
    if (cap_on_sequence_ && (sequence_ == 0)) {
      __sync_val_compare_and_swap(&sequence_, 0, MAX_ORDER_ID);
    } else {
      __sync_sub_and_fetch(&sequence_, 1);
    }
  }
