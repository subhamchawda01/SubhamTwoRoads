...
void AccountThread::Connect() {
  if (!connected_) {
    ...
    HFSAT::ORS::ORSControllerThread::GetUniqueInstance().run();
  }
}

void AccountThread::Initialize(std::string t_output_log_dir_) {
  string exchange = m_settings.getValue("Exchange");
  if (exchange == "BMFEP" ||  "BMFEQ" == exchange || 
      "NSE_FO" == exchange || "NSE_CD" == exchange || "NSE_EQ" == exchange || 
      "MSSIM" == exchange) {
...
void AccountThread::AdjustSymbol(const char* symbol) { p_base_engine_->AdjustSymbol(symbol); }

void AccountThread::ChangeSpreadRatio(const char* symbol, const char* ratio) {
  p_base_engine_->ChangeSpreadRatio(symbol, ratio);
}
...
void AccountThread::Login() {
  ...
  if (!connected_) {
    ...
    if ( HFSAT::kExchSourceNSE != this_exchange_source_ &&
         HFSAT::kExchSourceNSE_FO != this_exchange_source_ &&
         HFSAT::kExchSourceNSE_EQ != this_exchange_source_ &&
         HFSAT::kExchSourceNSE_CD != this_exchange_source_ ) {
...
void AccountThread::OnOrderConf(int server_assigned_seqnum, const char* exch_assigned_seqnum, double price, int size,
                                int exch_assigned_seqnum_length_, uint64_t _exch_seq_, int entry_dt, 
				int64_t last_activity_refrence_) {
  ...
  p_this_order_->last_activity_reference_ = last_activity_refrence_;
...
void AccountThread::OnReject(int server_assigned_seqnum,
                             const ORSRejectionReason_t rejection_reason_ = kExchOrderReject, uint64_t _exch_seq_) {
  Order* p_this_order_ = order_manager_.GetOrderByOrderSequence(server_assigned_seqnum);
  ...
  if (rejection_reason_ == HFSAT::kORSRejectThrottleLimitReached) {
    bcast_manager_->BroadcastORSRejection(*p_this_order_, rejection_reason_);
  } else {
...
void AccountThread::OnRejectDueToFunds(int server_assigned_seqnum) {
  Order* p_this_order_ = order_manager_.GetOrderByOrderSequence(server_assigned_seqnum);
  dbglogger_ << "Funds Rejection called for sec_id : " << p_this_order_->security_id_ << "\n";
  if (p_this_order_ == NULL) return;

  sec_id_to_is_rej_funds_state_[p_this_order_->security_id_] = true;
  sec_id_to_reject_side_[p_this_order_->security_id_] = p_this_order_->buysell_;

  dbglogger_ << "OM\n" << order_manager_.DumpOMState() << "PM\n" << position_manager_.DumpPMState() << "\n";
  if (p_this_order_->buysell_ == kTradeTypeBuy) {
    dbglogger_ << "Order " << server_assigned_seqnum << " rejected by exchange due to lack of funds on buy side \n";
    position_manager_.DecBidSize(p_this_order_->security_id_, p_this_order_->size_remaining_, 0, 0);
  } else {
    dbglogger_ << "Order " << server_assigned_seqnum << " rejected by exchange due to lack of funds on sell side \n";
    position_manager_.DecAskSize(p_this_order_->security_id_, p_this_order_->size_remaining_, 0, 0);
  }
  order_manager_.DeactivateOrder(p_this_order_);
  dbglogger_.DumpCurrentBuffer();
  bcast_manager_->BroadcastExchRejectionDueToFunds(*p_this_order_, true);
}
...
void AccountThread::OnOrderExec(int server_assigned_seqnum, const char* symbol, TradeType_t trade_type, double price,
                                int size_executed, int size_remaining, uint64_t _exch_seq_, int32_t last_mod_dt, 
				int64_t last_activity_reference_) {
  // NO precision loss while printing price
  char print_price[20] = {'\0'};
  sprintf(print_price, "%.7f", price);

  dbglogger_ << " GOT TRADE: SAOS " <<  server_assigned_seqnum << " SYM " << symbol << " price " << price << " SE "<< size_executed
    << " ES " << _exch_seq_ << " LMD "<< last_mod_dt << " LAR " << last_activity_reference_ << "\n";
  ...
  if (p_this_order_ == NULL) {
    ...
    if (m_sec_id_ >= 0) {
      ...
      if (trade_type == kTradeTypeBuy) {
        position_manager_.AddBuyTrade(m_sec_id_, -1, size_executed, false);
      } else {
        position_manager_.AddSellTrade(m_sec_id_, -1, size_executed, false);
      }
  ...
  p_this_order_->exch_assigned_seq_ = _exch_seq_;
  p_this_order_->last_activity_reference_ = last_activity_reference_;
  if (size_remaining > 0) {
    p_this_order_->last_mod_dt_ = last_mod_dt;
  }
  ...
  if (trade_type == kTradeTypeBuy) {
    position_manager_.AddBuyTrade(p_this_order_->security_id_, p_this_order_->server_assigned_client_id_, size_executed,
                                  p_this_order_->ignore_from_global_pos);
    ...
    position_manager_.AddSellTrade(p_this_order_->security_id_, p_this_order_->server_assigned_client_id_,
                                   size_executed, p_this_order_->ignore_from_global_pos);
...
void AccountThread::OnTradeBust(int server_assigned_seqnum, const char* symbol, TradeType_t trade_type, double price,
                                int size_executed, int size_remaining) {
  Order* p_this_order_ = order_manager_.GetOrderByOrderSequence(server_assigned_seqnum);

  // Trade Bust by Exchange after say a full exec notification
  if (p_this_order_ == NULL) {
    const int m_sec_id_ = simple_security_symbol_indexer_.GetIdFromSecname(symbol);
    dbglogger_ << typeid(*this).name() << ':' << __func__ << " Order not found in OnTradeBust for SAOS "
               << server_assigned_seqnum << " sec_id " << m_sec_id_ << " sym " << symbol << "\n";
    dbglogger_.DumpCurrentBuffer();
    ...
    if (m_sec_id_ >= 0) {
      ...
      dbglogger_ << typeid(*this).name() << ':' << __func__ << " OnTradeBust: PM\n" << position_manager_.DumpPMState();
      dbglogger_.DumpCurrentBuffer();
    } else {
      dbglogger_ << typeid(*this).name() << ':' << __func__ << " m_sec_id < 0 in OnTradeBust for SAOS "
                 << server_assigned_seqnum << " sec_id " << m_sec_id_ << " sym " << symbol << "\n";
      dbglogger_.DumpCurrentBuffer();
    }
    return;
  ...
  // Else Order Exists with us, We will call a normal cancel but with size = size_executed
  // Trusting Exchange that in TradeBust, from experience while certifying,  LastQty will
  // correspond to our original size which was busted
  if (p_this_order_->buysell_ == kTradeTypeBuy) {
    position_manager_.DecBidSize(p_this_order_->security_id_, size_executed, 0, 0);
  } else {
    position_manager_.DecAskSize(p_this_order_->security_id_, size_executed, 0, 0);
  ...
  // deactive the order first before AdjustBestBidAskMap
  std::string p_this_order_str = p_this_order_->toString();
  const TradeType_t tr_type = p_this_order_->buysell_;
  int32_t saci = p_this_order_->server_assigned_client_id_;

  // Deactive Order
  order_manager_.DeactivateOrder(p_this_order_);

  dbglogger_ << "OnTradeBust\n" << p_this_order_str << "\n" << order_manager_.DumpOMState() << "PM\n"
             << position_manager_.DumpPMState() << "\n";
  dbglogger_.DumpCurrentBuffer();
  // Multiplying with -1 instead so that its easy to identify which was a trade bust

  LogORSTrade(symbol, tr_type, size_executed * -1, price, server_assigned_seqnum, -1, saci);
}
...
// Broadcast cancel for saos
void AccountThread::ForceCancelBroadcastSAOS(char const * _saos_){
  int32_t saos = std::atoi(_saos_);
  dbglogger_ << "Cancelling saos orders " << saos << "\n";
  //get all orders which are live
  std::vector<Order*> orders = order_manager_.GetFullDayOrders();

  for(auto order : orders){
    if(NULL != order && order->server_assigned_order_sequence_ == saos){
      dbglogger_ << "Sending BroadCastCancel\n";
...
// Broadcast Trade for saos
void AccountThread::ForceTradeBroadcastSAOS(char const * _saos_){
  int32_t saos = std::atoi(_saos_);
  dbglogger_ << "Sending Trade saos orders " << saos << "\n";
  //get all orders which are live
  std::vector<Order*> orders = order_manager_.GetFullDayOrders();

  for(auto order : orders){
    if(NULL != order && order->server_assigned_order_sequence_ == saos){
      dbglogger_ << "Sending BroadcastExecNotification\n";
      bcast_manager_->BroadcastExecNotification(*order);
      break;
    }
  }
}

// Broadcast reject for saos
void AccountThread::ForceRejectBroadcastSAOS(char const * _saos_){
  int32_t saos = std::atoi(_saos_);
  dbglogger_ << "Sending Reject saos orders " << saos << "\n";
  //get all orders which are live
  std::vector<Order*> orders = order_manager_.GetFullDayOrders();

  for(auto order : orders){
    if(NULL != order && order->server_assigned_order_sequence_ == saos){
      dbglogger_ << "Sending BroadcastCancelRejection \n";
      bcast_manager_->BroadcastCancelRejection(*order, HFSAT::kExchCancelReject);
...
// Cancel all pending orders. Also make sure that no new orders can be placed while this is happening
void AccountThread::ForceRecoverFundRejects(int sec_id) {
  dbglogger_ << "Starting force recovery from fund rejects: " << sec_id << "\n";
  if (sec_id < 0) {
    // invalid security provided
    return;
  }

  sec_id_to_is_rej_funds_state_[sec_id] = false;
  sec_id_to_reject_side_[sec_id] = kTradeTypeNoInfo;

  dbglogger_ << "Forced recovery from fund rejects for sec_id : " << sec_id << "\n";
  dbglogger_ << "OM\n" << order_manager_.DumpOMState() << "PM\n" << position_manager_.DumpPMState() << "\n";
  dbglogger_.DumpCurrentBuffer();
}
...
void AccountThread::OnBatchCxlAlert(int32_t user_id_){
    ...
    e.setSubject("Error: Received Batch ORDER CANCELLATION 9002 : " + std::string(hostname));
