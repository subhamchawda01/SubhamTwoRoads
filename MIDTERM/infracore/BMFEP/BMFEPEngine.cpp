...
void BMFEPEngine::SendOrder(const Order *rp_order_, BMFEPFIX::NewOrder *p_target_new_order_) {
  if (true == is_logged_in_ && true == allow_new_orders_) {
    ...
    HFSAT::QueryT2T::GetUniqueInstance().End(rp_order_->server_assigned_client_id_);
  ...
  } else if (false == is_logged_in_) {
    DBGLOG_CLASS_FUNC_LINE_ERROR << "BMFEP SENDING ORDER WITHOUT BEING LOGGED IN : " << id_ << DBGLOG_ENDL_FLUSH;
  } else if (false == allow_new_orders_) {
    DBGLOG_CLASS_FUNC_LINE_ERROR << "BMFEP NEW ORDERS ARE DIABLED : " << id_ << DBGLOG_ENDL_FLUSH;
    p_engine_listener_->OnReject(rp_order_->server_assigned_order_sequence_, kORSRejectNewOrdersDisabled);
  }
}
...
int BMFEPEngine::onInputAvailable(int sock) {
  if (!run_engine_) {
...
void BMFEPEngine::onExecReport(HFSAT::ORS::BMFEPFIX::ExecutionReport &exec_report_) {
  ...
  switch (ordstatus_) {
    ...
    default: {
      if (on_tradebust_saos_processed_.find(server_assigned_order_sequence_) != on_tradebust_saos_processed_.end()) {
        dbglogger_ << " TrdBust Already Processed Saos : " << server_assigned_order_sequence_ << "\n";
        on_tradebust_saos_processed_.erase(server_assigned_order_sequence_);

        return;
      }

      on_tradebust_saos_processed_[server_assigned_order_sequence_] = true;
      // Anything with tag150=H is a bust irrespective of Tag 39
      if (exectype_ == 'H')
        p_engine_listener_->OnTradeBust(symbol_.c_str(), exec_report_.getExecTag31(),
                                        (exec_report_.getExecTag54() == 1 ? kTradeTypeBuy : kTradeTypeSell),
                                        exec_report_.getExecTag32(), exec_report_.getExecTag151(),
                                        exec_report_.getExecTag37());
...
void BMFEPEngine::SendOrder(Order *order) {
  SendOrder(order, container_.new_order_map[order->security_id_]);
}

void BMFEPEngine::CancelOrder(Order *order) {
  CancelOrder(order, container_.cancel_order_map[order->security_id_]);
}

void BMFEPEngine::ModifyOrder(Order *order, ORS::Order *orig_order) {
  ModifyOrder(order, container_.cancel_replace_map[order->security_id_]);
}
