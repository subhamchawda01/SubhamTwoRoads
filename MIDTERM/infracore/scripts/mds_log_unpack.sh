...
update_datacopy_status(){
  status=$1 ;
  ssh $USER@$DEST_SERVER /bin/bash << EOF 
  echo "$status" > $datacopy_tracker_file ; 
EOF
}
...
if [ $ERRORCODE -eq 0 ] ; then 
  update_datacopy_status "SYNCED_HS1" ;
else 
  update_datacopy_status "FAILED_HS1SYNC" ; 
fi 

SSH_VARS="-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o ConnectTimeout=60"
ssh -n -f  $SSH_VARS dvctrader@52.0.55.252 "~/basetrade/scripts/send_to_disks.sh $HS1_TMP_PATH/$ARCH_NAME $EXCHANGE $CURRENT_LOCATION $dest_path $exch_timing"
...
chown -R dvcinfra:infra $dest_dir
