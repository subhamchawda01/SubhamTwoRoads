...
my $USAGE="$0 margin_file_name shortcode";
if ( $#ARGV < 1 ) { print "$USAGE\n"; exit ( 0 ); }

my $margin_file_name_ = $ARGV[0];
...
# force buffer to flush after each write
    GP->autoflush(1);
    print GP "set xdata time; \n set timefmt \"\%s\"; set grid; \n plot \'$temp_file_name_\' using 1:$targetcol_ with lines title \"$margin_file_basename_.$shortcode_\" \n; ";
    close GP;
