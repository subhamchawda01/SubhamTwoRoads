...
SHORTCODE_SYMBOL_EXEC=$HOME/infracore_install/bin/get_exchange_symbol
OUR_PRESENCE_IN_MARKET_STATS_EXEC=$HOME/infracore_install/bin/get_our_presence_in_market_stats
...
if [ $PLOT_OPTION = "VR" ]
then
	outfile_=$HOME/plots/$SHORTCODE.txt$YYYYMMDD;
	~/basetrade_install/bin/get_periodic_volume_on_day $SHORTCODE $YYYYMMDD $PERIOD > $outfile_
	MAX=`cat $outfile_ | awk '{if($2>max){max=$2}}END{print max}'`
	cat $outfile_ | awk -v max=${MAX} '{print $1, $2*20/max}' > $outfile_'_temp'
	GNUPLOT_CMD=$GNUPLOT_CMD', "'$outfile_'_temp" using 1:2 with boxes title "our_volume*20/our_max_volume"'
	echo $GNUPLOT_CMD | gnuplot -persist
	rm -f $outfile_ $outfile_'_temp'
else
	echo $GNUPLOT_CMD | gnuplot -persist
fi
