...
for ( my $i = 2; $i <= $#ARGV; $i ++ )
{
	...
	push ( @product_exchange_vec_, "HKEX" );
	push ( @product_exchange_vec_, "OSE" );
	last;
    }
    else
    {
...
for ( my $j = 0 ; $j < $max_days_at_a_time_ ; $j ++ ) 
{
	...
	my $is_exchange_ = 0;
	if ( $product_exchange_ eq "CME" || $product_exchange_ eq "EUREX" || $product_exchange_ eq "BMF" || $product_exchange_ eq "TMX" || $product_exchange_ eq "LIFFE" || $product_exchange_ eq "HKEX" || $product_exchange_ eq "OSE" ) 
	{
...
#	print STDERR "Trying $tradingdate_, $product_exchange_ $pnl_filename_\n";
	...
	if ( -e $pnl_filename_ ) 
	{
	    open PNL_FILE_HANDLE, "< $pnl_filename_" or PrintStacktraceAndDie ( "Fatal error while opening $pnl_filename_. Exiting." );
