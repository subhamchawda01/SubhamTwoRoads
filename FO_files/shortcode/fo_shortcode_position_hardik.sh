...
generate_shortcode_position_file_OPT() {
	...
	awk 'NR==FNR { fo_pos[$NF]=$(NF-1); next } { if (fo_pos[$(NF-1)]) {print fo_pos[$(NF-1)],$(NF-1),$NF} else {print "INVALID",$(NF-1),$NF}}' $datasource_exchsymbol_file $fo_positions_OPT_Inverted | awk '{if($3 != 0) print $0}' > OPT_exchsymbol_position
	...
	echo "Symbol Position" > position_to_close_OPT

	awk 'NR==FNR { if($NF != "INVALID") fo_pos[$(NF-1)]=$NF; next } { if (fo_pos[$(NF-2)]) print fo_pos[$(NF-2)],$NF}' OPT_exchsymbol_shortcode OPT_exchsymbol_position >> position_to_close_OPT
...
generate_shortcode_position_file_FUT() {
	...
	echo "Symbol Position" > position_to_close_FUT
