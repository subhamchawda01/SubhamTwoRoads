...
def GenerateAvgPositionsFO():
  f = open("fo_under_ban_positions.txt","r+");
  lines = f.readlines()[1:];
  resultFUT = {}
  resultOPT = {}
  for x in lines:
  ...
  fileAvgFUT=open("fo_under_ban_positions_FUT.txt","w");
  fileAvgInvFUT=open("fo_under_ban_positions_FUT_Inverted.txt","w");
  fileAvgFUT.write("Symbol\tPositions\n");
  fileAvgInvFUT.write("Symbol\tPositions\n");
  fileAvgOPT=open("fo_under_ban_positions_OPT.txt","w");
  fileAvgInvOPT=open("fo_under_ban_positions_OPT_Inverted.txt","w");
...
if __name__ == '__main__':
  ...
  for fileSelected in filesSelected:
    ...
    for index,row in fo_pos_df.iterrows():
      if (index.split(' ')[1]) in banList:
        if index in symbol_position.keys():
          symbol_position[index]+=int(row['Positions']); 
        else:
          symbol_position[index]=int(row['Positions']);
  filePosition=open("fo_under_ban_positions.txt","w");
